...
private:
  ...
  V3UniqueNames
      m_lvboundNames;         // For generating unique temporary variable names
  ...
  void replaceBoundLvalue(AstNode *nodep, AstNode *condp) {
    ...
    if (m_assigndlyp) {
      ...
      AstNode *newp =
          new AstAssign(m_assigndlyp->fileline(),
    ...
    if (AstIf *ifp = VN_AS(prep->user2p(), If)) {
      ...
      AstVar *const varp = new AstVar(fl, AstVarType::MODULETEMP,
                                      m_lvboundNames.get(prep), prep->dtypep());
      m_modp->addStmtp(varp);
      AstNode *const abovep =
          prep->backp(); // Grab above point before we replace 'prep'
      prep->replaceWith(new AstVarRef(fl, varp, VAccess::WRITE));
      AstIf *newp = new AstIf(
          fl, condp,
          (needDly ? static_cast<AstNode *>(new AstAssignDly(
                         fl, prep, new AstVarRef(fl, varp, VAccess::READ)))
  ...
  virtual void visit(AstCountBits *nodep) override {
    ...
    dropop[0] = VN_IS(nodep->rhsp(), Const) &&
                VN_AS(nodep->rhsp(), Const)->num().isAnyX();
    dropop[1] = VN_IS(nodep->thsp(), Const) &&
                VN_AS(nodep->thsp(), Const)->num().isAnyX();
    dropop[2] = VN_IS(nodep->fhsp(), Const) &&
                VN_AS(nodep->fhsp(), Const)->num().isAnyX();
  ...
  virtual void visit(AstConst *nodep) override {
    if (m_constXCvt && nodep->num().isFourState()) {
      ...
      if (debug() >= 9)
        nodep->dumpTree(cout, "  Const_old: ");
      // CONST(num) -> VARREF(newvarp)
      //          -> VAR(newvarp)
      //          -> INITIAL(VARREF(newvarp,
      //          OR(num_No_Xs,AND(random,num_1s_Where_X))
      ...
      if (v3Global.opt.xAssign() != "unique") {
        ...
        AstVar *const newvarp = new AstVar(nodep->fileline(), AstVarType::XTEMP,
                                           m_xrandNames.get(nodep),
        ...
        // In the future, we should stuff the initp into the module's
        // constructor.
        ...
        if (debug() >= 9)
          newref1p->dumpTree(cout, "     _new: ");
        if (debug() >= 9)
          newvarp->dumpTree(cout, "     _new: ");
        if (debug() >= 9)
          newinitp->dumpTree(cout, "     _new: ");
        VL_DO_DANGLING(nodep->deleteTree(), nodep);
      }
    }
  }
  ...
  virtual void visit(AstSel *nodep) override {
    ...
    if (!nodep->user1SetOnce()) {
      ...
      // See if the condition is constant true (e.g. always in bound due to
      // constant select) Note below has null backp(); the Edit function knows
      // how to deal with that.
      condp = V3Const::constifyEdit(condp);
      if (condp->isOne()) {
        // We don't need to add a conditional; we know the existing expression
        // is ok
  ...
  virtual void visit(AstArraySel *nodep) override {
    ...
    if (!nodep->user1SetOnce()) {
      ...
      if (const AstNodeVarRef *varrefp = VN_CAST(basefromp, NodeVarRef)) {
        ...
        // If it's a PARAMETER[bit], then basefromp may be a constant instead of
        // a varrefp
      } else {
      ...
      if (debug() >= 9)
        nodep->dumpTree(cout, "arraysel_old: ");
      ...
      // Note below has null backp(); the Edit function knows how to deal with
      // that.
      condp = V3Const::constifyEdit(condp);
      if (condp->isOne()) {
        // We don't need to add a conditional; we know the existing expression
        ...
        AstNode *newp = new AstCondBound(bitp->fileline(), condp, bitp,
...
public:
  ...
  explicit UnknownVisitor(AstNetlist *nodep)
      : m_lvboundNames{"__Vlvbound"}, m_xrandNames{"__Vxrand"} {
    iterate(nodep);
  }
...
void V3Unknown::unknownAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("unknown", 0,
