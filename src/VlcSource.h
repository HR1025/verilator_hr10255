...
public:
  // CONSTRUCTORS
  VlcSourceCount(int lineno, int column) : m_lineno{lineno}, m_column{column} {}
...
public:
  // CONSTRUCTORS
  explicit VlcSource(const string &name) : m_name{name} {}
  ~VlcSource() = default;
  ...
  void incCount(int lineno, int column, vluint64_t count, bool ok) {
    ...
    if (lit == m_lines.end())
      lit = m_lines.insert(std::make_pair(lineno, ColumnMap())).first;
    ColumnMap &cmap = lit->second;
    ColumnMap::iterator cit = cmap.find(column);
    if (cit == cmap.end()) {
      cit = cmap.insert(std::make_pair(column, VlcSourceCount(lineno, column)))
                .first;
    }
