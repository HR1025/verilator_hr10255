...
constexpr int INFILTER_IPC_BUFSIZ =
    (64 * 1024); // For debug, try this as a small number
constexpr int INFILTER_CACHE_MAX =
    (64 * 1024); // Maximum bytes to cache if same file read twice
    ...
    void loadStats() {
      if (!m_stat.st_mtime) {
        ...
        if (err != 0) {
          ...
          m_stat.st_mtime = 1;
          m_exists = false;
          // Not an error... This can occur due to `line directives in the .vpp
          // files
          UINFO(1, "-Info: File not statable: " << filename() << endl);
        }
      }
    }
    bool operator<(const DependFile &rhs) const {
...
inline std::vector<string> V3FileDependImp::getAllDeps() const {
  ...
  for (const auto &itr : m_filenameList) {
    if (!itr.target() && itr.exists())
      r.push_back(itr.filename());
  }
  return r;
}

inline void V3FileDependImp::writeTimes(const string &filename,
                                        const string &cmdlineIn) {
  const std::unique_ptr<std::ofstream> ofp{V3File::new_ofstream(filename)};
  if (ofp->fail())
    v3fatal("Can't write " << filename);
  ...
  *ofp << "# DESCR"
       << "IPTION: Verilator output: Timestamp data for --skip-identical.  "
          "Delete at will.\n";
  *ofp << "C \"" << cmdline << "\"\n";
...
inline bool V3FileDependImp::checkTimes(const string &filename,
                                        const string &cmdlineIn) {
  const std::unique_ptr<std::ifstream> ifp{
      V3File::new_ifstream_nodepend(filename)};
  if (ifp->fail()) {
  ...
  while (!ifp->eof()) {
    ...
    *ifp >> chkIno;
    if (ifp->eof())
      break; // Needed to read final whitespace before found eof
    time_t chkCstime;
    *ifp >> chkCstime;
    time_t chkCnstime;
    *ifp >> chkCnstime;
    ...
    *ifp >> quote;
    ...
    //        <<chkCnstime<<" mt="<<chkMstime<<"."<<chkMnstime<<" fn =
    //        "<<chkFilename<<endl);
    // UINFO(9," nowSt  s="<<chkStat.st_size<<" mt="<<chkStat.st_mtime<<"
    // ct="<<chkStat.st_ctime<<" fn = "<<chkFilename<<endl);
    if (filename != chkFilename) { // Other then the .dat file itself, as we
                                   // were writing it at the time...
      // We'd like this rule:
      ...
      if (!(chkStat.st_size >= chkSize && chkStat.st_ino == chkIno &&
        ...
        UINFO(2, "   --check-times failed: out-of-date "
                     << chkFilename << "; " << chkStat.st_size << "=?"
                     << chkSize << " " << chkStat.st_ctime << "."
                     << VL_STAT_CTIME_NSEC(chkStat) << "=?" << chkCstime << "."
                     << chkCnstime << " " << chkStat.st_mtime << "."
...
void V3File::createMakeDir() {
  ...
  if (!created) {
    ...
    if (v3Global.opt.hierTop())
      V3Os::createDir(v3Global.opt.hierTopDataDir());
  }
}
...
class VInFilterImp final {
  ...
  std::map<const std::string, std::string>
      m_contentsMap;      // Cache of file contents
...
private:
  ...
  bool readContentsFilter(const string &filename, StrList &outl) {
    ...
    if (line.find("Content-Length") != string::npos) {
      ...
      if (line != "")
        v3error("--pipe-filter protocol error, unexpected: " << line);
      return false;
    }
#else
  ...
  void checkFilter(bool hang) {
    ...
    if (!m_pidExited && waitpid(m_pid, &m_pidStatus, hang ? 0 : WNOHANG)) {
      UINFO(1, "--pipe-filter: Exited, status "
                   << m_pidStatus << " exit=" << WEXITSTATUS(m_pidStatus)
  ...
  void writeFilter(const string &out) {
    ...
    while (!m_readEof && out.length() > offset) {
      errno = 0;
      const int got =
          write(m_writeFd, (out.c_str()) + offset, out.length() - offset);
  ...
  void startFilter(const string &command) {
    ...
    if (pid < 0)
      v3fatal("--pipe-filter: fork failed: " << strerror(errno));
    if (pid == 0) { // Child
      UINFO(6, "In child\n");
      close(fd_stdin[P_WR]);
...
protected:
  ...
  bool readWholefile(const string &filename, StrList &outl) {
    ...
    if (listSize(outl) < INFILTER_CACHE_MAX) {
      ...
      // This isn't so important if it's just an open(), but filtering can be
      // slow
...
bool VInFilter::readWholefile(const string &filename,
                              VInFilter::StrList &outl) {
  if (!m_impp)
    v3fatalSrc("readWholefile on invalid filter");
  return m_impp->readWholefile(filename, outl);
}

//######################################################################
// V3OutFormatter: A class for printing to a file, with automatic indentation of
// C++ code.

V3OutFormatter::V3OutFormatter(const string &filename,
...
bool V3OutFormatter::tokenEnd(const char *cp) {
  return (tokenStart(cp, "end") || tokenStart(cp, "endcase") ||
...
void V3OutFormatter::putBreak() {
  if (!m_nobreak) {
    // char s[1000]; sprintf(s, "{%d,%d}", m_column, m_parenVec.top());
    // putsNoTracking(s);
    if (exceededWidth()) {
      putcNoTracking('\n');
      if (!m_parenVec.empty())
        putsNoTracking(indentSpaces(m_parenVec.top()));
    }
  }
}
  ...
  if (lang == LA_XML) {
    ...
    for (const char c : namein) {
      if (c == '\\' || c == '"') {
        ...
        const string octal = string("\\") + cvtToStr((c >> 6) & 3) +
...
void V3OutCFile::putsGuard() {
  ...
  m_guard = true;
  string var = VString::upcase(string("VERILATED_") +
                               V3Os::filenameNonDir(filename()) + "_");
  for (char &c : var) {
    if (!isalnum(c))
      c = '_';
  }
...
class VIdProtectImp final {
  // MEMBERS
  std::map<const std::string, std::string>
      m_nameMap;                              // Map of old name into new name
...
public:
  ...
  string passthru(const string &old) {
    ...
    if (it != m_nameMap.end()) {
      ...
      UASSERT(old == it->second, "Passthru request for '" + old +
                                     "' after already --protect-ids of it.");
    } else {
  ...
  string protectIf(const string &old, bool doIt) {
    if (!v3Global.opt.protectIds() || old.empty() || !doIt)
      return old;
    const auto it = m_nameMap.find(old);
    if (it != m_nameMap.end()) {
      return it->second;
  ...
  string protectWordsIf(const string &old, bool doIt) {
    // Split at " " (for traces), "." (for scopes), "->", "(", "&", ")" (for
    // self pointers)
    if (!(doIt && v3Global.opt.protectIds()))
      return old;
    string out;
