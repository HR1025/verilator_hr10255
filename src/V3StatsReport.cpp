...
class StatsReport final {
  ...
  void stars() {
    ...
    for (const auto &itr : s_allStats) {
      ...
      if (repp->stage() == "*" && repp->printit()) {
        if (maxWidth < repp->name().length())
          maxWidth = repp->name().length();
        byName.emplace(repp->name(), repp);
      }
    }
  ...
  void stages() {
    ...
    for (const string &i : stages)
      os << "  " << std::left << std::setw(9) << i;
    os << '\n';
...
void V3Statistic::dump(std::ofstream &os) const {
  if (perf()) {
    ...
    os << "  " << std::right << std::fixed << std::setprecision(0)
       << std::setw(9) << count();
  }
}
...
void V3Stats::statsStage(const string &name) {
  ...
  lastWallTime = wallTime;
  V3Stats::addStatPerf("Stage, Elapsed time (sec), " + digitName,
                       wallTimeDelta);
...
void V3Stats::statsReport() {
  ...
  // Open stats file
  const string filename = v3Global.opt.hierTopDataDir() + "/" +
                          v3Global.opt.prefix() + "__stats.txt";
