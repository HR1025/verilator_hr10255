...
//      for (deque<AstVarScope*>::iterator it = m_inVarps.begin();
//      it!=m_inVarps.end(); ++it) {
...
//      simvis.main(nodep);
//      // Read outputs
//      for (deque<AstVarScope*>::iterator it = m_outVarps.begin();
...
public:
  ...
  SimStackNode(AstFuncRef *funcp, V3TaskConnects *tconnects)
      : m_funcp{funcp}, m_tconnects{tconnects} {}
...
private:
  ...
  // Checking:
  //  AstVar(Scope)::user1()  -> VarUsage.  Set true to indicate tracking as
  //  lvalue/rvalue
  // Simulating:
  //  AstConst::user2()       -> bool. This AstConst (allocated by this class)
  //  is in use AstVar(Scope)::user3()  -> AstConst*. Input value of variable or
  //  node
  //    (and output for non-delayed assignments)
  //  AstVar(Scope)::user2()  -> AstCont*. Output value of variable (delayed
  //  assignments)
  ...
  // Checking:
  string m_whyNotOptimizable; ///< String explaining why not optimizable or
                              ///< nullptr to optimize
  ...
  std::vector<SimStackNode *>
      m_callStack; ///< Call stack for verbose error messages
  ...
  AstConst *allocConst(AstNode *nodep) {
    ...
    if (!freeList.empty()) {
      ...
      if (!constp->user2()) {
        ...
        // Mark the AstConst node as used, and move it to the back of the free
        // list. This ensures that when all AstConst instances within the list
        // are used, then the front of the list will be marked as used, in which
        // case the enclosing 'if' will fail and we fall back to allocation.
        constp->user2(1);
        freeList.pop_front();
    ...
    if (allocNewConst) {
      ...
      constp = new AstConst(nodep->fileline(), AstConst::DtypedValue(),
                            nodep->dtypep(), 0);
...
private:
  ...
  void badNodeType(AstNode *nodep) {
    // Call for default node types, or other node types we don't know how to
    // handle
    checkNodeInfo(nodep);
    if (optimizable()) {
      // Hmm, what is this then?
      // In production code, we'll just not optimize.  It should be fixed
      // though.
      clearOptimizable(
          nodep,
          "Unknown node type, perhaps missing visitor in SimulateVisitor");
#ifdef VL_DEBUG
      UINFO(0, "Unknown node type in SimulateVisitor: "
  ...
  virtual void visit(AstAlways *nodep) override {
    if (jumpingOver(nodep))
      return;
    checkNodeInfo(nodep);
    iterateChildren(nodep);
  }
  virtual void visit(AstSenTree *nodep) override {
    // Sensitivities aren't inputs per se; we'll keep our tree under the same
    // sens.
  }
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (nodep->access().isReadOrRW()) {
      if (!(vscp->user1() & VU_RV)) {
        ...
        const bool isConst =
            nodep->varp()->isParam() && nodep->varp()->valuep();
        AstNode *valuep =
            isConst ? fetchValueNull(nodep->varp()->valuep()) : nullptr;
        if (isConst && valuep) { // Propagate PARAM constants for constant
                                 // function analysis
    ...
    if (!m_checkOnly && optimizable()) { // simulating
      ...
      {
        // Return simulation value - copy by reference instead of value for
        // speed
        ...
        if (!valuep) {
          if (m_params) {
            clearOptimizable(
                nodep,
                "Language violation: reference to non-function-local variable");
          } else {
            nodep->v3fatalSrc("Variable value should have been set before any "
                              "visitor called.");
          }
  ...
  virtual void visit(AstVarXRef *nodep) override {
    ...
    if (m_scoped) {
      ...
      clearOptimizable(nodep,
                       "Language violation: Dotted hierarchical references not "
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    if (nodep->dpiImport()) {
      if (m_params) {
        nodep->v3error(
            "Constant function may not be DPI import (IEEE 1800-2017 13.4.3)");
      }
      clearOptimizable(nodep, "DPI import functions aren't simulatable");
    }
    if (nodep->underGenerate()) {
      nodep->v3error("Constant function may not be declared under generate "
  ...
  virtual void visit(AstEnumItemRef *nodep) override {
    ...
    if (!m_checkOnly && optimizable()) {
      ...
      if (valuep) {
        ...
        clearOptimizable(nodep,
                         "No value found for enum item"); // LCOV_EXCL_LINE
      }
    }
  }
  ...
  virtual void visit(AstNodeTriop *nodep) override {
    ...
    if (!m_checkOnly && optimizable()) {
      nodep->numberOperate(
          newConst(nodep)->num(), fetchConst(nodep->lhsp())->num(),
    ...
    if (m_checkOnly) {
      ...
      if (optimizable()) {
        if (fetchConst(nodep->lhsp())->num().isEqZero()) {
          AstConst cnst(nodep->fileline(), AstConst::WidthedValue(), 1,
                        1);       // a one
  ...
  void handleAssignArray(AstNodeAssign *nodep, AstArraySel *selp) {
    ...
    // To do better, we need the concept of lvalues, or similar, to know
    // where/how to insert
    checkNodeInfo(selp);
    ...
    if (!m_checkOnly && optimizable()) {
      ...
      if (AstInitArray *vscpnump =
              VN_CAST(fetchOutValueNull(vscp), InitArray)) {
        initp = vscpnump;
      } else if (AstInitArray *vscpnump =
                     VN_CAST(fetchValueNull(vscp), InitArray)) {
        initp = vscpnump;
      } else { // Assignment to unassigned variable, all bits are X
        // TODO generic initialization which builds X/arrays by recursion
        AstConst *outconstp = new AstConst(
      ...
      // Values are in the "real" tree under the InitArray so can eventually
      // extract it, Not in the usual setValue (pointed to by user2/3p)
      initp->addIndexValuep(index, valuep);
      if (debug() >= 9)
        initp->dumpTree(cout, "-array-");
  ...
  void handleAssignSel(AstNodeAssign *nodep, AstSel *selp) {
    ...
    if (!m_checkOnly && optimizable()) {
      ...
      if (AstConst *vscpnump = fetchOutConstNull(vscp)) {
        ...
        if (varrefp->varp()->basicp() &&
            varrefp->varp()->basicp()->isZeroInit()) {
          outconstp->num().setAllBits0();
        } else {
          outconstp->num().setAllBitsX();
        }
      }
      outconstp->num().opSelInto(fetchConst(nodep->rhsp())->num(), lsb,
                                 selp->widthConst());
  ...
  void handleAssignSelRecurse(AstNodeAssign *nodep, AstSel *selp,
    ...
    if (AstVarRef *varrefp = VN_CAST(selp->fromp(), VarRef)) {
      ...
      handleAssignSelRecurse(nodep, subselp, outVarrefpRef, sublsb /*ref*/,
                             depth + 1);
      if (optimizable()) {
        lsbRef = sublsb;
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    if (VN_IS(nodep, AssignDly)) {
      if (m_anyAssignComb)
        clearOptimizable(nodep, "Mix of dly/non-dly assigns");
      m_anyAssignDly = true;
      m_inDlyAssign = true;
    } else {
      if (m_anyAssignDly)
  ...
  virtual void visit(AstArraySel *nodep) override {
    checkNodeInfo(nodep);
    iterateChildren(nodep);
    if (AstInitArray *initp =
            VN_CAST(fetchValueNull(nodep->fromp()), InitArray)) {
      AstConst *indexp = fetchConst(nodep->bitp());
      uint32_t offset = indexp->num().toUInt();
      AstNode *itemp = initp->getIndexDefaultedValuep(offset);
      if (!itemp) {
        clearOptimizable(
            nodep, "Array initialization has too few elements, need element " +
                       cvtToStr(offset));
      } else {
        ...
        if (!itemp->isDefault()) {
          for (AstNode *ep = itemp->condsp(); ep; ep = ep->nextp()) {
            ...
            if (optimizable()) {
              ...
              match.opEq(fetchConst(nodep->exprp())->num(),
                         fetchConst(ep)->num());
              if (match.isNeqZero()) {
  ...
  virtual void visit(AstStop *nodep) override {
    ...
    if (m_params) { // This message seems better than an obscure $stop
      ...
      clearOptimizable(nodep, "$stop executed during function constification; "
                              "maybe indicates assertion firing");
    }
    checkNodeInfo(nodep);
  }
    ...
    if (m_checkOnly) {
      ...
      while (true) {
        ...
        if (loops++ > unrollCount() * 16) {
          clearOptimizable(nodep,
                           "Loop unrolling took too long; probably this is an"
  ...
  virtual void visit(AstFuncRef *nodep) override {
    ...
    for (V3TaskConnects::iterator it = tconnects.begin(); it != tconnects.end();
         ++it) {
      AstVar *portp = it->first;
      AstNode *pinp = it->second->exprp();
      if (pinp) { // Else too few arguments in function call - ignore it
        if (portp->isWritable()) {
          clearOptimizable(portp, "Language violation: Outputs/refs not "
                                  "allowed in constant functions");
          return;
        }
        // Evaluate pin value
    ...
    if (auto *const basicp = VN_CAST(funcp->fvarp(), Var)->basicp()) {
      AstConst cnst(funcp->fvarp()->fileline(), AstConst::WidthedValue(),
                    basicp->widthMin(), 0);
      if (basicp->isZeroInit()) {
    ...
    if (!m_checkOnly && optimizable()) {
      ...
      UASSERT_OBJ(funcp->fvarp(), nodep,
                  "Function reference points at non-function");
...
private:
  ...
  void mainGuts(AstNode *nodep) {
    iterate(nodep);
    UASSERT_OBJ(!m_jumpp, m_jumpp,
                "JumpGo branched to label that wasn't found");
  }
...
public:
  ...
  SimulateVisitor() {
    ...
    clear(); // We reuse this structure in the main loop, so put initializers
             // inside clear()
  }
  void clear() {
    m_whyNotOptimizable = "";
  ...
  virtual ~SimulateVisitor() override {
    for (const auto &pair : m_constps) {
      for (AstConst *const constp : pair.second) {
        delete constp;
      }
    }
    m_constps.clear();
    for (AstNode *ip : m_reclaimValuesp)
      delete ip;
