...
public:
  // CONSTRUCTORS
  explicit UndrivenVarEntry(
      AstVar *varp) { // Construction for when a var is used
...
private:
  ...
  bool bitNumOk(int bit) const {
    return bit >= 0 &&
           (bit * FLAGS_PER_BIT < static_cast<int>(m_bitFlags.size()));
  }
  bool usedFlag(int bit) const {
    return m_wholeFlags[FLAG_USED] ||
           m_bitFlags[bit * FLAGS_PER_BIT + FLAG_USED];
  }
  bool drivenFlag(int bit) const {
    return m_wholeFlags[FLAG_DRIVEN] ||
           m_bitFlags[bit * FLAGS_PER_BIT + FLAG_DRIVEN];
  }
      ...
      if (bit >= 0 &&
        ...
        if (lsb == msb) {
          ...
          if (bdtypep->littleEndian()) {
            bits += cvtToStr(lsb + bdtypep->lo()) + ":" +
                    cvtToStr(msb + bdtypep->lo());
          } else {
            bits += cvtToStr(msb + bdtypep->lo()) + ":" +
                    cvtToStr(lsb + bdtypep->lo());
          }
        }
        prev = false;
      }
...
public:
  ...
  bool isUsedNotDrivenBit(int bit, int width) const {
    for (int i = 0; i < width; i++) {
      if (bitNumOk(bit + i) &&
          (m_wholeFlags[FLAG_USED] ||
           m_bitFlags[(bit + i) * FLAGS_PER_BIT + FLAG_USED]) &&
  ...
  void reportViolations() {
    ...
    AstVar *nodep = m_varp;
    {
      ...
      if (nodep->isIfaceRef()) {
        ...
        // Ideally we'd report unused instance cells, but presumably a signal
        // inside one would get reported as unused
      } else if (allU && allD) {
        // It's fine
      } else if (!anyD && !anyU) {
        // UNDRIVEN is considered more serious - as is more likely a bug,
        // thus undriven+unused bits get UNUSED warnings, as they're not as
        // buggy.
        if (!unusedMatch(nodep)) {
          nodep->v3warn(UNUSED, ucfirst(what) << " is not driven, nor used: "
                                              << nodep->prettyNameQ());
          nodep->fileline()->modifyWarnOff(V3ErrorCode::UNUSED,
                                           true); // Warn only once
        }
      } else if (allD && !anyU) {
        if (!unusedMatch(nodep)) {
          nodep->v3warn(UNUSED, ucfirst(what) << " is not used: "
                                              << nodep->prettyNameQ());
          nodep->fileline()->modifyWarnOff(V3ErrorCode::UNUSED,
                                           true); // Warn only once
        }
      } else if (!anyD && allU) {
        nodep->v3warn(UNDRIVEN, ucfirst(what) << " is not driven: "
                                              << nodep->prettyNameQ());
        nodep->fileline()->modifyWarnOff(V3ErrorCode::UNDRIVEN,
                                         true); // Warn only once
      } else {
        ...
        if (anynotDU && !unusedMatch(nodep)) {
          nodep->v3warn(
              UNUSED, "Bits of " << what << " are not driven, nor used: "
                                 << nodep->prettyNameQ() << bitNames(BN_BOTH));
          setU = true;
        }
        if (anyDnotU && !unusedMatch(nodep)) {
          nodep->v3warn(UNUSED, "Bits of " << what << " are not used: "
                                           << nodep->prettyNameQ()
                                           << bitNames(BN_UNUSED));
          setU = true;
        }
        if (anyUnotD) {
          nodep->v3warn(UNDRIVEN, "Bits of " << what << " are not driven: "
                                             << nodep->prettyNameQ()
...
private:
  ...
  // STATE
  std::array<std::vector<UndrivenVarEntry *>, 3>
      m_entryps;                   // Nodes to delete when finished
  ...
  AstAlways *m_alwaysCombp =
      nullptr; // Current always if combo, otherwise nullptr
  ...
  UndrivenVarEntry *getEntryp(AstVar *nodep, int which_user) {
    if (!(which_user == 1 ? nodep->user1p() : nodep->user2p())) {
      ...
      // UINFO(9," Associate u="<<which_user<<" "<<cvtToHex(this)<<"
      // "<<nodep->name()<<endl);
  ...
  virtual void visit(AstVar *nodep) override {
    for (int usr = 1; usr < (m_alwaysCombp ? 3 : 2); ++usr) {
      ...
      if (nodep->isNonOutput() || nodep->isSigPublic() ||
          nodep->isSigUserRWPublic() ||
          (m_taskp && (m_taskp->dpiImport() || m_taskp->dpiExport()))) {
        entryp->drivenWhole();
      }
      if (nodep->isWritable() || nodep->isSigPublic() ||
  ...
  virtual void visit(AstArraySel *nodep) override {
    // Arrays are rarely constant assigned, so for now we punt and do all
    // entries
    iterateChildren(nodep);
  }
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    ...
    if (nodep->access().isWriteOrRW() &&
        !VN_IS(nodep,
               VarXRef)) { // Ignore interface variables and similar ugly items
      if (m_inProcAssign && !nodep->varp()->varType().isProcAssignable() &&
        ...
        nodep->v3warn(PROCASSWIRE,
                      "Procedural assignment to wire, perhaps intended var"
    ...
    for (int usr = 1; usr < (m_alwaysCombp ? 3 : 2); ++usr) {
      ...
      if (m_inBBox || nodep->access().isReadOrRW() || fdrv)
        entryp->usedWhole();
    }
  }
...
public:
  ...
  virtual ~UndrivenVisitor() override {
    for (UndrivenVarEntry *ip : m_entryps[1])
      ip->reportViolations();
    for (int usr = 1; usr < 3; ++usr) {
      for (UndrivenVarEntry *ip : m_entryps[usr])
        delete ip;
    }
  }
};
