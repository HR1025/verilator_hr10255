...
//          ASSIGN(X,...) IF( ..., ASSIGN(X,...), ASSIGN(X,...)) => deletes
//          first We don't do the opposite yet though (remove assigns in if
//          followed by outside if)
//
//*************************************************************************
...
public:
  ...
  LifeState() = default;
  ~LifeState() {
    V3Stats::addStatSum("Optimizations, Lifetime assign deletions",
                        m_statAssnDel);
...
class LifeVarEntry final {
  ...
  // First access was a set (and thus block above may have a set that can be
  // deleted
  bool m_setBeforeUse;
  // Was ever assigned (and thus above block may not preserve constant
  // propagation)
...
public:
  ...
  explicit LifeVarEntry(COMPLEXASSIGN) : m_setBeforeUse{false} {
    complexAssign();
  }
  explicit LifeVarEntry(CONSUMED) : m_setBeforeUse{false} { consumed(); }
  ~LifeVarEntry() = default;
  inline void
  simpleAssign(AstNodeAssign *assp) { // New simple A=.... assignment
    m_assignp = assp;
    m_constp = nullptr;
    m_everSet = true;
...
class LifeBlock final {
  // NODE STATE
  // Cleared each AstIf:
  //   AstVarScope::user1()   -> int.       Used in combining to detect
  //   duplicates

  // LIFE MAP
  //  For each basic block, we'll make a new map of what variables that if/else
  //  is changing
...
public:
  ...
  void checkRemoveAssign(const LifeMap::iterator &it) {
    ...
    if (!varp->isSigPublic()) {
      ...
      if (AstNode *oldassp = entp->assignp()) {
        ...
        if (debug() > 4)
          oldassp->dumpTree(cout, "       REMOVE/SAMEBLK ");
        entp->complexAssign();
  ...
  void lifeToAbove() {
    // Any varrefs under a if/else branch affect statements outside and after
    // the if/else
    if (!m_aboveLifep)
      v3fatalSrc("Pushing life when already at the top level");
    for (LifeMap::iterator it = m_map.begin(); it != m_map.end(); ++it) {
      ...
      if (it->second.everSet()) {
        // Record there may be an assignment, so we don't constant propagate
        // across the if.
  ...
  void dualBranch(LifeBlock *life1p, LifeBlock *life2p) {
    ...
    for (LifeMap::iterator it = life1p->m_map.begin();
         it != life1p->m_map.end(); ++it) {
      // When the if branch sets a var before it's used, mark that variable
      if (it->second.setBeforeUse())
        it->first->user1(1);
    }
    for (LifeMap::iterator it = life2p->m_map.begin();
         it != life2p->m_map.end(); ++it) {
      // When the else branch sets a var before it's used
      AstVarScope *nodep = it->first;
      if (it->second.setBeforeUse() && nodep->user1()) {
        // Both branches set the var, we can remove the assignment before the
        // IF.
  ...
  void lifeDump() {
    ...
    for (LifeMap::iterator it = m_map.begin(); it != m_map.end(); ++it) {
      UINFO(5, "     Ent:  " << (it->second.setBeforeUse() ? "[F]  " : "     ")
                             << it->first << endl);
...
private:
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    // For the next assignments, clear any variables that were read or written
    // in the block
    ifLifep->lifeToAbove();
  ...
  virtual void visit(AstCFunc *nodep) override {
    ...
    if (nodep->dpiImportPrototype() && !nodep->pure()) {
      m_sideEffect =
          true; // If appears on assign RHS, don't ever delete the assignment
    }
    iterateChildren(nodep);
  }
...
public:
  ...
  LifeVisitor(AstNode *nodep, LifeState *statep) {
    ...
    m_statep = statep;
    {
      ...
      iterate(nodep);
      if (m_lifep)
        VL_DO_CLEAR(delete m_lifep, m_lifep = nullptr);
    }
  }
...
class LifeTopVisitor final : public AstNVisitor {
  // Visit all top nodes searching for functions that are entry points we want
  // to start finding code within.
private:
  // STATE
  LifeState *m_statep; // Current state
...
public:
  // CONSTRUCTORS
  LifeTopVisitor(AstNetlist *nodep, LifeState *statep) : m_statep{statep} {
    iterate(nodep);
  }
...
void V3Life::lifeAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("life", 0,
