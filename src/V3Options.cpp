...
public:
  // TYPES
  using DirMap =
      std::map<const string, std::set<std::string>>; // Directory listing

  // STATE
  std::list<string> m_allArgs;     // List of every argument encountered
  std::list<string> m_incDirUsers; // Include directories (ordered)
  std::set<string>
      m_incDirUserSet; // Include directories (for removing duplicates)
  std::list<string> m_incDirFallbacks; // Include directories (ordered)
  std::set<string>
      m_incDirFallbackSet; // Include directories (for removing duplicates)
...
V3LangCode::V3LangCode(const char *textp) {
  ...
  for (int codei = V3LangCode::L_ERROR; codei < V3LangCode::_ENUM_END;
       ++codei) {
...
// Parse "--hierarchical-block
// orig_name,mangled_name,param0_name,param0_value,... " option. The format of
// value is as same as -G option. (can be string literal surrounded by ")
V3HierarchicalBlockOption::V3HierarchicalBlockOption(const string &opts) {
  ...
  for (string::const_iterator it = opts.begin(); it != opts.end();) {
    if (inStr) {
      if (*it == '\\') {
        ...
        if (it != opts.end()) {
          if (*it != ',') {
            cmdfl.v3error(hierBlock + " expects ',', but '" + *it +
                          "' is passed");
            break;
          }
          ++it;
      ...
      if (*it == '"') {
        cmdfl.v3error(hierBlock +
                      " does not allow '\"' in the middle of literal");
        break;
      }
  ...
  for (size_t i = 2; i + 1 < vals.size(); i += 2) {
    const bool inserted =
        m_parameters.insert(std::make_pair(vals[i], vals[i + 1])).second;
    if (!inserted) {
      cmdfl.v3error("Module name '" + vals[i] + "' is duplicated in " +
                    hierBlock);
    }
  }
}
...
void VTimescale::parseSlashed(FileLine *fl, const char *textp,
                              VTimescale &unitr, VTimescale &precr,
                              bool allowEmpty) {
  ...
  if (!precStr.empty()) {
    ...
    if (!unit.isNone() && !prec.isNone() && unit < prec) {
      fl->v3error("`timescale timeunit '"
                  << unitStr
                  << "' must be greater than or equal to timeprecision '"
                  << precStr << "'");
      return;
    }
    precr = prec;
...
string V3Options::fileExists(const string &filename) {
  ...
  if (!fileStatNormal(filenameOut))
    return ""; // Directory
  return filenameOut;
}

string V3Options::filePathCheckOneDir(const string &modname,
...
// Checks if a option needs to be stripped for child run of hierarchical
// Verilation. 0: Keep the option including its argument 1: Delete the option
// which has no argument 2: Delete the option and its argument
int V3Options::stripOptionsForChildRun(const string &opt, bool forTop) {
  if (opt == "Mdir" || opt == "clk" || opt == "f" || opt == "j" ||
      opt == "l2-name" || opt == "mod-prefix" || opt == "prefix" ||
      opt == "protect-lib" || opt == "protect-key" || opt == "threads" ||
      opt == "top-module" || opt == "v") {
    return 2;
  }
  if (opt == "build" ||
      (!forTop && (opt == "cc" || opt == "exe" || opt == "sc")) ||
...
string V3Options::filePath(
    FileLine *fl, const string &modname, const string &lastpath,
  ...
  if (m_relativeIncludes) {
    ...
    if (exists != "")
      return V3Os::filenameRealPath(exists);
  }
...
string V3Options::getenvSYSTEMC_ARCH() {
  ...
  if (var == "") {
    ...
    if (VL_UNCOVERABLE(VString::wildmatch(sysname.c_str(), "*solaris*"))) {
      var = "gccsparcOS5";
    } else if (VL_UNCOVERABLE(
                   VString::wildmatch(sysname.c_str(), "*cygwin*"))) {
      var = "cygwin";
    } else {
      var = "linux";
    }
...
bool V3Options::systemCFound() {
  return (systemCSystemWide() || (!getenvSYSTEMC_INCLUDE().empty() &&
                                  !getenvSYSTEMC_LIBDIR().empty()));
}
...
void V3Options::notify() {
  ...
  // Notify that all arguments have been passed and final modification can be
  // made.
  if (!outFormatOk() && !cdc() && !dpiHdrOnly() && !lintOnly() &&
      !preprocOnly() && !xmlOnly()) {
  ...
  if (m_build && (m_gmake || m_cmake)) {
    cmdfl->v3error(
        "--make cannot be used together with --build. Suggest see manual");
  }
  ...
  if (m_hierarchical && (m_hierChild || !m_hierBlocks.empty())) {
    cmdfl->v3error("--hierarchical must not be set with --hierarchical-child "
                   "or --hierarchical-block");
  }
  if (m_hierChild && m_hierBlocks.empty()) {
    cmdfl->v3error(
        "--hierarchical-block must be set when --hierarchical-child is set");
  }
  if (m_hierarchical && m_protectLib.empty() && m_protectKey.empty()) {
    // Key for hierarchical Verilation is fixed to be ccache friendly when the
    // aim of this run is not to create protec-lib.
  ...
  if (protectIds()) {
    if (allPublic()) {
      ...
      cmdfl->v3warn(E_UNSUPPORTED,
                    "Unsupported: Using --protect-ids with --public\n" //
                        + V3Error::warnMore() + "... Suggest remove --public.");
    }
    if (trace()) {
      cmdfl->v3warn(INSECURE, "Using --protect-ids with --trace may expose "
  ...
  if (m_outputSplitCFuncs < 0)
    m_outputSplitCFuncs = m_outputSplit;
  if (m_outputSplitCTrace < 0)
    m_outputSplitCTrace = m_outputSplit;
...
void V3Options::parseOpts(FileLine *fl, int argc, char **argv) {
  ...
  // Detailed error, since this is what we often get when run with minimal
  // arguments
  ...
  if (prefix() == "" && vFilesList.size() >= 1)
    m_prefix = string("V") +
               AstNode::encodeName(V3Os::filenameNonExt(*(vFilesList.begin())));
  if (modPrefix() == "")
    m_modPrefix = prefix();
...
void V3Options::parseOptsList(FileLine *fl, const string &optdir, int argc,
  ...
  for (int i = 0; i < argc; ++i) {
    addArg(argv[i]); // -f's really should be inserted in the middle, but this
                     // is for debug
  }
  ...
  // action: one of Set, OnOff, CbCall, CbOnOff, CbVal, CbPartialMatch, and
  // CbPartialMatchVal
  //   Set              : Set value to a variable, pointer_or_lambda must be a
  //   pointer to the
  //                      variable.
  //                      true is set to bool-ish variable when '-opt' is passed
  //                      to verilator. val is set to int and string variable
  //                      when '-opt val' is passed.
  //   OnOff            : Set value to a bool-ish variable, pointer_or_lambda
  //   must be a pointer
  //                      to bool or VOptionBool.
  //                      true is set if "-opt" is passed to verilator while
  //                      false is set if
  //                      "-no-opt" is given.
  ...
  //                      "-opt val" is passed to verilator, val is passed to
  //                      the lambda. If a function to be called is a member of
  //                      V3Options that only takes const string&,
  //                      callStrSetter(&V3Options::memberFunc) can be passed
  //                      instead of lambda as a syntax sugar.
  //   CbPartialMatch   : Call lambda or function that takes remaining string.
  //                      e.g. DECL_OPTION("-opt-", CbPartialMatch, [](const
  //                      char*optp) { cout << optp << endl; }); and "-opt-ABC"
  //                      is passed, "ABC" will be emit to stdout.
  //   CbPartialMatchVal: Call lambda or function that takes remaining string
  //   and value.
  //                      e.g. DECL_OPTION("-opt-", CbPartialMatchVal, [](const
  //                      char*optp, const char*valp) {
  //                               cout << optp << ":" << valp << endl; });
  //                      and "-opt-ABC VAL" is passed, "ABC:VAL" will be emit
  //                      to stdout.
  //
  // DECL_OPTION is not C-macro to get correct line coverage even when lambda is
  // passed. (If DECL_OPTION is a macro, then lambda would be collapsed into a
  // single line).

  // Plus options
  DECL_OPTION("+define+", CbPartialMatch,
              [this](const char *optp) { addDefine(optp, true); });
  ...
  DECL_OPTION("-comp-limit-syms", CbVal, [](int val) {
    VName::maxLength(val);
  }).undocumented();
  DECL_OPTION("-compiler", CbVal, [this, fl](const char *valp) {
    if (!strcmp(valp, "clang")) {
      ...
      m_compLimitMembers =
          0; // probably ok, and AFAIK doesn't support anon structs
  ...
  DECL_OPTION("-D", CbPartialMatch,
              [this](const char *valp) { addDefine(valp, false); });
  DECL_OPTION("-debug", CbCall, [this]() { setDebugMode(3); });
  DECL_OPTION("-debugi", CbVal, [this](int v) { setDebugMode(v); });
  DECL_OPTION("-debugi-", CbPartialMatchVal,
              [this](const char *optp, const char *valp) {
  ...
  DECL_OPTION("-debug-sigsegv", CbCall, throwSigsegv)
      .undocumented(); // See also --debug-abort
  ...
  DECL_OPTION("-FI", CbVal, [this, &optdir](const char *valp) {
    addForceInc(parseFileArg(optdir, valp));
  });
  DECL_OPTION("-f", CbVal, [this, fl, &optdir](const char *valp) {
  ...
  DECL_OPTION("-instr-count-dpi", CbVal, [this, fl](int val) {
    m_instrCountDpi = val;
    if (m_instrCountDpi < 0)
      fl->v3fatal("--instr-count-dpi must be non-negative: " << val);
  });
  ...
  const auto setLang = [this, fl](const char *valp) {
    ...
    if (optval.legal()) {
      ...
      fl->v3fatal("Unknown language specified: "
                  << valp << spell.bestCandidateMsg(valp));
    }
  };
  ...
  DECL_OPTION("-no-l2name", CbCall, [this]() {
    m_l2Name = "";
  }).undocumented(); // Historical
  DECL_OPTION("-l2name", CbCall, [this]() {
    m_l2Name = "v";
  }).undocumented(); // Historical
  ...
  DECL_OPTION("-O", CbPartialMatch, [this](const char *optp) {
    ...
    for (const char *cp = optp; *cp; ++cp) {
      ...
      case 'p':
        m_public = !flag;
        break; // With -Op so flag=0, we want public on so few optimizations
               // done
      //    q
      case 'r':
        m_oReorder = flag;
  ...
  DECL_OPTION("-pins-bv", CbVal, [this, fl](const char *valp) {
    ...
    if (m_pinsBv > 65)
      fl->v3fatal("--pins-bv maximum is 65: " << valp);
  });
  ...
  DECL_OPTION("-prefix", CbVal, [this](const char *valp) {
    m_prefix = valp;
    if (m_modPrefix == "")
      m_modPrefix = m_prefix;
  });
  DECL_OPTION("-private", CbCall, [this]() { m_public = false; });
  DECL_OPTION("-prof-c", OnOff, &m_profC);
  DECL_OPTION("-prof-cfuncs", CbCall,
              [this]() { m_profC = m_profCFuncs = true; });
  ...
  DECL_OPTION("-reloop-limit", CbVal, [this, fl](const char *valp) {
    ...
    if (m_reloopLimit < 2) {
      fl->v3error("--reloop-limit must be >= 2: " << valp);
    }
  });
  ...
  DECL_OPTION("-sv", CbCall,
              [this]() { m_defaultLanguage = V3LangCode::L1800_2017; });

  DECL_OPTION("-threads-coarsen", OnOff, &m_threadsCoarsen)
      .undocumented(); // Debug
  DECL_OPTION("-no-threads", CbCall, [this]() { m_threads = 0; });
  DECL_OPTION("-threads", CbVal, [this, fl](const char *valp) {
    m_threads = std::atoi(valp);
    if (m_threads < 0)
      fl->v3fatal("--threads must be >= 0: " << valp);
  });
  ...
  DECL_OPTION("-trace-threads", CbVal, [this, fl](const char *valp) {
    ...
    if (m_traceThreads < 0)
      fl->v3fatal("--trace-threads must be >= 0: " << valp);
  });
  ...
  DECL_OPTION("-unroll-count", Set, &m_unrollCount)
      .undocumented(); // Optimization tweak
  DECL_OPTION("-unroll-stmts", Set, &m_unrollStmts)
      .undocumented(); // Optimization tweak
  ...
  DECL_OPTION("-Werror-", CbPartialMatch, [this, fl](const char *optp) {
    ...
    if (code == V3ErrorCode::EC_ERROR) {
      if (!isFuture(optp))
        fl->v3fatal("Unknown warning specified: -Werror-" << optp);
    } else {
      V3Error::pretendError(code, true);
    }
  });
  DECL_OPTION("-Wfuture-", CbPartialMatch, [this](const char *optp) {
    // Note it may not be a future option, but one that is currently
    // implemented.
    addFuture(optp);
  });
  ...
  for (int i = 0; i < argc;) {
    ...
    if (!strcmp(argv[i], "-j") ||
        !strcmp(argv[i], "--j")) { // Allow gnu -- switches
      ++i;
...
void V3Options::parseOptsFile(FileLine *fl, const string &filename, bool rel) {
  ...
  while (!ifp->eof()) {
    ...
    for (string::const_iterator pos = line.begin(); pos != line.end();
         lastch = *pos++) {
      if (inCmt) {
        if (*pos == '*' && *(pos + 1) == '/') {
          inCmt = false;
          ++pos;
                 ...
                 (pos == line.begin() ||
                  isspace(lastch))) { // But allow /file//path
        break;                        // Ignore to EOL
      } else if (*pos == '#' &&
                 space_begin) { // Only # at [spaced] begin of line
        break;                  // Ignore to EOL
      } else if (*pos == '/' && *(pos + 1) == '*') {
        inCmt = true;
        space_begin = false;
  ...
  if (inCmt)
    fl->v3error("Unterminated /* comment inside -f file.");
  ...
  // Parse file using a state machine, taking into account quoted strings and
  // escaped chars
  enum state : uint8_t {
    ST_IN_OPTION,
    ST_ESCAPED_CHAR,
...
string V3Options::parseFileArg(const string &optdir,
...
void V3Options::setDebugMode(int level) {
  ...
  if (!m_dumpTree)
    m_dumpTree = 3; // Don't override if already set.
  m_stats = true;
  m_debugCheck = true;
