...
private:
  ...
  TristateVertex *makeVertex(AstNode *nodep) {
    TristateVertex *vertexp =
        reinterpret_cast<TristateVertex *>(nodep->user5p());
    if (!vertexp) {
  ...
  void graphWalkRecurseFwd(TristateVertex *vtxp, int level) {
    ...
    if (!vtxp->varp()) { // not a var where we stop the recursion
      ...
      for (V3GraphEdge *edgep = vtxp->inBeginp(); edgep;
           edgep = edgep->inNextp()) {
        TristateVertex *vvertexp =
            dynamic_cast<TristateVertex *>(edgep->fromp());
        if (const AstVarRef *refp = VN_CAST(vvertexp->nodep(), VarRef)) {
          if (refp->access().isWriteOrRW()
              // Doesn't hurt to not check if already set, but by doing so when
              // we print out the debug messages, we'll see this node at level 0
              // instead.
              && !vvertexp->isTristate()) {
            vvertexp->isTristate(true);
            graphWalkRecurseFwd(vvertexp, level + 1);
  ...
  void graphWalkRecurseBack(TristateVertex *vtxp, int level) {
    // Called only on a tristate node; propagate a feedsTri attribute
    // "backwards" towards any driving nodes, i.e. from a LHS VARREF back to a
    // driving RHS VARREF This way if the RHS VARREF is also tristated we'll
    // connect the enables up to the LHS VARREF. Otherwise if not marked
    // feedsTri() we'll drop the LHS' enables, if any
    if (!(vtxp->isTristate() || vtxp->feedsTri()))
      return; // tristate involved
    if (vtxp->user() == 3)
      return;
...
public:
  ...
  void graphWalk(AstNodeModule *nodep) {
    // if (debug() >= 9) m_graph.dumpDotFilePrefixed("tri_pre__" +
    // nodep->name());
    ...
    if (debug() >= 9)
      m_graph.dumpDotFilePrefixed("tri_pos__" + nodep->name());
  }
...
class TristatePinVisitor final : public TristateBaseVisitor {
  ...
  virtual void visit(AstVarRef *nodep) override {
    UASSERT_OBJ(!nodep->access().isRW(), nodep,
                "Tristate unexpected on R/W access flip");
  ...
  virtual void visit(AstArraySel *nodep) override {
    ...
    UASSERT_OBJ(!m_lvalue, nodep,
                "ArraySel conversion to output, under tristate node");
    iterateChildren(nodep);
  }
  virtual void visit(AstSliceSel *nodep) override {
    // Doesn't work because we'd set lvalue on the array index's var
    UASSERT_OBJ(!m_lvalue, nodep,
                "SliceSel conversion to output, under tristate node");
    iterateChildren(nodep);
  }
...
class TristateVisitor final : public TristateBaseVisitor {
  ...
  //   AstVar::user3p         -> AstPull* pullup/pulldown direction (input Var's
  //   user3p) AstVar::user4p         -> AstVar* pointer to output __out var
  //   (input Var's user2p)
  // See TristateGraph:
  //   AstVar::user5p         -> TristateVertex* for variable being built
  ...
  AstNode *newEnableDeposit(AstSel *selp, AstNode *enp) {
    // Form a "deposit" instruction for given enable, using existing select as a
    // template. Would be nicer if we made this a new AST type
    AstNode *newp = new AstShiftL(
        selp->fileline(),
  ...
  void setPullDirection(AstVar *varp, AstPull *pullp) {
    ...
    if (!oldpullp) {
      ...
      if (oldpullp->direction() != pullp->direction()) {
        pullp->v3warn(E_UNSUPPORTED,
                      "Unsupported: Conflicting pull directions.\n"
  ...
  void checkUnhandled(AstNode *nodep) {
    ...
    if ((nodep->op1p() && nodep->op1p()->user1p() &&
        ...
        (nodep->op4p() && nodep->op4p()->user1p() &&
         !VN_IS(nodep->op1p(), Var))) {
      nodep->v3warn(E_UNSUPPORTED, "Unsupported tristate construct: "
                                       << nodep->prettyTypeName());
    }
  }
  ...
  void insertTristates(AstNodeModule *nodep) {
    ...
    for (VarMap::iterator nextit, it = m_lhsmap.begin(); it != m_lhsmap.end();
         it = nextit) {
      nextit = it;
      ++nextit;
      ...
      AstVar *lhsp =
          invarp; // Variable to assign drive-value to (<in> or __out)
      if (!nodep->isTop() && invarp->isIO()) {
        ...
        if (invarp->user3p())
          UINFO(9, "propagate pull to " << outvarp << endl);
      ...
      for (auto refp : *refsp) {
        ...
        AstVar *newlhsp =
            new AstVar(lhsp->fileline(), AstVarType::MODULETEMP,
        ...
        AstNode *enassp = new AstAssignW(
            refp->fileline(),
            new AstVarRef(refp->fileline(), newenp, VAccess::WRITE),
            getEnp(refp));
        ...
        AstNode *ref1p =
            new AstVarRef(refp->fileline(), newlhsp, VAccess::READ);
        ...
        if (envarp) {
          AstNode *ref3p =
              new AstVarRef(refp->fileline(), newenp, VAccess::READ);
          enp = (!enp) ? ref3p : new AstOr(ref3p->fileline(), enp, ref3p);
        }
        AstNode *tmp = new AstNot(
            newenp->fileline(),
            new AstVarRef(newenp->fileline(), newenp, VAccess::READ));
        undrivenp =
            ((!undrivenp) ? tmp : new AstAnd(refp->fileline(), tmp, undrivenp));
      }
      ...
      if (envarp) {
        nodep->addStmtp(new AstAssignW(
            enp->fileline(),
            new AstVarRef(envarp->fileline(), envarp, VAccess::WRITE), enp));
      }
      // __out (child) or <in> (parent) = drive-value expression
      AstNode *assp = new AstAssignW(
          lhsp->fileline(),
          new AstVarRef(lhsp->fileline(), lhsp, VAccess::WRITE), orp);
      assp->user2(U2_BOTH); // Don't process further; already resolved
      if (debug() >= 9)
        assp->dumpTree(cout, "-lhsp-eqn: ");
      nodep->addStmtp(assp);
  ...
  virtual void visit(AstConst *nodep) override {
    ...
    if (m_graphing) {
      ...
      // Detect any Z consts and convert them to 0's with an enable that is also
      // 0.
      if (m_alhs && nodep->user1p()) {
        // A pin with 1'b0 or similar connection results in an assign with
        // constant on LHS due to the pinReconnectSimple call in visit AstPin.
  ...
  virtual void visit(AstSel *nodep) override {
    if (m_graphing) {
      ...
      if (m_alhs) {
        UINFO(9, dbgState() << nodep << endl);
        if (nodep->user1p()) {
          // Form a "deposit" instruction.  Would be nicer if we made this a new
          // AST type
          AstNode *newp = newEnableDeposit(nodep, nodep->user1p());
          nodep->fromp()->user1p(newp); // Push to varref (etc)
          if (debug() >= 9)
            newp->dumpTree(cout, "-assign-sel; ");
          m_tgraph.didProcess(nodep);
        }
        iterateChildren(nodep);
      } else {
    ...
    if (m_graphing) {
      ...
      if (m_alhs) {
        ...
        if (nodep->user1p()) {
          ...
          nodep->lhsp()->user1p(
              new AstSel(nodep->fileline(), enp->cloneTree(true),
                         nodep->rhsp()->width(), nodep->lhsp()->width()));
          nodep->rhsp()->user1p(
  ...
  virtual void visit(AstBufIf1 *nodep) override {
    ...
    if (m_graphing) {
      ...
      if (debug() >= 9)
        nodep->backp()->dumpTree(cout, "-bufif: ");
      if (m_alhs) {
        nodep->v3warn(E_UNSUPPORTED, "Unsupported LHS tristate construct: "
                                         << nodep->prettyTypeName());
        return;
    ...
    if (m_graphing) {
      ...
      AstNode *enp = new AstOr(
          ...
          new AstOr(
              ...
              new AstAnd(nodep->fileline(), en2p->cloneTree(false),
                         subexpr2p)));
  ...
  void visitCaseEq(AstNodeBiop *nodep, bool neq) {
    if (m_graphing) {
      ...
      // Unsupported: A === 3'b000 should compare with the enables, but we don't
      // do so at present, we only compare if there is a z in the equation.
      // Otherwise we'd need to attach an enable to every signal, then optimize
      // them away later when we determine the signal has no tristate
      iterateChildren(nodep);
      UINFO(9, dbgState() << nodep << endl);
      ...
      if (constp && constp->user1p() && varrefp) {
        ...
        if (neq)
          newp = new AstLogNot(fl, newp);
        UINFO(9, "       newceq " << newp << endl);
        if (debug() >= 9)
          nodep->dumpTree(cout, "-caseeq-old: ");
        if (debug() >= 9)
          newp->dumpTree(cout, "-caseeq-new: ");
        nodep->replaceWith(newp);
        VL_DO_DANGLING(pushDeletep(nodep), nodep);
      } else {
        checkUnhandled(nodep);
  ...
  void visitEqNeqWild(AstNodeBiop *nodep) {
    if (!VN_IS(nodep->rhsp(), Const)) {
      nodep->v3warn(E_UNSUPPORTED, // Says spac.
                    "Unsupported: RHS of ==? or !=? must be constant to be "
                    "synthesizable");
  ...
  virtual void visit(AstCountBits *nodep) override {
    ...
    dropop[0] = VN_IS(nodep->rhsp(), Const) &&
                VN_AS(nodep->rhsp(), Const)->num().isAnyZ();
    dropop[1] = VN_IS(nodep->thsp(), Const) &&
                VN_AS(nodep->thsp(), Const)->num().isAnyZ();
    dropop[2] = VN_IS(nodep->fhsp(), Const) &&
                VN_AS(nodep->fhsp(), Const)->num().isAnyZ();
    UINFO(4, " COUNTBITS(" << dropop[0] << dropop[1] << dropop[2] << " "
                           << nodep << endl);
    ...
    if (m_graphing) {
      ...
      if (dropop[0] || dropop[1] || dropop[2]) {
        // Unsupported: A $countones('0) should compare with the enables, but we
        // don't do so at present, we only compare if there is a z in the
        // equation. Otherwise we'd need to attach an enable to every signal,
        // then optimize them away later when we determine the signal has no
        // tristate
        ...
        AstNode *newp = new AstCountOnes(
            nodep->fileline(),
            new AstVarRef(nodep->fileline(), envarp, VAccess::READ));
  ...
  virtual void visit(AstPull *nodep) override {
    ...
    if (!m_graphing) {
      ...
      VL_DO_DANGLING(pushDeletep(nodep),
                     nodep); // Node must persist as user3p points to it
    }
  }
  ...
  //     N/C  input   in(from-resolver), __en(to-resolver-only),
  //     __out(to-resolver-only) N/C  inout   Spec says illegal N/C  output
  //     Unsupported; Illegal? wire input   in(from-resolver-with-wire-value),
  //     __en(from-resolver-wire),
  //                     __out(to-resolver-only)
  //     wire inout   in, __en, __out
  //     wire output  in, __en, __out
  //     const input  in(from-resolver-with-const-value),
  //     __en(from-resolver-const),
  //                     __out(to-resolver-only)
  ...
  virtual void visit(AstPin *nodep) override {
    if (m_graphing) {
      if (nodep->user2() & U2_GRAPHING)
        return; // This pin is already expanded
      nodep->user2(U2_GRAPHING);
      // Find child module's new variables.
      AstVar *enModVarp = static_cast<AstVar *>(nodep->modVarp()->user1p());
      if (!enModVarp) {
        // May have an output only that later connects to a tristate, so
        // simplify now.
      ...
      UINFO(9, dbgState() << nodep << endl);
      if (debug() >= 9)
        nodep->dumpTree(cout, "-pin-pre: ");

      // Empty/in-only; need Z to propagate
      const bool inDeclProcessing =
          (nodep->exprp() &&
      ...
      if (!nodep->exprp()) { // No-connect; covert to empty connection
        ...
        nodep->exprp(new AstVarRef(
            nodep->fileline(), ucVarp,
            // We converted, so use declaration output state
            nodep->modVarp()->declDirection().isWritable() ? VAccess::WRITE
                                                           : VAccess::READ));
        m_tgraph.setTristate(ucVarp);
        // We don't need a driver on the wire; the lack of one will default to
        // tristate
      } else if (inDeclProcessing) { // Not an input that was a converted
                                     // tristate
        // Input only may have driver in underneath module which would stomp
        // the input value.  So make a temporary connection.
        AstAssignW *reAssignp =
      ...
      // pinReconnectSimple needs to presume input or output behavior; we need
      // both Therefore, create the enable, output and separate input pin, then
      // pinReconnectSimple all Create the output enable pin, connect to new
      // signal
      AstNode *enrefp;
      {
        AstVar *enVarp =
        ...
        if (debug() >= 9)
          enpinp->dumpTree(cout, "-pin-ena: ");
      }
      // Create new output pin
      AstAssignW *outAssignp =
          nullptr; // If reconnected, the related assignment
      AstPin *outpinp = nullptr;
      AstVar *outModVarp = static_cast<AstVar *>(nodep->modVarp()->user4p());
      if (!outModVarp) {
        // At top, no need for __out as might be input only. Otherwise
        // resolvable.
        if (!m_modp->isTop()) {
        ...
        if (debug() >= 9)
          outpinp->dumpTree(cout, "-pin-opr: ");
        outAssignp = V3Inst::pinReconnectSimple(
            outpinp, m_cellp,
            true); // Note may change outpinp->exprp()
        if (debug() >= 9)
          outpinp->dumpTree(cout, "-pin-out: ");
        if (debug() >= 9 && outAssignp)
          outAssignp->dumpTree(cout, "-pin-out: ");
        // Must still iterate the outAssignp, as need to build output equation
      }

      // Existing pin becomes an input, and we mark each resulting signal as
      // tristate
      ...
      if (debug() >= 9)
        nodep->dumpTree(cout, "-pin-in:  ");
      if (debug() >= 9 && inAssignp)
        inAssignp->dumpTree(cout, "-pin-as:  ");

      // Connect enable to output signal
      AstVarRef
          *exprrefp; // Tristate variable that the Pin's expression refers to
      if (!outAssignp) {
        if (!outpinp) {
      ...
      if (exprrefp) {
        if (AstPull *pullp =
                static_cast<AstPull *>(nodep->modVarp()->user3p())) {
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (m_graphing) {
      ...
      if (nodep->access().isWriteOrRW() && m_tgraph.isTristate(nodep->varp())) {
        ...
        UASSERT_OBJ(!nodep->access().isRW(), nodep,
                    "Tristate unexpected on R/W access");
        m_tgraph.didProcess(nodep);
        mapInsertLhsVarRef(nodep);
      } else if (nodep->access().isReadOnly()
                 // Not already processed, nor varref from visit(AstPin)
                 // creation
                 && !nodep->user1p()
                 // Reference to another tristate variable
                 && m_tgraph.isTristate(nodep->varp())
                 // and in a position where it feeds upstream to another
                 // tristate
      ...
      if (m_alhs) {
      } // NOP; user1() already passed down from assignment
    }
  }
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (m_graphing) {
      ...
      if (nodep->isInoutish()
          ...
          // One example of problems is this:  "output z;  task t; z <=
          // {something}; endtask"
      ) {
      ...
      if (m_tgraph.isTristate(nodep)) {
        // nodep->isPulldown() || nodep->isPullup() handled in
        // TristateGraphVisitor
        m_tgraph.didProcess(nodep);
      }
    }
  }
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    // Not preserved, needs pointer instead: TristateGraph origTgraph =
    // m_tgraph;
    UASSERT_OBJ(m_tgraph.empty(), nodep,
                "Unsupported: NodeModule under NodeModule");
    {
      // Clear state
      m_graphing = false;
      m_tgraph.clear();
...
void V3Tristate::tristateAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("tristate", 0,
