...
void V3GraphVertex::rerouteEdges(V3Graph *graphp) {
  ...
  for (V3GraphEdge *iedgep = inBeginp(); iedgep; iedgep = iedgep->inNextp()) {
    for (V3GraphEdge *oedgep = outBeginp(); oedgep;
...
bool V3GraphVertex::inSize1() const {
  return !inEmpty() && !inBeginp()->inNextp();
}
bool V3GraphVertex::outSize1() const {
  return !outEmpty() && !outBeginp()->outNextp();
}
...
uint32_t V3GraphVertex::outHash() const {
  uint32_t hash = 0;
  for (V3GraphEdge *edgep = this->outBeginp(); edgep;
...
V3GraphEdge *V3GraphVertex::findConnectingEdgep(GraphWay way,
  ...
  while (aedgep && bedgep) {
    if (aedgep->furtherp(way) == waywardp)
      return aedgep;
    if (bedgep->furtherp(inv) == this)
      return bedgep;
...
void V3GraphEdge::init(V3Graph *graphp, V3GraphVertex *fromp,
                       V3GraphVertex *top, int weight, bool cutable) {
...
void V3Graph::clear() {
  ...
  for (V3GraphVertex *vertexp = verticesBeginp(); vertexp;
...
void V3Graph::userClearVertices() {
  ...
  for (V3GraphVertex *vertexp = verticesBeginp(); vertexp;
    ...
    vertexp->user(0);
    vertexp->userp(
        nullptr); // Its a union, but might be different size than user()
  }
}
...
void V3Graph::dumpEdge(std::ostream &os, V3GraphVertex *vertexp,
                       V3GraphEdge *edgep) {
  if (edgep->weight() &&
      (edgep->fromp() == vertexp || edgep->top() == vertexp)) {
    os << "\t\t";
    if (edgep->fromp() == vertexp)
      os << "-> " << edgep->top()->name();
...
void V3Graph::dumpDotFilePrefixed(const string &nameComment,
                                  bool colorAsSubgraph) const {
...
void V3Graph::dumpDotFilePrefixedAlways(const string &nameComment,
                                        bool colorAsSubgraph) const {
  dumpDotFile(v3Global.debugFilename(nameComment) + ".dot", colorAsSubgraph);
}
...
void V3Graph::dumpDotFile(const string &filename, bool colorAsSubgraph) const {
  ...
  *logp << "\t\t rankdir=" << dotRankDir() << "];\n";
  ...
  // We use a map here, as we don't want to corrupt anything (userp) in the
  // graph, and we don't care if this is slow.
  ...
  for (auto it = subgraphs.cbegin(); it != subgraphs.cend(); ++it) {
    ...
    *logp << "\tn" << vertexp->dotName() << (n++) << "\t[fontsize=8 "
    ...
    if (vertexp->rank())
      *logp << " r" << vertexp->rank();
    if (vertexp->fanout() != 0.0)
      *logp << " f" << vertexp->fanout();
    if (vertexp->color())
      *logp << "\\n c" << vertexp->color();
    *logp << "\"";
    *logp << ", color=" << vertexp->dotColor();
    if (vertexp->dotStyle() != "")
      *logp << ", style=" << vertexp->dotStyle();
    if (vertexp->dotShape() != "")
      *logp << ", shape=" << vertexp->dotShape();
    *logp << "];\n";
  }
  if (subgr != "")
    *logp << "\t};\n";
    ...
    for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
      ...
      if (edgep->weight()) {
        ...
        *logp << "\tn" << edgep->fromp()->dotName() << fromVnum << " -> n"
              ...
              << " ["
              //<<"fontsize=8 label=\""<<(edgep->name()!="" ? edgep->name() :
              //"\\E")<<"\""
              << "fontsize=8 label=\""
        ...
        if (edgep->dotStyle() != "")
          *logp << " style=" << edgep->dotStyle();
        // if (edgep->cutable()) *logp << ",constraint=false";  // to rank
        // without following edges
        *logp << "];\n";
      }
    }
  }
