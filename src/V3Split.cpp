...
//      ALWAYS
//              ASSIGN ({var} <= {cons})
//              Record as generating var_DLY (independent of use of var),
//              consumers ASSIGN ({var} = {cons} Record generator and consumer
...
public:
  ...
  virtual string name() const override {
    return string("POST ") + SplitNodeVertex::name();
  }
...
class SplitEdge VL_NOT_FINAL : public V3GraphEdge {
  uint32_t m_ignoreInStep =
      0; // Step number that if set to, causes this edge to be ignored
...
public:
  ...
  static bool followScoreboard(const V3GraphEdge *edgep) {
    ...
    if (!oedgep)
      v3fatalSrc("Following edge of non-SplitEdge type");
...
private:
  // NODE STATE
  // AstVarScope::user1p      -> Var SplitNodeVertex* for usage var, 0=not set
  // yet AstVarScope::user2p      -> Var SplitNodeVertex* for delayed assignment
  // var, 0=not set yet Ast*::user3p             -> Statement SplitLogicVertex*
  // (temporary only) Ast*::user4              -> Current ordering number
  // (reorderBlock usage)
...
protected:
  ...
  std::vector<SplitLogicVertex *>
      m_stmtStackps;            // Current statements being tracked
...
private:
  void scoreboardPli(AstNode *nodep) {
    ...
    if (!m_pliVertexp) {
      m_pliVertexp =
          new SplitPliVertex(&m_graph, nodep); // m_graph.clear() will delete it
    }
  ...
  void scoreboardPushStmt(AstNode *nodep) {
    ...
    UASSERT_OBJ(!nodep->user3p(), nodep,
                "user3p should not be used; cleared in processBlock");
    nodep->user3p(vertexp);
  }
  void scoreboardPopStmt() {
    // UINFO(9, "    pop" << endl);
    if (m_stmtStackps.empty())
      v3fatalSrc("Stack underflow");
...
protected:
  virtual void makeRvalueEdges(SplitVarStdVertex *vstdp) override {
    for (SplitLogicVertex *vxp : m_stmtStackps)
      new SplitRVEdge(&m_graph, vxp, vstdp);
  }
  ...
  void cleanupBlockGraph(AstNode *nodep) {
    ...
    // This follows all step-relevant edges excluding PostEdges, which are done
    // later
    m_graph.weaklyConnected(&SplitEdge::followScoreboard);

    // Add hard orderings between all nodes of same color, in the order they
    // appeared
    ...
    // Non-cutable violations should be impossible; as those edges are
    // program-order
    if (debug() >= 9)
      m_graph.dumpDotFilePrefixed(string("splitg_preo"), false);
    m_graph.acyclic(&SplitEdge::followCyclic);
    m_graph.rank(
        &SplitEdge::followCyclic); // Or order(), but that's more expensive
    if (debug() >= 9)
      m_graph.dumpDotFilePrefixed(string("splitg_opt"), false);
  }
  ...
  void processBlock(AstNode *nodep) {
    if (!nodep)
      return; // Empty lists are ignorable
    ...
    void *oldBlockUser3 =
        nodep->user3p(); // May be overloaded in below loop, save it
    nodep->user3p(nullptr);
    UASSERT_OBJ(
        nodep->firstAbovep(), nodep,
    ...
    if (!nodep->nextp()) {
      ...
      if (m_noReorderWhy != "") { // Jump or something nasty
        ...
        while (firstp->backp()->nextp() == firstp)
          firstp = firstp->backp();
  ...
  virtual void visit(AstAlways *nodep) override {
    ...
    if (debug() >= 9)
      nodep->dumpTree(cout, "   alwIn:: ");
    scoreboardClear();
    processBlock(nodep->bodysp());
    if (debug() >= 9)
      nodep->dumpTree(cout, "   alwOut: ");
  }
...
class IfColorVisitor final : public AstNVisitor {
  ...
  std::vector<AstNodeIf *>
      m_ifStack; // Stack of nested if-statements we're currently processing
...
public:
  ...
  const ColorSet &colors(AstNodeIf *nodep) const {
    ...
    UASSERT_OBJ(it != m_ifColors.end(), nodep,
                "Node missing from split color() map");
    return it->second;
  }
...
public:
  ...
  // into *newBlocksp.
  EmitSplitVisitor(AstAlways *nodep, const IfColorVisitor *ifColorp,
                   AlwaysVec *newBlocksp)
      : m_origAlwaysp{nodep}, m_ifColorp{ifColorp}, m_newBlocksp{newBlocksp} {
  ...
  void go() {
    ...
    for (unsigned int color : colors) {
      ...
      AstAlways *alwaysp = new AstAlways(m_origAlwaysp->fileline(),
...
protected:
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    for (unsigned int color : colors) {
      ...
      AstIf *clonep =
          new AstIf(nodep->fileline(), nodep->condp()->cloneTree(true),
...
public:
  ...
  virtual void visit(AstSplitPlaceholder *nodep) override {
...
protected:
  ...
  void colorAlwaysGraph() {
    ...
    for (V3GraphVertex *vertexp = m_graph.verticesBeginp(); vertexp;
        ...
        if (!oedgep->ignoreThisStep()) {
          ...
          if (debug() >= 9) {
            ...
            UINFO(0, "Cannot prune if-node due to edge "
                         << oedgep << " pointing to node " << nvxp->nodep()
                         << endl);
      ...
      for (V3GraphEdge *edgep = logicp->inBeginp(); edgep;
           edgep = edgep->inNextp()) {
        SplitEdge *oedgep = dynamic_cast<SplitEdge *>(edgep);
        oedgep->setIgnoreThisStep();
      }
    ...
    m_graph.weaklyConnected(&SplitEdge::followScoreboard);
    if (debug() >= 9)
      m_graph.dumpDotFilePrefixed("splitg_colored", false);
  }
...
void V3Split::splitReorderAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("reorder", 0,
...
void V3Split::splitAlwaysAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("split", 0,
                                v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}
