...
private:
  ...
  AstVar *findCreateVarTemp(FileLine *fl, AstCFunc *cfuncp) {
    ...
    if (!varp) {
      ...
      varp = new AstVar(fl, AstVarType::STMTTEMP, newvarname,
                        VFlagLogicPacked(), 32);
  ...
  void mergeEnd() {
    if (!m_mgAssignps.empty()) {
      ...
      UINFO(9, "End merge iter=" << items << " " << m_mgIndexHi << ":"
      ...
      if (items >= static_cast<uint32_t>(v3Global.opt.reloopLimit())) {
        ...
        AstNode *const initp =
            new AstAssign(fl, new AstVarRef(fl, itp, VAccess::WRITE),
                          new AstConst(fl, m_mgIndexLo));
        AstNode *const condp =
            new AstLte(fl, new AstVarRef(fl, itp, VAccess::READ),
                       new AstConst(fl, m_mgIndexHi));
        AstNode *const incp =
            new AstAssign(fl, new AstVarRef(fl, itp, VAccess::WRITE),
                          new AstAdd(fl, new AstConst(fl, 1),
                                     new AstVarRef(fl, itp, VAccess::READ)));
        ...
        lbitp->replaceWith(m_mgOffset > 0 ? new AstAdd(fl, lvrefp, offsetp)
                                          : lvrefp);
        ...
        if (m_mgSelRp) { // else constant and no replace
          ...
          rbitp->replaceWith(m_mgOffset < 0 ? new AstAdd(fl, rvrefp, offsetp)
                                            : rvrefp);
          VL_DO_DANGLING(rbitp->deleteTree(), lbitp);
        }
        if (debug() >= 9)
          initp->dumpTree(cout, "-new: ");
        if (debug() >= 9)
          whilep->dumpTree(cout, "-new: ");
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    if (m_mgSelLp) {                      // Old merge
      if (m_mgCfuncp == m_cfuncp          // In same function
          ...
          && (lindex == m_mgIndexLo - 1 ||
              lindex == m_mgIndexHi + 1) // Left index +/- 1
          && (m_mgConstRp ||
              lindex == rindex + m_mgOffset) // Same right index offset
      ) {
        ...
        UINFO(9, "Continue merge i=" << lindex << " " << m_mgIndexHi << ":"
                                     << m_mgIndexLo << " " << nodep << endl);
        ...
        UINFO(9,
              "End merge i="
                  << lindex << " " << m_mgIndexHi << ":" << m_mgIndexLo << " "
                  << nodep
    ...
    m_mgIndexHi = lindex;
    UINFO(9,
          "Start merge i=" << lindex << " o=" << m_mgOffset << nodep << endl);
  }
  //--------------------
...
void V3Reloop::reloopAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("reloop", 0,
