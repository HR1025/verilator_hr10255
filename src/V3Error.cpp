...
void V3Error::init() {
  ...
  if (VL_UNCOVERABLE(string(V3ErrorCode(V3ErrorCode::_ENUM_MAX).ascii()) !=
                     " MAX")) {
...
void V3Error::abortIfWarnings() {
  ...
  if (errorCount() && exwarn) {
    v3fatalExit("Exiting due to " << std::dec << errorCount()
                                  << " error(s), " //
                                  << warnCount() << " warning(s)\n");
  } else if (errorCount()) {
    v3fatalExit("Exiting due to " << std::dec << errorCount() << " error(s)\n");
  } else if (exwarn) {
    v3fatalExit("Exiting due to " << std::dec << warnCount()
                                  << " warning(s)\n");
  }
}
...
void V3Error::v3errorEnd(std::ostringstream &sstr, const string &locationStr) {
  ...
  if (s_errorCode == V3ErrorCode::EC_FATAL)
    __coverity_panic__(x);
#endif
  // Skip suppressed messages
  if (s_errorSuppressed
      // On debug, show only non default-off warning to prevent pages of
      // warnings
      && (!debug() || s_errorCode.defaultsOff()))
    return;
  string msg = msgPrefix() + sstr.str();
  if (s_errorSuppressed) { // If suppressed print only first line to reduce
                           // verbosity
  ...
  if (s_messages.find(msg) != s_messages.end())
    return;
  ...
  if (!s_errorSuppressed && !(s_errorCode == V3ErrorCode::EC_INFO ||
    ...
    if (s_errorCode >= V3ErrorCode::EC_FIRST_NAMED && !s_describedWeb) {
      ...
      std::cerr << warnMore() << "... For " << (anError ? "error" : "warning")
                ...
                << s_errorCode.ascii() << "?v=" << PACKAGE_VERSION_NUMBER_STRING
                << endl;
    }
    ...
    // If first warning is not the user's fault (internal/unsupported) then give
    // the website Not later warnings, as a internal may be caused by an earlier
    // problem
    if (s_tellManual == 0) {
    ...
    if (s_errorCode == V3ErrorCode::EC_FATAL ||
      ...
      if (!inFatal) {
        ...
        if (s_tellManual == 1) {
          std::cerr << warnMore()
                    << "... See the manual at "
                       "https://verilator.org/verilator_doc.html "
                       "for more assistance."
                    << endl;
          s_tellManual = 2;
        }
