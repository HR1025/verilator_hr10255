...
class SliceVisitor final : public AstNVisitor {
  ...
  //  AstArraySel::user1p()       -> AstVarRef. The VarRef that the final
  //  ArraySel points to
  ...
  AstNode *cloneAndSel(AstNode *nodep, int elements, int offset) {
    ...
    if (!arrayp) { // V3Width should have complained, but...
      if (!m_assignError) {
        nodep->v3error(nodep->prettyTypeName()
                       << " is not an unpacked array, but is in an unpacked "
                          "array context");
      } else {
    ...
    if (arrayp->rangep()->elementsConst() != elements) {
      if (!m_assignError) {
        nodep->v3error("Slices of arrays in assignments have different "
                       "unpacked dimensions, "
                       << elements << " versus "
    ...
    if (AstInitArray *initp = VN_CAST(nodep, InitArray)) {
      UINFO(9, "  cloneInitArray(" << elements << "," << offset << ") " << nodep
                                   << endl);
      ...
      UINFO(9, "  cloneCond(" << elements << "," << offset << ") " << nodep
                              << endl);
      ...
      UINFO(9, "  cloneSliceSel(" << elements << "," << offset << ") " << nodep
                                  << endl);
      ...
      newp = new AstArraySel(nodep->fileline(),
                             snodep->fromp()->cloneTree(false), leOffset);
    } else if (VN_IS(nodep, ArraySel) || VN_IS(nodep, NodeVarRef) ||
               VN_IS(nodep, NodeSel) || VN_IS(nodep, CMethodHard) ||
               VN_IS(nodep, MemberSel)) {
      UINFO(9, "  cloneSel(" << elements << "," << offset << ") " << nodep
                             << endl);
      const int leOffset = !arrayp->rangep()->littleEndian()
                               ? arrayp->rangep()->elementsConst() - 1 - offset
                               : offset;
      newp =
          new AstArraySel(nodep->fileline(), nodep->cloneTree(false), leOffset);
    } else {
      if (!m_assignError) {
        nodep->v3error(nodep->prettyTypeName()
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    if (!nodep->user1() && !VN_IS(nodep, AssignAlias)) {
      ...
      if (AstUnpackArrayDType *arrayp = VN_CAST(dtp, UnpackArrayDType)) {
        // Left and right could have different msb/lsbs/endianness, but
        // #elements is common and all variables are realigned to start at zero
        // Assign of a little endian'ed slice to a big endian one must reverse
        // the elements
        ...
        // Normal edit iterator will now iterate on all of the expansion
        // assignments This will potentially call this function again to resolve
        // next level of slicing
        return;
      }
      m_assignp = nodep;
  ...
  void expandBiOp(AstNodeBiop *nodep) {
    if (!nodep->user1()) {
      ...
      if (AstUnpackArrayDType *adtypep = VN_CAST(fromDtp, UnpackArrayDType)) {
        ...
        if (!VN_IS(nodep->lhsp()->dtypep()->skipRefp(), NodeArrayDType)) {
        ...
        } else if (!VN_IS(nodep->rhsp()->dtypep()->skipRefp(),
                          NodeArrayDType)) {
          nodep->rhsp()->v3error(
              "Slice operator "
          ...
          for (int index = 0; index < adtypep->rangep()->elementsConst();
               ++index) {
            // EQ(a,b) -> LOGAND(EQ(ARRAYSEL(a,0), ARRAYSEL(b,0)), ...[1])
            AstNodeBiop *clonep = VN_AS(
                nodep->cloneType(
...
void V3Slice::sliceAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("slice", 0,
