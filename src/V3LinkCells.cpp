...
public:
  ...
  virtual uint32_t rankAdder() const override {
    return m_modp->recursiveClone() ? (1 + v3Global.opt.moduleRecursionDepth())
                                    : 1;
  }
};
...
public:
  explicit LibraryVertex(V3Graph *graphp) : V3GraphVertex{graphp} {}
...
private:
  // NODE STATE
  //  Entire netlist:
  //   AstNodeModule::user1p()        // V3GraphVertex*    Vertex describing
  //   this module AstNodeModule::user2p()        // AstNodeModule*    clone
  //   used for de-recursing AstCell::user1p()              // ==V3NodeModule*
  //   if done, != if unprocessed AstCell::user2()               // bool   clone
  //   renaming completed
  ...
  AstNodeModule *resolveModule(AstNode *nodep, const string &modName) {
    ...
    if (!modp) {
      ...
      // true below -> other simulators treat modules in link-found files as
      // library cells
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    VL_RESTORER(m_modp);
    {
      ...
      if (nodep->fileline()->filebasenameNoExt() != nodep->prettyName() &&
        ...
        if (m_declfnWarned.find(nodep->fileline()->filename()) ==
            m_declfnWarned.end()) {
          m_declfnWarned.insert(nodep->fileline()->filename());
          nodep->v3warn(DECLFILENAME,
                        "Filename '" << nodep->fileline()->filebasenameNoExt()
      ...
      if (v3Global.opt.topModule() == ""
        ...
        if (!m_libVertexp)
          m_libVertexp = new LibraryVertex(&m_graph);
  ...
  virtual void visit(AstIfaceRefDType *nodep) override {
    ...
    if (modp) {
      if (VN_IS(modp, Iface)) {
        ...
        nodep->v3error(
            "Non-interface used as an interface: " << nodep->prettyNameQ());
      }
    }
    // Note cannot do modport resolution here; modports are allowed underneath
    // generates
  }
  ...
  virtual void visit(AstPackageImport *nodep) override {
    ...
    UASSERT_OBJ(nodep->packagep(), nodep,
                "Unlinked package"); // Parser should set packagep
    new V3GraphEdge(&m_graph, vertex(m_modp), vertex(nodep->packagep()), 1,
                    false);
  }
  ...
  virtual void visit(AstBind *nodep) override {
    ...
    if (modp) {
      ...
      {
        m_modp = modp;
        // Important that this adds to end, as next iterate assumes does all
        // cells
  ...
  virtual void visit(AstCell *nodep) override {
    ...
    if (nodep->user1p() == m_modp)
      return; // AstBind and AstNodeModule may call a cell twice
    if (nodep->modName() == m_origTopModuleName) {
      if (v3Global.opt.hierChild() && nodep->modName() == m_modp->origName()) {
        // Only the root of the recursive instantiation can be a hierarchical
        // block.
    ...
    // "()" the empty middle is identical to the empty rule that must find pins
    // in "(,)".
    if (nodep->pinsp() && !nodep->pinsp()->nextp() &&
        nodep->pinsp()->name() == "" && !nodep->pinsp()->exprp()) {
      pushDeletep(nodep->pinsp()->unlinkFrBackWithNext());
    }
    if (nodep->paramsp() && !nodep->paramsp()->nextp() &&
        nodep->paramsp()->name() == "" && !nodep->paramsp()->exprp()) {
    ...
    for (AstPin *nextp, *pinp = nodep->pinsp(); pinp; pinp = nextp) {
      ...
      if (pinp->dotStar()) {
        if (pinStar)
          pinp->v3error("Duplicate .* in an instance");
        pinStar = true;
    ...
    for (AstPin *pinp = nodep->pinsp(); pinp;
      ...
      if (pinp->name() == "")
        pinp->name("__pinNumber" + cvtToStr(pinp->pinNum()));
    }
    for (AstPin *pinp = nodep->paramsp(); pinp;
         pinp = VN_AS(pinp->nextp(), Pin)) {
      pinp->param(true);
      if (pinp->name() == "")
        pinp->name("__paramNumber" + cvtToStr(pinp->pinNum()));
    }
    if (nodep->modp()) {
      nodep->modName(nodep->modp()->name());
      // Note what pins exist
      std::unordered_set<string>
          ports; // Symbol table of all connected port names
      for (AstPin *pinp = nodep->pinsp(); pinp;
           pinp = VN_AS(pinp->nextp(), Pin)) {
        if (pinp->name() == "")
          pinp->v3error(
      ...
      // We search ports, rather than in/out declarations as they aren't
      // resolved yet, and it's easier to do it now than in V3LinkDot when we'd
      ...
      for (AstNode *portnodep = nodep->modp()->stmtsp(); portnodep;
        ...
        if (const AstPort *portp = VN_CAST(portnodep, Port)) {
          if (ports.find(portp->name()) == ports.end() &&
              ports.find("__pinNumber" + cvtToStr(portp->pinNum())) ==
                  ports.end()) {
            if (pinStar) {
    ...
    if (VN_IS(nodep->modp(), Iface)) {
      // Cell really is the parent's instantiation of an interface, not a normal
      // module Make sure we have a variable to refer to this cell, so can
      // <ifacename>.<innermember> in the same way that a child does.  Rename
      // though to avoid conflict with cell. This is quite similar to how
      // classes work; when unpacked classes are better supported may remap
      // interfaces to be more like a class.
      if (!nodep->hasIfaceVar()) {
        const string varName =
            nodep->name() + "__Viftop"; // V3LinkDot looks for this naming
        AstIfaceRefDType *idtypep = new AstIfaceRefDType(
            nodep->fileline(), nodep->name(), nodep->modp()->name());
        idtypep->ifacep(nullptr); // cellp overrides
        // In the case of arrayed interfaces, we replace cellp when de-arraying
        // in V3Inst
        idtypep->cellp(nodep); // Only set when real parent cell known.
        AstVar *varp;
        if (nodep->rangep()) {
          AstNodeArrayDType *arrp = new AstUnpackArrayDType(
              nodep->fileline(), VFlagChildDType(), idtypep,
  ...
  void readModNames() {
    ...
    const auto hierIt =
        vlstd::as_const(hierBlocks).find(v3Global.opt.topModule());
    UASSERT((hierIt != hierBlocks.end()) == v3Global.opt.hierChild(),
            "information of the top module must exist if --hierarchical-child "
            "is set");
    // Look at all modules, and store pointers to all module names
    for (AstNodeModule *nextp, *nodep = v3Global.rootp()->modulesp(); nodep;
         nodep = nextp) {
      nextp = VN_AS(nodep->nextp(), NodeModule);
      if (v3Global.opt.hierChild() &&
          nodep->name() == hierIt->second.origName()) {
        nodep->name(hierIt->first); // Change name of this module to be mangled
                                    // name considering parameter
      }
