...
private:
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (AstVarScope *newvscp =
            reinterpret_cast<AstVarScope *>(vscp->user4p())) {
...
private:
  ...
  //  AstVarScope::user4()    -> AstVarScope*: Passed to LifePostElim to
  //  substitute this var
  ...
  using LocMap =
      std::unordered_map<const AstVarScope *, std::set<LifeLocation>>;
  ...
  void squashAssignposts() {
    for (auto &itr : m_assignposts) {
      ...
      //      ... {no reads or writes of __PVT__q after the first write to
      //      __Vdly__q}
...
public:
  ...
  virtual ~LifePostDlyVisitor() override {
    V3Stats::addStat("Optimizations, Lifetime postassign deletions",
                     m_statAssnDel);
  }
};
...
void V3LifePost::lifepostAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("life_post", 0,
