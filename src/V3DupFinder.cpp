...
V3DupFinder::iterator V3DupFinder::findDuplicate(AstNode *nodep,
                                                 V3DupFinderUserSame *checkp) {
  const auto &er = equal_range(m_hasher(nodep));
  for (iterator it = er.first; it != er.second; ++it) {
    AstNode *const node2p = it->second;
    if (nodep == node2p)
      continue; // Same node is not a duplicate
    if (checkp && !checkp->isSame(nodep, node2p))
      continue; // User says it is not a duplicate
    if (!nodep->sameTree(node2p))
      continue; // Not the same trees
    // Found duplicate!
    return it;
  }
  return end();
...
void V3DupFinder::dumpFile(const string &filename, bool tree) {
  ...
  *logp << "    #InBucket   Occurrences\n";
  for (const auto &i : dist) {
    *logp << "    " << std::setw(9) << i.first << "  " << std::setw(12)
          << i.second << '\n';
  }
