...
protected:
  ...
  V3ListEnt<GraphAcycVertex *>
      m_work;                // List of vertices with optimization work left
...
private:
  using OrigEdgeList =
      std::list<V3GraphEdge *>; // List of orig edges, see also GraphAcyc's decl
  V3GraphEdge *origEdgep() const {
    OrigEdgeList *oEListp = static_cast<OrigEdgeList *>(userp());
    if (!oEListp)
      v3fatalSrc("No original edge associated with acyc edge " << this);
...
public:
  ...
  // yellow=we might still cut it, else oldEdge: yellowGreen=made uncutable,
  // red=uncutable
...
struct GraphAcycEdgeCmp {
  bool operator()(const V3GraphEdge *lhsp, const V3GraphEdge *rhsp) const {
    if (lhsp->weight() > rhsp->weight())
      return true; // LHS goes first
    if (lhsp->weight() < rhsp->weight())
      return false; // RHS goes first
    return false;
  }
};
...
private:
  using OrigEdgeList = std::list<V3GraphEdge *>; // List of orig edges, see also
                                                 // GraphAcycEdge's decl
  // GRAPH USERS
  //  origGraph
  //    GraphVertex::user()   GraphAycVerted* New graph node
  //  m_breakGraph
  //    GraphEdge::user()     OrigEdgeList*   Old graph edges
  //    GraphVertex::user     bool            Detection of loops in
  //    simplifyDupIterate
  // MEMBERS
  V3Graph *m_origGraphp; // Original graph
  V3Graph m_breakGraph;  // Graph with only breakable edges represented
  V3List<GraphAcycVertex *>
      m_work; // List of vertices with optimization work left
  std::vector<OrigEdgeList *>
      m_origEdgeDelp;          // List of deletions to do when done
  V3EdgeFuncP m_origEdgeFuncp; // Function that says we follow this edge (in
                               // original graph)
  uint32_t m_placeStep =
      0; // Number that user() must be equal to to indicate processing
  ...
  V3GraphEdge *edgeFromEdge(V3GraphEdge *oldedgep, V3GraphVertex *fromp,
                            V3GraphVertex *top) {
    // Make new breakGraph edge, with old edge as a template
    GraphAcycEdge *newEdgep = new GraphAcycEdge(
        &m_breakGraph, fromp, top, oldedgep->weight(), oldedgep->cutable());
  ...
  void addOrigEdgep(V3GraphEdge *toEdgep, V3GraphEdge *addEdgep) {
    ...
    // Note addEdge may already have a bunch of similar linked edge
    // representations.  Yuk.
    ...
    if (OrigEdgeList *addListp =
            static_cast<OrigEdgeList *>(addEdgep->userp())) {
      for (const auto &itr : *addListp)
        oEListp->push_back(itr);
...
public:
  ...
  ~GraphAcyc() {
    for (OrigEdgeList *ip : m_origEdgeDelp)
      delete ip;
...
void GraphAcyc::buildGraph(V3Graph *origGraphp) {
  ...
  for (V3GraphVertex *overtexp = origGraphp->verticesBeginp(); overtexp;
       overtexp = overtexp->verticesNextp()) {
    if (overtexp->color()) {
      GraphAcycVertex *avertexp =
          static_cast<GraphAcycVertex *>(overtexp->userp());
      buildGraphIterate(overtexp, avertexp);
    }
  }
}

void GraphAcyc::buildGraphIterate(V3GraphVertex *overtexp,
                                  GraphAcycVertex *avertexp) {
  // Make new edges
  for (V3GraphEdge *edgep = overtexp->outBeginp(); edgep;
       edgep = edgep->outNextp()) {
    if (origFollowEdge(edgep)) { // not cut
      V3GraphVertex *toVertexp = edgep->top();
      if (toVertexp->color()) {
        GraphAcycVertex *toAVertexp =
        ...
        V3GraphEdge *breakEdgep =
            new GraphAcycEdge(&m_breakGraph, avertexp, toAVertexp,
...
void GraphAcyc::simplifyNone(GraphAcycVertex *avertexp) {
  // Don't need any vertices with no inputs, There's no way they can have a
  // loop. Likewise, vertices with no outputs
  if (avertexp->isDelete())
    return;
...
void GraphAcyc::simplifyOne(GraphAcycVertex *avertexp) {
  // If a node has one input and one output, we can remove it and change the
  // edges
  if (avertexp->isDelete())
    return;
  if (avertexp->inSize1() && avertexp->outSize1()) {
    ...
    if (inVertexp != avertexp && outVertexp != avertexp) {
      ...
      // If both are breakable, we pick the one with less weight, else it's
      // arbitrary We can forget about the origEdge list for the "non-selected"
      // set of edges, as we need to break only one set or the other set of
      // edges, not both. (This is why we must give preference to the cutable
      // set.)
...
void GraphAcyc::simplifyOut(GraphAcycVertex *avertexp) {
  // If a node has one output that's not cutable, all its inputs can be
  // reassigned to the next node in the list
  if (avertexp->isDelete())
    return;
  if (avertexp->outSize1()) {
    ...
    if (!outEdgep->cutable()) {
      ...
      avertexp->setDelete(); // Mark so we won't delete it twice
      for (V3GraphEdge *nextp, *inEdgep = avertexp->inBeginp(); inEdgep;
           inEdgep = nextp) {
        nextp = inEdgep->inNextp();
        V3GraphVertex *inVertexp = inEdgep->fromp();
        if (inVertexp == avertexp) {
          if (debug())
            v3error("Non-cutable edge forms a loop, vertex=" << avertexp);
...
void GraphAcyc::simplifyDup(GraphAcycVertex *avertexp) {
  ...
  if (avertexp->isDelete())
    return;
  // Clear marks
  for (V3GraphEdge *edgep = avertexp->outBeginp(); edgep;
       edgep = edgep->outNextp()) {
    edgep->top()->userp(nullptr);
  }
  // Mark edges and detect duplications
  for (V3GraphEdge *nextp, *edgep = avertexp->outBeginp(); edgep;
       edgep = nextp) {
    ...
    if (prevEdgep) {
      if (!prevEdgep->cutable()) {
        ...
        //  cutable duplicates prev !cutable: know it's not a relevant loop,
        //  ignore it
        UINFO(8,
              "    DelDupEdge " << avertexp << " -> " << edgep->top() << endl);
        VL_DO_DANGLING(edgep->unlinkDelete(), edgep);
        ...
        UINFO(8,
              "    DelDupComb " << avertexp << " -> " << edgep->top() << endl);
...
void GraphAcyc::cutBackward(GraphAcycVertex *avertexp) {
  // If a cutable edge is from A->B, and there's a non-cutable edge B->A, then
  // must cut!
  if (avertexp->isDelete())
    return;
  // Clear marks
  ...
  for (V3GraphEdge *edgep = avertexp->inBeginp(); edgep;
       edgep = edgep->inNextp()) {
    if (!edgep->cutable())
      edgep->fromp()->user(true);
  }
  // Detect duplications
  for (V3GraphEdge *nextp, *edgep = avertexp->outBeginp(); edgep;
       edgep = nextp) {
...
void GraphAcyc::place() {
  ...
  // Make the vector properly sized right off the bat -- faster than
  // reallocating
  ...
  // Sort by weight, then by vertex (so that we completely process one vertex,
  // when possible)
  stable_sort(edges.begin(), edges.end(), GraphAcycEdgeCmp());

  // Process each edge in weighted order
  m_placeStep = 10;
  for (V3GraphEdge *edgep : edges)
    placeTryEdge(edgep);
}
...
void GraphAcyc::placeTryEdge(V3GraphEdge *edgep) {
  ...
  m_placeStep++;
  UINFO(8, "    PlaceEdge s" << m_placeStep << " w" << edgep->weight() << " "
                             << edgep->fromp() << endl);
  ...
  // If we come across user()==placestep, we've detected a loop and must back
  // out
...
bool GraphAcyc::placeIterate(GraphAcycVertex *vertexp, uint32_t currentRank) {
  ...
  if (vertexp->rank() >= currentRank)
    return false; // Already processed it
  if (vertexp->user() == m_placeStep)
  ...
  for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
       edgep = edgep->outNextp()) {
    if (edgep->weight() && !edgep->cutable()) {
      if (placeIterate(static_cast<GraphAcycVertex *>(edgep->top()),
                       currentRank + 1)) {
        // We don't need to reset user(); we'll use a different placeStep for
        // the next edge
        return true; // Loop detected
      }
    }
...
void GraphAcyc::main() {
  ...
  if (debug() >= 6)
    m_breakGraph.dumpDotFilePrefixed("acyc_pre");

  // Perform simple optimizations before any cuttings
  simplify(false);
  if (debug() >= 5)
    m_breakGraph.dumpDotFilePrefixed("acyc_simp");

  UINFO(4, " Cutting trivial loops\n");
  simplify(true);
  if (debug() >= 6)
    m_breakGraph.dumpDotFilePrefixed("acyc_mid");

  UINFO(4, " Ranking\n");
  m_breakGraph.rank(&V3GraphEdge::followNotCutable);
  if (debug() >= 6)
    m_breakGraph.dumpDotFilePrefixed("acyc_rank");

  UINFO(4, " Placement\n");
  place();
  if (debug() >= 6)
    m_breakGraph.dumpDotFilePrefixed("acyc_place");
  ...
  if (debug() >= 6)
    m_breakGraph.dumpDotFilePrefixed("acyc_done");
}
