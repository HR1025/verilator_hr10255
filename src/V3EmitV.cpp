...
class EmitVBaseVisitor VL_NOT_FINAL : public EmitCBaseVisitor {
  ...
  virtual void putfs(AstNode *nodep,
                     const string &str) = 0; // Fileline and node %% mark
  virtual void putqs(AstNode *nodep,
                     const string &str) = 0; // Fileline quiet w/o %% mark
  ...
  virtual void visit(AstNodeCase *nodep) override {
    ...
    if (const AstCase *casep = VN_CAST(nodep, Case)) {
      if (casep->priorityPragma())
        puts("priority ");
      if (casep->uniquePragma())
        puts("unique ");
      if (casep->unique0Pragma())
        puts("unique0 ");
    }
  ...
  virtual void visit(AstDisplay *nodep) override {
    visitNodeDisplay(nodep, nodep->filep(), nodep->fmtp()->text(),
                     nodep->fmtp()->exprsp());
  }
  virtual void visit(AstElabDisplay *nodep) override {
    visitNodeDisplay(nodep, nullptr, nodep->fmtp()->text(),
                     nodep->fmtp()->exprsp());
  }
  ...
  virtual void visit(AstSFormat *nodep) override {
    visitNodeDisplay(nodep, nodep->lhsp(), nodep->fmtp()->text(),
                     nodep->fmtp()->exprsp());
  }
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    if (const AstIf *ifp = VN_CAST(nodep, If)) {
      if (ifp->priorityPragma())
        puts("priority ");
      if (ifp->uniquePragma())
        puts("unique ");
      if (ifp->unique0Pragma())
        puts("unique0 ");
    }
    puts("if (");
    iterateAndNextNull(nodep->condp());
  ...
  virtual void visit(AstTextBlock *nodep) override {
    visit(static_cast<AstNodeSimpleText *>(nodep));
    {
      ...
      for (AstNode *childp = nodep->nodesp(); childp;
           childp = childp->nextp()) {
        iterate(childp);
      ...
      if (c == '%') {
        ...
        case 'd': {
          UASSERT_OBJ(nodep->dtypep(), nodep,
                      "emitVerilog() references undef node");
          iterateAndNextNull(nodep->dtypep());
          break;
        }
        default:
          nodep->v3fatalSrc("Unknown emitVerilog format code: %" << c);
          break;
        }
      }
    }
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    if (!m_suppressUnknown) {
      nodep->v3error("Internal: Unknown node type reached emitter: "
                     << nodep->prettyTypeName());
    }
  }
...
public:
  ...
  explicit EmitVBaseVisitor(bool suppressUnknown, AstSenTree *domainp)
      : m_suppressUnknown{suppressUnknown}, m_sensesp{domainp} {}
...
class EmitVFileVisitor final : public EmitVBaseVisitor {
  ...
  virtual void putsNoTracking(const string &str) override {
    ofp()->putsNoTracking(str);
  }

public:
  EmitVFileVisitor(AstNode *nodep, V3OutFile *ofp, bool trackText,
  ...
  virtual void putcOutput(char chr) override {
    if (chr == '\n') {
      ...
      if (m_column == 0) {
        ...
        m_os << V3OutFile::indentSpaces(m_flWidth -
                                        (m_prefixFl->ascii().length() + 1));
        m_os << " ";
        m_os << m_prefix;
      }
      m_column++;
...
public:
  ...
  EmitVPrefixedFormatter(std::ostream &os, const string &prefix, int flWidth)
      : V3OutFormatter{"__STREAM", V3OutFormatter::LA_VERILOG},
        m_os(os) // Need () or GCC 4.8 false warning
        ,
        m_prefix{prefix}, m_flWidth{flWidth} {
    m_column = 0;
    m_prefixFl =
        v3Global.rootp()->fileline(); // NETLIST's fileline instead of nullptr
                                      // to avoid nullptr checks
  }
...
class EmitVPrefixedVisitor final : public EmitVBaseVisitor {
  // MEMBERS
  EmitVPrefixedFormatter
      m_formatter; // Special verilog formatter (Way down the
                   // inheritance is another unused V3OutFormatter)
  // METHODS
  virtual void putsNoTracking(const string &str) override {
    m_formatter.putsNoTracking(str);
  }
  ...
  virtual void putfs(AstNode *nodep, const string &str) override {
    putfsqs(nodep, str, false);
  }
  virtual void putqs(AstNode *nodep, const string &str) override {
    putfsqs(nodep, str, true);
  }
  void putfsqs(AstNode *nodep, const string &str, bool quiet) {
    if (m_formatter.prefixFl() != nodep->fileline()) {
      ...
      if (m_formatter.column())
        puts("\n"); // This in turn will print the m_prefixFl
    }
    if (!quiet && nodep->user3())
      puts("%%");
    putbs(str);
  }

public:
  EmitVPrefixedVisitor(AstNode *nodep, std::ostream &os, const string &prefix,
                       int flWidth, AstSenTree *domainp, bool user3mark)
      : EmitVBaseVisitor{false, domainp}, m_formatter{os, prefix, flWidth} {
    if (user3mark)
      AstUser3InUse::check();
    iterate(nodep);
  }
...
void V3EmitV::debugEmitV(const string &stage) {
  UINFO(2, __FUNCTION__ << ": " << endl);
  const string filename = v3Global.opt.makeDir() + "/" + v3Global.opt.prefix() +
                          "__" + stage + ".v";
