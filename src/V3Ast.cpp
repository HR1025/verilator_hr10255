...
AstNode::AstNode(AstType t, FileLine *fl) : m_type{t}, m_fileline{fl} {
  editCountInc();
  m_nextp = nullptr;
  m_backp = nullptr;
  ...
  m_flags.unused =
      0; // Initializing this avoids a read-modify-write on construction
  m_user1u = VNUser(0);
  m_user1Cnt = 0;
  m_user2u = VNUser(0);
  m_user2Cnt = 0;
...
AstNode *AstNode::abovep() const {
  ...
  UASSERT_OBJ(!m_nextp || firstAbovep(), this,
              "abovep() not allowed when in midlist");
...
string AstNode::encodeName(const string &namein) {
  ...
  string out;
  for (string::const_iterator pos = namein.begin(); pos != namein.end();
       ++pos) {
    if ((pos == namein.begin())
            ? isalpha(pos[0]) // digits can't lead identifiers
            : isalnum(pos[0])) {
      out += pos[0];
    } else if (pos[0] == '_') {
      if (pos[1] == '_') {
...
string AstNode::prettyName(const string &namein) {
  ...
  for (const char *pos = namein.c_str(); *pos;) {
    ...
    if (pos[0] == '_' && pos[1] == '_') { // Short-circuit
      ...
      if (pos[0] == '_' && pos[1] == '_' && pos[2] == '0' && isxdigit(pos[3]) &&
          isxdigit(pos[4])) {
        char value = 0;
        value += 16 * (isdigit(pos[3]) ? (pos[3] - '0')
                                       : (tolower(pos[3]) - 'a' + 10));
        value +=
            (isdigit(pos[4]) ? (pos[4] - '0') : (tolower(pos[4]) - 'a' + 10));
        pretty += value;
        pos += 5;
        continue;
      }
  ...
  if (pretty[0] == 'T' && pretty.substr(0, 4) == "TOP.")
    pretty.replace(0, 4, "");
  if (pretty[0] == 'T' && pretty.substr(0, 5) == "TOP->")
    pretty.replace(0, 5, "");
  return pretty;
}
...
inline void AstNode::debugTreeChange(const AstNode *nodep, const char *prefix,
...
//                 <<prefix<<": "<<cvtToHex(this)<<"
//                 <e"<<AstNode::s_editCntGbl<<">"<<endl;
// if (debug()) {
  ...
  if (!nodep) { // verilog.y and lots of other places assume this
    ...
    if (oldtailp->m_nextp) {
      if (oldtailp->m_headtailp) {
        ...
        UDEBUGONLY(
            UASSERT_OBJ(!oldtailp->m_nextp, nodep,
        ...
        while (oldtailp->m_nextp)
          oldtailp = oldtailp->m_nextp;
      }
    }
    // Link it in
    ...
    newp->editCountInc();
    if (oldtailp->m_iterpp)
      *(oldtailp->m_iterpp) = newp; // Iterate on new item
  }
...
void AstNode::addNextHere(AstNode *newp) {
  ...
  // Backward links
  if (oldnextp)
    oldnextp->m_backp = addlastp;
  newp->m_backp = this;

  // Head/tail
  AstNode *oldheadtailp = this->m_headtailp;
  //    (!oldheadtailp)               // this was&is middle of list
  //    (oldheadtailp==this && !oldnext)// this was head AND tail (one node long
  //    list) (oldheadtailp && oldnextp)    // this was&is head of list of not
  //    just one node, not tail (oldheadtailp && !oldnextp)   // this was tail
  //    of list, might also
  ...
  if (oldheadtailp) {               // else in middle of list, no change
    if (oldheadtailp == this) {     // this was one node
      ...
      addlastp->m_headtailp =
          oldheadtailp;            // Tail needs to remember head (or nullptr)
  ...
  if (this->m_iterpp)
    *(this->m_iterpp) = newp; // Iterate on new item
...
void AstNode::relink(AstNRelinker *linkerp) {
  ...
  case AstNRelinker::RELINK_NEXT:
    backp->addNextHere(newp);
    break;
  case AstNRelinker::RELINK_OP1:
    relinkOneLink(backp->m_op1p /*ref*/, newp);
    break;
  case AstNRelinker::RELINK_OP2:
    relinkOneLink(backp->m_op2p /*ref*/, newp);
    break;
  case AstNRelinker::RELINK_OP3:
    relinkOneLink(backp->m_op3p /*ref*/, newp);
    break;
  case AstNRelinker::RELINK_OP4:
    relinkOneLink(backp->m_op4p /*ref*/, newp);
    break;
  default:
  ...
  if (linkerp->m_iterpp) {
    ...
    // This adds a unfortunate hot 8 bytes to every AstNode, but is faster than
    // passing across every function. If anyone has a cleaner way, I'd be
    // grateful.
...
void AstNode::relinkOneLink(
    AstNode *&pointpr, // Ref to pointer that gets set to newp
    AstNode *newp) {
  if (pointpr) {
...
void AstNode::deleteTree() {
  ...
  // unlinkFromBack or unlinkFromBackWithNext as appropriate before calling
  // this.
  UASSERT(!m_backp, "Delete called on node with backlink still set");
...
void AstNode::iterateChildren(AstNVisitor &v) {
  ...
  if (m_op2p)
    m_op2p->iterateAndNext(v);
  if (m_op3p)
    m_op3p->iterateAndNext(v);
  if (m_op4p)
    m_op4p->iterateAndNext(v);
}
...
void AstNode::iterateChildrenConst(AstNVisitor &v) {
  ...
  ASTNODE_PREFETCH(m_op4p);
  if (m_op1p)
    m_op1p->iterateAndNextConst(v);
  if (m_op2p)
    m_op2p->iterateAndNextConst(v);
  if (m_op3p)
    m_op3p->iterateAndNextConst(v);
  if (m_op4p)
...
void AstNode::iterateAndNext(AstNVisitor &v) {
  ...
  while (nodep) { // effectively: if (!this) return;  // Callers rely on this
    if (nodep->m_nextp)
      ASTNODE_PREFETCH(nodep->m_nextp->m_nextp);
    AstNode *niterp =
        nodep; // Pointer may get stomped via m_iterpp if the node is edited
    // Desirable check, but many places where multiple iterations are OK
    // UASSERT_OBJ(!niterp->m_iterpp, niterp, "IterateAndNext under
    ...
    if (!niterp)
      return; // Perhaps node deleted inside accept
...
void AstNode::iterateChildrenBackwards(AstNVisitor &v) {
  if (m_op1p)
    m_op1p->iterateListBackwards(v);
  if (m_op2p)
    m_op2p->iterateListBackwards(v);
  if (m_op3p)
    m_op3p->iterateListBackwards(v);
  if (m_op4p)
    m_op4p->iterateListBackwards(v);
}
...
AstNode *AstNode::iterateSubtreeReturnEdits(AstNVisitor &v) {
  ...
  AstNode *nodep =
      this; // Note "this" may point to bogus point later in this function
  if (VN_IS(nodep, Netlist)) {
    ...
    UASSERT_OBJ(nextnodepp, this,
                "Node's back doesn't point to forward to node itself");
    {
      VL_DO_DANGLING(nodep->accept(v),
                     nodep); // nodep to null as may be replaced
    }
...
void AstNode::cloneRelinkTree() {
  // private: Cleanup clone() operation on whole tree. Publicly call cloneTree()
  // instead.
...
bool AstNode::gateTreeIter() const {
  ...
  if (!isGateOptimizable())
    return false;
  if (m_op1p && !m_op1p->gateTreeIter())
    return false;
  if (m_op2p && !m_op2p->gateTreeIter())
    return false;
  if (m_op3p && !m_op3p->gateTreeIter())
    return false;
  if (m_op4p && !m_op4p->gateTreeIter())
    return false;
  return true;
}
...
bool AstNode::sameTreeIter(const AstNode *node1p, const AstNode *node2p,
  ...
  if (node1p->type() != node2p->type() ||
      node1p->dtypep() != node2p->dtypep() || !node1p->same(node2p) ||
  ...
  return (sameTreeIter(node1p->m_op1p, node2p->m_op1p, false, gateOnly) &&
          ...
          (ignNext ||
           sameTreeIter(node1p->m_nextp, node2p->m_nextp, false, gateOnly)));
}
...
void AstNode::checkTreeIter(AstNode *backp) {
  ...
  if (m_op1p)
    m_op1p->checkTreeIterList(this);
  if (m_op2p)
    m_op2p->checkTreeIterList(this);
  if (m_op3p)
    m_op3p->checkTreeIterList(this);
  if (m_op4p)
    m_op4p->checkTreeIterList(this);
}

void AstNode::checkTreeIterList(AstNode *backp) {
  // private: Check a (possible) list of nodes, this is always the head of the
  // list Audited to make sure this is never nullptr
  AstNode *headp = this;
  AstNode *tailp = this;
...
// cppcheck-suppress unusedFunction  // Debug only
void AstNode::dumpTreeGdb(
    const AstNode *nodep) { // For GDB only  // LCOV_EXCL_START
  if (!nodep) {
    cout << "<nullptr>" << endl;
    return;
  }
...
void AstNode::dumpTreeFileGdb(const AstNode *nodep,    // LCOV_EXCL_START
  ...
  const string filename =
      filenamep ? filenamep : v3Global.debugFilename("debug.tree", 98);
...
void AstNode::dumpPtrs(std::ostream &os) const {
  ...
  if (op1p())
    os << " op1p=" << cvtToHex(op1p());
  if (op2p())
    os << " op2p=" << cvtToHex(op2p());
  if (op3p())
    os << " op3p=" << cvtToHex(op3p());
  if (op4p())
    os << " op4p=" << cvtToHex(op4p());
  if (user1p())
    os << " user1p=" << cvtToHex(user1p());
  if (user2p())
    os << " user2p=" << cvtToHex(user2p());
  if (user3p())
    os << " user3p=" << cvtToHex(user3p());
  if (user4p())
    os << " user4p=" << cvtToHex(user4p());
  if (user5p())
    os << " user5p=" << cvtToHex(user5p());
  if (m_iterpp) {
...
void AstNode::dumpTree(std::ostream &os, const string &indent,
                       int maxDepth) const {
...
void AstNode::dumpTreeAndNext(std::ostream &os, const string &indent,
                              int maxDepth) const {
  // Audited to make sure this is never nullptr
...
void AstNode::dumpTreeFile(const string &filename, bool append, bool doDump,
  ...
  if (doDump) {
    { // Write log & close
      ...
      const std::unique_ptr<std::ofstream> logsp{
          V3File::new_ofstream(filename, append)};
      if (logsp->fail())
        v3fatal("Can't write " << filename);
      *logsp << "Verilator Tree Dump (format 0x3900) from <e" << std::dec
             << editCountLast();
  ...
  if (doCheck && (v3Global.opt.debugCheck() || v3Global.opt.dumpTree())) {
    ...
    if (AstNetlist *netp = VN_CAST(this, Netlist))
      V3Broken::brokenAll(netp);
  }
}
...
string AstNode::locationStr() const {
  ...
  while (backp) {
    if (VL_UNCOVERABLE(--itmax < 0)) {
      // Likely some circular back link, and V3Ast is trying to report a
      // low-level error
      UINFO(1,
            "Ran out of iterations finding locationStr on " << backp << endl);
  ...
  while (backp) {
    ...
    if ((modp = VN_CAST(backp, Module)) && !modp->hierName().empty()) {
      ...
      // VarRefs have not been flattened yet and do not contain location
      // information
...
void AstNode::dtypeChgSigned(bool flag) {
  ...
  dtypeChgWidthSigned(dtypep()->width(), dtypep()->widthMin(),
                      VSigning::fromBool(flag));
}
void AstNode::dtypeChgWidth(int width, int widthMin) {
  UASSERT_OBJ(
      dtypep(), this,
      "No dtype when changing width"); // Use ChgWidthSigned(...UNSIGNED)
                                       // otherwise
...
void AstNode::dtypeChgWidthSigned(int width, int widthMin, VSigning numeric) {
  if (!dtypep()) {
    // We allow dtypep() to be null, as before/during widthing dtypes are not
    // resolved
...
AstNodeDType *AstNode::findBitDType(int width, int widthMin,
                                    VSigning numeric) const {
  return v3Global.rootp()->typeTablep()->findLogicBitDType(
      fileline(), AstBasicDTypeKwd::BIT, width, widthMin, numeric);
}
AstNodeDType *AstNode::findLogicDType(int width, int widthMin,
                                      VSigning numeric) const {
  return v3Global.rootp()->typeTablep()->findLogicBitDType(
      fileline(), AstBasicDTypeKwd::LOGIC, width, widthMin, numeric);
}
AstNodeDType *AstNode::findLogicRangeDType(const VNumRange &range, int widthMin,
                                           VSigning numeric) const {
  return v3Global.rootp()->typeTablep()->findLogicBitDType(
      fileline(), AstBasicDTypeKwd::LOGIC, range, widthMin, numeric);
}
AstNodeDType *AstNode::findBitRangeDType(const VNumRange &range, int widthMin,
                                         VSigning numeric) const {
  return v3Global.rootp()->typeTablep()->findLogicBitDType(
      fileline(), AstBasicDTypeKwd::BIT, range, widthMin, numeric);
}
