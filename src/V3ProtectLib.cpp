...
private:
  ...
  AstTextBlock *m_comboIgnorePortsp =
      nullptr;                            // Combo ignore function port list
  ...
  void createSvFile(FileLine *fl, AstNodeModule *modp) {
    ...
    addComment(txtp, fl,
               "This module requires lib" + m_libName + ".a or lib" +
                   m_libName + ".so to work");
    addComment(txtp, fl,
    ...
    // Module declaration
    m_modPortsp =
        new AstTextBlock(fl, "module " + m_libName + " (\n", false, true);
    txtp->addNodep(m_modPortsp);
    txtp->addText(fl, ");\n\n");

    // Timescale
    if (v3Global.opt.hierChild() && v3Global.rootp()->timescaleSpecified()) {
      // Emit timescale for hierarhical verilation if input HDL specifies
      // timespec
      txtp->addText(fl, string("timeunit ") + modp->timeunit().ascii() + ";\n");
      txtp->addText(fl, string("timeprecision ") +
      ...
      addComment(txtp, fl,
                 string("timeunit ") + v3Global.rootp()->timeunit().ascii() +
                     ";");
      addComment(txtp, fl,
    ...
    m_hashValuep =
        new AstTextBlock(fl, "localparam int protectlib_hash__V = 32'd");
    ...
    txtp->addText(fl,
                  m_libName + "_protectlib_check_hash(protectlib_hash__V);\n");
    ...
    if (m_hasClk) {
      ...
      m_comboIgnoreParamsp = new AstTextBlock(
          fl, m_libName + "_protectlib_combo_ignore(\n", false, true);
    ...
    // Final
    txtp->addText(fl,
                  "final " + m_libName + "_protectlib_final(handle__V);\n\n");
  ...
  void createCppFile(FileLine *fl) {
    ...
    txtp->addText(fl,
                  "fprintf(stderr, \"%%Error: cannot use " + m_libName +
                      " library, "
    ...
    initialComment(txtp, fl);
    txtp->addText(fl, "void* " + m_libName +
                          "_protectlib_create(const char* scopep__V) {\n");
    ...
    finalComment(txtp, fl);
    txtp->addText(fl, "void " + m_libName +
                          "_protectlib_final(void* vhandlep__V) {\n");
    castPtr(fl, txtp);
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (nodep->direction() == VDirection::INPUT) {
      if (nodep->isUsedClock() ||
        ...
        UASSERT_OBJ(m_hasClk, nodep,
                    "checkIfClockExists() didn't find this clock");
        handleClock(nodep);
      } else {
  ...
  void handleClock(AstVar *varp) {
    ...
    if (m_hasClk) {
      ...
      m_clkSensp->addText(fl, "posedge " + varp->name() + " or negedge " +
                                  varp->name());
    }
  ...
  static void addLocalVariable(AstTextBlock *textp, AstVar *varp,
                               const char *suffix) {
    AstVar *newVarp = new AstVar(varp->fileline(), AstVarType::VAR,
                                 varp->name() + suffix, varp->dtypep());
  ...
  void handleOutput(AstVar *varp) {
    ...
    if (m_hasClk) {
      ...
      m_nbAssignsp->addText(fl, varp->name() + "_seq__V <= " + varp->name() +
                                    "_tmp__V;\n");
      m_seqAssignsp->addText(fl, varp->name() + " = " + varp->name() +
                                     "_seq__V;\n");
    }
    m_comboAssignsp->addText(fl, varp->name() + " = " + varp->name() +
                                     "_combo__V;\n");
    m_cComboParamsp->addText(fl, varp->dpiArgType(true, false) + "\n");
    m_cComboOutsp->addText(
...
public:
  explicit ProtectVisitor(AstNode *nodep)
      : m_libName{v3Global.opt.protectLib()}, m_topName{v3Global.opt.prefix()} {
    iterate(nodep);
  }
};
