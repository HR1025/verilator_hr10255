...
class EmitCGatherDependencies final : AstNVisitor {
  ...
  std::set<string>
      m_dependencies; // Header names to be included in output C++ file

  // METHODS
  void addSymsDependency() {
  ...
  void addModDependency(const AstNodeModule *modp) {
    if (const AstClass *const classp = VN_CAST(modp, Class)) {
      m_dependencies.insert(
          EmitCBaseVisitor::prefixNameProtect(classp->classOrPackagep()));
    } else {
  ...
  void addDTypeDependency(const AstNodeDType *nodep) {
    if (const AstClassRefDType *const dtypep = VN_CAST(nodep, ClassRefDType)) {
      m_dependencies.insert(EmitCBaseVisitor::prefixNameProtect(
          dtypep->classp()->classOrPackagep()));
    }
  }
  void addSelfDependency(const string &selfPointer, AstNode *nodep) {
    if (selfPointer.empty()) {
      // No self pointer (e.g.: function locals, const pool values, loose static
      // methods), so no dependency
    } else if (VString::startsWith(selfPointer, "this")) {
      // Dereferencing 'this', we need the definition of this module, which is
      // also the module that contains the variable.
  ...
  explicit EmitCGatherDependencies(AstCFunc *cfuncp) {
    // Strictly speaking, for loose methods, we could get away with just a
    // forward declaration of the receiver class, but their body very likely
    // includes at least one relative reference, so we are probably not loosing
    // much.
...
class EmitCImp final : EmitCFunc {
  // MEMBERS
  const AstNodeModule
      *const m_fileModp; // Files names/headers constructed using this module
  const bool m_slow;     // Creating __Slow file
  const std::set<string>
      *m_requiredHeadersp;     // Header files required by output file
  std::string m_subFileName;   // substring added to output filenames
  V3UniqueNames m_uniqueNames; // For generating unique file names

  // METHODS
  void openNextOutputFile(const std::set<string> &headers,
    ...
    if (v3Global.opt.lintOnly()) {
      // Unfortunately we have some lint checks here, so we can't just skip
      // processing. We should move them to a different stage.
      const string filename = VL_DEV_NULL;
      newCFile(filename, /* slow: */ m_slow, /* source: */ true);
      m_ofp = new V3OutCFile(filename);
    } else {
      string filename =
          v3Global.opt.makeDir() + "/" + prefixNameProtect(m_fileModp);
  ...
  void emitConfigureImp(const AstNodeModule *modp) {
    ...
    puts("\nvoid " + modName + "::" + protect("__Vconfigure") + "(" +
         symClassName() + "* _vlSymsp, bool first) {\n");
    puts("if (false && first) {}  // Prevent unused\n");
    puts("this->vlSymsp = _vlSymsp;\n"); // First, as later stuff needs it.
    if (v3Global.opt.coverage()) {
      puts(modName + "__" + protect("_configure_coverage") +
           "(this, first);\n");
    }
    puts("}\n");
    splitSizeInc(10);
  ...
  void emitCoverageImp() {
    if (v3Global.opt.coverage()) {
      puts("\n// Coverage\n");
      // Rather than putting out VL_COVER_INSERT calls directly, we do it via
      // this function. This gets around gcc slowness constructing all of the
      // template arguments.
      puts("void " + prefixNameProtect(m_modp) + "::__vlCoverInsert(");
      puts(v3Global.opt.threads() ? "std::atomic<uint32_t>" : "uint32_t");
      puts("* countp, bool enable, const char* filenamep, int lineno, int "
           "column,\n");
      puts("const char* hierp, const char* pagep, const char* commentp, const "
           "char* "
           "linescovp) "
           "{\n");
    ...
    if (v3Global.opt.savable()) {
      ...
      for (int de = 0; de < 2; ++de) {
        const string classname =
            de ? "VerilatedDeserialize" : "VerilatedSerialize";
        const string funcname = de ? "__Vdeserialize" : "__Vserialize";
        const string op = de ? ">>" : "<<";
        // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
        puts("void " + prefixNameProtect(modp) + "::" + protect(funcname) +
             "(" + classname + "& os) {\n");
        // Place a computed checksum to ensure proper structure save/restore
        // formatting OK if this hash includes some things we won't dump, since
          ...
          if (const AstVar *varp = VN_CAST(nodep, Var)) {
            if (varp->isIO() && modp->isTop() && optSystemC()) {
              ...
              for (AstUnpackArrayDType *arrayp =
                       VN_CAST(elementp, UnpackArrayDType);
              ...
              for (int v = 0; v < vects; ++v)
                puts("[__Vi" + cvtToStr(v) + "]");
              puts(";\n");
              for (int v = 0; v < vects; ++v)
                puts("}\n");
            }
  ...
  // Predicate to check if we actually need to emit anything into the common
  // implementation file. Used to avoid creating empty output files.
  ...
  void emitCommonImp(const AstNodeModule *modp) {
    const AstClass *const classp = VN_IS(modp, ClassPackage)
                                       ? VN_AS(modp, ClassPackage)->classp()
                                       : nullptr;
  ...
  void emitCFuncImp(const AstNodeModule *modp) {
    // Partition functions based on which module definitions they require, by
    // building a map from "AstNodeModules whose definitions are required" ->
    ...
    for (const auto &pair : depSet2funcps) {
      ...
      // Compute the hash of the dependencies, so we can add it to the filenames
      // to disambiguate them
      V3Hash hash;
      for (const string &name : *m_requiredHeadersp) {
        hash += name;
      }
  ...
  explicit EmitCImp(const AstNodeModule *modp, bool slow)
      : m_fileModp{modp}, m_slow{slow} {
    UINFO(5,
          "  Emitting implementation of " << prefixNameProtect(modp) << endl);

    m_modp = modp;

    // Emit implementation of this module, if this is an AstClassPackage, then
    // put the corresponding AstClass implementation in the same file as often
    // optimziations are possible when both are seen by the compiler
...
class EmitCTrace final : EmitCFunc {
  ...
  void openNextOutputFile() {
    ...
    string filename = (v3Global.opt.makeDir() + "/" + topClassName() + "_" +
                       protect("_Trace"));
    filename = m_uniqueNames.get(filename);
    if (m_slow)
      filename += "__Slow";
  ...
  void emitTraceInitOne(AstTraceDecl *nodep, int enumNum) {
    ...
    if (nodep->arrayRange().ranged())
      puts("+i*" + cvtToStr(nodep->widthWords()));
    puts(",");
    if (nodep->isScoped())
  ...
  int emitTraceDeclDType(AstNodeDType *nodep) {
    ...
    if (v3Global.opt.traceFormat().fst()) {
      // Skip over refs-to-refs, but stop before final ref so can get data type
      // name Alternatively back in V3Width we could push enum names from upper
      // typedefs
  ...
  void emitTraceChangeOne(AstTraceInc *nodep, int arrayindex) {
    ...
    puts(v3Global.opt.trueTraceThreads() && !nodep->full() ? "(base+"
                                                           : "(oldp+");
  ...
  virtual void visit(AstTraceDecl *nodep) override {
    ...
    if (nodep->arrayRange().ranged()) {
      puts("{int i; for (i=0; i<" + cvtToStr(nodep->arrayRange().elements()) +
           "; i++) {\n");
  ...
  explicit EmitCTrace(AstNodeModule *modp, bool slow) : m_slow{slow} {
    m_modp = modp;
    // Open output file
    openNextOutputFile();
    // Emit functions
...
void V3EmitC::emitcImp() {
  ...
  for (const AstNode *nodep = v3Global.rootp()->modulesp(); nodep;
    ...
    if (VN_IS(nodep, Class))
      continue; // Imped with ClassPackage
