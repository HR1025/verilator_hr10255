...
//              Enter all into std::multimap, sort by value and use a tree of <
//              and == compares.
//          "Diagonal" find of {rightmost,leftmost} bit {set,clear}
//              Ignoring mask, check each value is unique (using std::multimap
//              as above?) Each branch is then mask-and-compare operation (IE
...
#define CASE_ENCODER_GROUP_DEPTH                                               \
  8 // Levels of priority to be ORed together in top IF tree
...
private:
  ...
  virtual void visit(AstNodeCase *nodep) override {
    if (VN_IS(nodep, Case) && VN_AS(nodep, Case)->casex()) {
      nodep->v3warn(CASEX,
                    "Suggest casez (with ?'s) in place of casex (with X's)");
    }
...
private:
  ...
  bool m_caseNoOverlapsAllCovered =
      false; // Proven to be synopsys parallel_case compliant
  ...
  bool isCaseTreeFast(AstCase *nodep) {
    ...
    for (AstCaseItem *itemp = nodep->itemsp(); itemp;
         itemp = VN_AS(itemp->nextp(), CaseItem)) {
      for (AstNode *icondp = itemp->condsp(); icondp;
           icondp = icondp->nextp()) {
        if (icondp->width() > width)
          width = icondp->width();
        if (icondp->isDouble())
          opaque = true;
        if (!VN_IS(icondp, Const))
          width = CASE_BARF; // Can't parse; not a constant
        m_caseItems++;
      }
    }
    m_caseWidth = width;
    ...
    for (AstCaseItem *itemp = nodep->itemsp(); itemp;
      ...
      for (AstNode *icondp = itemp->condsp(); icondp;
        ...
        UASSERT_OBJ(iconstp, nodep,
                    "above 'can't parse' should have caught this");
    ...
    // Not done earlier, as we may now have a nullptr because it's just a ";"
    // NOP branch
    for (uint32_t i = 0; i < numCases; ++i) {
  ...
  AstNode *replaceCaseFastRecurse(AstNode *cexprp, int msb,
                                  uint32_t upperValue) {
    if (msb < 0) {
      // There's no space for a IF.  We know upperValue is thus down to a
      // specific exact value, so just return the tree value Note can't clone
      // here, as we're going to check for equivalence above
      ...
      AstNode *tree1p =
          replaceCaseFastRecurse(cexprp, msb - 1, upperValue | (1UL << msb));

      if (tree0p == tree1p) {
        // Same logic on both sides, so we can just return one of 'em
        return tree0p;
      }
      // We could have a "checkerboard" with A B A B, we can use the same IF on
      // both edges
      bool same = true;
      ...
      if (tree0p && !tree0p->user3())
        tree0p = tree0p->cloneTree(true);
      if (tree1p && !tree1p->user3())
        tree1p = tree1p->cloneTree(true);

      // Alternate scheme if we ever do multiple bits at a time:
      // V3Number nummask (cexprp, cexprp->width(), (1UL<<msb));
      // AstNode* and1p = new AstAnd(cexprp->fileline(),
      ...
      AstNode *eqp = new AstNeq(cexprp->fileline(),
                                new AstConst(cexprp->fileline(), 0), and1p);
  ...
  void replaceCaseFast(AstCase *nodep) {
    ...
    // Case expressions can't be linked twice, so clone them
    if (ifrootp && !ifrootp->user3())
      ifrootp = ifrootp->cloneTree(true);
  ...
  void replaceCaseComplicated(AstCase *nodep) {
      ...
      if (!itemp->condsp()) {
        ...
        for (AstNode *icondp = itemp->condsp(); icondp; icondp = icondNextp) {
          ...
          if (iconstp && neverItem(nodep, iconstp)) {
            ...
            condp = irangep->newAndFromInside(cexprp,
    ...
    VL_DO_DANGLING(cexprp->deleteTree(), cexprp);
    if (!hadDefault) {
      // If there was no default, add a empty one, this greatly simplifies below
      // code and constant propagation will just eliminate it for us later.
      nodep->addItemsp(new AstCaseItem(
          nodep->fileline(),
          new AstConst(nodep->fileline(), AstConst::BitTrue()), nullptr));
    }
    if (debug() >= 9)
      nodep->dumpTree(cout, "    _comp_COND: ");
    // Now build the IF statement tree
    // The tree can be quite huge.  Pull ever group of 8 out, and make a OR
    // tree. This reduces the depth for the bottom elements, at the cost of some
    // of the top elements.  If we ever have profiling data, we should pull out
    // the most common item from here and instead make it the first IF branch.
    int depth = 0;
    ...
    for (AstCaseItem *itemp = nodep->itemsp(); itemp;
      ...
      if (istmtsp)
        istmtsp->unlinkFrBackWithNext();
      // Expressioned clause
      AstNode *ifexprp = itemp->condsp()->unlinkFrBack();
      { // Prepare for next group
        if (++depth > CASE_ENCODER_GROUP_DEPTH)
          depth = 1;
        if (depth == 1) { // First group or starting new group
          itemnextp = nullptr;
          AstIf *newp = new AstIf(itemp->fileline(), ifexprp->cloneTree(true),
                                  nullptr, nullptr);
          if (groupnextp) {
          ...
          groupnextp = newp;
        } else { // Continue group, modify if condition to OR in this new
                 // condition
      ...
      { // Make the new lower IF and attach in the tree
        ...
        VL_DANGLING(ifexprp);
        if (depth ==
            CASE_ENCODER_GROUP_DEPTH) { // End of group - can skip the condition
    ...
    if (debug() >= 9)
      nodep->dumpTree(cout, "    _comp_TREE: ");
    // Handle any assertions
    replaceCaseParallel(nodep, false);
    // Replace the CASE... with IF...
    if (debug() >= 9 && grouprootp)
      grouprootp->dumpTree(cout, "     _new: ");
    if (grouprootp) {
  ...
  virtual void visit(AstCase *nodep) override {
    ...
    if (isCaseTreeFast(nodep) && v3Global.opt.oCase()) {
      ...
      // If a case statement is whole, presume signals involved aren't forming a
      // latch
      if (m_alwaysp)
...
void V3Case::caseAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("case", 0,
