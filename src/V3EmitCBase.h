...
public:
  ...
  static string protect(const string &name) {
  ...
  static string ifNoProtect(const string &in) {
  ...
  static string voidSelfAssign(const AstNodeModule *modp) {
    ...
    return className +
           "* const __restrict vlSelf VL_ATTR_UNUSED = static_cast<" +
           className + "*>(voidSelf);\n";
  }
  static string symClassName() {
    return v3Global.opt.prefix() + "_" + protect("_Syms");
  }
  static string symClassVar() {
    return symClassName() + "* __restrict vlSymsp";
  }
  ...
  static string funcNameProtect(const AstCFunc *nodep,
                                const AstNodeModule *modp = nullptr);
  static string
  prefixNameProtect(const AstNode *nodep) { // C++ name with prefix
  ...
  void emitCFuncHeader(const AstCFunc *funcp, const AstNodeModule *modp,
                       bool withScope);
