...
//*************************************************************************
// DESCRIPTION: Verilog::Preproc: Internal implementation of default
// preprocessor
//
...
class VDefineRef final {
  ...
  int m_parenLevel =
      0; // Parenthesis counting inside def args (for PARENT not child)
...
public:
  ...
  std::stack<V3PreLex *>
      m_includeStack;     ///< Stack of includers above current m_lexp
  ...
  FileLine *m_finFilelinep =
      nullptr; ///< Location of last returned token (internal only)
  ...
  std::stack<VPreIfEntry>
      m_ifdefStack;          ///< Stack of true/false emitting evaluations
...
private:
  ...
  void parsingOn() {
    m_off--;
    if (m_off < 0)
      fatalSrc("Underflow of parsing cmds");
  ...
  void statePop() {
    ...
    if (VL_UNCOVERABLE(m_states.empty())) {
      error(
          "InternalError: Pop of parser state with nothing on stack"); // LCOV_EXCL_LINE
...
public:
  ...
  void openFile(FileLine *fl, VInFilter *filterp,
  ...
  // METHODS, callbacks
  virtual void
  comment(const string &text) override; // Comment detected (if keepComments==2)
  virtual void include(
      const string &filename) override; // Request a include file be processed
  ...
  virtual string
  removeDefines(const string &text) override; // Remove defines in a text string
  ...
  ~V3PreProcImp() override {
    if (m_lexp)
      VL_DO_CLEAR(delete m_lexp, m_lexp = nullptr);
  }
};
...
void V3PreProcImp::undefineall() {
  for (DefinesMap::iterator nextit, it = m_defines.begin();
       it != m_defines.end(); it = nextit) {
    nextit = it;
    ++nextit;
...
string V3PreProcImp::commentCleanup(const string &text) {
  ...
  while ((pos = cmd.find("  ")) != string::npos)
    cmd.replace(pos, 2, " ");
  while (!cmd.empty() && isspace(cmd[cmd.size() - 1]))
...
bool V3PreProcImp::commentTokenMatch(string &cmdr, const char *strg) {
  int len = strlen(strg);
  if (VString::startsWith(cmdr, strg) &&
...
void V3PreProcImp::comment(const string &text) {
  ...
  if ((cp[0] == 'v' || cp[0] == 'V') &&
      VString::startsWith(cp + 1, "erilator")) {
    cp += strlen("verilator");
    if (*cp == '_') {
  ...
  if (synth) {
    ...
    if ((pos = cmd.find("public_flat_rw")) != string::npos) {
      // "/*verilator public_flat_rw @(foo) */" -> "/*verilator public_flat_rw*/
      // @(foo)"
...
void V3PreProc::debug(int level) {
  ...
  if (idatap->m_lexp)
    idatap->m_lexp->debug(debug() >= 5 ? debug() : 0);
}
...
void V3PreProcImp::unputDefrefString(const string &strg) {
  ...
  unputString(strg);
  // A define that inserts multiple newlines are really attributed to one source
  // line, so temporarily don't increment lineno. Must be after unput - applies
  // to new stream
...
string V3PreProcImp::trimWhitespace(const string &strg, bool trailing) {
  ...
  while (out.length() > leadspace && isspace(out[leadspace]))
    leadspace++;
  if (leadspace)
    out.erase(0, leadspace);
  // Remove trailing whitespace
  if (trailing) {
    string::size_type trailspace = 0;
    while (out.length() > trailspace &&
           isspace(out[out.length() - 1 - trailspace]))
      trailspace++;
    // Don't remove \{space_or_newline}
    if (trailspace && out.length() > trailspace &&
        out[out.length() - 1 - trailspace] == '\\')
      trailspace--;
    if (trailspace)
      out.erase(out.length() - trailspace, trailspace);
  }
  return out;
}
...
string V3PreProcImp::defineSubst(VDefineRef *refp) {
  ...
  { // Parse argument list into map
    unsigned numArgs = 0;
    string argName;
    int paren = 1; // (), {} and [] can use same counter, as must be matched
                   // pair per spec
    string token;
    bool quote = false;
    bool haveDefault = false;
    // Note there's a leading ( and trailing ), so parens==1 is the base parsing
    // level
    const string params =
        refp->params(); // Must keep str in scope to get pointer
    const char *cp = params.c_str();
    if (*cp == '(')
      cp++;
    for (; *cp; cp++) {
      // UINFO(4,"   Parse  Paren="<<paren<<"  Arg="<<numArgs<<"
      // token='"<<token<<"' Parse="<<cp<<endl);
      if (!quote && paren == 1) {
        if (*cp == ')' || *cp == ',') {
          string valueDef;
          if (haveDefault) {
          ...
          if (argName != "") {
            if (refp->args().size() > numArgs) {
              // A call `def( a ) must be equivalent to `def(a ), so
              // trimWhitespace At one point we didn't trim trailing whitespace,
              // but this confuses `"
              const string arg = trimWhitespace(refp->args()[numArgs], true);
              if (arg != "")
                valueDef = arg;
            } else if (!haveDefault) {
              error("Define missing argument '" + argName +
                    "' for: " + refp->name() + "\n");
              return " `" + refp->name() + " ";
            }
            numArgs++;
  ...
  { // Parse substitution define using arguments
    ...
    for (const char *cp = value.c_str(); (*cp) || argName != "";
         cp = (*cp ? cp + 1 : cp)) {
      ...
      if (isalpha(*cp) || *cp == '_' ||
          *cp == '$' // Won't replace system functions, since no $ in
                     // argValueByName
      ...
      if (!quote) {
        ...
        if (cp[0] == '`' && cp[1] == '`') {
          if (backslashesc) {
            ...
            out += "``"; // `` must get removed later, as `FOO```BAR must
                         // pre-expand FOO and BAR
            // See also removal in empty substitutes above
          }
          cp++;
          continue;
        } else if (cp[0] == '`' && cp[1] == '"') {
          out += "`\""; // `" means to put out a " without enabling quote mode
                        // (sort of)
...
void V3PreProcImp::openFile(FileLine *, VInFilter *filterp,
  ...
  for (const string &i : wholefile)
    flsp->contentp()->pushText(i);

  // Create new stream structure
  m_lexp->scanNewFile(flsp);
  addLineComment(1); // Enter

  // Filter all DOS CR's en-mass.  This avoids bugs with lexing CRs in the wrong
  // places. This will also strip them from strings, but strings aren't supposed
...
void V3PreProcImp::insertUnreadbackAtBol(const string &text) {
  ...
  // We don't always add a leading newline, as it may result in extra
  // unreadback(newlines).
  if (m_lineCmt == "") {
    m_lineCmtNl = true;
...
void V3PreProcImp::dumpDefines(std::ostream &os) {
  for (DefinesMap::const_iterator it = m_defines.begin(); it != m_defines.end();
       ++it) {
    os << "`define " << it->first;
...
int V3PreProcImp::getRawToken() {
  ...
  next_tok:
    if (m_lineAdd) {
      ...
      yyourtext("\n", 1);
      if (debug() >= 5)
        debugToken(VP_WHITE, "LNA");
      return VP_WHITE;
    }
    if (m_lineCmt != "") {
      // We have some `line directive or other processed data to return to the
      // user.
    ...
    if (m_lastLineno != m_lexp->m_tokFilelinep->lineno()) {
      ...
      error("Too many preprocessor tokens on a line (>" +
            cvtToStr(LINE_TOKEN_MAX) + "); perhaps recursive `define");
      tok = VP_EOF_ERROR;
    }
    ...
    if (tok == VP_EOF || tok == VP_EOF_ERROR) {
      ...
      if (tok == VP_EOF_ERROR)
        forceEof();
      // A EOF on an include, stream will find the EOF, after adding needed
      // `lines
      goto next_tok;
    }
...
void V3PreProcImp::debugToken(int tok, const char *cmtp) {
  ...
  if (debug() >= 5) {
    ...
    while ((pos = buf.find('\r')) != string::npos) {
      buf.replace(pos, 1, "\\r");
    }
    const string flcol = m_lexp->m_tokFilelinep->asciiLineCol();
    fprintf(stderr, "%s: %s %s %s(%d) dr%d:  <%d>%-10s: %s\n", flcol.c_str(),
            cmtp, (m_off ? "of" : "on"), procStateName(state()),
            static_cast<int>(m_states.size()),
            static_cast<int>(m_defRefs.size()), m_lexp->currentStartState(),
...
int V3PreProcImp::getStateToken() {
  ...
  next_tok:
    ...
    // Most states emit white space and comments between tokens. (Unless
    // collecting a string)
    if (tok == VP_WHITE && state() != ps_STRIFY)
      return tok;
    if (tok == VP_BACKQUOTE && state() != ps_STRIFY)
      tok = VP_TEXT;
    if (tok == VP_COMMENT) {
      if (!m_off) {
    ...
    if (tok == VP_SYMBOL_JOIN // not else if, can fallthru from above if()
      ...
      // a`` -> string doesn't include the ``, so can just grab next and
      // continue
      ...
      if (tok == VP_SYMBOL) {
        ...
        if (state() == ps_DEFNAME_IFDEF || state() == ps_DEFNAME_IFNDEF) {
          ...
          if (m_ifdefStack.empty()) {
            ...
            UINFO(4,
                  "Elsif " << m_lastSym << (enable ? " ON" : " OFF") << endl);
    ...
    case ps_DEFFORM: {
      if (tok == VP_DEFFORM) {
        ...
        error(string("Expecting define formal arguments. Found: ") +
              tokenName(tok) + "\n");
        goto next_tok;
      }
    }
    case ps_DEFVALUE: {
      ...
      if (tok == VP_DEFVALUE) {
        ...
        const string msg =
            string("Bad define text, unexpected ") + tokenName(tok) + "\n";
        fatalSrc(msg);
      }
      statePop();
    ...
    case ps_DEFPAREN: {
      if (tok == VP_TEXT && yyourleng() == 1 && yyourtext()[0] == '(') {
        ...
        error(string(
                  "Expecting ( to begin argument list for define reference `") +
              refp->name() + "\n");
        statePop();
        goto next_tok;
      }
    ...
    case ps_DEFARG: {
      ...
      if (tok == VP_DEFARG && yyourleng() == 1 && yyourtext()[0] == ',') {
        ...
        refp->nextarg("");
        goto next_tok;
      } else if (tok == VP_DEFARG && yyourleng() == 1 &&
                 yyourtext()[0] == ')') {
        ...
        if (m_defRefs.empty()) {
          statePop();
          if (state() == ps_JOIN) { // Handle {left}```FOO(ARG) where `FOO(ARG)
                                    // might be empty
        ...
        break;
      } else if (tok == VP_SYMBOL || tok == VP_STRING || tok == VP_TEXT ||
                 tok == VP_WHITE) {
        string rtn;
        rtn.assign(yyourtext(), yyourleng());
        ...
        error(string("Expecting ) or , to end argument list for define "
                     "reference. Found: ") +
              tokenName(tok));
        statePop();
        goto next_tok;
    ...
    case ps_STRIFY: {
      if (tok == VP_STRIFY) {
        ...
        // Note a `" MACRO_WITH(`") `" doesn't need to be handled (we don't need
        // a stack) That behavior isn't specified, and other simulators vary
        // widely
        string out = m_strify;
        m_strify = "";
    ...
    case VP_DEFREF: {
      ...
      if (!defExists(name)) { // Not found, return original string as-is
        ...
        if (params == "0" || params == "") { // Found, as simple substitution
          ...
          if (m_defRefs.empty()) {
            ...
            if (state() ==
                ps_JOIN) { // Handle {left}```FOO where `FOO might be empty
              if (VL_UNCOVERABLE(m_joinStack.empty())) {
                fatalSrc("`` join stack empty, but in a ``");
              }
              const string lhs = m_joinStack.top();
      ...
      fatalSrc("Bad case\n"); // FALLTHRU
      goto next_tok; // above fatal means unreachable, but fixes static analysis
                     // warning
    }
    case VP_ERROR: {
    ...
    case VP_STRIFY:
      ...
      // (it could be used as a include filename, for example, so need the
      // string token)
      statePush(ps_STRIFY);
      goto next_tok;
    case VP_SYMBOL:
...
int V3PreProcImp::getFinalToken(string &buf) {
  ...
  while (*bufp == '\n')
    bufp++;
  if ((tok == VP_TEXT || tok == VP_LINE) &&
      VString::startsWith(bufp, "`line ")) {
    int enter;
      ...
      if (int outBehind = (m_lexp->m_tokFilelinep->lastLineno() -
        ...
        if (debug() >= 5) {
          ...
          fprintf(stderr, "%s: FIN: readjust, fin at %d  request at %d\n",
                  flcol.c_str(), m_finFilelinep->lastLineno(),
...
string V3PreProcImp::getline() {
  // Get a single line from the parse stream.  Buffer unreturned text until the
  // newline.
  if (isEof())
    return "";
  const char *rtnp;
