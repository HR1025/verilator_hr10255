...
GraphPathChecker::GraphPathChecker(const V3Graph *graphp, V3EdgeFuncP edgeFuncp)
  ...
  for (V3GraphVertex *vxp = graphp->verticesBeginp(); vxp;
...
bool GraphPathChecker::pathExistsInternal(const V3GraphVertex *ap,
  ...
  if (costp)
    *costp = 1; // count 'a' toward the search cost
  ...
  for (V3GraphEdge *edgep = ap->outBeginp(); edgep && !foundPath;
