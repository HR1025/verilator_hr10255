...
private:
  ...
  void insertCast(AstNode *nodep,
                  int needsize) { // We'll insert ABOVE passed node
  ...
  virtual void visit(AstNodeBiop *nodep) override {
    ...
    if (nodep->sizeMattersLhs())
      ensureCast(nodep->lhsp());
    if (nodep->sizeMattersRhs())
      ensureCast(nodep->rhsp());
  }
  virtual void visit(AstNodeTriop *nodep) override {
    ...
    if (nodep->sizeMattersRhs())
      ensureCast(nodep->rhsp());
    if (nodep->sizeMattersThs())
      ensureCast(nodep->thsp());
  }
  virtual void visit(AstNodeQuadop *nodep) override {
    iterateChildren(nodep);
    nodep->user1(nodep->lhsp()->user1() | nodep->rhsp()->user1() |
                 nodep->thsp()->user1() | nodep->fhsp()->user1());
    if (nodep->sizeMattersLhs())
      ensureCast(nodep->lhsp());
    if (nodep->sizeMattersRhs())
      ensureCast(nodep->rhsp());
    if (nodep->sizeMattersThs())
      ensureCast(nodep->thsp());
    if (nodep->sizeMattersFhs())
      ensureCast(nodep->fhsp());
  }
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (nodep->access().isReadOnly() && !VN_IS(backp, CCast) &&
        VN_IS(backp, NodeMath) && !VN_IS(backp, ArraySel) &&
        !VN_IS(backp, RedXor) && backp->width() &&
...
void V3Cast::castAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("cast", 0,
