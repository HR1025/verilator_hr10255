...
private:
  ...
  virtual void visit(AstInitial *nodep) override {
    ...
    if (m_ftaskp) {
      VL_DO_DANGLING(
          nodep->replaceWith(nodep->bodysp()->unlinkFrBackWithNext()), nodep);
    }
  }
  virtual void visit(AstNodeCoverOrAssert *nodep) override {
    if (m_assertp)
      nodep->v3error("Assert not allowed under another assert");
    m_assertp = nodep;
    iterateChildren(nodep);
    m_assertp = nullptr;
  }
  virtual void visit(AstVar *nodep) override {
    ...
    if (m_classp && !nodep->isParam())
      nodep->varType(AstVarType::MEMBER);
    if (m_ftaskp)
      nodep->funcLocal(true);
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    if (m_underGenerate)
      nodep->underGenerate(true);
    // Remember the existing symbol table scope
    if (m_classp) {
      if (nodep->name() == "pre_randomize" ||
          nodep->name() == "post_randomize") {
  ...
  virtual void visit(AstSenItem *nodep) override {
    ...
    if (nodep->isClocked()) {
      ...
      if (sensp && !VN_IS(sensp, NodeVarRef) && !VN_IS(sensp, Const)) {
        ...
        const string newvarname =
            "__Vsenitemexpr" + cvtToStr(++m_senitemCvtNum);
        AstVar *newvarp = new AstVar(sensp->fileline(), AstVarType::MODULETEMP,
        ...
        sensp->replaceWith(
            new AstVarRef(sensp->fileline(), newvarp, VAccess::READ));
        AstAssignW *assignp = new AstAssignW(
            sensp->fileline(),
            new AstVarRef(sensp->fileline(), newvarp, VAccess::WRITE), sensp);
    ...
    if (!VN_IS(nodep->sensp(), NodeVarRef) &&
        ...
        && !nodep->isIllegal()) {
      if (debug())
        nodep->dumpTree(cout, "-tree: ");
      nodep->v3warn(E_UNSUPPORTED,
                    "Unsupported: Complex statement in sensitivity list");
    }
  }
  ...
  virtual void visit(AstNodePreSel *nodep) override {
    if (!nodep->attrp()) {
      iterateChildren(nodep);
      // Constification may change the fromp() to a constant, which will lose
      // the variable we're extracting from (to determine
      // MSB/LSB/endianness/etc.) So we replicate it in another node Note that
      // V3Param knows not to replace AstVarRef's under AstAttrOf's
      AstNode *basefromp = AstArraySel::baseFromp(nodep, false);
      if (AstNodeVarRef *varrefp = VN_CAST(
              basefromp, NodeVarRef)) { // Maybe varxref - so need to clone
        nodep->attrp(new AstAttrOf(nodep->fileline(), AstAttrType::VAR_BASE,
                                   varrefp->cloneTree(false)));
      } else if (AstUnlinkedRef *uvxrp = VN_CAST(
                     basefromp,
                     UnlinkedRef)) { // Maybe unlinked - so need to clone
        nodep->attrp(new AstAttrOf(nodep->fileline(), AstAttrType::VAR_BASE,
        ...
        if (basefromp)
          UINFO(1, "    Related node: " << basefromp << endl);
      } else {
        if (basefromp)
          UINFO(1, "    Related node: " << basefromp << endl);
        nodep->v3fatalSrc(
            "Illegal bit select; no signal/member being extracted from");
      }
    }
  }
  ...
  virtual void visit(AstCaseItem *nodep) override {
    ...
    // That saves us from having to search each case list twice, for
    // non-defaults and defaults
    iterateChildren(nodep);
  ...
  virtual void visit(AstPragma *nodep) override {
    if (nodep->pragType() == AstPragmaType::HIER_BLOCK) {
      ...
      // But .vlt files may still mark a module which is actually a protect-lib
      // wrapper hier_block. AstNodeModule::hierBlock() can be true only when
      // --hierarchical is specified.
  ...
  string expectFormat(AstNode *nodep, const string &format, AstNode *argp,
                      bool isScan) {
    ...
    for (const char ch : format) {
      if (!inPct && ch == '%') {
        ...
        case 'l': // %l - auto insert "library"
          ...
          if (m_modp)
            fmt = VString::quotePercent(m_modp->prettyName());
          break;
    ...
    if (argp && !isScan) {
      int skipCount =
          0; // number of args consume by any additional format strings
      while (argp) {
        if (skipCount) {
          argp = argp->nextp();
          skipCount--;
        ...
        const bool isFromString =
            (constp) ? constp->num().isFromString() : false;
        if (isFromString) {
          ...
          for (int i = 0; i < numchars; i++) {
            ...
            if (!inpercent && c == '%') {
              ...
              case '%':
                break;
              default:
                if (V3Number::displayedFmtLegal(c, isScan))
                  ++skipCount;
              }
            }
          }
  ...
  static void expectDescriptor(AstNode *nodep, AstNodeVarRef *filep) {
    if (!filep) {
      nodep->v3warn(E_UNSUPPORTED, "Unsupported: $fopen/$fclose/$f* descriptor "
                                   "must be a simple variable");
    }
    if (filep && filep->varp())
      filep->varp()->attrFileDescr(true);
  }
...
void V3LinkResolve::linkResolve(AstNetlist *rootp) {
  ...
  V3Global::dumpCheckGlobalTree("linkresolve", 0,
