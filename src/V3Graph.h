...
inline bool operator==(const GraphWay &lhs, const GraphWay &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const GraphWay &lhs, GraphWay::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(GraphWay::en lhs, const GraphWay &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  /// Thus different color if there's no directional circuit within the
  /// subgraphs. (I.E. all loops will occur within each color, not between
  /// them.)
  ...
  /// Make acyclical (into a tree) by breaking a minimal subset of cutable
  /// edges.
  ...
  void dumpDotFilePrefixed(const string &nameComment,
                           bool colorAsSubgraph = false) const;
...
protected:
  ...
  void inUnlink() {
    m_ins.reset();
  } // Low level; normally unlinkDelete is what you want
  void outUnlink() {
    m_outs.reset();
  } // Low level; normally unlinkDelete is what you want
protected:
...
public:
  ...
  virtual int sortCmp(const V3GraphVertex *rhsp) const {
    ...
    if (m_rank < rhsp->m_rank)
      return -1;
    if (m_rank > rhsp->m_rank)
      return 1;
    if (m_fanout < rhsp->m_fanout)
      return -1;
    if (m_fanout > rhsp->m_fanout)
      return 1;
    return 0;
...
public:
  // ENUMS
  enum Cutable : uint8_t {
    NOT_CUTABLE = false,
    CUTABLE = true
  }; // For passing to V3GraphEdge
protected:
  friend class V3Graph;
  friend class V3GraphVertex;
  friend class GraphAcyc;
  friend class GraphAcycEdge;
  V3ListEnt<V3GraphEdge *>
      m_outs; // Next Outbound edge for same vertex (linked list)
  V3ListEnt<V3GraphEdge *>
      m_ins; // Next Inbound edge for same vertex (linked list)
  //
...
public:
  ...
  V3GraphVertex *closerp(GraphWay way) const {
    return way.forward() ? fromp() : top();
  }
  V3GraphVertex *furtherp(GraphWay way) const {
    return way.forward() ? top() : fromp();
  }
  // STATIC ACCESSORS
  static bool followNotCutable(const V3GraphEdge *edgep) {
    return !edgep->m_cutable;
  }
