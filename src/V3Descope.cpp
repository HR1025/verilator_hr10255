...
private:
  ...
  // Construct the best self pointer to reference an object in 'scopep'  from a
  // CFunc in 'm_scopep'. Result may be relative ("this->[...]") or absolute
  // ("vlSyms->[...]").
  //
  // Using relative references allows V3Combine'ing code across multiple
  // instances of the same module.
  string descopedSelfPointer(const AstScope *scopep) {
    UASSERT(scopep, "Var/Func not scoped");
    UASSERT(!VN_IS(scopep->modp(), Class),
            "References to classes handled elsewhere");
    ...
    if (relativeRefOk && scopep == m_scopep) {
      return "this";
    } else if (relativeRefOk && !m_modSingleton &&
               scopep->aboveScopep() == m_scopep &&
               VN_IS(scopep->modp(), Module)) {
      // Reference to scope of instance directly under this module, can just
      // "this->cell", which can potentially be V3Combined, but note this
      // requires one extra pointer dereference which is slower, so we only use
      // it if the source scope is not a singleton.
      ...
      // Reference to something elsewhere, or relative references are disabled.
      // Use global variable
  ...
  void makePublicFuncWrappers() {
    ...
    // If for any given name only one function exists, we can use that function
    // directly. If multiple functions exist, we need to select the appropriate
    // scope.
    for (FuncMmap::iterator it = m_modFuncs.begin(); it != m_modFuncs.end();
         ++it) {
      ...
      ++nextIt1;
      bool moreOfSame1 =
          (nextIt1 != m_modFuncs.end() && nextIt1->first == name);
      if (moreOfSame1) {
        ...
        if (newfuncp->initsp())
          newfuncp->initsp()->unlinkFrBackWithNext()->deleteTree();
        if (newfuncp->stmtsp())
          newfuncp->stmtsp()->unlinkFrBackWithNext()->deleteTree();
        if (newfuncp->finalsp())
          newfuncp->finalsp()->unlinkFrBackWithNext()->deleteTree();
        newfuncp->name(name);
        ...
        for (FuncMmap::iterator eachIt = it;
          ...
          UINFO(6, "  at " << newfuncp->argTypes() << " und "
                           << funcp->argTypes() << endl);
          funcp->declPrivate(true);
          AstNode *argsp = nullptr;
          for (AstNode *stmtp = newfuncp->argsp(); stmtp;
               stmtp = stmtp->nextp()) {
            if (AstVar *portp = VN_CAST(stmtp, Var)) {
              if (portp->isIO() && !portp->isFuncReturn()) {
                AstNode *newp = new AstVarRef(
          ...
          if (moreOfSame) {
            AstIf *ifp = new AstIf(
                ...
                new AstEq(funcp->fileline(),
                          new AstCMath(funcp->fileline(), "this", 64),
        ...
        //                                   string("%%Error: ")+name+"() called
        //                                   with bad scope", nullptr));
        // newfuncp->addStmtsp(new AstStop(newfuncp->fileline()));
        if (debug() >= 9)
          newfuncp->dumpTree(cout, "   newfunc: ");
      } else {
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    ...
    if (!nodep->varScopep()) {
      UASSERT_OBJ(nodep->varp()->isFuncLocal(), nodep,
                  "unscoped reference can only appear to function locals at "
                  "this point");
      return;
    }
    ...
    if (varp->isFuncLocal()) {
      ...
      // Direct reference to class members are from within the class itself,
      // references from outside the class must go via AstMemberSel
  ...
  virtual void visit(AstCCall *nodep) override {
    ...
    if (VN_IS(scopep->modp(), Class)) {
      // Direct call to class methods are from within the class itself, method
      // calls from outside the class must go via AstCMethodCall
      nodep->selfPointer("this");
    } else {
      nodep->selfPointer(descopedSelfPointer(scopep));
    }
  ...
  virtual void visit(AstCFunc *nodep) override {
    VL_RESTORER(m_funcp);
    if (!nodep->user1()) {
      // Static functions should have been moved under the corresponding
      // AstClassPackage
...
void V3Descope::descopeAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("descope", 0,
