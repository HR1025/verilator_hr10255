...
class EmitCConstPool final : public EmitCConstInit {
  ...
  void maybeSplitCFile() {
    if (v3Global.opt.outputSplit() &&
        m_outFileSize < v3Global.opt.outputSplit())
      return;
  ...
  void emitVars(const AstConstPool *poolp) {
    ...
    for (AstNode *nodep = poolp->modp()->stmtsp(); nodep;
         nodep = nodep->nextp()) {
    ...
    if (varps.empty())
      return; // Constant pool is empty, so we are done

    stable_sort(varps.begin(), varps.end(),
                [](const AstVar *ap, const AstVar *bp) { //
    ...
    for (const AstVar *varp : varps) {
      ...
      const string nameProtect =
          topClassName() + "__ConstPool__" + varp->nameProtect();
      puts("\n");
      puts("extern const ");
  ...
  virtual void visit(AstConst *nodep) override {
    m_outFileSize += nodep->num().isString() ? 10
                     : nodep->isWide()       ? nodep->widthWords()
                                             : 1;
