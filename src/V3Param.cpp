...
//
//   After we complete parameters, the varp's will be wrong (point to old
//   module) and must be relinked.
//
//*************************************************************************
...
//######################################################################
// Hierarchical block and parameter db (modules without parameter is also
// handled)

class ParameterizedHierBlocks final {
  using HierBlockOptsByOrigName =
      std::multimap<std::string, const V3HierarchicalBlockOption *>;
  ...
  using GParamsMap = std::map<const std::string,
                              AstVar *>; // key:parameter name value:parameter

  // MEMBERS
  // key:Original module name, value:HiearchyBlockOption*
  // If a module is parameterized, the module is uniquiefied to overridden
  // parameters. This is why HierBlockOptsByOrigName is multimap.
...
public:
  ParameterizedHierBlocks(const V3HierBlockOptSet &hierOpts,
                          AstNetlist *nodep) {
    for (const auto &hierOpt : hierOpts) {
      m_hierBlockOptsByOrigName.insert(
          std::make_pair(hierOpt.second.origName(), &hierOpt.second));
      const V3HierarchicalBlockOption::ParamStrMap &params =
          hierOpt.second.params();
      ParamConstMap &consts = m_hierParams[&hierOpt.second];
      for (V3HierarchicalBlockOption::ParamStrMap::const_iterator pIt =
               params.begin();
           pIt != params.end(); ++pIt) {
        ...
        const bool inserted =
            consts.emplace(pIt->first, std::move(constp)).second;
      ...
      if (defParamIt != m_modParams.end()) {
        ...
        for (AstNode *stmtp = modp->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
          if (AstVar *varp = VN_CAST(stmtp, Var)) {
            if (varp->isGParam())
              defParamIt->second.emplace(varp->name(), varp);
          }
        }
      }
    }
  ...
  AstNodeModule *findByParams(const string &origName, AstPin *firstPinp,
    ...
    if (m_hierBlockOptsByOrigName.find(origName) ==
        m_hierBlockOptsByOrigName.end()) {
      return nullptr;
    }
    // This module is a hierarchical block. Need to replace it by the
    // protect-lib wrapper.
    ...
    UASSERT_OBJ(paramsIt != m_modParams.end(), modp,
                origName << " must be registered");
    HierMapIt hierIt;
    for (hierIt = candidates.first; hierIt != candidates.second; ++hierIt) {
      ...
      for (AstPin *pinp = firstPinp; pinp; pinp = VN_AS(pinp->nextp(), Pin)) {
        ...
        if (AstVar *modvarp = pinp->modVarp()) {
          ...
          UASSERT_OBJ(paramIt != paramsIt->second.end(), modvarp,
                      "must be registered");
          ...
          UINFO(5,
                "Comparing " << modvarp->name() << " " << constp << std::endl);
      ...
      if (found && paramIdx == hierIt->second->params().size())
        break;
    }
    UASSERT_OBJ(hierIt != candidates.second, firstPinp,
                "No protect-lib wrapper found");
    ...
    const auto it =
        vlstd::as_const(m_hierBlockMod).find(hierIt->second->mangledName());
    if (it == m_hierBlockMod.end())
      return nullptr;
    return it->second;
  }
  static bool areSame(AstConst *pinValuep, AstConst *hierOptParamp) {
    ...
    // in the previous run. Bitwidth of pinValuep is before width analysis, so
    // pinValuep is casted to hierOptParamp width.
    ...
    if (hierOptParamp->isDouble()) {
      ...
      if (pinValuep->isDouble()) { // Need to cast to int
        // Parameter is actually an integral type, but passed value is floating
        // point. Conversion from real to integer uses rounding in V3Width.cpp
...
class ParamProcessor final {
  // NODE STATE - Local
  //   AstVar::user4()        // int    Global parameter number (for naming new
  //   module)
  //                          //        (0=not processed, 1=iterated, but no
  //                          number,
  ...
  using IfaceRefRefs =
      std::deque<std::pair<AstIfaceRefDType *, AstIfaceRefDType *>>;
  ...
  std::map<const std::string, ModInfo>
      m_modNameMap; // Hash of created module flavors by name
  ...
  std::map<const V3Hash, ValueMapValue>
      m_valueMap;      // Hash of node hash to (param value, name)
  int m_nextValue = 1; // Next value to use in m_valueMap

  AstNodeModule *m_modp = nullptr; // Current module being processed

  // Database to get protect-lib wrapper that matches parameters in hierarchical
  // Verilation
  ParameterizedHierBlocks m_hierBlocks;
  // Default parameter values key:parameter name, value:default value (can be
  // nullptr)
  ...
  static string paramValueKey(const AstNode *nodep) {
    ...
    if (const AstIfaceRefDType *const ifrtp = VN_CAST(nodep, IfaceRefDType)) {
      ...
      key += "}";
    } else if (const AstMemberDType *const dtypep =
                   VN_CAST(nodep, MemberDType)) {
      key += " ";
  ...
  string paramValueNumber(AstNode *nodep) {
    // TODO: This parameter value number lookup via a constructed key string is
    // not
    //       particularly robust for type parameters. We should really have a
    //       type equivalence predicate function.
    ...
    for (AstNode *stmtp = modp->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
      if (AstVar *varp = VN_CAST(stmtp, Var)) {
        if (varp->isIO() || varp->isGParam() || varp->isIfaceRef()) {
          // Cloning saved a pointer to the new node for us, so just follow that
          // link.
  ...
  // Check if parameter setting during instantiation is simple enough for
  // hierarchical verilation
  void checkSupportedParam(AstNodeModule *modp, AstPin *pinp) const {
    // InitArray and AstParamTypeDType are not supported because that can not be
    // set via -G option.
    if (pinp->modVarp()) {
      ...
      pinp->v3error(AstNode::prettyNameQ(modp->origName())
                    << " has hier_block metacomment, but 'parameter type' is "
                       "not supported");
    }
  }
  bool moduleExists(const string &modName) const {
    if (m_allModuleNames.find(modName) != m_allModuleNames.end())
      return true;
    if (m_modNameMap.find(modName) != m_modNameMap.end())
      return true;
    return false;
  }
  ...
  string parameterizedHierBlockName(AstNodeModule *modp, AstPin *paramPinsp) {
    ...
    UASSERT_OBJ(modp->hierBlock(), modp,
                "should be used for hierarchical block");
    ...
    if (paramsIt ==
        m_defaultParameterValues.end()) { // Not cached yet, so check parameters
      ...
      for (AstNode *stmtp = modp->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
        if (AstVar *varp = VN_CAST(stmtp, Var)) {
          if (varp->isGParam()) {
            ...
            // constp can be nullptr if the parameter is not used to instantiate
            // sub module. varp->valuep() is not contified yet in the case.
      ...
      paramsIt =
          m_defaultParameterValues.emplace(modp, std::move(params)).first;
    }
    if (paramsIt->second.empty())
      return modp->name(); // modp has no parameter
    ...
    for (auto &&defaultValue : paramsIt->second) {
      ...
      // This longname is not valid as verilog symbol, but ok, because it will
      // be hashed
      longname += "_" + defaultValue.first + "=";
      // constp can be nullptr
      if (constp)
        longname += constp->num().ascii(false);
    }
    ...
    // The hash is used as the module suffix to find a module name that is
    ...
    while (true) {
      ...
      for (string::size_type i = 1; i < hashStr.size(); ++i) {
        ...
        // Don't use '__' not to be encoded when this module is loaded later by
        // Verilator
        if (newName.at(newName.size() - 1) != '_')
          newName += '_';
  ...
  void deepCloneModule(AstNodeModule *srcModp, AstNode *cellp, AstPin *paramsp,
    ...
    // Note all module internal variables will be re-linked to the new modules
    // by clone However links outside the module (like on the upper cells) will
    // not.
    ...
    if (newmodp->level() <= m_modp->level())
      newmodp->level(m_modp->level() + 1);
    if ((newmodp->level() - srcModp->level()) >=
    ...
    for (AstPin *pinp = paramsp; pinp; pinp = VN_AS(pinp->nextp(), Pin)) {
      if (pinp->exprp()) {
        if (AstVar *modvarp = pinp->modVarp()) {
          ...
          if (modvarp->valuep())
            modvarp->valuep()->unlinkFrBack()->deleteTree();
          ...
          if (modptp->childDTypep())
            modptp->childDTypep()->unlinkFrBack()->deleteTree();
          // Set this parameter to value requested by cell
  ...
  const ModInfo *moduleFindOrClone(AstNodeModule *srcModp, AstNode *cellp,
  ...
  void cellPinCleanup(AstNode *nodep, AstPin *pinp, AstNodeModule *srcModp,
  ...
  void cellInterfaceCleanup(AstCell *nodep, AstNodeModule *srcModp,
                            string &longnamer, bool &any_overridesr,
                            IfaceRefRefs &ifaceRefRefs) {
    for (AstPin *pinp = nodep->pinsp(); pinp;
         pinp = VN_AS(pinp->nextp(), Pin)) {
      AstVar *modvarp = pinp->modVarp();
      if (modvarp->isIfaceRef()) {
        AstIfaceRefDType *portIrefp =
            VN_CAST(modvarp->subDTypep(), IfaceRefDType);
        if (!portIrefp && arraySubDTypep(modvarp->subDTypep())) {
          portIrefp =
              VN_CAST(arraySubDTypep(modvarp->subDTypep()), IfaceRefDType);
        }
        AstIfaceRefDType *pinIrefp = nullptr;
        AstNode *exprp = pinp->exprp();
        AstVar *varp = (exprp && VN_IS(exprp, VarRef))
                           ? VN_AS(exprp, VarRef)->varp()
                           : nullptr;
        if (varp && varp->subDTypep() &&
            VN_IS(varp->subDTypep(), IfaceRefDType)) {
          pinIrefp = VN_AS(varp->subDTypep(), IfaceRefDType);
        } else if (varp && varp->subDTypep() &&
                   arraySubDTypep(varp->subDTypep()) &&
        ...
        if (!portIrefp) {
          ...
          if (portIrefp->ifaceViaCellp() != pinIrefp->ifaceViaCellp()) {
            UINFO(9,
                  "     IfaceRefDType needs reconnect  " << pinIrefp << endl);
            ...
            if (portIrefp->ifacep() != pinIrefp->ifacep()
                // Might be different only due to param cloning, so check names
                // too
...
public:
  void cellDeparam(AstCell *nodep, AstNodeModule *modp,
                   const string &hierName) {
    m_modp = modp;
    // Cell: Check for parameters in the instantiation.
    // We always run this, even if no parameters, as need to look for
    // interfaces, and remove any recursive references
    UINFO(4, "De-parameterize: " << nodep << endl);
    // Create new module name with _'s between the constants
    if (debug() >= 10)
      nodep->dumpTree(cout, "-cell: ");
    ...
    // Must be a separate loop, as constant conversion may have changed some
    // pointers. if (debug()) nodep->dumpTree(cout, "-cel2: ");
    ...
    if (srcModp->hierBlock()) {
      ...
      for (AstPin *pinp = nodep->paramsp(); pinp;
           pinp = VN_AS(pinp->nextp(), Pin)) {
        cellPinCleanup(nodep, pinp, srcModp, longname /*ref*/,
                       any_overrides /*ref*/);
      }
    }
    IfaceRefRefs ifaceRefRefs;
    cellInterfaceCleanup(nodep, srcModp, longname /*ref*/,
                         any_overrides /*ref*/, ifaceRefRefs /*ref*/);

    if (!any_overrides) {
      UINFO(8,
            "Cell parameters all match original values, skipping expansion.\n");
    } else if (AstNodeModule *paramedModp = m_hierBlocks.findByParams(
                   srcModp->name(), nodep->paramsp(), m_modp)) {
      ...
      const ModInfo *modInfop = moduleFindOrClone(
          srcModp, nodep, nodep->paramsp(), newname, ifaceRefRefs);
...
class ParamVisitor final : public AstNVisitor {
  ...
  void visitModules() {
    ...
    // so since cells originally exist top->bottom we process in top->bottom
    // order too.
    while (!m_todoModps.empty()) {
      const auto itm = m_todoModps.cbegin();
      AstNodeModule *nodep = itm->second;
      m_todoModps.erase(itm);
      if (!nodep->user5SetOnce()) { // Process once; note clone() must clear so
                                    // we do it again
        m_modp = nodep;
        ...
        //
        // Process interface cells, then non-interface which may ref an
        // interface cell
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    if (m_modp) {
      ...
      iterateChildren(nodep);
                   ...
                   2 // Haven't added top yet, so level 2 is the top
               || VN_IS(nodep, Class) // Nor moved classes
               || VN_IS(nodep,
                        Package)) { // Likewise haven't done wrapTopPackages yet
  ...
  virtual void visit(AstClassRefDType *nodep) override {
    if (nodep->paramsp()) {
      nodep->paramsp()->v3warn(E_UNSUPPORTED,
                               "Unsupported: parameterized classes");
    ...
    if (nodep->isParam()) {
      if (!nodep->valuep()) {
        ...
        V3Const::constifyParamsEdit(
            nodep); // The variable, not just the var->init()
      }
    }
  }
    ...
    for (; candp; candp = candp->nextp()) {
      if (nodep->name() == candp->name()) {
        if (AstVar *varp = VN_CAST(candp, Var)) {
          ...
          UASSERT_OBJ(pinp->exprp(), pinp,
                      "Interface parameter pin missing expression");
          VL_DO_DANGLING(nodep->replaceWith(pinp->exprp()->cloneTree(false)),
                         nodep);
          return true;
        }
      }
  ...
  virtual void visit(AstVarXRef *nodep) override {
    // Check to see if the scope is just an interface because interfaces are
    // special
  ...
  virtual void visit(AstCellArrayRef *nodep) override {
    ...
    if (const AstConst *constp = VN_CAST(nodep->selp(), Const)) {
      ...
      nodep->v3error(
          "Could not expand constant selection inside dotted reference: "
  ...
  virtual void visit(AstGenIf *nodep) override {
    ...
    if (const AstConst *constp = VN_CAST(nodep->condp(), Const)) {
      ...
      nodep->condp()->v3error(
          "Generate If condition must evaluate to constant");
    }
  }

  //! Parameter substitution for generated for loops.
  //! @todo Unlike generated IF, we don't have to worry about short-circuiting
  //! the conditional
  //!       expression, since this is currently restricted to simple
  //!       comparisons. If we ever do move to more generic constant
  //!       expressions, such code will be needed here.
  virtual void visit(AstBegin *nodep) override {
    if (nodep->genforp()) {
      ...
      V3Width::widthParamsEdit(
          forp); // Param typed widthing will NOT recurse the body
      ...
      // Note V3Unroll will replace some AstVarRef's to the loop variable with
      // constants Don't remove any deleted nodes in m_unroller until whole
      // process finishes, (are held in m_unroller), as some AstXRefs may still
      // point to old nodes.
  ...
  virtual void visit(AstGenCase *nodep) override {
    ...
    V3Width::widthParamsEdit(nodep); // Param typed widthing will NOT recurse
                                     // the body, don't trigger errors yet.
...
public:
  // CONSTRUCTORS
  explicit ParamVisitor(AstNetlist *nodep) : m_processor{nodep} {
...
void V3Param::param(AstNetlist *rootp) {
  ...
  V3Global::dumpCheckGlobalTree("param", 0,
