...
//      RETURN -> JUMPBLOCK(statements with RETURN changed to JUMPGO, ...,
//      JUMPLABEL) WHILE(... BREAK) -> JUMPBLOCK(WHILE(... statements with BREAK
//      changed to JUMPGO),
//                                    ... JUMPLABEL)
//      WHILE(... CONTINUE) -> WHILE(JUMPBLOCK(... statements with CONTINUE
//      changed to JUMPGO,
//                                    ... JUMPPABEL))
//
//*************************************************************************
...
private:
  ...
  std::vector<AstNodeBlock *>
      m_blockStack; // All begin blocks above current node
  ...
  AstJumpLabel *findAddLabel(AstNode *nodep, bool endOfIter) {
    ...
    if (VN_IS(nodep, JumpLabel))
      return VN_AS(nodep, JumpLabel); // Done
    ...
    while (underp && VN_IS(underp, Var))
      underp = underp->nextp();
    UASSERT_OBJ(underp, nodep,
                "Break/disable/continue not under expected statement");
    ...
    if (VN_IS(underp, JumpLabel)) {
      ...
      for (AstNode *nextp, *varp = underp; varp; varp = nextp) {
        nextp = varp->nextp();
        if (VN_IS(varp, Var))
          blockp->addPrev(varp->unlinkFrBack());
      }
      // Label goes last
  ...
  virtual void visit(AstRepeat *nodep) override {
    ...
    AstNode *initsp = new AstAssign(
        nodep->fileline(),
        new AstVarRef(nodep->fileline(), varp, VAccess::WRITE), countp);
    AstNode *decp = new AstAssign(
        nodep->fileline(),
        new AstVarRef(nodep->fileline(), varp, VAccess::WRITE),
  ...
  virtual void visit(AstWait *nodep) override {
    ...
    // Statements we'll just execute immediately; equivalent to if they followed
    // this
  ...
  virtual void visit(AstDisable *nodep) override {
    ...
    // }
    if (!blockp) {
      nodep->v3error("disable isn't underneath a begin with name: "
                     << nodep->prettyNameQ());
  ...
  virtual void visit(AstVarRef *nodep) override {
    if (m_loopInc && nodep->varp())
      nodep->varp()->usedLoopIdx(true);
  }
...
void V3LinkJump::linkJump(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("link", 0,
