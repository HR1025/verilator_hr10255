...
// Each module:
//      Each combo block
//          For each variable that comes from combo block and is generated AFTER
//          a usage
//              Add __Vlast_{var} to local section, init to current value (just
//              use array?) Change = if any var != last.
...
public:
  ...
  void maybeCreateTopChg() {
    ...
    // Create a wrapper change detection function that calls each change
    // detection function
    m_tlChgFuncp = new AstCFunc{m_scopetopp->fileline(), "_change_request",
                                m_scopetopp, "QData"};
    ...
    m_chgFuncp->addStmtsp(
        new AstChangeDet{m_scopetopp->fileline(), nullptr, nullptr});
  }
  void maybeCreateMidChg() {
    ...
    if (!m_chgFuncp || v3Global.opt.outputSplitCFuncs() < m_numStmts) {
      ...
      if (!m_tlChgFuncp->stmtsp()) {
        ...
        UASSERT_OBJ(returnp, m_scopetopp,
                    "Lost CReturn in top change function");
        ...
        AstNode *const newp =
            new AstCReturn{m_scopetopp->fileline(),
...
private:
  ...
  void newChangeDet() {
    ...
    AstChangeDet *const changep =
        new AstChangeDet{m_vscp->fileline(), m_varEqnp->cloneTree(true),
                         m_newRvEqnp->cloneTree(true)};
    m_statep->m_chgFuncp->addStmtsp(changep);
    AstAssign *const initp =
        new AstAssign{m_vscp->fileline(), m_newLvEqnp->cloneTree(true),
  ...
  virtual void visit(AstUnpackArrayDType *nodep) override {
    for (int index = 0; index < nodep->elementsConst(); ++index) {
      ...
      {
        m_varEqnp = new AstArraySel{nodep->fileline(),
                                    m_varEqnp->cloneTree(true), index};
        m_newLvEqnp = new AstArraySel{nodep->fileline(),
                                      m_newLvEqnp->cloneTree(true), index};
        m_newRvEqnp = new AstArraySel{nodep->fileline(),
                                      m_newRvEqnp->cloneTree(true), index};
  ...
  virtual void visit(AstNodeUOrStructDType *nodep) override {
    if (nodep->packedUnsup()) {
      ...
      if (debug())
        nodep->dumpTree(cout, "-DETECTARRAY-class-");
      m_vscp->v3warn(E_DETECTARRAY,
                     "Unsupported: Can't detect changes on complex variable"
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    if (debug())
      nodep->dumpTree(cout, "-DETECTARRAY-general-");
    m_vscp->v3warn(E_DETECTARRAY,
                   "Unsupported: Can't detect changes on complex variable"
                   " (probably with UNOPTFLAT warning suppressed): "
                       << m_vscp->varp()->prettyNameQ());
...
public:
  ...
  ChangedInsertVisitor(AstVarScope *vscp, ChangedState *statep) {
    // DPI export trigger should never need change detect. See similar
    // assertions in V3Order (OrderVisitor::nodeMarkCircular), and V3GenClk
    // (GenClkRenameVisitor::genInpClk).
    ...
    m_detects = 0;
    {
      ...
      AstVar *const newvarp = new AstVar{
          varp->fileline(), AstVarType::MODULETEMP, newvarname, varp};
      m_statep->m_topModp->addStmtp(newvarp);
      m_newvscp =
          new AstVarScope{m_vscp->fileline(), m_statep->m_scopetopp, newvarp};
      m_statep->m_scopetopp->addVarp(m_newvscp);

      m_varEqnp = new AstVarRef{m_vscp->fileline(), m_vscp, VAccess::READ};
      m_newLvEqnp =
          new AstVarRef{m_vscp->fileline(), m_newvscp, VAccess::WRITE};
...
public:
  // CONSTRUCTORS
  ChangedVisitor(AstNetlist *nodep, ChangedState *statep) : m_statep{statep} {
    iterate(nodep);
  }
...
void V3Changed::changedAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("changed", 0,
