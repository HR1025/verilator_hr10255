...
private:
  ...
  public:
    ...
    VxHolder(const V3GraphVertex *vxp, uint32_t pos, uint32_t numBlockingEdges)
        : m_vxp{vxp}, m_pos{pos}, m_numBlockingEdges{numBlockingEdges} {}
    // METHODS
    const V3GraphVertex *vertexp() const { return m_vxp; }
    // Decrement blocking edges count, return true if the vertex is
    // newly unblocked
    bool unblock() {
      UASSERT_OBJ(m_numBlockingEdges > 0, vertexp(),
                  "Underflow of blocking edges");
      m_numBlockingEdges--;
  ...
  public:
    ...
    // CONSTRUCTORS
    explicit VxHolderCmp(const T_Compare &lessThan) : m_lessThan{lessThan} {}
    // METHODS
    bool operator()(const VxHolder &a, const VxHolder &b) const {
      if (m_lessThan.operator()(a.vertexp(), b.vertexp()))
        return true;
      if (m_lessThan.operator()(b.vertexp(), a.vertexp()))
        return false;
  ...
  std::map<const V3GraphVertex *, VxHolder>
      m_waitingVertices;                   // List of waiting vertices
...
public:
  ...
  explicit GraphStream(const V3Graph *graphp, GraphWay way = GraphWay::FORWARD,
      ...
      : m_vxHolderCmp{lessThan}, m_readyVertices{m_vxHolderCmp},
        m_last{m_readyVertices.end()}, m_way{way} {
    uint32_t pos = 0;
    for (const V3GraphVertex *vxp = graphp->verticesBeginp(); vxp;
      ...
      if (way == GraphWay::FORWARD) {
        if (vxp->inEmpty()) {
          ...
          uint32_t depCount = 0;
          for (V3GraphEdge *depp = vxp->inBeginp(); depp;
               depp = depp->inNextp()) {
            ++depCount;
          }
        ...
        if (vxp->outEmpty()) {
          ...
          uint32_t depCount = 0;
          for (V3GraphEdge *depp = vxp->outBeginp(); depp;
               depp = depp->outNextp()) {
            ++depCount;
          }
          VxHolder newVx(vxp, pos++, depCount);
