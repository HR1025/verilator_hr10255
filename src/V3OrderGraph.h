...
class OrderEitherVertex VL_NOT_FINAL : public V3GraphVertex {
  ...
  AstSenTree
      *m_domainp; // Clock domain (nullptr = to be computed as we iterate)
  bool m_isFromInput =
      false; // From input, or derived therefrom (conservatively false)
protected:
  OrderEitherVertex(V3Graph *graphp, const OrderEitherVertex &old)
      : V3GraphVertex{graphp, old}, m_scopep{old.m_scopep},
        m_domainp{old.m_domainp}, m_isFromInput{old.m_isFromInput} {}
...
public:
  ...
  virtual bool
  domainMatters() = 0; // Must be in same domain when cross edge to this vertex
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::VERTEX_INPUTS;
  }
...
public:
  OrderLogicVertex(V3Graph *graphp, AstScope *scopep, AstSenTree *domainp,
                   AstNode *nodep)
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::VERTEX_LOGIC;
  }
...
protected:
  OrderVarVertex(V3Graph *graphp, const OrderVarVertex &old)
      : OrderEitherVertex{graphp, old}, m_varScp{old.m_varScp},
        m_isClock{old.m_isClock}, m_isDelayed{old.m_isDelayed} {}

public:
  OrderVarVertex(V3Graph *graphp, AstScope *scopep, AstVarScope *varScp)
      : OrderEitherVertex{graphp, scopep, nullptr}, m_varScp{varScp} {}
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::VERTEX_VARSTD;
  }
  virtual string name() const override {
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::VERTEX_VARPRE;
  }
  virtual string name() const override {
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::VERTEX_VARPOST;
  }
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::VERTEX_VARPORD;
  }
  virtual string name() const override {
...
protected:
  ...
  V3ListEnt<OrderMoveVertex *>
      m_pomWaitingE; // List of nodes needing inputs to become ready
  V3ListEnt<OrderMoveVertex *>
      m_readyVerticesE; // List of ready under domain/scope
public:
  // CONSTRUCTORS
  OrderMoveVertex(V3Graph *graphp, OrderLogicVertex *logicp)
      : V3GraphVertex{graphp}, m_logicp{logicp}, m_state{POM_WAIT},
        m_domScopep{nullptr} {}
...
public:
  MTaskMoveVertex(V3Graph *graphp, OrderLogicVertex *logicp,
                  const OrderEitherVertex *varp, const AstScope *scopep,
                  const AstSenTree *domainp)
      : V3GraphVertex{graphp}, m_logicp{logicp}, m_varp{varp}, m_scopep{scopep},
        m_domainp{domainp} {
    UASSERT(!(logicp && varp),
...
public:
  ...
  static bool followComboConnected(const V3GraphEdge *edgep) {
    ...
    if (!oedgep)
      v3fatalSrc("Following edge of non-OrderEdge type");
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::EDGE_COMBOCUT;
  }
...
class OrderPostCutEdge final : public OrderEdge {
  ...
  // Breakable if the output var feeds back to input combo logic or another
  // clock pin in which case we'll need a change detect loop around this var.
...
public:
  OrderPostCutEdge(V3Graph *graphp, V3GraphVertex *fromp, V3GraphVertex *top)
      : OrderEdge{graphp, fromp, top, WEIGHT_COMBO, CUTABLE} {}
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::EDGE_POSTCUT;
  }
...
public:
  ...
  virtual OrderVEdgeType type() const override {
    return OrderVEdgeType::EDGE_PRECUT;
  }
