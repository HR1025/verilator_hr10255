...
public:
  ...
  void emitClassMake() {
    ...
    V3OutMkFile of(v3Global.opt.makeDir() + "/" + v3Global.opt.prefix() +
                   "_classes.mk");
    of.putsHeader();
    of.puts("# DESCR"
    ...
    of.puts(v3Global.opt.trace() && v3Global.opt.traceFormat().fst() ? "1"
                                                                     : "0");
    of.puts("\n");
    of.puts("# Tracing threaded output mode?  0/1/N threads (from "
            "--trace-thread)\n");
    of.puts("VM_TRACE_THREADS = ");
    of.puts(cvtToStr(v3Global.opt.trueTraceThreads()));
    of.puts("\n");
    of.puts("# Separate FST writer thread? 0/1 (from --trace-fst with "
            "--trace-thread > 0)\n");
    of.puts("VM_TRACE_FST_WRITER_THREAD = ");
    of.puts(v3Global.opt.traceThreads() && v3Global.opt.traceFormat().fst()
                ? "1"
                : "0");
    of.puts("\n");
    ...
    for (int support = 0; support < 3; ++support) {
      for (const bool &slow : {false, true}) {
        ...
        of.puts(support == 2   ? "VM_GLOBAL"
                : support == 1 ? "VM_SUPPORT"
                               : "VM_CLASSES");
        of.puts(slow ? "_SLOW" : "_FAST");
        of.puts(" += \\\n");
        if (support == 2 && v3Global.opt.hierChild()) {
          // Do nothing because VM_GLOBAL is necessary per executable. Top
          // module will have them.
  ...
  void emitOverallMake() {
    ...
    of.puts(string("SYSTEMC_INCLUDE ?= ") + V3Options::getenvSYSTEMC_INCLUDE() +
            "\n");
    of.puts("# SystemC library directory with libsystemc.a (from "
            "$SYSTEMC_LIBDIR)\n");
    ...
    if (v3Global.opt.systemC() && !V3Options::systemCFound()) {
      v3fatal(
          ...
          "and need $SYSTEMC_LIBDIR in environment or when Verilator "
          "configured\n"
    ...
    of.puts("# Deprecated: SystemC architecture to find link library path "
            "(from $SYSTEMC_ARCH)\n");
    ...
    if (v3Global.opt.exe()) {
      ...
      for (const string &cppfile : cppFiles) {
        ...
        of.puts("\t$(OBJCACHE) $(CXX) $(CXXFLAGS) $(CPPFLAGS) $(OPT_FAST) -c "
                "-o $@ $<\n");
      }

      of.puts("\n### Link rules... (from --exe)\n");
      of.puts(v3Global.opt.exeName() + ": $(VK_USER_OBJS) $(VK_GLOBAL_OBJS) "
                                       "$(VM_PREFIX)__ALL.a $(VM_HIER_LIBS)\n");
      of.puts("\t$(LINK) $(LDFLAGS) $^ $(LOADLIBES) $(LDLIBS) $(LIBS) "
              "$(SC_LIBS) -o $@\n");
      of.puts("\n");
    }
    ...
    if (!v3Global.opt.protectLib().empty()) {
      ...
      // The rule to create .a is defined in verilated.mk, so just define
      // dependency here.
      of.puts(v3Global.opt.protectLibName(false) + ": " + protectLibDeps +
              "\n");
      of.puts("\n");
      if (v3Global.opt.hierChild()) {
        // Hierarchical child does not need .so because hierTop() will create
        // .so from .a
        ...
        // but some symbols that are referred as "DPI-C" can not be found at
        // this moment. So add dynamic_lookup
...
class EmitMkHierVerilation final {
  ...
  void emitCommonOpts(V3OutMkFile &of) const {
    ...
    const string verilator_wrapper =
        V3Os::filenameDir(fullpath_bin) + "/verilator";
  ...
  void emit(V3OutMkFile &of) const {
    ...
    of.puts("#\n");
    of.puts("# The main makefile " + v3Global.opt.prefix() +
            ".mk calls this makefile\n");
    of.puts("\n");
    ...
    // Build hierarchical libraries as soon as possible to get maximum
    // parallelism
    ...
    // Instead of direct execute of "cd $(VM_HIER_RUN_DIR) &&
    // $(VM_HIER_VERILATOR)", call via make to get message of "Entering
    // directory" and "Leaving directory". This will make some editors and IDEs
    // happy when viewing a logfile.
    of.puts("# VM_HIER_LAUNCH_VERILATOR_ARGSFILE must be passed as a command "
            "argument\n");
    ...
    // Top level module
    {
      const string argsFile =
          v3Global.hierPlanp()->topCommandArgsFileName(false);
      ...
      for (const auto &itr : *m_planp)
        of.puts(itr.second->hierWrapper(true) + " ");
      of.puts("\n");
    ...
    for (V3HierBlockPlan::const_iterator it = m_planp->begin();
         it != m_planp->end(); ++it) {
...
public:
  explicit EmitMkHierVerilation(const V3HierBlockPlan *planp)
      : m_planp{planp}, m_makefile{v3Global.opt.makeDir() + "/" +
                                   v3Global.opt.prefix() + "_hier.mk"} {
