...
constexpr int CDC_WEIGHT_ASYNC =
    0x1000; // Weight for edges that feed async logic
...
public:
  CdcEitherVertex(V3Graph *graphp, AstScope *scopep, AstNode *nodep)
      : V3GraphVertex{graphp}, m_scopep{scopep}, m_nodep{nodep},
        m_srcDomainSet{false}, m_dstDomainSet{false}, m_asyncPath{false} {}
...
public:
  CdcVarVertex(V3Graph *graphp, AstScope *scopep, AstVarScope *varScp)
      : CdcEitherVertex{graphp, scopep, varScp}, m_varScp{varScp} {}
...
public:
  CdcLogicVertex(V3Graph *graphp, AstScope *scopep, AstNode *nodep,
                 AstSenTree *sensenodep)
      : CdcEitherVertex{graphp, scopep, nodep}, m_hazard{false}, m_isFlop{
                                                                     false} {
...
private:
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    // Keeping line+filename lengths separate is much faster than calling
    // ascii().length()
...
private:
  ...
  uint32_t m_userGeneration =
      0;                   // Generation count to avoid slow userClearVertices
  ...
  CdcVarVertex *makeVarVertex(AstVarScope *varscp) {
    ...
    if (!vertexp) {
      ...
      if (varscp->varp()->isPrimaryIO()) {
        // Create IO vertex - note it's relative to the pointed to var, not
        // where we are now This allows reporting to easily print the input
        // statement
        CdcLogicVertex *ioVertexp = new CdcLogicVertex(
            &m_graph, varscp->scopep(), varscp->varp(), nullptr);
    ...
    if (m_inSenItem) {
      ...
      // TODO: In the future mark it here and do normal clock tree glitch checks
      // also
    } else if (varscp
                   ->user2()) { // It was detected in a sensitivity list earlier
  ...
  void warnAndFile(AstNode *nodep, V3ErrorCode code, const string &msg) {
    ...
    if (!told_file) {
      told_file = true;
      std::cerr << V3Error::msgPrefix() << "     See details in "
                << m_ofFilename << endl;
    }
    *m_ofp << "%Warning-" << code.ascii() << ": " << nodep->fileline() << " "
           << msg << '\n';
  }

  void setNodeHazard(AstNode *nodep) {
    // Need to not clear if warnings are off (rather than when report it)
    // as bypassing this warning may turn up another path that isn't warning
    // off'ed. We can't modifyWarnOff here, as one instantiation might not be an
    // issue until we find a hitting flop. Furthermore, a module like a "Or"
    // module would only get flagged once, even though the signals feeding it
    // are radically different.
    if (!m_domainp || m_domainp->hasCombo()) {
      // Source flop logic in a posedge block is OK for reset (not async though)
      if (m_logicVertexp &&
          !nodep->fileline()->warnIsOff(V3ErrorCode::CDCRSTLOGIC)) {
  ...
  void analyze() {
    ...
    if (debug() > 6)
      m_graph.dumpDotFilePrefixed("cdc_pre");
    //
  ...
  CdcEitherVertex *traceAsyncRecurse(CdcEitherVertex *vertexp, bool mark) {
    // First pass: Return vertex of any hazardous stuff attached, or nullptr if
    // OK If first pass returns true, second pass calls asyncPath() on
    // appropriate nodes
    if (vertexp->user() >= m_userGeneration)
      return nullptr; // Processed - prevent loop
    ...
    if (CdcLogicVertex *vvertexp = dynamic_cast<CdcLogicVertex *>(vertexp)) {
      ...
      if (vvertexp->varScp()->varp()->isPrimaryInish()) {
        ...
        for (V3GraphEdge *edgep = vertexp->inBeginp(); edgep;
             edgep = edgep->inNextp()) {
          CdcEitherVertex *eFromVertexp =
              static_cast<CdcEitherVertex *>(edgep->fromp());
          eFromVertexp->asyncPath(true);
        }
        return nullptr;
      }
      // Also ok if from flop, but partially trace the flop so more obvious to
      // users
    ...
    for (V3GraphEdge *edgep = vertexp->inBeginp(); edgep;
      ...
      if (submarkp && !mark_outp)
        mark_outp = submarkp;
    }

    if (mark)
      vertexp->asyncPath(true);
    return mark_outp;
  }
  ...
  void dumpAsync(CdcVarVertex *vertexp, CdcEitherVertex *markp) {
    ...
    *m_ofp << "\n";
    *m_ofp << "\n";
    CdcEitherVertex *targetp =
        vertexp; // One example destination flop (of possibly many)
    for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
      CdcEitherVertex *eToVertexp =
          static_cast<CdcEitherVertex *>(edgep->top());
      if (!eToVertexp)
        targetp = eToVertexp;
      if (CdcLogicVertex *vvertexp =
              dynamic_cast<CdcLogicVertex *>(eToVertexp)) {
        if (vvertexp->isFlop() // IE the target flop that is upsetting us
            && edgep->weight() >=
                   CDC_WEIGHT_ASYNC) { // And var feeds an async reset line
          targetp = eToVertexp;
    ...
    // UINFO(9," finalflop  "<<targetp->name()<<"
    // "<<targetp->nodep()->fileline()<<endl);
    warnAndFile(markp->nodep(), V3ErrorCode::CDCRSTLOGIC,
                "Logic in path that feeds async reset, via signal: " +
                    nodep->prettyNameQ());
  ...
  bool dumpAsyncRecurse(CdcEitherVertex *vertexp, const string &prefix,
    ...
    if (vertexp->user() >= m_userGeneration)
      return false; // Processed - prevent loop
    vertexp->user(m_userGeneration);
    if (!vertexp->asyncPath() && level != 0)
      return false; // Not part of path
    ...
    string front = pad(filelineWidth(), nodep->fileline()->ascii() + ":") +
                   " " + prefix + " +- ";
    if (VN_IS(nodep, VarScope)) {
      *m_ofp << front << "Variable: " << nodep->prettyName() << '\n';
    } else {
      V3EmitV::verilogPrefixedTree(nodep, *m_ofp, prefix + " +- ",
                                   filelineWidth(), vertexp->srcDomainp(),
                                   true);
      if (debug()) {
    ...
    nextsep = " | ";
    if (level)
      *m_ofp << V3OutFile::indentSpaces(filelineWidth()) << " " << prefix
             << nextsep << "\n";

    if (CdcLogicVertex *vvertexp = dynamic_cast<CdcLogicVertex *>(vertexp)) {
      // Now that we've printed a path with this hazard, don't bother to print
      // any more Otherwise, we'd get a path for almost every destination flop
  ...
  void edgeReport() {
    ...
    const string filename = v3Global.opt.makeDir() + "/" +
                            v3Global.opt.prefix() + "__cdc_edges.txt";
    ...
    for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
      ...
      if (CdcVarVertex *vvertexp = dynamic_cast<CdcVarVertex *>(itp)) {
        AstVar *varp = vvertexp->varScp()->varp();
        {
          string what = "wire";
          if (varp->isPrimaryIO())
            what = varp->direction().prettyName();

          std::ostringstream os;
          os.setf(std::ios::left);
          // Module name - doesn't work due to flattening having lost the
          // original so we assume the modulename matches the filebasename
          const string fname =
              vvertexp->varScp()->fileline()->filebasename() + ":";
  ...
  void edgeDomainRecurse(CdcEitherVertex *vertexp, bool traceDests, int level) {
    ...
    if (vertexp->user() >= m_userGeneration)
      return; // Mid-Processed - prevent loop
    vertexp->user(m_userGeneration);
    ...
    if (traceDests) {
      for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
        ...
        if (eToVertexp->dstDomainp())
          senouts.insert(eToVertexp->dstDomainp());
      }
    } else {
      for (V3GraphEdge *edgep = vertexp->inBeginp(); edgep;
           edgep = edgep->inNextp()) {
        ...
        if (eFromVertexp->srcDomainp())
          senouts.insert(eFromVertexp->srcDomainp());
      }
    }
    ...
    if (senedited)
      senoutp = VN_AS(V3Const::constifyExpensiveEdit(senoutp), SenTree);
    if (traceDests) {
      vertexp->dstDomainSet(
          true); // Note it's set - domainp may be null, so can't use that
    ...
    if (m_scopep) {
      ...
      if (nodep->access().isWriteOrRW()) {
        ...
        if (varvertexp->cntAsyncRst()) {
          // UINFO(9," edgeasync "<<varvertexp->name()<<" to
          // "<<m_logicVertexp<<endl);
          new V3GraphEdge(&m_graph, varvertexp, m_logicVertexp,
                          CDC_WEIGHT_ASYNC);
        } else {
          // UINFO(9," edgena    "<<varvertexp->name()<<" to
          // "<<m_logicVertexp<<endl);
...
public:
  ...
  explicit CdcVisitor(AstNode *nodep) {
    ...
    const string filename =
        v3Global.opt.makeDir() + "/" + v3Global.opt.prefix() + "__cdc.txt";
    m_ofp = V3File::new_ofstream(filename);
    if (m_ofp->fail())
      v3fatal("Can't write " << filename);
    m_ofFilename = filename;
    *m_ofp << "CDC Report for " << v3Global.opt.prefix() << '\n';
    *m_ofp << "Each dump below traces logic from inputs/source flops to "
              "destination flop(s).\n";
    *m_ofp << "First source logic is listed, then a variable that logic "
              "generates,\n";
    ...
    *m_ofp << "%% Indicates the operator considered potentially hazardous.\n";
    ...
    analyze();
    if (debug() >= 1)
      edgeReport(); // Not useful to users at the moment
    if (false) {
      *m_ofp << "\nDBG-test-dumper\n";
