...
private:
  ...
  // TYPES
  using ImplTypedefMap =
      std::map<const std::pair<void *, std::string>, AstTypedef *>;
  ...
  string nameFromTypedef(AstNode *nodep) {
    ...
    if (AstTypedef *typedefp = VN_CAST(nodep->backp(), Typedef)) {
      ...
      // This name is not guaranteed to be globally unique (due to later
      // parameterization)
      string above;
  ...
  bool nestedIfBegin(AstBegin *nodep) { // Point at begin inside the GenIf
    ...
    // The genblk name will get attached to the if true/false LOWER begin
    // block(s)
    //    1: GENIF
    ...
    return (nodep->implied() // User didn't provide begin/end
            && VN_IS(backp, GenIf) &&
            VN_CAST(backp, GenIf)->elsesp() == nodep &&
  ...
  virtual void visit(AstEnumItem *nodep) override {
    ...
    if (nodep->rangep()) {
      if (VL_UNCOVERABLE(
              !VN_IS(nodep->rangep()->leftp(), Const) // LCOV_EXCL_START
      ...
      for (int i = left; i != (right + increment);
           i += increment, offset_from_init++) {
        const string name = nodep->name() + cvtToStr(i);
        AstNode *valuep = nullptr;
        if (nodep->valuep()) {
          valuep =
              new AstAdd(nodep->fileline(), nodep->valuep()->cloneTree(true),
                         new AstConst(nodep->fileline(), AstConst::Unsized32(),
                                      offset_from_init));
        }
        AstNode *newp =
            new AstEnumItem(nodep->fileline(), name, nullptr, valuep);
        if (addp) {
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (VN_IS(nodep->subDTypep(), ParseTypeDType)) {
      ...
      if (!dtypep) {
        ...
        dtypep->unlinkFrBack();
        AstNode *newp =
            new AstParamTypeDType(nodep->fileline(), nodep->varType(),
    ...
    if (nodep->valuep()) {
      ...
      if (nodep->isParam() || (m_ftaskp && nodep->isNonOutput())) {
        // 1. Parameters and function inputs: It's a default to use if not
        // overridden
      } else if (!m_ftaskp && !VN_IS(m_modp, Class) && nodep->isNonOutput()) {
        nodep->v3warn(E_UNSUPPORTED,
                      "Unsupported: Default value on module input: "
                          << nodep->prettyNameQ());
        nodep->valuep()->unlinkFrBack()->deleteTree();
      } // 2. Under modules/class, it's an initial value to be loaded at time 0
        // via an AstInitial
      else if (m_valueModp) {
        // Making an AstAssign (vs AstAssignW) to a wire is an error, suppress
        // it
        FileLine *newfl = new FileLine(fl);
        newfl->warnOff(V3ErrorCode::PROCASSWIRE, true);
        auto *assp = new AstAssign(
            newfl, new AstVarRef(newfl, nodep->name(), VAccess::WRITE),
  ...
  virtual void visit(AstAttrOf *nodep) override {
    ...
    if (nodep->attrType() == AstAttrType::DT_PUBLIC) {
      ...
      if (!VN_IS(m_modp, Module)) {
        m_varp->v3warn(SPLITVAR, m_varp->prettyNameQ()
                                     << " has split_var metacomment, "
                                        "but will not be split because it is "
  ...
  virtual void visit(AstAlwaysPublic *nodep) override {
    // AlwaysPublic was attached under a var, but it's a statement that should
    // be at the same level as the var
    cleanFileline(nodep);
    iterateChildren(nodep);
    if (m_varp) {
      nodep->unlinkFrBack();
      ...
      nodep->addStmtp(new AstVarRef(nodep->fileline(), m_varp,
  ...
  virtual void visit(AstDefImplicitDType *nodep) override {
    ...
    if (it != m_implTypedef.end()) {
      ...
      // Definition must be inserted right after the variable (etc) that needed
      // it AstVar, AstTypedef, AstNodeFTask are common containers
      AstNode *backp = nodep->backp();
      for (; backp; backp = backp->backp()) {
        if (VN_IS(backp, Var) || VN_IS(backp, Typedef) ||
            VN_IS(backp, NodeFTask))
          break;
      }
      UASSERT_OBJ(
          backp, nodep,
      ...
      if (VN_IS(backp, Typedef)) {
        ...
        defp = new AstTypedef(nodep->fileline(), nodep->name(), nullptr,
                              VFlagChildDType(), dtypep);
        m_implTypedef.insert(std::make_pair(
            std::make_pair(nodep->containerp(), defp->name()), defp));
  ...
  virtual void visit(AstForeach *nodep) override {
    ...
    while (AstDot *dotp = VN_CAST(bracketp, Dot)) {
      bracketp = dotp->rhsp();
    }
    if (AstSelBit *selp = VN_CAST(bracketp, SelBit)) {
      ...
      nodep->v3error("Syntax error; foreach missing bracketed index variable "
                     "(IEEE 1800-2017 12.7.3)");
      VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
      return;
    }
    AstNode *arrayp =
        nodep->arrayp(); // Maybe different node since bracketp looked
    if (!VN_IS(arrayp, ParseRef) && !VN_IS(arrayp, Dot)) {
      ...
      arrayp->v3warn(E_UNSUPPORTED,
                     "Unsupported: foreach on non-simple variable reference");
      VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
      return;
    }
    // Split into for loop
    ...
    for (AstNode *varsp = lastVarsp; varsp; varsp = varsp->backp()) {
      ...
      AstNode *varp = new AstVar(fl, AstVarType::BLOCKTEMP, varsp->name(),
                                 nodep->findSigned32DType());
      // These will be the left and right dimensions and size of the array:
      AstNode *leftp =
          new AstAttrOf(fl, AstAttrType::DIM_LEFT, arrayp->cloneTree(false),
                        new AstConst(fl, dimension));
      AstNode *rightp =
          new AstAttrOf(fl, AstAttrType::DIM_RIGHT, arrayp->cloneTree(false),
                        new AstConst(fl, dimension));
      AstNode *sizep =
          new AstAttrOf(fl, AstAttrType::DIM_SIZE, arrayp->cloneTree(false),
                        new AstConst(fl, dimension));
      AstNode *stmtsp = varp;
      // Assign left-dimension into the loop var:
      stmtsp->addNext(new AstAssign(
          fl, new AstVarRef(fl, varp->name(), VAccess::WRITE), leftp));
      ...
      AstNode *comparep = new AstCond(
          ...
          new AstGte(fl, new AstVarRef(fl, varp->name(), VAccess::READ),
                     rightp));
      ...
      AstNode *incp = new AstAssign(
          ...
          new AstAdd(fl, new AstVarRef(fl, varp->name(), VAccess::READ),
                     new AstCond(fl, countupp, new AstConst(fl, 1),
                                 new AstConst(fl, -1))));
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    VL_RESTORER(m_lifetime);
    {
      ...
      if (m_modp && nodep->timeunit().isNone())
      ...
      if (m_lifetime.isNone()) {
        m_lifetime =
            VN_IS(nodep, Class) ? VLifetime::AUTOMATIC : VLifetime::STATIC;
      }
      iterateChildren(nodep);
    }
    m_valueModp = nodep;
  ...
  void visitIterateNoValueMod(AstNode *nodep) {
    // Iterate a node which shouldn't have any local variables moved to an
    // Initial
    cleanFileline(nodep);
    {
      VL_RESTORER(m_valueModp);
  ...
  virtual void visit(AstBegin *nodep) override {
    ...
    if (nodep->genforp()) {
      ++m_genblkNum;
      if (nodep->name() == "")
        nodep->name("genblk" + cvtToStr(m_genblkNum));
    }
  ...
  virtual void visit(AstGenIf *nodep) override {
    cleanFileline(nodep);
    bool nestedIf = (VN_IS(nodep->backp(), Begin) &&
                     nestedIfBegin(VN_CAST(nodep->backp(), Begin)));
    if (nestedIf) {
      iterateChildren(nodep);
    } else {
      ++m_genblkNum;
  ...
  virtual void visit(AstTimingControl *nodep) override {
    ...
    if (alwaysp && alwaysp->keyword() == VAlwaysKwd::ALWAYS_COMB) {
      ...
      if (nodep->stmtsp())
        alwaysp->addStmtp(nodep->stmtsp()->unlinkFrBackWithNext());
      VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
    }
  }
...
void V3LinkParse::linkParse(AstNetlist *rootp) {
  ...
  V3Global::dumpCheckGlobalTree("linkparse", 0,
