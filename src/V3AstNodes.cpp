...
// We need these here, because the classes they point to aren't defined when we
// declare the class
...
AstIface *AstIfaceRefDType::ifaceViaCellp() const {
  return ((m_cellp && m_cellp->modp()) ? VN_AS(m_cellp->modp(), Iface)
                                       : m_ifacep);
}
...
void AstNodeUOrStructDType::repairMemberCache() {
  ...
  for (AstMemberDType *itemp = membersp(); itemp;
    ...
    if (m_members.find(itemp->name()) != m_members.end()) {
      itemp->v3error(
          "Duplicate declaration of member name: " << itemp->prettyNameQ());
    } else {
...
const char *AstNodeUOrStructDType::broken() const {
  ...
  for (MemberNameMap::const_iterator it = m_members.begin();
       it != m_members.end(); ++it) {
...
AstExecGraph::AstExecGraph(FileLine *fileline)
    : ASTGEN_SUPER_ExecGraph(fileline), m_depGraphp{new V3Graph} {}

AstExecGraph::~AstExecGraph() {
  VL_DO_DANGLING(delete m_depGraphp, m_depGraphp);
}

AstNode *AstInsideRange::newAndFromInside(AstNode *exprp, AstNode *lhsp,
                                          AstNode *rhsp) {
...
AstNetlist::AstNetlist()
    : ASTGEN_SUPER_Netlist(new FileLine(FileLine::builtInFilename())),
      m_typeTablep{new AstTypeTable(fileline())}, m_constPoolp{new AstConstPool(
                                                      fileline())} {
...
bool AstVar::isSigPublic() const {
  return (m_sigPublic ||
          (v3Global.opt.allPublic() && !isTemp() && !isGenVar()));
}
...
bool AstVar::isScUint() const {
  return (
      (isSc() && v3Global.opt.pinsScUint() && width() >= 2 && width() <= 64) &&
      !isScBv());
}
bool AstVar::isScBigUint() const {
  return ((isSc() && v3Global.opt.pinsScBigUint() && width() >= 65 &&
           width() <= 512) &&
          !isScBv());
}
...
void AstVar::combineType(AstVarType type) {
  ...
  // when we combine wires cross-hierarchy we need a union of all
  // characteristics.
  m_varType = type;
  // These flags get combined with the existing settings of the flags.
  if (type == AstVarType::TRIWIRE || type == AstVarType::TRI0 ||
      type == AstVarType::TRI1) {
    m_tristate = true;
  }
...
string AstVar::vlArgType(bool named, bool forReturn, bool forFunc,
                         const string &namespc, bool asRef) const {
  UASSERT_OBJ(!forReturn, this,
              "Internal data is never passed as return, but as first argument");
  string ostatic;
  if (isStatic() && namespc.empty())
    ostatic = "static ";
  ...
  if (forFunc && isReadOnly() && isRef)
    ostatic = ostatic + "const ";
...
string AstVar::vlEnumType() const {
  ...
  const bool strtype =
      bdtypep && bdtypep->keyword() == AstBasicDTypeKwd::STRING;
...
string AstVar::vlEnumDir() const {
  ...
  if (AstBasicDType *bdtypep = basicp()) {
    if (bdtypep->keyword().isDpiCLayout())
      out += "|VLVF_DPI_CLAY";
  }
  return out;
}
...
string AstVar::cPubArgType(bool named, bool forReturn) const {
  ...
  if (isWide()) {
    ...
    if (!forReturn && (isWritable() || direction().isRefOrConstRef()))
      arg += "&";
    if (named)
      arg += " " + name();
  }
...
public:
  virtual string openArray(const AstVar *) const {
  ...
  virtual string primitive(const AstVar *varp) const {
    ...
    if (keyword.isDpiUnsignable() && !varp->basicp()->isSigned())
      type = "unsigned ";
...
string AstVar::dpiArgType(bool named, bool forReturn) const {
  if (forReturn) {
    ...
    class converter final : public dpiTypesToStringConverter {
      virtual string bitLogicVector(const AstVar *varp,
                                    bool isBit) const override {
...
string AstVar::dpiTmpVarType(const string &varName) const {
  class converter final : public dpiTypesToStringConverter {
    ...
    virtual string primitive(const AstVar *varp) const override {
      ...
      if (varp->isWritable() ||
          VN_IS(varp->dtypep()->skipRefp(), UnpackArrayDType)) {
        if (!varp->isWritable() &&
            varp->basicp()->keyword() == AstBasicDTypeKwd::CHANDLE)
          type = "const " + type;
      }
  ...
  public:
    explicit converter(const string &name) : m_name(name) {}
  };
...
string AstVar::scType() const {
  if (isScBigUint()) {
    ...
    return (string("sc_bv<") + cvtToStr(widthMin()) +
            "> "); // Keep the space so don't get >>
  } else if (widthMin() == 1) {
    return "bool";
  } else if (widthMin() <= VL_IDATASIZE) {
...
AstVar *AstVar::scVarRecurse(AstNode *nodep) {
  ...
  if (AstVar *anodep = VN_CAST(nodep, Var)) {
    ...
    if (nodep->op1p()) {
      if (AstVar *p = scVarRecurse(nodep->op1p()))
        return p;
    }
    if (nodep->op2p()) {
      if (AstVar *p = scVarRecurse(nodep->op2p()))
        return p;
    }
    if (nodep->op3p()) {
      if (AstVar *p = scVarRecurse(nodep->op3p()))
        return p;
    }
    if (nodep->op4p()) {
...
AstNodeDType::CTypeRecursed AstNodeDType::cTypeRecurse(bool compound) const {
  ...
  if (const auto *adtypep = VN_CAST(dtypep, AssocArrayDType)) {
    ...
    if (adtypep->boundp())
      info.m_type += ", " + cvtToStr(adtypep->boundConst() + 1);
    info.m_type += ">";
  } else if (const auto *adtypep = VN_CAST(dtypep, ClassRefDType)) {
    info.m_type =
        "VlClassRef<" + EmitCBaseVisitor::prefixNameProtect(adtypep) + ">";
  } else if (const auto *adtypep = VN_CAST(dtypep, UnpackArrayDType)) {
    if (adtypep->isCompound())
      compound = true;
    ...
    if (bdtypep->keyword() == AstBasicDTypeKwd::CHARPTR) {
      ...
      info.m_type = "VlMTaskVertex";
               ...
               8) { // Handle unpacked arrays; not bdtypep->width
  ...
  UASSERT_OBJ(!compound || info.m_dims.empty(), this,
              "Declaring C array inside compound type");
...
std::pair<uint32_t, uint32_t> AstNodeDType::dimensions(bool includeBasic) {
  ...
  for (AstNodeDType *dtypep = this; dtypep;) {
    ...
    if (const AstNodeArrayDType *adtypep = VN_CAST(dtypep, NodeArrayDType)) {
      ...
      if (includeBasic && (adtypep->isRanged() || adtypep->isString()))
        packed++;
...
void AstScope::cloneRelink() {
  if (m_aboveScopep && m_aboveScopep->clonep())
    m_aboveScopep->clonep();
  if (m_aboveCellp && m_aboveCellp->clonep())
    m_aboveCellp->clonep();
...
string AstScopeName::scopePrettyNameFormatter(AstText *scopeTextp) const {
  string out;
  for (AstText *textp = scopeTextp; textp;
       textp = VN_AS(textp->nextp(), Text)) {
    out += textp->text();
  }
  // TOP will be replaced by top->name()
  if (out.substr(0, 10) == "__DOT__TOP")
    out.replace(0, 10, "");
  if (out.substr(0, 7) == "__DOT__")
    out.replace(0, 7, "");
  if (out.substr(0, 1) == ".")
    out.replace(0, 1, "");
...
bool AstSenTree::hasClocked() const {
  ...
  for (AstSenItem *senp = sensesp(); senp;
...
AstTypeTable::AstTypeTable(FileLine *fl) : ASTGEN_SUPER_TypeTable(fl) {
  for (int i = 0; i < AstBasicDTypeKwd::_ENUM_MAX; ++i)
    m_basicps[i] = nullptr;
}

void AstTypeTable::clearCache() {
  // When we mass-change widthMin in V3WidthCommit, we need to correct the
  // table. Just clear out the maps; the search functions will be used to
  // rebuild the map
  for (auto &itr : m_basicps)
    itr = nullptr;
  m_detailedMap.clear();
...
void AstTypeTable::repairCache() {
  // After we mass-change widthMin in V3WidthCommit, we need to correct the
  // table.
  clearCache();
  for (AstNode *nodep = typesp(); nodep; nodep = nodep->nextp()) {
    if (AstBasicDType *bdtypep = VN_CAST(nodep, BasicDType)) {
...
AstQueueDType *AstTypeTable::findQueueIndexDType(FileLine *fl) {
  if (VL_UNLIKELY(!m_queueIndexp)) {
    AstQueueDType *newp =
        new AstQueueDType(fl, AstNode::findUInt32DType(), nullptr);
    addTypesp(newp);
    m_queueIndexp = newp;
  }
...
AstBasicDType *AstTypeTable::findBasicDType(FileLine *fl,
...
AstBasicDType *AstTypeTable::findLogicBitDType(FileLine *fl,
                                               AstBasicDTypeKwd kwd, int width,
...
AstBasicDType *AstTypeTable::findInsertSameDType(AstBasicDType *nodep) {
  ...
  nodep->generic(true);
  // No addTypesp; the upper function that called new() is responsible for
  // adding
  return nodep;
}

AstConstPool::AstConstPool(FileLine *fl)
    : ASTGEN_SUPER_ConstPool(fl), m_modp{new AstModule(fl, "@CONST-POOL@")},
...
AstVarScope *AstConstPool::createNewEntry(const string &name, AstNode *initp) {
  ...
  AstVar *const varp =
      new AstVar(fl, AstVarType::MODULETEMP, name, initp->dtypep());
  varp->isConst(true);
  varp->isStatic(true);
...
static bool sameInit(const AstInitArray *ap, const AstInitArray *bp) {
  ...
  // - the default/inititem children might be in different order yet still yield
  // the same table See note in AstInitArray::same about the same. This function
  // instead compares by initializer value, rather than by tree structure.
  const AstUnpackArrayDType *const aDTypep =
      VN_AS(ap->dtypep(), UnpackArrayDType);
  const AstUnpackArrayDType *const bDTypep =
      VN_AS(bp->dtypep(), UnpackArrayDType);
  if (!aDTypep->subDTypep()->sameTree(
          bDTypep->subDTypep())) { // Element types differ
    return false;
  }
  if (!aDTypep->rangep()->sameTree(bDTypep->rangep())) { // Ranges differ
    return false;
  }
  // Compare initializer arrays by value. Note this is only called when they
  // hash the same, so they likely run at most once per call to
  // 'AstConstPool::findTable'.
...
AstVarScope *AstConstPool::findTable(AstInitArray *initp) {
  ...
  for (AstNode *nodep = initp->initsp(); nodep; nodep = nodep->nextp()) {
    ...
    UASSERT_OBJ(VN_IS(valuep, Const), valuep,
                "Const pool table entry must be Const");
  }
  ...
  for (auto it = er.first; it != er.second; ++it) {
    ...
    const AstInitArray *const init2p =
        VN_AS(varScopep->varp()->valuep(), InitArray);
...
static bool sameInit(const AstConst *ap, const AstConst *bp) {
  // Similarly to the AstInitArray comparison, we ignore the dtype instance, so
  // long as they are compatible. For now, we assume the dtype is not relevant,
  // as we only call this from V3Prelim which is a late pass. Compare
  // initializers by value. This checks widths as well.
...
void AstWhile::addBeforeStmt(AstNode *newp, AstNode *belowp) {
  ...
  if (belowp == precondsp()) {
    // Must have been first statement in precondsp list, so newp is new first
    // statement
...
static std::string nodeAddr(const AstNode *nodep) {
  return v3Global.opt.dumpTreeAddrids() ? v3Global.ptrToId(nodep)
                                        : cvtToHex(nodep);
}
...
void AstNode::dump(std::ostream &str) const {
  ...
  if (user1p())
    str << " u1=" << nodeAddr(user1p());
  if (user2p())
    str << " u2=" << nodeAddr(user2p());
  if (user3p())
    str << " u3=" << nodeAddr(user3p());
  if (user4p())
    str << " u4=" << nodeAddr(user4p());
  if (user5p())
    str << " u5=" << nodeAddr(user5p());
  if (hasDType()) {
    ...
    if (dtypep())
      str << " %Error-dtype-exp=null,got=" << nodeAddr(dtypep());
  }
  if (name() != "") {
    if (VN_IS(this, Const)) {
...
void AstNodeUniop::dump(std::ostream &str) const {
...
void AstClass::insertCache(AstNode *nodep) {
  ...
  if (doit) {
    if (m_members.find(nodep->name()) != m_members.end()) {
      nodep->v3error(
          "Duplicate declaration of member name: " << nodep->prettyNameQ());
    } else {
...
bool AstClass::isClassExtendedFrom(const AstClass *refClassp,
                                   const AstClass *baseClassp) {
  // TAIL RECURSIVE
  if (!refClassp || !baseClassp)
    return false;
  if (refClassp == baseClassp)
    return true;
  if (!refClassp->extendsp())
    return false;
...
void AstInitArray::dump(std::ostream &str) const {
  ...
  for (const auto &itr : mapr) {
    ...
    str << " [" << itr.first
        << "]=" << reinterpret_cast<const void *>(itr.second);
  }
}
...
void AstNodeTermop::dump(std::ostream &str) const {
  this->AstNodeMath::dump(str);
}
...
void AstNodeDType::dumpSmall(std::ostream &str) const {
  str << "(" << (generic() ? "G/" : "")
      << ((isSigned() && !isDouble()) ? "s" : "") << (isNosign() ? "n" : "")
      << (isDouble() ? "d" : "") << (isString() ? "str" : "");
...
void AstNodeTriop::dump(std::ostream &str) const {
  this->AstNodeMath::dump(str);
}
...
void AstNodeVarRef::dump(std::ostream &str) const {
  this->AstNodeMath::dump(str);
  if (classOrPackagep())
    str << " pkg=" << nodeAddr(classOrPackagep());
  str << " " << access().arrow() << " ";
}
...
void AstVar::dump(std::ostream &str) const {
  ...
  if (isSc())
    str << " [SC]";
  if (isPrimaryIO())
    str << (isInoutish() ? " [PIO]" : (isWritable() ? " [PO]" : " [PI]"));
  if (isIO())
    str << " " << direction().ascii();
  if (isConst())
    str << " [CONST]";
  ...
  if (isDpiOpenArray())
    str << " [DPIOPENA]";
  if (!attrClocker().unknown())
    str << " [" << attrClocker().ascii() << "] ";
  if (!lifetime().isNone())
    str << " [" << lifetime().ascii() << "] ";
  str << " " << varType();
}
...
void AstClassOrPackageRef::dump(std::ostream &str) const {
  ...
  if (classOrPackageNodep())
    str << " cpkg=" << nodeAddr(classOrPackageNodep());
  str << " -> ";
  if (classOrPackageNodep()) {
...
void AstNodeFTask::dump(std::ostream &str) const {
  ...
  if (dpiOpenParent())
    str << " [DPIOPENPARENT]";
  if ((dpiImport() || dpiExport()) && cname() != name())
    str << " [c=" << cname() << "]";
}
...
void AstCFunc::dump(std::ostream &str) const {
  ...
  if (isStatic())
    str << " [STATIC]";
  if (dpiExportDispatcher())
    str << " [DPIED]";
  if (dpiExportImpl())
    str << " [DPIEI]";
  if (dpiImportPrototype())
    str << " [DPIIP]";
  if (dpiImportWrapper())
    str << " [DPIIW]";
  if (dpiContext())
