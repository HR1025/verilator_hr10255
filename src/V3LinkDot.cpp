...
//                     ^--- varref(input_ifref).modport ->
//                     IfaceDTypeRef(iface,modport)
//      FindVisitor:
...
//          #7: Record sym of IfaceRefDType and aliased interface and/or modport
//          (after #4,#5)
//      insertAllScopeAliases():
//          #8: Insert modport's symbols under IfaceRefDType (after #7)
//      ResolveVisitor:
//          #9: Resolve general variables, which may point into the interface or
//          modport (after #8)
//      LinkResolve:
...
private:
  // NODE STATE
  // Cleared on Netlist
  //  AstNodeModule::user1p()         // VSymEnt*.      Last symbol created for
  //  this node AstNodeModule::user2()          // bool.          Currently
  //  processing for recursion check
  //  ...  Note maybe more than one, as can be multiple hierarchy places
  //  AstVarScope::user2p()           // AstVarScope*.  Base alias for AstInline
  //  of this signal AstVar::user2p()                // AstFTask*.     If a
  //  function variable, the task
  //                                                    that links to the
  //                                                    variable
  //  AstVar::user4()                 // bool.          True if port set for
  //  this variable AstNodeBlock::user4()           // bool.          Did name
  //  processing AstNodeModule::user4()          // bool.          Live module
...
private:
  ...
  std::set<std::pair<AstNodeModule *, std::string>>
      m_implicitNameSet; // For [module][signalname] if we can implicitly create
                         // it
  std::array<ScopeAliasMap, SAMN__MAX>
      m_scopeAliasMap; // Map of <lhs,rhs> aliases
  std::vector<VSymEnt *>
      m_ifaceVarSyms;          // List of AstIfaceRefDType's to be imported
...
public:
  ...
  void preErrorDump() {
    ...
    if (!diddump && v3Global.opt.dumpTree()) {
      ...
      v3Global.rootp()->dumpTreeFile(
          v3Global.debugFilename("linkdot-preerr.tree"));
    }
  }

  // CONSTRUCTORS
  LinkDotState(AstNetlist *rootp, VLinkDotStep step) : m_syms{rootp} {
  ...
  void checkDuplicate(VSymEnt *lookupSymp, AstNode *nodep, const string &name) {
    ...
    //
    // Note we only check for conflicts at the same level; it's ok if one block
    // hides another We also wouldn't want to not insert it even though it's
    // lower down
  ...
  void insertDUnit(AstNetlist *nodep) {
    ...
    symp->parentp(
        rootEntp()); // Needed so backward search can find name of top module
    symp->fallbackp(nullptr);
    rootEntp()->insert("$unit ",
                       symp); // Space so can never name conflict with user code
    //
  ...
  VSymEnt *insertTopCell(AstNodeModule *nodep, const string &scopename) {
    ...
    VSymEnt *symp = new VSymEnt(&m_syms, nodep);
    UINFO(9, "      INSERTtop se" << cvtToHex(symp) << "  " << scopename << " "
                                  << nodep << endl);
    symp->parentp(
        rootEntp()); // Needed so backward search can find name of top module
    ...
    if (forScopeCreation())
      m_nameScopeSymMap.emplace(scopename, symp);
    return symp;
  }
  VSymEnt *insertCell(VSymEnt *abovep, VSymEnt *modSymp, AstCell *nodep,
    ...
    UINFO(9, "      INSERTcel se" << cvtToHex(symp) << "  " << scopename
                                  ...
                                  << " mods=se" << cvtToHex(modSymp)
                                  << " node=" << nodep << endl);
    symp->parentp(abovep);
    symp->fallbackp(dunitEntp()); // Needed so can find $unit stuff
    nodep->user1p(symp);
    if (nodep->modp())
      nodep->modp()->user1p(symp);
    checkDuplicate(abovep, nodep, nodep->origName());
    abovep->reinsert(nodep->origName(), symp);
    if (forScopeCreation() && abovep != modSymp &&
        !modSymp->findIdFlat(nodep->name())) {
      // If it's foo_DOT_bar, we need to be able to find it under "foo_DOT_bar"
      // too. Duplicates are possible, as until resolve generates might have 2
      // same cells under an if
  ...
  VSymEnt *insertInline(VSymEnt *abovep, VSymEnt *modSymp, AstCellInline *nodep,
    ...
    // This references to another Sym, and eventually resolves to a module with
    // a prefix
    UASSERT_OBJ(abovep, nodep, "Null symbol table inserting node");
    VSymEnt *symp = new VSymEnt(&m_syms, nodep);
    UINFO(9, "      INSERTinl se" << cvtToHex(symp) << "  " << basename
                                  << " above=se" << cvtToHex(abovep)
                                  << " mods=se" << cvtToHex(modSymp)
                                  << " node=" << nodep << endl);
    symp->parentp(abovep);
  ...
  VSymEnt *insertBlock(VSymEnt *abovep, const string &name, AstNode *nodep,
    ...
    // A fake point in the hierarchy, corresponding to a begin or function/task
    // block After we remove begins these will go away Note we fallback to the
    // symbol table of the parent, as we want to find variables there However,
    // cells walk the graph, so cells will appear under the begin/ftask itself
    UASSERT_OBJ(abovep, nodep, "Null symbol table inserting node");
    VSymEnt *symp = new VSymEnt(&m_syms, nodep);
    UINFO(9, "      INSERTblk se"
                 << cvtToHex(symp) << "  above=se" << cvtToHex(abovep)
    ...
    // Duplicates are possible, as until resolve generates might have 2 same
    // cells under an if
  ...
  void implicitOkAdd(AstNodeModule *nodep, const string &varname) {
    ...
    if (nodep) {
      ...
      if (it == m_implicitNameSet.end())
        m_implicitNameSet.emplace(nodep, varname);
    }
  }
  bool implicitOk(AstNodeModule *nodep, const string &varname) {
    return nodep && (m_implicitNameSet.find(std::make_pair(nodep, varname)) !=
  ...
  void computeIfaceVarSyms() {
    for (VSymEnt *varSymp : m_ifaceVarSyms) {
      ...
      if (!ifacerefp->ifaceViaCellp()) {
        if (!ifacerefp->cellp()) { // Probably a NotFoundModule, or a normal
                                   // module if made mistake
          UINFO(1, "Associated cell "
                       << AstNode::prettyNameQ(ifacerefp->cellName()) << endl);
      ...
      if (ifacerefp->isModport()) {
        ...
        if (!ok) {
          const string suggest = suggestSymFallback(
      ...
      // Alias won't expand until interfaces and modport names are known; see
      // notes at top
  ...
  void insertScopeAlias(SAMNum samn, VSymEnt *lhsp, VSymEnt *rhsp) {
    ...
    UINFO(9, "   insertScopeAlias se" << cvtToHex(lhsp) << " se"
                                      << cvtToHex(rhsp) << endl);
    UASSERT_OBJ(!(VN_IS(rhsp->nodep(), Cell) &&
                  !VN_IS(VN_AS(rhsp->nodep(), Cell)->modp(), Iface)),
  ...
  void computeScopeAliases() {
    ...
    for (int samn = 0; samn < SAMN__MAX; ++samn) {
      for (ScopeAliasMap::iterator it = m_scopeAliasMap[samn].begin();
        ...
        UINFO(9, "  iiasa: Insert alias se"
                     << lhsp << " (" << lhsp->nodep()->typeName() << ") <- se"
                     << srcp << " " << srcp->nodep() << endl);
        // srcp should be an interface reference pointing to the interface we
        // want to import
      ...
      // m_scopeAliasMap[samn].clear();  // Done with it, but put into debug
      // file
    }
  }

private:
  VSymEnt *findWithAltFallback(VSymEnt *symp, const string &name,
...
public:
  VSymEnt *findDotted(FileLine *refLocationp, VSymEnt *lookupSymp,
                      const string &dotname, string &baddot, VSymEnt *&okSymp) {
    ...
    while (leftname != "") { // foreach dotted part of xref name
      ...
      UINFO(8, "         id " << ident << " alt " << altIdent << " left "
                              << leftname << " at se" << lookupSymp << endl);
      ...
      if (firstId) {
        ...
        AstCellInline *inlinep = lookupSymp
          ...
          if (AstNodeModule *modp = cellp->modp()) {
            if (modp->hierBlock()) {
              ...
              refLocationp->v3error(
                  "Dotted reference to instance that refers to "
  ...
  VSymEnt *findSymPrefixed(VSymEnt *lookupSymp, const string &dotname,
                           string &baddot) {
    ...
    UINFO(8, "    findSymPrefixed "
                 ...
                 << ((lookupSymp->symPrefix() == "")
                         ? ""
                         : lookupSymp->symPrefix() + dotname)
...
class LinkDotFindVisitor final : public AstNVisitor {
  // STATE
  LinkDotState
      *m_statep; // State to pass between visitors, including symbol table
  AstNodeModule *m_classOrPackagep = nullptr; // Current package
  VSymEnt *m_modSymp = nullptr;               // Symbol Entry for current module
  VSymEnt *m_curSymp =
      nullptr;    // Symbol Entry for current table, where to lookup/insert
  ...
  void makeImplicitNew(AstClass *nodep) {
    ...
    UINFO(8,
          "Made implicit new for " << nodep->name() << ": " << nodep << endl);
  ...
  virtual void visit(AstNodeModule *nodep) override {
    // Called on top module from Netlist, other modules from the cell creating
    // them, and packages
    UINFO(8, "   " << nodep << endl);
    ...
    if (doit && nodep->user2()) {
      ...
      UASSERT_OBJ(!nodep->dead(), nodep,
                  "Module in instance tree mislabeled as dead?");
  ...
  virtual void visit(AstClass *nodep) override {
    ...
    VL_RESTORER(m_modWithNum);
    {
      ...
      nodep->user4(true);
      // Implicit new needed?
      if (!m_explicitNew && m_statep->forPrimary())
        makeImplicitNew(nodep);
    }
  }
  ...
  virtual void visit(AstCell *nodep) override {
    ...
    if (nodep->recursive() && m_inRecursion)
      return;
    iterateChildren(nodep);
    ...
    if ((pos = origname.rfind('.')) != string::npos) {
      ...
      string baddot;
      VSymEnt *okSymp;
      aboveSymp = m_statep->findDotted(nodep->fileline(), aboveSymp, scope,
                                       baddot, okSymp);
    ...
    }
    {
      m_scope = m_scope + "." + nodep->name();
      m_curSymp = m_modSymp =
          m_statep->insertCell(aboveSymp, m_modSymp, nodep, m_scope);
      m_blockp = nullptr;
  ...
  virtual void visit(AstCellInline *nodep) override {
    ...
    VSymEnt *aboveSymp = m_curSymp;
    // If baz__DOT__foo__DOT__bar, we need to find baz__DOT__foo and add bar to
    // it.
    ...
    if ((pos = dottedname.rfind("__DOT__")) != string::npos) {
      ...
      string baddot;
      VSymEnt *okSymp;
      aboveSymp = m_statep->findDotted(nodep->fileline(), aboveSymp, dotted,
                                       baddot, okSymp);
      UASSERT_OBJ(aboveSymp, nodep,
  ...
  virtual void visit(AstNodeBlock *nodep) override {
    ...
    if (nodep->name() == "") {
      ...
      {
        m_blockp = nodep;
        m_curSymp = m_statep->insertBlock(m_curSymp, nodep->name(), nodep,
                                          m_classOrPackagep);
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    UINFO(5, "   " << nodep << endl);
    UASSERT_OBJ(m_curSymp && m_modSymp, nodep,
                "Function/Task not under module?");
    if (nodep->name() == "new")
      m_explicitNew = true;
    ...
    VSymEnt *upSymp = m_curSymp;
    {
      ...
      const string name =
          string{nodep->isExternProto() ? "extern " : ""} + nodep->name();
      m_curSymp =
          m_statep->insertBlock(m_curSymp, name, nodep, m_classOrPackagep);
      ...
      if (!nodep->isExternProto() && nodep->fvarp() &&
        ...
        if (dtypep) {
          ...
          dtypep =
              new AstBasicDType(nodep->fileline(), AstBasicDTypeKwd::LOGIC);
        }
        AstVar *newvarp =
        ...
        // Explicit insert required, as the var name shadows the upper level's
        // task name
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (nodep->isFuncLocal() && nodep->lifetime().isStatic()) {
      nodep->v3warn(E_UNSUPPORTED,
                    "Unsupported: 'static' function/task variables");
    ...
    if (!m_statep->forScopeCreation()) {
      ...
      if (!foundp) {
        ...
        if (foundp->parentp() == m_curSymp // Only when on same level
          ...
          if (!(m_ftaskp && m_ftaskp->dpiImport()) &&
              (!m_ftaskp ||
               m_ftaskp != foundp->nodep()) // Not the function's
      ...
      if (ins) {
        if (m_statep->forPrimary() && nodep->isGParam() &&
          ...
          // replace it with a new value. It will keep the same FileLine
          // information.
        ...
        VSymEnt *insp = m_statep->insertSym(m_curSymp, nodep->name(), nodep,
                                            m_classOrPackagep);
        if (m_statep->forPrimary() && nodep->isGParam()) {
          ++m_paramNum;
          VSymEnt *symp = m_statep->insertSym(
              m_curSymp, "__paramNumber" + cvtToStr(m_paramNum), nodep,
              m_classOrPackagep);
          symp->exported(false);
        }
  ...
  virtual void visit(AstTypedefFwd *nodep) override {
    UASSERT_OBJ(m_curSymp, nodep, "Typedef not under module/package/$unit");
    iterateChildren(nodep);
    // No need to insert, only the real typedef matters, but need to track for
    // errors
    nodep->user1p(m_curSymp);
  }
  virtual void visit(AstParamTypeDType *nodep) override {
    UASSERT_OBJ(m_curSymp, nodep,
                "Parameter type not under module/package/$unit");
    iterateChildren(nodep);
    m_statep->insertSym(m_curSymp, nodep->name(), nodep, m_classOrPackagep);
    if (m_statep->forPrimary() && nodep->isGParam()) {
      ++m_paramNum;
  ...
  virtual void visit(AstCFunc *nodep) override {
    // For dotted resolution, ignore all AstVars under functions, otherwise
    // shouldn't exist
    UASSERT_OBJ(!m_statep->forScopeCreation(), nodep,
                "No CFuncs expected in tree yet");
  }
    ...
    if (nodep->name() == "*") {
      ...
      if (!impp) {
        nodep->v3error("Import object not found: '"
                       << nodep->packagep()->prettyName()
                       << "::" << nodep->prettyName() << "'");
      }
    }
    m_curSymp->importFromPackage(m_statep->symsp(), srcp, nodep->name());
    UINFO(9, "    Link Done: " << nodep << endl);
    // No longer needed, but can't delete until any multi-instantiated modules
    // are expanded
  }
  ...
  virtual void visit(AstWithParse *nodep) override {
    ...
    if (const auto argp = VN_CAST(funcrefp->pinsp(), Arg)) {
      ...
      if (argp->nextp())
        argp->nextp()->v3error(
            "'with' function expects only up to one argument");
    ...
    if (nodep->exprp()) { // Else empty expression and pretend no "with"
      ...
      const auto newp =
          new AstWith(nodep->fileline(), indexArgRefp, valueArgRefp,
  ...
  virtual void visit(AstWith *nodep) override {
    ...
    VSymEnt *const oldCurSymp = m_curSymp;
    {
      ++m_modWithNum;
      m_curSymp =
          m_statep->insertBlock(m_curSymp, "__Vwith" + cvtToStr(m_modWithNum),
                                nodep, m_classOrPackagep);
      m_curSymp->fallbackp(oldCurSymp);
      ...
      m_statep->insertSym(m_curSymp, nodep->valueArgRefp()->name(),
                          nodep->valueArgRefp(), nullptr);
    }
  }
...
private:
  ...
  void pinImplicitExprRecurse(AstNode *nodep) {
    // Under a pin, Check interconnect expression for a pin reference or a
    // concat. Create implicit variable as needed
    ...
    // perhaps shouldn't create an implicit variable.  But, we've warned...
      ...
      if (nodep->op1p())
        pinImplicitExprRecurse(nodep->op1p());
      if (nodep->op2p())
        pinImplicitExprRecurse(nodep->op2p());
      if (nodep->op3p())
        pinImplicitExprRecurse(nodep->op3p());
      if (nodep->op4p())
        pinImplicitExprRecurse(nodep->op4p());
      if (nodep->nextp())
        pinImplicitExprRecurse(nodep->nextp());
    }
  }
  ...
  virtual void visit(AstPin *nodep) override {
    ...
    if (nodep->exprp()             // Else specifically unconnected pin
        && !nodep->svImplicit()) { // SV 19.11.3: .name pins don't allow
                                   // implicit decls
  ...
  virtual void visit(AstPort *nodep) override {
    ...
    if (!refp) {
      ...
      nodep->v3error(
          "Pin is not an in/out/inout/interface: " << nodep->prettyNameQ());
    } else {
      if (refp->user4()) {
      ...
      refp->user4(true);
      VSymEnt *symp =
          m_statep->insertSym(m_statep->getNodeSym(m_modp),
  ...
  virtual void visit(AstAssignAlias *nodep) override {
    ...
    UASSERT_OBJ(!m_statep->forPrimary(), nodep,
                "Assign aliases unexpected pre-dot");
  ...
  virtual void visit(AstClassOrPackageRef *nodep) override {
    if (auto *fwdp = VN_CAST(nodep->classOrPackageNodep(), TypedefFwd)) {
      ...
      VSymEnt *foundp =
          m_statep->getNodeSym(fwdp)->findIdFallback(fwdp->name());
      if (foundp &&
          (VN_IS(foundp->nodep(), Class) || VN_IS(foundp->nodep(), Package))) {
        nodep->classOrPackagep(VN_AS(foundp->nodep(), NodeModule));
      } else if (foundp && VN_IS(foundp->nodep(), ParamTypeDType)) {
        UASSERT(m_statep->forPrimary(),
                "Param types should have been resolved");
        nodep->classOrPackageNodep(VN_AS(foundp->nodep(), ParamTypeDType));
      } else {
        if (foundp)
          UINFO(1, "found nodep = " << foundp->nodep() << endl);
        nodep->v3error("Forward typedef used as class/package does not resolve "
                       "to class/package: "
                       << nodep->prettyNameQ() << '\n'
                       << nodep->warnContextPrimary() << '\n'
  ...
  virtual void visit(AstTypedefFwd *nodep) override {
    VSymEnt *foundp =
        m_statep->getNodeSym(nodep)->findIdFallback(nodep->name());
    if (!foundp && v3Global.opt.pedantic()) {
      ...
      nodep->v3error("Forward typedef unused or does not resolve to a data "
                     "type (IEEE 1800-2017 6.18): "
                     << nodep->prettyNameQ());
    }
...
class LinkDotScopeVisitor final : public AstNVisitor {
  ...
  virtual void visit(AstScope *nodep) override {
    ...
    UASSERT_OBJ(m_statep->forScopeCreation(), nodep,
                "Scopes should only exist right after V3Scope");
    // Using the CELL names, we created all hierarchy.  We now need to match
    // this Scope up with the hierarchy created by the CELL names.
  ...
  virtual void visit(AstVarScope *nodep) override {
    if (!nodep->varp()->isFuncLocal() && !nodep->varp()->isClassMember()) {
      ...
      if (nodep->varp()->isIfaceRef() && nodep->varp()->isIfaceParent()) {
        UINFO(9, "Iface parent ref var " << nodep->varp()->name() << " "
                                         << nodep << endl);
        ...
        UASSERT_OBJ(cellSymp, nodep,
                    "No symbol for interface instance: "
                        << nodep->prettyNameQ(ifcellname));
        UINFO(5, "       Found interface instance: se"
                     << cvtToHex(cellSymp) << " " << cellSymp->nodep() << endl);
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    VSymEnt *symp =
        m_statep->insertBlock(m_modSymp, nodep->name(), nodep, nullptr);
  ...
  virtual void visit(AstAssignVarScope *nodep) override {
    ...
    VSymEnt *rhsSymp;
    {
      ...
      while (!symp) {
        scopename = refp ? refp->name()
                         : (inl.size() ? (inl + xrefp->name()) : xrefp->name());
        string baddot;
        VSymEnt *okSymp;
        symp = m_statep->findDotted(nodep->rhsp()->fileline(), m_modSymp,
      ...
      if (!symp) {
        UINFO(9, "No symbol for interface alias rhs ("
                     << string(refp ? "VARREF " : "VARXREF ") << scopename
                     << ")" << endl);
      }
    ...
    VSymEnt *lhsSymp;
    {
      ...
      string baddot;
      VSymEnt *okSymp;
      VSymEnt *symp = m_statep->findDotted(nodep->lhsp()->fileline(), m_modSymp,
                                           scopename, baddot, okSymp);
      UASSERT_OBJ(symp, nodep, "No symbol for interface alias lhs");
      UINFO(5, "       Found a linked scope LHS: " << scopename << "  se"
                                                   << cvtToHex(symp) << " "
                                                   << symp->nodep() << endl);
      lhsSymp = symp;
    }
    // Remember the alias - can't do it yet because we may have additional
    // symbols to be added, or maybe an alias of an alias
...
class LinkDotIfaceVisitor final : public AstNVisitor {
  ...
  virtual void visit(AstModport *nodep) override {
    ...
    VSymEnt *const oldCurSymp = m_curSymp;
    {
      ...
      m_curSymp =
          m_statep->insertBlock(m_curSymp, nodep->name(), nodep, nullptr);
      m_curSymp->fallbackp(oldCurSymp);
      iterateChildren(nodep);
    }
  }
  virtual void visit(AstModportFTaskRef *nodep) override {
    ...
    iterateChildren(nodep);
    if (nodep->isExport())
      nodep->v3warn(E_UNSUPPORTED, "Unsupported: modport export");
    ...
    if (!symp) {
      ...
      // Make symbol under modport that points at the _interface_'s var, not the
      // modport.
      nodep->ftaskp(ftaskp);
      VSymEnt *subSymp = m_statep->insertSym(m_curSymp, nodep->name(), ftaskp,
                                             nullptr /*package*/);
      m_statep->insertScopeAlias(LinkDotState::SAMN_MODPORT, subSymp, symp);
    } else {
      nodep->v3error(
          "Modport item is not a function/task: " << nodep->prettyNameQ());
    }
    if (m_statep->forScopeCreation()) {
  ...
  virtual void visit(AstModportVarRef *nodep) override {
    ...
    if (!symp) {
      ...
      // Make symbol under modport that points at the _interface_'s var via the
      // modport. (Need modport still to test input/output markings)
      nodep->varp(varp);
      ...
      nodep->v3error(
          "Modport item is not a variable: " << nodep->prettyNameQ());
    }
    if (m_statep->forScopeCreation()) {
...
private:
  ...
  struct DotStates {
    ...
    string ascii() const {
      static const char *const names[] = {"NONE", "PACKAGE", "SCOPE", "FINAL",
                                          "MEMBER"};
  ...
  void createImplicitVar(VSymEnt *lookupSymp, AstVarRef *nodep,
                         AstNodeModule *modp, VSymEnt *moduleSymp,
                         bool noWarn) {
    ...
    if (!nodep->varp()) {
      if (!noWarn) {
        if (nodep->fileline()->warnIsOff(V3ErrorCode::I_DEF_NETTYPE_WIRE)) {
          ...
          nodep->v3error(
              "Signal definition not found, and implicit disabled with "
              "`default_nettype: "
              << nodep->prettyNameQ() << '\n'
      ...
      m_statep->insertSym(moduleSymp, newp->name(), newp,
                          nullptr /*classOrPackagep*/);
    }
  }
  AstVar *foundToVarp(const VSymEnt *symp, AstNode *nodep, VAccess access) {
    ...
    if (!symp) {
      ...
      } // else other simulators don't warn about reading, and IEEE doesn't say
        // illegal
      return varp;
    } else {
      return nullptr;
  ...
  void taskFuncSwapCheck(AstNodeFTaskRef *nodep) {
    if (nodep->taskp() && VN_IS(nodep->taskp(), Task) &&
        VN_IS(nodep, FuncRef)) {
      nodep->v3error(
          "Illegal call of a task as a function: " << nodep->prettyNameQ());
    }
  }
  ...
  AstVar *findIfaceTopVarp(AstNode *nodep, VSymEnt *parentEntp,
                           const string &name) {
    ...
    UASSERT_OBJ(ifaceTopVarp, nodep,
  ...
  virtual void visit(AstCell *nodep) override {
    ...
    VL_RESTORER(m_pinSymp);
    {
      ...
      // Need to pass the module info to this cell, so we can link up the pin
      // names However can't use m_curSymp as pin connections need to use the
  ...
  virtual void visit(AstPin *nodep) override {
    ...
    if (!nodep->modVarp()) {
      ...
      } else if (AstParamTypeDType *refp =
  ...
  virtual void visit(AstDot *nodep) override {
    ...
    const bool start =
        (m_ds.m_dotPos == DP_NONE); // Save, as m_dotp will be changed
    {
      if (start) { // Starting dot sequence
        if (debug() >= 9)
          nodep->dumpTree("-dot-in: ");
      ...
      if (VN_IS(nodep->lhsp(), ParseRef) && nodep->lhsp()->name() == "this") {
      ...
      } else if (VN_IS(nodep->lhsp(), ParseRef) &&
                 nodep->lhsp()->name() == "super") {
        VSymEnt *classSymp = m_ds.m_dotSymp;
        do {
          classSymp = classSymp->parentp();
        } while (classSymp && !VN_IS(classSymp->nodep(), Class));
        if (!classSymp) {
          ...
          if (!classp->extendsp()) {
            nodep->v3error(
                "'super' used on non-extended class (IEEE 1800-2017 8.15)");
            m_ds.m_dotErr = true;
          } else {
        ...
        // if (!start) { nodep->lhsp()->v3error("Package reference may not be
        // embedded in dotted reference"); m_ds.m_dotErr=true; }
      ...
      if (VN_IS(nodep->lhsp(), LambdaArgRef))
        m_ds.m_unlinkedScopep = nodep->lhsp();
      if (!m_ds.m_dotErr) { // Once something wrong, give up
        ...
        if (start && m_ds.m_dotPos == DP_SCOPE)
  ...
  virtual void visit(AstParseRef *nodep) override {
    ...
    // Generally resolved during Primay, but might be at param time under
    // AstUnlinkedRef
    ...
    if (m_ds.m_dotPos == DP_FINAL &&
        VN_IS(m_ds.m_unlinkedScopep, LambdaArgRef) &&
        nodep->name() == "index") {
      // 'with' statement's 'item.index'
      iterateChildren(nodep);
      const auto newp = new AstLambdaArgRef(
          nodep->fileline(), m_ds.m_unlinkedScopep->name() + "__DOT__index",
          true);
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
      return;
    } else if (m_ds.m_dotPos == DP_MEMBER) {
      // Found a Var, everything following is membership.  {scope}.{var}.HERE
      // {member}
      ...
      if (m_ds.m_dotPos == DP_PACKAGE) {
        ...
        allowScope = true;
        allowVar = true;
        UASSERT_OBJ(VN_IS(m_ds.m_dotp->lhsp(), ClassOrPackageRef),
      ...
      if (allowScope) {
        foundp = m_statep->findDotted(nodep->fileline(), m_ds.m_dotSymp,
                                      nodep->name(), baddot,
      ...
      if (!foundp) {
        ...
        if (allowScope) {
          ...
          if (VN_IS(cellp->modp(), Iface)) {
            ...
            VSymEnt *parentEntp =
                cellEntp->parentp(); // Container of the var; probably a module
                                     // or generate begin
            AstVar *ifaceRefVarp =
            ...
            AstNode *newp = new AstVarRef(ifaceRefVarp->fileline(),
                                          ifaceRefVarp, VAccess::READ);
            nodep->replaceWith(newp);
            VL_DO_DANGLING(pushDeletep(nodep), nodep);
          } else if (VN_IS(cellp->modp(), NotFoundModule)) {
            cellp->modNameFileline()->v3error(
                "Cannot find file containing interface: "
        ...
        if (ifacerefp) {
          UASSERT_OBJ(ifacerefp->ifaceViaCellp(), ifacerefp,
                      "Unlinked interface");
          ...
          if (m_ds.m_dotText != "") {
            ...
            if (varp->attrSplitVar()) {
              refp->v3warn(
                  SPLITVAR,
                  varp->prettyNameQ()
                      << " has split_var metacomment but will not be split "
                         "because"
            ...
            if (m_ds.m_unresolved && m_ds.m_unlinkedScopep) {
              ...
              if (pos != string::npos && varp->isParam() &&
                ...
                newp =
                    new AstUnlinkedRef(nodep->fileline(), refp, refp->name(),
        ...
        UINFO(9, "cell-ref-to-modport " << m_ds.m_dotText << "  " << nodep
                                        << endl);
        UINFO(9, "unlinked " << m_ds.m_unlinkedScopep << endl);
        UINFO(9, "dotSymp " << m_ds.m_dotSymp << " " << m_ds.m_dotSymp->nodep()
                            << endl);
        ...
        if (!m_ds.m_dotSymp || !VN_IS(m_ds.m_dotSymp->nodep(), Cell) ||
          ...
          nodep->v3error("Modport not referenced as <interface>."
                   ...
                   !VN_IS(VN_AS(m_ds.m_dotSymp->nodep(), Cell)->modp(),
                          Iface)) {
          ...
          m_ds.m_dotPos = DP_SCOPE;
          UINFO(9, " cell -> iface varref " << foundp->nodep() << endl);
          AstNode *newp = new AstVarRef(ifaceRefVarp->fileline(), ifaceRefVarp,
                                        VAccess::READ);
      ...
      } else if (AstLambdaArgRef *argrefp =
      ...
      if (!ok) {
        ...
        const bool checkImplicit =
            (!m_ds.m_dotp && m_ds.m_dotText == "" && !isCell);
        const bool err =
        ...
        if (checkImplicit) {
          ...
          // Else if a scope is allowed, making a signal won't help error
          // cascade
          AstVarRef *newp =
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    // ParseRefs are used the first pass (forPrimary) so we shouldn't get can't
    // find errors here now that we have a VarRef. No checkNoDot; created and
    // iterated from a parseRef
    iterateChildren(nodep);
    if (!nodep->varp()) {
      UINFO(9,
            " linkVarRef se" << cvtToHex(m_curSymp) << "  n=" << nodep << endl);
      ...
      if (VL_UNCOVERABLE(!nodep->varp())) {
        nodep->v3error(
            "Can't find definition of signal, again: " // LCOV_EXCL_LINE
            << nodep->prettyNameQ());
      }
    }
  }
  virtual void visit(AstVarXRef *nodep) override {
    // VarRef: Resolve its reference
    // We always link even if varp() is set, because the module we choose may
    // change due to creating new modules, flattening, etc.
    if (nodep->user3SetOnce())
      return;
    UINFO(8, "     " << nodep << endl);
    // No checkNoDot; created and iterated from a parseRef
    if (!m_modSymp) {
      // Module that is not in hierarchy.  We'll be dead code eliminating it
      // later.
      ...
      if (nodep->inlinedDots() != "") { // Correct for current scope
        ...
        dotSymp = m_statep->findDotted(nodep->fileline(), dotSymp, inl, baddot,
                                       okSymp);
      ...
      dotSymp = m_statep->findDotted(nodep->fileline(), dotSymp,
                                     nodep->dotted(), baddot,
                                     okSymp); // Maybe nullptr
      if (!m_statep->forScopeCreation()) {
        VSymEnt *foundp =
            m_statep->findSymPrefixed(dotSymp, nodep->name(), baddot);
        AstVar *varp =
            foundp ? foundToVarp(foundp, nodep, nodep->access()) : nullptr;
        nodep->varp(varp);
        ...
        if (!m_statep->forPrearray() && !m_statep->forScopeCreation()) {
          if (VN_IS(nodep->dtypep(), IfaceRefDType)) {
            AstVarRef *newrefp = new AstVarRef(nodep->fileline(), nodep->varp(),
                                               nodep->access());
        ...
        if (!vscp) {
          ...
          while (vscp->user2p()) { // If V3Inline aliased it, pick up the new
                                   // signal
            UINFO(7, "         Resolved pre-alias "
                         << vscp << endl); // Also prints taskp
  ...
  virtual void visit(AstVar *nodep) override {
    checkNoDot(nodep);
    iterateChildren(nodep);
    if (m_statep->forPrimary() && nodep->isIO() && !m_ftaskp &&
        !nodep->user4()) {
  ...
  virtual void visit(AstNodeFTaskRef *nodep) override {
    ...
    if (m_ds.m_dotp && m_ds.m_dotPos == DP_PACKAGE) {
      ...
      AstClassOrPackageRef *cpackagerefp =
          VN_AS(m_ds.m_dotp->lhsp(), ClassOrPackageRef);
      if (cpackagerefp->name() == "process" ||
          cpackagerefp->name() == "local") {
        nodep->v3warn(E_UNSUPPORTED, "Unsupported: " << AstNode::prettyNameQ(
                                         cpackagerefp->name()));
      }
      ...
      AstNode *newp = new AstMethodCall(
          nodep->fileline(), varEtcp, VFlagChildDType(), nodep->name(), argsp);
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
      return;
    } else {
    ...
    if (nodep->classOrPackagep() && nodep->taskp()) {
      ...
      if (nodep->classOrPackagep()) { // Look only in specified package
        ...
        if (nodep->inlinedDots() != "") { // Correct for current scope
          ...
          dotSymp = m_statep->findDotted(nodep->fileline(), dotSymp, inl,
                                         baddot, okSymp);
          if (!dotSymp) {
            okSymp->cellErrorScopes(nodep);
      ...
      VSymEnt *foundp =
          m_statep->findSymPrefixed(dotSymp, nodep->name(), baddot);
      AstNodeFTask *taskp = foundp ? VN_CAST(foundp->nodep(), NodeFTask)
                                   : nullptr; // Maybe nullptr
      if (taskp) {
        nodep->taskp(taskp);
        nodep->classOrPackagep(foundp->classOrPackagep());
        UINFO(7, "         Resolved " << nodep << endl); // Also prints taskp
          ...
          if (nodep->pli()) {
            if (v3Global.opt.bboxSys()) {
              ...
              if (VN_IS(nodep, FuncRef)) {
                newp = new AstConst(nodep->fileline(),
                                    AstConst::StringToParse(), "'0");
              } else {
  ...
  virtual void visit(AstSelBit *nodep) override {
    ...
    if (m_ds.m_dotPos == DP_SCOPE) { // Already under dot, so this is
                                     // {modulepart} DOT {modulepart}
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    if (nodep->isExternDef()) {
      if (!m_curSymp->findIdFallback("extern " + nodep->name())) {
        nodep->v3error("extern not found that declares " +
                       nodep->prettyNameQ());
      }
    }
    if (nodep->isExternProto()) {
      if (!m_curSymp->findIdFallback(nodep->name())) {
        nodep->v3error("definition not found for extern " +
                       nodep->prettyNameQ());
      }
    }
    VSymEnt *oldCurSymp = m_curSymp;
          ...
          if (!cpackagerefp) {
            ...
            if (foundp) {
              if (AstClass *classp = VN_CAST(foundp->nodep(), Class)) {
                UINFO(8, "Import to " << nodep << " from export class "
                                      << classp << endl);
                if (classp == nodep) {
  ...
  virtual void visit(AstRefDType *nodep) override {
    ...
    if (AstNode *cpackagep = nodep->classOrPackageOpp()) {
      if (AstClassOrPackageRef *cpackagerefp =
    ...
    if (nodep->typeofp()) { // Really is a typeof not a reference
      ...
      VSymEnt *foundp;
      if (nodep->classOrPackagep()) {
        foundp = m_statep->getNodeSym(nodep->classOrPackagep())
                     ->findIdFlat(nodep->name());
      } else {
      ...
      if (AstTypedef *defp =
        ...
        nodep->classOrPackagep(foundp->classOrPackagep());
                     ...
                     foundp ? VN_CAST(foundp->nodep(), ParamTypeDType)
                            : nullptr) {
        nodep->refDTypep(defp);
        nodep->classOrPackagep(foundp->classOrPackagep());
      } else if (AstClass *defp =
                     foundp ? VN_AS(foundp->nodep(), Class) : nullptr) {
  ...
  virtual void visit(AstDpiExport *nodep) override {
    ...
    if (!taskp) {
      nodep->v3error("Can't find definition of exported task/function: "
                     << nodep->prettyNameQ());
    } else if (taskp->dpiExport()) {
      nodep->v3error(
          "Function was already DPI Exported, duplicate not allowed: "
          << nodep->prettyNameQ());
    } else {
      taskp->dpiExport(true);
      if (nodep->cname() != "")
        taskp->cname(nodep->cname());
    }
    VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
  }
  virtual void visit(AstPackageImport *nodep) override {
...
void V3LinkDot::linkDotGuts(AstNetlist *rootp, VLinkDotStep step) {
  ...
  if (LinkDotState::debug() >= 5 || v3Global.opt.dumpTree() >= 9) {
    v3Global.rootp()->dumpTreeFile(
        v3Global.debugFilename("prelinkdot-find.tree"));
  }
  if (step == LDS_PRIMARY || step == LDS_PARAMED) {
    ...
    if (LinkDotState::debug() >= 5 || v3Global.opt.dumpTree() >= 9) {
      v3Global.rootp()->dumpTreeFile(
          v3Global.debugFilename("prelinkdot-param.tree"));
    }
  } else if (step == LDS_ARRAYED) {
  } else if (step == LDS_SCOPED) {
    // Well after the initial link when we're ready to operate on the flat
    // design, process AstScope's.  This needs to be separate pass after whole
    // hierarchy graph created.
    LinkDotScopeVisitor visitors{rootp, &state};
    v3Global.assertScoped(true);
    if (LinkDotState::debug() >= 5 || v3Global.opt.dumpTree() >= 9) {
      v3Global.rootp()->dumpTreeFile(
          v3Global.debugFilename("prelinkdot-scoped.tree"));
    }
  } else {
    v3fatalSrc("Bad case");
  }
