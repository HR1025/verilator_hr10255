...
private:
  ...
  void emitInt() {
    const string filename =
        v3Global.opt.makeDir() + "/" + topClassName() + "__main.cpp";
    ...
    puts("const std::unique_ptr<VerilatedContext> contextp{new "
         "VerilatedContext};\n");
    puts("contextp->commandArgs(argc, argv);\n");
    puts("\n");

    puts("// Construct the Verilated model, from Vtop.h generated from "
         "Verilating\n");
    puts("const std::unique_ptr<" + topClassName() + "> topp{new " +
         topClassName() + "{contextp.get()}};\n");
    puts("\n");
    ...
    puts(/**/ "VL_DEBUG_IF(VL_PRINTF(\"+ Exiting without $finish; no events "
              "left\\n\"););\n");
    puts("}\n");
    puts("\n");
