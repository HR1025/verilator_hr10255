...
inline int AstNode::widthMin() const {
  return dtypep() ? dtypep()->widthMin() : 0;
}
...
inline bool AstNode::isDouble() const {
  return dtypep() && VN_IS(dtypep(), BasicDType) &&
         VN_AS(dtypep(), BasicDType)->isDouble();
}
...
inline bool AstNodeDType::isFourstate() const {
...
inline int AstNodeArrayDType::elementsConst() const {
...
inline void AstIfaceRefDType::cloneRelink() {
  if (m_cellp && m_cellp->clonep())
    m_cellp = m_cellp->clonep();
  if (m_ifacep && m_ifacep->clonep())
    m_ifacep = m_ifacep->clonep();
  if (m_modportp && m_modportp->clonep())
    m_modportp = m_modportp->clonep();
}
