...
private:
  ...
  void initWithNumber() {
    if (m_num.isDouble()) {
      ...
      dtypeSetLogicUnsized(m_num.width(),
...
public:
  ...
  AstConst(FileLine *fl, StringToParse, const char *sourcep)
      : ASTGEN_SUPER_Const(fl), m_num(this, sourcep) {
    initWithNumber();
  }
  ...
  AstConst(FileLine *fl, uint32_t num)
      : ASTGEN_SUPER_Const(fl), m_num(this, 32, num) {
    dtypeSetLogicUnsized(m_num.width(), 0, VSigning::UNSIGNED);
  }
  class Unsized32 {}; // for creator type-overload selection
  AstConst(FileLine *fl, Unsized32,
           uint32_t num) // Unsized 32-bit integer of specified value
      : ASTGEN_SUPER_Const(fl), m_num(this, 32, num) {
  ...
  AstConst(FileLine *fl, Signed32,
           int32_t num) // Signed 32-bit integer of specified value
      : ASTGEN_SUPER_Const(fl), m_num(this, 32, num) {
    m_num.width(32, true);
  ...
  AstConst(FileLine *fl, Unsized64, vluint64_t num)
      : ASTGEN_SUPER_Const(fl), m_num(this, 64, 0) {
    m_num.setQuad(num);
  ...
  AstConst(FileLine *fl, SizedEData, vluint64_t num)
      : ASTGEN_SUPER_Const(fl), m_num(this, VL_EDATASIZE, 0) {
    m_num.setQuad(num);
    dtypeSetLogicSized(VL_EDATASIZE, VSigning::UNSIGNED);
  }
  class RealDouble {}; // for creator type-overload selection
  AstConst(FileLine *fl, RealDouble, double num)
      : ASTGEN_SUPER_Const(fl), m_num(this, 64) {
    m_num.setDouble(num);
    dtypeSetDouble();
  }
  class String {}; // for creator type-overload selection
  AstConst(FileLine *fl, String, const string &num)
      : ASTGEN_SUPER_Const(fl), m_num(V3Number::String(), this, num) {
    dtypeSetString();
  }
  class BitFalse {};
  AstConst(FileLine *fl,
           BitFalse) // Shorthand const 0, dtype should be a logic of size 1
      : ASTGEN_SUPER_Const(fl), m_num(this, 1, 0) {
    dtypeSetBit();
  }
  // Shorthand const 1 (or with argument 0/1), dtype should be a logic of size 1
  class BitTrue {};
  AstConst(FileLine *fl, BitTrue, bool on = true)
      : ASTGEN_SUPER_Const(fl), m_num(this, 1, on) {
    dtypeSetBit();
  }
  class Null {};
  AstConst(FileLine *fl, Null)
      : ASTGEN_SUPER_Const(fl), m_num(V3Number::Null{}, this) {
    dtypeSetBit(); // Events 1 bit, objects 64 bits, so autoExtend=1 and use bit
                   // here
    initWithNumber();
  }
...
public:
  AstEmptyQueue(FileLine *fl) : ASTGEN_SUPER_EmptyQueue(fl) {}
...
public:
  ...
  AstRange(FileLine *fl, int left, int right) : ASTGEN_SUPER_Range(fl) {
    setOp2p(new AstConst(fl, left));
    setOp3p(new AstConst(fl, right));
  }
  AstRange(FileLine *fl, const VNumRange &range) : ASTGEN_SUPER_Range(fl) {
...
public:
  explicit AstUnsizedRange(FileLine *fl) : ASTGEN_SUPER_UnsizedRange(fl) {}
...
class AstGatePin final : public AstNodeMath {
  // Possibly expand a gate primitive input pin value to match the range of the
  // gate primitive
public:
...
class AstClassPackage final : public AstNodeModule {
  ...
  AstClass *m_classp = nullptr; // Class package this is under (weak pointer,
                                // hard link is other way)
public:
...
public:
  AstClass(FileLine *fl, const string &name) : ASTGEN_SUPER_Class(fl, name) {}
  ...
  void classOrPackagep(AstClassPackage *classpackagep) {
    m_classOrPackagep = classpackagep;
  }
  ...
  // Accepts nullptrs
  static bool isClassExtendedFrom(const AstClass *refClassp,
                                  const AstClass *baseClassp);
};
...
public:
  AstParamTypeDType(FileLine *fl, AstVarType type, const string &name,
                    VFlagChildDType, AstNodeDType *dtp)
      : ASTGEN_SUPER_ParamTypeDType(fl), m_varType{type}, m_name{name} {
  ...
  virtual bool similarDType(AstNodeDType *samep) const override {
    ...
    return type() == samep->type() && sp &&
           this->subDTypep()->skipRefp()->similarDType(
               sp->subDTypep()->skipRefp());
  }
  virtual int widthAlignBytes() const override {
    return dtypep()->widthAlignBytes();
  }
  virtual int widthTotalBytes() const override {
    return dtypep()->widthTotalBytes();
  }
  // METHODS
...
public:
  AstTypedef(FileLine *fl, const string &name, AstNode *attrsp, VFlagChildDType,
             AstNodeDType *dtp)
      : ASTGEN_SUPER_Typedef(fl), m_name{name} {
    childDTypep(dtp); // Only for parser
    addAttrsp(attrsp);
    dtypep(nullptr); // V3Width will resolve
    m_attrPublic = false;
  ...
  AstNodeDType *childDTypep() const { return VN_AS(op1p(), NodeDType); }
  void childDTypep(AstNodeDType *nodep) { setOp1p(nodep); }
  virtual AstNodeDType *subDTypep() const {
    return dtypep() ? dtypep() : childDTypep();
  }
  void addAttrsp(AstNode *nodep) { addNOp4p(nodep); }
  AstNode *attrsp() const {
    return op4p();
  } // op4 = Attributes during early parse
  // METHODS
  virtual string name() const override { return m_name; }
  virtual bool maybePointedTo() const override { return true; }
  virtual bool hasDType() const override { return true; }
...
public:
  AstTypedefFwd(FileLine *fl, const string &name)
      : ASTGEN_SUPER_TypedefFwd(fl), m_name{name} {}
...
class AstDefImplicitDType final : public AstNodeDType {
  // For parsing enum/struct/unions that are declared with a variable rather
  // than typedef This allows "var enum {...} a,b" to share the enum definition
  // for both variables After link, these become typedefs
private:
  string m_name;
  void *m_containerp; // In what scope is the name unique, so we can know what
                      // are duplicate definitions (arbitrary value)
  const int m_uniqueNum;

public:
  AstDefImplicitDType(FileLine *fl, const string &name, void *containerp,
                      VFlagChildDType, AstNodeDType *dtp)
      : ASTGEN_SUPER_DefImplicitDType(fl), m_name{name},
        m_containerp{containerp}, m_uniqueNum{uniqueNumInc()} {
    childDTypep(dtp); // Only for parser
    dtypep(nullptr);  // V3Width will resolve
  }
  ...
  virtual bool same(const AstNode *samep) const override {
    const AstDefImplicitDType *sp =
        static_cast<const AstDefImplicitDType *>(samep);
...
public:
  AstAssocArrayDType(FileLine *fl, VFlagChildDType, AstNodeDType *dtp,
                     AstNodeDType *keyDtp)
  ...
  virtual const char *broken() const override {
    ...
    BROKEN_RTN(
        !((m_keyDTypep && !childDTypep() && m_keyDTypep->brokeExists()) ||
  ...
  virtual void cloneRelink() override {
    ...
    if (m_keyDTypep && m_keyDTypep->clonep())
      m_keyDTypep = m_keyDTypep->clonep();
  }
  virtual bool same(const AstNode *samep) const override {
    const AstAssocArrayDType *asamep =
        static_cast<const AstAssocArrayDType *>(samep);
    if (!asamep->subDTypep())
      return false;
    if (!asamep->keyDTypep())
      return false;
  ...
  virtual bool similarDType(AstNodeDType *samep) const override {
    ...
    return type() == samep->type() && asamep->subDTypep() &&
           subDTypep()->skipRefp()->similarDType(
               asamep->subDTypep()->skipRefp());
  }
  ...
  virtual AstNodeDType *getChild2DTypep() const override {
    return keyChildDTypep();
  }
  // op1 = Range of variable
  AstNodeDType *childDTypep() const { return VN_AS(op1p(), NodeDType); }
  void childDTypep(AstNodeDType *nodep) { setOp1p(nodep); }
  ...
  virtual void virtRefDType2p(AstNodeDType *nodep) override {
    keyDTypep(nodep);
  }
  //
  AstNodeDType *keyDTypep() const {
  ...
  virtual int widthTotalBytes() const override {
    return subDTypep()->widthTotalBytes();
  }
...
public:
  AstBracketArrayDType(FileLine *fl, VFlagChildDType, AstNodeDType *dtp,
                       AstNode *elementsp)
...
public:
  AstPackArrayDType(FileLine *fl, VFlagChildDType, AstNodeDType *dtp,
                    AstRange *rangep)
...
public:
  AstUnpackArrayDType(FileLine *fl, VFlagChildDType, AstNodeDType *dtp,
                      AstRange *rangep)
  ...
  virtual string prettyDTypeName() const override;
  virtual bool same(const AstNode *samep) const override {
    const AstUnpackArrayDType *sp =
        static_cast<const AstUnpackArrayDType *>(samep);
...
private:
  struct Members {
    AstBasicDTypeKwd
        m_keyword; // (also in VBasicTypeKey) What keyword created basic type
    VNumRange m_nrange; // (also in VBasicTypeKey) Numeric msb/lsb (if
                        // non-opaque keyword)
...
public:
  AstBasicDType(FileLine *fl, AstBasicDTypeKwd kwd,
                const VSigning &signst = VSigning::NOSIGN)
  ...
  AstBasicDType(FileLine *fl, AstBasicDTypeKwd kwd, VSigning numer,
                int wantwidth, int widthmin)
      : ASTGEN_SUPER_BasicDType(fl) {
    init(kwd, numer, wantwidth, widthmin, nullptr);
  }
  AstBasicDType(FileLine *fl, AstBasicDTypeKwd kwd, VSigning numer,
                VNumRange range, int widthmin)
      : ASTGEN_SUPER_BasicDType(fl) {
    init(kwd, numer, range.elements(), widthmin, nullptr);
    m.m_nrange =
        range; // as init() presumes lsb==0, but range.lsb() might not be
  }
  // See also addRange in verilog.y
private:
  void init(AstBasicDTypeKwd kwd, VSigning numer, int wantwidth,
            int wantwidthmin, AstRange *rangep) {
    ...
    if (keyword() == AstBasicDTypeKwd::LOGIC_IMPLICIT) {
      if (rangep || wantwidth)
        m.m_keyword = AstBasicDTypeKwd::LOGIC;
    }
    ...
    if (!rangep && (wantwidth || wantwidthmin >= 0)) { // Constant width
      if (wantwidth > 1)
        m.m_nrange.init(wantwidth - 1, 0, false);
...
public:
  ...
  AstBasicDTypeKwd
  keyword() const { // Avoid using - use isSomething accessors instead
    return m.m_keyword;
  }
  ...
  bool isDpiLogicVec() const { // DPI uses svLogicVecVal
    return keyword().isFourstate() &&
           !(keyword() == AstBasicDTypeKwd::LOGIC && !isRanged());
  }
  ...
  bool implicit() const {
    return keyword() == AstBasicDTypeKwd::LOGIC_IMPLICIT;
  }
  VNumRange declRange() const {
    return isRanged() ? VNumRange{left(), right()} : VNumRange{};
  }
...
class AstConstDType final : public AstNodeDType {
  ...
  // When more generic types are supported AstConstDType will be propagated
  // further.
private:
...
public:
  AstClassRefDType(FileLine *fl, AstClass *classp, AstNode *paramsp)
      : ASTGEN_SUPER_ClassRefDType(fl), m_classp{classp} {
    dtypep(this);
    addNOp4p(paramsp);
  }
  ...
  virtual void cloneRelink() override {
    if (m_classp && m_classp->clonep())
      m_classp = m_classp->clonep();
  }
  virtual bool same(const AstNode *samep) const override {
    const AstClassRefDType *asamep =
        static_cast<const AstClassRefDType *>(samep);
  ...
  virtual string name() const override {
    return classp() ? classp()->name() : "<unlinked>";
  }
  virtual AstBasicDType *basicp() const override { return nullptr; }
  virtual AstNodeDType *skipRefp() const override {
    return (AstNodeDType *)this;
...
private:
  ...
  AstIface *m_ifacep =
      nullptr; // Pointer to interface; note cellp() should override
...
public:
  AstIfaceRefDType(FileLine *fl, const string &cellName,
                   const string &ifaceName)
      : ASTGEN_SUPER_IfaceRefDType(fl), m_modportFileline{nullptr},
  ...
  AstIfaceRefDType(FileLine *fl, FileLine *modportFl, const string &cellName,
      ...
      : ASTGEN_SUPER_IfaceRefDType(fl), m_modportFileline{modportFl},
...
private:
  ...
  AstNodeDType *m_refDTypep =
      nullptr;   // data type pointed to, BELOW the AstTypedef
  string m_name; // Name of an AstTypedef
  AstNodeModule *m_classOrPackagep = nullptr; // Package hierarchy
public:
  AstRefDType(FileLine *fl, const string &name)
      : ASTGEN_SUPER_RefDType(fl), m_name{name} {}
  AstRefDType(FileLine *fl, const string &name, AstNode *classOrPackagep,
              AstNode *paramsp)
      : ASTGEN_SUPER_RefDType(fl), m_name{name} {
  ...
  virtual void cloneRelink() override {
    if (m_typedefp && m_typedefp->clonep())
      m_typedefp = m_typedefp->clonep();
    if (m_refDTypep && m_refDTypep->clonep())
      m_refDTypep = m_refDTypep->clonep();
  }
  virtual bool same(const AstNode *samep) const override {
    const AstRefDType *asamep = static_cast<const AstRefDType *>(samep);
    return (m_typedefp == asamep->m_typedefp &&
            m_refDTypep == asamep->m_refDTypep && m_name == asamep->m_name &&
            m_classOrPackagep == asamep->m_classOrPackagep);
  }
  virtual bool similarDType(AstNodeDType *samep) const override {
    return skipRefp()->similarDType(samep->skipRefp());
  ...
  virtual int widthAlignBytes() const override {
    return dtypeSkipRefp()->widthAlignBytes();
  }
  virtual int widthTotalBytes() const override {
    return dtypeSkipRefp()->widthTotalBytes();
  }
  virtual void name(const string &flag) override { m_name = flag; }
  // op1 = Range of variable
...
private:
  ...
  string m_name;             // Name of variable
  string m_tag; // Holds the string of the verilator tag -- used in XML output.
  int m_lsb = -1; // Within this level's packed struct, the LSB of the first bit
                  // of the member
                  // UNSUP: int m_randType;    // Randomization type (IEEE)
public:
  AstMemberDType(FileLine *fl, const string &name, VFlagChildDType,
                 AstNodeDType *dtp)
      : ASTGEN_SUPER_MemberDType(fl), m_name{name} {
    childDTypep(dtp); // Only for parser
    dtypep(nullptr);  // V3Width will resolve
    refDTypep(nullptr);
  }
  ...
  // op1 = Range of variable (Note don't need virtual - AstVar isn't a
  // NodeDType)
  AstNodeDType *dtypeSkipRefp() const { return subDTypep()->skipRefp(); }
  virtual AstNodeDType *skipRefp() const override {
    return subDTypep()->skipRefp();
...
class AstEmptyQueueDType final : public AstNodeDType {
  // For EmptyQueue
public:
  explicit AstEmptyQueueDType(FileLine *fl) : ASTGEN_SUPER_EmptyQueueDType(fl) {
    dtypep(this);
  }
...
public:
  explicit AstVoidDType(FileLine *fl) : ASTGEN_SUPER_VoidDType(fl) {
    dtypep(this);
  }
...
public:
  ...
  AstEnumItem(FileLine *fl, const string &name, AstNode *rangep, AstNode *initp)
      : ASTGEN_SUPER_EnumItem(fl), m_name{name} {
    addNOp1p(rangep);
    addNOp2p(initp);
  }
...
public:
  AstEnumItemRef(FileLine *fl, AstEnumItem *itemp,
      ...
      : ASTGEN_SUPER_EnumItemRef(fl), m_itemp{itemp}, m_classOrPackagep{
                                                          classOrPackagep} {
    dtypeFrom(m_itemp);
  }
...
public:
  AstEnumDType(FileLine *fl, VFlagChildDType, AstNodeDType *dtp,
               AstNode *itemsp)
      : ASTGEN_SUPER_EnumDType(fl), m_uniqueNum{uniqueNumInc()} {
    childDTypep(dtp); // Only for parser
    refDTypep(nullptr);
    addNOp2p(itemsp);
    dtypep(nullptr); // V3Width will resolve
  ...
  virtual string name() const override { return m_name; }
  virtual void name(const string &flag) override { m_name = flag; }
  AstEnumItem *itemsp() const {
    return VN_AS(op2p(), EnumItem);
  } // op2 = AstEnumItem's
  // METHODS
  virtual AstBasicDType *basicp() const override {
    return subDTypep()->basicp();
...
class AstParseTypeDType final : public AstNodeDType {
  // Parents: VAR
  // During parsing, this indicates the type of a parameter is a "parameter
  // type" e.g. the data type is a container of any data type
public:
  explicit AstParseTypeDType(FileLine *fl) : ASTGEN_SUPER_ParseTypeDType(fl) {}
...
private:
  void init(AstNode *fromp) {
    if (fromp && VN_IS(fromp->dtypep()->skipRefp(), AssocArrayDType)) {
      // Strip off array to find what array references
      dtypeFrom(
          VN_AS(fromp->dtypep()->skipRefp(), AssocArrayDType)->subDTypep());
    }
  }
...
public:
  ...
  virtual void numberOperate(V3Number &out, const V3Number &from,
                             const V3Number &bit) override {
    V3ERROR_NA;
  }
  virtual string emitVerilog() override { return "%k(%l%f[%r])"; }
...
class AstSelBit final : public AstNodePreSel {
  ...
  // selection Gets replaced during link with AstArraySel or AstSel
public:
...
class AstSel final : public AstNodeTriop {
  ...
  // different, but that doesn't play well with V3Inst and bidirects which don't
  // know direction
private:
  VNumRange m_declRange; // Range of the 'from' array if isRanged() is set, else
                         // invalid
...
public:
  AstSel(FileLine *fl, AstNode *fromp, AstNode *lsbp, AstNode *widthp)
      : ASTGEN_SUPER_Sel(fl, fromp, lsbp, widthp), m_declElWidth{1} {
  ...
  AstSel(FileLine *fl, AstNode *fromp, int lsb, int bitwidth)
      : ASTGEN_SUPER_Sel(fl, fromp, new AstConst(fl, lsb),
                         new AstConst(fl, bitwidth)),
        m_declElWidth{1} {
...
public:
  ...
  virtual int instrCount() const override {
    return 10;
  } // Removed before matters
  AstNode *fromp() const {
    return op1p();
  } // op1 = Extracting what (nullptr=TBD during parsing)
  // For widthConst()/loConst etc, see declRange().elements() and other
  // VNumRange methods
  VNumRange &declRange() { return m_declRange; }
  const VNumRange &declRange() const { return m_declRange; }
  void declRange(const VNumRange &flag) { m_declRange = flag; }
...
class AstMethodCall final : public AstNodeFTaskRef {
  ...
  // PARENTS: stmt/math
  // Not all calls are statments vs math.  AstNodeStmt needs isStatement() to
  // deal. Don't need the class we are extracting from, as the "fromp()"'s
  // datatype can get us to it
public:
  AstMethodCall(FileLine *fl, AstNode *fromp, VFlagChildDType,
                const string &name, AstNode *pinsp)
      : ASTGEN_SUPER_MethodCall(fl, false, name, pinsp) {
    setOp2p(fromp);
    dtypep(nullptr); // V3Width will resolve
  }
  AstMethodCall(FileLine *fl, AstNode *fromp, const string &name,
                AstNode *pinsp)
      : ASTGEN_SUPER_MethodCall(fl, false, name, pinsp) {
    setOp2p(fromp);
  }
...
public:
  AstCMethodHard(FileLine *fl, AstNode *fromp, VFlagChildDType,
                 const string &name, AstNode *pinsp)
      : ASTGEN_SUPER_CMethodHard(fl, false), m_name{name} {
    setOp1p(fromp);
    dtypep(nullptr); // V3Width will resolve
    addNOp2p(pinsp);
  }
  AstCMethodHard(FileLine *fl, AstNode *fromp, const string &name,
                 AstNode *pinsp)
      : ASTGEN_SUPER_CMethodHard(fl, false), m_name{name} {
    setOp1p(fromp);
    addNOp2p(pinsp);
...
private:
  ...
  bool m_usedParam : 1;   // Parameter is referenced (on link; later signals not
                          // setup)
  ...
  bool
      m_isStatic : 1; // Static C variable (for Verilog see instead isAutomatic)
  bool m_isPulldown : 1;      // Tri0
  bool m_isPullup : 1;        // Tri1
  bool m_isIfaceParent : 1;   // dtype is reference to interface present in this
                              // module
...
public:
  AstVar(FileLine *fl, AstVarType type, const string &name, VFlagChildDType,
         AstNodeDType *dtp)
      : ASTGEN_SUPER_Var(fl), m_name{name}, m_origName{name} {
    init();
    combineType(type);
    childDTypep(dtp); // Only for parser
    dtypep(nullptr);  // V3Width will resolve
  ...
  AstVar(FileLine *fl, AstVarType type, const string &name, VFlagLogicPacked,
         int wantwidth)
      : ASTGEN_SUPER_Var(fl), m_name{name}, m_origName{name} {
    init();
    combineType(type);
  ...
  AstVar(FileLine *fl, AstVarType type, const string &name, VFlagBitPacked,
         int wantwidth)
      : ASTGEN_SUPER_Var(fl), m_name{name}, m_origName{name} {
    init();
    combineType(type);
  ...
  virtual bool hasDType() const override { return true; }
  virtual bool maybePointedTo() const override { return true; }
  virtual string origName() const override {
    return m_origName;
  } // * = Original name
  ...
  void direction(const VDirection &flag) {
    m_direction = flag;
    if (m_direction == VDirection::INOUT)
      m_tristate = true;
  }
  /**
   * @brief  获取 IO 的方向
  ...
  string dpiArgType(bool named,
                    bool forReturn) const; // Return DPI-C type for argument
  string dpiTmpVarType(const string &varName) const;
  // Return Verilator internal type for argument: CData, SData, IData, WData
  string vlArgType(bool named, bool forReturn, bool forFunc,
                   const string &namespc = "", bool asRef = false) const;
  string vlEnumType() const; // Return VerilatorVarType: VLVT_UINT32, etc
  string vlEnumDir() const;  // Return VerilatorVarDir: VLVD_INOUT, etc
  string vlPropDecl(
      const string &propName) const; // Return VerilatorVarProps declaration
  ...
  // op3 = Initial value that never changes (static const), or constructor
  // argument for MTASKSTATE variables
  ...
  void propagateAttrFrom(AstVar *fromp) {
    ...
    if (fromp->attrClockEn())
      attrClockEn(true);
    if (fromp->attrFileDescr())
      attrFileDescr(true);
    if (fromp->attrIsolateAssign())
      attrIsolateAssign(true);
  }
  bool gateMultiInputOptimizable() const {
    // Ok to gate optimize; must return false if propagateAttrFrom would do
    // anything
  ...
  void combineType(AstVar *typevarp) {
    ...
    if (typevarp->isSigPublic())
      sigPublic(true);
    if (typevarp->isSigModPublic())
      sigModPublic(true);
    if (typevarp->isSigUserRdPublic())
      sigUserRdPublic(true);
    if (typevarp->isSigUserRWPublic())
      sigUserRWPublic(true);
    if (typevarp->attrScClocked())
      attrScClocked(true);
  }
...
public:
  AstDefParam(FileLine *fl, const string &path, const string &name,
              AstNode *rhsp)
...
class AstImplicit final : public AstNode {
  ...
  // Parents: MODULE
public:
  AstImplicit(FileLine *fl, AstNode *exprsp) : ASTGEN_SUPER_Implicit(fl) {
    addNOp1p(exprsp);
  }
...
class AstScope final : public AstNode {
  ...
  // Children: NODEBLOCK
private:
  // An AstScope->name() is special: . indicates an uninlined scope, __DOT__ an
  // inlined scope
  string m_name; // Name
  AstScope *const
      m_aboveScopep; // Scope above this one in the hierarchy (nullptr if top)
  AstCell
      *const m_aboveCellp; // Cell above this in the hierarchy (nullptr if top)
...
public:
  AstScope(FileLine *fl, AstNodeModule *modp, const string &name,
      ...
      : ASTGEN_SUPER_Scope(fl), m_name{name}, m_aboveScopep{aboveScopep},
        m_aboveCellp{aboveCellp}, m_modp{modp} {}
  ...
  AstVarScope *varsp() const {
    return VN_AS(op1p(), VarScope);
  } // op1 = AstVarScope's
...
class AstTopScope final : public AstNode {
  ...
  // unique sensitivity lists). Parent: Top level AstModule Children:
  // AstSenTree, AstScope
  friend class AstNetlist; // Only the AstNetlist can create one
  AstTopScope(FileLine *fl, AstScope *ascopep) : ASTGEN_SUPER_TopScope(fl) {
    addOp2p(ascopep);
  }
...
private:
  ...
  bool m_circular : 1; // Used in circular ordering dependency, need change
                       // detect
  bool m_trace : 1;    // Tracing is turned on for this scope
public:
  AstVarScope(FileLine *fl, AstScope *scopep, AstVar *varp)
      : ASTGEN_SUPER_VarScope(fl), m_scopep{scopep}, m_varp{varp} {
...
public:
  ...
  AstVarRef(FileLine *fl, AstVarScope *varscp, const VAccess &access)
      : ASTGEN_SUPER_VarRef(fl, varscp->varp()->name(), varscp->varp(),
                            access) {
    varScopep(varscp);
  }
...
class AstVarXRef final : public AstNodeVarRef {
  ...
  // Includes pin on a cell, as part of a ASSIGN statement to connect I/Os until
  // AstScope
private:
  string
      m_dotted; // Dotted part of scope the name()'ed reference is under or ""
  string m_inlinedDots; // Dotted hierarchy flattened out
public:
  AstVarXRef(FileLine *fl, const string &name, const string &dotted,
             const VAccess &access)
      : ASTGEN_SUPER_VarXRef(fl, name, nullptr, access), m_dotted{dotted} {}
  AstVarXRef(FileLine *fl, AstVar *varp, const string &dotted,
             const VAccess &access)
      : ASTGEN_SUPER_VarXRef(fl, varp->name(), varp, access), m_dotted{dotted} {
    dtypeFrom(varp);
  }
  ...
  virtual bool same(const AstNode *samep) const override {
    ...
    return (selfPointer() == asamep->selfPointer() &&
            varp() == asamep->varp() && name() == asamep->name() &&
...
public:
  AstAddrOfCFunc(FileLine *fl, AstCFunc *funcp)
      : ASTGEN_SUPER_AddrOfCFunc(fl), m_funcp{funcp} {
...
private:
  ...
  AstParamTypeDType *m_modPTypep =
      nullptr;               // Param type this pin connects to on submodule.
...
public:
  AstPin(FileLine *fl, int pinNum, const string &name, AstNode *exprp)
      : ASTGEN_SUPER_Pin(fl), m_pinNum{pinNum}, m_name{name} {
    setNOp1p(exprp);
  }
  AstPin(FileLine *fl, int pinNum, AstVarRef *varname, AstNode *exprp)
      : ASTGEN_SUPER_Pin(fl), m_pinNum{pinNum}, m_name{varname->name()} {
    setNOp1p(exprp);
  }
  ...
  bool dotStar() const {
    return name() == ".*";
  } // Fake name for .* connections until linked
  /**
   * @brief 引脚号
   */
  int pinNum() const { return m_pinNum; }
...
public:
  AstArg(FileLine *fl, const string &name, AstNode *exprp)
      : ASTGEN_SUPER_Arg(fl), m_name{name} {
    setNOp1p(exprp);
  }
  ...
  // op1 = Expression connected to pin, nullptr if unconnected
  AstNode *exprp() const { return op1p(); }
  bool emptyConnectNoNext() const {
    return !exprp() && name() == "" && !nextp();
  }
};
...
public:
  AstModule(FileLine *fl, const string &name, bool program = false)
      : ASTGEN_SUPER_Module(fl, name), m_isProgram{program} {}
  ASTNODE_NODE_FUNCS(Module)
  virtual string verilogKwd() const override {
    return m_isProgram ? "program" : "module";
  }
...
public:
  AstPackageExport(FileLine *fl, AstPackage *packagep, const string &name)
      : ASTGEN_SUPER_PackageExport(fl), m_name{name}, m_packagep{packagep} {}
  ...
  virtual void cloneRelink() override {
    if (m_packagep && m_packagep->clonep())
...
public:
  AstPackageImport(FileLine *fl, AstPackage *packagep, const string &name)
      : ASTGEN_SUPER_PackageImport(fl), m_name{name}, m_packagep{packagep} {}
...
class AstIface final : public AstNodeModule {
  // A module declaration
public:
  AstIface(FileLine *fl, const string &name) : ASTGEN_SUPER_Iface(fl, name) {}
...
class AstMemberSel final : public AstNodeMath {
  ...
  // Children: varref|arraysel, math
private:
  // Don't need the class we are extracting from, as the "fromp()"'s datatype
  // can get us to it
  string m_name;
  AstVar *m_varp =
      nullptr; // Post link, variable within class that is target of selection
public:
  AstMemberSel(FileLine *fl, AstNode *fromp, VFlagChildDType,
               const string &name)
      : ASTGEN_SUPER_MemberSel(fl), m_name{name} {
    setOp1p(fromp);
    dtypep(nullptr); // V3Width will resolve
  }
  AstMemberSel(FileLine *fl, AstNode *fromp, AstNodeDType *dtp)
      : ASTGEN_SUPER_MemberSel(fl), m_name{dtp->name()} {
    setOp1p(fromp);
    dtypep(dtp);
  }
...
public:
  AstModportFTaskRef(FileLine *fl, const string &name, bool isExport)
      : ASTGEN_SUPER_ModportFTaskRef(fl), m_name{name}, m_export{isExport} {}
  ...
  virtual string name() const override { return m_name; }
  virtual void cloneRelink() override {
    if (m_ftaskp && m_ftaskp->clonep())
      m_ftaskp = m_ftaskp->clonep();
  }
  bool isImport() const { return !m_export; }
  bool isExport() const { return m_export; }
  AstNodeFTask *ftaskp() const {
    return m_ftaskp;
  } // [After Link] Pointer to variable
...
class AstModportVarRef final : public AstNode {
  ...
  // The storage for the variable itself is inside the interface, thus this is a
  // reference PARENT: AstModport
private:
  string m_name;            // Name of the variable referenced
...
public:
  AstModportVarRef(FileLine *fl, const string &name, VDirection::en direction)
      : ASTGEN_SUPER_ModportVarRef(fl), m_name{name}, m_direction{direction} {}
...
public:
  AstModport(FileLine *fl, const string &name, AstNode *varsp)
      : ASTGEN_SUPER_Modport(fl), m_name{name} {
    addNOp1p(varsp);
  }
  virtual string name() const override { return m_name; }
  virtual bool maybePointedTo() const override { return true; }
...
public:
  AstIntfRef(FileLine *fl, const string &name)
      : ASTGEN_SUPER_IntfRef(fl), m_name{name} {}
  virtual string name() const override { return m_name; }
...
public:
  AstCell(FileLine *fl, FileLine *mfl, const string &instName,
          const string &modName, AstPin *pinsp, AstPin *paramsp,
          AstRange *rangep)
      : ASTGEN_SUPER_Cell(fl), m_modNameFileline{mfl}, m_name{instName},
        m_origName{instName}, m_modName{modName}, m_hasIfaceVar{false},
   ...
   */
  AstPin *pinsp() const {
    return VN_AS(op1p(), Pin);
  } // op1 = List of cell ports
  /**
   * @brief 暂时未知
   */
  AstPin *paramsp() const {
    return VN_AS(op2p(), Pin);
...
private:
  ...
  string m_origModName; // Original name of the module, ignoring name() changes,
                        // for dot lookup
...
public:
  AstCellInline(FileLine *fl, const string &name, const string &origModName,
      ...
      : ASTGEN_SUPER_CellInline(fl), m_name{name}, m_origModName{origModName},
        m_timeunit{timeunit} {}
...
public:
  AstCellRef(FileLine *fl, const string &name, AstNode *cellp, AstNode *exprp)
      : ASTGEN_SUPER_CellRef(fl), m_name{name} {
    addNOp1p(cellp);
    addNOp2p(exprp);
  }
...
public:
  AstCellArrayRef(FileLine *fl, const string &name, AstNode *selectExprp)
      : ASTGEN_SUPER_CellArrayRef(fl), m_name{name} {
...
public:
  AstUnlinkedRef(FileLine *fl, AstNode *refp, const string &name, AstNode *crp)
      : ASTGEN_SUPER_UnlinkedRef(fl), m_name{name} {
    addNOp1p(refp);
    addNOp2p(crp);
  }
...
public:
  AstBind(FileLine *fl, const string &name, AstNode *cellsp)
      : ASTGEN_SUPER_Bind(fl), m_name{name} {
    UASSERT_OBJ(VN_IS(cellsp, Cell), cellsp,
                "Only instances allowed to be bound");
    addNOp1p(cellsp);
  }
...
public:
  AstPort(FileLine *fl, int pinnum, const string &name)
      : ASTGEN_SUPER_Port(fl), m_pinNum{pinnum}, m_name{name} {}
  ASTNODE_NODE_FUNCS(Port)
  virtual string name() const override { return m_name; } // * = Port name
  int pinNum() const {
    return m_pinNum;
  } // * = Pin number, for order based instantiation
  AstNode *exprp() const {
    return op1p();
  } // op1 = Expression connected to port
};
...
public:
  AstParseRef(FileLine *fl, VParseRefExp expect, const string &name,
              AstNode *lhsp = nullptr, AstNodeFTaskRef *ftaskrefp = nullptr)
      : ASTGEN_SUPER_ParseRef(fl), m_expect{expect}, m_name{name} {
    setNOp1p(lhsp);
    setNOp2p(ftaskrefp);
  }
  ...
  void ftaskrefp(AstNodeFTaskRef *nodep) {
    setNOp2p(nodep);
...
public:
  AstClassOrPackageRef(FileLine *fl, const string &name,
                       AstNode *classOrPackageNodep, AstNode *paramsp)
      : ASTGEN_SUPER_ClassOrPackageRef(fl), m_name{name},
  ...
  virtual bool same(const AstNode *samep) const override {
    return (m_classOrPackageNodep ==
            static_cast<const AstClassOrPackageRef *>(samep)
                ->m_classOrPackageNodep);
  }
  virtual void dump(std::ostream &str = std::cout) const override;
  virtual string name() const override { return m_name; } // * = Var name
  ...
  AstNodeModule *classOrPackagep() const {
    return VN_AS(m_classOrPackageNodep, NodeModule);
  }
...
public:
  AstDot(FileLine *fl, bool colon, AstNode *lhsp, AstNode *rhsp)
      : ASTGEN_SUPER_Dot(fl), m_colon{colon} {
    setOp1p(lhsp);
    setOp2p(rhsp);
  }
  ASTNODE_NODE_FUNCS(Dot)
  // For parser, make only if non-null package
  static AstNode *newIfPkg(FileLine *fl, AstNode *packageOrClassp,
                           AstNode *rhsp) {
    if (!packageOrClassp)
      return rhsp;
...
public:
  explicit AstUnbounded(FileLine *fl) : ASTGEN_SUPER_Unbounded(fl) {
    dtypeSetSigned32();
  }
...
class AstDpiExport final : public AstNode {
  ...
  // (Probably AstNodeFTaskRef should be renamed AstNodeFTaskCall and have-a
  // AstNodeFTaskRef)
private:
...
public:
  AstDpiExport(FileLine *fl, const string &vname, const string &cname)
      : ASTGEN_SUPER_DpiExport(fl), m_name{vname}, m_cname{cname} {}
...
class AstLambdaArgRef final : public AstNodeMath {
  ...
  // These are not AstVarRefs because we need to be able to delete/clone lambdas
  // during optimizations and AstVar's are painful to remove.
private:
  string m_name; // Name of variable
...
public:
  AstLambdaArgRef(FileLine *fl, const string &name, bool index)
      : ASTGEN_SUPER_LambdaArgRef(fl), m_name{name}, m_index(index) {}
...
public:
  AstWith(FileLine *fl, AstLambdaArgRef *indexArgRefp,
          AstLambdaArgRef *valueArgRefp, AstNode *exprp)
...
public:
  ...
  AstSenItem(FileLine *fl, VEdgeType edgeType, AstNode *varrefp)
      : ASTGEN_SUPER_SenItem(fl), m_edgeType{edgeType} {
    setOp1p(varrefp);
  }
  AstSenItem(FileLine *fl, Combo)
      : ASTGEN_SUPER_SenItem(fl), m_edgeType{VEdgeType::ET_COMBO} {}
  AstSenItem(FileLine *fl, Illegal)
      : ASTGEN_SUPER_SenItem(fl), m_edgeType{VEdgeType::ET_ILLEGAL} {}
  AstSenItem(FileLine *fl, Initial)
      : ASTGEN_SUPER_SenItem(fl), m_edgeType{VEdgeType::ET_INITIAL} {}
  AstSenItem(FileLine *fl, Settle)
      : ASTGEN_SUPER_SenItem(fl), m_edgeType{VEdgeType::ET_SETTLE} {}
  AstSenItem(FileLine *fl, Never)
      : ASTGEN_SUPER_SenItem(fl), m_edgeType{VEdgeType::ET_NEVER} {}
  ...
  bool hasVar() const {
    return !(isCombo() || isInitial() || isSettle() || isNever());
  }
};
...
private:
  bool m_multi =
      false; // Created from combo logic by ORing multiple clock domains
public:
  AstSenTree(FileLine *fl, AstSenItem *sensesp) : ASTGEN_SUPER_SenTree(fl) {
    addNOp1p(sensesp);
  }
...
public:
  AstFinal(FileLine *fl, AstNode *bodysp) : ASTGEN_SUPER_Final(fl, bodysp) {}
...
public:
  AstAlways(FileLine *fl, VAlwaysKwd keyword, AstSenTree *sensesp,
            AstNode *bodysp)
      : ASTGEN_SUPER_Always(fl, bodysp), m_keyword{keyword} {
    addNOp1p(sensesp);
  }
  ASTNODE_NODE_FUNCS(Always)
  //
  virtual void dump(std::ostream &str) const override;
  AstSenTree *sensesp() const {
    return VN_AS(op1p(), SenTree);
  } // op1 = Sensitivity list
...
class AstAssignW final : public AstNodeAssign {
  // Like assign, but wire/assign's in verilog, the only setting of the
  // specified variable
public:
...
public:
  AstPull(FileLine *fl, AstNode *lhsp, bool direction) : ASTGEN_SUPER_Pull(fl) {
    setOp1p(lhsp);
...
public:
  AstComment(FileLine *fl, const string &name, bool showAt = false)
      : ASTGEN_SUPER_Comment(fl), m_showAt{showAt}, m_name{name} {}
...
public:
  ...
  virtual AstNode *cloneType(AstNode *condp, AstNode *expr1p,
                             AstNode *expr2p) override {
    return new AstCond(this->fileline(), condp, expr1p, expr2p);
  }
};

class AstCondBound final : public AstNodeCond {
  // Conditional ?: statement, specially made for safety checking of array
  // bounds Parents:  MATH Children: MATH
public:
...
class AstCoverDecl final : public AstNodeStmt {
  ...
  // Children: none
private:
  AstCoverDecl *m_dataDeclp; // [After V3CoverageJoin] Pointer to duplicate
...
public:
  ...
  virtual void cloneRelink() override {
    if (m_dataDeclp && m_dataDeclp->clonep())
      m_dataDeclp = m_dataDeclp->clonep();
  }
  virtual void dump(std::ostream &str) const override;
  ...
  virtual bool same(const AstNode *samep) const override {
    ...
    return (fileline() == asamep->fileline() &&
            linescov() == asamep->linescov() && hier() == asamep->hier() &&
  ...
  AstCoverDecl *dataDeclThisp() {
    return dataDeclNullp() ? dataDeclNullp() : this;
  }
};
...
public:
  AstCoverInc(FileLine *fl, AstCoverDecl *declp)
      : ASTGEN_SUPER_CoverInc(fl), m_declp{declp} {}
...
public:
  AstCoverToggle(FileLine *fl, AstCoverInc *incp, AstNode *origp,
                 AstNode *changep)
  ...
  virtual int instrCount() const override {
    return 3 + INSTR_COUNT_BRANCH + INSTR_COUNT_LD;
  }
  virtual bool same(const AstNode *samep) const override { return true; }
  virtual bool isGateOptimizable() const override { return false; }
  virtual bool isPredictOptimizable() const override { return true; }
...
class AstDelay final : public AstNodeStmt {
  // Delay statement
public:
  AstDelay(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Delay(fl) {
    setOp1p(lhsp);
  }
...
public:
  AstCase(FileLine *fl, VCaseType casex, AstNode *exprp, AstNode *casesp)
      : ASTGEN_SUPER_Case(fl, exprp, casesp), m_casex{casex} {}
...
public:
  ...
  virtual int instrCount() const override {
    return widthInstrs() + INSTR_COUNT_BRANCH;
  }
  AstNode *condsp() const {
    return op1p();
  } // op1 = list of possible matching expressions
...
class AstSFormatF final : public AstNode {
  ...
  char
      m_missingArgChar; // Format code when argument without format, 'h'/'o'/'b'
  VTimescale m_timeunit; // Parent module time unit
public:
  class NoFormat {};
  AstSFormatF(FileLine *fl, const string &text, bool hidden, AstNode *exprsp,
              char missingArgChar = 'd')
      : ASTGEN_SUPER_SFormatF(fl), m_text{text}, m_hidden{hidden},
        m_hasFormat{true}, m_missingArgChar{missingArgChar} {
    dtypeSetString();
    addNOp1p(exprsp);
    addNOp2p(nullptr);
  }
  AstSFormatF(FileLine *fl, NoFormat, AstNode *exprsp,
              char missingArgChar = 'd', bool hidden = true)
      : ASTGEN_SUPER_SFormatF(fl), m_text{""}, m_hidden{hidden},
        m_hasFormat{false}, m_missingArgChar{missingArgChar} {
    dtypeSetString();
    addNOp1p(exprsp);
    addNOp2p(nullptr);
  }
  ...
  void addExprsp(AstNode *nodep) {
    addOp1p(nodep);
  }                                          // op1 = Expressions to output
  ...
  bool
  formatScopeTracking() const { // Track scopeNamep();  Ok if false positive
    return (name().find("%m") != string::npos ||
...
public:
  AstDisplay(FileLine *fl, AstDisplayType dispType, const string &text,
             AstNode *filep, AstNode *exprsp, char missingArgChar = 'd')
      : ASTGEN_SUPER_Display(fl) {
    setOp1p(new AstSFormatF(fl, text, true, exprsp, missingArgChar));
    setNOp3p(filep);
    m_displayType = dispType;
  }
  AstDisplay(FileLine *fl, AstDisplayType dispType, AstNode *filep,
             AstNode *exprsp, char missingArgChar = 'd')
      : ASTGEN_SUPER_Display(fl) {
    setOp1p(
        new AstSFormatF(fl, AstSFormatF::NoFormat(), exprsp, missingArgChar));
    setNOp3p(filep);
    m_displayType = dispType;
  }
  ...
  virtual bool isOutputter() const override {
    return true;
  } // SPECIAL: $display makes output
  virtual bool isUnlikely() const override { return true; }
  virtual bool same(const AstNode *samep) const override {
    return displayType() ==
           static_cast<const AstDisplay *>(samep)->displayType();
  }
  virtual int instrCount() const override { return INSTR_COUNT_PLI; }
...
public:
  AstDumpCtl(FileLine *fl, VDumpCtlType ctlType, AstNode *exprp = nullptr)
      : ASTGEN_SUPER_DumpCtl(fl), m_ctlType{ctlType} {
    setNOp1p(exprp);
  }
...
public:
  ...
  virtual bool same(const AstNode *samep) const override {
    return displayType() ==
           static_cast<const AstElabDisplay *>(samep)->displayType();
  }
  virtual int instrCount() const override { return INSTR_COUNT_PLI; }
  AstDisplayType displayType() const { return m_displayType; }
  void displayType(AstDisplayType type) { m_displayType = type; }
...
public:
  ...
  AstSFormat(FileLine *fl, AstNode *lhsp, AstNode *exprsp,
             char missingArgChar = 'd')
      : ASTGEN_SUPER_SFormat(fl) {
    setOp1p(
        new AstSFormatF(fl, AstSFormatF::NoFormat(), exprsp, missingArgChar));
...
class AstSysFuncAsTask final : public AstNodeStmt {
  // Call what is normally a system function (with a return) in a non-return
  // context Parents: stmtlist Children: a system function
public:
...
public:
  AstSysIgnore(FileLine *fl, AstNode *exprsp) : ASTGEN_SUPER_SysIgnore(fl) {
    addNOp1p(exprsp);
  }
...
class AstFClose final : public AstNodeStmt {
  ...
  // Children: file which must be a varref
public:
  AstFClose(FileLine *fl, AstNode *filep) : ASTGEN_SUPER_FClose(fl) {
    setNOp2p(filep);
  }
...
class AstFFlush final : public AstNodeStmt {
  ...
  // Children: file which must be a varref
public:
  AstFFlush(FileLine *fl, AstNode *filep) : ASTGEN_SUPER_FFlush(fl) {
    setNOp2p(filep);
  }
...
class AstFRead final : public AstNodeMath {
  ...
  // Children: count
public:
  AstFRead(FileLine *fl, AstNode *memp, AstNode *filep, AstNode *startp,
           AstNode *countp)
  ...
  virtual bool isPure() const override {
    return false;
  } // SPECIAL: has 'visual' ordering
  virtual bool isOutputter() const override {
    return true;
  } // SPECIAL: makes output
  virtual bool cleanOut() const override { return false; }
  virtual bool same(const AstNode *samep) const override { return true; }
...
class AstFRewind final : public AstNodeMath {
  ...
  // Children: file which must be a varref
public:
  AstFRewind(FileLine *fl, AstNode *filep) : ASTGEN_SUPER_FRewind(fl) {
    setNOp2p(filep);
  }
...
class AstFTell final : public AstNodeMath {
  ...
  // Children: file which must be a varref
public:
  AstFTell(FileLine *fl, AstNode *filep) : ASTGEN_SUPER_FTell(fl) {
    setNOp2p(filep);
  }
...
public:
  AstFScanF(FileLine *fl, const string &text, AstNode *filep, AstNode *exprsp)
      : ASTGEN_SUPER_FScanF(fl), m_text{text} {
    addNOp1p(exprsp);
    setNOp2p(filep);
  }
...
public:
  AstSScanF(FileLine *fl, const string &text, AstNode *fromp, AstNode *exprsp)
      : ASTGEN_SUPER_SScanF(fl), m_text{text} {
    addNOp1p(exprsp);
    setOp2p(fromp);
  }
...
public:
  AstNodeReadWriteMem(AstType t, FileLine *fl, bool hex, AstNode *filenamep,
                      AstNode *memp, AstNode *lsbp, AstNode *msbp)
...
public:
  AstWriteMem(FileLine *fl, bool hex, AstNode *filenamep, AstNode *memp,
              AstNode *lsbp, AstNode *msbp)
      : ASTGEN_SUPER_WriteMem(fl, hex, filenamep, memp, lsbp, msbp) {}
  ASTNODE_NODE_FUNCS(WriteMem)
  virtual string verilogKwd() const override {
    return (isHex() ? "$writememh" : "$writememb");
  }
...
public:
  AstMonitorOff(FileLine *fl, bool off)
      : ASTGEN_SUPER_MonitorOff(fl), m_off{off} {}
  ASTNODE_NODE_FUNCS(MonitorOff)
  virtual string verilogKwd() const override {
    return m_off ? "$monitoroff" : "$monitoron";
  }
  virtual bool isGateOptimizable() const override {
    return false;
  } // Though deleted before opt
...
class AstSystemT final : public AstNodeStmt {
  // $system used as task
public:
  AstSystemT(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_SystemT(fl) {
    setOp1p(lhsp);
  }
...
public:
  AstSystemF(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_SystemF(fl) {
    setOp1p(lhsp);
  }
...
class AstValuePlusArgs final : public AstNodeMath {
  // Parents: expr
  // Child: variable to set.  If nullptr then this is a $test$plusargs instead
  // of $value$plusargs
public:
...
public:
  AstGenFor(FileLine *fl, AstNode *initsp, AstNode *condp, AstNode *incsp,
            AstNode *bodysp)
...
public:
  AstWhile(FileLine *fl, AstNode *condp, AstNode *bodysp,
  ...
  AstNode *incsp() const {
    return op4p();
  } // op4 = increment (if from a FOR loop)
...
public:
  explicit AstBreak(FileLine *fl) : ASTGEN_SUPER_Break(fl) {}
...
public:
  explicit AstContinue(FileLine *fl) : ASTGEN_SUPER_Continue(fl) {}
...
public:
  AstDisable(FileLine *fl, const string &name)
      : ASTGEN_SUPER_Disable(fl), m_name{name} {}
...
public:
  explicit AstDisableFork(FileLine *fl) : ASTGEN_SUPER_DisableFork(fl) {}
...
public:
  explicit AstWaitFork(FileLine *fl) : ASTGEN_SUPER_WaitFork(fl) {}
...
private:
  ...
  int m_labelNum =
      0; // Set by V3EmitCSyms to tell final V3Emit what to increment
public:
  // After construction must call ->labelp to associate with appropriate label
  AstJumpBlock(FileLine *fl, AstNode *stmtsp) : ASTGEN_SUPER_JumpBlock(fl) {
    addNOp1p(stmtsp);
  }
  virtual const char *broken() const override;
  virtual void cloneRelink() override;
...
public:
  AstJumpLabel(FileLine *fl, AstJumpBlock *blockp)
      : ASTGEN_SUPER_JumpLabel(fl), m_blockp{blockp} {}
...
public:
  AstJumpGo(FileLine *fl, AstJumpLabel *labelp)
      : ASTGEN_SUPER_JumpGo(fl), m_labelp{labelp} {}
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f^ %r)"; }
  virtual string emitC() override {
    return "VL_CHANGEXOR_%li(%lw, %P, %li, %ri)";
  }
...
public:
  AstConsAssoc(FileLine *fl, AstNode *defaultp) : ASTGEN_SUPER_ConsAssoc(fl) {
    setNOp1p(defaultp);
  }
...
class AstConsDynArray final : public AstNodeMath {
  ...
  // Children: expression (elements or other queues)
public:
  explicit AstConsDynArray(FileLine *fl, AstNode *lhsp = nullptr,
...
class AstConsQueue final : public AstNodeMath {
  ...
  // Children: expression (elements or other queues)
public:
  explicit AstConsQueue(FileLine *fl, AstNode *lhsp = nullptr,
                        AstNode *rhsp = nullptr)
...
public:
  ...
  AstBegin(FileLine *fl, const string &name, AstNode *stmtsp,
           bool generate = false, bool implied = false)
      : ASTGEN_SUPER_Begin(fl, name, stmtsp),
        m_generate{generate}, m_implied{implied} {}
...
public:
  ...
  AstNode *exprp() const {
    return op1p();
  } // op1 = LHS expression to compare with
...
class AstInitItem final : public AstNode {
  ...
  // This container is present so that the value underneath may get replaced
  // with a new nodep and the upper AstInitArray's map will remain correct
  // (pointing to this InitItem)
public:
  // Parents: INITARRAY
  AstInitItem(FileLine *fl, AstNode *valuep) : ASTGEN_SUPER_InitItem(fl) {
    addOp1p(valuep);
  }
...
class AstInitArray final : public AstNode {
  ...
  // If default is specified, the vector may be sparse, and not provide each
  // value. Key values are C++ array style, with lo() at index 0 Parents:
  // ASTVAR::init() Children: AstInitItem
public:
...
public:
  ...
  virtual void dump(std::ostream &str) const override;
  virtual const char *broken() const override {
    for (KeyItemMap::const_iterator it = m_map.begin(); it != m_map.end();
         ++it) {
...
class AstNew final : public AstNodeFTaskRef {
  // New as constructor
  // Don't need the class we are extracting from, as the "fromp()"'s datatype
  // can get us to it Parents: math|stmt Children: varref|arraysel, math
public:
...
class AstNewCopy final : public AstNodeMath {
  ...
  // Children: varref|arraysel, math
public:
  AstNewCopy(FileLine *fl, AstNode *rhsp) : ASTGEN_SUPER_NewCopy(fl) {
...
public:
  ...
  AstPragma(FileLine *fl, AstPragmaType pragType)
      : ASTGEN_SUPER_Pragma(fl), m_pragType{pragType} {}
...
class AstPrintTimeScale final : public AstNodeStmt {
  ...
  VTimescale m_timeunit; // Parent module time unit
public:
  explicit AstPrintTimeScale(FileLine *fl) : ASTGEN_SUPER_PrintTimeScale(fl) {}
...
public:
  AstStop(FileLine *fl, bool maybe) : ASTGEN_SUPER_Stop(fl) {}
...
public:
  explicit AstFinish(FileLine *fl) : ASTGEN_SUPER_Finish(fl) {}
...
public:
  AstNullCheck(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_NullCheck(fl, lhsp) {
    dtypeFrom(lhsp);
  }
...
class AstTimeFormat final : public AstNodeStmt {
  // Parents: stmtlist
public:
  AstTimeFormat(FileLine *fl, AstNode *unitsp, AstNode *precisionp,
                AstNode *suffixp, AstNode *widthp)
...
private:
  uint32_t m_code =
      0; // Trace identifier code; converted to ASCII by trace routines
...
public:
  AstTraceDecl(FileLine *fl, const string &showname,
               ...
               AstNode *valuep, const VNumRange &bitRange,
               const VNumRange &arrayRange, bool isScoped)
      : ASTGEN_SUPER_TraceDecl(fl), m_showname{showname}, m_bitRange{bitRange},
        ...
        ,
        m_varType{varp->varType()}, m_declKwd{varp->declKwd()},
        m_declDirection{varp->declDirection()}, m_isScoped{isScoped} {
    dtypeFrom(valuep);
    addNOp1p(valuep);
  }
...
private:
  ...
  const uint32_t m_baseCode; // Trace code base value in function containing
                             // this AstTraceInc

public:
  AstTraceInc(FileLine *fl, AstTraceDecl *declp, bool full,
              uint32_t baseCode = 0)
      : ASTGEN_SUPER_TraceInc(fl), m_declp{declp}, m_full{full}, m_baseCode{
                                                                     baseCode} {
    dtypeFrom(declp);
...
public:
  explicit AstScopeName(FileLine *fl) : ASTGEN_SUPER_ScopeName(fl) {
    dtypeSetUInt64();
  }
  ...
  string
  scopePrettySymName() const { // Name for __Vscope variable including children
    return scopePrettyNameFormatter(scopeAttrp());
  }
  string
  scopePrettyDpiName() const { // Name for __Vscope variable including children
...
public:
  AstUdpTable(FileLine *fl, AstNode *bodysp) : ASTGEN_SUPER_UdpTable(fl) {
    addNOp1p(bodysp);
  }
...
public:
  AstUdpTableLine(FileLine *fl, const string &text)
      : ASTGEN_SUPER_UdpTableLine(fl), m_text{text} {}
...
public:
  ...
  AstRand(FileLine *fl, Reset, AstNodeDType *dtp, bool reset)
      : ASTGEN_SUPER_Rand(fl), m_reset{reset} {
    dtypep(dtp);
  }
  AstRand(FileLine *fl, AstNode *seedp, bool urandom)
      : ASTGEN_SUPER_Rand(fl), m_urandom(urandom) {
    setNOp1p(seedp);
  }
...
public:
  AstTime(FileLine *fl, const VTimescale &timeunit)
      : ASTGEN_SUPER_Time(fl), m_timeunit{timeunit} {
    dtypeSetUInt64();
  }
...
public:
  AstTimeD(FileLine *fl, const VTimescale &timeunit)
      : ASTGEN_SUPER_TimeD(fl), m_timeunit{timeunit} {
    dtypeSetDouble();
  }
...
class AstUCFunc final : public AstNodeMath {
  // User's $c function
  // Perhaps this should be an AstNodeListop; but there's only one list math
  // right now
public:
  AstUCFunc(FileLine *fl, AstNode *exprsp) : ASTGEN_SUPER_UCFunc(fl) {
    addNOp1p(exprsp);
  }
  ...
  virtual bool isPure() const override {
    return false;
  } // SPECIAL: User may order w/other sigs
  virtual bool isOutputter() const override { return true; }
  virtual bool isGateOptimizable() const override { return false; }
...
public:
  AstNegate(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Negate(fl, lhsp) {
    dtypeFrom(lhsp);
  }
...
public:
  AstNegateD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_NegateD(fl, lhsp) {
    dtypeSetDouble();
  }
...
public:
  AstRedAnd(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_RedAnd(fl, lhsp) {
    dtypeSetBit();
  }
  ...
  virtual string emitVerilog() override { return "%f(& %l)"; }
  virtual string emitC() override {
    return "VL_REDAND_%nq%lq(%nw,%lw, %P, %li)";
  }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool sizeMattersLhs() const override { return false; }
...
public:
  AstRedOr(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_RedOr(fl, lhsp) {
    dtypeSetBit();
  }
...
public:
  AstRedXor(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_RedXor(fl, lhsp) {
    dtypeSetBit();
  }
...
class AstLenN final : public AstNodeUniop {
  // Length of a string
public:
  AstLenN(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_LenN(fl, lhsp) {
    dtypeSetSigned32();
  }
...
public:
  AstLogNot(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_LogNot(fl, lhsp) {
    dtypeSetBit();
  }
  ...
  virtual string emitVerilog() override { return "%f(! %l)"; }
  virtual string emitC() override {
    return "VL_LOGNOT_%nq%lq(%nw,%lw, %P, %li)";
  }
  virtual string emitSimpleOperator() override { return "!"; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  AstNot(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Not(fl, lhsp) {
    dtypeFrom(lhsp);
  }
...
class AstExtend final : public AstNodeUniop {
  // Expand a value into a wider entity by 0 extension.  Width is implied from
  // nodep->width()
public:
  AstExtend(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Extend(fl, lhsp) {}
  ...
  virtual string emitVerilog() override { return "%l"; }
  virtual string emitC() override {
    return "VL_EXTEND_%nq%lq(%nw,%lw, %P, %li)";
  }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
class AstExtendS final : public AstNodeUniop {
  // Expand a value into a wider entity by sign extension.  Width is implied
  // from nodep->width()
public:
  AstExtendS(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_ExtendS(fl, lhsp) {}
  AstExtendS(FileLine *fl, AstNode *lhsp, int width)
      // Important that widthMin be correct, as opExtend requires it after
      // V3Expand
  ...
  virtual string emitVerilog() override { return "%l"; }
  virtual string emitC() override {
    return "VL_EXTENDS_%nq%lq(%nw,%lw, %P, %li)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool sizeMattersLhs() const override {
...
class AstSigned final : public AstNodeUniop {
  // $signed(lhs)
public:
  AstSigned(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Signed(fl, lhsp) {
    UASSERT_OBJ(!v3Global.assertDTypesResolved(), this,
                "not coded to create after dtypes resolved");
  }
...
class AstUnsigned final : public AstNodeUniop {
  // $unsigned(lhs)
public:
  AstUnsigned(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Unsigned(fl, lhsp) {
    UASSERT_OBJ(!v3Global.assertDTypesResolved(), this,
                "not coded to create after dtypes resolved");
  }
...
class AstRToIS final : public AstNodeUniop {
  // $rtoi(lhs)
public:
  AstRToIS(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_RToIS(fl, lhsp) {
    dtypeSetSigned32();
  }
...
class AstRToIRoundS final : public AstNodeUniop {
  // Convert real to integer, with arbitrary sized output (not just "integer"
  // format)
public:
...
public:
  AstIToRD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_IToRD(fl, lhsp) {
    dtypeSetDouble();
  }
...
public:
  AstISToRD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_ISToRD(fl, lhsp) {
    dtypeSetDouble();
  }
...
public:
  AstCLog2(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_CLog2(fl, lhsp) {}
...
public:
  ...
  AstCountBits(FileLine *fl, AstNode *exprp, AstNode *ctrl1p, AstNode *ctrl2p)
      : ASTGEN_SUPER_CountBits(fl, exprp, ctrl1p, ctrl2p,
                               ctrl2p->cloneTree(false)) {}
  AstCountBits(FileLine *fl, AstNode *exprp, AstNode *ctrl1p, AstNode *ctrl2p,
               AstNode *ctrl3p)
      : ASTGEN_SUPER_CountBits(fl, exprp, ctrl1p, ctrl2p, ctrl3p) {}
  ASTNODE_NODE_FUNCS(CountBits)
  virtual void numberOperate(V3Number &out, const V3Number &expr,
                             const V3Number &ctrl1, const V3Number &ctrl2,
                             const V3Number &ctrl3) override {
    out.opCountBits(expr, ctrl1, ctrl2, ctrl3);
  }
  virtual string emitVerilog() override {
    return "%f$countbits(%l, %r, %f, %o)";
  }
...
public:
  AstIsUnknown(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_IsUnknown(fl, lhsp) {
    dtypeSetBit();
  }
...
public:
  AstOneHot(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_OneHot(fl, lhsp) {
    dtypeSetBit();
  }
...
public:
  AstOneHot0(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_OneHot0(fl, lhsp) {
    dtypeSetBit();
  }
...
class AstCast final : public AstNode {
  // Cast to appropriate data type - note lhsp is value, to match AstTypedef,
  // AstCCast, etc
public:
...
class AstCastParse final : public AstNode {
  // Cast to appropriate type, where we haven't determined yet what the data
  // type is
public:
...
class AstCastSize final : public AstNode {
  // Cast to specific size; signed/twostate inherited from lower element per
  // IEEE
public:
...
public:
  AstFEof(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_FEof(fl, lhsp) {}
  ...
  virtual string emitC() override {
    return "(%li ? feof(VL_CVT_I_FP(%li)) : true)";
  }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool sizeMattersLhs() const override { return false; }
...
public:
  AstFGetC(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_FGetC(fl, lhsp) {}
  ...
  virtual string emitC() override {
    return "(%li ? fgetc(VL_CVT_I_FP(%li)) : -1)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool sizeMattersLhs() const override { return false; }
...
public:
  AstLogD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_LogD(fl, lhsp) {}
...
public:
  AstLog10D(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_Log10D(fl, lhsp) {}
...
public:
  AstExpD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_ExpD(fl, lhsp) {}
...
public:
  AstSqrtD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_SqrtD(fl, lhsp) {}
...
public:
  AstFloorD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_FloorD(fl, lhsp) {}
...
public:
  AstCeilD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_CeilD(fl, lhsp) {}
...
public:
  AstSinD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_SinD(fl, lhsp) {}
...
public:
  AstCosD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_CosD(fl, lhsp) {}
...
public:
  AstTanD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_TanD(fl, lhsp) {}
...
public:
  AstAsinD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_AsinD(fl, lhsp) {}
...
public:
  AstAcosD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_AcosD(fl, lhsp) {}
...
public:
  AstAtanD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_AtanD(fl, lhsp) {}
...
public:
  AstSinhD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_SinhD(fl, lhsp) {}
...
public:
  AstCoshD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_CoshD(fl, lhsp) {}
...
public:
  AstTanhD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_TanhD(fl, lhsp) {}
...
public:
  AstAsinhD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_AsinhD(fl, lhsp) {}
...
public:
  AstAcoshD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_AcoshD(fl, lhsp) {}
...
public:
  AstAtanhD(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_AtanhD(fl, lhsp) {}
...
class AstToLowerN final : public AstNodeUniop {
  // string.tolower()
public:
  AstToLowerN(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_ToLowerN(fl, lhsp) {
    dtypeSetString();
  }
...
class AstToUpperN final : public AstNodeUniop {
  // string.toupper()
public:
  AstToUpperN(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_ToUpperN(fl, lhsp) {
    dtypeSetString();
  }
...
class AstTimeImport final : public AstNodeUniop {
  // Take a constant that represents a time and needs conversion based on time
  // units
  VTimescale m_timeunit; // Parent module time unit
public:
...
public:
  AstAtoN(FileLine *fl, AstNode *lhsp, FmtType fmt)
      : ASTGEN_SUPER_AtoN(fl, lhsp), m_fmt{fmt} {
  ...
  virtual string emitC() override {
    switch (m_fmt) {
    case ATOI:
      return "VL_ATOI_N(%li, 10)";
    case ATOHEX:
      return "VL_ATOI_N(%li, 16)";
    case ATOOCT:
      return "VL_ATOI_N(%li, 8)";
    case ATOBIN:
      return "VL_ATOI_N(%li, 2)";
    case ATOREAL:
      return "std::atof(%li.c_str())";
    default:
      V3ERROR_NA;
    }
  }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f|| %r)"; }
  virtual string emitC() override {
    return "VL_LOGOR_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return "||"; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f&& %r)"; }
  virtual string emitC() override {
    return "VL_LOGAND_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return "&&"; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f<-> %r)"; }
  virtual string emitC() override {
    return "VL_LOGEQ_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return "<->"; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f-> %r)"; }
  virtual string emitC() override {
    return "VL_LOGIF_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return "->"; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f< %r)"; }
  virtual string emitC() override {
    return "VL_LTS_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f> %r)"; }
  virtual string emitC() override {
    return "VL_GTS_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f>= %r)"; }
  virtual string emitC() override {
    return "VL_GTES_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f<= %r)"; }
  virtual string emitC() override {
    return "VL_LTES_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f<< %r)"; }
  virtual string emitC() override {
    return "VL_SHIFTL_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f>> %r)"; }
  virtual string emitC() override {
    return "VL_SHIFTR_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override {
    return (rhsp()->isWide() || rhsp()->isQuad()) ? "" : ">>";
  }
...
public:
  AstShiftRS(FileLine *fl, AstNode *lhsp, AstNode *rhsp, int setwidth = 0)
    ...
    // V3Expand
    if (setwidth)
      dtypeSetLogicSized(setwidth, VSigning::SIGNED);
  }
  ...
  virtual string emitVerilog() override { return "%k(%l %f>>> %r)"; }
  virtual string emitC() override {
    return "VL_SHIFTRS_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f* %r)"; }
  virtual string emitC() override {
    return "VL_MULS_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool emitCheckMaxWords() override { return true; }
  virtual bool cleanOut() const override { return false; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f/ %r)"; }
  virtual string emitC() override {
    return "VL_DIV_%nq%lq%rq(%lw, %P, %li, %ri)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
  ...
  virtual bool sizeMattersRhs() const override { return true; }
  virtual int instrCount() const override {
    return widthInstrs() * INSTR_COUNT_INT_DIV;
  }
};
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f/ %r)"; }
  virtual string emitC() override {
    return "VL_DIVS_%nq%lq%rq(%lw, %P, %li, %ri)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f%% %r)"; }
  virtual string emitC() override {
    return "VL_MODDIV_%nq%lq%rq(%lw, %P, %li, %ri)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f%% %r)"; }
  virtual string emitC() override {
    return "VL_MODDIVS_%nq%lq%rq(%lw, %P, %li, %ri)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%k(%l %f** %r)"; }
  virtual string emitC() override {
    return "VL_POW_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual bool emitCheckMaxWords() override { return true; }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  ...
  virtual bool sizeMattersRhs() const override { return false; }
  virtual int instrCount() const override {
    return widthInstrs() * INSTR_COUNT_INT_MUL * 10;
  }
};
...
public:
  ...
  virtual string emitC() override {
    return "VL_CONCAT_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
...
public:
  ...
  virtual string emitC() override {
    return "VL_REPLICATE_%nq%lq%rq(%nw,%lw,%rw, %P, %li, %ri)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
...
public:
  ...
  virtual string emitVerilog() override { return "%f{%r{%k%l}}"; }
  virtual string emitC() override {
    return "VL_REPLICATEN_NN%rq(0,0,%rw, %li, %ri)";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return true; }
  virtual bool cleanRhs() const override { return true; }
...
public:
  ...
  virtual string emitC() override {
    return isWide() ? "VL_ASSIGN_W(%nw, %P, %li)" : "%li";
  }
  virtual bool cleanOut() const override { return false; }
  virtual bool cleanLhs() const override { return false; }
  virtual bool cleanRhs() const override { return false; }
...
public:
  AstCompareNN(FileLine *fl, AstNode *lhsp, AstNode *rhsp, bool ignoreCase)
      : ASTGEN_SUPER_CompareNN(fl, lhsp, rhsp), m_ignoreCase{ignoreCase} {
    dtypeSetUInt32();
  }
  ...
  virtual string emitC() override {
    return m_ignoreCase ? "VL_CMP_NN(%li,%ri,true)"
                        : "VL_CMP_NN(%li,%ri,false)";
  }
  virtual string emitSimpleOperator() override { return ""; }
  virtual bool cleanOut() const override { return true; }
  virtual bool cleanLhs() const override { return true; }
...
class AstFell final : public AstNodeMath {
  ...
  // Children: expression
public:
  AstFell(FileLine *fl, AstNode *exprp) : ASTGEN_SUPER_Fell(fl) {
    addOp1p(exprp);
  }
  ...
  AstSenTree *sentreep() const {
    return VN_AS(op2p(), SenTree);
  } // op2 = clock domain
  void sentreep(AstSenTree *sentreep) {
    addOp2p(sentreep);
  } // op2 = clock domain
  virtual bool same(const AstNode *samep) const override { return true; }
};
...
class AstRose final : public AstNodeMath {
  ...
  // Children: expression
public:
  AstRose(FileLine *fl, AstNode *exprp) : ASTGEN_SUPER_Rose(fl) {
    addOp1p(exprp);
  }
...
class AstSampled final : public AstNodeMath {
  ...
  // Children: expression
public:
  AstSampled(FileLine *fl, AstNode *exprp) : ASTGEN_SUPER_Sampled(fl) {
    addOp1p(exprp);
  }
...
class AstStable final : public AstNodeMath {
  ...
  // Children: expression
public:
  AstStable(FileLine *fl, AstNode *exprp) : ASTGEN_SUPER_Stable(fl) {
    addOp1p(exprp);
  }
...
public:
  AstPattern(FileLine *fl, AstNode *itemsp) : ASTGEN_SUPER_Pattern(fl) {
    addNOp2p(itemsp);
  }
  ...
  AstNodeDType *childDTypep() const { return VN_AS(op1p(), NodeDType); }
  void childDTypep(AstNodeDType *nodep) { setOp1p(nodep); }
  AstNode *itemsp() const {
    return op2p();
  } // op2 = AstPatReplicate, AstPatMember, etc
};
...
public:
  ...
  AstNode *keyp() const {
    return op2p();
  } // op2 = assignment key (Const, id Text)
  AstNode *repp() const {
    return op3p();
  } // op3 = replication count, or nullptr for count 1
...
public:
  AstPropClocked(FileLine *fl, AstSenItem *sensesp, AstNode *disablep,
                 AstNode *propp)
...
public:
  AstNodeCoverOrAssert(AstType t, FileLine *fl, AstNode *propp, AstNode *passsp,
      ...
      : AstNodeStmt{t, fl}, m_immediate{immediate}, m_name{name} {
    addOp1p(propp);
    addNOp4p(passsp);
  }
  ...
  AstNode *passsp() const {
    return op4p();
  } // op4 = statements (assert/cover passes)
...
class AstAssertIntrinsic final : public AstNodeCoverOrAssert {
  // A $cast or other compiler inserted assert, that must run even without
  // --assert option
public:
  ASTNODE_NODE_FUNCS(AssertIntrinsic)
  AstAssertIntrinsic(FileLine *fl, AstNode *propp, AstNode *passsp,
                     AstNode *failsp, bool immediate, const string &name = "")
...
public:
  AstNodeSimpleText(AstType t, FileLine *fl, const string &textp,
                    bool tracking = false)
...
public:
  explicit AstTextBlock(FileLine *fl, const string &textp = "",
                        bool tracking = false, bool commas = false)
      : ASTGEN_SUPER_TextBlock(fl, textp, tracking), m_commas(commas) {}
...
public:
  AstScHdr(FileLine *fl, const string &textp) : ASTGEN_SUPER_ScHdr(fl, textp) {}
...
public:
  AstScImp(FileLine *fl, const string &textp) : ASTGEN_SUPER_ScImp(fl, textp) {}
...
public:
  AstScInt(FileLine *fl, const string &textp) : ASTGEN_SUPER_ScInt(fl, textp) {}
...
class AstUCStmt final : public AstNodeStmt {
  // User $c statement
public:
  AstUCStmt(FileLine *fl, AstNode *exprsp) : ASTGEN_SUPER_UCStmt(fl) {
    addNOp1p(exprsp);
  }
...
public:
  AstNodeFile(AstType t, FileLine *fl, const string &name) : AstNode(t, fl) {
    m_name = name;
  }
...
class AstVFile final : public AstNodeFile {
  ...
  // Parents:  NETLIST
public:
  AstVFile(FileLine *fl, const string &name) : ASTGEN_SUPER_VFile(fl, name) {}
...
public:
  AstCFile(FileLine *fl, const string &name)
      : ASTGEN_SUPER_CFile(fl, name), m_slow{false}, m_source{false},
        m_support{false} {}
...
private:
  ...
  bool m_dontCombine : 1;   // V3Combine shouldn't compare this func tree, it's
                            // special
  bool m_declPrivate : 1;   // Declare it private
  bool m_formCallTree : 1;  // Make a global function to call entire tree of
                            // functions
  ...
  bool m_dpiExportDispatcher : 1; // This is the DPI export entry point (i.e.:
                                  // called by user)
  bool m_dpiExportImpl : 1;       // DPI export implementation (called from DPI
                                  // dispatcher via lookup)
  bool m_dpiImportPrototype : 1;  // This is the DPI import prototype (i.e.:
                                  // provided by user)
  bool m_dpiImportWrapper : 1; // Wrapper for invoking DPI import prototype from
                               // generated code
  bool m_dpiContext : 1; // Declared as 'context' DPI import/export function
public:
  AstCFunc(FileLine *fl, const string &name, AstScope *scopep,
  ...
  virtual bool same(const AstNode *samep) const override {
    ...
    return ((isTrace() == asamep->isTrace()) &&
            (rtnTypeVoid() == asamep->rtnTypeVoid()) &&
            (argTypes() == asamep->argTypes()) &&
            (ctorInits() == asamep->ctorInits()) &&
            isLoose() == asamep->isLoose() &&
            (!(dpiImportPrototype() || dpiExportImpl()) ||
             name() == asamep->name()));
  }
  //
  virtual void name(const string &name) override { m_name = name; }
  ...
  string rtnTypeVoid() const {
    return ((m_rtnType == "") ? "void" : m_rtnType);
  }
...
class AstCMethodCall final : public AstNodeCCall {
  ...
  // Children: Args to the function
public:
  AstCMethodCall(FileLine *fl, AstNode *fromp, AstCFunc *funcp,
...
class AstCReturn final : public AstNodeStmt {
  ...
  // Children: Math
public:
  AstCReturn(FileLine *fl, AstNode *lhsp) : ASTGEN_SUPER_CReturn(fl) {
    setOp1p(lhsp);
  }
...
public:
  ...
  AstCMath(FileLine *fl, AstNode *exprsp)
      : ASTGEN_SUPER_CMath(fl), m_cleanOut{true}, m_pure{false} {
    addOp1p(exprsp);
    dtypeFrom(exprsp);
  }
  AstCMath(FileLine *fl, const string &textStmt, int setwidth,
           bool cleanOut = true)
      : ASTGEN_SUPER_CMath(fl), m_cleanOut{cleanOut}, m_pure{true} {
...
public:
  AstCReset(FileLine *fl, AstVarRef *exprsp) : ASTGEN_SUPER_CReset(fl) {
    addNOp1p(exprsp);
  }
  ...
  virtual bool isPredictOptimizable() const override { return false; }
  virtual bool same(const AstNode *samep) const override { return true; }
  AstVarRef *varrefp() const {
    return VN_AS(op1p(), VarRef);
  } // op1 = varref to reset
};
...
class AstCStmt final : public AstNodeStmt {
  // Emit C statement
public:
  AstCStmt(FileLine *fl, AstNode *exprsp) : ASTGEN_SUPER_CStmt(fl) {
    addNOp1p(exprsp);
  }
  AstCStmt(FileLine *fl, const string &textStmt) : ASTGEN_SUPER_CStmt(fl) {
    addNOp1p(new AstText(fl, textStmt, true));
  }
...
public:
  AstCUse(FileLine *fl, VUseType useType, const string &name)
      : ASTGEN_SUPER_CUse(fl), m_useType{useType}, m_name{name} {}
...
public:
  explicit AstMTaskBody(FileLine *fl) : ASTGEN_SUPER_MTaskBody(fl) {}
...
public:
  ...
  AstBasicDType *findLogicBitDType(FileLine *fl, AstBasicDTypeKwd kwd,
                                   int width, int widthMin, VSigning numeric);
  AstBasicDType *findLogicBitDType(FileLine *fl, AstBasicDTypeKwd kwd,
...
class AstConstPool final : public AstNode {
  ...
  std::unordered_multimap<uint32_t, AstVarScope *>
      m_tables; // Constant tables (unpacked arrays)
  std::unordered_multimap<uint32_t, AstVarScope *>
      m_consts;             // Constant tables (scalars)
...
public:
  ...
  // Find a table (unpacked array) within the constant pool which is initialized
  // with the given value, or create one if one does not already exists. The
  // returned VarScope *might* have a different dtype than the given
  // initp->dtypep(), including a different element type, but it will always
  // have the same size and element width. In contexts where this matters, the
  // caller must handle the dtype difference as appropriate.
  AstVarScope *findTable(AstInitArray *initp);
  // Find a constant within the constant pool which is initialized with the
  // given value, or create one if one does not already exists. If 'mergeDType'
  // is true, then the returned VarScope *might* have a different type than the
  // given initp->dtypep(). In contexts where this matters, the caller must
  // handle the dtype difference as appropriate. If 'mergeDType' is false, the
  // returned VarScope will have _->dtypep()->sameTree(initp->dtypep()) return
  // true.
...
private:
  AstTypeTable
      *const m_typeTablep; // Reference to top type table, for faster lookup
  AstConstPool
      *const m_constPoolp; // Reference to constant pool, for faster lookup
  AstPackage *m_dollarUnitPkgp = nullptr; // $unit
  AstCFunc *m_evalp = nullptr;            // The '_eval' function
  AstExecGraph *m_execGraphp =
      nullptr; // Execution MTask graph for threads>1 mode
  AstVarScope *m_dpiExportTriggerp = nullptr; // The DPI export trigger variable
  AstTopScope *m_topScopep =
      nullptr;                // The singleton AstTopScope under the top module
...
public:
  ...
  AstNodeFile *filesp() const {
    return VN_AS(op2p(), NodeFile);
  } // op2 = List of files
  ...
  AstPackage *dollarUnitPkgAddp() {
    if (!m_dollarUnitPkgp) {
      m_dollarUnitPkgp =
          new AstPackage(fileline(), AstPackage::dollarUnitName());
