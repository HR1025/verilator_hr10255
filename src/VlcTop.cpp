  ...
  while (!is.eof()) {
    ...
    if (line[0] == 'C') {
      ...
      for (; secspace < line.length(); secspace++) {
        if (line[secspace] == '\'' && line[secspace + 1] == ' ')
          break;
      }
...
void VlcTop::rank() {
  ...
  for (const auto &i : m_points) {
    ...
    if (pointp->testsCovering())
      remaining.addData(pointp->pointNum(), 1);
  }
...
void VlcTop::annotateCalc() {
  ...
  for (const auto &i : m_points) {
    ...
    if (!filename.empty() && lineno != 0) {
      ...
      unsigned thresh = (!threshStr.empty()) ? std::atoi(threshStr.c_str())
                                             : opt.annotateMin();
      const bool ok = (point.count() >= thresh);
      UINFO(9, "AnnoCalc count " << filename << ":" << lineno << ":"
                                 << point.column() << " " << point.count()
                                 << " " << point.linescov() << '\n');
      // Base coverage
      ...
      for (const char *covp = linescov.c_str(); true; ++covp) {
        if (!*covp || *covp == ',') { // Ending
          ...
          while (std::isdigit(*covp))
            ++covp;
          --covp; // Will inc in for loop
          if (!range)
            start = std::atoi(digitsp);
...
void VlcTop::annotateCalcNeeded() {
  ...
  if (totOk != totCases)
    cout << "See lines with '%00' in " << opt.annotateOut() << '\n';
}
...
void VlcTop::annotateOutputFiles(const string &dirname) {
  ...
  for (auto &si : m_sources) {
    ...
    UINFO(1,
          "annotateOutputFile " << filename << " -> " << outfilename << endl);
      ...
      if (lit != lines.end()) {
        ...
        for (auto &ci : cmap) {
          ...
          if (first) {
            ...
            line =
                indent + "verilator_coverage: (next point on previous line)\n";
          }
        }
      }
