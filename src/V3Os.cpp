...
void V3Os::setenvStr(const string &envvar, const string &value,
                     const string &why) {
  if (why != "") {
...
#else
  // setenv() replaced by putenv() in Solaris environment. Prototype is
  // different putenv() requires NAME=VALUE format
...
string V3Os::filenameFromDirBase(const string &dir, const string &basename) {
  // Don't return ./{filename} because if filename was absolute, that makes it
  // relative
  if (dir == ".") {
    return basename;
  } else {
  ...
  for (string::size_type pos = 0; pos < filename.length(); ++pos) {
    if ((filename[pos] == '$') && (pos + 1 < filename.length())) {
      ...
      if (((brackets == CURLY) && (filename[endpos + 1] != '}')) ||
          ((brackets == PAREN) && (filename[endpos + 1] != ')'))) {
        v3fatal("Unmatched brackets in variable substitution in file: " +
                filename);
      }
      const string envvar = filename.substr(pos + 1, endpos - pos);
      string envvalue;
      if (!envvar.empty())
        envvalue = getenvStr(envvar, "");
      if (!envvalue.empty()) {
        out += envvalue;
        if (brackets == NONE) {
          pos = endpos;
...
string V3Os::filenameRealPath(const string &filename) {
  ...
  // If there is a ../ that goes down from the 'root' of this path it is
  // preserved.
...
vluint64_t V3Os::rand64(std::array<vluint64_t, 2> &stater) {
  ...
  stater[1] ^= stater[0];
  stater[0] =
      (((stater[0] << 55) | (stater[0] >> 9)) ^ stater[1] ^ (stater[1] << 14));
...
string V3Os::trueRandom(size_t size) {
...
#if defined(_WIN32) || defined(__MINGW32__)
  const NTSTATUS hr = BCryptGenRandom(nullptr, reinterpret_cast<BYTE *>(data),
  ...
  if (VL_UNCOVERABLE(!is.read(data, size))) {
    v3fatal(
        "Could not open /dev/urandom, no source of randomness. " // LCOV_EXCL_LINE
...
uint64_t V3Os::memUsageBytes() {
  ...
  if (GetProcessMemoryInfo(process, &pmc, sizeof(pmc))) {
    // The best we can do using simple Windows APIs is to get the size of the
    // working set.
...
int V3Os::system(const string &command) {
  ...
  if (VL_UNCOVERABLE(ret == -1)) {
    ...
    UASSERT(WIFEXITED(ret),
            "system(" << command << ") returned unexpected value of " << ret);
