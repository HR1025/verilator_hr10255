...
private:
  ...
  // METHODS
  string assertDisplayMessage(AstNode *nodep, const string &prefix,
                              const string &message) {
    return (string("[%0t] " + prefix + ": ") +
            nodep->fileline()->filebasename() + ":" +
  ...
  void replaceDisplay(AstDisplay *nodep, const string &prefix) {
    ...
    nodep->fmtp()->text(
        assertDisplayMessage(nodep, prefix, nodep->fmtp()->text()));
  ...
  AstVarRef *newMonitorNumVarRefp(AstNode *nodep, VAccess access) {
    ...
    const auto varrefp =
        new AstVarRef(nodep->fileline(), m_monitorNumVarp, access);
  ...
  AstVarRef *newMonitorOffVarRefp(AstNode *nodep, VAccess access) {
    ...
    const auto varrefp =
        new AstVarRef(nodep->fileline(), m_monitorOffVarp, access);
    varrefp->classOrPackagep(v3Global.rootp()->dollarUnitPkgAddp());
    return varrefp;
  }
  AstNode *newIfAssertOn(AstNode *nodep, bool force) {
    ...
    AstNode *newp = new AstIf(
        ...
        (force ? new AstConst(fl, AstConst::BitTrue())
               : // If assertions are off, have constant propagation rip them
                 // out later
             // This allows syntax errors and such to be detected normally.
             (v3Global.opt.assertOn()
                  ? static_cast<AstNode *>(new AstCMath(
                        fl, "vlSymsp->_vm_contextp__->assertOn()", 1))
                  : static_cast<AstNode *>(
  ...
  AstNode *newFireAssertUnchecked(AstNode *nodep, const string &message) {
    ...
    AstDisplay *dispp = new AstDisplay(
        nodep->fileline(), AstDisplayType::DT_ERROR, message, nullptr, nullptr);
  ...
  void newPslAssertion(AstNodeCoverOrAssert *nodep, AstNode *failsp) {
    if (m_beginp && nodep->name() == "")
      nodep->name(m_beginp->name());
    ...
    if (nodep->immediate()) {
      UASSERT_OBJ(!sentreep, nodep,
                  "Immediate assertions don't have sensitivity");
    } else {
      UASSERT_OBJ(sentreep, nodep,
                  "Concurrent assertions must have sensitivity");
    ...
    if (AstCover *snodep = VN_CAST(nodep, Cover)) {
      ...
      if (!v3Global.opt.coverageUser()) {
        ...
        covincp
            ->unlinkFrBackWithNext(); // next() might have  AstAssign for trace
        if (message != "")
          covincp->declp()->comment(message);
        bodysp = covincp;
      }

      if (bodysp && passsp)
        bodysp = bodysp->addNext(passsp);
      ...
      if (passsp)
        passsp = newIfAssertOn(passsp, force);
      if (failsp)
        failsp = newIfAssertOn(failsp, force);
      if (!failsp)
        failsp = newFireAssertUnchecked(nodep, "'assert' failed.");
    ...
    AstNode *newp;
    if (sentreep) {
      newp = new AstAlways(nodep->fileline(), VAlwaysKwd::ALWAYS, sentreep,
                           bodysp);
    } else {
      newp = bodysp;
    }
    ...
    if (selfDestruct) {
      ...
      // if it wasn't constructed nicely or has other errors without needing
      // --coverage.
  ...
  virtual void visit(AstIf *nodep) override {
    ...
    if (nodep->uniquePragma() || nodep->unique0Pragma()) {
      ...
      if (!propp)
        propp = new AstConst(nodep->fileline(), AstConst::BitFalse());

      // Note: if this ends with an 'else', then we don't need to validate that
      // one of the predicates evaluates to true.
      AstNode *ohot = ((allow_none || hasDefaultElse)
                           ? static_cast<AstNode *>(
                                 new AstOneHot0(nodep->fileline(), propp))
                           : static_cast<AstNode *>(
                                 new AstOneHot(nodep->fileline(), propp)));
      AstIf *checkifp = new AstIf(
      ...
      if (nodep->fullPragma() || nodep->priorityPragma()) {
        ...
        if (!has_default) {
          nodep->addItemsp(new AstCaseItem(
              ...
              newFireAssert(nodep,
                            "synthesis full_case, but non-match found")));
        }
      }
      if (nodep->parallelPragma() || nodep->uniquePragma() ||
          nodep->unique0Pragma()) {
        // Need to check that one, and only one of the case items match at any
        // moment If there's a default, we allow none to match, else exactly one
        // must match
        ++m_statAsFull;
        if (!has_default && !nodep->itemsp()) {
            ...
            for (AstNode *icondp = itemp->condsp(); icondp;
              ...
              if (AstInsideRange *rcondp = VN_CAST(icondp, InsideRange)) {
                onep = rcondp->newAndFromInside(
                    nodep->exprp(), rcondp->lhsp()->cloneTree(true),
                    rcondp->rhsp()->cloneTree(true));
              } else if (nodep->casex() || nodep->casez() ||
                         nodep->caseInside()) {
          ...
          AstNode *ohot =
              (allow_none ? static_cast<AstNode *>(
                                new AstOneHot0(nodep->fileline(), propp))
                          : static_cast<AstNode *>(
                                new AstOneHot(nodep->fileline(), propp)));
          AstIf *ifp = new AstIf(
              nodep->fileline(), new AstLogNot(nodep->fileline(), ohot),
              newFireAssert(
                  nodep, "synthesis parallel_case, but multiple matches found"),
              nullptr);
  ...
  virtual void visit(AstPast *nodep) override {
    ...
    UASSERT_OBJ(ticks >= 1, nodep,
                "0 tick should have been checked in V3Width");
    ...
    for (uint32_t i = 0; i < ticks; ++i) {
      ...
      AstNode *assp = new AstAssignDly(
          nodep->fileline(),
          new AstVarRef(nodep->fileline(), outvarp, VAccess::WRITE), inp);
  ...
  virtual void visit(AstDisplay *nodep) override {
    ...
    if (nodep->displayType() == AstDisplayType::DT_INFO) {
      ...
      const auto newsetp =
          new AstAssign{fl, newMonitorNumVarRefp(nodep, VAccess::WRITE),
      ...
      AstIf *ifp = new AstIf{
          fl,
          new AstLogAnd{
              fl, new AstLogNot{fl, newMonitorOffVarRefp(nodep, VAccess::READ)},
      ...
      // Need one-shot
      const auto fl = nodep->fileline();
      const auto varp = new AstVar{fl, AstVarType::MODULETEMP,
                                   "__Vstrobe" + cvtToStr(m_modStrobeNum++),
                                   nodep->findBitDType()};
      ...
      nodep->replaceWith(newsetp);
      // Add "always_comb if (__Vstrobe) begin $display(...); __Vstrobe = '0;
      // end"
      AstNode *stmtsp = nodep;
      AstIf *ifp = new AstIf{fl, new AstVarRef{fl, varp, VAccess::READ}, stmtsp,
                             nullptr};
      ifp->branchPred(VBranchPred::BP_UNLIKELY);
      AstNode *newp = new AstAlwaysPostponed{fl, ifp};
  ...
  virtual void visit(AstMonitorOff *nodep) override {
    const auto newp = new AstAssign(
        nodep->fileline(), newMonitorOffVarRefp(nodep, VAccess::WRITE),
...
public:
  ...
  virtual ~AssertVisitor() override {
    V3Stats::addStat("Assertions, assert non-immediate statements",
                     m_statAsNotImm);
...
void V3Assert::assertAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("assert", 0,
