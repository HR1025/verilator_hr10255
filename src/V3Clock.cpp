...
//              Replace UNTILSTABLEs with loops until specified signals become
//              const.
...
private:
  ...
  void visit(AstVarRef *nodep) override {
    UASSERT_OBJ(!nodep->access().isRW(), nodep,
                "Cannot handle a READWRITE reference");
...
public:
  static AstNode *main(AstNode *nodep) {
    return ConvertWriteRefsToRead(nodep).m_result;
  }
};
...
private:
  // NODE STATE
  // Cleared each Module:
  //  AstVarScope::user1p()   -> AstVarScope*.  Temporary signal that was
  //  created.
  ...
  AstSenTree *m_lastSenp =
      nullptr; // Last sensitivity match, so we can detect duplicates.
  ...
  AstVarScope *getCreateLastClk(AstVarScope *vscp) {
    if (vscp->user1p())
      return static_cast<AstVarScope *>(vscp->user1p());
    AstVar *varp = vscp->varp();
    if (!varp->width1()) {
      varp->v3warn(E_UNSUPPORTED,
                   "Unsupported: Clock edge on non-single bit signal: "
                       << varp->prettyNameQ());
    }
    string newvarname = (string("__Vclklast__") +
                         vscp->scopep()->nameDotless() + "__" + varp->name());
    AstVar *newvarp = new AstVar(vscp->fileline(), AstVarType::MODULETEMP,
    ...
    if (v3Global.opt.xInitialEdge())
      fromp = new AstNot(fromp->fileline(), fromp);
    AstNode *newinitp = new AstAssign(
        vscp->fileline(),
        new AstVarRef(newvarp->fileline(), newvscp, VAccess::WRITE), fromp);
  ...
  AstNode *createSenItemEquation(AstSenItem *nodep) {
    ...
    if (nodep->edgeType() == VEdgeType::ET_ILLEGAL) {
      nodep->v3warn(E_UNSUPPORTED, "Unsupported: Complicated event expression "
                                   "in sensitive activity list");
      return nullptr;
    }
    ...
    if (nodep->edgeType() == VEdgeType::ET_POSEDGE) {
      ...
      newp = new AstAnd(nodep->fileline(),
                        new AstVarRef(nodep->fileline(),
                                      nodep->varrefp()->varScopep(),
                                      VAccess::READ),
                        new AstNot(nodep->fileline(),
                                   new AstVarRef(nodep->fileline(), lastVscp,
                                                 VAccess::READ)));
  ...
  AstNode *createSenseEquation(AstSenItem *nodesp) {
    ...
    for (AstSenItem *senp = nodesp; senp;
  ...
  AstIf *makeActiveIf(AstSenTree *sensesp) {
    ...
    UASSERT_OBJ(senEqnp, sensesp,
                "No sense equation, shouldn't be in sequent activation.");
  ...
  AstCFunc *makeTopFunction(const string &name, bool slow = false) {
    AstCFunc *const funcp =
        new AstCFunc{m_topScopep->fileline(), name, m_topScopep->scopep()};
  ...
  void splitCheck(AstCFunc *ofuncp) {
    if (!v3Global.opt.outputSplitCFuncs() || !ofuncp->stmtsp())
      return;
    if (EmitCBaseCounterVisitor(ofuncp->stmtsp()).count() <
    ...
    if (ofuncp->finalsp())
      tempp->addStmtsp(ofuncp->finalsp()->unlinkFrBackWithNext());
    while (tempp->stmtsp()) {
      ...
      if (!funcp || (func_stmts + stmts) > v3Global.opt.outputSplitCFuncs()) {
        // Make a new function
        funcp = new AstCFunc{ofuncp->fileline(),
                             ofuncp->name() + cvtToStr(++funcnum),
                             m_topScopep->scopep()};
        funcp->dontCombine(true);
        funcp->isStatic(false);
        funcp->isLoose(true);
  ...
  virtual void visit(AstTopScope *nodep) override {
    ...
    if (AstVarScope *const dpiExportTriggerp =
  ...
  virtual void visit(AstCoverToggle *nodep) override {
    ...
    AstNode *changeRdp =
        ConvertWriteRefsToRead::main(changeWrp->cloneTree(false));
    AstIf *newp = new AstIf(nodep->fileline(),
                            new AstXor(nodep->fileline(), origp, changeRdp),
                            incp, nullptr);
    // We could add another IF to detect posedges, and only increment if so.
    // It's another whole branch though versus a potential memory miss.
    // We'll go with the miss.
    newp->addIfsp(
        new AstAssign(nodep->fileline(), changeWrp, origp->cloneTree(false)));
...
void V3Clock::clockAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("clock", 0,
