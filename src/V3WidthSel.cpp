...
#define iterateChildren DO_NOT_iterateChildern_IN_V3WidthSel
  ...
  struct FromData {
    AstNodeDType
        *m_errp; // Node that was found, for error reporting if not known type
    AstNodeDType *m_dtypep; // Data type for the 'from' slice
    VNumRange m_fromRange;  // Numeric range bounds for the 'from' slice
    FromData(AstNodeDType *errp, AstNodeDType *dtypep,
             const VNumRange &fromRange)
  ...
  FromData fromDataForArray(AstNode *nodep, AstNode *basefromp) {
    ...
    UASSERT_OBJ(basefromp && basefromp->dtypep(), nodep,
                "Select with no from dtype");
    ...
    if (const AstNodeArrayDType *adtypep = VN_CAST(ddtypep, NodeArrayDType)) {
    ...
    } else if (const AstNodeUOrStructDType *adtypep =
      ...
      if (adtypep->isString() && VN_IS(nodep, SelBit)) {
        ...
        UASSERT_OBJ(
            ...
            nodep,
            "Non-constant variable range; errored earlier"); // in
                                                             // constifyParam(bfdtypep)
        fromRange = adtypep->declRange();
      } else {
        nodep->v3error(
            "Illegal bit or array select; type does not have a bit range, or "
            << "bad dimension: data type is " << errp->prettyDTypeNameQ());
      }
    } else {
      nodep->v3error("Illegal bit or array select; type already selected, or "
                     "bad dimension: "
  ...
  AstNode *newSubNeg(AstNode *lhsp, vlsint32_t rhs) {
    ...
    if (rhs == 0) {
      ...
      // Optional vs just making add/sub below, but saves constification some
      // work
      ...
      // We must make sure sub gets sign of original value, not from the
      // constant
      newp->dtypeFrom(lhsp);
      return newp;
    } else { // rhs < 0;
      AstNode *newp = new AstAdd(
          lhsp->fileline(), lhsp,
          new AstConst(lhsp->fileline(), AstConst::Unsized32(), -rhs));
      // We must make sure sub gets sign of original value, not from the
      // constant
  ...
  AstNode *newSubNeg(vlsint32_t lhs, AstNode *rhsp) {
    ...
    AstNode *newp = new AstSub(
        rhsp->fileline(),
        new AstConst(rhsp->fileline(), AstConst::Unsized32(), lhs), rhsp);
  ...
  AstNode *newSubLsbOf(AstNode *underp, const VNumRange &fromRange) {
    ...
    // SUB #'s Not needed when LSB==0 and MSB>=0 (ie [0:-13] must still get
    // added!)
    if (!fromRange.ranged()) {
      // vector without range, or 0 lsb is ok, for example a INTEGER x; y =
      // x[21:0];
      return underp;
    } else {
      if (fromRange.littleEndian()) {
        // reg [1:3] was swapped to [3:1] (lsbEndianedp==3) and needs a
        // SUB(3,under)
  ...
  AstNodeDType *sliceDType(AstPackArrayDType *nodep, int msb, int lsb) {
    // Return slice needed for msb/lsb, either as original dtype or a new slice
    // dtype
    if (nodep->declRange().elements() ==
            (msb - lsb + 1) // Extracting whole of original array
      ...
      AstNodeDType *vardtypep = new AstPackArrayDType(
          nodep->fileline(),
  ...
  virtual void visit(AstSelBit *nodep) override {
    ...
    if (debug() >= 9)
      nodep->backp()->dumpTree(cout, "--SELBT0: ");
    ...
    if (AstUnpackArrayDType *adtypep = VN_CAST(ddtypep, UnpackArrayDType)) {
      ...
      if (debug() >= 9)
        newp->dumpTree(cout, "--SELBTn: ");
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
    } else if (AstPackArrayDType *adtypep = VN_CAST(ddtypep, PackArrayDType)) {
      // SELBIT(array, index) -> SEL(array, index*width-of-subindex,
      // width-of-subindex)
      AstNode *subp = rhsp;
      if (fromRange.littleEndian()) {
      ...
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
                   ...
                   VN_CAST(ddtypep, AssocArrayDType)) {
      ...
      AstNode *subp = rhsp;
      AstCMethodHard *newp =
          new AstCMethodHard(nodep->fileline(), fromp, "at", subp);
      newp->dtypeFrom(
          adtypep->subDTypep()); // Need to strip off queue reference
      if (debug() >= 9)
        newp->dumpTree(cout, "--SELBTq: ");
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
      ...
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
    } else if (VN_IS(ddtypep,
                     NodeUOrStructDType)) { // A bit from the packed struct
      // SELBIT(range, index) -> SEL(array, index, 1)
      AstSel *newp =
          new AstSel(nodep->fileline(), fromp, newSubLsbOf(rhsp, fromRange),
                     // Unsized so width from user
  ...
  virtual void visit(AstSelExtract *nodep) override {
    ...
    if (VN_IS(ddtypep, UnpackArrayDType)) {
      ...
      if (lsb > msb) {
        nodep->v3warn(SELRANGE, "[" << msb << ":" << lsb
                                    << "] Range extract has backward bit "
                                       "ordering, perhaps you wanted ["
                                    << lsb << ":" << msb << "]");
        int x = msb;
        msb = lsb;
        lsb = x;
      ...
      UASSERT_OBJ(newp->widthMin() == newp->widthConst(), nodep,
                  "Width mismatch");
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
    } else if (VN_IS(ddtypep, BasicDType)) {
      ...
      AstNode *widthp =
          new AstConst(msbp->fileline(),
                       ...
                       msb + 1 - lsb);
      AstSel *newp = new AstSel(nodep->fileline(), fromp,
                                newSubLsbOf(lsbp, fromRange), widthp);
      newp->declRange(fromRange);
      UINFO(6, "   new " << newp << endl);
    ...
    if (!msbp->backp())
      VL_DO_DANGLING(pushDeletep(msbp), msbp);
    if (!lsbp->backp())
      VL_DO_DANGLING(pushDeletep(lsbp), lsbp);
  }
  ...
  void replaceSelPlusMinus(AstNodePreSel *nodep) {
    ...
    // Below 2 lines may change nodep->widthp()
    if (debug() >= 9)
      nodep->dumpTree(cout, "--SELPM0: ");
    V3Width::widthParamsEdit(
        nodep->rhsp()); // constifyEdit doesn't ensure widths finished
    V3Const::constifyEdit(
        nodep->rhsp()); // May relink pointed to node, ok if not const
    V3Const::constifyParamsEdit(nodep->thsp()); // May relink pointed to node
    checkConstantOrReplace(nodep->thsp(),
                           "Width of :+ or :- bit extract isn't a constant");
    if (debug() >= 9)
      nodep->dumpTree(cout, "--SELPM3: ");
    ...
    if (width < 0)
      nodep->v3error("Width of :+ or :- is < 0: " << widthp->prettyName());
    const FromData fromdata = fromDataForArray(nodep, fromp);
    AstNodeDType *ddtypep = fromdata.m_dtypep;
    const VNumRange fromRange = fromdata.m_fromRange;
    if (VN_IS(ddtypep, UnpackArrayDType)) {
      ...
      if (const AstPackArrayDType *adtypep = VN_CAST(ddtypep, PackArrayDType)) {
        ...
        newwidthp = new AstConst(nodep->fileline(), AstConst::Unsized32(),
                                 width * elwidth);
      }
      AstNode *newlsbp = nullptr;
      if (VN_IS(nodep, SelPlus)) {
        if (fromRange.littleEndian()) {
          // SELPLUS(from,lsb,width) -> SEL(from, (vector_msb-width+1)-sel,
          // width)
    ...
    if (!rhsp->backp())
      VL_DO_DANGLING(pushDeletep(rhsp), rhsp);
    if (!widthp->backp())
      VL_DO_DANGLING(pushDeletep(widthp), widthp);
  }
