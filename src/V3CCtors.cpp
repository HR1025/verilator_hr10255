...
//      for all AstVar, create a creates a AstCReset node in an _ctor_var_reset
//      AstCFunc. for all AstCoverDecl, move the declaration into a
//      _configure_coverage AstCFunc. For each variable that needs reset, add a
//      AstCReset node.
//
...
private:
  ...
  AstCFunc *makeNewFunc() {
    ...
    AstCFunc *const funcp =
        new AstCFunc{m_modp->fileline(), funcName, nullptr, "void"};
...
public:
  ...
  V3CCtorsBuilder(AstNodeModule *nodep, const string &basename, VCtorType type)
      : m_modp{nodep}, m_basename{basename}, m_type{type} {
    // Note: The constructor is always called, even if empty, so we must always
    // create at least one.
    m_newFunctions.push_back(makeNewFunc());
  }
...
void V3CCtors::evalAsserts() {
  AstNodeModule *const modp =
      v3Global.rootp()->modulesp(); // Top module wrapper
  AstCFunc *const funcp =
  ...
  for (AstNodeModule *modp = v3Global.rootp()->modulesp(); modp;
    ...
    // Process each module in turn
    {
      V3CCtorsBuilder var_reset{modp, "_ctor_var_reset",
                                VN_IS(modp, Class) ? VCtorType::CLASS
                                                   : VCtorType::MODULE};

      for (AstNode *np = modp->stmtsp(); np; np = np->nextp()) {
        if (AstVar *const varp = VN_CAST(np, Var)) {
          if (!varp->isIfaceParent() && !varp->isIfaceRef() &&
              !varp->noReset() && !varp->isParam()) {
            const auto vrefp =
    ...
    if (v3Global.opt.coverage()) {
      V3CCtorsBuilder configure_coverage{modp, "_configure_coverage",
                                         VCtorType::COVERAGE};
      for (AstNode *np = modp->stmtsp(); np; np = np->nextp()) {
