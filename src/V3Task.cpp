...
class TaskBaseVertex VL_NOT_FINAL : public V3GraphVertex {
  AstNode *m_impurep =
      nullptr;             // Node causing impure function w/ outside references
  bool m_noInline = false; // Marked with pragma
public:
  explicit TaskBaseVertex(V3Graph *graphp) : V3GraphVertex{graphp} {}
...
public:
  explicit TaskCodeVertex(V3Graph *graphp) : TaskBaseVertex{graphp} {}
...
private:
  ...
  // TYPES
  using VarToScopeMap =
      std::map<std::pair<AstScope *, AstVar *>, AstVarScope *>;
...
private:
  ...
  virtual void visit(AstNodeFTaskRef *nodep) override {
    ...
    // We make multiple edges if a task is called multiple times from another
    // task.
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    UINFO(9, "  TASK " << nodep << endl);
    {
      ...
      if (nodep->classMethod())
        m_curVxp->noInline(true); // Until V3Task supports it
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (nodep->varp()->user4u().toGraphVertex() != m_curVxp) {
      if (m_curVxp->pure() && !nodep->varp()->isXTemp())
  ...
  virtual void visit(AstClass *nodep) override {
    ...
    UASSERT_OBJ(m_ctorp, nodep,
                "class constructor missing"); // LinkDot always makes it
...
private:
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (nodep->varp()->user2p()) { // It's being converted to an alias.
      UINFO(9, "    relinkVar " << cvtToHex(nodep->varp()->user2p()) << " "
                                << nodep << endl);
  ...
  unpackDimsAndStrides(AstNodeDType *dtypep) {
    ...
    if (AstUnpackArrayDType *unpackp =
      ...
      const std::vector<AstUnpackArrayDType *> dims =
          unpackp->unpackDimensions();
      dimStrides.resize(dims.size(), {nullptr, 0});
      dimStrides.back() = {dims.back(), 1};
      for (ssize_t i = dims.size() - 2; i >= 0; --i) {
        dimStrides[i].first = dims[i];
        dimStrides[i].second =
            dimStrides[i + 1].second * dims[i + 1]->elementsConst();
      }
    }
    return dimStrides;
  }
  static bool dpiToInternalFrStmt(AstVar *portp, const string &frName,
                                  string &frstmt, string &ket) {
    ket.clear();
    if (portp->basicp() &&
        portp->basicp()->keyword() == AstBasicDTypeKwd::CHANDLE) {
...
class TaskGatherWrittenVisitor final : public AstNVisitor {
  ...
  virtual void visit(AstVarRef *nodep) override {
    if (nodep->access().isReadOnly())
      return; // Ignore read reference
    AstVarScope *const varScopep = nodep->varScopep();
    if (varScopep->user5())
      return;               // Ignore already processed variable
    ...
    // irrelevant for code ordering. This is simply an optimization to avoid
    // adding useless nodes to the ordering graph in V3Order.
    if (varScopep->varp()->isFuncLocal())
      return;
...
private:
  ...
  using DpiCFuncs =
      std::map<const string,
               std::tuple<AstNodeFTask *, std::string, AstCFunc *>>;

  // STATE
  TaskStateVisitor *m_statep;      // Common state between visitors
  AstNodeModule *m_modp = nullptr; // Current module
  AstTopScope *const m_topScopep =
      v3Global.rootp()->topScopep(); // The AstTopScope
  ...
  AstVarScope *createFuncVar(AstCFunc *funcp, const string &name,
                             AstVar *examplep) {
    AstVar *newvarp =
    ...
    AstVarScope *newvscp =
        new AstVarScope(funcp->fileline(), m_scopep, newvarp);
    m_scopep->addVarp(newvscp);
    return newvscp;
  }
  AstVarScope *createInputVar(AstCFunc *funcp, const string &name,
                              AstBasicDTypeKwd kwd) {
  ...
  AstVarScope *createVarScope(AstVar *invarp, const string &name) {
    if (invarp->isParam() && VN_IS(invarp->valuep(), InitArray)) {
      ...
      return v3Global.rootp()->constPoolp()->findTable(
          VN_AS(invarp->valuep(), InitArray));
    } else {
  ...
  AstNode *createInlinedFTask(AstNodeFTaskRef *refp, const string &namePrefix,
    ...
    if (debug() >= 9)
      beginp->dumpTreeAndNext(cout, "-newbegi:");
    //
    ...
    for (const auto &itr : tconnects) {
      ...
      if (!pinp) {
        ...
        VL_DO_DANGLING(argp->unlinkFrBack()->deleteTree(),
                       argp); // Args no longer needed
        //
        if (portp->isWritable() && VN_IS(pinp, Const)) {
          ...
          if (AstVarRef *varrefp = VN_CAST(pinp, VarRef)) {
            ...
            pinp->v3warn(E_TASKNSVAR, "Unsupported: Function/task input "
                                      "argument is not simple variable");
          }
    ...
    {
      AstBegin *tempp =
          new AstBegin(beginp->fileline(), "[EditWrapper]", beginp);
    ...
    //
    if (debug() >= 9)
      beginp->dumpTreeAndNext(cout, "-iotask: ");
    return beginp;
  }

  AstNode *createNonInlinedFTask(AstNodeFTaskRef *refp,
                                 const string &namePrefix, AstVarScope *outvscp,
                                 AstCNew *&cnewpr) {
    // outvscp is the variable for functions only, if nullptr, it's a task
    UASSERT_OBJ(refp->taskp(), refp, "Unlinked?");
    AstCFunc *cfuncp = m_statep->ftaskCFuncp(refp->taskp());
    UASSERT_OBJ(cfuncp, refp,
                "No non-inline task associated with this task call?");
    //
    AstNode *beginp = new AstComment(refp->fileline(),
                                     string("Function: ") + refp->name(), true);
    AstNodeCCall *ccallp;
    if (VN_IS(refp, New)) {
      ...
      ccallp = new AstCMethodCall(refp->fileline(),
                                  mrefp->fromp()->unlinkFrBack(), cfuncp);
    ...
    V3TaskConnects tconnects =
        V3Task::taskConnects(refp, refp->taskp()->stmtsp());
    for (const auto &itr : tconnects) {
      AstVar *portp = itr.first;
      ...
      if (!pinp) {
        ...
        if (portp->isWritable() && VN_IS(pinp, Const)) {
          ...
          pinp->replaceWith(
              new AstVarRef(newvscp->fileline(), newvscp, VAccess::WRITE));
          AstAssign *assp = new AstAssign(
              pinp->fileline(), pinp,
    ...
    if (refp->taskp()->dpiContext()) {
      ...
      ccallp->addArgsp(new AstCMath(refp->fileline(),
                                    "\"" + refp->fileline()->filename() + "\"",
                                    64, true));
      // __Vlineno
      ccallp->addArgsp(
          new AstConst(refp->fileline(), refp->fileline()->lineno()));
    }
    ...
    if (outvscp)
      ccallp->addArgsp(
          new AstVarRef(refp->fileline(), outvscp, VAccess::WRITE));

    if (debug() >= 9)
      beginp->dumpTreeAndNext(cout, "-nitask: ");
    return beginp;
  }

  string dpiSignature(AstNodeFTask *nodep, AstVar *rtnvarp) const {
    // Return fancy signature for DPI function. Variable names are not included
    // so differences in only argument names will not matter (as required by the
    // standard).
    string dpiproto;
    if (nodep->pure())
      dpiproto += "pure ";
    ...
    for (AstNode *stmtp = nodep->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
      if (const AstVar *portp = VN_CAST(stmtp, Var)) {
        if (portp->isIO() && !portp->isFuncReturn() && portp != rtnvarp) {
          ...
          if (nodep->dpiImport())
            dpiproto += portp->dpiArgType(false, false);
        }
      }
    }
    dpiproto += ")";
  ...
  static AstNode *createAssignInternalToDpi(AstVar *portp, bool isPtr,
                                            const string &frSuffix,
                                            const string &toSuffix) {
    const string stmt =
        V3Task::assignInternalToDpi(portp, isPtr, frSuffix, toSuffix);
    return new AstCStmt(portp->fileline(), stmt);
  }

  AstNode *createAssignDpiToInternal(AstVarScope *portvscp,
                                     const string &frName) {
    // Create assignment from DPI temporary into internal format
    // DPI temporary is scalar or 1D array (if unpacked array)
    // Internal representation is scalar, 1D, or multi-dimensional array
    // (similar to SV)
    AstVar *portp = portvscp->varp();
    string frstmt;
    string ket;
    const bool useSetWSvlv =
        TaskDpiUtils::dpiToInternalFrStmt(portp, frName, frstmt, ket);
    // Use a AstCMath, as we want V3Clean to mask off bits that don't make
    // sense.
    ...
    for (int i = 0; i < total; ++i) {
      AstNode *srcp =
          new AstVarRef(portvscp->fileline(), portvscp, VAccess::WRITE);
      // extract a scalar from multi-dimensional array (internal format)
      for (auto &&dimStride : dimStrides) {
        const size_t dimIdx =
            (i / dimStride.second) % dimStride.first->elementsConst();
      ...
      if (useSetWSvlv) {
        ...
        linesp->addNext(srcp);
        linesp->addNext(new AstText(portvscp->fileline(),
                                    "," + frName + " + " +
  ...
  AstCFunc *makeDpiExportDispatcher(AstNodeFTask *nodep, AstVar *rtnvarp) {
    ...
    AstCFunc *const funcp =
        new AstCFunc(nodep->fileline(), nodep->cname(), m_scopep,
    ...
    { // Create dispatch wrapper
      ...
      // First time init (faster than what the compiler does if we did a
      // singleton
      stmt += "if (VL_UNLIKELY(__Vfuncnum == -1)) __Vfuncnum = "
              "Verilated::exportFuncNum(\"" +
              nodep->cname() + "\");\n";
      // If the find fails, it will throw an error
      stmt += "const VerilatedScope* __Vscopep = Verilated::dpiScope();\n";
      // If dpiScope is fails and is null; the exportFind function throws and
      // error
      string cbtype = VIdProtect::protect(v3Global.opt.prefix() + "__Vcb_" +
                                          nodep->cname() + "_t");
      stmt +=
          cbtype + " __Vcb = (" + cbtype +
    ...
    for (AstNode *stmtp = nodep->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
      if (AstVar *portp = VN_CAST(stmtp, Var)) {
        if (portp->isIO() && !portp->isFuncReturn() && portp != rtnvarp) {
          ...
          if (args != "") {
            argnodesp =
                argnodesp->addNext(new AstText(portp->fileline(), args, true));
            args = "";
          }
          AstVarScope *outvscp =
              createFuncVar(funcp, portp->name() + tmpSuffixp, portp);
          // No information exposure; is already visible in import/export func
          // template
          ...
          if (portp->isNonOutput()) {
            std::string frName =
                portp->isInoutish() && portp->basicp()->isDpiPrimitive() &&
                        portp->dtypep()->skipRefp()->arrayUnpackedElements() ==
                            1
                    ? "*"
                    : "";
            frName += portp->name();
    ...
    for (AstNode *stmtp = nodep->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
      if (AstVar *portp = VN_CAST(stmtp, Var)) {
        if (portp->isIO() && portp->isWritable() && !portp->isFuncReturn()) {
          funcp->addStmtsp(
              createAssignInternalToDpi(portp, true, tmpSuffixp, ""));
        }
      }
    }
    ...
    if (rtnvarp) {
      ...
      funcp->addStmtsp(
          createAssignInternalToDpi(rtnvarp, false, tmpSuffixp, ""));
  ...
  AstCFunc *makeDpiImportPrototype(AstNodeFTask *nodep, AstVar *rtnvarp) {
    if (nodep->cname() != AstNode::prettyName(nodep->cname())) {
      nodep->v3error(
          "DPI function has illegal characters in C identifier name: "
    ...
    const string rtnType = rtnvarp            ? rtnvarp->dpiArgType(true, true)
                           : nodep->dpiTask() ? "int"
                                              : "";
    AstCFunc *const funcp =
        new AstCFunc(nodep->fileline(), nodep->cname(), m_scopep, rtnType);
  ...
  AstCFunc *getDpiFunc(AstNodeFTask *nodep, AstVar *rtnvarp) {
    UASSERT_OBJ(nodep->dpiImport() || nodep->dpiExport(), nodep,
                "Not a DPI function");
    ...
    // unique cname as it is illegal for the user to attach multiple tasks with
    // different signatures to one DPI cname.
    const auto it = m_dpiNames.find(nodep->cname());
    if (it == m_dpiNames.end()) {
      // First time encountering this cname. Create Import prototype / Export
      // entry point
      AstCFunc *const funcp = nodep->dpiExport()
                                  ? makeDpiExportDispatcher(nodep, rtnvarp)
                                  : makeDpiImportPrototype(nodep, rtnvarp);
      m_dpiNames.emplace(nodep->cname(),
                         std::make_tuple(nodep, signature, funcp));
      return funcp;
    } else {
      ...
      if (signature != firstSignature) {
        ...
        nodep->v3error(
            "Duplicate declaration of DPI function with different signature: "
            << nodep->prettyNameQ() << '\n'
            << nodep->warnContextPrimary() << '\n'
            << nodep->warnMore()                               //
          ...
          if (!portp->basicp()) {
            portp->v3warn(E_UNSUPPORTED,
                          "Unsupported: DPI argument of type "
                              ...
                              << portp->warnMore()
                              << "... For best portability, use bit, byte, "
                                 "int, or longint");
  ...
  void bodyDpiImportFunc(AstNodeFTask *nodep, AstVarScope *rtnvscp,
                         AstCFunc *cfuncp, AstCFunc *dpiFuncp) {
    const char *const tmpSuffixp = V3Task::dpiTemporaryVarSuffix();
        ...
        if (portp->isIO() && !portp->isFuncReturn() && portvscp != rtnvscp &&
          ...
          if (portp->isDpiOpenArray()) {
            ...
            if (portp->isWritable() && portp->basicp()->isDpiPrimitive()) {
              if (!VN_IS(portp->dtypep()->skipRefp(), UnpackArrayDType))
                args += "&";
            }
    ...
    {                // Call the imported function
      if (rtnvscp) { // isFunction will no longer work as we unlinked the return
                     // var
        ...
        cfuncp->addStmtsp(
            new AstText(nodep->fileline(), stmt, /* tracking: */ true));
      }
    ...
    for (AstNode *stmtp = cfuncp->argsp(); stmtp; stmtp = stmtp->nextp()) {
      if (AstVar *portp = VN_CAST(stmtp, Var)) {
        portp->protect(
            false); // No additional exposure - already part of shown proto
        if (portp->isIO() && (portp->isWritable() || portp->isFuncReturn()) &&
            !portp->isDpiOpenArray()) {
          AstVarScope *portvscp =
              VN_AS(portp->user2p(),
                    VarScope); // Remembered when we created it earlier
          cfuncp->addStmtsp(
  ...
  AstVarScope *makeDpiExporTrigger() {
    ...
    if (!dpiExportTriggerp) {
      // Create the global DPI export trigger flag the first time we encounter a
      // DPI export. This flag is set any time a DPI export is invoked, and
      // cleared at the end of eval.
      FileLine *const fl = m_topScopep->fileline();
      AstVar *const varp = new AstVar{
          fl, AstVarType::VAR, "__Vdpi_export_trigger", VFlagBitPacked{}, 1};
  ...
  AstCFunc *makeUserFunc(AstNodeFTask *nodep, bool ftaskNoInline) {
    // Given a already cloned node, make a public C function, or a non-inline C
    // function Probably some of this work should be done later, but... should
    // the type of the function be bool/uint32/64 etc (based on lookup) or
    // IData?
    AstNode::user2ClearTree();
    ...
    if (nodep->isFunction()) {
      ...
      if (!portp->isFuncReturn())
        nodep->v3error("Not marked as function return var");
      if (nodep->dpiImport() || nodep->dpiExport()) {
        ...
        if (!bdtypep->isDpiPrimitive()) {
          ...
          if (bdtypep->isDpiLogicVec()) {
            portp->v3error(
                "DPI function may not return a 4-state type "
    ...
    if (nodep->dpiImport() || nodep->dpiExport()) {
      ...
      if (!dpiFuncp)
        return nullptr; // There was an error, so bail
      if (nodep->dpiImport() && nodep->dpiOpenParent()) {
        // No need to make more than just the DPI Import prototype, the children
        // will create the wrapper implementations.
    ...
    // Unless public, v3Descope will not uniquify function names even if
    // duplicate per-scope, so make it unique now.
    string suffix; // So, make them unique
    if (!nodep->taskPublic() && !nodep->classMethod())
      suffix = "_" + m_scopep->nameDotless();
    const string name =
        ((nodep->name() == "new") ? "new" : prefix + nodep->name() + suffix);
    AstCFunc *cfuncp = new AstCFunc(nodep->fileline(), name, m_scopep,
    ...
    if (nodep->dpiExport()) {
      ...
      snp->dpiExport(true); // The AstScopeName is really a statement(ish) for
                            // tracking, not a function
      snp->unlinkFrBack();
    ...
    if (nodep->dpiImport())
      bodyDpiImportFunc(nodep, rtnvscp, cfuncp, dpiFuncp);

    // Return statement
    if (rtnvscp && nodep->taskPublic()) {
      cfuncp->addFinalsp(new AstCReturn(
          rtnvscp->fileline(),
          new AstVarRef(rtnvscp->fileline(), rtnvscp, VAccess::READ)));
    }
    // Replace variable refs
    // Iteration requires a back, so put under temporary node
    {
    ...
    if (cfuncp->dpiExportImpl()) {
      // Mark all non-local variables written by the DPI exported function as
      // being updated by DPI exports. This ensures correct ordering and change
      // detection later.
      const std::vector<AstVarScope *> writtenps =
          TaskGatherWrittenVisitor::gather(cfuncp);
      if (!writtenps.empty()) {
        ...
        AstAssign *const assignp = new AstAssign{
            fl, new AstVarRef{fl, dpiExportTriggerp, VAccess::WRITE},
            new AstConst{fl, AstConst::BitTrue{}}};
        // Add as first statement (to avoid issues with early returns) to
        // exported function
        if (cfuncp->stmtsp()) {
        ...
        // Add an always block sensitive to the DPI export trigger flag, and add
        // an AstDpiExportUpdated node under it for each variable that are
        // writen by the exported function.
        AstAlways *const alwaysp = new AstAlways{
            fl, VAlwaysKwd::ALWAYS,
            new AstSenTree{fl,
                           new AstSenItem{fl, VEdgeType::ET_HIGHEDGE,
  ...
  virtual void visit(AstNodeFTaskRef *nodep) override {
    ...
    if (debug() >= 9)
      nodep->dumpTree(cout, "-inlfunc:");
    UASSERT_OBJ(m_scopep, nodep, "func ref not under scope");
    const string namePrefix =
        ((VN_IS(nodep, FuncRef) ? "__Vfunc_" : "__Vtask_") +
         nodep->taskp()->shortName() + "__" + cvtToStr(m_modNCalls++));
    // Create output variable
    AstVarScope *outvscp = nullptr;
    if (nodep->taskp()->isFunction()) {
      // Not that it's a FUNCREF, but that we're calling a function (perhaps as
      // a task)
      outvscp = createVarScope(VN_AS(nodep->taskp()->fvarp(), Var),
    ...
    if (VN_IS(nodep, New)) {
      ...
      UASSERT_OBJ(nodep->taskp()->isFunction(), nodep,
                  "func reference to non-function");
      AstVarRef *outrefp =
          new AstVarRef(nodep->fileline(), outvscp, VAccess::READ);
      nodep->replaceWith(outrefp);
      // Insert new statements
      visitp = insertBeforeStmt(nodep, beginp);
    } else {
      if (nodep->taskp()->isFunction()) {
        nodep->v3warn(IGNOREDRETURN, "Ignoring return value of non-void "
                                     "function (IEEE 1800-2017 13.4.1)");
      }
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    m_insMode = IM_BEFORE;
    m_insStmtp =
        nodep->stmtsp(); // Might be null if no statements, but we won't use it
    if (!nodep->user1SetOnce()) { // Just one creation needed per function
      ...
      if (v3Global.opt.protectIds() && nodep->taskPublic()) {
        ...
        // Hence any external references wouldn't be able to find the refed
        // public object.
      ...
      if (nodep->dpiImport() || nodep->dpiExport() || nodep->taskPublic() ||
        ...
        if (nodep->isConstructor())
          m_statep->remapFuncClassp(nodep, clonedFuncp);

        AstCFunc *cfuncp =
            makeUserFunc(clonedFuncp, m_statep->ftaskNoInline(nodep));
        if (cfuncp) {
      ...
      // Any variables inside the function still have varscopes pointing to
      // them. We're going to delete the vars, so delete the varscopes.
      if (nodep->isFunction()) {
...
public:
  // CONSTRUCTORS
  TaskVisitor(AstNetlist *nodep, TaskStateVisitor *statep) : m_statep{statep} {
    iterate(nodep);
  }
...
V3TaskConnects V3Task::taskConnects(AstNodeFTaskRef *nodep,
                                    AstNode *taskStmtsp) {
  ...
  for (AstNode *stmtp = taskStmtsp; stmtp; stmtp = stmtp->nextp()) {
    if (AstVar *portp = VN_CAST(stmtp, Var)) {
      if (portp->isIO()) {
        tconnects.push_back(
            std::make_pair(portp, static_cast<AstArg *>(nullptr)));
        nameToIndex.insert(std::make_pair(
            portp->name(), tpinnum)); // For name based connections
        tpinnum++;
    ...
    if (argp->name() != "") {
      ...
      if (it == nameToIndex.end()) {
        ...
        if (tconnects[it->second].second) {
          pinp->v3error("Duplicate argument "
                        << argp->prettyNameQ() << " in function call to "
                        << nodep->taskp()->prettyTypeName());
        }
      ...
      if (ppinnum >= tpinnum) {
        if (sformatp) {
          tconnects.push_back(
              std::make_pair(sformatp, static_cast<AstArg *>(nullptr)));
  ...
  if (reorganize) {
    // To simplify downstream, put argument list back into pure pinnumber
    // ordering
    while (nodep->pinsp()) {
  ...
  if (debug() >= 9) { // LCOV_EXCL_START
    ...
    for (int i = 0; i < tpinnum; ++i) {
      UINFO(0, "   pin " << i << "  conn=" << cvtToHex(tconnects[i].second)
                         << endl);
    }
  } // LCOV_EXCL_STOP
  return tconnects;
...
string V3Task::assignInternalToDpi(AstVar *portp, bool isPtr,
  ...
  // Internal representation is scalar, 1D, or multi-dimensional array (similar
  // to SV) DPI temporary is scalar or 1D array (if unpacked array)
  string stmt;
  string ket;
  // Someday we'll have better type support, and this can make variables and
  // casts. But for now, we'll just text-bash it.
  ...
  if (AstUnpackArrayDType *unpackp =
    ...
    if (unpackDim > 0)
      UASSERT_OBJ(unpackSize > 0, portp, "size must be greater than 0");
  }
  if (portp->basicp()->isDpiBitVec() || portp->basicp()->isDpiLogicVec()) {
    ...
    const bool isChandle = portp->basicp() && portp->basicp()->keyword() ==
                                                  AstBasicDTypeKwd::CHANDLE;
    const bool isString = portp->basicp() && portp->basicp()->keyword() ==
...
string V3Task::assignDpiToInternal(const string &lhsName, AstVar *varp) {
  ...
  string ket;
  const bool useSetWSvlv =
      TaskDpiUtils::dpiToInternalFrStmt(varp, frName, frstmt, ket);
...
void V3Task::taskAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("task", 0,
