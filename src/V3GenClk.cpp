...
private:
  ...
  //  AstVarRef::user3()      -> bool.  Signal is replaced activation (already
  //  done)
  ...
  AstVarScope *genInpClk(AstVarScope *vscp) {
    if (vscp->user2p()) {
      ...
      // In order to create a __VinpClk* for a signal, it needs to be marked
      // circular. The DPI export trigger is never marked circular by V3Order
      // (see comments in OrderVisitor::nodeMarkCircular). The only other place
      // where one might mark a node circular is in this pass (V3GenClk), if the
      // signal is assigned but was previously used as a clock. The DPI export
      // trigger is only ever assigned in a DPI export called from outside eval,
      // or from a DPI import, which are not discovered by GenClkReadVisitor
      // (note that impure tasks - i.e.: those setting non-local variables -
      // cannot be no-inline, see V3Task), hence the DPI export trigger should
      // never be marked circular. Note that ordering should still be correct as
      // there will be a change detect on any signals set from a DPI export that
      // might have dependents scheduled earlier.
      ...
      AstVarScope *newvscp =
          new AstVarScope(vscp->fileline(), m_scopetopp, newvarp);
...
void V3GenClk::genClkAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("genclk", 0,
