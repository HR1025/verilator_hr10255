...
public:
  ...
  InstrCountVisitor(AstNode *nodep, bool assertNoDups, std::ostream *osp)
      : m_startNodep{nodep}, m_assertNoDups{assertNoDups}, m_osp{osp} {
    if (nodep)
      iterate(nodep);
  }
...
private:
  uint32_t startVisitBase(AstNode *nodep) {
    if (m_assertNoDups && !m_inCFunc) {
      ...
      nodep->user5p(
          const_cast<void *>(reinterpret_cast<const void *>(m_startNodep)));
    }
  ...
  void markCost(AstNode *nodep) {
    if (m_osp)
      nodep->user4(m_instrCount + 1); // Else don't mark to avoid writeback
  }
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    if (nodep->branchPred().likely())
      elseCount = 0;

    if (ifCount >= elseCount) {
      m_instrCount = savedCount + ifCount;
      if (nodep->elsesp())
        nodep->elsesp()->user4(0); // Don't dump it
    } else {
      m_instrCount = savedCount + elseCount;
      if (nodep->ifsp())
        nodep->ifsp()->user4(0); // Don't dump it
    }
  }
  virtual void visit(AstNodeCond *nodep) override {
    ...
    if (ifCount < elseCount) {
      m_instrCount = savedCount + elseCount;
      if (nodep->expr1p())
        nodep->expr1p()->user4(0); // Don't dump it
    } else {
      m_instrCount = savedCount + ifCount;
      if (nodep->expr2p())
        nodep->expr2p()->user4(0); // Don't dump it
    }
  }
  virtual void visit(AstActive *nodep) override {
    ...
    // root:
    markCost(nodep);
    UASSERT_OBJ(nodep == m_startNodep, nodep,
                "Multiple actives, or not start node");
  }
...
public:
  // CONSTRUCTORS
  InstrCountDumpVisitor(AstNode *nodep, std::ostream *osp) : m_osp{osp} {
...
private:
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    if (unsigned costPlus1 = nodep->user4()) {
      *m_osp << "  " << indent() << "cost " << std::setw(6) << std::left
             << (costPlus1 - 1) << "  " << nodep << '\n';
      iterateChildren(nodep);
    }
    --m_depth;
...
uint32_t V3InstrCount::count(AstNode *nodep, bool assertNoDups,
                             std::ostream *osp) {
  InstrCountVisitor visitor{nodep, assertNoDups, osp};
  if (osp)
    InstrCountDumpVisitor dumper(nodep, osp);
