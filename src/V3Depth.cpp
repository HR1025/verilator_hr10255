...
private:
  ...
  void createDeepTemp(AstNode *nodep) {
    ...
    AstAssign *const assp = new AstAssign{
        nodep->fileline(),
        new AstVarRef{nodep->fileline(), varp, VAccess::WRITE}, nodep};
  ...
  virtual void visit(AstNodeMath *nodep) override {
    ...
    if (m_stmtp &&
        ...
        !VN_IS(nodep->backp(),
               NodeStmt) // Not much point if we're about to use it
    ) {
      m_maxdepth = m_depth;
...
public:
  // CONSTRUCTORS
  explicit DepthVisitor(AstNetlist *nodep) : m_tempNames{"__Vdeeptemp"} {
    iterate(nodep);
  }
...
void V3Depth::depthAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("depth", 0,
