...
private:
  ...
  static AstNode *newWordAssign(AstNodeAssign *placep, int word, AstNode *lhsp,
                                AstNode *rhsp) {
  ...
  static void addWordAssign(AstNodeAssign *placep, int word, AstNode *lhsp,
                            AstNode *rhsp) {
  ...
  static void fixCloneLvalue(AstNode *nodep) {
    ...
    if (VN_IS(nodep, VarRef))
      VN_AS(nodep, VarRef)->access(VAccess::READ);
    // Iterate
    if (nodep->op1p())
      fixCloneLvalue(nodep->op1p());
    if (nodep->op2p())
      fixCloneLvalue(nodep->op2p());
    if (nodep->op3p())
      fixCloneLvalue(nodep->op3p());
    if (nodep->op4p())
  ...
  static AstNode *newAstWordSelClone(AstNode *nodep, int word) {
    ...
    if (nodep->isWide() && word >= 0 && word < nodep->widthWords()) {
      ...
      quadfromp->dtypeSetBitUnsized(VL_QUADSIZE, quadfromp->widthMin(),
                                    VSigning::UNSIGNED);
      return new AstCCast{fl,
                          new AstShiftR{fl, quadfromp,
                                        new AstConst{fl, VL_EDATASIZE},
                                        VL_EDATASIZE},
                          VL_EDATASIZE};
  ...
  static AstNode *newWordGrabShift(FileLine *fl, int word, AstNode *lhsp,
                                   int shift) {
    // Extract the expression to grab the value for the specified word, if it's
    // the shift of shift bits from lhsp
    AstNode *newp;
    ...
    if (const int loffset = VL_BITBIT_E(shift)) {
      ...
      newp = new AstOr{
          fl,
          new AstAnd{
              fl, new AstConst{fl, AstConst::SizedEData(), VL_MASK_E(loffset)},
              new AstShiftR{
                  fl, lhip,
  ...
  static AstNode *newWordSel(FileLine *fl, AstNode *fromp, AstNode *lsbp,
    ...
    if (wordOffset >= static_cast<uint32_t>(fromp->widthWords())) {
      ...
      if (VN_IS(lsbp, Const)) {
        wordp = new AstConst{
            lfl, wordOffset + VL_BITWORD_E(VN_AS(lsbp, Const)->toUInt())};
      } else {
        wordp =
            new AstShiftR{lfl, lsbp->cloneTree(true),
  ...
  static AstNode *dropCondBound(AstNode *nodep) {
    ...
    // if (VN_IS(nodep, CondBound) && VN_IS(VN_AS(nodep, CondBound)->lhsp(),
    // Lte)) {
  ...
  bool expandWide(AstNodeAssign *nodep, AstArraySel *rhsp) {
    ...
    UASSERT_OBJ(
        !VN_IS(nodep->dtypep()->skipRefp(), UnpackArrayDType), nodep,
  ...
  bool expandWide(AstNodeAssign *nodep, AstNot *rhsp) {
    ...
    for (int w = 0; w < nodep->widthWords(); ++w) {
      addWordAssign(nodep, w,
                    new AstNot{fl, newAstWordSelClone(rhsp->lhsp(), w)});
    }
    return true;
  }
  //-------- Biops
  ...
  virtual void visit(AstSel *nodep) override {
    ...
    UASSERT_OBJ(nodep->widthMin() == nodep->widthConst(), nodep,
                "Width mismatch");
    if (VN_IS(nodep->backp(), NodeAssign) &&
      ...
      AstNode *const lowp = new AstShiftR{
          nfl, lowwordp, newSelBitBit(nodep->lsbp()), nodep->width()};
      ...
      if (nodep->widthConst() > 1) {
        ...
        AstNode *const midMsbp =
            new AstAdd{lfl, new AstConst{lfl, midMsbOffset},
        ...
        AstNode *const midshiftp = new AstSub{
            lfl, new AstConst{lfl, VL_EDATASIZE}, newSelBitBit(nodep->lsbp())};
      ...
      if (nodep->widthConst() > VL_EDATASIZE) {
        ...
        AstNode *const hishiftp = new AstCond{
            ...
            // 64 - (lbs % VL_EDATASIZE)
            new AstSub{lfl, new AstConst{lfl, 64},
      ...
      AstNode *newp = lowp;
      if (midp)
        newp = new AstOr{nfl, midp, newp};
      if (hip)
        newp = new AstOr{nfl, hip, newp};
      ...
      if (nodep->isQuad() && !fromp->isQuad()) {
        fromp = new AstCCast{fl, fromp, nodep};
      }
      // {large}>>32 requires 64-bit shift operation; then cast
      AstNode *newp =
          new AstShiftR{fl, fromp, dropCondBound(lsbp), fromp->width()};
      newp->dtypeFrom(fromp);
      if (!nodep->isQuad() && fromp->isQuad()) {
        newp = new AstCCast{fl, newp, nodep};
      }
      newp->dtypeFrom(nodep);
      VL_DO_DANGLING(replaceWithDelete(nodep, newp), nodep);
    }
  ...
  bool expandWide(AstNodeAssign *nodep, AstSel *rhsp) {
    UASSERT_OBJ(nodep->widthMin() == rhsp->widthConst(), nodep,
                "Width mismatch");
    if (!doExpand(nodep))
      return false;
    if (VN_IS(rhsp->lsbp(), Const) && VL_BITBIT_E(rhsp->lsbConst()) == 0) {
      ...
      for (int w = 0; w < nodep->widthWords(); ++w) {
        addWordAssign(nodep, w,
                      newAstWordSelClone(rhsp->fromp(), w + VL_BITWORD_E(lsb)));
      }
      return true;
    } else {
      ...
      for (int w = 0; w < nodep->widthWords(); ++w) {
        ...
        AstNode *const lowp = new AstShiftR{
            rfl, lowwordp, newSelBitBit(rhsp->lsbp()), VL_EDATASIZE};
        // Upper bits
        const V3Number zero{nodep, VL_EDATASIZE, 0};
        AstNode *const midwordp = // SEL(from,[1+wordnum])
            newWordSel(ffl, rhsp->fromp()->cloneTree(true), rhsp->lsbp(),
                       w + 1);
        AstNode *const midshiftp = new AstSub{
            lfl, new AstConst{lfl, VL_EDATASIZE}, newSelBitBit(rhsp->lsbp())};
        AstNode *const midmayp =
            new AstShiftL{rfl, midwordp, midshiftp, VL_EDATASIZE};
        AstNode *const midp = new AstCond{
            rfl,
            new AstEq{rfl, new AstConst{rfl, 0}, newSelBitBit(rhsp->lsbp())},
  ...
  bool expandLhs(AstNodeAssign *nodep, AstSel *lhsp) {
    ...
    if (VN_IS(lhsp->lsbp(), Const)) {
      ...
      for (int bit = lsb; bit < (msb + 1); bit++)
        maskset.setBit(bit, 1);
      ...
      if (destwide) {
        ...
        for (int w = 0; w < destp->widthWords(); ++w) {
          if (w >= VL_BITWORD_E(lsb) && w <= VL_BITWORD_E(msb)) {
            ...
            if (!ones) {
              oldvalp = new AstAnd{lfl,
                                   new AstConst{lfl, AstConst::SizedEData(),
                                                maskold.edataWord(w)},
                                   oldvalp};
            }
            ...
            // of words).
            if (w == destp->widthWords() - 1 &&
                VL_BITBIT_E(destp->widthMin()) != 0) {
        ...
        if (destp->isQuad() && !rhsp->isQuad()) {
          rhsp = new AstCCast{nfl, rhsp, nodep};
        }
        AstNode *oldvalp = destp->cloneTree(true);
        fixCloneLvalue(oldvalp);
        if (!ones) {
          oldvalp = new AstAnd{lfl, new AstConst{lfl, maskold}, oldvalp};
        }
        ...
        AstNode *const shifted = new AstShiftL{
            lfl, rhsp, new AstConst{lfl, static_cast<uint32_t>(lsb)},
            destp->width()};
        AstNode *const cleaned =
            new AstAnd{lfl, shifted, new AstConst{lfl, cleanmask}};
        AstNode *const newp =
            new AstAssign{nfl, destp, new AstOr{lfl, oldvalp, cleaned}};
      ...
      if (destwide && lhsp->widthConst() == 1) {
        ...
        AstNode *const destp = lhsp->fromp()->unlinkFrBack();
        AstNode *oldvalp =
            newWordSel(lfl, destp->cloneTree(true), lhsp->lsbp());
        fixCloneLvalue(oldvalp);
        if (!ones) {
          oldvalp = new AstAnd{
              lfl,
        ...
        AstNode *const newp = new AstAssign{
            ...
            new AstOr{lfl, oldvalp,
                      new AstShiftL{lfl, rhsp, shiftp, VL_EDATASIZE}}};
        insertBefore(nodep, newp);
        return true;
      } else if (destwide) {
        UINFO(8, "    ASSIGNSEL(varlsb,wide) -- NoOp -- " << nodep << endl);
        //   For wide destp, we can either form a equation for every destination
        //   word,
        ...
        // doing every word is better for temporaries and if we're setting most
        // words since it may result in better substitution optimizations later.
        //   This results in so much code, we're better off leaving a function
        //   call.
        ...
        for (int bit = 0; bit < lhsp->widthConst(); bit++)
          maskwidth.setBit(bit, 1);
        ...
        AstNode *newp = new AstShiftL{lfl, rhsp, lhsp->lsbp()->cloneTree(true),
                                      destp->width()};
  ...
  virtual void visit(AstConcat *nodep) override {
    ...
    if (nodep->isWide()) {
      ...
      if (nodep->isQuad() && !lhsp->isQuad()) {
        lhsp = new AstCCast{fl, lhsp, nodep};
      }
      if (nodep->isQuad() && !rhsp->isQuad()) {
        rhsp = new AstCCast{fl, rhsp, nodep};
      }
      AstNode *const newp = new AstOr{
          fl,
          new AstShiftL{fl, lhsp, new AstConst{fl, rhsshift}, nodep->width()},
          rhsp};
  ...
  bool expandWide(AstNodeAssign *nodep, AstConcat *rhsp) {
    ...
    // When we have x={x,foo} backwards is better, when x={foo,x} forward is
    // better However V3Subst tends to rip this up, so not worth optimizing now.
    for (int w = 0; w < rhsp->widthWords(); ++w) {
      addWordAssign(nodep, w,
                    new AstOr{fl,
                              newWordGrabShift(fl, w, rhsp->lhsp(), rhsshift),
                              newAstWordSelClone(rhsp->rhsp(), w)});
    }
    return true;
  }
  ...
  bool expandWide(AstNodeAssign *nodep, AstReplicate *rhsp) {
    ...
    for (int w = 0; w < rhsp->widthWords(); ++w) {
      ...
      if (lhswidth == 1) {
        ...
        for (unsigned repnum = 1; repnum < times; repnum++) {
          newp = new AstOr{
              fl, newWordGrabShift(rfl, w, lhsp, lhswidth * repnum), newp};
        }
      }
      addWordAssign(nodep, w, newp);
    }
  ...
  void visitEqNeq(AstNodeBiop *nodep) {
    ...
    if (nodep->lhsp()->isWide()) {
      ...
      for (int w = 0; w < nodep->lhsp()->widthWords(); ++w) {
        AstNode *const eqp =
            new AstXor{fl, newAstWordSelClone(nodep->lhsp(), w),
                       newAstWordSelClone(nodep->rhsp(), w)};
        newp = newp ? new AstOr{fl, newp, eqp} : eqp;
      }
      if (VN_IS(nodep, Neq)) {
        newp =
            new AstNeq{fl, new AstConst{fl, AstConst::SizedEData(), 0}, newp};
      } else {
  ...
  virtual void visit(AstRedOr *nodep) override {
    ...
    if (nodep->lhsp()->isWide()) {
      ...
      AstNode *const newp = new AstNeq{
          fl,
          new AstConst{fl, AstConst::WidthedValue(), longOrQuadWidth(nodep), 0},
          lhsp};
      VL_DO_DANGLING(replaceWithDelete(nodep, newp), nodep);
    }
  }
  virtual void visit(AstRedAnd *nodep) override {
    ...
    if (nodep->lhsp()->isWide()) {
      ...
      newp = new AstEq{
          fl, new AstConst{fl, AstConst::SizedEData(), VL_MASK_E(VL_EDATASIZE)},
          newp};
      VL_DO_DANGLING(replaceWithDelete(nodep, newp), nodep);
    } else {
      ...
      AstNode *const lhsp = nodep->lhsp()->unlinkFrBack();
      AstNode *const newp =
          new AstEq{fl, new AstConst{fl, wordMask(lhsp)}, lhsp};
      VL_DO_DANGLING(replaceWithDelete(nodep, newp), nodep);
    }
  }
  virtual void visit(AstRedXor *nodep) override {
    ...
    // We don't reduce non-wide XORs, as its more efficient to use a temp
    // register, which the inlined function does nicely.
  }
...
void V3Expand::expandAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("expand", 0,
