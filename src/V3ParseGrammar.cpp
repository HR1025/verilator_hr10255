...
AstNode *V3ParseGrammar::createSupplyExpr(FileLine *fileline,
...
AstRange *V3ParseGrammar::scrubRange(AstNodeRange *nrangep) {
  ...
  for (AstNodeRange *nodep = nrangep, *nextp; nodep; nodep = nextp) {
    ...
    if (!VN_IS(nodep, Range)) {
      nodep->v3error("Unsupported or syntax error: Unsized range in instance "
  ...
  if (nrangep && nrangep->nextp()) {
    ...
    nrangep->nextp()->v3warn(
        E_UNSUPPORTED, "Unsupported: Multidimensional instances/interfaces.");
...
AstNodeDType *V3ParseGrammar::createArray(AstNodeDType *basep,
  ...
  if (nrangep) { // Maybe no range - return unmodified base type
    while (nrangep->nextp())
      nrangep = VN_AS(nrangep->nextp(), NodeRange);
    while (nrangep) {
      AstNodeRange *prevp = VN_AS(nrangep->backp(), NodeRange);
      if (prevp)
        nrangep->unlinkFrBack();
      AstRange *rangep = VN_CAST(nrangep, Range);
      if (rangep && isPacked) {
        arrayp = new AstPackArrayDType(rangep->fileline(), VFlagChildDType(),
                                       arrayp, rangep);
      } else if (rangep && (VN_IS(rangep->leftp(), Unbounded) ||
                            VN_IS(rangep->rightp(), Unbounded))) {
        arrayp = new AstQueueDType(nrangep->fileline(), VFlagChildDType(),
                                   arrayp, rangep->rightp()->cloneTree(true));
      } else if (rangep) {
        arrayp = new AstUnpackArrayDType(rangep->fileline(), VFlagChildDType(),
                                         arrayp, rangep);
      } else if (VN_IS(nrangep, UnsizedRange)) {
        arrayp = new AstUnsizedArrayDType(nrangep->fileline(),
                                          VFlagChildDType(), arrayp);
        ...
        arrayp = new AstBracketArrayDType(nrangep->fileline(),
                                          VFlagChildDType(), arrayp, keyp);
      } else {
...
AstVar *V3ParseGrammar::createVariable(FileLine *fileline, const string &name,
  ...
  if (GRAMMARP->m_varIO == VDirection::NONE &&
    ...
    // Just a port list with variable name (not v2k format); AstPort already
    // created
    if (dtypep)
      fileline->v3warn(E_UNSUPPORTED,
  ...
  // UINFO(0,"CREVAR "<<fileline->ascii()<<"
  // decl="<<GRAMMARP->m_varDecl.ascii()<<"
  ...
  if (type == AstVarType::GENVAR) {
    if (arrayp)
      fileline->v3error("Genvars may not be arrayed: " << name);
  }
  ...
  AstVar *nodep =
      new AstVar(fileline, type, name, VFlagChildDType(), arrayDTypep);
  nodep->addAttrsp(attrsp);
  nodep->ansi(m_pinAnsi);
  nodep->declTyped(m_varDeclTyped);
  nodep->lifetime(m_varLifetime);
  if (GRAMMARP->m_varDecl != AstVarType::UNKNOWN)
    nodep->combineType(GRAMMARP->m_varDecl);
  if (GRAMMARP->m_varIO != VDirection::NONE) {
  ...
  if (GRAMMARP->m_varDecl == AstVarType::SUPPLY0) {
    nodep->addNext(
        V3ParseGrammar::createSupplyExpr(fileline, nodep->name(), 0));
  }
  if (GRAMMARP->m_varDecl == AstVarType::SUPPLY1) {
    nodep->addNext(
        V3ParseGrammar::createSupplyExpr(fileline, nodep->name(), 1));
  }
  ...
  // Remember the last variable created, so we can attach attributes to it in
  // later parsing
...
#define vl_decodexdigit(c)                                                     \
  ((isdigit(c) ? ((c) - '0') : (tolower((c)) - 'a' + 10)))
          newtext +=
              (char)(16 * vl_decodexdigit(cp[1]) + vl_decodexdigit(cp[2]));
          cp += 2;
        } else if (isalnum(*cp)) {
