...
class EmitCHeader final : public EmitCConstInit {
  ...
  void emitDesignVarDecls(const AstNodeModule *modp) {
    ...
    const auto emitCurrentList = [this, &first, &varList, &lastAnon]() {
      ...
      if (lastAnon) { // Output as anons
        ...
        if (anonL1s != 1)
          puts("// Anonymous structures to workaround compiler member-count "
               "bugs\n");
    ...
    for (const AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
      if (const AstVar *const varp = VN_CAST(nodep, Var)) {
        if (varp->isIO() || varp->isSignal() || varp->isClassMember() ||
            varp->isTemp()) {
  ...
  void emitCtorDtorDecls(const AstNodeModule *modp) {
    if (!VN_IS(modp,
               Class)) { // Classes use CFuncs with isConstructor/isDestructor
  ...
  void emitInternalMethodDecls(const AstNodeModule *modp) {
    ...
    if (v3Global.opt.savable()) {
      ...
      puts("void " + protect("__Vdeserialize") +
  ...
  void emitEnums(const AstNodeModule *modp) {
    ...
    for (const AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
      ...
      if (edtypep->width() > 64) {
        putsDecoration("// enum " + tdefp->nameProtect() +
    ...
    for (const AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
      if (const AstCFunc *const funcp = VN_CAST(nodep, CFunc)) {
        ...
        if (funcp->isMethod() != inClassBody) // Only methods go inside class
          continue;
        if (funcp->isMethod() &&
            funcp->isLoose()) // Loose methods are declared lazily
          continue;
        funcsp.push_back(funcp);
      }
    }

    std::stable_sort(funcsp.begin(), funcsp.end(),
  ...
  explicit EmitCHeader(const AstNodeModule *modp) {
    ...
    const string filename =
        v3Global.opt.makeDir() + "/" + prefixNameProtect(modp) + ".h";
...
void V3EmitC::emitcHeaders() {
  ...
  for (const AstNode *nodep = v3Global.rootp()->modulesp(); nodep;
    ...
    if (VN_IS(nodep, Class))
      continue; // Declared with the ClassPackage
