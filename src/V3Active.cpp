...
//          OPTIMIZE: When support async clocks, fold into that active if
//          possible
...
private:
  ...
  string typestr() const { //   "
    switch (m_type) {
    case VT_BLOCK:
      return "(||)"; // basic block node
    case VT_BRANCH:
      return "(&&)"; // if/else branch mode
    case VT_OUTPUT:
      return "(out)"; // var assignment
    default:
      return "??"; // unknown
    }
  }
...
public:
  LatchDetectGraphVertex(V3Graph *graphp, const string &name,
                         VertexType type = VT_BLOCK)
      : V3GraphVertex{graphp}, m_name{name}, m_type{type} {}
  virtual string name() const override { return m_name + " " + typestr(); }
  virtual string dotColor() const override {
    return user() ? "green" : "black";
  }
...
protected:
  ...
  std::vector<AstVarRef *>
      m_outputs; // Vector of lvalues encountered on this pass
  ...
  // Recursively traverse the graph to determine whether every control 'BLOCK'
  // has an assignment to the output we are currently analysing (the output
  // whose 'user() is set), if so return true. Where a BLOCK contains a BRANCH,
  // both the if and else sides of the branch must return true for the BRANCH to
  // evalute to true. A BLOCK however needs only a single one of its siblings to
  // evaluate true in order to evaluate true itself. On output vertex only
  // evaluates true if it is the vertex we are analyzing on this check
...
public:
  ...
  LatchDetectGraphVertex *addPathVertex(LatchDetectGraphVertex *parent,
                                        const string &name,
                                        bool branch = false) {
    m_curVertexp =
        new LatchDetectGraphVertex{this, name,
                                   branch ? LatchDetectGraphVertex::VT_BRANCH
                                          : LatchDetectGraphVertex::VT_BLOCK};
    new V3GraphEdge{this, parent, m_curVertexp, 1};
    return m_curVertexp;
  }
  // Add a new output variable vertex and store a pointer to it in the user1
  // field of the variables AstNode
  LatchDetectGraphVertex *addOutputVertex(AstVarRef *nodep) {
    LatchDetectGraphVertex *outVertexp = new LatchDetectGraphVertex{
        this, nodep->name(), LatchDetectGraphVertex::VT_OUTPUT};
  ...
  // Run latchCheckInternal on each variable assigned by the always block to see
  // if all control paths make an assignment. Detected latches are flagged in
  // the variables AstVar
  void latchCheck(AstNode *nodep, bool latch_expected) {
    ...
    for (const auto &vrp : m_outputs) {
      ...
      vertp->user(
          true); // Identify the output vertex we are checking paths _to_
      ...
      if (latch_detected && !latch_expected) {
        nodep->v3warn(
            ...
            "Latch inferred for signal "
                << vrp->prettyNameQ()
                << " (not all control paths of combinational always assign a "
                   "value)\n"
                << nodep->warnMore()
                << "... Suggest use of always_latch for intentional latches");
        if (debug() >= 9)
          dumpDotFilePrefixed("latch_" + vrp->name());
      }
      vertp->user(false); // Clear again (see above)
      vrp->varp()->isLatched(latch_detected);
    }
    // Should _all_ variables assigned in always_latch be latches? Probably, but
    // this only warns if none of them are
...
private:
  ...
  AstActive *m_iActivep =
      nullptr; // For current scope, the IActive we're building
  AstActive *m_cActivep =
      nullptr; // For current scope, the SActive(combo) we're building
...
private:
  ...
  virtual void visit(AstVarRef *nodep) {
    ...
    if (nodep->access().isWriteOrRW() && varp->isSignal() &&
        !varp->isUsedLoopIdx()) {
  ...
  virtual void visit(AstNodeIf *nodep) {
    if (!nodep->isBoundsCheck()) {
      ...
      LatchDetectGraphVertex *branchp =
          m_graph.addPathVertex(parentp, "BRANCH", true);
    ...
    if (m_check != CT_SEQ) {
      ...
      if (m_check == CT_INITIAL) {
        nodep->v3warn(INITIALDLY,
                      "Delayed assignments (<=) in initial or final block\n"
                          << nodep->warnMore()
    ...
    if (m_check == CT_SEQ && m_assignp &&
      ...
      if (!m_alwaysp->fileline()->warnIsOff(V3ErrorCode::BLKSEQ) &&
        ...
        m_alwaysp->fileline()->modifyWarnOff(
            V3ErrorCode::BLKSEQ,
            true); // Complain just once for the entire always
...
private:
  ...
  virtual void visit(AstFinal *nodep) override {
    ...
    ActiveDlyVisitor dlyvisitor{nodep, ActiveDlyVisitor::CT_INITIAL};
    if (!m_scopeFinalp) {
      m_scopeFinalp = new AstCFunc(nodep->fileline(),
                                   "_final_" + m_namer.scopep()->nameDotless(),
                                   m_namer.scopep());
  ...
  void visitAlways(AstNode *nodep, AstSenTree *oldsensesp, VAlwaysKwd kwd) {
    ...
    if (oldsensesp && oldsensesp->sensesp() &&
        oldsensesp->sensesp()->isNever()) {
      // Never executing.  Kill it.
      UASSERT_OBJ(!oldsensesp->sensesp()->nextp(), nodep,
                  "Never senitem should be alone, else the never should be "
                  "eliminated.");
      VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
      return;
    }
    ...
    if (!combo && !sequent)
      combo = true; // If no list, Verilog 2000: always @ (*)
    if (combo && sequent) {
      nodep->v3warn(E_UNSUPPORTED,
                    "Unsupported: Mixed edge (pos/negedge) and activity "
    ...
    if (combo && !sequent) {
      ...
      // OPTIMIZE: We could substitute a constant for things in the sense list,
      // for example always (posedge RESET) { if (RESET).... }  we know RESET is
      // true. Summarize a long list of combo inputs as just "combo"
#ifndef __COVERITY__ // Else dead code on next line.
      if (combo) {
        oldsensesp->addSensesp(
    ...
    if (oldsensesp) {
      VL_DO_DANGLING(oldsensesp->unlinkFrBackWithNext()->deleteTree(),
                     oldsensesp);
    }
    ...
    if (nodep->varrefp()) {
      if (AstBasicDType *basicp = nodep->varrefp()->dtypep()->basicp()) {
        if (basicp->isEventValue()) {
          // Events need to be treated as active high so we only activate on
          // event being
          // 1
...
void V3Active::activeAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("active", 0,
