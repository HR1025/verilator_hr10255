...
public:
  ...
  static string quoteBackslash(const string &str) {
    return quoteAny(str, '\\', '\\');
  }
  // Replace any %'s with %%
  ...
  // Replace all occurrences of the word 'from' in 'str' with 'to'. A word is
  // considered to be a consecutive sequence of the characters [a-zA-Z0-9_].
  // Sub-words are not replaced. e.g.: replaceWords("one apple bad_apple",
  // "apple", "banana") -> "one banana bad_apple"
  static string replaceWord(const string &str, const string &from,
                            const string &to);
...
public:
  ...
  explicit VHashSha256(const string &data) : VHashSha256{} { insert(data); }
  ~VHashSha256() = default;
...
//######################################################################
// VSpellCheck - Find near-match spelling suggestions given list of
// possibilities

class VSpellCheck final {
  // CONSTANTS
  static constexpr unsigned NUM_CANDIDATE_LIMIT =
      10000; // Avoid searching huge netlists
  static constexpr unsigned LENGTH_LIMIT =
      100; // Maximum string length to search
  // TYPES
...
public:
  ...
  void pushCandidate(const string &s) {
    if (m_candidates.size() < NUM_CANDIDATE_LIMIT)
      m_candidates.push_back(s);
  }
...
private:
  ...
  static void selfTestSuggestOne(bool matches, const string &c,
