...
//                      For now we'll say it's set if at top of block (not under
//                      IF, etc)
//              Need x = _x_dly at bottom of active if "x" is never referenced
//              on LHS
//                      in the active, and above rule applies too.
//                      (If so, use x on LHS, not _x_dly.)
//
//      If a signal is set in multiple always blocks, we need a dly read & set
//      with multiple clock sensitivities.  We have 3 options:
...
//          Since all 3 require a top activation cleanup, we do #2 which is
//          easiest.
//
...
private:
  ...
  //                                       (valid when AstVarScope::user1p is
  //                                       valid)
  //  AstVarScope::user4p()   -> AstAlwaysPost*.  Post block for this variable
  //  AstVarScope::user5p()   -> AstVarRef*. Last blocking or non-blocking
  //  reference AstVar::user2()         -> bool.  Set true if already made
  //  warning AstVarRef::user2()      -> bool.  Set true if already processed
  //  AstVarRef::user5()      -> bool.  Set true if was blocking reference
  //  AstAlwaysPost::user2()  -> ActActive*.  Points to activity block of signal
  //                                      (valid when AstAlwaysPost::user4p is
  //                                      valid)
  //  AstAlwaysPost::user4()  -> AstIf*.  Last IF (__Vdlyvset__) created under
  //  this AlwaysPost
  // Cleared each scope/active:
  //  AstAssignDly::user3()   -> AstVarScope*.  __Vdlyvset__ created for this
  //  assign AstAlwaysPost::user3()  -> AstVarScope*.  __Vdlyvset__ last
  //  referenced in IF
  ...
  AstAssignDly *m_nextDlyp =
      nullptr;              // Next delayed assignment in a list of assignments
  bool m_inDly = false;     // True in delayed assignments
  bool m_inLoop = false;    // True in for loops
  bool m_inInitial = false; // True in initial blocks
  using VarMap =
      std::map<const std::pair<AstNodeModule *, std::string>, AstVar *>;
  VarMap m_modVarMap;       // Table of new var names created under module
  VDouble0 m_statSharedSet; // Statistic tracking
  std::unordered_map<const AstVarScope *, int>
      m_scopeVecMap; // Next var number for each scope
  ...
  AstVarScope *createVarSc(AstVarScope *oldvarscp, const string &name,
                           int width /*0==fromoldvar*/,
                           AstNodeDType *newdtypep) {
    // Because we've already scoped it, we may need to add both the AstVar and
    // the AstVarScope
    UASSERT_OBJ(oldvarscp->scopep(), oldvarscp, "Var unscoped");
    AstVar *varp;
    AstNodeModule *addmodp = oldvarscp->scopep()->modp();
    // We need a new AstVar, but only one for all scopes, to match the new
    // AstVarScope
  ...
  void checkActivePost(AstVarRef *varrefp, AstActive *oldactivep) {
    // Check for MULTIDRIVEN, and if so make new sentree that joins old & new
    // sentree
    UASSERT_OBJ(oldactivep, varrefp,
                "<= old dly assignment not put under sensitivity block");
  ...
  AstNode *createDlyArray(AstAssignDly *nodep, AstNode *lhsp) {
    ...
    UASSERT_OBJ(
        !VN_IS(arrayselp->dtypep()->skipRefp(), UnpackArrayDType), nodep,
        "ArraySel with unpacked arrays should have been removed in V3Slice");
    UINFO(4, "AssignDlyArray: " << nodep << endl);
    //
    //=== Dimensions: __Vdlyvdim__
    std::deque<AstNode *>
        dimvalp; // Assignment value for each dimension of assignment
    AstNode *dimselp = arrayselp;
    for (; VN_IS(dimselp, ArraySel);
         dimselp = VN_AS(dimselp, ArraySel)->fromp()) {
    ...
    UASSERT_OBJ(varrefp->varScopep(), varrefp,
                "Var didn't get varscoped in V3Scope.cpp");
    varrefp->unlinkFrBack();
    AstVar *oldvarp = varrefp->varp();
    const int modVecNum = m_scopeVecMap[varrefp->varScopep()]++;
    //
    std::deque<AstNode *>
        dimreadps; // Read value for each dimension of assignment
    for (unsigned dimension = 0; dimension < dimvalp.size(); dimension++) {
      ...
      if (VN_IS(dimp, Const)) { // bit = const, can just use it
        ...
        const string bitvarname =
            (string("__Vdlyvdim") + cvtToStr(dimension) + "__" +
             oldvarp->shortName() + "__v" + cvtToStr(modVecNum));
        AstVarScope *bitvscp = createVarSc(varrefp->varScopep(), bitvarname,
                                           dimp->width(), nullptr);
        AstAssign *bitassignp = new AstAssign(
            nodep->fileline(),
            new AstVarRef(nodep->fileline(), bitvscp, VAccess::WRITE), dimp);
        nodep->addNextHere(bitassignp);
        dimreadps.push_front(
            new AstVarRef(nodep->fileline(), bitvscp, VAccess::READ));
      }
    }
    //
    ...
    if (bitselp) {
      ...
      if (VN_IS(bitselp->fromp(), Const)) {
        ...
        const string bitvarname =
            (string("__Vdlyvlsb__") + oldvarp->shortName() + "__v" +
             cvtToStr(modVecNum));
        AstVarScope *bitvscp = createVarSc(varrefp->varScopep(), bitvarname,
                                           lsbvaluep->width(), nullptr);
        AstAssign *bitassignp = new AstAssign(
            nodep->fileline(),
            new AstVarRef(nodep->fileline(), bitvscp, VAccess::WRITE),
            lsbvaluep);
        nodep->addNextHere(bitassignp);
    ...
    if (VN_IS(nodep->rhsp(), Const)) {
      ...
      string valvarname = (string("__Vdlyvval__") + oldvarp->shortName() +
                           "__v" + cvtToStr(modVecNum));
      AstVarScope *valvscp = createVarSc(varrefp->varScopep(), valvarname, 0,
                                         nodep->rhsp()->dtypep());
    ...
    if (nodep->user3p()) {
      // Simplistic optimization.  If the previous statement in same scope was
      // also a =>, then we told this nodep->user3 we can use its Vdlyvset
      // rather than making a new one. This is good for code like:
      //    for (i=0; i<5; i++)  vector[i] <= something;
      setvscp = VN_AS(nodep->user3p(), VarScope);
      ++m_statSharedSet;
    } else { // Create new one
      string setvarname = (string("__Vdlyvset__") + oldvarp->shortName() +
                           "__v" + cvtToStr(modVecNum));
    ...
    for (int dimension = int(dimreadps.size()) - 1; dimension >= 0;
         --dimension) {
      selectsp =
          new AstArraySel(nodep->fileline(), selectsp, dimreadps[dimension]);
    }
    if (bitselp) {
    ...
    if (finalp) {
      ...
      if (setinitp)
        oldactivep->addStmtsp(setinitp);
    } else { // first time we've dealt with this memory
      finalp = new AstAlwaysPost(nodep->fileline(), nullptr /*sens*/,
                                 nullptr /*body*/);
    ...
    if (finalp->user3p() == setvscp) {
      ...
      finalp->user4p(
          postLogicp); // and the associated IF, as we may be able to reuse it
    }
  ...
  virtual void visit(AstAssignDly *nodep) override {
    ...
    m_nextDlyp =
        VN_CAST(nodep->nextp(),
                AssignDly); // Next assignment in same block, maybe nullptr.
    if (m_cfuncp) {
      nodep->v3warn(
          E_UNSUPPORTED,
    ...
    if (VN_IS(nodep->lhsp(), ArraySel) ||
      ...
      if (m_inLoop) {
        nodep->v3warn(BLKLOOPINIT,
                      "Unsupported: Delayed assignment to array inside for "
    ...
    if (!nodep->user2Inc()) { // Not done yet
      if (m_inDly && nodep->access().isWriteOrRW()) {
        ...
        if (!m_activep->hasClocked()) {
          nodep->v3error(
              "Internal: Blocking <= assignment in non-clocked block, should "
        ...
        if (!dlyvscp) { // First use of this delayed variable
          const string newvarname =
              (string("__Vdly__") + nodep->varp()->shortName());
...
void V3Delayed::delayedAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("delayed", 0,
