...
public:
  ...
  static void setenvStr(const string &envvar, const string &value,
                        const string &why);
  ...
  static string
  filenameNonDirExt(const string &filename) { ///< Return basename of filename
    return filenameNonExt(filenameNonDir(filename));
  }
  static string
  filenameDir(const string &filename); ///< Return directory part of filename
  /// Return filename with env vars removed
  static string filenameSubstitute(const string &filename);
  static string
  filenameRealPath(const string &filename); ///< Return realpath of filename
  ...
  static void
  u_sleep(int64_t usec); ///< Sleep for a given number of microseconds.
  /// Return wall time since epoch in microseconds, or 0 if not implemented
  static uint64_t timeUsecs();
  static uint64_t
  memUsageBytes(); ///< Return memory usage in bytes, or 0 if not implemented
