...
//   Search for dtypes referencing class, and create CUse for forward
//   declaration
//
//*************************************************************************
...
class CUseVisitor final : public AstNVisitor {
  ...
  virtual void visit(AstClassRefDType *nodep) override {
    ...
    // Need to include extends() when we implement, but no need for pointers to
    // know
  ...
  virtual void visit(AstNodeDType *nodep) override {
    if (nodep->user1SetOnce())
      return; // Process once
    if (nodep->virtRefDTypep())
      iterate(nodep->virtRefDTypep());
    if (nodep->virtRefDType2p())
      iterate(nodep->virtRefDType2p());
  }
  virtual void visit(AstNode *nodep) override {
    if (nodep->user1SetOnce())
      return; // Process once
    if (nodep->dtypep() && !nodep->dtypep()->user1())
...
public:
  // CONSTRUCTORS
  explicit CUseVisitor(AstNodeModule *modp) : m_modp(modp) { iterate(modp); }
...
void V3CUse::cUseAll() {
  ...
  V3Global::dumpCheckGlobalTree("cuse", 0,
