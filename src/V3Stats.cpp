...
private:
  ...
  using NameMap =
      std::map<const std::string, int>; // Number of times a name appears
  ...
  VDouble0 m_statAbove[AstType::_ENUM_END]
                      [AstType::_ENUM_END]; // Nodes of given type
  std::array<VDouble0, VBranchPred::_ENUM_END>
      m_statPred;           // Nodes of given type
  ...
  void allNodes(AstNode *nodep) {
    ...
    if (m_counting) {
      ...
      if (m_cfuncp && !m_cfuncp->slow())
        m_statInstrFast += nodep->instrCount();
    }
  }
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    if (!m_fast) {
      ...
      // Need to do even if !m_counting because maybe determining upstream
      // if/else
      double ifInstrs = 0.0;
...
public:
  ...
  virtual ~StatsVisitor() override {
    ...
    V3Stats::addStat(m_stage, "Instruction count, fast critical",
                     m_statInstrFast);
    // Vars
    ...
    for (int type = 0; type < AstType::_ENUM_END; type++) {
      ...
      if (count != 0.0) {
        V3Stats::addStat(m_stage,
                         string("Node count, ") + AstType(type).ascii(), count);
      }
    }
    for (int type = 0; type < AstType::_ENUM_END; type++) {
      for (int type2 = 0; type2 < AstType::_ENUM_END; type2++) {
        ...
        if (count != 0.0) {
          V3Stats::addStat(m_stage,
                           (string("Node pairs, ") + AstType(type).ascii() +
                            "_" + AstType(type2).ascii()),
                           count);
        }
      }
    }
    ...
    for (int type = 0; type < VBranchPred::_ENUM_END; type++) {
      ...
      if (count != 0.0) {
        V3Stats::addStat(
            m_stage,
            (string("Branch prediction, ") + VBranchPred(type).ascii()), count);
      }
    }
  }
};
