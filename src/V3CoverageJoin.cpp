...
private:
  ...
  void detectDuplicates() {
    ...
    for (AstCoverToggle *nodep : m_toggleps)
      dupFinder.insert(nodep->origp());
    // Find if there are any duplicates
    for (AstCoverToggle *nodep : m_toggleps) {
      // nodep->backp() is null if we already detected it's a duplicate and
      // unlinked it.
      if (nodep->backp()) {
        // Want to choose a base node, and keep finding duplicates that are
        // identical. This prevents making chains where a->b, then c->d, then
        // b->c, as we'll find a->b, a->c, a->d directly.
        while (true) {
...
void V3CoverageJoin::coverageJoin(AstNetlist *rootp) {
  ...
  V3Global::dumpCheckGlobalTree("coveragejoin", 0,
