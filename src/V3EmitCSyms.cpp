...
class EmitCSyms final : EmitCBaseVisitor {
  ...
  struct ScopeData {
    ...
    ScopeData(const string &symName, const string &prettyName, int timeunit,
        ...
        : m_symName{symName}, m_prettyName{prettyName},
          m_timeunit{timeunit}, m_type{type} {}
  };
  struct ScopeFuncData {
    ...
    ScopeFuncData(AstScopeName *scopep, AstCFunc *funcp, AstNodeModule *modp)
        : m_scopep{scopep}, m_cfuncp{funcp}, m_modp{modp} {}
  };
  struct ScopeVarData {
    string m_scopeName;
    ...
    ScopeVarData(const string &scopeName, const string &varBasePretty,
        ...
        : m_scopeName{scopeName}, m_varBasePretty{varBasePretty}, m_varp{varp},
          m_modp{modp}, m_scopep{scopep} {}
  };
  ...
  std::map<const std::string, ScopeFuncData>
      m_scopeFuncs; // Each {scope,dpi-export-func}
  std::map<const std::string, ScopeVarData>
      m_scopeVars;                            // Each {scope,public-var}
  ...
  static void nameCheck(AstNode *nodep) {
    ...
    if (nodep->name() != "" &&
        !(VN_IS(nodep, CFunc) && (VN_AS(nodep, CFunc)->isConstructor() ||
  ...
  static string scopeSymString(const string &scpname) {
    ...
    while ((pos = out.find("__PVT__")) != string::npos)
      out.replace(pos, 7, "");
    if (out.substr(0, 10) == "TOP__DOT__")
      out.replace(0, 10, "");
    if (out.substr(0, 4) == "TOP.")
      out.replace(0, 4, "");
  ...
  void varsExpand() {
    // We didn't have all m_scopes loaded when we encountered variables, so
    // expand them now It would be less code if each module inserted its own
    // variables. Someday.  For now public isn't common.
    for (std::vector<ScopeModPair>::iterator itsc = m_scopes.begin();
      ...
      for (std::vector<ModVarPair>::iterator it = m_modVars.begin();
        ...
        if (modp == smodp) {
          ...
          string scpName;
          string varBase;
          if (whole.substr(0, 10) == "__DOT__TOP")
            whole.replace(0, 10, "");
          ...
          m_scopeVars.insert(std::make_pair(
              scpSym + " " + varp->name(),
  ...
  virtual void visit(AstCellInline *nodep) override {
    if (v3Global.opt.vpi()) {
      const string type = (nodep->origModName() == "__BEGIN__")
                              ? "SCOPE_OTHER"
                              : "SCOPE_MODULE";
      ...
      m_vpiScopeCandidates.insert(std::make_pair(
          name, ScopeData(scopeSymString(name), name_dedot, timeunit, type)));
    }
  }
  virtual void visit(AstScope *nodep) override {
    if (VN_IS(m_modp, Class))
      return; // The ClassPackage is what is visible
    nameCheck(nodep);
...
void EmitCSyms::emitSymHdr() {
  ...
  if (v3Global.opt.profThreads())
    puts("#include \"verilated_profiler.h\"\n");
  ...
  for (AstNodeModule *nodep = v3Global.rootp()->modulesp(); nodep;
    ...
    if (VN_IS(nodep, Class))
      continue; // Class included earlier
  ...
  if (v3Global.opt.profThreads()) {
    ...
    if (v3Global.opt.mtasks()) {
      for (const V3GraphVertex *vxp =
        ...
        if (maxProfilerId < mtp->profilerId())
          maxProfilerId = mtp->profilerId();
      }
    }
  ...
  puts("\n// CONSTRUCTORS\n");
  puts(symClassName() + "(VerilatedContext* contextp, const char* namep, " +
       topClassName() + "* modelp);\n");
...
void EmitCSyms::checkSplit(bool usesVfinal) {
  if (m_ofp && (!v3Global.opt.outputSplitCFuncs() ||
                m_numStmts < v3Global.opt.outputSplitCFuncs())) {
    return;
  }
  ...
  m_numStmts = 0;
  string filename = v3Global.opt.makeDir() + "/" + symClassName() + "__" +
  ...
  emitSymImpPreamble();
  puts("void " + symClassName() + "::" + symClassName() + "_" +
       cvtToStr(m_funcNum) + "(");
  if (usesVfinal) {
    puts("int __Vfinal");
  }
  ...
  if (v3Global.opt.vpi()) {
    ...
    for (ScopeNames::const_iterator it = m_scopeNames.begin();
      ...
      if ((name.find('.') == string::npos) &&
          (it->second.m_type == "SCOPE_MODULE")) {
        puts("__Vhier." + method + "(0, &" +
             protect("__Vscope_" + it->second.m_symName) + ");\n");
      }
    }

    for (auto it = m_vpiScopeHierarchy.cbegin();
         it != m_vpiScopeHierarchy.cend(); ++it) {
...
void EmitCSyms::emitSymImp() {
  ...
  const string filename =
      v3Global.opt.makeDir() + "/" + symClassName() + ".cpp";
  AstCFile *cfilep = newCFile(filename, true /*slow*/, true /*source*/);
  cfilep->support(true);
  ...
  if (v3Global.opt.savable()) {
    for (const bool &de : {false, true}) {
      ...
      puts("void " + symClassName() + "::" + protect(funcname) + "(" +
           classname + "& os) {\n");
      puts("// Internal state\n");
      if (v3Global.opt.trace())
  ...
  if (v3Global.opt.mtasks())
    puts("delete __Vm_threadPoolp;\n");
  puts("}\n\n");

  // Constructor
  puts(symClassName() + "::" + symClassName() +
       "(VerilatedContext* contextp, const char* namep," + topClassName() +
       "* modelp)\n");
  ...
  puts("// Setup each module's pointer back to symbol table (for public "
       "functions)\n");
  for (const auto &i : m_scopes) {
    AstScope *scopep = i.first;
    AstNodeModule *modp = i.second;
  ...
  if (!m_scopeNames.empty()) { // Setup scope names
    puts("// Setup scopes\n");
    for (ScopeNames::iterator it = m_scopeNames.begin();
         it != m_scopeNames.end(); ++it) {
      checkSplit(false);
  ...
  // Everything past here is in the __Vfinal loop, so start a new split file if
  // needed
  closeSplit();
  ...
  if (v3Global.dpi()) {
    ...
    for (auto it = m_scopeFuncs.begin(); it != m_scopeFuncs.end(); ++it) {
      ...
      if (funcp->dpiExportImpl()) {
        checkSplit(true);
        puts(protect("__Vscope_" + scopep->scopeSymName()) +
             ".exportInsert(__Vfinal, ");
        putsQuoted(
            funcp->cname()); // Not protected - user asked for import/export
        puts(", (void*)(&");
    ...
    for (auto it = m_scopeVars.begin(); it != m_scopeVars.end(); ++it) {
      ...
      if (AstBasicDType *basicp = varp->basicp()) {
        ...
        for (AstNodeDType *dtypep = varp->dtypep(); dtypep;) {
          dtypep = dtypep->skipRefp(); // Skip AstRefDType/AstTypedef, or return
                                       // same node
          if (const AstNodeArrayDType *adtypep =
                  VN_CAST(dtypep, NodeArrayDType)) {
            bounds += " ,";
      ...
      puts(protect("__Vscope_" + it->second.m_scopeName) +
           ".varInsert(__Vfinal,");
...
void EmitCSyms::emitDpiHdr() {
  UINFO(6, __FUNCTION__ << ": " << endl);
  const string filename =
      v3Global.opt.makeDir() + "/" + topClassName() + "__Dpi.h";
  ...
  m_ofp->putsHeader();
  puts("// DESCR"
       "IPTION: Verilator output: Prototypes for DPI import and export "
       "functions.\n");
  puts("//\n");
  puts("// Verilator includes this file in all generated .cpp files that use "
       "DPI functions.\n");
  puts("// Manually include this file where DPI .c import functions are "
       "declared to ensure\n");
...
void EmitCSyms::emitDpiImp() {
  UINFO(6, __FUNCTION__ << ": " << endl);
  const string filename =
      v3Global.opt.makeDir() + "/" + topClassName() + "__Dpi.cpp";
  ...
  puts("// function names, you will get multiple definition link errors from "
       "here.\n");
