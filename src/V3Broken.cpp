...
// This visitor does not edit nodes, and is called at error-exit, so should use
// constant iterators
...
static class BrokenCntGlobal {
  // This is a 7 bit generation counter, stored in the bottom 7 bits of
  // AstNode::m_brokenState, used to mark a node as being present under the root
  // AstNetlist in the current traversal. A value 0 is invalid, as the AstNode
  // constructor uses that to initialize m_brokenState
...
public:
  uint8_t get() {
    UASSERT(MIN_VALUE <= m_count && m_count <= MAX_VALUE,
...
private:
  // MEMBERS
  std::unordered_set<const AstNode *>
      m_allocated; // Set of all nodes allocated but not freed
...
public:
  ...
  void deleted(const AstNode *nodep) {
    ...
    if (VL_UNCOVERABLE(m_allocated.erase(nodep) == 0)) {
      nodep->v3fatalSrc(
          "Deleting AstNode object that was not allocated or already freed");
    }
    // LCOV_EXCL_STOP
  }
  bool isAllocated(const AstNode *nodep) const {
  ...
  void checkForLeaks() {
    ...
    // Those with backp() are probably under a parent that was leaked and has no
    // backp()
...
private:
  // MEMBERS
  std::unordered_set<const AstNode *>
      m_linkable; // Set of all nodes allocated but not freed

public:
  // METHODS
  void clear() { m_linkable.clear(); }
  inline void addLinkable(const AstNode *nodep) { m_linkable.emplace(nodep); }
  inline bool isLinkable(const AstNode *nodep) const {
    return m_linkable.count(nodep) != 0;
  }
} s_linkableTable;

bool V3Broken::isLinkable(const AstNode *nodep) {
  return s_linkableTable.isLinkable(nodep);
}
...
private:
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    if (nodep->maybePointedTo())
      s_linkableTable.addLinkable(nodep);
...
class BrokenCheckVisitor final : public AstNVisitor {
  ...
  const uint8_t m_brokenCntCurrentNotUnder =
      s_brokenCntGlobal.get(); // Top bit is clear
  const uint8_t m_brokenCntCurrentUnder =
      m_brokenCntCurrentNotUnder | 0x80; // Top bit is set
  ...
  // Variable references in current function that do not reference an in-scope
  // local
...
private:
  ...
  void processEnter(AstNode *nodep) {
    ...
    if (nodep->dtypep()) {
      UASSERT_OBJ(nodep->dtypep()->brokeExists(), nodep,
                  "Broken link in node->dtypep() to "
                      << cvtToHex(nodep->dtypep()));
      UASSERT_OBJ(nodep->dtypep(), nodep,
                  "Non-dtype link in node->dtypep() to "
                      << cvtToHex(nodep->dtypep()));
    }
    if (v3Global.assertDTypesResolved()) {
      ...
      if (const AstNodeDType *dnodep = VN_CAST(nodep, NodeDType))
        checkWidthMin(dnodep);
    }
    checkWidthMin(nodep);
  }
  void processExit(AstNode *nodep) {
    nodep->brokenState(m_brokenCntCurrentNotUnder);
  }
  ...
  void pushLocalScope() {
    if (m_cfuncp)
      m_localsStack.emplace_back();
  }
  void popLocalScope() {
    if (m_cfuncp)
      m_localsStack.pop_back();
  }
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    processAndIterate(nodep);
    // m_inScope because some Vars have initial variable references without
    // scopes This might false fire with some debug flags, as not certain we
    // don't have temporary clear varScopep's during some an infrequent dump
    // just before we re-LinkDot.
    UASSERT_OBJ(!(v3Global.assertScoped() && m_inScope && nodep->varp() &&
                  !nodep->varScopep()),
                nodep, "VarRef missing VarScope pointer");
    if (m_cfuncp) {
  ...
  virtual void visit(AstCFunc *nodep) override {
    ...
    for (const auto &pair : m_suspectRefs) {
      UASSERT_OBJ(
          m_localVars.count(pair.first) == 0, pair.second,
  ...
  virtual void visit(AstNodeStmt *nodep) override {
    // For local variable checking act as if any statement introduces a new
    // scope. This is aggressive but conservatively correct.
    pushLocalScope();
    processAndIterate(nodep);
    popLocalScope();
  }
...
void V3Broken::brokenAll(AstNetlist *nodep) {
  ...
  if (VL_UNCOVERABLE(inBroken)) {
    ...
    UINFO(
        1,
        "Broken called under broken, skipping recursion.\n"); // LCOV_EXCL_LINE
  } else {
    inBroken = true;
...
void V3Broken::selfTest() {
  // Exercise addNewed and deleted for coverage, as otherwise only used with
  // VL_LEAK_CHECKS
