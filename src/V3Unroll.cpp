...
//      (Eventually, a better way would be to simulate the entire loop; ala
//      V3Table.) Convert remaining FORs to WHILEs
//
//*************************************************************************
...
private:
  ...
  bool cantUnroll(AstNode *nodep, const char *reason) {
    if (m_generate)
      nodep->v3warn(E_UNSUPPORTED,
                    "Unsupported: Can't unroll generate for; " << reason);
  ...
  bool bodySizeOverRecurse(AstNode *nodep, int &bodySize, int bodyLimit) {
    ...
    if (bodySize > bodyLimit)
      return true;
    if (bodySizeOverRecurse(nodep->op1p(), bodySize, bodyLimit))
      return true;
    if (bodySizeOverRecurse(nodep->op2p(), bodySize, bodyLimit))
      return true;
    if (bodySizeOverRecurse(nodep->op3p(), bodySize, bodyLimit))
      return true;
    if (bodySizeOverRecurse(nodep->op4p(), bodySize, bodyLimit))
      return true;
    // Tail recurse.
  ...
  bool forUnrollCheck(AstNode *nodep,
                      AstNode *initp, // Maybe under nodep (no nextp), or
                                      // standalone (ignore nextp)
    ...
    if (initp)
      UINFO(6, "    Init " << initp << endl);
    if (precondsp)
      UINFO(6, "    Pcon " << precondsp << endl);
    if (condp)
      UINFO(6, "    Cond " << condp << endl);
    if (incp)
      UINFO(6, "    Inc  " << incp << endl);

    // Initial value check
    AstAssign *initAssp = VN_CAST(initp, Assign);
    if (!initAssp)
      return cantUnroll(nodep, "no initial assignment");
    ...
    if (!incAssp)
      return cantUnroll(nodep, "no increment assignment");
    if (incAssp->nextp())
      return cantUnroll(nodep, "multiple increments");
    ...
    if (VN_IS(nodep, GenFor) && !m_forVarp->isGenVar()) {
      nodep->v3error(
          "Non-genvar used in generate for: " << m_forVarp->prettyNameQ());
    ...
    // and the initp statements will reference a variable outside of the initp
    // scope alas, failing to simulate.
    AstConst *constInitp = VN_CAST(initAssp->rhsp(), Const);
    if (!constInitp)
      return cantUnroll(nodep, "non-constant initializer");
    ...
    if (m_varAssignHit)
      return cantUnroll(nodep, "genvar assigned *inside* loop");
    ...
    if (!m_generate) {
      ...
      if (!canSimulate(condp))
        return cantUnroll(condp, "Unable to simulate condition");
      ...
      if (loops > 0)
        bodyLimit = v3Global.opt.unrollStmts() / loops;
  ...
  bool simulateTree(AstNode *nodep, const V3Number *loopValue, AstNode *dtypep,
    ...
    if (loopValue) {
      ...
      AstBegin *tempp =
          new AstBegin(nodep->fileline(), "[EditWrapper]", clonep);
  ...
  bool countLoops(AstAssign *initp, AstNode *condp, AstNode *incp, int max,
                  int &outLoopsr) {
    outLoopsr = 0;
  ...
  bool forUnroller(AstNode *nodep, AstAssign *initp, AstNode *condp,
                   AstNode *precondsp, AstNode *incp, AstNode *bodysp) {
    ...
    if (incp &&
        !VN_IS(nodep, GenFor)) { // Generates don't need to increment loop index
      ...
      while (true) {
        ...
        if (!res.isEqOne()) {
          ...
          if (m_generate) {
            ...
            oneloopp =
                new AstBegin(oneloopp->fileline(), nname, oneloopp, true);
          }
          VL_DO_CLEAR(pushDeletep(m_varValuep), m_varValuep = nullptr);
          if (newbodysp) {
          ...
          if (++times > unrollCount() * 3) {
            nodep->v3error("Loop unrolling took too long;"
                           " probably this is an infinite loop, or set "
                           "--unroll-count above "
                           << unrollCount());
            break;
          }
          ...
          if (!simulateTree(incpass->rhsp(), &loopValue, incpass,
                            newLoopValue)) {
            nodep->v3error("Loop unrolling failed");
            return false;
          }
    ...
    if (bodysp)
      VL_DO_DANGLING(pushDeletep(bodysp), bodysp);
    if (precondsp)
      VL_DO_DANGLING(pushDeletep(precondsp), precondsp);
    if (initp)
      VL_DO_DANGLING(pushDeletep(initp), initp);
    if (incp && !incp->backp())
      VL_DO_DANGLING(pushDeletep(incp), incp);
    if (debug() >= 9 && newbodysp)
      newbodysp->dumpTree(cout, "-  _new: ");
    return true;
  }
  ...
  virtual void visit(AstWhile *nodep) override {
    ...
    if (m_varModeCheck || m_varModeReplace) {
      ...
      if (nodep->condp())
        V3Const::constifyEdit(nodep->condp()); // condp may change
      // Grab initial value
      AstNode *initp = nullptr; // Should be statement before the while.
      if (nodep->backp()->nextp() == nodep)
        initp = nodep->backp();
      if (initp)
        VL_DO_DANGLING(V3Const::constifyEdit(initp), initp);
      if (nodep->backp()->nextp() == nodep)
        initp = nodep->backp();
      // Grab assignment
      ...
      if (nodep->incsp())
        V3Const::constifyEdit(nodep->incsp());
      // cppcheck-suppress duplicateCondition
      if (nodep->incsp()) {
        incp = nodep->incsp();
      } else {
        for (incp = nodep->bodysp(); incp && incp->nextp();
             incp = incp->nextp()) {
        }
        if (incp)
          VL_DO_DANGLING(V3Const::constifyEdit(incp), incp);
  ...
  virtual void visit(AstGenFor *nodep) override {
    ...
    if (m_varModeCheck || m_varModeReplace) {
      ...
      // Constify before unroll call, as it may change what is underneath.
      if (nodep->initsp())
        V3Const::constifyEdit(nodep->initsp()); // initsp may change
      if (nodep->condp())
        V3Const::constifyEdit(nodep->condp()); // condp may change
      if (nodep->incsp())
        V3Const::constifyEdit(nodep->incsp()); // incsp may change
  ...
  virtual void visit(AstVarRef *nodep) override {
    if (m_varModeCheck && nodep->varp() == m_forVarp &&
        nodep->varScopep() == m_forVscp && nodep->access().isWriteOrRW()) {
      UINFO(8, "   Itervar assigned to: " << nodep << endl);
      m_varAssignHit = true;
    }

    if (m_varModeReplace && nodep->varp() == m_forVarp &&
        nodep->varScopep() == m_forVscp && nodep->access().isReadOnly()) {
...
void V3Unroll::unrollAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("unroll", 0,
