...
enum class Mergeable {
  ...
  NO_COND_ASSIGN, // Tree cannot be merged because it contains an assignment to
                  // a condition
...
private:
  // STATE
  bool m_condAssign =
      false; // Does this tree contain an assignment to a condition variable??
...
private:
  ...
  // AstNode::user2       -> Flag marking node as included in merge because
  // cheap to duplicate
  ...
  CheckMergeableVisitor
      m_checkMergeable;       // Sub visitor for encapsulation & speed
  ...
  // Predicate to check if an expression yields only 0 or 1 (i.e.: a 1-bit
  // value)
  static bool yieldsOneOrZero(const AstNode *nodep) {
    ...
    if (const AstShiftR *const shiftp = VN_CAST(nodep, ShiftR)) {
      ...
      if (const AstConst *const constp = VN_CAST(shiftp->rhsp(), Const)) {
        ...
        const int width = vrefp && !vrefp->varp()->dtypep()->isSigned()
                              ? vrefp->varp()->widthMin()
                              : shiftp->width();
        if (constp->toSInt() >= width - 1)
          return true;
      }
      return false;
    }
    if (VN_IS(nodep, Eq) || VN_IS(nodep, Neq) || VN_IS(nodep, Lt) ||
        VN_IS(nodep, Lte) || VN_IS(nodep, Gt) || VN_IS(nodep, Gte)) {
      return true;
    }
    ...
    if (const AstNodeCond *const condp = VN_CAST(nodep, NodeCond)) {
      return yieldsOneOrZero(condp->expr1p()) &&
             yieldsOneOrZero(condp->expr2p());
    }
  ...
  // Apply (1'b1 & _) cleaning mask if necessary. This is required because this
  // pass is after V3Clean, and sometimes we have an AstAnd with a 1-bit
  // condition on one side, but a more than 1-bit value on the other side, so we
  // need to keep only the LSB.
  static AstNode *maskLsb(AstNode *nodep) {
    if (yieldsOneOrZero(nodep))
      return nodep;
  ...
  AstNode *foldAndUnlink(AstNode *rhsp, bool condTrue) {
    if (rhsp->sameTree(m_mgCondp)) {
      ...
      AstNode *const resp = condTrue ? condp->expr1p()->unlinkFrBack()
      ...
      if (AstAnd *const andp = VN_CAST(rhsp, And)) {
        ...
        return new AstAnd{andp->fileline(), andp->lhsp()->cloneTree(false),
                          resp};
      }
    } else if (AstAnd *const andp = VN_CAST(rhsp, And)) {
    ...
    } else if (VN_IS(rhsp, WordSel) || VN_IS(rhsp, VarRef) ||
               VN_IS(rhsp, Const)) {
  ...
  void mergeEnd(int lineno) {
    ...
    // We might want to recursively merge an AstIf. We stash it in this
    // variable.
    ...
    if (m_mgFirstp != m_mgLastp) {
      UINFO(6, "MergeCond - First: " << m_mgFirstp << " Last: " << m_mgLastp
                                     << endl);
      ++m_statMerges;
      if (m_listLenght > m_statLongestList)
        m_statLongestList = m_listLenght;
      ...
      AstIf *const resultp =
          new AstIf(m_mgCondp->fileline(), m_mgCondp, nullptr, nullptr);
      ...
      // There was nothing to merge this AstNodeIf with, but try to merge it's
      // branches
      recursivep = ifp;
    }
    // Reset state
    ...
    if (AstNodeAssign *const assignp = VN_CAST(nodep, NodeAssign)) {
      ...
      if (assignp->lhsp()->widthMin() == 1) {
        ...
        if (AstAnd *const andp = VN_CAST(assignp->rhsp(), And)) {
          if (andp->lhsp()->sameTree(m_mgCondp) ||
              andp->rhsp()->sameTree(m_mgCondp)) {
            return true;
          }
        }
        // Is it simply 'lhs = cond'?
        if (assignp->rhsp()->sameTree(m_mgCondp))
          return true;
      }
    }
    return false;
  ...
  // Check if this node is cheap enough that duplicating it in two branches of
  // an AstIf and is hence not likely to cause a performance degradation if
  // doing so.
  bool isCheapNode(AstNode *nodep) const {
    if (VN_IS(nodep, Comment))
      return true;
    if (AstNodeAssign *const assignp = VN_CAST(nodep, NodeAssign)) {
      // Check LHS
      ...
      if (!VN_IS(rhsp, VarRef) && !VN_IS(rhsp, Const))
        return false;
  ...
  void addToList(AstNode *nodep, AstNode *condp, int line) {
    ...
    if (!m_mgFirstp) {
      UASSERT_OBJ(condp, nodep,
                  "Cannot start new list without condition " << line);
      m_mgFirstp = nodep;
      m_mgCondp = condp;
      m_listLenght = 0;
      m_markVars.mark(condp);
      // Add any preceding nodes to the list that would allow us to extend the
      // merge range
      for (;;) {
        AstNode *const backp = m_mgFirstp->backp();
        if (!backp || backp->nextp() != m_mgFirstp)
          break; // Don't move up the tree
        if (m_checkMergeable(backp) != Mergeable::YES)
          break;
  ...
  // If this node is the next expected node and is helpful to add to the list,
  // do so, otherwise end the current merge. Return ture if added, false if
  // ended merge.
  ...
  void mergeEndIfIncompatible(AstNode *nodep, AstNode *condp) {
    if (m_mgFirstp && (m_mgNextp != nodep || !condp->sameTree(m_mgCondp))) {
      // Node in different list, or has different condition. Finish current
      // list.
      mergeEnd(__LINE__);
    }
  }
  ...
  virtual void visit(AstNodeStmt *nodep) override {
    if (m_mgFirstp && addIfHelpfulElseEndMerge(nodep))
      return;
    iterateChildren(nodep);
  }
  virtual void visit(AstNode *nodep) override {}
...
public:
  ...
  virtual ~MergeCondVisitor() override {
    ...
    V3Stats::addStat("Optimizations, MergeCond merged items",
                     m_statMergedItems);
    V3Stats::addStat("Optimizations, MergeCond longest merge",
                     m_statLongestList);
  }
};
...
void V3MergeCond::mergeAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("merge_cond", 0,
