...
private:
  // NODE STATE
  //  AstNode::user4() -> V3Hash.  Hash value of this node (hash of 0 is
  //  illegal)
  ...
  V3Hash hashNodeAndIterate(AstNode *nodep, bool hashDType, bool hashChildren,
    ...
    if (m_cacheInUser4 && nodep->user4()) {
      ...
      if (hashDType && nodep != nodep->dtypep())
        iterateNull(nodep->dtypep()); // Node dtype
      if (hashChildren)
        iterateChildrenConst(nodep); // Children
      if (m_cacheInUser4)
        nodep->user4(m_hash.value());
      return m_hash;
    }
  }
  ...
  virtual void visit(AstInitArray *nodep) override {
    // Hash unpacked array initializers by value, as the order of initializer
    // nodes does not matter, and we want semantically equivalent initializers
    // to map to the same hash.
    AstUnpackArrayDType *const dtypep =
        VN_CAST(nodep->dtypep(), UnpackArrayDType);
    m_hash += hashNodeAndIterate(
        nodep, HASH_DTYPE, /* hashChildren: */ !dtypep, [=]() {
          if (dtypep) {
...
public:
  // CONSTRUCTORS
  explicit HasherVisitor(AstNode *nodep) : m_cacheInUser4{true} {
    iterate(nodep);
  }
  explicit HasherVisitor(const AstNode *nodep) : m_cacheInUser4{false} {
