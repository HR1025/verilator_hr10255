...
public:
  explicit EmitCLazyDecls(EmitCBaseVisitor &emitter) : m_emitter(emitter) {}
...
private:
  ...
  std::vector<AstChangeDet *>
      m_blkChangeDetVec;        // All encountered changes in block
...
public:
  ...
  void splitSizeInc(AstNode *nodep) {
    splitSizeInc(EmitCBaseCounterVisitor(nodep).count());
  }
  void splitSizeReset() { m_splitSize = 0; }
  bool splitNeeded() const {
    return v3Global.opt.outputSplit() &&
           m_splitSize >= v3Global.opt.outputSplit();
  }
  ...
  void displayArg(AstNode *dispp, AstNode **elistp, bool isScan,
                  const string &vfmt, bool ignore, char fmtLetter);
  ...
  void emitScIQW(AstVar *nodep) {
    UASSERT_OBJ(nodep->isSc(), nodep,
                "emitting SystemC operator on non-SC variable");
    // clang-format off
  ...
  void emitOpName(AstNode *nodep, const string &format, AstNode *lhsp,
                  AstNode *rhsp, AstNode *thsp);
  ...
  void emitConstant(AstConst *nodep, AstVarRef *assigntop,
                    const string &assignString);
  ...
  string emitVarResetRecurse(const AstVar *varp, const string &varNameProtected,
                             AstNodeDType *dtypep, int depth,
                             const string &suffix);
  void doubleOrDetect(AstChangeDet *changep, bool &gotOne);
  void emitChangeDet();
  void emitConstInit(AstNode *initp) {
    // We should refactor emit to produce output into a provided buffer, not go
    // through members variables. That way we could just invoke the appropriate
    // emitter as needed.
  ...
  virtual void visit(AstCFunc *nodep) override {
    ...
    if (nodep->isLoose()) {
      m_lazyDecls.declared(
          nodep);               // Defined here, so no longer needs declaration
    ...
    for (AstNode *subnodep = nodep->argsp(); subnodep;
         subnodep = subnodep->nextp()) {
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    if (AstSel *selp = VN_CAST(nodep->lhsp(), Sel)) {
      ...
      // Wide functions assign into the array directly, don't need separate
      // assign statement
  ...
  virtual void visit(AstAssocSel *nodep) override {
    ...
    AstAssocArrayDType *adtypep =
        VN_AS(nodep->fromp()->dtypep(), AssocArrayDType);
  ...
  virtual void visit(AstCMethodCall *nodep) override {
    const AstCFunc *const funcp = nodep->funcp();
    UASSERT_OBJ(!funcp->isLoose(), nodep,
                "Loose method called via AstCMethodCall");
    iterate(nodep->fromp());
    putbs("->");
    puts(funcp->nameProtect());
    emitCCallArgs(nodep, "");
  ...
  virtual void visit(AstCNew *nodep) override {
    ...
    puts("vlSymsp"); // TODO make this part of argsp, and eliminate when
                     // unnecessary
    if (nodep->argsp())
      comma = true;
    for (AstNode *subnodep = nodep->argsp(); subnodep;
  ...
  virtual void visit(AstDumpCtl *nodep) override {
    ...
    case VDumpCtlType::OFF:
      // Currently ignored as both Vcd and Fst do not support them, as would
      // need "X" dump
      break;
    case VDumpCtlType::ON:
      ...
      break;
    default:
      nodep->v3fatalSrc("Bad case, unexpected " << nodep->ctlType().ascii());
    }
  }
  ...
  virtual void visit(AstSFormatF *nodep) override {
    displayNode(nodep, nodep->scopeNamep(), nodep->text(), nodep->exprsp(),
                false);
  }
  ...
  void checkMaxWords(AstNode *nodep) {
    if (nodep->widthWords() > VL_VALUE_STRING_MAX_WORDS) {
      nodep->v3error("String of "
                     << nodep->width()
                     << " bits exceeds hardcoded limit "
                        "VL_VALUE_STRING_MAX_WORDS in verilatedos.h");
    }
  }
  ...
  virtual void visit(AstNodeReadWriteMem *nodep) override {
    ...
    uint32_t array_lo = 0;
    {
      ...
      if (!varrefp) {
        ...
        // nodep->memp() below will when verilated code is compiled create a C++
        // template
      } else if (const AstUnpackArrayDType *adtypep = VN_CAST(
                     varrefp->varp()->dtypeSkipRefp(), UnpackArrayDType)) {
        putbs(", ");
  ...
  virtual void visit(AstFClose *nodep) override {
    ...
    puts("); ");
    iterateAndNextNull(
        nodep->filep()); // For safety, so user doesn't later WRITE with it.
    puts(" = 0;\n");
  }
  ...
  virtual void visit(AstWhile *nodep) override {
    ...
    iterateAndNextNull(
        nodep->precondsp()); // Need to recompute before next loop
    puts("}\n");
  }
  ...
  virtual void visit(AstTime *nodep) override {
    ...
    if (nodep->timeunit().isNone())
      nodep->v3fatalSrc("$time has no units");
  ...
  virtual void visit(AstTimeD *nodep) override {
    ...
    if (nodep->timeunit().isNone())
      nodep->v3fatalSrc("$realtime has no units");
    puts(cvtToStr(nodep->timeunit().multiplier() /
                  v3Global.rootp()->timeprecision().multiplier()));
    puts(")");
  }
  ...
  virtual void visit(AstNodeUniop *nodep) override {
    if (nodep->emitCheckMaxWords() &&
      ...
      nodep->v3warn(E_UNSUPPORTED,
                    "Unsupported: " << nodep->prettyOperatorName()
                                    << " operator of " << nodep->width()
                                    << " bits exceeds hardcoded limit "
                                       "VL_MULS_MAX_WORDS in verilatedos.h");
    }
  ...
  virtual void visit(AstNodeTriop *nodep) override {
    UASSERT_OBJ(!emitSimpleOk(nodep), nodep,
                "Triop cannot be described in a simple way");
    emitOpName(nodep, nodep->emitC(), nodep->lhsp(), nodep->rhsp(),
               nodep->thsp());
  }
  ...
  virtual void visit(AstNodeCond *nodep) override {
    ...
    if (nodep->expr1p()->isWide()) {
      emitOpName(nodep, nodep->emitC(), nodep->condp(), nodep->expr1p(),
                 nodep->expr2p());
    } else {
      putbs("(");
      iterateAndNextNull(nodep->condp());
  ...
  virtual void visit(AstSel *nodep) override {
    ...
    emitOpName(nodep, nodep->emitC(), nodep->fromp(), nodep->lsbp(),
               nodep->thsp());
  }
  ...
  virtual void visit(AstStreamL *nodep) override {
    ...
    if (!nodep->isWide()) {
      ...
      if (isPow2 && sliceSize <= (nodep->isQuad() ? sizeof(uint64_t)
                                                  : sizeof(uint32_t))) {
        ...
        puts(", ");
        uint32_t rd_log2 =
            V3Number::log2b(VN_AS(nodep->rhsp(), Const)->toUInt());
  ...
  virtual void visit(AstCountBits *nodep) override {
    ...
    if (nodep->lhsp()->isWide()) {
      puts(cvtToStr(
          nodep->lhsp()->widthWords())); // Note argument width, not node width
                                         // (which is always 32)
      puts(", ");
    }
    iterateAndNextNull(nodep->lhsp());
  ...
  virtual void visit(AstAddrOfCFunc *nodep) override {
    // Note: Can be thought to handle more, but this is all that is needed right
    // now
    AstCFunc *const funcp = nodep->funcp();
    UASSERT_OBJ(funcp->isLoose(), nodep,
                "Cannot take address of non-loose method");
    puts("&");
    puts(funcNameProtect(funcp));
  }
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    if (!v3Global.opt.lintOnly()) { // An internal problem, so suppress
      nodep->v3fatalSrc(
          "Unknown node type reached emitter: " << nodep->prettyTypeName());
    }
    // LCOV_EXCL_STOP
  }

  EmitCFunc() : m_lazyDecls(*this) {
