...
public:
  ...
  explicit GateGraphBaseVisitor(V3Graph *graphp) : m_graphp{graphp} {}
...
public:
  ...
  // Returns only the result from the LAST vertex iterated over
  // Note: This behaves differently than iterateInEdges() in that it will
  // traverse
  //          all edges that exist when it is initially called, whereas
  //          iterateInEdges() will stop traversing edges if one is deleted
  VNUser iterateCurrentOutEdges(GateGraphBaseVisitor &v,
                                VNUser vu = VNUser(0)) {
    VNUser ret = VNUser(0);
...
class GateVarVertex final : public GateEitherVertex {
  ...
  AstNode *m_rstSyncNodep =
      nullptr; // Used as reset and not in SenItem, in clocked always
  AstNode *m_rstAsyncNodep =
      nullptr; // Used as reset and in SenItem, in clocked always
public:
  GateVarVertex(V3Graph *graphp, AstScope *scopep, AstVarScope *varScp)
      : GateEitherVertex{graphp, scopep}, m_varScp{varScp} {}
  ...
  virtual VNUser accept(GateGraphBaseVisitor &v,
                        VNUser vu = VNUser(0)) override {
...
class GateLogicVertex final : public GateEitherVertex {
  AstNode *m_nodep;
  AstActive
      *m_activep; // Under what active; nullptr is ok (under cfunc or such)
  bool m_slow;    // In slow block
public:
  GateLogicVertex(V3Graph *graphp, AstScope *scopep, AstNode *nodep,
                  AstActive *activep, bool slow)
      : GateEitherVertex{graphp, scopep}, m_nodep{nodep}, m_activep{activep},
        m_slow{slow} {}
...
private:
  ...
  // STATE
  bool m_buffersOnly; // Set when we only allow simple buffering, no equations
                      // (for clocks)
  AstNodeVarRef *m_lhsVarRef =
    ...
    if (m_buffersOnly &&
        !(VN_IS(nodep->rhsp(), VarRef)
          ...
          || (VN_IS(nodep->rhsp(), Not) &&
              VN_IS(VN_AS(nodep->rhsp(), Not)->lhsp(), VarRef) &&
              VN_AS(VN_AS(nodep->rhsp(), Not)->lhsp(), VarRef)
                  ->varp()
                  ->isUsedClock()))) {
...
private:
  ...
  // AstVarScope::user2       -> bool: Signal used in SenItem in *this* always
  // statement AstVar::user2            -> bool: Warned about SYNCASYNCNET
  ...
  GateLogicVertex *m_logicVertexp =
      nullptr; // Current statement being tracked, nullptr=ignored
  ...
  void iterateNewStmt(AstNode *nodep, const char *nonReducibleReason,
    ...
    if (m_scopep) {
      ...
      m_logicVertexp =
          new GateLogicVertex(&m_graph, m_scopep, nodep, m_activep, m_inSlow);
      ...
      if (consumeReason)
        m_logicVertexp->setConsumed(consumeReason);
      if (VN_IS(nodep, SenItem))
        m_logicVertexp->setConsumed("senItem");
      iterateChildren(nodep);
  ...
  GateVarVertex *makeVarVertex(AstVarScope *varscp) {
    GateVarVertex *vertexp =
        reinterpret_cast<GateVarVertex *>(varscp->user1p());
    if (!vertexp) {
      UINFO(6, "New vertex " << varscp << endl);
      vertexp = new GateVarVertex(&m_graph, m_scopep, varscp);
      varscp->user1p(vertexp);
      if (varscp->varp()->isSigPublic()) {
        // Public signals shouldn't be changed, pli code might be messing with
        // them
      ...
      if (varscp->varp()->isUsedClock())
        vertexp->setConsumed("clock");
    }
    return vertexp;
  }

  void optimizeSignals(bool allowMultiIn);
  bool elimLogicOkOutputs(GateLogicVertex *consumeVertexp,
                          const GateOkVisitor &okVisitor);
  void optimizeElimVar(AstVarScope *varscp, AstNode *substp,
                       AstNode *consumerp);
  void warnSignals();
  void consumedMark();
  ...
  virtual void visit(AstNetlist *nodep) override {
    ...
    if (debug() > 6)
      m_graph.dumpDotFilePrefixed("gate_pre");
    warnSignals(); // Before loss of sync/async pointers
    // Decompose clock vectors -- need to do this before removing redundant
    // edges
    decomposeClkVectors();
    ...
    m_graph.userClearVertices(); // vertex->user(): bool. Indicates we've set it
                                 // as consumed
    ...
    if (v3Global.opt.oDedupe()) {
      dedupe();
      if (debug() >= 6)
        m_graph.dumpDotFilePrefixed("gate_dedup");
    }
    if (v3Global.opt.oAssemble()) {
      mergeAssigns();
      if (debug() >= 6)
        m_graph.dumpDotFilePrefixed("gate_assm");
    }
  ...
  virtual void visit(AstActive *nodep) override {
    ...
    m_activeReducible =
        !(nodep->hasClocked()); // Seq logic outputs aren't reducible
    m_activep = nodep;
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    if (m_scopep) {
      ...
      if (m_inSenItem) {
        ...
        // For SYNCASYNCNET
        varscp->user2(true);
      } else if (m_activep && m_activep->hasClocked() &&
                 nodep->access().isReadOnly()) {
        if (varscp->user2()) {
          if (!vvertexp->rstAsyncNodep())
            vvertexp->rstAsyncNodep(nodep);
        } else {
          if (!vvertexp->rstSyncNodep())
            vvertexp->rstSyncNodep(nodep);
        }
      }
  ...
  virtual void visit(AstNodeProcedure *nodep) override {
    VL_RESTORER(m_inSlow);
    {
      ...
      iterateNewStmt(nodep,
                     (nodep->isJustOneBodyStmt() ? nullptr : "Multiple Stmts"),
                     nullptr);
    }
  }
  ...
  virtual void visit(AstConcat *nodep) override {
    UASSERT_OBJ(!(VN_IS(nodep->backp(), NodeAssign) &&
                ...
                nodep,
                "Concat on LHS of assignment; V3Const should have deleted it");
    iterateChildren(nodep);
  }
  ...
  virtual void visit(AstNode *nodep) override {
    iterateChildren(nodep);
    if (nodep->isOutputter() && m_logicVertexp)
      m_logicVertexp->setConsumed("outputter");
  }
...
void GateVisitor::optimizeSignals(bool allowMultiIn) {
  for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
    ...
    if (GateVarVertex *vvertexp = dynamic_cast<GateVarVertex *>(itp)) {
      if (vvertexp->inEmpty()) {
        vvertexp->clearReducibleAndDedupable(
            "inEmpty");        // Can't deal with no sources
        if (!vvertexp->isTop() // Ok if top inputs are driverless
          ...
          if (false) {
            // If we warned here after constant propagation, what the user
            // considered reasonable logic may have disappeared.  Issuing a
            // warning would thus be confusing.  V3Undriven now handles this.
      ...
      if (!vvertexp->reducible()) {
        ...
        if (logicVertexp->reducible()) {
          ...
          if (doit && multiInputs) {
            if (!allowMultiIn)
              doit = false;
            // Doit if one input, or not used, or used only once, ignoring
            // traces
            int n = 0;
            for (V3GraphEdge *edgep = vvertexp->outBeginp(); edgep;
              ...
              if (!consumeVertexp
                       ->slow()) { // Not tracing or other slow path junk
          ...
          if (!doit) {
            if (allowMultiIn && (debug() >= 9)) {
              UINFO(9, "Not ok simp" << okVisitor.isSimple() << " mi"
                                     << multiInputs << " ob"
                                     << vvertexp->outBeginp() << " on"
                                     << (vvertexp->outBeginp()
            ...
            if (debug() >= 5)
              logicp->dumpTree(cout, "    elimVar:  ");
            if (debug() >= 5)
              substp->dumpTree(cout, "      subst:  ");
            ++m_statSigs;
            ...
            for (V3GraphEdge *edgep = vvertexp->outBeginp(); edgep;) {
              ...
              if (!elimLogicOkOutputs(consumeVertexp, okVisitor /*ref*/)) {
                ...
                for (GateVarRefList::const_iterator it = rhsVarRefs.begin();
                  ...
                  UINFO(9,
                        "         Point-to-new vertex " << newvarscp << endl);
...
bool GateVisitor::elimLogicOkOutputs(GateLogicVertex *consumeVertexp,
  ...
  // Replacement logic usually has shorter input list, so faster to build list
  // based on it
  const GateVarRefList &rhsVarRefs = okVisitor.rhsVarRefs();
  for (GateVarRefList::const_iterator it = rhsVarRefs.begin();
       it != rhsVarRefs.end(); ++it) {
  ...
  for (V3GraphEdge *edgep = consumeVertexp->outBeginp(); edgep;
       edgep = edgep->outNextp()) {
    ...
    if (varscopes.find(vscp) != varscopes.end()) {
      UINFO(9,
            "    Block-unopt, insertion generates input vscp " << vscp << endl);
      return false;
    }
  }
  return true;
...
void GateVisitor::replaceAssigns() {
  for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
       itp = itp->verticesNextp()) {
    if (GateVarVertex *vvertexp = dynamic_cast<GateVarVertex *>(itp)) {
      // Take the Comments/assigns that were moved to the VarScope and change
      // them to a simple value assignment
...
void GateVisitor::consumedMarkRecurse(GateEitherVertex *vertexp) {
  ...
  vertexp->user(true);
  if (!vertexp->consumed())
    vertexp->setConsumed("propagated");
  // Walk sources and mark them too
  for (V3GraphEdge *edgep = vertexp->inBeginp(); edgep;
       edgep = edgep->inNextp()) {
    GateEitherVertex *eFromVertexp =
        static_cast<GateEitherVertex *>(edgep->fromp());
    consumedMarkRecurse(eFromVertexp);
  }
}

void GateVisitor::consumedMove() {
  // Remove unused logic (logic that doesn't hit a combo block or a display
  // statement) We need the "usually" block logic to do a better job at this
  for (V3GraphVertex *vertexp = m_graph.verticesBeginp(); vertexp;
       vertexp = vertexp->verticesNextp()) {
  ...
  for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
    ...
    if (GateVarVertex *vvertexp = dynamic_cast<GateVarVertex *>(itp)) {
      ...
      if (ap && sp && !vscp->varp()->user2()) {
        ...
        // then report a warning in a new place - we should report them all at
        // once. Instead we'll disable if any disabled
...
private:
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    if (nodep->varScopep() == m_elimVarScp) {
      ...
      // It's possible we substitute into something that will be reduced more
      // later, however, as we never delete the top Always/initial statement,
      // all should be well.
      m_didReplace = true;
      ...
      // If replacing with logic, an error/warning is likely to want to point to
      // the logic IE what we're replacing with. However a VARREF should point
      // to the original as it's otherwise confusing to throw warnings that
      // point to a PIN rather than where the pin us used.
      if (VN_IS(substp, VarRef))
        substp->fileline(nodep->fileline());
      // Make the substp an rvalue like nodep. This facilitates the hashing in
      // dedupe.
...
void GateVisitor::optimizeElimVar(AstVarScope *varscp, AstNode *substp,
                                  AstNode *consumerp) {
  if (debug() >= 5)
    consumerp->dumpTree(cout, "    elimUsePre: ");
  GateElimVisitor elimVisitor{consumerp, varscp, substp, nullptr};
  if (elimVisitor.didReplace()) {
    if (debug() >= 9)
      consumerp->dumpTree(cout, "    elimUseCns: ");
    ...
    if (debug() >= 5)
      consumerp->dumpTree(cout, "    elimUseDne: ");
...
private:
  ...
  // Ast*::user3p     -> AstActive* of assign, for isSame() in test for
  // duplicate
  //                     Set to nullptr if this assign's tree was later replaced
  // Ast*::user5p     -> AstNode* of assign if condition, for isSame() in test
  // for duplicate
  //                     Set to nullptr if this assign's tree was later replaced
  // AstUser1InUse    m_inuser1;      (Allocated for use in GateVisitor)
  // AstUser2InUse    m_inuser2;      (Allocated for use in GateVisitor)
  AstUser3InUse m_inuser3;
  // AstUser4InUse    m_inuser4;      (Allocated for use in V3Hasher via
  // V3DupFinder)
  AstUser5InUse m_inuser5;

  V3DupFinder m_dupFinder; // Duplicate finder for rhs of assigns
  std::unordered_set<AstNode *>
      m_nodeDeleteds; // Any node in this hash was deleted

  VL_DEBUG_FUNC; // Declare debug()

  bool same(AstNode *node1p, AstNode *node2p) {
    // Regarding the complexity of this funcition 'same':
    // Applying this comparison function to a a set of n trees pairwise is
    // O(n^2) in the number of comparisons (number of pairs). AstNode::sameTree
    // itself, is O(sizeOfTree) in the worst case, which happens if the operands
    // of sameTree are indeed identical copies, which means this line is
    // O(n^2*sizeOfTree), iff you are comparing identical copies of the same
    // tree. In practice the identity comparison over the pointers, and the
    // short circuiting in sameTree means that for comparing the same tree
    // instance to itself, or trees of different types/shapes is a lot closer to
    // O(1), so this 'same' function is Omega(n^2) and O(n^2*sizeOfTree), and in
    // practice as we are mostly comparing the same instance to itself or
    // different trees, the complexity should be closer to the lower bound.
    //
    // Also if you see where this 'same' function is used within isSame, it's
    // only ever comparing AstActive nodes, which are very likely not to compare
    // equals (and for the purposes of V3Gate, we probably only care about them
    // either being identical instances, or having the same sensitivities
    // anyway, so if this becomes a problem, it can be improved which should
    // also speed things up), and AstNodeMath for if conditions, which are
    // hopefully small, and to be safe they should probably be only considered
    // same when identical instances (otherwise if writing the condition between
    // 2 ifs don't really merge).
...
public:
  ...
  // About to replace a node; any node we're tracking refers to oldp, change it
  // to newp. This might be a variable on the lhs of the duplicate tree, or
  // could be a rhs variable in a tree we're not replacing (or not yet anyways)
  void hashReplace(AstNode *oldp, AstNode *newp) {
  ...
  bool isReplaced(AstNode *nodep) {
    // Assignment may have been hashReplaced, if so consider non-match
    // (effectively removed)
    UASSERT_OBJ(!VN_IS(nodep, NodeAssign), nodep,
                "Dedup attempt on non-assign");
  ...
  virtual bool isSame(AstNode *node1p, AstNode *node2p) override {
    ...
    if (isReplaced(node1p) || isReplaced(node2p)) {
      // UINFO(9, "isSame hit on replaced "<<(void*)node1p<<"
      // "<<(void*)node2p<<endl);
      return false;
    }
  ...
  AstNodeAssign *hashAndFindDupe(AstNodeAssign *assignp, AstNode *extra1p,
                                 AstNode *extra2p) {
    // Legal for extra1p/2p to be nullptr, we'll compare with other assigns with
    // extras also nullptr
    ...
    // return anything in the hash that has the same pointer
    // (V3DupFinder::findDuplicate) So dupit is either a different, duplicate
    // rhsp, or the end of the hash.
  ...
  void check() {
    for (const auto &itr : m_dupFinder) {
      ...
      if (!isReplaced(nodep)) {
        ...
        UASSERT_OBJ(!activep ||
                        (!VN_DELETED(activep) && VN_IS(activep, Active)),
                    nodep, "V3DupFinder check failed, lost active pointer");
...
class GateDedupeVarVisitor final : public GateBaseVisitor {
  ...
  //    and the assign must be the only node under the if, other than the
  //    ifcond)
  // Any other ordering or node type, except for an AstComment, makes it not
  // dedupable
private:
  // STATE
...
public:
  ...
  // PUBLIC METHODS
  AstNodeVarRef *findDupe(AstNode *nodep, AstVarScope *consumerVarScopep,
                          AstActive *activep) {
    m_assignp = nullptr;
    m_ifCondp = nullptr;
    m_always = false;
...
//######################################################################
// Recurse through the graph, looking for duplicate expressions on the rhs of an
// assign
...
private:
  ...
  virtual VNUser visit(GateVarVertex *vvertexp, VNUser) override {
    ...
    if (m_depth > GATE_DEDUP_MAX_DEPTH)
      return VNUser(
          0); // Break loops; before user2 set so hit this vertex later
    if (vvertexp->varScp()->user2())
      return VNUser(0);
    ...
    if (vvertexp->inSize1()) {
      ...
      if (dupVarRefp) { // visit(GateLogicVertex*...) returned match
        ...
        GateLogicVertex *lvertexp =
            static_cast<GateLogicVertex *>(edgep->fromp());
        ...
        if (okVisitor.isSimple()) {
          ...
          for (V3GraphEdge *outedgep = vvertexp->outBeginp(); outedgep;) {
            ...
            UINFO(9, "elim src vtx" << lvertexp << " node " << lvertexp->nodep()
                                    << endl);
            UINFO(9, "elim cons vtx" << consumeVertexp << " node " << consumerp
                                     << endl);
            UINFO(9, "elim var vtx " << vvertexp << " node "
                                     << vvertexp->varScp() << endl);
            UINFO(9, "replace with " << dupVarRefp << endl);
            if (lvertexp == consumeVertexp) {
              UINFO(9, "skipping as self-recirculates\n");
            } else {
              GateElimVisitor elimVisitor(consumerp, vvertexp->varScp(),
          ...
          // replaceAssigns() does the deleteTree on lvertexNodep in a later
          // step
  ...
  virtual VNUser visit(GateLogicVertex *lvertexp, VNUser vu) override {
    ...
    GateVarVertex *consumerVvertexpp =
...
private:
  ...
  AstSel *merge(AstSel *pre, AstSel *cur) {
    ...
    if (!pstart || !pwidth || !cstart || !cwidth)
      return nullptr; // too complicated
        ...
        if (AstNodeAssign *assignp = VN_CAST(lvertexp->nodep(), NodeAssign)) {
          ...
          if (VN_IS(assignp->lhsp(), Sel) && lvertexp->outSize1()) {
            UINFO(9, "assing to the nodep["
                         << VN_AS(assignp->lhsp(), Sel)->lsbConst() << "]"
                         << endl);
            ...
            if (AstSel *newselp = merge(preselp, curselp)) {
              ...
              m_assignp->dtypeChgWidthSigned(
                  m_assignp->width() + assignp->width(),
                  m_assignp->width() + assignp->width(), VSigning::SIGNED);
...
private:
  ...
  // VISITORS
  virtual void visit(AstNodeVarRef *nodep) override {
    UINFO(9, "CLK DECOMP Concat search var (off = " << m_offset << ") - "
                                                    << nodep << endl);
    if (nodep->varScopep() == m_vscp && !nodep->user2() && !m_found) {
      ...
      m_found = true;
      UINFO(9, "CLK DECOMP Concat found var (off = " << m_offset << ") - "
                                                     << nodep << endl);
    }
    m_offset += nodep->dtypep()->width();
  }
  virtual void visit(AstConcat *nodep) override {
    UINFO(9, "CLK DECOMP Concat search (off = " << m_offset << ") - " << nodep
                                                << endl);
    iterate(nodep->rhsp());
    iterate(nodep->lhsp());
  }
...
public:
  ...
  bool concatOffset(AstConcat *concatp, AstVarScope *vscp, int &offsetr) {
    ...
    // Iterate
    iterate(concatp);
    UINFO(9, "CLK DECOMP Concat Offset (found = "
                 << m_found << ") (" << m_found_offset << ") - " << concatp
                 << " : " << vscp << endl);
...
public:
  ...
  GateClkDecompState(int offset, AstVarScope *vsp)
      : m_offset{offset}, m_last_vsp{vsp} {}
...
private:
  ...
  virtual VNUser visit(GateVarVertex *vvertexp, VNUser vu) override {
    ...
    GateClkDecompState *currState =
        reinterpret_cast<GateClkDecompState *>(vu.c());
    ...
    if (const AstAssignW *assignp = VN_CAST(lvertexp->nodep(), AssignW)) {
      ...
      if (AstSel *rselp = VN_CAST(assignp->rhsp(), Sel)) {
        if (VN_IS(rselp->lsbp(), Const) && VN_IS(rselp->widthp(), Const)) {
          if (clk_offset < rselp->lsbConst() ||
              clk_offset > rselp->msbConst()) {
            UINFO(9, "CLK DECOMP Sel [ "
                         << rselp->msbConst() << " : " << rselp->lsbConst()
                         << " ] dropped clock (" << clk_offset << ")" << endl);
            return VNUser(0);
          }
      ...
      if (const AstSel *lselp = VN_CAST(assignp->lhsp(), Sel)) {
        ...
        if (vrp->dtypep()->width() == 1 && m_seen_clk_vectors) {
          ...
          AstNode *rhsp = assignp->rhsp();
          rhsp->replaceWith(
              new AstVarRef(rhsp->fileline(), m_clk_vsp, VAccess::READ));
...
public:
  ...
  virtual ~GateClkDecompGraphVisitor() override {
    V3Stats::addStat("Optimizations, Clocker seen vectors",
                     m_total_seen_clk_vectors);
...
void V3Gate::gateAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("gate", 0,
