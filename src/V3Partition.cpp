...
public:
  ...
  PartPropagateCp(V3Graph *graphp, GraphWay way, T_CostAccessor *accessp,
      ...
      : GraphAlg<>{graphp, edgeFuncp}, m_way{way}, m_accessp{accessp},
  ...
  void cpHasIncreased(V3GraphVertex *vxp, uint32_t newInclusiveCp) {
    ...
    for (V3GraphEdge *edgep = vxp->beginp(m_way); edgep;
         edgep = edgep->nextp(m_way)) {
      if (!m_edgeFuncp(edgep))
        continue;
...
private:
  void checkNewCpVersusEdges(V3GraphVertex *vxp, GraphWay way,
                             uint32_t cp) const {
  ...
  void partInitCriticalPaths(bool checkOnly) {
    ...
    while (const V3GraphVertex *cvxp = order.nextp()) {
      ...
      uint32_t cpCost = 0;
      for (V3GraphEdge *edgep = vxp->inBeginp(); edgep;
           edgep = edgep->inNextp()) {
  ...
  void go() {
    ...
    PartPropagateCp<PartPropagateCpSelfTest> prop(&m_graph, GraphWay::FORWARD,
                                                  this, true);
    ...
    for (const auto &i : m_vx) {
      if (!i->inBeginp())
        prop.cpHasIncreased(i, 1 /* inclusive CP starts at 1 */);
    }
...
public:
  ...
  public:
    ...
    void setCritPathCost(V3GraphVertex *vxp, GraphWay way,
                         uint32_t cost) const {
...
public:
  ...
  LogicMTask(V3Graph *graphp, MTaskMoveVertex *mtmvVxp)
    ...
    for (unsigned int &i : m_critPathCost)
      i = 0;
  ...
  static uint32_t stepCost(uint32_t cost) {
    ...
    UASSERT_STATIC(stepCost <= ((cost * 11 / 10)),
                   "stepped cost error exceeded");
    return stepCost;
#else
    return cost;
#endif
  ...
  void addRelative(GraphWay way, LogicMTask *relativep) {
    ...
    edges.set(relativep,
              relativep->stepCost() + relativep->critPathCost(way.invert()));
  }
  ...
  void checkRelativesCp(GraphWay way) const {
    ...
    for (EdgeSet::const_reverse_iterator it = edges.rbegin();
      ...
      partCheckCachedScoreVsActual(cachedCp,
  ...
  virtual string name() const override {
    ...
    out << "mt" << m_serialId << "." << this << " [b"
        << m_critPathCost[GraphWay::FORWARD] << " a"
        << m_critPathCost[GraphWay::REVERSE] << " c" << cost();
    return out.str();
  }

  void setCritPathCost(GraphWay way, uint32_t cost) {
    m_critPathCost[way] = cost;
  }
...
private:
  static bool pathExistsFromInternal(LogicMTask *fromp, LogicMTask *top,
    ...
    for (const V3GraphEdge *followp = fromp->outBeginp(); followp;
      ...
      if (followp == excludedEdgep)
        continue;
      LogicMTask *const nextp = dynamic_cast<LogicMTask *>(followp->top());
      if (pathExistsFromInternal(nextp, top, nullptr, generation))
        return true;
    }
    return false;
  }
...
public:
  ...
  static void dumpCpFilePrefixed(const V3Graph *graphp,
    ...
    if (osp->fail())
      v3fatalStatic("Can't write " << filename);
    ...
    *osp << "totalCost = " << totalCost
         << " (should match the computed critical path cost (CP) for the "
            "graph)\n";
...
private:
  ...
  // By using the bottom bits for flags, we can still use < to compare IDs
  // without masking.
  vluint64_t m_id; // <63:2> Serial number for ordering, <1> subtype
                   // (SiblingMC), <0> removed
  ...
  explicit MergeCandidate(bool isSiblingMC) {
    ...
    serial +=
        ID_INCREMENT; // +ID_INCREMENT so doesn't set the special bottom bits
...
static_assert(sizeof(MergeCandidate) == sizeof(vluint64_t),
...
static_assert(sizeof(SiblingMC) ==
                  sizeof(MergeCandidate) + 2 * sizeof(LogicMTask *),
              "Should not have a vtable");
...
public:
  ...
  MTaskEdge(V3Graph *graphp, LogicMTask *fromp, LogicMTask *top, int weight)
      : V3GraphEdge{graphp, fromp, top, weight}, MergeCandidate{
                                                     /* isSiblingMC: */ false} {
...
// Normally this would be a virtual function, but we save space by not having a
// vtable, and we know we only have 2 possible subclasses.
bool MergeCandidate::mergeWouldCreateCycle() const {
  return isSiblingMC()
             ? static_cast<const SiblingMC *>(this)->mergeWouldCreateCycle()
...
public:
  // CONSTRUCTORS
  explicit PartParallelismEst(const V3Graph *graphp) : m_graphp{graphp} {}
...
// vertex.
static void partInitHalfCriticalPaths(GraphWay way, V3Graph *mtasksp,
                                      bool checkOnly) {
...
#endif
    for (V3GraphEdge *edgep = vertexp->beginp(rev); edgep;
         edgep = edgep->nextp(rev)) {
#if VL_DEBUG
      ...
      UASSERT_OBJ(relatives.find(edgep->furtherp(rev)) == relatives.end(),
                  mtaskp, "Should be no redundant edges in mtasks graph");
      relatives.insert(edgep->furtherp(rev));
#endif
      LogicMTask *const relativep =
...
static void partInitCriticalPaths(V3Graph *mtasksp) {
  ...
  for (V3GraphVertex *vxp = mtasksp->verticesBeginp(); vxp;
       vxp = vxp->verticesNextp()) {
    for (V3GraphEdge *edgep = vxp->outBeginp(); edgep;
    ...
    for (V3GraphEdge *edgep = donorp->beginp(way); edgep;
      ...
      if (recipientp->hasRelative(way, tedgep->furtherMTaskp(way))) {
        ...
        if (way == GraphWay::REVERSE) {
          newEdgep =
              new MTaskEdge(mtasksp, tedgep->fromMTaskp(), recipientp, 1);
        } else {
          newEdgep = new MTaskEdge(mtasksp, recipientp, tedgep->toMTaskp(), 1);
        }
        if (sbp)
          sbp->addElem(newEdgep);
      }
    }
  }
}
...
public:
  ...
  PartContraction(V3Graph *mtasksp, uint32_t scoreLimit, bool slowAsserts)
      : m_mtasksp{mtasksp}, m_scoreLimit{scoreLimit},
        m_slowAsserts{slowAsserts}, m_sb{&mergeCandidateScore, slowAsserts} {}
  ...
  void go() {
    ...
    if (maxMTasks == 0) { // Unspecified so estimate
      if (v3Global.opt.threads() > 1) {
        maxMTasks =
            (PART_DEFAULT_MAX_MTASKS_PER_THREAD * v3Global.opt.threads());
      } else {
    ...
    while (true) {
      ...
      MergeCandidate *const mergeCanp =
          const_cast<MergeCandidate *>(m_sb.bestp());
      if (!mergeCanp) {
      ...
      if (actualScore > m_scoreLimit) {
        ...
        if (m_sb.needsRescore()) {
          ...
          if (mtaskCount > maxMTasks) {
            ...
            UINFO(1, "Critical path limit was=" << oldLimit << " now="
                                                << m_scoreLimit << endl);
            continue;
          }
          // Really stop
...
private:
  NewCp newCp(GraphWay way, LogicMTask *mtaskp, LogicMTask *otherp,
    ...
    const uint32_t origRelativesCp =
  ...
  void removeSiblingMCsWith(LogicMTask *mtaskp) {
    for (SibpSet::iterator it = m_mtask2sibs[mtaskp].begin();
      ...
      LogicMTask *const otherp =
          (pairp->bp() == mtaskp) ? pairp->ap() : pairp->bp();
  ...
  void contract(MergeCandidate *mergeCanp) {
    ...
    UINFO(9,
          "recipient = " << recipientp->id() << ", donor = " << donorp->id()
    ...
    PartPropagateCp<LogicMTask::CpCostAccessor> forwardPropagator(
        m_mtasksp, GraphWay::FORWARD, &cpAccess, m_slowAsserts);
    PartPropagateCp<LogicMTask::CpCostAccessor> reversePropagator(
    ...
    // updates.
    if (m_slowAsserts)
      partCheckCriticalPaths(m_mtasksp);
    ...
    unsigned edges = 0;
    for (V3GraphEdge *edgep = recipientp->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
      LogicMTask *const postreqp = dynamic_cast<LogicMTask *>(edgep->top());
      siblingPairFromRelatives(GraphWay::REVERSE, postreqp, false);
      ++edges;
      if (edges > PART_SIBLING_EDGE_LIMIT)
        break;
    }
    edges = 0;
    for (V3GraphEdge *edgep = recipientp->inBeginp(); edgep;
         edgep = edgep->inNextp()) {
  ...
  static uint32_t mergeCandidateScore(const MergeCandidate *pairp) {
    ...
    v3fatalSrc("Failed to cast pairp to either MTaskEdge or SiblingMC in "
               "mergeCandidateScore");
    return 0;
  }
  ...
  static uint32_t siblingScore(const SiblingMC *sibsp) {
    ...
    const uint32_t mergedCpCostFwd =
        std::max(ap->critPathCost(GraphWay::FORWARD),
                 bp->critPathCost(GraphWay::FORWARD));
    const uint32_t mergedCpCostRev =
        std::max(ap->critPathCost(GraphWay::REVERSE),
  ...
  static int shortestWaywardCpInclusive(const void *vap, const void *vbp) {
    ...
    const LogicMTask *const ap =
        *reinterpret_cast<const LogicMTask *const *>(vap);
    const LogicMTask *const bp =
        *reinterpret_cast<const LogicMTask *const *>(vbp);
  ...
  void siblingPairFromRelatives(GraphWay way, V3GraphVertex *mtaskp,
    ...
    for (V3GraphEdge *edgep = mtaskp->beginp(way); edgep;
         edgep = edgep->nextp(way)) {
      LogicMTask *const prereqp =
          dynamic_cast<LogicMTask *>(edgep->furtherp(way));
...
private:
  ...
  using Olv2MTaskMap =
      std::unordered_map<const OrderLogicVertex *, LogicMTask *>;
...
public:
  // CONSTRUCTORs
  explicit PartFixDataHazards(V3Graph *mtasksp) : m_mtasksp{mtasksp} {}
  // METHODS
private:
  void findAdjacentTasks(OvvSet::iterator ovvIt, TasksByRank *tasksByRankp) {
    ...
    for (V3GraphEdge *edgep = (*ovvIt)->inBeginp(); edgep;
         edgep = edgep->inNextp()) {
      OrderLogicVertex *const logicp =
          dynamic_cast<OrderLogicVertex *>(edgep->fromp());
      if (!logicp)
        continue;
      if (logicp->domainp()->hasInitial() || logicp->domainp()->hasSettle())
        continue;
    ...
    for (V3GraphEdge *edgep = (*ovvIt)->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
      OrderLogicVertex *const logicp =
          dynamic_cast<OrderLogicVertex *>(edgep->fromp());
      if (!logicp)
...
public:
  void go() {
    ...
    for (OvvSet::iterator ovvit = ovvSet.begin(); ovvit != ovvSet.end();
         ++ovvit) {
    ...
    UINFO(4, "PartFixDataHazards() merged "
                 << m_mergesDone << " pairs of nodes in "
                 << (V3Os::timeUsecs() - startUsecs) << " usecs.\n");
  }
...
// The thread schedule, containing all information needed later. Note that this
// is a simple aggregate data type and the only way to get hold of an instance
// of it is via PartPackMTasks::pack, which is moved from there and is const,
// which means we can only acquire a const reference to is so no further
// modifications are allowed, so all members are public (attributes).
...
public:
  ...
  // MEMBERS
  // Allocation of sequence of MTasks to threads. Can be considered a map from
  // thread ID to the sequence of MTasks to be executed by that thread.
...
private:
  ...
  explicit ThreadSchedule(uint32_t nThreads) : threads{nThreads} {}
...
public:
  // Returns the number of cross-thread dependencies of the given MTask. If > 0,
  // the MTask must test whether its dependencies are ready before starting, and
  // therefore may need to block.
  uint32_t crossThreadDependencies(const ExecMTask *mtaskp) const {
    ...
    for (V3GraphEdge *edgep = mtaskp->inBeginp(); edgep;
      ...
      if (threadId(prevp) != thisThreadId)
        ++result;
    }
    return result;
  }
...
void ThreadSchedule::dumpDotFilePrefixedAlways(
...
void ThreadSchedule::dumpDotFile(const string &filename) const {
  ...
  *logp << "  graph[layout=\"neato\" labelloc=t labeljust=l label=\""
        << filename << "\"]\n";
  ...
  *logp << "\n  // Threads\n";
  ...
  for (int i = 0; i < v3Global.opt.threads(); i++) {
    *logp << "  t" << i << " [label=\"Thread " << i
          << "\" width=" << threadBoxWidth << " pos=\"" << (-threadBoxWidth / 2)
          << "," << -i << "!\" style=\"filled\" fillcolor=\"grey\"] \n";
  }
  ...
  *logp << "\n  // MTasks\n";
  ...
  for (const V3GraphVertex *vxp = depGraph->verticesBeginp(); vxp;
       vxp = vxp->verticesNextp()) {
  ...
  auto emitMTask = [&](const ExecMTask *mtaskp) {
    ...
    const double nodeWidth =
        minWidth * (static_cast<double>(mtaskp->cost()) / minCost);
    const double x = mtaskXPos(mtaskp, nodeWidth);
    const int y = -thread;
    string label = "label=\"" + mtaskp->name() + " (" +
                   cvtToStr(startTime(mtaskp)) + ":" +
                   std::to_string(endTime(mtaskp)) + ")" + "\"";
    *logp << "  " << mtaskp->name() << " [" << label << " width=" << nodeWidth
          << " pos=\"" << x << "," << y << "!\"]\n";
  };
  ...
  *logp << "\n  // MTask dependencies\n";
  for (const V3GraphVertex *vxp = depGraph->verticesBeginp(); vxp;
       vxp = vxp->verticesNextp()) {
    if (const ExecMTask *const mtaskp = dynamic_cast<const ExecMTask *>(vxp)) {
      for (V3GraphEdge *edgep = mtaskp->outBeginp(); edgep;
           edgep = edgep->outNextp()) {
...
public:
  ...
  explicit PartPackMTasks(uint32_t nThreads = v3Global.opt.threads(),
      ...
      : m_nThreads{nThreads}, m_sandbagNumerator{sandbagNumerator},
        m_sandbagDenom{sandbagDenom} {}
  ~PartPackMTasks() = default;

private:
  // METHODS
  uint32_t completionTime(const ThreadSchedule &schedule,
                          const ExecMTask *mtaskp, uint32_t threadId) {
    const ThreadSchedule::MTaskState &state = schedule.mtaskState.at(mtaskp);
    UASSERT(state.threadId != ThreadSchedule::UNASSIGNED,
    ...
    UINFO(6, "Sandbagged end time for " << mtaskp->name() << " on th "
                                        << threadId << " = "
  ...
  bool isReady(ThreadSchedule &schedule, const ExecMTask *mtaskp) {
    for (V3GraphEdge *edgeInp = mtaskp->inBeginp(); edgeInp;
         edgeInp = edgeInp->inNextp()) {
      const ExecMTask *const prevp =
          dynamic_cast<ExecMTask *>(edgeInp->fromp());
...
public:
  // Pack an MTasks from given graph into m_nThreads threads, return the
  // schedule.
  const ThreadSchedule pack(const V3Graph &mtaskGraph) {
    ...
    // MTasks ready to be assigned next. All their dependencies are already
    // assigned.
    ...
    for (V3GraphVertex *vxp = mtaskGraph.verticesBeginp(); vxp;
      ...
      if (isReady(schedule, mtaskp))
        readyMTasks.insert(mtaskp);
    }
    ...
    while (!readyMTasks.empty()) {
      ...
      for (uint32_t threadId = 0; threadId < m_nThreads; ++threadId) {
        for (ExecMTask *const mtaskp : readyMTasks) {
          ...
          for (V3GraphEdge *edgep = mtaskp->inBeginp(); edgep;
               edgep = edgep->inNextp()) {
            const ExecMTask *const priorp =
                dynamic_cast<ExecMTask *>(edgep->fromp());
            const uint32_t priorEndTime =
                completionTime(schedule, priorp, threadId);
      ...
      UINFO(6, "Will schedule " << bestMtaskp->name() << " onto thread "
                                << bestThreadId << endl);

      // Reference to thread in schedule we are assigning this MTask to.
      std::vector<const ExecMTask *> &bestThread =
          schedule.threads[bestThreadId];
      ...
      if (!bestThread.empty())
        schedule.mtaskState[bestThread.back()].nextp = bestMtaskp;
      ...
      for (V3GraphEdge *edgeOutp = bestMtaskp->outBeginp(); edgeOutp;
        ...
        // Dependent MTask should not be ready yet, since dependency is just
        // being assigned
    ...
    if (debug() >= 4)
      schedule.dumpDotFilePrefixedAlways("schedule");
...
void V3Partition::debugMTaskGraphStats(const V3Graph *graphp,
                                       const string &stage) {
  if (!debug())
    return;
  ...
  if (mtaskCount < 1000) {
    ...
    filePrefix += stage;
    if (debug() >= 4)
      graphp->dumpDotFilePrefixedAlways(filePrefix);
  }
...
void V3Partition::hashGraphDebug(const V3Graph *graphp, const char *debugName) {
  ...
  if (!v3Global.opt.debugNondeterminism())
    return;
  ...
  for (const V3GraphVertex *vxp = graphp->verticesBeginp(); vxp;
       vxp = vxp->verticesNextp()) {
    for (const V3GraphEdge *edgep = vxp->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
      const V3GraphVertex *const top = edgep->top();
...
void V3Partition::setupMTaskDeps(V3Graph *mtasksp,
  ...
  for (V3GraphVertex *itp = mtasksp->verticesBeginp(); itp;
    ...
    for (LogicMTask::VxList::const_iterator vit = vertexListp->begin();
      ...
      for (V3GraphEdge *outp = (*vit)->outBeginp(); outp;
        ...
        const MTaskMoveVertex *const top =
...
void V3Partition::go(V3Graph *mtasksp) {
  ...
  uint32_t totalGraphCost = 0;
  {
    ...
    for (V3GraphVertex *vxp = m_fineDepsGraphp->verticesBeginp(); vxp;
      ...
      UASSERT_OBJ(mtmvVxp, vxp,
                  "Every vertex here should be an MTaskMoveVertex");
...
void add(std::unordered_map<int, vluint64_t> &cmap, int id, vluint64_t cost) {
  cmap[id] += cost;
}

using EstimateAndProfiled =
    std::pair<uint64_t, vluint64_t>; // cost est, cost profiled
...
static void normalizeCosts(Costs &costs) {
  ...
  if (sumCostEstimate) {
    ...
    const double estToProfile = static_cast<double>(sumCostProfiled) /
                                static_cast<double>(sumCostEstimate);
  ...
  for (auto &est : costs) {
    ...
    if (maxCost < costEstimate)
      maxCost = costEstimate;
    if (maxCost < costProfiled)
      maxCost = costProfiled;
    UINFO(9, "Post uint scale: ce = " << est.second.first
                                      << " cp=" << est.second.second << endl);
  }
  vluint64_t scaleDownTo =
      10000000; // Extra room for future algorithms to add costs
  if (maxCost > scaleDownTo) {
    const double scaleup =
        static_cast<double>(scaleDownTo) / static_cast<double>(maxCost);
    UINFO(5, "Scaling data to within 32-bits by multiply by="
                 << scaleup << ", maxCost=" << maxCost << endl);
    for (auto &est : costs) {
...
static void fillinCosts(V3Graph *execMTaskGraphp) {
  ...
  for (const V3GraphVertex *vxp = execMTaskGraphp->verticesBeginp(); vxp;
    ...
    // This estimate is 64 bits, but the final mtask graph algorithm needs 32
    // bits
  ...
  for (const V3GraphVertex *vxp = execMTaskGraphp->verticesBeginp(); vxp;
    ...
    UASSERT(costEstimate <= (1UL << 31),
            "cost scaling math would overflow uint32");
    UASSERT(costProfiled <= (1UL << 31),
            "cost scaling math would overflow uint32");
  ...
  if (missingProfiles) {
    if (FileLine *const fl = V3Config::getProfileDataFileLine()) {
      fl->v3warn(PROFOUTOFDATE, "Profile data for mtasks may be out of date. "
                                    << missingProfiles << " of "
                                    << totalEstimates << " mtasks had no data");
    }
  }
}
...
static void finalizeCosts(V3Graph *execMTaskGraphp) {
  ...
  while (const V3GraphVertex *vxp = ser.nextp()) {
    ...
    // pole"
    for (V3GraphEdge *edgep = mtp->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
  ...
  for (V3GraphVertex *vxp = execMTaskGraphp->verticesBeginp(); vxp;) {
    ...
    if (!bodyp->stmtsp()) { // Kill this empty mtask
      ...
      for (V3GraphEdge *inp = mtp->inBeginp(); inp; inp = inp->inNextp()) {
        for (V3GraphEdge *outp = mtp->outBeginp(); outp;
             outp = outp->outNextp()) {
...
static void addMTaskToFunction(const ThreadSchedule &schedule,
                               const uint32_t threadId, AstCFunc *funcp,
  ...
  if (const uint32_t nDependencies = schedule.crossThreadDependencies(mtaskp)) {
    // This mtask has dependencies executed on another thread, so it may block.
    // Create the task state variable and wait to be notified.
    const string name = "__Vm_mtaskstate_" + cvtToStr(mtaskp->id());
    AstBasicDType *const mtaskStateDtypep =
        v3Global.rootp()->typeTablep()->findBasicDType(
            fl, AstBasicDTypeKwd::MTASKSTATE);
    AstVar *const varp =
        new AstVar(fl, AstVarType::MODULETEMP, name, mtaskStateDtypep);
    varp->valuep(new AstConst(fl, nDependencies));
    varp->protect(
        false); // Do not protect as we still have references in AstText
    modp->addStmtp(varp);
  ...
  if (v3Global.opt.profThreads()) {
    ...
    addStrStmt("vlSymsp->_vm_profiler.startCounter(" +
               cvtToStr(mtaskp->profilerId()) + ");\n");
  }
  ...
  if (v3Global.opt.profThreads()) {
    ...
    addStrStmt("vlSymsp->_vm_profiler.stopCounter(" +
               cvtToStr(mtaskp->profilerId()) + ");\n");
  }
  if (v3Global.opt.profThreads()) {
  ...
  // For any dependent mtask that's on another thread, signal one dependency
  // completion.
  for (V3GraphEdge *edgep = mtaskp->outBeginp(); edgep;
       edgep = edgep->outNextp()) {
...
static const std::vector<AstCFunc *>
createThreadFunctions(const ThreadSchedule &schedule) {
  AstNodeModule *const modp = v3Global.rootp()->topModulep();
  FileLine *const fl = modp->fileline();
  ...
  for (const std::vector<const ExecMTask *> &thread : schedule.threads) {
    ...
    funcp->addStmtsp(new AstCStmt(
        fl, "vlSelf->__Vm_mtaskstate_final.signalUpstreamDone(even_cycle);\n"));
  }
  ...
  AstBasicDType *const mtaskStateDtypep =
      v3Global.rootp()->typeTablep()->findBasicDType(
          fl, AstBasicDTypeKwd::MTASKSTATE);
  AstVar *const varp = new AstVar(fl, AstVarType::MODULETEMP,
                                  "__Vm_mtaskstate_final", mtaskStateDtypep);
...
static void addThreadStartToExecGraph(AstExecGraph *const execGraphp,
  ...
  for (uint32_t i = 0; i <= last; ++i) {
    ...
    if (i != last) {
      ...
      addTextStmt("vlSymsp->__Vm_threadPoolp->workerp(" + cvtToStr(i) +
                  ")->addTask(");
  ...
  addStrStmt("vlSelf->__Vm_mtaskstate_final.waitUntilUpstreamDone(vlSymsp->__"
             "Vm_even_cycle);\n");
}
...
static void implementExecGraph(AstExecGraph *const execGraphp) {
  ...
  const ThreadSchedule &schedule =
      PartPackMTasks().pack(*execGraphp->mutableDepGraphp());

  // Create a function to be run by each thread. Note this moves all
  // AstMTaskBody nodes form the AstExecGrap into the AstCFunc created
  ...
  // Start the thread functions at the point this AstExecGraph is located in the
  // tree.
