...
public:
  ...
  bool operator<(const VarTspSorter &other) const {
...
class VariableOrder final {
  ...
  struct VarAttributes {
    uint32_t stratum; // Roughly equivalent to alignment requirement, to avoid
                      // padding
    bool anonOk;      // Can be emitted as part of anonymous structure
  };

  AstUser1Allocator<AstVar, VarAttributes>
      m_attributes; // Attributes used for sorting
  ...
  void tspSortVars(std::vector<AstVar *> &varps) {
    ...
    for (AstVar *const varp : varps) {
      m2v[varp->mtaskIds()].push_back(varp);
    }

    // Create a TSP sort state for each unique MTaskIdSet, except for the empty
    // set
    ...
    for (const V3TSP::TspStateBase *const stateBasep : sortedStates) {
      const VarTspSorter *const statep =
          dynamic_cast<const VarTspSorter *>(stateBasep);
  ...
  void orderModuleVars(AstNodeModule *modp) {
    ...
    for (AstNode *nodep = modp->stmtsp(), *nextp; nodep; nodep = nextp) {
      ...
      if (AstVar *const varp = VN_CAST(nodep, Var)) {
        ...
        attributes.stratum =
            (varp->isUsedClock() && varp->widthMin() == 1)   ? 0
...
public:
  static void processModule(AstNodeModule *modp) {
    VariableOrder().orderModuleVars(modp);
  }
};
...
void V3VariableOrder::orderAll() {
  ...
  V3Global::dumpCheckGlobalTree("variableorder", 0,
