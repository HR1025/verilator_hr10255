...
static void PlainMoudle(MoudleMsg& moudleMsg, std::unordered_map<std::string, MoudleMsg>& hierCellsNetLists)
{
    /**
     ...
     * @sa          参见具体使用处
     */
    auto initMoudleMsgCopy = 
    [](const MoudleMsg& moudleMsg) -> MoudleMsg
    {
        MoudleMsg result;
    ...
    {
        const std::string subMoudleDefName = moudleMsg.mouldeDefInstanceMap[subMoudleInstanceName];
        // 1 - 判断子模块是否是标准单元
        // Hint : 标准单元不需要展开，直接拷贝走就可以了
        if(IsStdCell(subMoudleDefName))
        {
            result.subMoudleInstanceNames.push_back(subMoudleInstanceName);
            result.mouldeDefInstanceMap[subMoudleInstanceName] = moudleMsg.mouldeDefInstanceMap[subMoudleInstanceName];
            result.subMoudlePorts[subMoudleInstanceName] = moudleMsg.subMoudlePorts[subMoudleInstanceName];
            continue;
        }
        // 2 - 展开非标准单元的子模块
        ...
        struct PortMsgHash
        {
            ...
            {
                return static_cast<size_t>(portMsg.arraySize * 2 + portMsg.portDefName.size() * 5);
            }
        };
        ...
        for (auto originPortMsg : subMoudleMsg.wires)
        {
            ...
            portMsg.portDefName = subMoudleInstanceName + "_" + originPortMsg.portDefName;
            result.wires.push_back(portMsg);
        }

        // 2.2 - 做子模块的 in, out, inout 到父模块的映射
        std::unordered_map<std::string, std::vector<PortInstanceFormalMsg>> portsDefInstanceMap;
        auto& subMoudlePorts = moudleMsg.subMoudlePorts[subMoudleInstanceName];
        for(auto& subMoudlePort : subMoudlePorts)
        {
            portsDefInstanceMap[subMoudlePort.portDefName] = subMoudlePort.portInstanceFormalMsgs;
        }

        // 3 - 将子模块中的标准单元实例搬移到 result 中去
        for(const auto& originSubSubMoudleInstanceName : subMoudleMsg.subMoudleInstanceNames)
        {
            // 3.1 - 插入新的标准单元实例名称
            std::string subSubMoudleInstanceName = subMoudleInstanceName + originSubSubMoudleInstanceName;
            result.subMoudleInstanceNames.push_back(subSubMoudleInstanceName);

            // 3.2 - 记录新的标准单元实例的类型
            result.mouldeDefInstanceMap[subSubMoudleInstanceName] = subMoudleMsg.mouldeDefInstanceMap[originSubSubMoudleInstanceName];
  
            // 3.3 - 将原来的标准单元的形参替换为新的映射关系
            {
                const auto& originSubPortInstanceMsgs  = subMoudleMsg.subMoudlePorts[originSubSubMoudleInstanceName];
                ...
                for(const auto& originPortInstanceMsg : originSubPortInstanceMsgs)
                {
                    ...
                    auto& subMoudlePorts = moudleMsg.subMoudlePorts[subMoudleInstanceName];
                    for(auto& originPortInstanceFormalMsg : originPortInstanceMsg.portInstanceFormalMsgs)
                    {
                        ...
                        else if(portsDefInstanceMap.count(originPortInstanceFormalMsg.portInstanceName) != 0)
                        {
                            if (originPortInstanceFormalMsg.isArray == false)
                            {
                                portInstanceFormalMsg = portsDefInstanceMap[originPortInstanceFormalMsg.portInstanceName][0];
                            }
                            else
                            {
                                ...
                                // TODO : 此处应该被列为 bug 修复
                                if (portsDefInstanceMap[originPortInstanceFormalMsg.portInstanceName].size() == 1 && portsDefInstanceMap[originPortInstanceFormalMsg.portInstanceName][0].isArray == false)
                                {
                                    portInstanceFormalMsg = portsDefInstanceMap[originPortInstanceFormalMsg.portInstanceName][0];
                                ...
                                {
                                    portInstanceFormalMsg = portsDefInstanceMap[originPortInstanceFormalMsg.portInstanceName][index];
                                }
                            }
                        }
                        // 3 - wire 类型向父类做转换 
                        ...
                        {
                            portInstanceFormalMsg = originPortInstanceFormalMsg;
                            portInstanceFormalMsg.portInstanceName = subMoudleInstanceName + "_" + portInstanceFormalMsg.portInstanceName;
                            std::cout << "originPortInstanceFormalMsg.portInstanceName is " << originPortInstanceFormalMsg.portInstanceName << std::endl;

                        }
                        portInstanceMsg.portInstanceFormalMsgs.push_back(portInstanceFormalMsg);
                    }
...
{
public:
    NetListsPlain(std::unordered_map<std::string, MoudleMsg>& hierCellsNetLists);
...
NetListsPlain::NetListsPlain(std::unordered_map<std::string, MoudleMsg>& hierCellsNetLists)
{
    ...
    std::sort(moduleDefNameOrderByLevel.begin(), moduleDefNameOrderByLevel.end(), 
...
void V3EmitPlainNetLists::emitPlainNetLists(std::unordered_map<std::string, MoudleMsg>& hierCellsNetLists,
