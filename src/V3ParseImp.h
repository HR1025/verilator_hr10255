...
struct VMemberQualifiers {
  union {
    ...
    struct {
      uint32_t
          m_local : 1; // Local class item (ignored until warning implemented)
      uint32_t m_protected : 1; // Protected class item (ignored until warning
                                // implemented)
      uint32_t m_rand : 1;      // Rand property/member qualifier
      uint32_t m_randc : 1;   // Randc property/member qualifier (ignored until
                              // supported)
  ...
  static VMemberQualifiers combine(const VMemberQualifiers &a,
                                   const VMemberQualifiers &b) {
    VMemberQualifiers q;
    q.m_flags = a.m_flags | b.m_flags;
    return q;
  }
  void applyToNodes(AstNodeFTask *nodesp) const {
    for (AstNodeFTask *nodep = nodesp; nodep;
         nodep = VN_AS(nodep->nextp(), NodeFTask)) {
      if (m_local)
        nodep->isHideLocal(true);
      if (m_protected)
        nodep->isHideProtected(true);
      if (m_virtual)
      ...
      if (m_const || m_rand || m_randc) {
        nodep->v3error(
            "Syntax error: 'const'/'rand'/'randc' not allowed before "
...
class V3ParseImp final {
  ...
  std::deque<V3ParseBisonYYSType>
      m_tokensAhead; // Tokens we parsed ahead of parser
  ...
  std::deque<string>
      m_ppBuffers; // Preprocessor->lex buffer of characters to process

  AstNode
      *m_tagNodep; // Points to the node to set to m_tag or nullptr to not set.
...
public:
  ...
  static int debugBison() {
    ...
    if (VL_UNLIKELY(level < 0))
      level = v3Global.opt.debugSrcLevel("bison");
    return level;
  }
  static int debugFlex() {
    static int level = -1;
    if (VL_UNLIKELY(level < 0))
      level = v3Global.opt.debugSrcLevel("flex");
    return level;
  }
  static int debug() {
    static int level = -1;
  ...
  void timescaleMod(FileLine *fl, AstNodeModule *modp, bool unitSet,
                    double unitVal, bool precSet, double precVal);
  VTimescale timeLastUnit() const { return m_timeLastUnit; }

  FileLine *lexFileline() const { return m_lexFileline; }
  FileLine *lexCopyOrSameFileLine() {
    return lexFileline()->copyOrSameFileLine();
  }
  ...
  void ppPushText(const string &text) {
    ...
    if (lexFileline()->contentp())
      lexFileline()->contentp()->pushText(text);
  }
  size_t ppInputToLex(char *buf, size_t max_size);

  static V3ParseImp *parsep() { return s_parsep; }

  // TODO: Many of these functions are the old interface; they'd be better as
  // non-static and called as READP->newString(...) etc. These can be called by
  // either parser or lexer, as not lex/parser-position aware
  ...
  // Bison sometimes needs error context without a token, so remember last
  // token's line Only use this if do not have and cannot get a token-relevent
  // fileline
  FileLine *bisonLastFileline() const { return m_bisonLastFileline; }

  // Return next token, for bison, since bison isn't class based, use a global
  // THIS
  AstNetlist *rootp() const { return m_rootp; }
  FileLine *copyOrSameFileLine() {
    return bisonLastFileline()->copyOrSameFileLine();
  }
  ...
  int lexPrevToken() const {
    return m_lexPrevToken;
  } // Parser -> lexer communication
  size_t flexPpInputToLex(char *buf, size_t max_size) {
    return ppInputToLex(buf, max_size);
  }
