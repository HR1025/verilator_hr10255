...
// Mark all classes whose instances are randomized member variables of marked
// classes Each marked class:
...
private:
  // NODE STATE
  // Cleared on Netlist
  //  AstClass::user1()       -> bool.  Set true to indicate needs randomize
  //  processing
  ...
  BaseToDerivedMap m_baseToDerivedMap; // Mapping from base classes to classes
                                       // that extend them
  ...
  void markMembers(AstClass *nodep) {
    for (auto *classp = nodep; classp;
      ...
      for (auto *memberp = classp->stmtsp(); memberp;
...
private:
  ...
  // Cleared on Netlist
  //  AstClass::user1()       -> bool.  Set true to indicate needs randomize
  //  processing AstEnumDType::user2()   -> AstVar*.  Pointer to table with enum
  //  values
  // AstUser1InUse    m_inuser1;      (Allocated for use in
  // RandomizeMarkVisitor)
  ...
  AstNodeStmt *newRandStmtsp(FileLine *fl, AstNodeVarRef *varrefp,
                             int offset = 0,
                             AstMemberDType *memberp = nullptr) {
    if (auto *structDtp = VN_CAST(memberp ? memberp->subDTypep()->skipRefp()
                                          : varrefp->dtypep()->skipRefp(),
                                  StructDType)) {
      AstNodeStmt *stmtsp = nullptr;
      offset += memberp ? memberp->lsb() : 0;
      for (auto *smemberp = structDtp->membersp(); smemberp;
           smemberp = VN_AS(smemberp->nextp(), MemberDType)) {
        auto *randp = newRandStmtsp(
  ...
  virtual void visit(AstClass *nodep) override {
    iterateChildren(nodep);
    if (!nodep->user1())
      return; // Doesn't need randomize, or already processed
    ...
    for (auto *classp = nodep; classp;
      ...
      for (auto *memberp = classp->stmtsp(); memberp;
        ...
        if (!memberVarp || !memberVarp->isRand())
          continue;
...
void V3Randomize::randomizeNetlist(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("randomize", 0,
...
AstFunc *V3Randomize::newRandomizeFunc(AstClass *nodep) {
  ...
  if (!funcp) {
    auto *dtypep = nodep->findBitDType(
        32, 32, VSigning::SIGNED); // IEEE says int return of 0/1
    auto *fvarp =
        new AstVar(nodep->fileline(), AstVarType::MEMBER, "randomize", dtypep);
