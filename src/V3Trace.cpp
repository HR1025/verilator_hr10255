...
public:
  ...
  TraceActivityVertex(V3Graph *graphp, vlsint32_t code)
      : V3GraphVertex{graphp}, m_insertp{nullptr} {
    m_activityCode = code;
    m_slow = false;
  }
  ...
  AstNode *insertp() const {
    if (!m_insertp)
      v3fatalSrc("Null insertp; probably called on a special always/slow.");
    return m_insertp;
  }
...
private:
  ...
  AstScope *const m_topScopep =
      v3Global.rootp()->topScopep()->scopep(); // The top AstScope
  ...
  void detectDuplicates() {
    ...
    for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
         itp = itp->verticesNextp()) {
      if (TraceTraceVertex *const vvertexp =
              dynamic_cast<TraceTraceVertex *>(itp)) {
        ...
        if (nodep->valuep() && !vvertexp->duplicatep()) {
          ...
          if (dupit != dupFinder.end()) {
            ...
            TraceTraceVertex *const dupvertexp =
                dynamic_cast<TraceTraceVertex *>(
                    dupDeclp->user1u().toGraphVertex());
  ...
  void graphSimplify(bool initial) {
    if (initial) {
      ...
      for (V3GraphVertex *nextp, *itp = m_graph.verticesBeginp(); itp;
           itp = nextp) {
        nextp = itp->verticesNextp();
        if (TraceVarVertex *const vvertexp =
                dynamic_cast<TraceVarVertex *>(itp)) {
          vvertexp->rerouteEdges(&m_graph);
          vvertexp->unlinkDelete(&m_graph);
        }
      }
      // Remove multiple variables connecting funcs to traces
      // We do this twice, as then we have fewer edges to multiply out in the
      // below expansion.
      ...
      for (V3GraphVertex *nextp, *itp = m_graph.verticesBeginp(); itp;
           itp = nextp) {
        nextp = itp->verticesNextp();
        if (TraceCFuncVertex *const vvertexp =
                dynamic_cast<TraceCFuncVertex *>(itp)) {
          vvertexp->rerouteEdges(&m_graph);
          vvertexp->unlinkDelete(&m_graph);
        }
      }
    ...
    for (const V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
      ...
      if (const TraceTraceVertex *const vvertexp =
        ...
        if (alwaysEdgep) {
          for (V3GraphEdge *nextp, *edgep = vvertexp->inBeginp(); edgep;
               edgep = nextp) {
            nextp = edgep->inNextp();
            if (edgep != alwaysEdgep)
              edgep->unlinkDelete();
          }
        }
      }
    ...
    for (V3GraphVertex *nextp, *itp = m_graph.verticesBeginp(); itp;
         itp = nextp) {
      nextp = itp->verticesNextp();
      if (TraceActivityVertex *const vtxp =
              dynamic_cast<TraceActivityVertex *>(itp)) {
  ...
  uint32_t assignactivityNumbers() {
    ...
    for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
         itp = itp->verticesNextp()) {
      if (TraceActivityVertex *const vvertexp =
              dynamic_cast<TraceActivityVertex *>(itp)) {
  ...
  void sortTraces(TraceVec &traces, uint32_t &nFullCodes, uint32_t &nChgCodes) {
    ...
    for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
      ...
      if (TraceTraceVertex *const vtxp =
        ...
        if (debug() >= 9)
          vtxp->nodep()->dumpTree(cout, "-   trnode: ");
        ...
        } else if (actSet.count(TraceActivityVertex::ACTIVITY_SLOW) &&
                   actSet.size() > 1) {
    ...
    while (it != end) {
      ...
      for (; it != end && it->first == actSet; ++it) {
        if (!it->second->duplicatep()) {
          ...
          cost *=
              declp->arrayRange().ranged() ? declp->arrayRange().elements() : 1;
      ...
      // Leave alone always changing, never changing and signals only set in
      // slow code
      if (actSet.count(TraceActivityVertex::ACTIVITY_ALWAYS))
        continue;
      if (actSet.count(TraceActivityVertex::ACTIVITY_NEVER))
        continue;
      if (actSet.count(TraceActivityVertex::ACTIVITY_SLOW))
        continue;
  ...
  AstNode *selectActivity(FileLine *flp, uint32_t acode,
                          const VAccess &access) {
    return new AstArraySel(flp, new AstVarRef(flp, m_activityVscp, access),
                           acode);
  }
  ...
  void addActivitySetter(AstNode *insertp, uint32_t code) {
    ...
    AstAssign *const setterp =
        new AstAssign(fl, selectActivity(fl, code, VAccess::WRITE),
  ...
  void createActivityFlags() {
    ...
    AstNodeDType *const newScalarDtp =
        new AstBasicDType(flp, VFlagLogicPacked(), 1);
    ...
    AstNodeDType *const newArrDtp =
        new AstUnpackArrayDType(flp, newScalarDtp, newArange);
    v3Global.rootp()->typeTablep()->addTypesp(newArrDtp);
    AstVar *const newvarp = new AstVar(flp, AstVarType::MODULETEMP,
  ...
  AstCFunc *newCFunc(bool full, AstCFunc *topFuncp, int &funcNump,
                     uint32_t baseCode = 0) {
    // Create new function
    ...
    AstCFunc *const funcp =
        new AstCFunc(flp, baseName + cvtToStr(funcNump++), m_topScopep);
    funcp->isTrace(true);
    ...
    if (isTopFunc) {
      ...
      // Setup base references. Note in rare occasions we can end up with an
      // empty trace sub function, hence the VL_ATTR_UNUSED attributes.
      if (full) {
    ...
    while (it != traces.cend()) {
      ...
      for (; nCodes < maxCodes && it != traces.end(); ++it) {
        ...
        if (const TraceTraceVertex *const canonVtxp = vtxp->duplicatep()) {
          ...
          if (!topFuncp) {
            topFuncp = newCFunc(/* full: */ true, nullptr, topFuncNum);
          }
      ...
      if (topFuncp) { // might be nullptr if all trailing entries were
                      // duplicates
  ...
  void createChgTraceFunctions(const TraceVec &traces, uint32_t nAllCodes,
    ...
    while (it != traces.end()) {
      ...
      for (; nCodes < maxCodes && it != traces.end(); ++it) {
        ...
        // Create top function if not yet created
        if (!topFuncp) {
          topFuncp = newCFunc(/* full: */ false, nullptr, topFuncNum);
        }
        ...
        if (!subFuncp || subStmts > splitLimit) {
          ...
          subStmts = 0;
          subFuncp =
              newCFunc(/* full: */ false, topFuncp, subFuncNum, baseCode);
          prevActSet = nullptr;
          ifp = nullptr;
        }
        ...
        if (!prevActSet || actSet != *prevActSet) {
          FileLine *const flp = m_topScopep->fileline();
          const bool always =
              actSet.count(TraceActivityVertex::ACTIVITY_ALWAYS) != 0;
        ...
        // Add TraceInc node
        AstTraceInc *const incp = new AstTraceInc(declp->fileline(), declp,
                                                  /* full: */ false, baseCode);
        ifp->addIfsp(incp);
        subStmts += EmitCBaseCounterVisitor(incp).count();
  ...
  void createCleanupFunction() {
    ...
    AstCFunc *const cleanupFuncp =
        new AstCFunc(fl, "trace_cleanup", m_topScopep);
    ...
    cleanupFuncp->addStmtsp(new AstCStmt(
        m_topScopep->fileline(), string("vlSymsp->__Vm_activity = false;\n")));

    // Clear fine grained activity flags
    for (uint32_t i = 0; i < m_activityNumber; ++i) {
      AstNode *const clrp =
          new AstAssign(fl, selectActivity(fl, i, VAccess::WRITE),
  ...
  void createTraceFunctions() {
    ...
    if (debug() >= 6)
      m_graph.dumpDotFilePrefixed("trace_pre");
    graphSimplify(true);
    if (debug() >= 6)
      m_graph.dumpDotFilePrefixed("trace_simplified");
    graphOptimize();
    if (debug() >= 6)
      m_graph.dumpDotFilePrefixed("trace_optimized");
    ...
    // We will split functions such that each have to dump roughly the same
    // amount of data for this we need to keep tack of the number of codes used
    // by the trace functions.
    uint32_t nFullCodes =
        0; // Number of non-duplicate codes (need to go into full* dump)
    uint32_t nChgCodes =
        0; // Number of non-consant codes (need to go in to chg* dump)
    sortTraces(traces, nFullCodes, nChgCodes);

    UINFO(5,
          "nFullCodes: " << nFullCodes << " nChgCodes: " << nChgCodes << endl);
    ...
    const int parallelism =
        1; // Note: will bump this later, code below works for any value
    ...
    // Remove refs to traced values from TraceDecl nodes, these have now moved
    // under TraceInc
  ...
  virtual void visit(AstNetlist *nodep) override {
    ...
    // are assigned as duplicate varscopes must result in the same tracing
    // code#.
  ...
  virtual void visit(AstCFunc *nodep) override {
    ...
    if (!m_finding) { // If public, we need a unique activity code to allow for
                      // sets directly in this func
      if (nodep->funcPublic() || nodep->dpiExportImpl() ||
          nodep == v3Global.rootp()->evalp()) {
        V3GraphVertex *const activityVtxp =
            getActivityVertexp(nodep, nodep->slow());
  ...
  virtual void visit(AstVarRef *nodep) override {
    if (m_tracep) {
      ...
      UASSERT_OBJ(nodep->access().isReadOnly(), nodep,
                  "Lvalue in trace?  Should be const.");
      ...
      V3GraphVertex *const varVtxp =
          nodep->varScopep()->user1u().toGraphVertex();
...
public:
  ...
  explicit TraceVisitor(AstNetlist *nodep)
      : m_alwaysVtxp{new TraceActivityVertex{
            &m_graph, TraceActivityVertex::ACTIVITY_ALWAYS}} {
    iterate(nodep);
  }
...
void V3Trace::traceAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("trace", 0,
