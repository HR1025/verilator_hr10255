...
static void makeVlToString(AstClass *nodep) {
  ...
  funcp->argTypes("const VlClassRef<" +
                  EmitCBaseVisitor::prefixNameProtect(nodep) + ">& obj");
...
static void makeToString(AstClass *nodep) {
  ...
  funcp->isStatic(false);
  funcp->protect(false);
  AstNode *const exprp = new AstCMath{
      nodep->fileline(), R"(std::string("'{") + to_string_middle() + "}")", 0};
  exprp->dtypeSetString();
  funcp->addStmtsp(new AstCReturn{nodep->fileline(), exprp});
  nodep->addStmtp(funcp);
}
static void makeToStringMiddle(AstClass *nodep) {
  AstCFunc *const funcp = new AstCFunc{nodep->fileline(), "to_string_middle",
                                       nullptr, "std::string"};
  funcp->isConst(true);
  funcp->isStatic(false);
  funcp->protect(false);
...
void V3Common::commonAll() {
  ...
  for (AstNode *nodep = v3Global.rootp()->modulesp(); nodep;
       nodep = nodep->nextp()) {
  ...
  V3Global::dumpCheckGlobalTree("common", 0,
