...
class AstUserAllocatorBase VL_NOT_FINAL {
  ...
  static_assert(std::is_base_of<AstNode, T_Node>::value,
                "T_Node must be an AstNode type");

private:
  std::vector<T_Data *> m_allocated;

  inline T_Data *getUserp(const T_Node *nodep) const {
    // This simplifies statically as T_UserN is constant. In C++17, use 'if
    // constexpr'.
    if (T_UserN == 1) {
...
protected:
  ...
  virtual ~AstUserAllocatorBase() {
    ...
    for (T_Data *const p : m_allocated) {
      delete p;
    }
  }
...
// User pointer allocator classes. T_Node is the type of node the allocator
// should be applied to and is simply there for a bit of extra type safety.
// T_Data is the type of the data structure managed by the allocator.
