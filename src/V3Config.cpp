...
// function that takes a reference of this type to join multiple entities into
// one.
...
public:
  ...
  void update(const V3ConfigWildcardResolver &other) {
    ...
    for (it = other.m_mapResolved.begin(); it != other.m_mapResolved.end();
         ++it) {
      m_mapResolved[it->first].update(it->second);
    }
    for (it = other.m_mapWildcard.begin(); it != other.m_mapWildcard.end();
         ++it) {
...
public:
  ...
  V3ConfigVarAttr(AstAttrType type, AstSenTree *sentreep)
      : m_type{type}, m_sentreep{sentreep} {}
};
  ...
  void apply(AstVar *varp) {
    for (const_iterator it = begin(); it != end(); ++it) {
      ...
      if (it->m_type == AstAttrType::VAR_PUBLIC_FLAT_RW && it->m_sentreep) {
        newp->addNext(
            new AstAlwaysPublic(varp->fileline(), it->m_sentreep, nullptr));
      }
    }
  }
};
...
public:
  ...
  void apply(AstNodeFTask *ftaskp) const {
    if (m_noinline)
      ftaskp->addStmtsp(
          new AstPragma(ftaskp->fileline(), AstPragmaType::NO_INLINE_TASK));
    if (m_public)
      ftaskp->addStmtsp(
          new AstPragma(ftaskp->fileline(), AstPragmaType::PUBLIC_TASK));
    // Only functions can have isolate (return value)
    if (VN_IS(ftaskp, Func))
      ftaskp->attrIsolateAssign(m_isolate);
  }
};
...
class V3ConfigModule final {
  ...
  std::unordered_set<std::string>
      m_coverageOffBlocks;              // List of block names for coverage_off
...
public:
  ...
  void update(const V3ConfigModule &m) {
    ...
    for (const string &i : m.m_coverageOffBlocks)
      m_coverageOffBlocks.insert(i);
    if (!m_inline) {
      m_inline = m.m_inline;
...
class V3ConfigFile final {
  using LineAttrMap =
      std::map<int, V3ConfigLineAttribute>; // Map line->bitset of attributes
  using IgnLines = std::multiset<V3ConfigIgnoresLine>; // list of {line,code,on}
  using WaiverSetting =
      std::pair<V3ErrorCode, std::string>;    // Waive code if string matches
  ...
  struct {
    ...
    IgnLines::const_iterator
        it;       // Point with next linenumber > current line number
  ...
  bool lineMatch(int lineno, AstPragmaType type) {
    if (m_lineAttrs.find(0) != m_lineAttrs.end() && m_lineAttrs[0][type])
      return true;
    if (m_lineAttrs.find(lineno) == m_lineAttrs.end())
      return false;
...
public:
  ...
  void update(const V3ConfigFile &file) {
    ...
    for (const auto &itr : file.m_lineAttrs) {
      m_lineAttrs[itr.first] |= itr.second;
    }
    // Copy in all ignores
    for (const auto &ignLine : file.m_ignLines)
      m_ignLines.insert(ignLine);
    // Update the iterator after the list has changed
    m_lastIgnore.it = m_ignLines.begin();
    m_waivers.reserve(m_waivers.size() + file.m_waivers.size());
    m_waivers.insert(m_waivers.end(), file.m_waivers.begin(),
                     file.m_waivers.end());
  }
  void addLineAttribute(int lineno, AstPragmaType attr) {
    m_lineAttrs[lineno].set(attr);
  }
  ...
  void applyCase(AstCase *nodep) {
    ...
    if (lineMatch(lineno, AstPragmaType::FULL_CASE))
      nodep->fullPragma(true);
    if (lineMatch(lineno, AstPragmaType::PARALLEL_CASE))
  ...
  inline void applyIgnores(FileLine *filelinep) {
    ...
    if (m_lastIgnore.lineno != filelinep->lineno()) {
      ...
      for (; m_lastIgnore.it != m_ignLines.end(); ++m_lastIgnore.it) {
        if (m_lastIgnore.it->m_lineno > curlineno)
          break;
        // UINFO(9, "     Hit " << *m_lastIt << endl);
        filelinep->warnOn(m_lastIgnore.it->m_code, m_lastIgnore.it->m_on);
      }
      if (false && debug() >= 9) {
        for (IgnLines::const_iterator it = m_lastIgnore.it;
             it != m_ignLines.end(); ++it) {
          UINFO(9, "     NXT " << *it << endl);
        }
      }
...
class V3ConfigResolver final {
  ...
  static V3ConfigResolver
      s_singleton; // Singleton (not via local static, as that's slow)
...
public:
  ...
  void addProfileData(FileLine *fl, const string &model, const string &key,
                      vluint64_t cost) {
    if (!m_profileFileLine)
      m_profileFileLine = fl;
    if (cost == 0)
      cost = 1; // Cost 0 means delete (or no data)
  ...
  FileLine *getProfileDataFileLine() const {
    return m_profileFileLine;
  } // Maybe null
};
...
void V3Config::addCoverageBlockOff(const string &module,
                                   const string &blockname) {
  V3ConfigResolver::s().modules().at(module).addCoverageBlockOff(blockname);
}

void V3Config::addIgnore(V3ErrorCode code, bool on, const string &filename,
                         int min, int max) {
  if (filename == "*") {
    ...
    if (max)
      V3ConfigResolver::s().files().at(filename).addIgnore(code, max, !on);
    V3ConfigResolver::s().files().flush();
  }
}

void V3Config::addInline(FileLine *fl, const string &module,
                         const string &ftask, bool on) {
  if (ftask.empty()) {
    ...
    if (!on) {
      ...
      V3ConfigResolver::s().modules().at(module).ftasks().at(ftask).setNoInline(
          on);
    }
  }
}
...
void V3Config::addProfileData(FileLine *fl, const string &model,
                              const string &key, vluint64_t cost) {
  V3ConfigResolver::s().addProfileData(fl, model, key, cost);
}

void V3Config::addVarAttr(FileLine *fl, const string &module,
                          const string &ftask, const string &var,
                          AstAttrType attr, AstSenTree *sensep) {
  ...
  if (var.empty()) {
    if (attr == AstAttrType::VAR_ISOLATE_ASSIGNMENTS) {
      if (ftask.empty()) {
        fl->v3error(
            "isolate_assignments only applies to signals or functions/tasks");
      } else {
        V3ConfigResolver::s()
            .modules()
            .at(module)
      ...
      if (ftask.empty()) {
        ...
        V3ConfigResolver::s().modules().at(module).ftasks().at(ftask).setPublic(
            true);
      }
    } else {
    ...
    if (ftask.empty()) {
      ...
      mod.ftasks().at(ftask).vars().at(var).push_back(
          V3ConfigVarAttr(attr, sensep));
    }
  }
}

void V3Config::addWaiver(V3ErrorCode code, const string &filename,
...
void V3Config::applyVarAttr(AstNodeModule *modulep, AstNodeFTask *ftaskp,
  ...
  V3ConfigVar *vp;
  V3ConfigModule *modp =
      V3ConfigResolver::s().modules().resolve(modulep->name());
  if (!modp)
    return;
  if (ftaskp) {
    V3ConfigFTask *ftp = modp->ftasks().resolve(ftaskp->name());
...
bool V3Config::waive(FileLine *filelinep, V3ErrorCode code,
                     const string &message) {
  V3ConfigFile *filep =
      V3ConfigResolver::s().files().resolve(filelinep->filename());
  if (!filep)
    return false;
