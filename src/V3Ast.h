...
}; // Used by parser.y to select constructor that sets childDType
...
#define BROKEN_RTN(test)                                                       \
  do {                                                                         \
    if (VL_UNCOVERABLE(test))                                                  \
      return "'" #test "' @ " __FILE__ ":" VL_STRINGIFY(__LINE__);             \
  } while (false)
// For broken() function, return error string if a base of this class has a
// match
#define BROKEN_BASE_RTN(test)                                                  \
  do {                                                                         \
    const char *reasonp = (test);                                              \
    if (VL_UNCOVERABLE(reasonp))                                               \
      return reasonp;                                                          \
  } while (false)

// (V)erilator (N)ode is: Returns true if and only if AstNode is of the given
// AstNode subtype, and not nullptr.
#define VN_IS(nodep, nodetypename)                                             \
  (AstNode::privateIs<Ast##nodetypename, decltype(nodep)>(nodep))

// (V)erilator (N)ode cast: Similar to dynamic_cast, but more efficient, use
// this instead. Cast to given type if node is of such type, otherwise returns
// nullptr. If 'nodep' is nullptr, return nullptr. Pointer constness is
// preserved, i.e.: given a 'const AstNode*', a 'const Ast<nodetypename>*' is
// returned.
...
// 指针的 const 属性将被保留，例如给定一个 'const AstNode*' ，然后一个
// 'Ast<nodetypename>*' 将会被返回。
#define VN_AS(nodep, nodetypename)                                             \
  (AstNode::privateAs<Ast##nodetypename, decltype(nodep)>(nodep))
...
inline bool operator==(const AstType &lhs, const AstType &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const AstType &lhs, AstType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(AstType::en lhs, const AstType &rhs) {
  return lhs == rhs.m_e;
}
inline std::ostream &operator<<(std::ostream &os, const AstType &rhs) {
  return os << rhs.ascii();
}
...
public:
  ...
  inline VLifetime() : m_e{NONE} {}
  // cppcheck-suppress noExplicitConstructor
  inline VLifetime(en _e) : m_e{_e} {}
...
inline bool operator==(const VLifetime &lhs, const VLifetime &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VLifetime &lhs, VLifetime::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VLifetime::en lhs, const VLifetime &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  enum en : uint8_t {
    ...
    //          // For non-simple data types only e.g. no tristates/delayed
    //          vars.
  ...
  inline VAccess() : m_e{READ} {}
  // cppcheck-suppress noExplicitConstructor
  inline VAccess(en _e) : m_e{_e} {}
  explicit inline VAccess(int _e)
      : m_e(static_cast<en>(_e)) {} // Need () or GCC 4.8 false warning
  operator en() const { return m_e; }
  VAccess invert() const {
    return (m_e == READWRITE) ? VAccess(m_e)
                              : (m_e == WRITE ? VAccess(READ) : VAccess(WRITE));
  }
...
inline bool operator==(const VAccess &lhs, const VAccess &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VAccess &lhs, VAccess::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VAccess::en lhs, const VAccess &rhs) {
  return lhs == rhs.m_e;
}
inline std::ostream &operator<<(std::ostream &os, const VAccess &rhs) {
  return os << rhs.ascii();
}
...
public:
  ...
  inline VSigning() : m_e{UNSIGNED} {}
  // cppcheck-suppress noExplicitConstructor
  inline VSigning(en _e) : m_e{_e} {}
...
inline bool operator==(const VSigning &lhs, const VSigning &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VSigning &lhs, VSigning::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VSigning::en lhs, const VSigning &rhs) {
  return lhs == rhs.m_e;
}
...
inline bool operator==(const AstPragmaType &lhs, AstPragmaType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(AstPragmaType::en lhs, const AstPragmaType &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  enum en : uint8_t {
    ...
    ET_SETTLE,  // Like combo but for initial wire resolutions after initial
                // statement
    ET_NEVER    // Never occurs (optimized away)
  };
  enum en m_e;
  bool clockedStmt() const {
    static const bool clocked[] = {false, false, true,  true,  true,
  ...
  VEdgeType invert() const {
    ...
    case ET_LOWEDGE:
      return ET_HIGHEDGE;
    default:
      UASSERT_STATIC(0, "Inverting bad edgeType()");
    }
    return VEdgeType::ET_ILLEGAL;
  }
  const char *ascii() const {
    static const char *const names[] = {"%E-edge", "ANY",    "BOTH", "POS",
                                        "NEG",     "HIGH",   "LOW",  "COMBO",
...
inline bool operator==(const VEdgeType &lhs, const VEdgeType &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VEdgeType &lhs, VEdgeType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VEdgeType::en lhs, const VEdgeType &rhs) {
  return lhs == rhs.m_e;
}
...
inline bool operator==(const AstAttrType &lhs, AstAttrType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(AstAttrType::en lhs, const AstAttrType &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  int width() const {
    switch (m_e) {
    case BIT:
      return 1; // scalar, can't bit extract unless ranged
    case BYTE:
      return 8;
    case CHANDLE:
      return 64;
    ...
    case TIME:
      return 64;
    case STRING:
      return 64; // opaque  // Just the pointer, for today
    case SCOPEPTR:
      return 0; // opaque
    case CHARPTR:
      return 0; // opaque
  ...
  bool isSigned() const {
    return m_e == BYTE || m_e == SHORTINT || m_e == INT || m_e == LONGINT ||
           m_e == INTEGER || m_e == DOUBLE;
  }
  bool isUnsigned() const {
    return m_e == CHANDLE || m_e == EVENTVALUE || m_e == STRING ||
           m_e == SCOPEPTR || m_e == CHARPTR || m_e == UINT32 ||
           m_e == UINT64 || m_e == BIT || m_e == LOGIC || m_e == TIME;
  }
  ...
  bool isZeroInit() const { // Otherwise initializes to X
    return (m_e == BIT || m_e == BYTE || m_e == CHANDLE || m_e == EVENTVALUE ||
            m_e == INT || m_e == LONGINT || m_e == SHORTINT || m_e == STRING ||
            m_e == DOUBLE);
  }
  bool isIntNumeric() const { // Enum increment supported
    return (m_e == BIT || m_e == BYTE || m_e == INT || m_e == INTEGER ||
            m_e == LOGIC || m_e == LONGINT || m_e == SHORTINT ||
  ...
  bool isDpiCLayout() const { // Uses standard C layout, for DPI runtime access
    return (m_e == BIT || m_e == BYTE || m_e == CHANDLE || m_e == INT ||
            m_e == LONGINT || m_e == DOUBLE || m_e == SHORTINT ||
            m_e == UINT32 || m_e == UINT64);
  }
...
inline bool operator==(const AstBasicDTypeKwd &lhs,
                       const AstBasicDTypeKwd &rhs) {
  return lhs.m_e == rhs.m_e;
}
...
public:
  ...
  operator en() const { return m_e; }
  const char *ascii() const {
    static const char *const names[] = {"NONE",  "INPUT", "OUTPUT",
                                        "INOUT", "REF",   "CONSTREF"};
    return names[m_e];
  }
  ...
  bool isWritable() const {
    return m_e == OUTPUT || m_e == INOUT || m_e == REF;
  }
  bool isRefOrConstRef() const { return m_e == REF || m_e == CONSTREF; }
};
inline bool operator==(const VDirection &lhs, const VDirection &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VDirection &lhs, VDirection::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VDirection::en lhs, const VDirection &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  enum en m_e;
  // CONSTRUCTOR - note defaults to *UNKNOWN*
  inline VJoinType() : m_e{JOIN} {}
  // cppcheck-suppress noExplicitConstructor
  inline VJoinType(en _e) : m_e{_e} {}
...
inline bool operator==(const VJoinType &lhs, const VJoinType &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VJoinType &lhs, VJoinType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VJoinType::en lhs, const VJoinType &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  const char *ascii() const {
    static const char *const names[] = {
        ...
        "TRIWIRE",    "TRI0",     "TRI1",   "PORT",     "BLOCKTEMP",
  ...
  bool isSignal() const {
    return (m_e == WIRE || m_e == WREAL || m_e == IMPLICITWIRE ||
            ...
            m_e == SUPPLY0 || m_e == SUPPLY1 || m_e == VAR);
  }
  bool isContAssignable() const { // In Verilog, always ok in SystemVerilog
    return (m_e == SUPPLY0 || m_e == SUPPLY1 || m_e == WIRE || m_e == WREAL ||
            m_e == IMPLICITWIRE || m_e == TRIWIRE || m_e == TRI0 ||
            m_e == TRI1 || m_e == PORT || m_e == BLOCKTEMP ||
            m_e == MODULETEMP || m_e == STMTTEMP || m_e == XTEMP ||
            m_e == IFACEREF);
  }
  bool isProcAssignable() const {
    return (m_e == GPARAM || m_e == LPARAM || m_e == GENVAR || m_e == VAR ||
            m_e == BLOCKTEMP || m_e == MODULETEMP || m_e == STMTTEMP ||
            m_e == XTEMP || m_e == IFACEREF || m_e == MEMBER);
  }
  bool isTemp() const {
    return (m_e == BLOCKTEMP || m_e == MODULETEMP || m_e == STMTTEMP ||
            m_e == XTEMP);
  }
};
inline bool operator==(const AstVarType &lhs, const AstVarType &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const AstVarType &lhs, AstVarType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(AstVarType::en lhs, const AstVarType &rhs) {
  return lhs == rhs.m_e;
}
...
inline bool operator==(const VBranchPred &lhs, VBranchPred::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VBranchPred::en lhs, const VBranchPred &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  operator en() const { return m_e; }
  const char *ascii() const {
    static const char *const names[] = {"always", "always_ff", "always_latch",
                                        "always_comb"};
    return names[m_e];
  }
};
inline bool operator==(const VAlwaysKwd &lhs, const VAlwaysKwd &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VAlwaysKwd &lhs, VAlwaysKwd::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VAlwaysKwd::en lhs, const VAlwaysKwd &rhs) {
  return lhs == rhs.m_e;
}
...
inline bool operator==(const VCaseType &lhs, const VCaseType &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VCaseType &lhs, VCaseType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VCaseType::en lhs, const VCaseType &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  enum en m_e;
  AstDisplayType() : m_e{DT_DISPLAY} {}
  // cppcheck-suppress noExplicitConstructor
  AstDisplayType(en _e) : m_e{_e} {}
  explicit inline AstDisplayType(int _e)
      : m_e(static_cast<en>(_e)) {} // Need () or GCC 4.8 false warning
  operator en() const { return m_e; }
  bool addNewline() const { return m_e != DT_WRITE; }
  bool needScopeTracking() const {
    return m_e != DT_DISPLAY && m_e != DT_WRITE;
  }
  const char *ascii() const {
    static const char *const names[] = {"display", "write", "monitor", "strobe",
...
public:
  ...
  enum en m_e;
  inline VDumpCtlType() : m_e{ON} {}
  // cppcheck-suppress noExplicitConstructor
  inline VDumpCtlType(en _e) : m_e{_e} {}
  explicit inline VDumpCtlType(int _e)
      : m_e(static_cast<en>(_e)) {} // Need () or GCC 4.8 false warning
  operator en() const { return m_e; }
  const char *ascii() const {
    static const char *const names[] = {"$dumpfile",  "$dumpvars",  "$dumpall",
                                        "$dumpflush", "$dumplimit", "$dumpoff",
                                        "$dumpon"};
    return names[m_e];
  }
};
...
inline bool operator==(const VDumpCtlType &lhs, VDumpCtlType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VDumpCtlType::en lhs, const VDumpCtlType &rhs) {
  return lhs == rhs.m_e;
}
...
inline bool operator==(const VParseRefExp &lhs, VParseRefExp::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VParseRefExp::en lhs, const VParseRefExp &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  VNumRange(int left, int right)
      : m_left{left}, m_right{right}, m_ranged{true} {}
  ~VNumRange() = default;
  // MEMBERS
...
public:
  ...
  bool isFwdClass() const {
    return m_e == IMP_FWD_CLASS || m_e == INT_FWD_CLASS;
  }
  operator en() const { return m_e; }
  const char *ascii() const {
    static const char *const names[] = {"IMP_INC", "INT_INC", "IMP_FWD",
                                        "INT_FWD"};
    return names[m_e];
  }
};
inline bool operator==(const VUseType &lhs, const VUseType &rhs) {
  return lhs.m_e == rhs.m_e;
}
inline bool operator==(const VUseType &lhs, VUseType::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(VUseType::en lhs, const VUseType &rhs) {
  return lhs == rhs.m_e;
}
...
public:
  ...
  int m_widthMin;     // From AstNodeDType: If unsized, bitwidth of minimum
                      // implementation
  VSigning m_numeric; // From AstNodeDType: Node is signed
  AstBasicDTypeKwd
      m_keyword; // From AstBasicDType: What keyword created basic type
  VNumRange
      m_nrange; // From AstBasicDType: Numeric msb/lsb (if non-opaque keyword)
  bool operator==(const VBasicTypeKey &rhs) const {
    return m_width == rhs.m_width && m_widthMin == rhs.m_widthMin &&
           m_numeric == rhs.m_numeric && m_keyword == rhs.m_keyword &&
  ...
  bool operator<(const VBasicTypeKey &rhs) const {
    ...
    if (!(m_width == rhs.m_width))
      return false; // lhs > rhs
    if ((m_widthMin < rhs.m_widthMin))
      return true;
    if (!(m_widthMin == rhs.m_widthMin))
      return false; // lhs > rhs
    if ((m_numeric < rhs.m_numeric))
      return true;
    if (!(m_numeric == rhs.m_numeric))
      return false; // lhs > rhs
    if ((m_keyword < rhs.m_keyword))
      return true;
    if (!(m_keyword == rhs.m_keyword))
      return false; // lhs > rhs
    if ((m_nrange < rhs.m_nrange))
      return true;
    if (!(m_nrange == rhs.m_nrange))
      return false; // lhs > rhs
    return false;
...
protected:
  static void allocate(int id, uint32_t &cntGblRef, bool &userBusyRef) {
    ...
    UASSERT_STATIC(!userBusyRef,
                   "Conflicting user use; AstUser" + cvtToStr(id) +
  ...
  static void free(int id, uint32_t &cntGblRef, bool &userBusyRef) {
    UASSERT_STATIC(userBusyRef,
                   "Free of User" + cvtToStr(id) + "() not under AstUserInUse");
  ...
  static void clearcnt(int id, uint32_t &cntGblRef, const bool &userBusyRef) {
    UASSERT_STATIC(userBusyRef, "Clear of User" + cvtToStr(id) +
                                    "() not under AstUserInUse");
  ...
  static void checkcnt(int id, uint32_t &, const bool &userBusyRef) {
    UASSERT_STATIC(userBusyRef, "Check of User" + cvtToStr(id) +
                                    "() failed, not under AstUserInUse");
  }
};
...
protected:
  ...
  AstNode *m_oldp =
      nullptr; // The old node that was linked to this point in the tree
...
class AstNode VL_NOT_FINAL {
  ...
  AstNode **m_iterpp;   // Pointer to node iterating on, change it if we replace
                        // this node.
  const AstType m_type; // 节点子类型标识符
  // ^ ASTNODE_PREFETCH depends on above ordering of members

  // AstType is 2 bytes, so we can stick another 6 bytes after it to utilize
  // what would otherwise be padding (on a 64-bit system). We stick the
  // attribute flags, broken state, and the clone count here.
  ...
  // State variable used by V3Broken for consistency checking. The top bit of
  // this is byte is a flag, representing V3Broken is currently proceeding under
  // this node. The bottom 7 bits are a generation number. This is hot when
  // --debug-checks so we access as a whole to avoid bit field masking resulting
  // in unnecessary read-modify-write ops.
  ...
  // Only assert this on known platforms, as it only affects performance, not
  // correctness
  static_assert(sizeof(m_type) + sizeof(m_flags) + sizeof(m_brokenState) +
                        sizeof(m_cloneCnt) <=
                    sizeof(void *),
  ...
  AstNode
      *m_headtailp; // When at begin/end of list, the opposite end of the list
  ...
  AstNode *m_clonep; // Pointer to clone of/ source of this module (for *LAST*
                     // cloneTree() ONLY)
  static int s_cloneCntGbl; // Count of which userp is set

  // This member ordering both allows 64 bit alignment and puts associated data
  // together
...
public:
  ...
  static void debugTreeChange(const AstNode *nodep, const char *prefix,
                              int lineno, bool next);
...
public:
  ...
  AstNode *
  abovep() const; // Parent node above, only when no nextp() as otherwise slow
  ...
  AstNode *clonep() const {
    return ((m_cloneCnt == s_cloneCntGbl) ? m_clonep : nullptr);
  }
  ...
  bool brokeExistsAbove() const {
    return brokeExists() && (m_brokenState >> 7);
  }
  bool brokeExistsBelow() const {
    return brokeExists() && !(m_brokenState >> 7);
  }
  // Note: brokeExistsBelow is not quite precise, as it is true for sibling
  // nodes as well
  ...
  // CONSTANTS
  // The following are relative dynamic costs (~ execution cycle count) of
  // various operations. They are used by V3InstCount to estimate the relative
  // execution time of code fragments.
  static constexpr int INSTR_COUNT_BRANCH = 4; // Branch
  static constexpr int INSTR_COUNT_CALL =
      INSTR_COUNT_BRANCH + 10;                     // Subroutine call
  ...
  static constexpr int INSTR_COUNT_TIME =
      INSTR_COUNT_CALL + 5;                  // Determine simulation time
  ...
  string
  shortName() const; // Name with __PVT__ removed for concatenating scopes
  static string dedotName(const string &namein); // Name with dots removed
  static string
  prettyName(const string &namein); // Name for printing out to the user
  static string
  prettyNameQ(const string &namein) { // Quoted pretty name (for errors)
    return string("'") + prettyName(namein) + "'";
  }
  static string encodeName(
      const string &namein); // Encode user name into internal C representation
  static string
  encodeNumber(vlsint64_t num); // Encode number into internal C representation
  static string
  vcdName(const string &namein); // Name for printing out to vcd files
  /**
   * @brief 获取"修整"过的名字
   */
  ...
  string prettyTypeName()
      const; // "VARREF" for error messages (NOT dtype's pretty name)
  virtual string prettyOperatorName() const {
    return "operator " + prettyTypeName();
  }
  ...
  int widthMin() const;
  int widthMinV() const {
    return v3Global.widthMinUsage() == VWidthMinUsage::VERILOG_WIDTH
               ? widthMin()
               : width();
  }
  ...
  void editCountInc() {
    m_editCount =
        ++s_editCntGbl; // Preincrement, so can "watch AstNode::s_editCntGbl=##"
  }
  ...
  void dtypeSetBitSized(int width, VSigning numeric) {
    dtypep(
        findBitDType(width, width, numeric)); // Since sized, widthMin is width
  }
  ...
  void dtypeSetLogicSized(int width, VSigning numeric) {
    dtypep(findLogicDType(width, width,
                          numeric)); // Since sized, widthMin is width
  }
  ...
  // Data type locators
  AstNodeDType *findBitDType() {
    return findBasicDType(AstBasicDTypeKwd::LOGIC);
  }
  AstNodeDType *findDoubleDType() {
    return findBasicDType(AstBasicDTypeKwd::DOUBLE);
  }
  AstNodeDType *findStringDType() {
    return findBasicDType(AstBasicDTypeKwd::STRING);
  }
  AstNodeDType *findSigned32DType() {
    return findBasicDType(AstBasicDTypeKwd::INTEGER);
  }
  AstNodeDType *findUInt32DType() {
    return findBasicDType(AstBasicDTypeKwd::UINT32);
  }
  AstNodeDType *findUInt64DType() {
    return findBasicDType(AstBasicDTypeKwd::UINT64);
  }
  AstNodeDType *findCHandleDType() {
    return findBasicDType(AstBasicDTypeKwd::CHANDLE);
  }
  ...
  string warnContextSecondary() const {
    return fileline()->warnContextSecondary();
  }
  ...
  void addHereThisAsNext(
      AstNode *newp); // Adds at old place of this, this becomes next
  void replaceWith(AstNode *newp); // Replace current node in tree with new node
  AstNode *unlinkFrBack(AstNRelinker *linkerp =
                            nullptr); // Unlink this from whoever points to it.
  // Unlink this from whoever points to it, keep entire next list with unlinked
  // node
  ...
  AstNode *cloneTree(
      bool cloneNextLink); // Not const, as sets clonep() on original nodep
  ...
  void dumpTreeAndNext(std::ostream &os = std::cout,
                       const string &indent = "    ", int maxDepth = 0) const;
  void dumpTreeFile(const string &filename, bool append = false,
                    bool doDump = true, bool doCheck = true);
  static void dumpTreeFileGdb(const AstNode *nodep,
                              const char *filenamep = nullptr);
  ...
  // GateDedupable is a slightly larger superset of GateOptimzable (eg,
  // AstNodeIf)
  ...
  // Another AstNode* may have a pointer into this node, other then normal
  // front/back/etc.
...
public:
  ...
  inline static T *privateCast(AstNode *nodep) {
    ...
    return nodep && privateTypeTest<T>(nodep) ? reinterpret_cast<T *>(nodep)
                                              : nullptr;
  }
  template <typename T, typename E>
  inline static const T *privateCast(const AstNode *nodep) {
    ...
    static_assert(!impossibleCast<T, E>(),
                  "Unnecessary VN_CAST, node cannot be this type.");
    return nodep && privateTypeTest<T>(nodep)
               ? reinterpret_cast<const T *>(nodep)
               : nullptr;
  }
  ...
  template <typename T, typename E> inline static T *privateAs(AstNode *nodep) {
    ...
    UASSERT_OBJ(!nodep || privateTypeTest<T>(nodep), nodep,
                "AstNode is not of expected type, but instead has type '"
                    << nodep->typeName() << "'");
...
#define ASTNODE_BASE_FUNCS(name)                                               \
  ...
  Ast##name *clonep() const {                                                  \
    return static_cast<Ast##name *>(AstNode::clonep());                        \
  }

class AstNodeMath VL_NOT_FINAL : public AstNode {
  // Math -- anything that's part of an expression tree
protected:
  AstNodeMath(AstType t, FileLine *fl) : AstNode{t, fl} {}
...
public:
  ...
  virtual string
  emitVerilog() = 0; /// Format string for verilog writing; see V3EmitV
...
protected:
  AstNodeTermop(AstType t, FileLine *fl) : AstNodeMath{t, fl} {}
...
class AstNodeUniop VL_NOT_FINAL : public AstNodeMath {
  // Unary math
protected:
  AstNodeUniop(AstType t, FileLine *fl, AstNode *lhsp) : AstNodeMath{t, fl} {
    dtypeFrom(lhsp);
    setOp1p(lhsp);
  }
...
public:
  ...
  virtual bool
  sizeMattersLhs() const = 0; // True if output result depends on lhs size
  virtual bool doubleFlavor() const {
    return false;
  } // D flavor of nodes with both flavors?
  // Signed flavor of nodes with both flavors?
  virtual bool signedFlavor() const { return false; }
  virtual bool stringFlavor() const {
    return false;
  } // N flavor of nodes with both flavors?
...
public:
  ...
  virtual void numberOperate(V3Number &out, const V3Number &lhs,
                             const V3Number &rhs) = 0;
  virtual bool
  cleanLhs() const = 0; // True if LHS must have extra upper bits zero
  virtual bool
  cleanRhs() const = 0; // True if RHS must have extra upper bits zero
  virtual bool
  sizeMattersLhs() const = 0; // True if output result depends on lhs size
  virtual bool
  sizeMattersRhs() const = 0; // True if output result depends on rhs size
  virtual bool doubleFlavor() const {
    return false;
  } // D flavor of nodes with both flavors?
  // Signed flavor of nodes with both flavors?
...
class AstNodeTriop VL_NOT_FINAL : public AstNodeMath {
  // Trinary math
protected:
  AstNodeTriop(AstType t, FileLine *fl, AstNode *lhs, AstNode *rhs,
               AstNode *ths)
      : AstNodeMath{t, fl} {
    setOp1p(lhs);
    setOp2p(rhs);
...
public:
  ...
  cleanRhs() const = 0; // True if RHS must have extra upper bits zero
  virtual bool
  cleanThs() const = 0; // True if THS must have extra upper bits zero
  virtual bool
  sizeMattersLhs() const = 0; // True if output result depends on lhs size
  virtual bool
  sizeMattersRhs() const = 0; // True if output result depends on rhs size
  virtual bool
  sizeMattersThs() const = 0; // True if output result depends on ths size
  virtual int instrCount() const override { return widthInstrs(); }
  virtual bool same(const AstNode *) const override { return true; }
};
...
class AstNodeQuadop VL_NOT_FINAL : public AstNodeMath {
  // Quaternary math
protected:
  AstNodeQuadop(AstType t, FileLine *fl, AstNode *lhs, AstNode *rhs,
                AstNode *ths, AstNode *fhs)
      : AstNodeMath{t, fl} {
    setOp1p(lhs);
    setOp2p(rhs);
    setOp3p(ths);
...
public:
  ...
  cleanThs() const = 0; // True if THS must have extra upper bits zero
  virtual bool
  cleanFhs() const = 0; // True if THS must have extra upper bits zero
  virtual bool
  sizeMattersLhs() const = 0; // True if output result depends on lhs size
  virtual bool
  sizeMattersRhs() const = 0; // True if output result depends on rhs size
  ...
  sizeMattersThs() const = 0; // True if output result depends on ths size
  virtual bool
  sizeMattersFhs() const = 0; // True if output result depends on ths size
  virtual int instrCount() const override { return widthInstrs(); }
  virtual bool same(const AstNode *) const override { return true; }
};
...
protected:
  AstNodeCond(AstType t, FileLine *fl, AstNode *condp, AstNode *expr1p,
              AstNode *expr2p)
...
public:
  ...
  virtual void numberOperate(V3Number &out, const V3Number &lhs,
                             const V3Number &rhs, const V3Number &ths) override;
  ...
  virtual bool cleanOut() const override {
    return false;
  } // clean if e1 & e2 clean
  ...
  virtual AstNode *cloneType(AstNode *condp, AstNode *expr1p,
                             AstNode *expr2p) = 0;
};
...
protected:
  AstNodePreSel(AstType t, FileLine *fl, AstNode *fromp, AstNode *rhs,
                AstNode *ths)
      : AstNode{t, fl} {
    setOp1p(fromp);
    setOp2p(rhs);
...
protected:
  AstNodeProcedure(AstType t, FileLine *fl, AstNode *bodysp) : AstNode{t, fl} {
    addNOp2p(bodysp);
  }
...
protected:
  AstNodeStmt(AstType t, FileLine *fl, bool statement = true)
      : AstNode{t, fl}, m_statement{statement} {}
...
public:
  ...
  // Clone single node, just get same type back.
  virtual AstNode *cloneType(AstNode *lhsp, AstNode *rhsp) = 0;
  // So iteration hits the RHS which is "earlier" in execution order, it's op1,
  // not op2
...
private:
  ...
  bool m_isBoundsCheck; // True if this if node was inserted for array bounds
                        // checking
protected:
  AstNodeIf(AstType t, FileLine *fl, AstNode *condp, AstNode *ifsp,
            AstNode *elsesp)
      : AstNodeStmt{t, fl} {
    setOp1p(condp);
    addNOp2p(ifsp);
...
public:
  ...
  AstNode *notParallelp() const {
    return op3p();
  } // op3 = assertion code for non-full case's
...
protected:
  AstNodeVarRef(AstType t, FileLine *fl, const string &name,
                const VAccess &access)
      : AstNodeMath{t, fl}, m_access{access}, m_name{name} {
    this->varp(nullptr);
  }
  AstNodeVarRef(AstType t, FileLine *fl, const string &name, AstVar *varp,
                const VAccess &access)
      : AstNodeMath{t, fl}, m_access{access}, m_name{name} {
...
public:
  ...
  void access(const VAccess &flag) {
    m_access = flag;
  } // Avoid using this; Set in constructor
...
protected:
  ...
  AstNodeText(AstType t, FileLine *fl, const string &textp) : AstNode{t, fl} {
...
private:
  ...
  int m_widthMin; // (also in AstTypeTable::Key) If unsized, bitwidth of minimum
                  // implementation
...
protected:
  // CONSTRUCTORS
  AstNodeDType(AstType t, FileLine *fl) : AstNode{t, fl} {
    m_width = 0;
    m_widthMin = 0;
    m_generic = false;
  }
...
public:
  ...
  // For backward compatibility inherit width and signing from the subDType/base
  // type
  ...
  int widthWords() const { return VL_WORDS_I(width()); }
  int widthMin()
      const { // If sized, the size, if unsized the min digits to represent it
  ...
  bool isLiteralType()
      const; // Does this represent a C++ LiteralType? (can be constexpr)
...
protected:
  AstNodeUOrStructDType(AstType t, FileLine *fl, VSigning numericUnpack)
      : AstNodeDType{t, fl}, m_uniqueNum{uniqueNumInc()} {
...
public:
  ...
  virtual bool isCompound() const override {
    return false;
  } // Because don't support unpacked
  // For basicp() we reuse the size to indicate a "fake" basic type of same size
  virtual AstBasicDType *basicp() const override {
    return (isFourstate() ? VN_AS(findLogicRangeDType(VNumRange{width() - 1, 0},
                                                      width(), numeric()),
                                  BasicDType)
                          : VN_AS(findBitRangeDType(VNumRange{width() - 1, 0},
                                                    width(), numeric()),
                                  BasicDType));
  }
  virtual AstNodeDType *skipRefp() const override {
    return (AstNodeDType *)this;
  }
...
protected:
  AstNodeArrayDType(AstType t, FileLine *fl) : AstNodeDType{t, fl} {}
...
public:
  ...
  virtual const char *broken() const override {
    BROKEN_RTN(
        !((m_refDTypep && !childDTypep() && m_refDTypep->brokeExists()) ||
          (!m_refDTypep && childDTypep())));
    return nullptr;
  }
  virtual void cloneRelink() override {
    if (m_refDTypep && m_refDTypep->clonep())
      m_refDTypep = m_refDTypep->clonep();
  }
  virtual bool same(const AstNode *samep) const override {
    const AstNodeArrayDType *asamep =
        static_cast<const AstNodeArrayDType *>(samep);
  ...
  virtual bool similarDType(AstNodeDType *samep) const override {
    const AstNodeArrayDType *asamep =
        static_cast<const AstNodeArrayDType *>(samep);
    return (
        asamep && type() == samep->type() && hi() == asamep->hi() &&
        rangenp()->sameTree(asamep->rangenp()) &&
        subDTypep()->skipRefp()->similarDType(asamep->subDTypep()->skipRefp()));
  }
  virtual AstNodeDType *getChildDTypep() const override {
    return childDTypep();
  }
  ...
  AstRange *rangep() const {
    return VN_AS(op2p(), Range);
  } // op2 = Array(s) of variable
  ...
  virtual int widthAlignBytes() const override {
    return subDTypep()->widthAlignBytes();
  }
  virtual int widthTotalBytes() const override {
...
class AstNodeSel VL_NOT_FINAL : public AstNodeBiop {
  // Single bit range extraction, perhaps with non-constant selection or array
  // selection
protected:
...
protected:
  AstNodeStream(AstType t, FileLine *fl, AstNode *lhsp, AstNode *rhsp)
    ...
    if (lhsp->dtypep())
      dtypeSetLogicSized(lhsp->dtypep()->width(), VSigning::UNSIGNED);
  }
...
class AstNodeCCall VL_NOT_FINAL : public AstNodeStmt {
  ...
  // Functions are not statements, while tasks are. AstNodeStmt needs
  // isStatement() to deal.
  AstCFunc *m_funcp;
  string m_argTypes;

protected:
  AstNodeCCall(AstType t, FileLine *fl, AstCFunc *funcp,
               AstNode *argsp = nullptr)
      : AstNodeStmt{t, fl, true}, m_funcp{funcp} {
    addNOp2p(argsp);
  }
...
private:
  ...
  uint64_t m_dpiOpenParent =
      0;                 // DPI import open array, if !=0, how many callees
...
protected:
  AstNodeFTask(AstType t, FileLine *fl, const string &name, AstNode *stmtsp)
      : AstNode{t, fl}, m_name{name}, m_taskPublic{false},
        m_attrIsolateAssign{false}, m_classMethod{false}, m_externProto{false},
        m_externDef{false}, m_prototype{false}, m_dpiExport{false},
        m_dpiImport{false}, m_dpiContext{false},
        m_dpiOpenChild{false}, m_dpiTask{false}, m_isConstructor{false},
        m_isHideLocal{false}, m_isHideProtected{false}, m_pure{false},
        m_pureVirtual{false}, m_underGenerate{false}, m_virtual{false} {
    addNOp3p(stmtsp);
...
protected:
  AstNodeFTaskRef(AstType t, FileLine *fl, bool statement, AstNode *namep,
                  AstNode *pinsp)
      : AstNodeStmt{t, fl, statement} {
    setOp1p(namep);
    addNOp3p(pinsp);
  }
  AstNodeFTaskRef(AstType t, FileLine *fl, bool statement, const string &name,
                  AstNode *pinsp)
      : AstNodeStmt{t, fl, statement}, m_name{name} {
    addNOp3p(pinsp);
  }
...
private:
  ...
  string
      m_origName; // Name of the module, ignoring name() changes, for dot lookup
  ...
  bool m_recursiveClone : 1; // If recursive, what module it clones, otherwise
                             // nullptr
...
protected:
  AstNodeModule(AstType t, FileLine *fl, const string &name)
      : AstNode{t, fl}, m_name{name}, m_origName{name}, m_modPublic{false},
        m_modTrace{false}, m_inLibrary{false}, m_dead{false},
        m_hierBlock{false}, m_internal{false}, m_recursive{false},
...
public:
  ...
  AstActive *activesp() const {
    return VN_AS(op3p(), Active);
  } // op3 = List of i/sblocks
  // METHODS
...
protected:
  AstNodeRange(AstType t, FileLine *fl) : AstNode{t, fl} {}
...
inline void AstNVisitor::iterateAndNextConstNull(AstNode *nodep) {
  if (VL_LIKELY(nodep))
    nodep->iterateAndNextConst(*this);
}
