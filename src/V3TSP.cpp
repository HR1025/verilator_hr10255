...
public:
  ...
  void makeMinSpanningTree(TspGraphTmpl *mstp) {
    ...
    for (V3GraphVertex *vxp = verticesBeginp(); vxp;
    ...
    for (V3GraphEdge *edgep = start_vertexp->outBeginp(); edgep;
    ...
    while (!pendingEdges.empty()) {
      ...
      UASSERT(visited_set.find(from_vertexp) != visited_set.end(),
              "Can't find vertex");
      ...
      if (visited_set.find(neighborp) == visited_set.end()) {
        ...
        UINFO(6, "bestCost = " << bestCost << "  from " << from_vertexp->key()
  ...
  void perfectMatching(const std::vector<T_Key> &oddKeys, TspGraphTmpl *outp) {
    ...
    for (VertexListIt it = odds.begin(); it != odds.end(); ++it) {
      for (V3GraphEdge *edgep = (*it)->outBeginp(); edgep;
           edgep = edgep->outNextp()) {
        pendingEdges.insert(edgep);
      }
    }
    ...
    UASSERT(unmatchedOdds.empty(),
            "Algorithm should have processed all vertices");
  }

  void combineGraph(const TspGraphTmpl &g) {
    std::unordered_set<vluint32_t> edges_done;
    for (V3GraphVertex *vxp = g.verticesBeginp(); vxp;
         vxp = vxp->verticesNextp()) {
      Vertex *fromp = castVertexp(vxp);
      for (V3GraphEdge *edgep = fromp->outBeginp(); edgep;
           edgep = edgep->outNextp()) {
  ...
  void findEulerTourRecurse(std::unordered_set<unsigned> *markedEdgesp,
                            Vertex *startp, std::vector<T_Key> *sortedOutp) {
    ...
    do {
      ...
      for (V3GraphEdge *edgep = cur_vertexp->outBeginp(); edgep;
           edgep = edgep->outNextp()) {
  ...
  void dumpGraph(std::ostream &os, const string &nameComment) const {
    ...
    for (V3GraphVertex *vxp = verticesBeginp(); vxp;
      ...
      os << " " << tspvp->key() << '\n';
      for (V3GraphEdge *edgep = tspvp->outBeginp(); edgep;
           edgep = edgep->outNextp()) {
        Vertex *neighborp = castVertexp(edgep->top());
  ...
  void findEulerTour(std::vector<T_Key> *sortedOutp) {
    ...
    if (debug() >= 6)
      dumpDotFilePrefixed("findEulerTour");
...
void V3TSP::tspSort(const V3TSP::StateVec &states, V3TSP::StateVec *resultp) {
  ...
  for (V3TSP::StateVec::const_iterator it = states.begin(); it != states.end();
       ++it) {
...
public:
  TspTestState(unsigned xpos, unsigned ypos)
      : m_xpos{xpos}, m_ypos{ypos}, m_serial{++s_serialNext} {}
...
void V3TSP::selfTestStates() {
  // Linear test -- coords all along the x-axis
  {
    ...
    if (VL_UNCOVERABLE(expect != result)) {
      for (V3TSP::StateVec::iterator it = result.begin(); it != result.end();
           ++it) {
        const TspTestState *statep = dynamic_cast<const TspTestState *>(*it);
        cout << statep->xpos() << " ";
      }
      cout << endl;
      v3fatalSrc("TSP linear self-test fail. Result (above) did not match "
                 "expectation.");
    }
  }
  ...
  // Test that tspSort() will rotate the list for minimum cost.
  {
    ...
    if (VL_UNCOVERABLE(expect != result)) {
      ...
      cout << endl;
      v3fatalSrc("TSP 2d cycle=false self-test fail. Result (above) did not "
                 "match expectation.");
    }
  }
}
...
void V3TSP::selfTestString() {
  ...
  if (VL_UNCOVERABLE(expect != result)) {
    ...
    cout << endl;
    v3fatalSrc(
        "TSP string self-test fail. Result (above) did not match expectation.");
  }
}
