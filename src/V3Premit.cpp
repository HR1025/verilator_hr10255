...
constexpr int STATIC_CONST_MIN_WIDTH =
    256; // Minimum size to extract to static constant
...
private:
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    if (nodep->access().isWriteOrRW()) {
      ...
      if (nodep->varp()->user3()) {
        if (!m_noopt)
          UINFO(4, "Block has LHS+RHS var: " << nodep << endl);
        m_noopt = true;
      }
    }
  }
...
private:
  ...
  AstWhile *m_inWhilep =
      nullptr; // Inside while loop, special statement additions
  AstTraceInc *m_inTracep =
      nullptr;               // Inside while loop, special statement additions
  ...
  void checkNode(AstNode *nodep) {
    ...
    // UINFO(9, "     Detail stmtp=" << (m_stmtp?"Y":"N") << " U=" <<
    // (nodep->user1()?"Y":"N")
  ...
  void createDeepTemp(AstNode *nodep, bool noSubst) {
    if (nodep->user1SetOnce())
      return; // Only add another assignment for this node
    ...
    const bool useConstPool =
        constp                                         // Is a constant
    ...
    if (useConstPool) {
      ...
      // Keep as local temporary. Name based on hash of node for output
      // stability.
      varp = new AstVar(fl, AstVarType::STMTTEMP, m_tempNames.get(nodep),
                        nodep->dtypep());
      m_cfuncp->addInitsp(varp);
      // Put assignment before the referencing statement
      insertBeforeStmt(
          new AstAssign(fl, new AstVarRef(fl, varp, VAccess::WRITE), nodep));
    }
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    startStatement(nodep);
    {
      ...
      if (noopt && !nodep->user1()) {
        nodep->user1(true);
        // Need to do this even if not wide, as e.g. a select may be on a wide
        // operator
    ...
    if (!nodep->user2SetOnce()) {
      ...
      if (shiftp && shiftp->num().mostSetBitP1() > 32) {
        shiftp->v3error(
            ...
            << " (This isn't a shift of 32 bits, but a shift of 2^32, or 4 "
               "billion!)\n");
      }
      if (nodep->widthMin() <= 64 // Else we'll use large operators which work
                                  // right C operator's width must be < maximum
                                  // shift which is based on Verilog width
          && nodep->width() < (1LL << nodep->rhsp()->widthMin())) {
        AstNRelinker replaceHandle;
        nodep->unlinkFrBack(&replaceHandle);
        AstNode *constzerop;
        int m1value = nodep->widthMin() -
                      1; // Constant of width-1; not changing dtype width
        if (nodep->signedFlavor()) {
          ...
          constzerop = new AstNegate(
              ...
              new AstShiftR(nodep->fileline(), nodep->lhsp()->cloneTree(false),
                            new AstConst(nodep->fileline(), m1value),
                            nodep->width()));
        } else {
        ...
        AstCond *newp = new AstCond(nodep->fileline(),
  ...
  virtual void visit(AstDisplay *nodep) override {
    ...
    if (v3Global.opt.autoflush()) {
      ...
      while (searchp && VN_IS(searchp, Comment))
      ...
      if (searchp && VN_IS(searchp, Display) &&
        ...
        nodep->addNextHere(new AstFFlush(
            nodep->fileline(), AstNode::cloneTreeNull(nodep->filep(), true)));
      }
    }
  }
...
public:
  // CONSTRUCTORS
  explicit PremitVisitor(AstNetlist *nodep) : m_tempNames{"__Vtemp"} {
    iterate(nodep);
  }
...
void V3Premit::premitAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("premit", 0,
