...
protected:
  ...
  V3LangCode numberToLang(int filenameno) const {
    return m_languages[filenameno];
  }
...
public:
  ...
  // METHODS
  void
  pushText(const string &text); // Add arbitrary text (need not be line-by-line)
...
class FileLine final {
  // CONSTANTS
  static constexpr unsigned SHOW_SOURCE_MAX_LENGTH =
      400; // Don't show source lines > this long
...
public:
  ...
  explicit FileLine(FileLine *fromp)
      : m_firstLineno{fromp->m_firstLineno},
        m_firstColumn{fromp->m_firstColumn}, m_lastLineno{fromp->m_lastLineno},
        m_lastColumn{fromp->m_lastColumn}, m_filenameno{fromp->m_filenameno},
        m_contentLineno{fromp->m_contentLineno},
        m_contentp{fromp->m_contentp}, m_parent{fromp->m_parent},
        m_warnOn{fromp->m_warnOn}, m_waive{fromp->m_waive} {}
  ...
  void language(V3LangCode lang) {
    singleton().numberToLang(filenameno(), lang);
  }
  void filename(const string &name) {
    m_filenameno = singleton().nameToNumber(name);
  }
  ...
  // as the parser errors etc generally make more sense pointing at the last
  // parse point
  ...
  bool filenameIsGlobal() const {
    return (filename() == commandLineFilename() ||
  ...
  // Returns -1 if (*this) should come before rhs after sorted. 1 for the
  // opposite case. 0 for equivalent.
  int operatorCompare(const FileLine &rhs) const {
    if (m_filenameno != rhs.m_filenameno)
      return (m_filenameno < rhs.m_filenameno) ? -1 : 1;
    ...
    if (m_lastLineno != rhs.m_lastLineno)
      return (m_lastLineno < rhs.m_lastLineno) ? -1 : 1;
    if (m_lastColumn != rhs.m_lastColumn)
      return (m_lastColumn < rhs.m_lastColumn) ? -1 : 1;
    for (size_t i = 0; i < m_warnOn.size(); ++i) {
      if (m_warnOn[i] != rhs.m_warnOn[i])
        return (m_warnOn[i] < rhs.m_warnOn[i]) ? -1 : 1;
    }
