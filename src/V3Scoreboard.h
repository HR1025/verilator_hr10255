...
template <typename T_Key, typename T_Value,
          class T_KeyCompare = std::less<T_Key>>
...
private:
  ...
  std::unordered_map<T_Key, T_Value>
      m_keys;      // Map each key to its value. Not sorted.
...
public:
  ...
  protected:
    ...
    const_iterator(typename Val2Keys::iterator valIt,
                   typename KeySet::iterator keyIt, SortByValueMap *sbvmp)
        : m_keyIt{keyIt}, m_valIt{valIt}, m_sbvmp{sbvmp}, m_end{false} {}
    ...
    void advanceUntilValid() {
      ...
      UASSERT(m_keyIt != m_valIt->second.end(),
              "Algorithm should have key left");
    }
  ...
  public:
    ...
    // CONSTRUCTORS
    explicit iterator(SortByValueMap *sbvmp) : const_iterator{sbvmp} {}
...
public:
  ...
  explicit V3Scoreboard(UserScoreFnp scoreFnp, bool slowAsserts)
      : m_scoreFnp{scoreFnp}, m_slowAsserts{slowAsserts} {}
  ...
  bool contains(const T_Elem *elp) const {
    if (m_unknown.find(elp) != m_unknown.end())
      return true;
  ...
  const T_Elem *bestp() {
    ...
    if (VL_UNLIKELY(result == m_sorted.end()))
      return nullptr;
    return (*result).key();
  }
