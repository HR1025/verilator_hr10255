...
private:
  ...
  virtual void visit(AstPin *nodep) override {
    ...
    if (!nodep->user1SetOnce()) {
      ...
      UASSERT_OBJ(
          exprp->width() == nodep->modVarp()->width(), nodep,
      ...
      if (nodep->modVarp()->isInoutish()) {
        ...
        m_cellp->addNextHere(assp);
        if (debug() >= 9)
          assp->dumpTree(cout, "     _new: ");
      } else if (nodep->modVarp()->isIfaceRef() ||
                 (VN_IS(nodep->modVarp()->subDTypep(), UnpackArrayDType) &&
                  VN_IS(VN_AS(nodep->modVarp()->subDTypep(), UnpackArrayDType)
                            ->subDTypep(),
                        IfaceRefDType))) {
...
private:
  // STATE
  std::map<const std::string, AstVar *>
      m_modVarNameMap; // Per module, name of cloned variables
...
private:
  ...
  virtual void visit(AstVar *nodep) override {
    if (VN_IS(nodep->dtypep(), UnpackArrayDType) &&
        VN_IS(VN_AS(nodep->dtypep(), UnpackArrayDType)->subDTypep(),
              IfaceRefDType)) {
      ...
      for (int i = arrdtype->lo(); i <= arrdtype->hi(); ++i) {
        const string varNewName =
            nodep->name() + "__BRA__" + cvtToStr(i) + "__KET__";
        ...
        if (!m_deModVars.find(varNewName)) {
          ...
          varNewp->origName(varNewp->origName() + "__BRA__" + cvtToStr(i) +
                            "__KET__");
  ...
  virtual void visit(AstCell *nodep) override {
    ...
    if (nodep->rangep()) {
      ...
      for (int i = 0; i < m_cellRangep->elementsConst(); i++) {
        m_instSelNum = m_cellRangep->littleEndian()
                           ? (m_cellRangep->elementsConst() - 1 - i)
                           : i;
        ...
        // Somewhat illogically, we need to rename the original name of the cell
        // too. as that is the name users expect for dotting The spec says we
        // add [x], but that won't work in C...
        newp->name(newp->name() + "__BRA__" + cvtToStr(instNum) + "__KET__");
        newp->origName(newp->origName() + "__BRA__" + cvtToStr(instNum) +
                       "__KET__");
        UINFO(8, "    CELL loop  " << newp << endl);

        // If this AstCell is actually an interface instantiation, also clone
        // the IfaceRef within the same parent module as the cell
        if (isIface) {
          AstUnpackArrayDType *arrdtype =
          ...
          varNewp->name(varNewp->name() + "__BRA__" + cvtToStr(instNum) +
                        "__KET__");
          varNewp->origName(varNewp->origName() + "__BRA__" +
                            cvtToStr(instNum) + "__KET__");
          varNewp->dtypep(ifaceRefp);
  ...
  virtual void visit(AstPin *nodep) override {
    ...
    if (m_cellRangep) {
      ...
      std::pair<uint32_t, uint32_t> pinDim =
          nodep->modVarp()->dtypep()->dimensions(false);
      std::pair<uint32_t, uint32_t> expDim =
      ...
      if (expDim.first == pinDim.first && expDim.second == pinDim.second + 1) {
        // Connection to array, where array dimensions match the instant
        // dimension
        AstRange *rangep =
            VN_AS(nodep->exprp()->dtypep(), UnpackArrayDType)->rangep();
        const int arraySelNum =
            rangep->littleEndian()
        ...
        if (m_cellRangep->littleEndian()) {
          nodep->exprp()->v3warn(LITENDIAN,
                                 "Little endian instance range connecting to "
        ...
        if (!inputPin &&
            ...
            && !VN_IS(exprp, Concat) && !VN_IS(exprp, Replicate) &&
            !VN_IS(exprp, Sel)) {
        ...
        exprp = new AstSel(exprp->fileline(), exprp, pinwidth * m_instSelNum,
                           pinwidth);
        nodep->exprp(exprp);
      } else {
      ...
      if (AstUnpackArrayDType *arrp =
        ...
        if (!constp) {
          nodep->v3warn(E_UNSUPPORTED, "Unsupported: Non-constant index when "
                                       "passing interface to module");
          return;
        }
      ...
      // Clone the var referenced by the pin, and clone each var referenced by
      // the varref Clone pin varp:
      for (int in = 0; in < pinArrp->elementsConst(); ++in) { // 0 = leftmost
        int i = pinArrp->left() + in * pinArrp->declRange().leftToRightInc();
        const string varNewName =
            pinVarp->name() + "__BRA__" + cvtToStr(i) + "__KET__";
        ...
        if (!pinVarp->backp()) {
          ...
          AstIfaceRefDType *ifaceRefp =
              VN_AS(pinArrp->subDTypep(), IfaceRefDType);
          ifaceRefp->cellp(nullptr);
        ...
        if (!varNewp) {
          if (debug() >= 9)
            m_deModVars.dump(); // LCOV_EXCL_LINE
        ...
        if (AstSliceSel *slicep = VN_CAST(newp->exprp(), SliceSel)) {
          ...
          int slice_index = slicep->declRange().left() +
                            in * slicep->declRange().leftToRightInc();
        ...
        } else if (auto *exprArrp =
                       VN_CAST(varrefp->dtypep(), UnpackArrayDType)) {
          expr_i =
              exprArrp->left() + in * exprArrp->declRange().leftToRightInc();
        }

        const string newname =
            varrefp->name() + "__BRA__" + cvtToStr(expr_i) + "__KET__";
...
private:
  ...
  static AstNode *extendOrSel(FileLine *fl, AstNode *rhsp, AstNode *cmpWidthp) {
    if (cmpWidthp->width() > rhsp->width()) {
      rhsp =
          (rhsp->isSigned() ? static_cast<AstNode *>(new AstExtendS{fl, rhsp})
  ...
  static AstAssignW *pinReconnectSimple(AstPin *pinp, AstCell *cellp,
    ...
    if (!pinp->exprp()) {
      ...
      if (pinVarp->direction() == VDirection::INPUT &&
        ...
        pinp->exprp(
            new AstConst(pinp->fileline(), AstConst::StringToParse(), "'1"));
      } else if (pinVarp->direction() == VDirection::INPUT &&
                 cellp->modp()->unconnectedDrive().isSetFalse()) {
        pinp->exprp(
            new AstConst(pinp->fileline(), AstConst::StringToParse(), "'0"));
      } else {
        return nullptr;
      }
    }
    ...
    if (!alwaysCvt && connectRefp &&
      ...
      string newvarname =
          (string(pinVarp->isWritable() ? "__Vcellout" : "__Vcellinp")
           ...
           + (forTristate ? "t" : "") + "__" + cellp->name() + "__" +
           pinp->name());
      AstVar *newvarp = new AstVar(pinVarp->fileline(), AstVarType::MODULETEMP,
                                   newvarname, pinVarp);
      ...
      if (pinVarp->isInoutish()) {
        pinVarp->v3fatalSrc(
            "Unsupported: Inout connections to pins must be"
        ...
        pinp->exprp(
            new AstVarRef(newvarp->fileline(), newvarp, VAccess::WRITE));
        ...
        pinp->exprp(
            new AstVarRef(pinexprp->fileline(), newvarp, VAccess::READ));
      }
      if (assignp)
        cellp->addNextHere(assignp);
...
void V3Inst::instAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("inst", 0,
...
void V3Inst::dearrayAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("dearray", 0,
