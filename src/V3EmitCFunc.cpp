...
bool EmitCFunc::emitSimpleOk(AstNodeMath *nodep) {
  ...
  if (nodep->op2p()) {
    if (nodep->op2p()->isWide())
      return false;
  }
  if (nodep->op3p()) {
    if (nodep->op3p()->isWide())
      return false;
  }
  return true;
}

void EmitCFunc::emitOpName(AstNode *nodep, const string &format, AstNode *lhsp,
...
#define COMMA                                                                  \
  ...
  putbs("");
  for (string::const_iterator pos = format.begin(); pos != format.end();
       ++pos) {
    if (pos[0] == ',') {
      ...
      if (pos[1] == ' ')
        ++pos; // Must do even if no nextComma
    } else if (pos[0] == '%') {
      ++pos;
      bool detail = false;
      ...
      if (detail) {
        ...
        case 'i':
          ...
          break;
        default:
          nodep->v3fatalSrc("Unknown emitOperator format code: %[nlrt]"
                            << pos[0]);
          break;
        }
      }
...
void EmitCFunc::displayArg(AstNode *dispp, AstNode **elistp, bool isScan,
  ...
  if ((fmtLetter == '#' || fmtLetter == 'd') && !isScan &&
    ...
    const double mantissabits =
        ignore ? 0 : (argp->widthMin() - ((fmtLetter == 'd') ? 1 : 0));
  ...
  if (!ignore) {
    ...
    if (fmtLetter == 't' || fmtLetter == '^') {
      ...
      emitDispState.pushArg(' ', nullptr,
                            cvtToStr((int)fmtp->timeunit().powerOfTen()));
    }
  } else {
...
void EmitCFunc::displayNode(AstNode *nodep, AstScopeName *scopenamep,
  ...
  for (; pos != vformat.end(); ++pos) {
    // UINFO(1, "Parse '" << *pos << "'  IP" << inPct << " List " <<
    // cvtToHex(elistp) << endl);
    if (!inPct && pos[0] == '%') {
      inPct = true;
      ignore = false;
      vfmt = "";
      ...
      case '%':
        emitDispState.pushFormat(
            "%%"); // We're printf'ing it, so need to quote the %
        break;
      case '*':
        vfmt += pos[0];
        inPct = true; // Get more digits
      ...
      // Spec: h d o b c l
      case 'b':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'b');
        break;
      case 'c':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'c');
        break;
      case 't':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 't');
        break;
      case 'd':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, '#');
        break; // Unsigned decimal
      case 'o':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'o');
        break;
      case 'h': // FALLTHRU
      case 'x':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'x');
        break;
      case 's':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 's');
        break;
      case 'e':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'e');
        break;
      case 'f':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'f');
        break;
      case 'g':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'g');
        break;
      case '^':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, '^');
        break; // Realtime
      case 'v':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'v');
        break;
      case 'u':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'u');
        break;
      case 'z':
        displayArg(nodep, &elistp, isScan, vfmt, ignore, 'z');
        break;
      case 'm': {
  ...
  if (VL_UNCOVERABLE(elistp)) {
    // expectFormat also checks this, and should have found it first, so
    // internal
    elistp->v3error(
        "Internal: Extra arguments for $display-like format"); // LCOV_EXCL_LINE
  }
  displayEmit(nodep, isScan);
}

void EmitCFunc::emitCCallArgs(const AstNodeCCall *nodep,
                              const string &selfPointer) {
  puts("(");
  bool comma = false;
  if (nodep->funcp()->isLoose() && !nodep->funcp()->isStatic()) {
    UASSERT_OBJ(!selfPointer.empty(), nodep,
...
void EmitCFunc::emitConstant(AstConst *nodep, AstVarRef *assigntop,
                             const string &assignString) {
  // Put out constant set to the specified variable, or given variable in a
  // string
  if (nodep->num().isFourState()) {
    ...
    { // Upper e.g. 8 words
      ...
      for (int word = VL_WORDS_I(upWidth) - 1; word >= 0; word--) {
        // Only 32 bits - llx + long long here just to appease CPP format
        // warning
    ...
    if (int(nodep->num().toDouble()) == nodep->num().toDouble() &&
      ...
      // Not %g as will not always put in decimal point, so not obvious to
      // compiler is a real number
...
void EmitCFunc::emitSetVarConstant(const string &assignString,
                                   AstConst *constp) {
  if (!constp->isWide()) {
    puts(assignString);
    puts(" = ");
...
void EmitCFunc::emitVarReset(AstVar *varp) {
  ...
  const string varNameProtected = VN_IS(m_modp, Class)
                                      ? varp->nameProtect()
                                      : "vlSelf->" + varp->nameProtect();
  if (varp->isIO() && m_modp->isTop() && optSystemC()) {
    // System C top I/O doesn't need loading, as the lower level subinst code
    // does it.}
  } else if (varp->isParam()) {
    UASSERT_OBJ(varp->valuep(), varp, "No init for a param?");
    // If a simple CONST value we initialize it using an enum
    // If an ARRAYINIT we initialize it using an initial block similar to a
    // signal puts("// parameter "+varp->nameProtect()+" =
    // "+varp->valuep()->name()+"\n");
  ...
  if (AstAssocArrayDType *adtypep = VN_CAST(dtypep, AssocArrayDType)) {
    ...
    if (dtypep->isWide()) { // Handle unpacked; not basicp->isWide
      ...
      if (varp->valuep()) {
        ...
        if (!constp)
          varp->v3fatalSrc("non-const initializer for variable");
...
void EmitCFunc::doubleOrDetect(AstChangeDet *changep, bool &gotOne) {
  ...
  if (!changep->rhsp()) {
    ...
    UASSERT_OBJ(VN_IS(lhsp, VarRef) || VN_IS(lhsp, ArraySel), changep,
                "Not ref?");
    UASSERT_OBJ(VN_IS(rhsp, VarRef) || VN_IS(rhsp, ArraySel), changep,
                "Not ref?");
    for (int word = 0;
