...
private:
  // NODE STATE
  // Entire Netlist:
  //  AstNodeModule::user1()  -> int. Count of number of cells referencing this
  //  module. AstVar::user1()         -> int. Count of number of references
  ...
  void checkVarRef(AstNodeVarRef *nodep) {
    if (nodep->classOrPackagep() && m_elimCells)
  ...
  virtual void visit(AstNodeModule *nodep) override {
    if (m_modp)
      m_modp->user1Inc(); // e.g. Class under Package
    VL_RESTORER(m_modp);
    {
      m_modp = nodep;
      if (!nodep->dead()) {
        ...
        if (AstClass *classp = VN_CAST(nodep, Class)) {
          if (classp->extendsp())
            classp->extendsp()->user1Inc();
          if (classp->classOrPackagep())
            classp->classOrPackagep()->user1Inc();
          m_classesp.push_back(classp);
          // TODO we don't reclaim dead classes yet - graph implementation
          // instead?
          classp->user1Inc();
        }
      }
  ...
  virtual void visit(AstCFunc *nodep) override {
    ...
    checkAll(nodep);
    if (nodep->scopep())
      nodep->scopep()->user1Inc();
  }
  virtual void visit(AstScope *nodep) override {
    iterateChildren(nodep);
    checkAll(nodep);
    if (nodep->aboveScopep())
      nodep->aboveScopep()->user1Inc();
    // Class packages might have no children, but need to remain as
    // long as the class they refer to is needed
    if (VN_IS(m_modp, Class) || VN_IS(m_modp, ClassPackage))
      nodep->user1Inc();
  ...
  virtual void visit(AstMemberSel *nodep) override {
    ...
    if (nodep->fromp()->dtypep())
      nodep->fromp()->dtypep()->user1Inc(); // classref
    checkAll(nodep);
  }
  ...
  virtual void visit(AstTypedef *nodep) override {
    ...
    if (nodep->attrPublic() && m_modp && VN_IS(m_modp, Package))
      m_modp->user1Inc();
  }
  ...
  virtual void visit(AstVar *nodep) override {
    iterateChildren(nodep);
    checkAll(nodep);
    if (nodep->isSigPublic() && m_modp && VN_IS(m_modp, Package))
      m_modp->user1Inc();
    ...
    for (bool retry = true; retry;) {
      ...
      for (AstNodeModule *modp = v3Global.rootp()->modulesp(); modp;
        ...
        if (modp->dead() ||
          ...
          // Recurse, as cells may not be directly under the module but in a
          // generate
  ...
  bool mightElimVar(AstVar *nodep) {
    if (nodep->isSigPublic())
      return false; // Can't elim publics!
    if (nodep->isIO() || nodep->isClassMember())
      return false;
    if (nodep->isTemp() && !nodep->isTrace())
      return true;
    ...
    for (bool retry = true; retry;) {
      ...
      for (auto &itr : m_classesp) {
        if (AstClass *nodep = itr) { // nullptr if deleted earlier
          if (nodep->user1() == 0) {
            if (nodep->extendsp())
              nodep->extendsp()->user1Inc(-1);
            if (nodep->classOrPackagep())
              nodep->classOrPackagep()->user1Inc(-1);
            VL_DO_DANGLING(pushDeletep(nodep->unlinkFrBack()), nodep);
            itr = nullptr;
            retry = true;
          }
  ...
  void deadCheckVar() {
    ...
    for (AstVarScope *vscp : m_vscsp) {
      if (vscp->user1() == 0) {
        ...
        for (AssignMap::iterator itr = eqrange.first; itr != eqrange.second;
             ++itr) {
    ...
    for (bool retry = true; retry;) {
      retry = false;
      for (std::vector<AstVar *>::iterator it = m_varsp.begin();
           it != m_varsp.end(); ++it) {
        AstVar *varp = *it;
        if (!varp)
          continue;
    ...
    for (std::vector<AstNode *>::iterator it = m_dtypesp.begin();
         it != m_dtypesp.end(); ++it) {
      if ((*it)->user1() == 0) {
        AstNodeUOrStructDType *classp;
        // It's possible that there if a reference to each individual member,
        // but not to the dtype itself.  Check and don't remove the parent dtype
        // if members are still alive.
...
public:
  ...
  DeadVisitor(AstNetlist *nodep, bool elimUserVars, bool elimDTypes,
      ...
      : m_elimUserVars{elimUserVars}, m_elimDTypes{elimDTypes}, m_elimCells{
                                                                    elimCells} {
    ...
    if (elimCells)
      deadCheckCells();
    deadCheckClasses();
    // Modules after vars, because might be vars we delete inside a mod we
    // delete
    deadCheckMod();
...
void V3Dead::deadifyModules(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("deadModules", 0,
...
void V3Dead::deadifyDTypes(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("deadDtypes", 0,
...
void V3Dead::deadifyAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("deadAll", 0,
                                v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}
...
void V3Dead::deadifyAllScoped(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("deadAllScoped", 0,
                                v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}
