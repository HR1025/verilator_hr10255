...
string VString::replaceWord(const string &str, const string &from,
                            const string &to) {
  string result = str;
  const size_t len = from.size();
  UASSERT_STATIC(len > 0, "Cannot replace empty string");
  for (size_t pos = 0; (pos = result.find(from, pos)) != string::npos;
       pos += len) {
...
static inline void sha256Block(uint32_t *h,
                               const uint32_t *chunk) VL_ATTR_ALWINLINE;
    ...
    for (unsigned j = 0; j < 16; ++j) {
      if (i == 0) {
        ...
        const uint32_t s0 = shaRotr32(w[(j + 1) & 0xf], 7) ^
                            shaRotr32(w[(j + 1) & 0xf], 18) ^
                            (w[(j + 1) & 0xf] >> 3);
        const uint32_t s1 = shaRotr32(w[(j + 14) & 0xf], 17) ^
                            shaRotr32(w[(j + 14) & 0xf], 19) ^
                            (w[(j + 14) & 0xf] >> 10);
        w[j] = w[j] + s0 + w[(j + 9) & 0xf] + s1;
      }
      const uint32_t s1 =
          shaRotr32(ah[4], 6) ^ shaRotr32(ah[4], 11) ^ shaRotr32(ah[4], 25);
      const uint32_t ch = (ah[4] & ah[5]) ^ (~ah[4] & ah[6]);
      const uint32_t temp1 = ah[7] + s1 + ch + sha256K[i << 4 | j] + w[j];
      const uint32_t s0 =
          shaRotr32(ah[0], 2) ^ shaRotr32(ah[0], 13) ^ shaRotr32(ah[0], 22);
...
void VHashSha256::insert(const void *datap, size_t length) {
  UASSERT(!m_final,
          "Called VHashSha256::insert after finalized the hash value");
  m_totLength += length;
  ...
  m_remainder = string(reinterpret_cast<const char *>(chunkp + posBegin),
                       chunkLen - posEnd);
}
...
string VName::dehash(const string &in) {
  ...
  // keeps track of the position after the most recently found instance of
  // __DOT__
  for (string::size_type last_dot_pos = 0; last_dot_pos < in.size();) {
    ...
    auto search_end = next_dot_pos == string::npos
                          ? std::end(in)
                          : std::begin(in) + next_dot_pos;

    // Search for __Vhsh between the two dots.
    auto begin_vhsh = std::search(search_begin, search_end, std::begin(VHSH),
                                  std::end(VHSH) - 1);
    if (begin_vhsh != search_end) {
      std::string vhsh(begin_vhsh, search_end);
      const auto &it = s_dehashMap.find(vhsh);
      UASSERT(it != s_dehashMap.end(),
              "String not in reverse hash map '" << vhsh << "'");
      ...
      // Append the bit that was lost to truncation but retrieved from the
      // dehash map.
      dehashed += it->second;
    }
...
VSpellCheck::EditDistance VSpellCheck::editDistance(const string &s,
                                                    const string &t) {
  ...
  if (tLen == 0)
    return sLen;
  if (sLen >= LENGTH_LIMIT)
    return sLen;
  if (tLen >= LENGTH_LIMIT)
    return tLen;
...
VSpellCheck::EditDistance VSpellCheck::cutoffDistance(size_t goal_len,
  ...
  if (max_length - min_length <= 1)
    return std::max(max_length / 3, static_cast<size_t>(1));
  return (max_length + 2) / 3;
}

string VSpellCheck::bestCandidateInfo(const string &goal,
                                      EditDistance &distancer) const {
  ...
  for (const string &candidate : m_candidates) {
    ...
    const EditDistance min_distance =
        (cLen > gLen ? (cLen - gLen) : (gLen - cLen));
    if (min_distance >= distancer)
      continue; // Short-circuit if already better

    const EditDistance cutoff = cutoffDistance(gLen, cLen);
    if (min_distance > cutoff)
      continue; // Short-circuit if already too bad
...
void VSpellCheck::selfTestSuggestOne(bool matches, const string &c,
                                     const string &goal, EditDistance dist) {
  EditDistance gdist;
  VSpellCheck speller;
