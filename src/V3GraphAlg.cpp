  ...
  void vertexIterate(V3GraphVertex *vertexp) {
    ...
    for (V3GraphEdge *nextp, *edgep = vertexp->outBeginp(); edgep;
      ...
      if (followEdge(edgep)) {
        ...
        V3GraphEdge *prevEdgep =
            static_cast<V3GraphEdge *>(outVertexp->userp());
        if (!prevEdgep) { // No previous assignment
          ...
          if (saveOld) {
            ...
            if (m_sumWeights)
              edgep->weight(prevEdgep->weight() + edgep->weight());
...
public:
  ...
  void go() {
    ...
    for (V3GraphVertex *vxp = m_graphp->verticesBeginp(); vxp;
         vxp = vxp->verticesNextp()) {
      V3GraphEdge *deletep = nullptr;
      for (V3GraphEdge *edgep = vxp->outBeginp(); edgep;
           edgep = edgep->outNextp()) {
        if (deletep)
          VL_DO_CLEAR(deletep->unlinkDelete(), deletep = nullptr);
...
private:
  ...
  std::vector<V3GraphVertex *>
      m_callTrace; // List of everything we hit processing so far

  void main() {
    // Use Tarjan's algorithm to find the strongly connected subgraphs.
    // Node State:
    //     Vertex::user     // DFS number indicating possible root of subtree,
    //     0=not iterated Vertex::color    // Output subtree number (fully
    //     processed)
  ...
  void vertexIterate(V3GraphVertex *vertexp) {
    ...
    for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
      ...
      if (followEdge(edgep)) {
        ...
        if (!top->color()) { // Dest not in a component
          if (vertexp->user() > top->user())
            vertexp->user(top->user());
        }
      }
    }
    if (vertexp->user() == thisDfsNum) { // New head of subtree
      ...
      while (!m_callTrace.empty()) {
        ...
        if (popVertexp->user() >=
            thisDfsNum) { // Lower node is part of this subtree
...
private:
  ...
  void vertexIterate(V3GraphVertex *vertexp, uint32_t currentRank) {
    ...
    // directly
    while (m_callTrace.size() <= currentRank)
      m_callTrace.push_back(vertexp);
...
private:
  ...
  V3GraphVertex *vertexIterateAll(V3GraphVertex *vertexp) {
    if (V3GraphVertex *newVertexp =
            static_cast<V3GraphVertex *>(vertexp->userp())) {
      return newVertexp;
    } else {
...
void V3Graph::sortVertices() {
  ...
  this->verticesUnlink();
  for (V3GraphVertex *ip : vertices)
    ip->verticesPushBack(this);
}
...
double V3Graph::orderDFSIterate(V3GraphVertex *vertexp) {
  ...
  if (vertexp->user() == 2)
    return vertexp->fanout(); // Already processed it
  UASSERT_OBJ(vertexp->user() != 1, vertexp,
              "Loop found, backward edges should be dead");
  vertexp->user(1);
  double fanout = 0;
  for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
       edgep = edgep->outNextp()) {
    if (edgep->weight())
      fanout += orderDFSIterate(edgep->m_top);
  }
