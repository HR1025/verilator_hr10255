...
//
// Hierarchical Verilation internally creates protect-lib for each hierarchical
// block. Upper modules read wrapper for the protect-lib instead of the actual
// design.
//
...
//    that are marked with /*verilator hier_block*/ metacomment in Verilator run
//    a).
...
//    3-1) Delete the upper modules of the hierarchical block because the top
//    module in run b) is
//         hierarchical block, not the top module of run c).
//    3-2) If the top module of the run b) or c) instantiates other hierarchical
//    blocks that is
//         parameterized,
//         module and task names are renamed to the original name to be
//         compatible with the hier module to be called.
//
//         Parameterized modules have unique name by V3Param.cpp. The unique
//         name contains '__' and Verilator encodes '__' when loading such
//         symbols.
// 4) V3LinkDot.cpp checks dotted access across hierarchical block boundary.
// 5) In V3Dead.cpp, some parameters of parameterized modules are protected not
// to be deleted even
//    if the parameter is not referred. This protection is necessary to match
//    step 6) below.
// 6) In V3Param.cpp, use protect-lib wrapper of parameterized module made in b)
// and c).
//    If a hierarchical block is a parameterized module and instantiated in
//    multiple locations, all parameters must exactly match.
// 7) In V3HierBlock.cpp, relationship among hierarchical blocks are checked in
// run a).
...
//       mangled_name :Mangled name of parameterized modules (named in
//       V3Param.cpp).
//                     Same as module_name for non-parameterized hierarchical
//                     block.
...
static void V3HierWriteCommonInputs(const V3HierBlock *hblockp,
                                    std::ostream *of, bool forCMake) {
  string topModuleFile;
  if (hblockp)
    topModuleFile = hblockp->vFileIfNecessary();
  if (!forCMake) {
  ...
  for (const string &i : libraryFiles) {
    if (V3Os::filenameRealPath(i) != topModuleFile)
      *of << "-v " << i << "\n";
  }
}
...
V3HierBlock::StrGParams V3HierBlock::stringifyParams(const GParams &gparams,
                                                     bool forGOption) {
  StrGParams strParams;
  for (const auto &gparam : gparams) {
    if (const AstConst *constp = VN_CAST(gparam->valuep(), Const)) {
      string s;
      // Only constant parameter needs to be set to -G because already checked
      // in V3Param.cpp. See also ParamVisitor::checkSupportedParam() in the
      // file.
      if (constp->isDouble()) {
        ...
        const int len = VL_SNPRINTF(hexFpStr.data(), hexFpStr.size(), "%a",
                                    constp->num().toDouble());
        UASSERT_OBJ(0 < len && static_cast<size_t>(len) < hexFpStr.size(),
                    constp, " is not properly converted to string");
        s = hexFpStr.data();
      } else if (constp->isString()) {
        s = constp->num().toString();
        if (!forGOption)
          s = VString::quoteBackslash(s);
...
V3StringList V3HierBlock::commandArgs(bool forCMake) const {
  ...
  for (StrGParams::const_iterator paramIt = gparamsStr.begin();
       paramIt != gparamsStr.end(); ++paramIt) {
...
string V3HierBlock::hierSomeFile(bool withDir, const char *prefix,
...
string V3HierBlock::hierWrapper(bool withDir) const {
  return hierSomeFile(withDir, "", ".sv");
}

string V3HierBlock::hierMk(bool withDir) const {
  return hierSomeFile(withDir, "V", ".mk");
}

string V3HierBlock::hierLib(bool withDir) const {
  return hierSomeFile(withDir, "lib", ".a");
}
...
void V3HierBlock::writeCommandArgsFile(bool forCMake) const {
  std::unique_ptr<std::ofstream> of{
      V3File::new_ofstream(commandArgsFileName(forCMake))};
  *of << "--cc\n";
  ...
  for (const string &opt : commandOpts)
    *of << opt << "\n";
...
class HierBlockUsageCollectVisitor final : public AstNVisitor {
  ...
  AstModule *m_hierBlockp =
      nullptr; // The nearest parent module that is a hierarchical block
  ModuleSet m_referred; // Modules that have hier_block pragma
  V3HierBlock::GParams
      m_gparams; // list of variables that is AstVarType::GPARAM
  ...
  virtual void visit(AstModule *nodep) override {
    ...
    UINFO(5, "Checking " << nodep->prettyNameQ() << " from "
                         << (m_hierBlockp ? m_hierBlockp->prettyNameQ()
                                          : string("null"))
                         << std::endl);
    VL_RESTORER(m_modp);
    ...
    if (nodep->hierBlock()) {
      ...
      for (const AstModule *modp : m_referred)
        m_planp->registerUsage(nodep, modp);
...
void V3HierBlockPlan::add(const AstNodeModule *modp,
                          const std::vector<AstVar *> &gparams) {
  const iterator it = m_blocks.find(modp);
  if (it == m_blocks.end()) {
    V3HierBlock *hblockp = new V3HierBlock(modp, gparams);
    UINFO(3, "Add " << modp->prettyNameQ() << " with " << gparams.size()
                    << " parameters" << std::endl);
    m_blocks.emplace(modp, hblockp);
  }
}

void V3HierBlockPlan::registerUsage(const AstNodeModule *parentp,
                                    const AstNodeModule *childp) {
...
V3HierBlockPlan::HierVector V3HierBlockPlan::hierBlocksSorted() const {
  using ChildrenMap =
      std::unordered_map<const V3HierBlock *,
                         std::unordered_set<const V3HierBlock *>>;
  ...
  // Use index instead of iterator because new elements will be added in this
  // loop
  for (size_t i = 0; i < sorted.size(); ++i) {
    // This hblockp is already leaf.
    const V3HierBlock *hblockp = sorted[i];
    const V3HierBlock::HierBlockSet &p = hblockp->parents();
    for (V3HierBlock::HierBlockSet::const_iterator it = p.begin();
         it != p.end(); ++it) {
      // Delete hblockp from parrents. If a parent does not have a child
      // anymore, then it is a leaf too.
      const auto parentIt = childrenOfHierBlock.find(*it);
      UASSERT_OBJ(parentIt != childrenOfHierBlock.end(), (*it)->modp(),
                  "must be included");
      const V3HierBlock::HierBlockSet::size_type erased =
          parentIt->second.erase(hblockp);
