...
private:
  // VISITORS
  virtual void visit(AstVarRef *nodep) override {
    if (nodep->varp() && nodep->varp()->user4())
      m_found = true;
  }
  virtual void visit(AstNode *nodep) override { iterateChildren(nodep); }
...
// This visitor can be used in the post-expanded Ast from V3Expand, where the
// Ast satisfies:
...
class ConstBitOpTreeVisitor final : public AstNVisitor {
  ...
  // Holds a node to be added as a term in the reduction tree, it's equivalent
  // op count, and a bool indicating if the term is clean (0/1 value, or if the
  // top bits might be dirty)
  ...
  struct LeafInfo final { // Leaf node (either AstConst or AstVarRef)
    ...
    int width() const {
      ...
      if (!m_refp->isWide()) {
        ...
        const int bitsInMSW =
            VL_BITBIT_E(width) ? VL_BITBIT_E(width) : VL_EDATASIZE;
  ...
  struct BitPolarityEntry final { // Found bit polarity during iterate()
    ...
    BitPolarityEntry(const LeafInfo &info, bool pol, int bit)
        : m_info{info}, m_polarity{pol}, m_bit{bit} {}
    BitPolarityEntry() = default;
  };

  class Restorer
      final { // Restore the original state unless disableRestore() is called
  ...
  public:
    explicit Restorer(ConstBitOpTreeVisitor &visitor)
        : m_visitor{visitor}, m_polaritiesSize{visitor.m_bitPolarities.size()},
          m_frozenSize{visitor.m_frozenNodes.size()}, m_ops{visitor.m_ops},
          m_polarity{visitor.m_polarity}, m_restore{true} {}
    ~Restorer() {
  ...
  class VarInfo final {
    // MEMBERS
    int m_knownResult =
        -1; // -1: result is not known, 0 or 1: result of this tree
    ConstBitOpTreeVisitor
        *const m_parentp;    // ConstBitOpTreeVisitor that holds this VarInfo
  ...
  public:
    // METHODS
    bool hasConstResult() const {
      return m_knownResult >= 0 || m_bitPolarity.isAllX();
    }
    // The constant result. Only valid if hasConstResult() returned true.
    bool getConstResult() const {
      // Note that this condition covers m_knownResult == -1 but
      // m_bitPolarity.isAllX(), in which case the result is 0
      return m_knownResult == 1;
    }
    const AstVarRef *refp() const { return m_refp; }
    bool sameVarAs(const AstNodeVarRef *otherp) const {
      return m_refp->same(otherp);
    }
    void setPolarity(bool compBit, int bit) {
      ...
      if (m_bitPolarity.bitIsX(
              bit)) { // The bit is not yet marked with either polarity
        ...
        if (m_parentp->isXorTree()) {
          UASSERT_OBJ(compBit && sameFlag, m_refp,
                      "Only true is set in Xor tree");
          // a ^ a ^ b == b so we can ignore a
          m_bitPolarity.setBit(bit, 'x');
        } else { // And, Or
          // Can ignore this nodep as the bit is already marked with the same
          // polarity
          if (sameFlag)
            return; // a & a == a, b | b == b
    ...
    // Return reduction term for this VarInfo, together with the number of ops
    // in the term, and a boolean indicating if the term is clean (1-bit vs
    // multi-bit value)
    ResultTerm getResultTerm() const {
      ...
      // Get the term we are referencing (the WordSel, if wide, otherwise just
      // the VarRef)
      ...
      if (isPow2(maskVal)) {
        // If we only want a single bit, shift it out instead of a masked
        // compare. Shifts don't go through the flags register on x86 and are
        // hence faster. This is also always fewer or same ops as mask and
        // compare, but with shorter instructions on x86.
        ...
        const bool needsMasking =
            maskVal != VL_MASK_Q(m_width) || hasRedundantSignBits;
        if (needsMasking) {
        ...
        if (m_parentp->isXorTree()) {
          ...
          ++ops;
          clean = false;
          // VL_REDXOR_* returns IData, set width accordingly to avoid
          // unnecessary casts
  ...
  public:
    ...
    VarInfo(ConstBitOpTreeVisitor *parent, AstVarRef *refp, int width)
        : m_parentp{parent}, m_refp{refp}, m_width{width}, m_bitPolarity{
                                                               refp, m_width} {
  ...
  LeafInfo *m_leafp =
      nullptr;            // AstConst or AstVarRef that currently looking for
  AstNode *const m_rootp; // Root of this AST subtree

  std::vector<AstNode *> m_frozenNodes; // Nodes that cannot be optimized
  std::vector<BitPolarityEntry>
      m_bitPolarities; // Polarity of bits found during iterate()
  std::vector<std::unique_ptr<VarInfo>>
      m_varInfos; // VarInfo for each variable, [0] is nullptr
  ...
  bool isXorTree() const {
    return VN_IS(m_rootp, Xor) || VN_IS(m_rootp, RedXor);
  }

#define CONST_BITOP_RETURN_IF(cond, nodep)                                     \
  if (setFailed(cond, #cond, nodep, __LINE__))                                 \
  return

#define CONST_BITOP_SET_FAILED(reason, nodep)                                  \
  setFailed(true, reason, nodep, __LINE__)

  bool setFailed(bool fail, const char *reason, AstNode *nodep, int line) {
    if (fail && !m_failed) {
      UINFO(9, "cannot optimize " << m_rootp << " reason:" << reason
                                  << " called from line:" << line
                                  << " when checking:" << nodep << std::endl);
  ...
  void incrOps(const AstNode *nodep, int line) {
    ++m_ops;
    UINFO(9, "Increment to " << m_ops << " " << nodep << " called from line "
                             << line << "\n");
  }
  VarInfo &getVarInfo(const LeafInfo &ref) {
    ...
    if (!nodep)
      nodep = ref.m_refp->varp(); // Not scoped
    int baseIdx = nodep->user4();
    if (baseIdx == 0) { // Not set yet
      baseIdx = m_varInfos.size();
      const int numWords = ref.m_refp->dtypep()->isWide()
                               ? ref.m_refp->dtypep()->widthWords()
                               : 1;
  ...
  virtual void visit(AstNot *nodep) override {
    ...
    if (AstCCast *castp = VN_CAST(lhsp, CCast))
      lhsp = castp->lhsp();
    CONST_BITOP_RETURN_IF(!VN_IS(lhsp, VarRef) && !VN_IS(lhsp, Xor) &&
                              !VN_IS(lhsp, RedXor) && !VN_IS(lhsp, ShiftR),
                          lhsp);
    incrOps(nodep, __LINE__);
  ...
  virtual void visit(AstConst *nodep) override {
    CONST_BITOP_RETURN_IF(!m_leafp, nodep);
    UASSERT_OBJ(!m_leafp->m_constp, nodep,
                m_leafp->m_constp << " is already set");
  ...
  virtual void visit(AstRedXor *nodep) override {
    ...
    AstNode *lhsp = nodep->lhsp();
    if (AstCCast *const castp = VN_CAST(lhsp, CCast))
      lhsp = castp->lhsp();
  ...
  virtual void visit(AstNodeBiop *nodep) override {
    if (VN_IS(nodep, And) && isConst(nodep->lhsp(), 1)) { // 1 & _
      ...
      for (const bool right : {false, true}) {
        ...
        if (leafInfo.m_constp || m_failed) {
          ...
          restorer.restoreNow();
          // Reach past a cast then add to frozen nodes to be added to final
          // reduction
          if (AstCCast *const castp = VN_CAST(opp, CCast))
            opp = castp->lhsp();
        ...
        restorer.disableRestore(); // Now all checks passed
        if (leafInfo.m_refp) {
          // The conditional on the lsb being in range is necessary for some
          // degenerate case, e.g.: (IData)((QData)wide[0] >> 32), or
          // <1-bit-var> >> 1, which is just zero
          if (leafInfo.m_lsb < leafInfo.width()) {
            m_bitPolarities.emplace_back(
                leafInfo, isXorTree() || leafInfo.m_polarity, leafInfo.m_lsb);
          } else if (isAndTree() && leafInfo.m_polarity) {
            // If there is a constant 0 term in an And tree, we must include it.
            // Fudge this by adding a bit with both polarities, which will
            // simplify to zero
    ...
    } else if ((isAndTree() && VN_IS(nodep, Eq)) ||
               (isOrTree() && VN_IS(nodep, Neq))) {
      Restorer restorer{*this};
      ...
      if (AstAnd *const andp =
              VN_CAST(nodep->rhsp(), And)) { // comp == (mask & v)
        const LeafInfo &mask = findLeaf(andp->lhsp(), true);
        CONST_BITOP_RETURN_IF(!mask.m_constp || mask.m_lsb != 0, andp->lhsp());
        ...
        // Mark all bits checked by this comparison
        const int maxBitIdx =
            std::min(ref.m_lsb + compNum.width(), ref.width());
        for (int bitIdx = ref.m_lsb; bitIdx < maxBitIdx; ++bitIdx) {
          const int maskIdx = bitIdx - ref.m_lsb;
          if (maskNum.bitIs0(maskIdx))
            continue;
      ...
      CONST_BITOP_SET_FAILED("Mixture of different ops cannot be optimized",
                             nodep);
    }
  }
  ...
  ConstBitOpTreeVisitor(AstNode *nodep, unsigned externalOps)
      : m_ops{externalOps}, m_rootp{nodep} {
    ...
    UASSERT_OBJ(isXorTree() || m_polarity, nodep,
                "must be the original polarity");
  }
...
public:
  ...
  // &{v[0], v[1]} => 2'b11 == (2'b11 & v)
  static AstNode *simplify(AstNode *nodep, int resultWidth,
                           unsigned externalOps, VDouble0 &reduction) {
    UASSERT_OBJ(1 <= resultWidth && resultWidth <= 64, nodep,
                "resultWidth out of range");

    // Walk tree, gathering all terms referenced in expression
    ConstBitOpTreeVisitor visitor{nodep, externalOps};

    // If failed on root node is not optimizable, or there are no variable
    // terms, then done
    if (visitor.m_failed || visitor.m_varInfos.size() == 1)
      return nullptr;

    // FileLine used for constructing all new nodes in this function
    FileLine *const fl = nodep->fileline();

    // Get partial result each term referenced, count total number of ops and
    // keep track of whether we have clean/dirty terms. visitor.m_varInfos
    // appears in deterministic order, so the optimized tree is deterministic as
    // well.
    ...
    for (auto &&v : visitor.m_varInfos) {
      if (!v)
        continue; // Skip nullptr at m_varInfos[0]
      if (v->hasConstResult()) {
        // If a constant term is known, we can either drop it or the whole tree
        // is constant
    ...
    for (AstNode *const termp : visitor.m_frozenNodes) {
      ...
      if (VN_IS(termp, Eq) || VN_IS(termp, Neq) || VN_IS(termp, Lt) ||
          VN_IS(termp, Lte) || VN_IS(termp, Gt) || VN_IS(termp, Gte)) {
        hasCleanTerm = true;
      } else {
    ...
    const bool needsCleaning =
        visitor.isAndTree() ? !hasCleanTerm : hasDirtyTerm;
    ...
    if (debug() >= 9) { // LCOV_EXCL_START
      ...
      for (AstNode *const termp : termps)
        termp->dumpTree("Reduced term: ");
      for (AstNode *const termp : visitor.m_frozenNodes)
        termp->dumpTree("Frozen term: ");
    ...
    if (termps.empty() && visitor.m_frozenNodes.empty()) {
      ...
      AstNode *const resultp = needsFlip
    ...
    const auto reduce = [&visitor, fl](AstNode *lhsp,
                                       AstNode *rhsp) -> AstNode * {
      if (!lhsp)
        return rhsp;
    ...
    // Set width of masks to expected result width. This is required to prevent
    // later removal of the masking node e.g. by the "AND with all ones" rule.
    // If the result width happens to be 1, we still need to ensure the AstAnd
    // is not dropped, so use a wider maks in this special case.
    const int maskWidth = resultWidth == 1 ? VL_IDATASIZE : resultWidth;

    // Apply final polarity flip
    if (needsFlip) {
      if (needsCleaning) {
        // Cleaning will be added below. Use a NOT which is a byte shorter on
        // x86
        resultp = new AstNot{fl, resultp};
      } else {
        // Keep result clean by using XOR(1, _)
        AstConst *const maskp =
            new AstConst{fl, AstConst::WidthedValue{}, maskWidth, 1};
...
private:
  ...
  // STATE
  bool m_params =
      false; // If true, propagate parameterized and true numbers only
  ...
  static uint32_t s_globalPassNum; // Counts number of times ConstVisitor
                                   // invoked as global pass
  V3UniqueNames
      m_concswapNames; // For generating unique temporary variable names
  ...
  bool operandAsvConst(const AstNode *nodep) {
    ...
    if (!bnodep)
      return false;
    if (!VN_IS(bnodep->lhsp(), Const))
      return false;
    const AstNodeBiComAsv *rnodep = VN_CAST(bnodep->rhsp(), NodeBiComAsv);
    if (!rnodep)
      return false;
    if (rnodep->type() != bnodep->type())
      return false;
    if (rnodep->width() != bnodep->width())
      return false;
    if (rnodep->lhsp()->width() != bnodep->lhsp()->width())
      return false;
    if (!VN_IS(rnodep->lhsp(), Const))
      return false;
    return true;
  }
  ...
  bool operandAsvLUp(const AstNode *nodep) {
    ...
    // BIASV(BIASV(CONSTa,CONSTc),BIASV(c...,d...))) // hits normal constant
    // propagation BIASV(CONST_a_c,BIASV(c...,d...)))
    //
    // Idea for the future: All BiComAsvs could be lists, sorted by if they're
    // constant
    const AstNodeBiComAsv *bnodep = VN_CAST(nodep, NodeBiComAsv);
    if (!bnodep)
      return false;
    ...
    if (!lnodep)
      return false;
    if (lnodep->type() != bnodep->type())
      return false;
    if (lnodep->width() != bnodep->width())
      return false;
  ...
  bool matchRedundantClean(AstAnd *andp) {
    ...
    UASSERT_OBJ(constp && constp->isOne(), andp->lhsp(),
                "TRREEOPC must meet this condition");
    AstNode *rhsp = andp->rhsp();
    AstCCast *ccastp = nullptr;
    auto isEqOrNeq = [](AstNode *nodep) -> bool {
    ...
    if (isEqOrNeq(rhsp)) {
      ...
      if (isEqOrNeq(tmpp->lhsp())) {
        if (tmpp->width() == andp->width()) {
          ...
          ccastp = new AstCCast{andp->fileline(), tmpp->lhsp()->unlinkFrBack(),
                                andp};
        }
      }
    }
  ...
  static bool operandAndOrSame(const AstNode *nodep) {
    ...
    return (lp && rp && lp->width() == rp->width() &&
            lp->type() == rp->type() &&
  ...
  bool matchAndCond(AstAnd *nodep) {
    // Push down a AND into conditional, when one side of conditional is
    // constant (otherwise we'd be trading one operation for two operations)
    ...
    if (!condp)
      return false;
    if (!VN_IS(condp->expr1p(), Const) && !VN_IS(condp->expr2p(), Const))
      return false;
    AstConst *maskp = VN_CAST(nodep->lhsp(), Const);
    if (!maskp)
      return false;
    UINFO(4, "AND(CONSTm, CONDcond(c, i, e))->CONDcond(c, AND(m,i), AND(m, e)) "
                 << nodep << endl);
  ...
  bool matchMaskedOr(AstAnd *nodep) {
    // Masking an OR with terms that have no bits set under the mask is replaced
    // with masking only the remaining terms. Canonical example as generated by
    // V3Expand is: 0xff & (a << 8 | b >> 24) --> 0xff & (b >> 24)
    ...
    // Predicate for checking whether the bottom 'significantBits' bits of the
    // given expression are all zeroes.
    ...
    if (orLIsRedundant && orRIsRedundant) {
      nodep->replaceWith(new AstConst(
          nodep->fileline(), AstConst::DtypedValue(), nodep->dtypep(), 0));
      VL_DO_DANGLING(nodep->deleteTree(), nodep);
      return true;
  ...
  bool matchMaskedShift(AstAnd *nodep) {
    // Drop redundant masking of right shift result. E.g: 0xff & ((uint32_t)a >>
    // 24). This commonly appears after V3Expand and the simplification in
    // matchMaskedOr. Similarly, drop redundant masking of left shift result.
  ...
  bool matchBitOpTree(AstNode *nodep) {
    if (nodep->widthMin() != 1)
      return false;
    if (!v3Global.opt.oConstBitOpTree())
      return false;
    ...
    if (andp && isConst(andp->lhsp(), 1)) { // 1 & BitOpTree
      newp = ConstBitOpTreeVisitor::simplify(andp->rhsp(), width, 1,
                                             m_statBitOpReduction);
    } else { // BitOpTree
      newp = ConstBitOpTreeVisitor::simplify(nodep, width, 0,
                                             m_statBitOpReduction);
    }
  ...
  bool operandShiftOp(const AstNodeBiop *nodep) {
    ...
    if (!lhsp || !(VN_IS(lhsp, And) || VN_IS(lhsp, Or) || VN_IS(lhsp, Xor)))
      return false;
    if (nodep->width() != lhsp->width())
      return false;
    if (nodep->width() != lhsp->lhsp()->width())
      return false;
    if (nodep->width() != lhsp->rhsp()->width())
      return false;
    return true;
  }
  bool operandShiftShift(const AstNodeBiop *nodep) {
    ...
    const AstNodeBiop *lhsp = VN_CAST(nodep->lhsp(), NodeBiop);
    if (!lhsp || !(VN_IS(lhsp, ShiftL) || VN_IS(lhsp, ShiftR)))
      return false;
    // We can only get rid of a<<b>>c or a<<b<<c, with constant b & c
    // because bits may be masked in that process, or (b+c) may exceed the word
    // width.
    if (!(VN_IS(nodep->rhsp(), Const) && VN_IS(lhsp->rhsp(), Const)))
      return false;
  ...
  bool operandSelExtend(AstSel *nodep) {
    ...
    if (!(m_doV && extendp && VN_IS(nodep->lsbp(), Const) &&
          VN_IS(nodep->widthp(), Const) && nodep->lsbConst() == 0 &&
  ...
  bool operandSelBiLower(AstSel *nodep) {
    ...
    if (!(m_doV && bip && VN_IS(nodep->lsbp(), Const) &&
          VN_IS(nodep->widthp(), Const) && nodep->lsbConst() == 0))
      return false;
    if (debug() >= 9)
      nodep->dumpTree(cout, "SEL(BI)-in:");
    ...
    if (debug() >= 9)
      bip->dumpTree(cout, "SEL(BI)-ou:");
    VL_DO_DANGLING(replaceWChild(nodep, bip), nodep);
    return true;
  }
  bool operandSelShiftLower(AstSel *nodep) {
    // AND({a}, SHIFTR({b}, {c})) is often shorthand in C for Verilog {b}[{c} :+
    // {a}] becomes thought other optimizations
    // SEL(SHIFTR({a},{b}),{lsb},{width}) -> SEL({a},{lsb+b},{width})
    AstShiftR *shiftp = VN_CAST(nodep->fromp(), ShiftR);
    if (!(m_doV && shiftp && VN_IS(shiftp->rhsp(), Const) &&
          VN_IS(nodep->lsbp(), Const) && VN_IS(nodep->widthp(), Const))) {
      return false;
    }
    ...
    if (bp->isWide() || bp->num().isFourState() || bp->num().isNegative() ||
        lp->isWide() || lp->num().isFourState() || lp->num().isNegative()) {
      return false;
    }
    const int newLsb = lp->toSInt() + bp->toSInt();
    if (newLsb + nodep->widthConst() > ap->width())
      return false;
    //
    UINFO(9, "SEL(SHIFTR(a,b),l,w) -> SEL(a,l+b,w)\n");
    if (debug() >= 9)
      nodep->dumpTree(cout, "SEL(SH)-in:");
    AstSel *newp = new AstSel(nodep->fileline(), ap->unlinkFrBack(), newLsb,
                              nodep->widthConst());
    newp->dtypeFrom(nodep);
    if (debug() >= 9)
      newp->dumpTree(cout, "SEL(SH)-ou:");
  ...
  bool operandBiExtendConstShrink(AstNodeBiop *nodep) {
    ...
    if (!constp)
      return false;
    if (!constp->num().isBitsZero(constp->width() - 1, subsize))
      return false;
    //
    if (debug() >= 9)
      nodep->dumpTree(cout, "BI(EXTEND)-in:");
    smallerp->unlinkFrBack();
    VL_DO_DANGLING(extendp->unlinkFrBack()->deleteTree(),
                   extendp); // aka nodep->lhsp.
    nodep->rhsp(smallerp);
    ...
    if (debug() >= 9)
      nodep->dumpTree(cout, "BI(EXTEND)-ou:");
    return true;
  }
  ...
  AstNode *afterComment(AstNode *nodep) {
    ...
    while (nodep && VN_IS(nodep, Comment)) {
      nodep = nodep->nextp();
    }
    return nodep;
  }
  ...
  bool isTPure(AstNode *nodep) {
    ...
    // Eventually we'll recurse through tree when unknown, memoizing results so
    // far, but for now can disable en-mass until V3Purify takes effect.
  ...
  bool isTreePureRecurse(AstNode *nodep) {
    ...
    if (!nodep->isPure())
      return false;
    if (nodep->op1p() && !isTreePureRecurse(nodep->op1p()))
      return false;
    if (nodep->op2p() && !isTreePureRecurse(nodep->op2p()))
      return false;
    if (nodep->op3p() && !isTreePureRecurse(nodep->op3p()))
      return false;
    if (nodep->op4p() && !isTreePureRecurse(nodep->op4p()))
      return false;
    return true;
  }
  ...
  bool warnSelect(AstSel *nodep) {
    ...
    if (m_warn && VN_IS(nodep->lsbp(), Const) &&
        VN_IS(nodep->widthp(), Const) && doit) {
      const int maxDeclBit =
          nodep->declRange().hiMaxSelect() * nodep->declElWidth() +
          (nodep->declElWidth() - 1);
      if (VN_AS(nodep->lsbp(), Const)->num().isFourState() ||
        ...
        if (nodep->declElWidth() == 0) {
          ...
          msbLsbProtected =
              std::to_string(nodep->msbConst() / nodep->declElWidth()) + ":" +
  ...
  static bool operandsSame(AstNode *node1p, AstNode *node2p) {
    // For now we just detect constants & simple vars, though it could be more
    // generic
  ...
  bool ifSameAssign(const AstNodeIf *nodep) {
    ...
    if (!ifp || ifp->nextp())
      return false; // Must be SINGLE statement
    if (!elsep || elsep->nextp())
      return false;
    if (ifp->type() != elsep->type())
      return false; // Can't mix an assigndly and an assign
    if (!ifp->lhsp()->sameGateTree(elsep->lhsp()))
      return false;
    if (!ifp->rhsp()->gateTree())
      return false;
    if (!elsep->rhsp()->gateTree())
      return false;
    return true;
  }
  bool operandIfIf(const AstNodeIf *nodep) {
    ...
    if (!lowerIfp || lowerIfp->nextp())
      return false;
    if (nodep->type() != lowerIfp->type())
      return false;
    if (afterComment(lowerIfp->elsesp()))
      return false;
    return true;
  }
  ...
  bool ifAdjacentSel(const AstSel *lhsp, const AstSel *rhsp) {
    if (!v3Global.opt.oAssemble())
      return false; // opt disabled
    if (!lhsp || !rhsp)
      return false;
    const AstNode *lfromp = lhsp->fromp();
    const AstNode *rfromp = rhsp->fromp();
    if (!lfromp || !rfromp || !lfromp->sameGateTree(rfromp))
      return false;
    ...
    if (!lstart || !rstart || !lwidth || !rwidth)
  ...
  bool ifMergeAdjacent(AstNode *lhsp, AstNode *rhsp) {
    ...
    if (rselp && !lselp && lhsp->sameGateTree(rselp->fromp()))
      lselp = new AstSel(lhsp->fileline(), lhsp->cloneTree(false), 0,
                         lhsp->width());
    // {b, a[j:k]} a[k-1:i]
    ...
    const int rend = (rstart->toSInt() + rwidth->toSInt());
    // a[i:j] a[j-1:k]
    if (rend == lstart->toSInt())
      return true;
    // a[i:0] a[msb:j]
    if (rend == rfromp->width() && lstart->toSInt() == 0)
      return true;
    return false;
  }
  bool concatMergeable(const AstNode *lhsp, const AstNode *rhsp) {
    ...
    if (!v3Global.opt.oAssemble())
      return false; // opt disabled
    if (lhsp->type() != rhsp->type())
      return false;
    if (!ifConcatMergeableBiop(lhsp))
      return false;
    ...
    if (lad && rad)
      return true;
    // {a[] & b[]&c[], a[] & b[]&c[]}
    if (lad && concatMergeable(lp->rhsp(), rp->rhsp()))
      return true;
    // {a[]&b[] & c[], a[]&b[] & c[]}
    if (rad && concatMergeable(lp->lhsp(), rp->lhsp()))
      return true;
    // {(a[]&b[])&(c[]&d[]), (a[]&b[])&(c[]&d[])}
  ...
  bool operandsSameSize(AstNode *lhsp, AstNode *rhsp) {
  ...
  void replaceNum(AstNode *oldp, const V3Number &num) {
    ...
    UASSERT_OBJ(
        !(VN_IS(oldp, Const) && !VN_AS(oldp, Const)->num().isFourState()), oldp,
        "Already constant??");
    AstNode *newp = new AstConst(oldp->fileline(), num);
    newp->dtypeFrom(oldp);
    if (debug() > 5)
      oldp->dumpTree(cout, "  const_old: ");
    if (debug() > 5)
  ...
  void replaceNumSigned(AstNodeBiop *nodep, uint32_t val) {
    ...
    if (m_warn &&
        !(VN_IS(nodep->lhsp(), Const) && VN_IS(nodep->rhsp(), Const))) {
      nodep->v3warn(UNSIGNED,
  ...
  void replaceNumLimited(AstNodeBiop *nodep, uint32_t val) {
    ...
    if (m_warn)
      nodep->v3warn(CMPCONST, "Comparison is constant due to limited range");
    VL_DO_DANGLING(replaceNum(nodep, val), nodep);
  }
  ...
  void replaceZeroChkPure(AstNode *nodep, AstNode *checkp) {
    ...
    // If ever change the operation note AstAnd rule specially ignores this
    // created pattern
    if (isTPure(checkp)) {
      VL_DO_DANGLING(replaceNum(nodep, 0), nodep);
    } else {
      AstNode *newp =
          new AstAnd(nodep->fileline(), new AstConst(nodep->fileline(), 0),
                     checkp->unlinkFrBack());
      newp->dtypeFrom(nodep);
      nodep->replaceWith(newp);
      VL_DO_DANGLING(nodep->deleteTree(), nodep);
  ...
  void replaceWChild(AstNode *nodep, AstNode *childp) {
    ...
    // If replacing a SEL for example, the data type comes from the parent (is
    // less wide). This may adversely affect the operation of the node being
    // replaced.
  ...
  void replaceAsv(AstNodeBiop *nodep) {
    ...
    rp->lhsp(ap);
    rp->rhsp(bp);
    if (VN_IS(rp->lhsp(), Const) && VN_IS(rp->rhsp(), Const))
      replaceConst(rp);
  ...
  void replaceAndOr(AstNodeBiop *nodep) {
    //  OR  (AND (CONSTll,lr), AND(CONSTrl==ll,rr))    -> AND (CONSTll,
    //  OR(lr,rr)) OR  (AND (CONSTll,lr), AND(CONSTrl,    rr=lr)) -> AND
    //  (OR(ll,rl), rr)
    ...
    if (operandsSame(llp, rlp)) {
      ...
      nodep->v3fatalSrc(
          "replaceAndOr on something operandAndOrSame shouldn't have matched");
    }
  ...
  void replaceConcatSel(AstConcat *nodep) {
    ...
    AstSel *newselp =
        new AstSel(lselp->fromp()->fileline(), rselp->fromp()->cloneTree(false),
                   rstart, lwidth + rwidth);
    UINFO(5, "merged two adjacent sel " << lselp << " and " << rselp
                                        << " to one " << newselp << endl);
  ...
  void replaceConcatMerge(AstConcat *nodep) {
    ...
    if (concatMergeable(lp, rp)) {
      ...
      lp->dtypeChgWidthSigned(newlp->width(), newlp->width(),
                              VSigning::UNSIGNED);
  ...
  void replaceExtend(AstNode *nodep, AstNode *arg0p) {
    // -> EXTEND(nodep)
    // like a AstExtend{$rhsp}, but we need to set the width correctly from base
    // node
    arg0p->unlinkFrBack();
    AstNode *const newp =
        (VN_IS(nodep, ExtendS)
  ...
  void replacePowShift(AstNodeBiop *nodep) { // Pow or PowS
    ...
    AstShiftL *newp = new AstShiftL(nodep->fileline(),
                                    new AstConst(nodep->fileline(), 1), rhsp);
    newp->dtypeFrom(nodep);
  ...
  void replaceMulShift(AstMul *nodep) { // Mul, but not MulS as not simple shift
    ...
    const int amount =
        VN_AS(nodep->lhsp(), Const)->num().mostSetBitP1() - 1; // 2^n->n+1
    AstNode *opp = nodep->rhsp()->unlinkFrBack();
    AstShiftL *newp = new AstShiftL(nodep->fileline(), opp,
                                    new AstConst(nodep->fileline(), amount));
    newp->dtypeFrom(nodep);
    nodep->replaceWith(newp);
    VL_DO_DANGLING(nodep->deleteTree(), nodep);
  }
  void replaceDivShift(AstDiv *nodep) { // Mul, but not MulS as not simple shift
    ...
    const int amount =
        VN_AS(nodep->rhsp(), Const)->num().mostSetBitP1() - 1; // 2^n->n+1
    AstNode *opp = nodep->lhsp()->unlinkFrBack();
    AstShiftR *newp = new AstShiftR(nodep->fileline(), opp,
                                    new AstConst(nodep->fileline(), amount));
    newp->dtypeFrom(nodep);
    nodep->replaceWith(newp);
    VL_DO_DANGLING(nodep->deleteTree(), nodep);
  ...
  void
  replaceModAnd(AstModDiv *nodep) { // Mod, but not ModS as not simple shift
    ...
    mask.setMask(amount);
    AstNode *opp = nodep->lhsp()->unlinkFrBack();
    AstAnd *newp = new AstAnd(nodep->fileline(), opp,
  ...
  void replaceShiftOp(AstNodeBiop *nodep) {
    UINFO(5, "SHIFT(AND(a,b),CONST)->AND(SHIFT(a,CONST),SHIFT(b,CONST)) "
                 << nodep << endl);
    AstNRelinker handle;
  ...
  void replaceShiftShift(AstNodeBiop *nodep) {
    ...
    if (debug() >= 9)
      nodep->dumpTree(cout, "  repShiftShift_old: ");
    AstNodeBiop *lhsp = VN_AS(nodep->lhsp(), NodeBiop);
    lhsp->unlinkFrBack();
    AstNode *ap = lhsp->lhsp()->unlinkFrBack();
    ...
    if (nodep->type() == lhsp->type()) {
      ...
      // We know shift amounts are constant, but might be a mixed left/right
      // shift
      int shift1 = VN_AS(shift1p, Const)->toUInt();
      if (VN_IS(lhsp, ShiftR))
        shift1 = -shift1;
      int shift2 = VN_AS(shift2p, Const)->toUInt();
      if (VN_IS(nodep, ShiftR))
        shift2 = -shift2;
      const int newshift = shift1 + shift2;
      VL_DO_DANGLING(shift1p->deleteTree(), shift1p);
      VL_DO_DANGLING(shift2p->deleteTree(), shift2p);
      ...
      newp->dtypeFrom(nodep);
      newp = new AstAnd(nodep->fileline(), newp,
                        new AstConst(nodep->fileline(), mask));
      newp->dtypeFrom(nodep);
      nodep->replaceWith(newp);
      VL_DO_DANGLING(nodep->deleteTree(), nodep);
  ...
  bool replaceAssignMultiSel(AstNodeAssign *nodep) {
    ...
    // Skip if we're not const'ing an entire module (IE doing only one assign,
    // etc)
    if (!m_modp)
      return false;
    ...
    if (!varref2p)
      return false;
    if (!varref1p->sameGateTree(varref2p))
      return false;
    ...
    if (!varNotReferenced(nodep->rhsp(), varref1p->varp()))
      return false;
    if (!varNotReferenced(nextp->rhsp(), varref2p->varp()))
      return false;
    // If a variable is marked split_var, access to the variable should not be
    // merged.
    if (varref1p->varp()->attrSplitVar() || varref2p->varp()->attrSplitVar())
      return false;
    // Swap?
    ...
    AstNode *newp;
    if (lsbFirstAssign) {
      newp = nodep->cloneType(
          new AstSel(sel1p->fileline(), varref1p->unlinkFrBack(),
  ...
  bool varNotReferenced(AstNode *nodep, AstVar *varp, int level = 0) {
    ...
    // Return false if referenced, or tree too deep to be worth it, or side
    // effects
    if (!nodep)
      return true;
    if (level > 2)
      return false;
    if (!nodep->isPure())
      return false; // For example a $fgetc can't be reordered
    if (VN_IS(nodep, NodeVarRef) && VN_AS(nodep, NodeVarRef)->varp() == varp)
      return false;
  ...
  bool replaceNodeAssign(AstNodeAssign *nodep) {
    if (VN_IS(nodep->lhsp(), VarRef) && VN_IS(nodep->rhsp(), VarRef) &&
        VN_AS(nodep->lhsp(), VarRef)
            ->sameNoLvalue(VN_AS(nodep->rhsp(), VarRef)) &&
        !VN_IS(nodep, AssignDly)) {
      // X = X.  Quite pointless, though X <= X may override another earlier
      // assignment
      ...
      if (need_temp) {
        ...
        // and RHS.  In that case, we must use temporaries, or {a,b}={b,a} will
        // break.
      ...
      AstSel *sel2p =
          new AstSel(conp->fileline(), rhs2p, lsb2, msb2 - lsb2 + 1);
      // Make new assigns of same flavor as old one
      //*** Not cloneTree; just one node.
      AstNode *newp = nullptr;
      if (!need_temp) {
        AstNodeAssign *asn1ap =
            VN_AS(nodep->cloneType(lc1p, sel1p), NodeAssign);
        AstNodeAssign *asn2ap =
            VN_AS(nodep->cloneType(lc2p, sel2p), NodeAssign);
        ...
        // We could create just one temp variable, but we'll get better
        // optimization if we make one per term.
        AstVar *const temp1p = new AstVar(
            sel1p->fileline(), AstVarType::BLOCKTEMP,
            m_concswapNames.get(sel1p), VFlagLogicPacked(), msb1 - lsb1 + 1);
        AstVar *const temp2p = new AstVar(
            sel2p->fileline(), AstVarType::BLOCKTEMP,
            m_concswapNames.get(sel2p), VFlagLogicPacked(), msb2 - lsb2 + 1);
        m_modp->addStmtp(temp1p);
        m_modp->addStmtp(temp2p);
        AstNodeAssign *asn1ap =
            VN_AS(nodep->cloneType(
                      new AstVarRef(sel1p->fileline(), temp1p, VAccess::WRITE),
                      sel1p),
                  NodeAssign);
        AstNodeAssign *asn2ap =
            VN_AS(nodep->cloneType(
                      new AstVarRef(sel2p->fileline(), temp2p, VAccess::WRITE),
                      sel2p),
                  NodeAssign);
        AstNodeAssign *asn1bp =
            VN_AS(nodep->cloneType(lc1p, new AstVarRef(sel1p->fileline(),
                                                       temp1p, VAccess::READ)),
                  NodeAssign);
        AstNodeAssign *asn2bp =
            VN_AS(nodep->cloneType(lc2p, new AstVarRef(sel2p->fileline(),
                                                       temp2p, VAccess::READ)),
                  NodeAssign);
      ...
      if (debug() >= 9 && newp)
        newp->dumpTreeAndNext(cout, "     _new: ");
      ...
      if (sWidth > dWidth) {
        streamp =
            new AstSel(streamp->fileline(), streamp, sWidth - dWidth, dWidth);
      }
  ...
  bool operandBoolShift(const AstNode *nodep) {
    // boolean test of AND(const,SHIFTR(x,const)) -> test of
    // AND(SHIFTL(x,const), x)
    if (!VN_IS(nodep, And))
      return false;
    if (!VN_IS(VN_AS(nodep, And)->lhsp(), Const))
      return false;
    if (!VN_IS(VN_AS(nodep, And)->rhsp(), ShiftR))
      return false;
    const AstShiftR *shiftp = VN_AS(VN_AS(nodep, And)->rhsp(), ShiftR);
    if (!VN_IS(shiftp->rhsp(), Const))
      return false;
    if (static_cast<uint32_t>(nodep->width()) <=
        VN_AS(shiftp->rhsp(), Const)->toUInt()) {
      return false;
    }
    return true;
  }
  void replaceBoolShift(AstNode *nodep) {
    ...
    AstAnd *newp = new AstAnd(nodep->fileline(),
                              new AstConst(nodep->fileline(), val), fromp);
  ...
  void replaceWithSimulation(AstNode *nodep) {
    ...
    if (!simvis.optimizable()) {
      ...
      if (!errorp)
        errorp = nodep;
      nodep->v3error("Expecting expression to be constant, but can't determine "
                     "constant for "
                     << nodep->prettyTypeName() << '\n'
                     << errorp->warnOther() << "... Location of non-constant "
                     << errorp->prettyTypeName() << ": "
                     << simvis.whyNotMessage());
      VL_DO_DANGLING(replaceZero(nodep), nodep);
    } else {
      // Fetch the result
      AstNode *valuep =
          simvis.fetchValueNull(nodep); // valuep is owned by Simulate
  ...
  int operandConcatMove(AstConcat *nodep) {
    ...
    if (bcConcp) {
      ...
      if (VL_BITBIT_I(ap->width() + bp->width()) == 0)
        return 2; // Transform 2: to abConc
    } else {      // abConcp
      // Unless lhs is already 32 bits due to above, reorder it
      if (VL_BITBIT_I(nodep->lhsp()->width()) != 0)
        return 1; // Transform 1: to bcConc
    }
    return 0; // ok
  }
  void moveConcat(AstConcat *nodep) {
    ...
    // putting additional CONCATs on the RHS leads to fewer assembler
    // operations. However, we'll end up with lots of wide moves if we make huge
    // trees like that, so on 32 bit boundaries, we'll do the opposite form.
  ...
  void replaceLogEq(AstLogEq *nodep) {
    // LOGEQ(a,b) =>
    // AstLogAnd{AstLogOr{AstLogNot{a},b},AstLogOr{AstLogNot{b},a}}
    AstNode *lhsp = nodep->lhsp()->unlinkFrBack();
    AstNode *rhsp = nodep->rhsp()->unlinkFrBack();
    // Do exactly as IEEE says, might result in extra terms, so in future may do
    // differently
    AstLogAnd *newp = new AstLogAnd(
        nodep->fileline(),
        new AstLogOr(nodep->fileline(), new AstLogNot(nodep->fileline(), lhsp),
                     rhsp),
  ...
  void replaceSelSel(AstSel *nodep) {
    ...
    if (VN_IS(lsb1p, Const) && VN_IS(lsb2p, Const)) {
      newlsbp =
          new AstConst(lsb1p->fileline(), VN_AS(lsb1p, Const)->toUInt() +
      ...
      newlsbp = new AstAdd(lsb1p->fileline(), lsb2p,
  ...
  bool operandRepRep(AstReplicate *nodep) {
    ...
    cnt1p->unlinkFrBack();
    cnt2p->unlinkFrBack();
    AstReplicate *newp = new AstReplicate(nodep->fileline(), from2p,
                                          cnt1p->toUInt() * cnt2p->toUInt());
    newp->dtypeFrom(nodep);
    nodep->replaceWith(newp);
    VL_DO_DANGLING(nodep->deleteTree(), nodep);
    return true;
  ...
  bool operandConcatSame(AstConcat *nodep) {
    ...
    if (!operandsSame(from1p, from2p))
      return false;
    //
    from1p->unlinkFrBack();
    AstReplicate *newp =
        new AstReplicate(nodep->fileline(), from1p, cnt1 + cnt2);
    newp->dtypeFrom(nodep);
    nodep->replaceWith(newp);
    VL_DO_DANGLING(nodep->deleteTree(), nodep);
    return true;
  ...
  void replaceSelIntoBiop(AstSel *nodep) {
    ...
    //
    fromp->lhsp(new AstSel(nodep->fileline(), bilhsp, lsbp->cloneTree(true),
  ...
  virtual void visit(AstArraySel *nodep) override {
    ...
    if (VN_IS(nodep->bitp(), Const) &&
        ...
        // Need to make sure it's an array object so don't mis-allow a constant
        // (bug509.)
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    ...
    if (m_doV && nodep->varp()->valuep() && !m_attrp) {
      ...
      iterateAndNextNull(
          nodep->varp()->valuep()); // May change nodep->varp()->valuep()
      ...
      if (nodep->access().isReadOnly() &&
        ...
        if (operandConst(valuep)) {
          ...
          // Earlier recursion of InitArray made sure each array value is
          // constant This exception is fairly fragile, i.e. doesn't support
          // arrays of arrays or other stuff
          AstNode *newp = valuep->cloneTree(false);
          nodep->replaceWith(newp);
          VL_DO_DANGLING(nodep->deleteTree(), nodep);
          did = true;
    ...
    if (!did && m_required) {
      nodep->v3error(
          "Expecting expression to be constant, but variable isn't const: "
  ...
  virtual void visit(AstEnumItemRef *nodep) override {
    ...
    if (nodep->itemp()->valuep()) {
      ...
      if (nodep->itemp()->user4()) {
        nodep->v3error(
            "Recursive enum value: " << nodep->itemp()->prettyNameQ());
      } else {
    ...
    if (!did && m_required) {
      nodep->v3error(
          "Expecting expression to be constant, but enum value isn't const: "
    ...
    if (m_doNConst &&
      ...
      if (nodep->isClocked()) { // A constant can never get a pos/negedge
        if (onlySenItemInSenTree(nodep)) {
          nodep->replaceWith(
              new AstSenItem(nodep->fileline(), AstSenItem::Never()));
          VL_DO_DANGLING(nodep->deleteTree(), nodep);
        } else {
          VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
        }
      ...
      if (invert)
        nodep->edgeType(nodep->edgeType().invert());
  ...
  struct SenItemCmp {
    bool operator()(const AstSenItem *lhsp, const AstSenItem *rhsp) const {
      ...
      if (lhsp->varrefp() && !rhsp->varrefp())
        return false;
      if (lhsp->varrefp() && rhsp->varrefp()) {
        if (lhsp->varrefp()->name() < rhsp->varrefp()->name())
          return true;
        if (lhsp->varrefp()->name() > rhsp->varrefp()->name())
          return false;
        // But might be same name with different scopes
        if (lhsp->varrefp()->varScopep() < rhsp->varrefp()->varScopep())
          return true;
        if (lhsp->varrefp()->varScopep() > rhsp->varrefp()->varScopep())
          return false;
        // Or rarely, different data types
        if (lhsp->varrefp()->dtypep() < rhsp->varrefp()->dtypep())
          return true;
        if (lhsp->varrefp()->dtypep() > rhsp->varrefp()->dtypep())
          return false;
      }
      // Sort by edge, AFTER variable, as we want multiple edges for same var
      // adjacent. note the SenTree optimizer requires this order (more general
      // first, less general last)
      if (lhsp->edgeType() < rhsp->edgeType())
        return true;
      if (lhsp->edgeType() > rhsp->edgeType())
        return false;
      return false;
  ...
  virtual void visit(AstSenTree *nodep) override {
    ...
    if (m_doExpensive) {
      ...
      //   SENTREE(... SENGATE(x,a), SENGATE(SENITEM(x),b) ...)  =>
      //   SENGATE(x,OR(a,b))
      ...
      // Sort the sensitivity names so "posedge a or b" and "posedge b or a" end
      // up together. Also, remove duplicate assignments, and fold POS&NEGs into
      // ANYEDGEs Make things a little faster; check first if we need a sort
        ...
        if (ritemp) {
          if ((litemp->varrefp() && ritemp->varrefp() &&
            ...
            if ((litemp->edgeType() ==
                ...
                || (litemp->edgeType() ==
                        VEdgeType::ET_POSEDGE // POS  or NEG -> BOTH
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    iterateChildren(nodep);
    if (m_doNConst && replaceNodeAssign(nodep))
      return;
  }
  ...
  virtual void visit(AstAssignW *nodep) override {
    ...
    AstNodeVarRef *varrefp =
        VN_CAST(nodep->lhsp(),
                VarRef); // Not VarXRef, as different refs may set different
                         // values to each hierarchy
    if (m_wremove && !m_params && m_doNConst && m_modp &&
        operandConst(nodep->rhsp()) &&
        !VN_AS(nodep->rhsp(), Const)->num().isFourState() &&
      ...
      VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
      // Set the initial value right in the variable so we can constant
      // propagate
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    if (m_doNConst) {
      if (const AstConst *constp = VN_CAST(nodep->condp(), Const)) {
        ...
        UINFO(4, "IF({a}) ASSIGN({b},{c}) else ASSIGN({b},{d}) => ASSIGN({b}, "
                 "{a}?{c}:{d})\n");
  ...
  virtual void visit(AstDisplay *nodep) override {
    // DISPLAY(SFORMAT(text1)),DISPLAY(SFORMAT(text2)) ->
    // DISPLAY(SFORMAT(text1+text2))
    iterateChildren(nodep);
    if (stmtDisplayDisplay(nodep))
      return;
  }
  bool stmtDisplayDisplay(AstDisplay *nodep) {
    ...
    // DISPLAY(SFORMAT(text1+text2))
    if (!m_modp)
      return false; // Don't optimize under single statement
    ...
    if ((prevp->filep() && !nodep->filep()) ||
      ...
      return false;
    if (!prevp->fmtp() || prevp->fmtp()->nextp() || !nodep->fmtp() ||
        nodep->fmtp()->nextp())
      return false;
    ...
    if (!nformatp)
      return false;
    // We don't merge scopeNames as can have only one and might be different
    // scopes (late in process) Also rare for real code to print %m multiple
    // times in same message
    if (nformatp->scopeNamep() && pformatp->scopeNamep())
      return false;
    // We don't early merge arguments as might need to later print warnings with
    // right line numbers, nor scopeNames as might be different scopes (late in
    // process)
    if (!m_doCpp && pformatp->exprsp())
      return false;
    if (!m_doCpp && nformatp->exprsp())
      return false;
    // Avoid huge merges
    static constexpr int DISPLAY_MAX_MERGE_LENGTH = 500;
    if (pformatp->text().length() + nformatp->text().length() >
        DISPLAY_MAX_MERGE_LENGTH)
      return false;
    //
    UINFO(9,
          "DISPLAY(SF({a})) DISPLAY(SF({b})) -> DISPLAY(SF({a}+{b}))" << endl);
    ...
    if (!prevp->addNewline() && nodep->addNewline())
      pformatp->text(pformatp->text() + "\n");
    if (nformatp->exprsp())
      pformatp->addExprsp(nformatp->exprsp()->unlinkFrBackWithNext());
  ...
  virtual void visit(AstSFormatF *nodep) override {
    ...
    // This eliminates a pile of wide temps, and makes the C a whole lot more
    // readable.
    iterateChildren(nodep);
    bool anyconst = false;
          ...
          default: // Most operators, just move to next argument
            if (argp) {
              ...
              if (VN_IS(argp, Const)) { // Convert it
                const string out =
                    VN_AS(argp, Const)->num().displayed(nodep, fmt);
    ...
    if (!nodep->exprsp() && nodep->name().find('%') == string::npos &&
        !nodep->hidden()) {
  ...
  virtual void visit(AstFuncRef *nodep) override {
    iterateChildren(nodep);
    if (m_params) { // Only parameters force us to do constant function call
                    // propagation
  ...
  // If output of a presel didn't get consted, chances are V3Param didn't visit
  // properly
  ...
  virtual void visit(AstJumpGo *nodep) override {
    ...
    if (m_doExpensive) {
      ...
      // Often caused by "return" in a Verilog function.  The Go is pointless,
      // remove.
      if (!nodep->nextp()) {
  ...
  virtual void visit(AstJumpBlock *nodep) override {
    ...
    // Note this assumes all AstJumpGos are underneath the given label; V3Broken
    // asserts this
    iterateChildren(nodep);
    ...
    if (m_doExpensive && !nodep->user4()) {
      ...
      if (nodep->stmtsp())
        underp = nodep->stmtsp()->unlinkFrBackWithNext();
      if (underp) {
  ...
  //  TREE_SKIP_VISIT("AstNODETYPE")    # Rename normal visit to visitGen and
  //  don't iterate
  //-----
  // clang-format off
  ...
  // Note we can't convert EqCase/NeqCase to Eq/Neq here because that would
  // break 3'b1x1==3'b101
  ...
  virtual void visit(AstNode *nodep) override {
    ...
    if (m_required) {
      if (VN_IS(nodep, NodeDType) || VN_IS(nodep, Range)) {
        ...
        nodep->v3error(
            "Expecting expression to be constant, but can't convert a "
...
public:
  ...
  ConstVisitor(ProcMode pmode, bool globalPass)
      : m_globalPass{globalPass},
        m_concswapNames{
            globalPass ? ("__Vconcswap_" + cvtToStr(s_globalPassNum++)) : ""} {
    // clang-format off
        switch (pmode) {
  ...
  virtual ~ConstVisitor() override {
    if (m_doCpp) {
      V3Stats::addStat("Optimizations, Const bit op reduction",
                       m_statBitOpReduction);
    }
  }
...
AstNode *V3Const::constifyParamsEdit(AstNode *nodep) {
  ...
  if (AstVar *varp = VN_CAST(nodep, Var)) {
    ...
    if (varp->valuep())
      visitor.mainAcceptEdit(varp->valuep());
  } else {
...
void V3Const::constifyAllLint(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("const", 0,
...
void V3Const::constifyCpp(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("const_cpp", 0,
                                v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}
...
void V3Const::constifyAllLive(AstNetlist *nodep) {
  ...
  // IE doesn't prune dead statements, as we need to do some usability checks
  // after this
  UINFO(2, __FUNCTION__ << ": " << endl);
  {
