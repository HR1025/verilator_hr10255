...
//      Copy all width() values to widthMin() so RANGE, etc can still see orig
//      widths
//
//*************************************************************************
...
private:
  ...
  //  AstNode::user2()        -> bool.  True indicates widthMin has been
  //  propagated AstNodeDType::user3()   -> AstNodeDType*.  Alternative node
  //  with C size
  ...
  void setClean(AstNode *nodep, bool isClean) {
    ...
    const bool wholeUint = (nodep->widthMin() == VL_IDATASIZE ||
  ...
  void insertClean(AstNode *nodep) { // We'll insert ABOVE passed node
    ...
    AstNode *cleanp = new AstAnd(nodep->fileline(),
                                 new AstConst(nodep->fileline(), mask), nodep);
...
void V3Clean::cleanAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("clean", 0,
