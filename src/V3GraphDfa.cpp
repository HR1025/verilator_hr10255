...
DfaVertex *DfaGraph::findStart() {
  ...
  if (!startp)
    v3fatalSrc("No start point in NFA graph");
  return startp;
}
...
class GraphNfaToDfa final : GraphAlg<> {
  ...
  // Edges from NFA to NFA come from the user, and indicate input or epsilon
  // transitions Edges from DFA to NFA indicate the NFA from which that DFA was
  // formed. Edges from DFA to DFA indicate a completed input transition
private:
  // TYPES
  ...
  DfaVertex *newDfaVertex(DfaVertex *nfaTemplatep = nullptr) {
    ...
    if (nfaTemplatep && nfaTemplatep->start())
      vertexp->start(true);
    if (nfaTemplatep && nfaTemplatep->accepting())
      vertexp->accepting(true);
  ...
  uint32_t hashDfaOrigins(DfaVertex *dfaStatep) {
    ...
    // Record a checksum, so we can search for it later by the list of nfa
    // nodes. The order of the nodes is not deterministic; the hash thus must
  ...
  bool compareDfaOrigins(const DfaStates &nfasWithInput, DfaVertex *dfa2p) {
    ...
    if (!num1s)
      v3fatalSrc("DFA node construction that contains no NFA states");
  ...
  void findNfasWithInput(DfaVertex *dfaStatep, const DfaInput &input,
  ...
  void main() {
    ...
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("dfa_nfa");
    ...
    while (!workps.empty()) { // While work
      ...
      for (V3GraphEdge *nfaEdgep = nfaStatep->outBeginp(); nfaEdgep;
        ...
        // UINFO(9,"   Consider "<<nfaEdgep->top()<<" EP
        // "<<cNfaEdgep->epsilon()<<endl);
        if (cNfaEdgep->epsilon() &&
    ...
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("dfa_start");
    ...
    while (!dfaUnprocps.empty()) {
      ...
      for (int inIt : inputs) {
        ...
        if (debug() >= 6)
          m_graphp->dumpDotFilePrefixed("step");
      }
    }
    ...
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("dfa_withnfa");
    ...
    UINFO(5, "Done.\n");
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("dfa_done");
  }
...
private:
  ...
  bool isDead(DfaVertex *vertexp) {
    ...
    if (vertexp->accepting() || vertexp->start())
      return false;
    for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
      if (edgep->top() != vertexp)
        return false;
    }
    return true;
  }
    ...
    for (V3GraphVertex *vertexp = m_graphp->verticesBeginp(); vertexp;
      ...
      if (DfaVertex *vvertexp = dynamic_cast<DfaVertex *>(vertexp)) {
        if (vvertexp->accepting()) {
          for (V3GraphEdge *nextp, *edgep = vertexp->outBeginp(); edgep;
               edgep = nextp) {
  ...
  void optimize_orphans() {
    ...
    // (Otherwise we might have nodes on the list twice, and reference after
    // deleting them.)
    m_graphp->userClearVertices();
...
public:
  DfaGraphReduce(V3Graph *graphp, V3EdgeFuncP edgeFuncp)
      : GraphAlg<>{graphp, edgeFuncp} {
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("opt_in");
    optimize_accepting_out();
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("opt_acc");
    optimize_orphans();
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("opt_orph");
    optimize_no_outbound();
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("opt_noout");
  }
...
private:
  ...
  void add_complement_edges() {
    ...
    if (!acceptp)
      v3fatalSrc("No accepting vertex in DFA");
...
public:
  DfaGraphComplement(V3Graph *dfagraphp, V3EdgeFuncP edgeFuncp)
    ...
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("comp_in");
    ...
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("comp_preswap");

    VL_DO_CLEAR(m_tempNewerReject->unlinkDelete(graphp()),
                m_tempNewerReject = nullptr);
    if (debug() >= 6)
      m_graphp->dumpDotFilePrefixed("comp_out");
  }
