...
//
//*************************************************************************
// V3SplitVar divides a variable into multiple variables to avoid UNOPTFLAT
// warning and get better perfomance. Variables to be split must be marked by
// /*verilator split_var*/ metacomment. There are sveral kinds of data types
// that may cause the warning. 1) Unpacked arrays 2) Packed arrays 3) Unpacked
// structs 4) Packed structs 5) Bitfields within a signal. (Especially Verilog
// code predating structs/2D arrays.) 2-5 above are treated as bitfields in
// verilator.
//
...
//     always_comb begin
//        unpacked_array_var[1][0] =  unpacked_array_var[0][0]; // UNOPTFLAT
//        warning unpacked_array_var[1][1] = ~unpacked_array_var[0][1]; //
//        UNOPTFLAT warning
//     end
...
//     always_comb begin
//        unpacked_array_var1__BRA__0__KET__ =
//        unpacked_array_var0__BRA__0__KET__; unpacked_array_var1__BRA__1__KET__
//        = ~unpacked_array_var0__BRA__1__KET__;
//     end
...
//
//
// Two visitor classes are defined here, SplitUnpackedVarVisitor and
// SplitPackedVarVisitor.
//
// - SplitUnpackedVarVisitor class splits unpacked arrays. ( 1) in the
// explanation above.)
...
//   In addition to splitting unpacked arrays, the visitor collects the
//   following information for each module.
//     - AstVar
//     - AstVarRef
//     - AstSel
//   They are stored in a RefsInModule instance and will be used in
//   SplitPackedVarVisitor.
//
// - SplitPackedVarVisitor class splits packed variables ( 2), 3), 4), and 5) in
// the explanation
//   above.)
//   "unpacked_array0", "unpacked_array_var1", and "packed_var" in
//   "Intermediate" are split by the class. Packed variables here include the
//   result of SplitUnpackedVarVisitor. The result of this class looks like
//   "Final" above. The class visits just necessary AstNode based on
//   RefsInModule collected in the preceding SplitUnpackedVarVisitor. The
//   visitor does not have to visit the entire AST because the necessary
//   information is already in RefsInModule.
//
//*************************************************************************
...
struct SplitVarImpl {
  // NODE STATE
  //  AstNodeModule::user1()  -> Block number counter for generating unique
  //  names
  AstUser1InUse m_user1InUse; // Only used in SplitUnpackedVarVisitor

  static AstNodeAssign *newAssign(FileLine *fileline, AstNode *lhsp,
                                  AstNode *rhsp, const AstVar *varp) {
  ...
  // These check functions return valid pointer to the reason text if a variable
  // cannot be split.

  // Check if a var type can be split
  static const char *cannotSplitVarTypeReason(AstVarType type) {
    // Only SplitUnpackedVarVisitor can split WREAL. SplitPackedVarVisitor
    // cannot.
    const bool ok = type == type.VAR || type == type.WIRE ||
                    type == type.PORT || type == type.WREAL;
    if (ok)
      return nullptr;
  ...
  static const char *cannotSplitVarDirectionReason(VDirection dir) {
    if (dir == VDirection::REF)
      return "it is a ref argument";
    if (dir == VDirection::INOUT)
      return "it is an inout port";
    return nullptr;
  }
  ...
  static const char *cannotSplitConnectedPortReason(const AstPin *pinp) {
    ...
    if (const char *const reason =
            cannotSplitVarDirectionReason(varp->direction())) {
      return reason;
    }
    return nullptr;
  }
  ...
  static const char *cannotSplitTaskReason(const AstNodeFTask *taskp) {
    if (taskp->prototype())
      return "the task is prototype declaration";
    if (taskp->dpiImport())
      return "the task is imported from DPI-C";
    if (taskp->dpiOpenChild())
      return "the task takes DPI-C open array";
    return nullptr;
  }
  ...
  static const char *cannotSplitVarCommonReason(const AstVar *varp) {
    if (const AstNodeFTask *const taskp = VN_CAST(varp->backp(), NodeFTask)) {
      if (const char *const reason = cannotSplitTaskReason(taskp))
        return reason;
    }
  ...
  void insertBeginCore(T_ALWAYSLIKE *ap, AstNodeStmt *stmtp,
                       AstNodeModule *modp) {
    if (ap->isJustOneBodyStmt() && ap->bodysp() == stmtp) {
      stmtp->unlinkFrBack();
      // Insert begin-end because temp value may be inserted to this block
      // later.
  ...
  void insertBeginCore(AstInitial *initp, AstNodeStmt *stmtp,
                       AstNodeModule *modp) {
...
static void warnNoSplit(const AstVar *varp, const AstNode *wherep,
                        const char *reasonp) {
  wherep->v3warn(
      SPLITVAR,
      varp->prettyNameQ()
...
class UnpackRef final {
  // m_nodep is called in this context (AstNodeStmt, AstCell, AstNodeFTask, or
  // AstAlways)
  AstNode *m_contextp;
  ...
  VAccess m_access;
  bool
      m_ftask; // true if the reference is in function/task. false if in module.
public:
  UnpackRef(AstNode *stmtp, AstVarRef *nodep, bool ftask)
      : m_contextp{stmtp}, m_nodep{nodep}, m_index{-1}, m_msb{0}, m_lsb{1},
        m_access{nodep->access()}, m_ftask{ftask} {}
  UnpackRef(AstNode *stmtp, AstArraySel *nodep, int idx, const VAccess &access,
            bool ftask)
      : m_contextp{stmtp}, m_nodep{nodep}, m_index{idx}, m_msb{0}, m_lsb{1},
        m_access{access}, m_ftask{ftask} {}
  UnpackRef(AstNode *stmtp, AstSliceSel *nodep, int msb, int lsb,
            const VAccess &access, bool ftask)
      : m_contextp{stmtp}, m_nodep{nodep}, m_index{msb == lsb ? msb : -1}
        // Equivalent to ArraySel
        ,
        m_msb{msb}, m_lsb{lsb}, m_access{access}, m_ftask{ftask} {}
...
public:
  ...
  void registerVar(AstVar *varp) {
    const bool inserted =
        m_map.insert(std::make_pair(varp, MapType::value_type::second_type()))
            .second;
    UASSERT_OBJ(inserted, varp, "already registered");
  }
  // Register the location where a variable is used.
  bool tryAdd(AstNode *context, AstVarRef *refp, AstArraySel *selp, int idx,
              bool ftask) {
    return addCore(refp, UnpackRef(context, selp, idx, refp->access(), ftask));
  }
  bool tryAdd(AstNode *context, AstVarRef *refp, AstSliceSel *selp, int msb,
              int lsb, bool ftask) {
    return addCore(refp,
                   UnpackRef(context, selp, msb, lsb, refp->access(), ftask));
  }
...
public:
  ...
  void remove(AstNode *nodep) {
    struct Visitor : public AstNVisitor {
      ...
      virtual void visit(AstVar *nodep) override {
        m_parent.m_vars.erase(nodep);
      }
      virtual void visit(AstVarRef *nodep) override {
        m_parent.m_refs.erase(nodep);
      }
  ...
  void visit(AstNVisitor *visitor) {
    for (AstVar *const varp : m_vars)
      visitor->iterate(varp);
    for (AstSel *const selp : m_sels) {
      // If m_refs includes VarRef from ArraySel, remove it
      // because the VarRef would not be visited in
      // SplitPackedVarVisitor::visit(AstSel*).
      ...
      UASSERT_OBJ(reinterpret_cast<uintptr_t>(selp->op1p()) != 1, selp,
                  "stale");
      visitor->iterate(selp);
    }
    for (AstVarRef *const vrefp : m_refs) {
      UASSERT_OBJ(reinterpret_cast<uintptr_t>(vrefp->op1p()) != 1, vrefp,
                  "stale");
...
using SplitVarRefsMap =
    std::map<AstNodeModule *, RefsInModule, AstNodeComparator>;
...
class SplitUnpackedVarVisitor final : public AstNVisitor, public SplitVarImpl {
  ...
  AstVar *newVar(FileLine *fl, AstVarType type, const std::string &name,
                 AstNodeDType *dtp) {
  ...
  virtual void visit(AstNodeStmt *nodep) override {
    setContextAndIterateChildren(nodep);
  }
  virtual void visit(AstCell *nodep) override {
    setContextAndIterateChildren(nodep);
  }
  virtual void visit(AstAlways *nodep) override {
    if (nodep->sensesp()) { // When visiting sensitivity list, always is the
                            // context
  ...
  virtual void visit(AstArraySel *nodep) override {
    if (AstVarRef *const refp = isTargetVref(nodep->fromp())) {
      ...
      if (indexp) { // OK
        UINFO(4,
              "add " << nodep << " for " << refp->varp()->prettyName() << "\n");
        ...
        warnNoSplit(refp->varp(), nodep->bitp(),
  ...
  virtual void visit(AstSliceSel *nodep) override {
    if (AstVarRef *const refp = isTargetVref(nodep->fromp())) {
      ...
      // declRange() of AstSliceSel is shifted by dtypep->declRange().lo() in
      // V3WidthSel.cpp restore the original decl range here.
      const VNumRange selRange{
          nodep->declRange().hi() + dtypep->declRange().lo(),
          nodep->declRange().lo() + dtypep->declRange().lo(),
          nodep->declRange().littleEndian()};
      UASSERT_OBJ(
          dtypep->lo() <= selRange.lo() && selRange.hi() <= dtypep->hi(), nodep,
  ...
  AstNode *toInsertPoint(AstNode *insertp) {
    if (AstNodeStmt *const stmtp = VN_CAST(insertp, NodeStmt)) {
      if (!stmtp->isStatement())
        insertp = stmtp->backp();
    }
    return insertp;
  }
  AstVarRef *createTempVar(AstNode *context, AstNode *nodep,
                           AstUnpackArrayDType *dtypep,
                           const std::string &name_prefix,
                           std::vector<AstVar *> &vars, int start_idx,
                           bool lvalue, bool ftask) {
    ...
    UINFO(3, varp->prettyNameQ() << " is created lsb:" << dtypep->lo()
                                 << " msb:" << dtypep->hi() << "\n");
    // Use AstAssign if true, otherwise AstAssignW
    const bool use_simple_assign = (context && VN_IS(context, NodeFTaskRef)) ||
                                   (assignp && VN_IS(assignp, Assign));

    for (int i = 0; i < dtypep->elementsConst(); ++i) {
      AstNode *lhsp = newVarRef(fl, vars.at(start_idx + i),
      ...
      if (use_simple_assign) {
        ...
        if (lvalue) {
          ...
          // If varp is RHS, this assignment comes just before the original
          // assignment
  ...
  void connectPort(AstVar *varp, std::vector<AstVar *> &vars,
    ...
    for (size_t i = 0; i < vars.size(); ++i) {
      AstNode *const nodes[] = {
          new AstArraySel{
              fl, newVarRef(fl, varp, lvalue ? VAccess::WRITE : VAccess::READ),
              static_cast<int>(i)},
  ...
  size_t collapse(UnpackRefMap &refs) {
    ...
    for (const auto &pair : refs) {
      UINFO(3, "In module " << m_modp->name() << " var "
                            << pair.first->prettyNameQ() << " which has "
                            << pair.second.size() << " refs will be split.\n");
      ...
      const bool needNext =
          VN_IS(subTypep, UnpackArrayDType); // Still unpacked array.
      ...
      for (vlsint32_t i = 0; i < dtypep->elementsConst(); ++i) {
        ...
        const std::string name =
            varp->name() +
            AstNode::encodeName('(' + cvtToStr(i + dtypep->lo()) + ')');
        AstVar *newp =
            newVar(varp->fileline(), AstVarType::VAR, name, subTypep);
      ...
      for (const UnpackRef &ref : pair.second) {
        ...
        if (ref.isSingleRef()) {
          newp = newVarRef(ref.nodep()->fileline(), vars.at(ref.index()),
                           ref.access());
        } else {
          ...
          AstVarRef *const newrefp =
              createTempVar(ref.context(), refp, adtypep, varp->name(), vars,
                            lsb, refp->access(), ref.ftask());
          newp = newrefp;
          refp->varp()->addNextHere(newrefp->varp());
          UINFO(3, "Create " << newrefp->varp()->prettyNameQ() << " for "
                             << refp << "\n");
        }
...
public:
  ...
  const SplitVarRefsMap &getPackedVarRefs() const {
    return m_refsForPackedSplit;
  }
  ...
  static const char *cannotSplitReason(const AstVar *nodep) {
    const std::pair<uint32_t, uint32_t> dim =
        nodep->dtypep()->dimensions(false);
    UINFO(7, nodep->prettyNameQ() << " pub:" << nodep->isSigPublic()
                                  << " pri:" << nodep->isPrimaryIO()
                                  << " io:" << nodep->isInoutish()
                                  << " typ:" << nodep->varType() << "\n");
    const char *reason = nullptr;
    ...
    if (!reason)
      reason = cannotSplitVarCommonReason(nodep);
    if (reason) {
      UINFO(5, "Check " << nodep->prettyNameQ() << " cannot split because"
                        << reason << ".\n");
    }
    return reason;
  }
...
public:
  SplitNewVar(int lsb, int bitwidth, AstVar *varp = nullptr)
      : m_lsb{lsb}, m_bitwidth{bitwidth}, m_varp{varp} {}
...
public:
  PackedVarRefEntry(AstSel *selp, int lsb, int bitwidth)
      : m_nodep{selp}, m_lsb{lsb}, m_bitwidth{bitwidth} {}
  PackedVarRefEntry(AstVarRef *refp, int lsb, int bitwidth)
      : m_nodep{refp}, m_lsb{lsb}, m_bitwidth{bitwidth} {}
  AstNode *nodep() const { return m_nodep; }
  int lsb() const { return m_lsb; }
  int msb() const { return m_lsb + m_bitwidth - 1; }
  int bitwidth() const { return m_bitwidth; }
...
class PackedVarRef final {
  struct SortByFirst {
    bool operator()(const std::pair<int, bool> &a,
                    const std::pair<int, bool> &b) const {
      if (a.first == b.first)
  ...
  AstBasicDType
      *m_basicp; // Cache the ptr since varp->dtypep()->basicp() is expensive
...
public:
  ...
  explicit PackedVarRef(AstVar *varp) : m_basicp{varp->dtypep()->basicp()} {}
  ...
  std::vector<SplitNewVar> splitPlan(bool skipUnused) const {
    ...
    points.reserve(m_lhs.size() * 2 +
                   2); // 2 points will be added per one PackedVarRefEntry
    for (const PackedVarRefEntry &ref : m_lhs) {
      points.emplace_back(
          std::make_pair(ref.lsb(), false)); // Start of a region
      points.emplace_back(
          std::make_pair(ref.msb() + 1, true)); // End of a region
    }
    if (skipUnused &&
        !m_rhs.empty()) { // Range to be read must be kept, so add points here
      ...
      UASSERT_OBJ(lsb <= msb, m_basicp,
                  "lsb:" << lsb << " msb:" << msb << " are wrong");
    ...
    for (size_t i = 0; i + 1 < points.size(); ++i) {
      ...
      if (bitwidth == 0 || refcount == 0)
        continue; // Vacant region
...
class SplitPackedVarVisitor final : public AstNVisitor, public SplitVarImpl {
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (const char *const reason = cannotSplitReason(nodep, true)) {
      ...
      const bool inserted =
          m_refs.insert(std::make_pair(nodep, PackedVarRef(nodep))).second;
      if (inserted)
        UINFO(3, nodep->prettyNameQ() << " is added to candidate list.\n");
    }
  }
  virtual void visit(AstVarRef *nodep) override {
    ...
    UASSERT_OBJ(varp->attrSplitVar(), varp,
                "split_var attribute must be attached");
    ...
    refit->second.append(PackedVarRefEntry(nodep, basicp->lo(), varp->width()),
                         nodep->access());
    UINFO(5, varp->prettyName() << " Entire bit of [" << basicp->lo()
                                << "+:" << varp->width() << "] \n");
  }
  virtual void visit(AstSel *nodep) override {
    ...
    if (consts[0] && consts[1]) {           // OK
      refit->second.append(
          PackedVarRefEntry(nodep,
                            consts[0]->toSInt() + refit->second.basicp()->lo(),
                            consts[1]->toUInt()),
          vrefp->access());
      ...
      warnNoSplit(vrefp->varp(), nodep,
                  "its bit range cannot be determined statically");
      if (!consts[0]) {
        UINFO(4, "LSB " << nodep->lsbp()
                        << " is expected to be constant, but not\n");
      }
      if (!consts[1]) {
        UINFO(4, "WIDTH " << nodep->widthp()
                          << " is expected to be constant, but not\n");
      }
      m_refs.erase(varp);
  ...
  static AstNode *extractBits(const PackedVarRefEntry &ref,
                              const SplitNewVar &var, const VAccess access) {
    FileLine *const fl = ref.nodep()->fileline();
    AstVarRef *const refp = new AstVarRef{fl, var.varp(), access};
    if (ref.lsb() <= var.lsb() &&
        var.msb() <= ref.msb()) { // Use the entire bits
      return refp;
    } else { // Use slice
      ...
      // LSB of varp is always 0. "lsb - var.lsb()" means this. see also
      // SplitNewVar
      return new AstSel{fl, refp, lsb - var.lsb(), bitwidth};
    }
  }
  static void connectPortAndVar(const std::vector<SplitNewVar> &vars,
                                AstVar *portp, AstNode *insertp) {
    ...
    for (const SplitNewVar &var : vars) {
      AstNode *rhsp = new AstSel{
          fl, new AstVarRef{fl, portp, !in ? VAccess::WRITE : VAccess::READ},
          var.lsb(), var.bitwidth()};
      AstNode *lhsp =
          new AstVarRef{fl, var.varp(), in ? VAccess::WRITE : VAccess::READ};
      if (!in)
        std::swap(lhsp, rhsp);
  ...
  void createVars(AstVar *varp, const AstBasicDType *basicp,
                  std::vector<SplitNewVar> &vars) {
    for (SplitNewVar &newvar : vars) {
      int left = newvar.msb();
      int right = newvar.lsb();
      if (basicp->littleEndian())
        std::swap(left, right);
      const std::string name =
          (left == right)
      ...
      case AstBasicDTypeKwd::BIT:
        dtypep = new AstBasicDType{varp->subDTypep()->fileline(),
                                   VFlagBitPacked(), newvar.bitwidth()};
        break;
      case AstBasicDTypeKwd::LOGIC:
        dtypep = new AstBasicDType{varp->subDTypep()->fileline(),
                                   VFlagLogicPacked(), newvar.bitwidth()};
        break;
      default:
        UASSERT_OBJ(false, basicp, "Only bit and logic are allowed");
      }
      dtypep->rangep(
          new AstRange{varp->fileline(), VNumRange{newvar.msb(), newvar.lsb(),
                                                   basicp->littleEndian()}});
  ...
  static void updateReferences(AstVar *varp, PackedVarRef &ref,
    ...
    for (const bool lvalue : {false, true}) { // Refer the new split variables
      ...
      for (PackedVarRefEntry &ref : refs) {
        auto varit = std::upper_bound(vars.begin(), vars.end(), ref.lsb(),
                                      SplitNewVar::Match());
        ...
        if (AstSenItem *const senitemp = ref.backSenItemp()) {
          ...
          oldsenrefp->replaceWith(new AstVarRef{senitemp->fileline(),
                                                varit->varp(), VAccess::READ});
        ...
        for (int residue = ref.msb() - varit->msb(); residue > 0;
          ...
          ++varit;
          UASSERT_OBJ(varit != vars.end(), ref.nodep(),
          ...
          if (AstSenItem *const senitemp = VN_CAST(prevp, SenItem)) {
            prevp = new AstSenItem{senitemp->fileline(), senitemp->edgeType(),
                                   new AstVarRef{senitemp->fileline(),
                                                 varit->varp(), VAccess::READ}};
            senitemp->addNextHere(prevp);
          } else {
            AstNode *const bitsp = extractBits(
                ref, *varit, lvalue ? VAccess::WRITE : VAccess::READ);
            prevp = new AstConcat{ref.nodep()->fileline(), bitsp, prevp};
          }
        }
        // If varp is an argument of task/func, need to update temporary var
        // everytime the var is updated. See also another call of
        // connectPortAndVar() in split()
        if (varp->isIO() && (varp->isFuncLocal() || varp->isFuncReturn()))
          connectPortAndVar(vars, varp, ref.nodep());
        if (!inSentitivityList)
          ref.replaceNodeWith(prevp);
  ...
  void split() {
    for (auto &pair : m_refs) {
      ...
      UINFO(3, "In module " << m_modp->name() << " var " << varp->prettyNameQ()
                            << " which has " << ref.lhs().size()
                            << " lhs refs and " << ref.rhs().size()
                            << " rhs refs will be split.\n");
      std::vector<SplitNewVar> vars =
      ...
      if (varp->isIO()) { // port cannot be deleted
        ...
        AstNode *rhsp = new AstVarRef{vars.front().varp()->fileline(),
                                      vars.front().varp(), VAccess::READ};
        FileLine *const fl = varp->fileline();
        for (size_t i = 1; i < vars.size(); ++i) {
          rhsp = new AstConcat{
              fl, new AstVarRef{fl, vars[i].varp(), VAccess::READ}, rhsp};
        }
        varp->addNextHere(
...
public:
  ...
  static const char *cannotSplitReason(const AstVar *nodep,
    ...
    if (AstBasicDType *const basicp = nodep->dtypep()->basicp()) {
      ...
      if (!((!checkUnpacked || dim.second == 0) &&
            nodep->dtypep()->widthMin() > 1))
...
void V3SplitVar::splitVariable(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("split_var", 0,
                                v3Global.opt.dumpTreeLevel(__FILE__) >= 9);
