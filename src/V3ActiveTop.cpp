...
private:
  ...
  // STATE
  SenTreeFinder
      m_finder; // Find global sentree's / add them under the AstTopScope
  ...
  virtual void visit(AstActive *nodep) override {
    ...
    if (sensesp->hasCombo()) {
      ...
      if (nodep->stmtsp())
        newp->addStmtsp(nodep->stmtsp()->cloneTree(true));
    ...
    if (wantp != sensesp) {
      ...
      if (nodep->sensesStorep()) {
        UASSERT_OBJ(
            sensesp == nodep->sensesStorep(), nodep,
...
void V3ActiveTop::activeTopAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("activetop", 0,
