...
class V3UniqueNames final {
  ...
  std::unordered_map<std::string, unsigned>
      m_multiplicity; // Suffix number for given key

public:
  V3UniqueNames() : m_prefix{""} {}
  explicit V3UniqueNames(const std::string &prefix) : m_prefix{prefix} {}

  // Return argument, prepended with the prefix if any, then appended with a
  // unique suffix each time we are called with the same argument.
  ...
  // Return hash of node as string, prepended with the prefix if any, appended
  // with a unique suffix each time we are called with a node that hashes to the
  // same value.
