...
private:
  ...
  VSymEnt *m_symTableNextId =
      nullptr; // Symbol table for next lexer lookup (parser use only)
  VSymEnt *m_symCurrentp; // Active symbol table for additions/lookups
  std::vector<VSymEnt *>
      m_sympStack; // Stack of upper nodes with pending symbol tables

public:
  // CONSTRUCTORS
  explicit V3ParseSym(AstNetlist *rootp) : m_syms{rootp} {
...
public:
  ...
  void nextId(AstNode *entp) {
    if (entp) {
      UINFO(9, "symTableNextId under " << entp << "-" << entp->type().ascii()
                                       << endl);
  ...
  void pushNewUnder(AstNode *nodep, VSymEnt *parentp) {
    if (!parentp)
      parentp = symCurrentp();
    VSymEnt *symp = findNewTable(
        nodep); // Will set user4p, which is how we connect table to node
  ...
  void popScope(AstNode *nodep) {
    if (VL_UNCOVERABLE(symCurrentp()->nodep() != nodep)) { // LCOV_EXCL_START
      ...
      nodep->v3fatalSrc("Symbols suggest ending "
                        << symCurrentp()->nodep()->prettyTypeName()
  ...
  AstNode *findEntUpward(const string &name) const {
    // Lookup the given string as an identifier, return type of the id, scanning
    // upward
