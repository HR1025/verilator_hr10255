...
protected:
  ...
  virtual void visit(AstConst *nodep) override {
    ...
    if (num.isString()) {
      ...
      // Note the double {{ initializer. The first { starts the initializer of
      // the VlWide, and the second starts the initializer of m_storage within
      // the VlWide.
      puts("{");
      ofp()->putsNoTracking("{");
      if (m_inUnpacked)
        puts(" // VlWide " + cvtToStr(m_unpackedWord));
      puts("\n");
      for (uint32_t n = 0; n < size; ++n) {
        if (n)
          puts((n % 4) ? ", " : ",\n");
      ...
      const char *const fmt =
          !m_inUnpacked && (static_cast<int>(dnum) == dnum && -1000 < dnum &&
                            dnum < 1000)
      ...
      const char *const fmt = !m_inUnpacked && (qnum < 10)
                                  ? ("%" PRIx64 "ULL")
                                  : ("0x%016" PRIx64 "ULL");
      ofp()->printf(fmt, qnum);
    } else {
  ...
  virtual void visit(AstNode *nodep) override { // LCOV_EXCL_START
    nodep->v3fatalSrc("Unknown node type reached EmitCConstInit: "
