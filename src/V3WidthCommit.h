...
private:
  ...
  virtual void visit(AstSigned *nodep) override {
    VL_DO_DANGLING(
        replaceWithSignedVersion(nodep, nodep->lhsp()->unlinkFrBack()), nodep);
  }
...
public:
  ...
  static AstConst *newIfConstCommitSize(AstConst *nodep) {
    if (((nodep->dtypep()->width() != nodep->num().width()) ||
         !nodep->num().sized()) &&
        !nodep->num()
             .isString()) { // Need to force the number from unsized to sized
...
private:
  ...
  void classEncapCheck(AstNode *nodep, AstNode *defp, AstClass *defClassp) {
    ...
    if (const auto varp = VN_CAST(defp, Var)) {
      ...
      nodep->v3fatalSrc("ref to unhandled definition type "
                        << defp->prettyTypeName());
    }
    if (local || prot) {
      ...
      if (local && defClassp && refClassp != defClassp) {
        how = "'local'";
      } else if (prot && defClassp &&
                 !AstClass::isClassExtendedFrom(refClassp, defClassp)) {
        how = "'protected'";
      }
      if (how) {
        UINFO(9, "refclass " << refClassp << endl);
        UINFO(9, "defclass " << defClassp << endl);
        nodep->v3warn(E_ENCAPSULATED,
                      nodep->prettyNameQ()
                          << " is hidden as " << how
  ...
  void visitIterateNodeDType(AstNodeDType *nodep) {
    // Rather than use dtypeChg which may make new nodes, we simply edit in
    // place, as we don't need to preserve any widthMin's, and every dtype with
    // the same width gets an identical edit.
    if (nodep->user1SetOnce())
      return; // Process once
    nodep->widthMinFromWidth();
    // Too late to any unspecified sign to be anything but unsigned
    if (nodep->numeric().isNosign())
      nodep->numeric(VSigning::UNSIGNED);
    iterateChildren(nodep);
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    if (nodep->classMethod() && nodep->pureVirtual() && VN_IS(m_modp, Class) &&
      ...
      nodep->v3error("Illegal to have 'pure virtual' in non-virtual class "
                     "(IEEE 1800-2017 8.21)");
    }
  }
  virtual void visit(AstNodeVarRef *nodep) override {
    iterateChildren(nodep);
    editDType(nodep);
    classEncapCheck(nodep, nodep->varp(),
                    VN_CAST(nodep->classOrPackagep(), Class));
  }
  virtual void visit(AstNodeFTaskRef *nodep) override {
    iterateChildren(nodep);
    editDType(nodep);
    classEncapCheck(nodep, nodep->taskp(),
                    VN_CAST(nodep->classOrPackagep(), Class));
  }
