...
class EmitCModel final : public EmitCFunc {
  ...
  void emitHeader(AstNodeModule *modp) {
    ...
    const string filename =
        v3Global.opt.makeDir() + "/" + topClassName() + ".h";
    ...
    puts("//\n");
    puts("// This header should be included by all source files instantiating "
         "the design.\n");
    ...
    if (v3Global.opt.trace())
      puts("class " + v3Global.opt.traceClassLang() + ";\n");
    emitModCUse(
        modp,
        VUseType::INT_FWD_CLASS); // Note: This is needed for cell forwarding
    ...
    for (AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
      if (const AstCell *const cellp = VN_CAST(nodep, Cell)) {
        puts(prefixNameProtect(cellp->modp()) + "* const " +
             cellp->nameProtect() + ";\n");
      }
    }

    // root instance pointer (for access to internals, including public_flat
    // items).
    ...
    if (optSystemC()) {
      ...
      puts(
          "/// If contextp is null, then the model will use the default global "
          "context\n");
      ...
      puts("/// Destroy the model; called (often implicitly) by application "
           "code\n");
      puts("virtual ~" + topClassName() + "();\n");
    }
    ...
    if (!optSystemC()) {
      puts("/// Evaluate the model.  Application must call when inputs "
           "change.\n");
    }
    ...
    puts("void eval_step();\n");
    if (!optSystemC()) {
      puts("/// Evaluate at end of a timestep for tracing, when using "
           "eval_step().\n");
      puts("/// Application must call after all eval() and before time "
           "changes.\n");
    ...
    if (!optSystemC()) {
      puts("/// Retrieve name of this model instance (as passed to "
           "constructor).\n");
    ...
    // Emit DPI export dispatcher declarations
    {
      ...
      stable_sort(funcps.begin(), funcps.end(),
                  [](const AstNode *ap, const AstNode *bp) {
    ...
    if (v3Global.opt.savable()) {
      ...
      puts("friend VerilatedSerialize& operator<<(VerilatedSerialize& os, " //
           + topClassName() + "& rhs);\n");
      puts(
          "friend VerilatedDeserialize& operator>>(VerilatedDeserialize& os, " +
          topClassName() + "& rhs);\n");
    }
  ...
  void emitSettleLoop(AstNodeModule *modp, bool initial) {
    ...
    puts(" loop\\n\"););\n");
    if (initial)
      puts(topModNameProtected + "__" + protect("_eval_settle") +
           "(&(vlSymsp->TOP));\n");
    ...
    if (v3Global.opt.profThreads() && !initial) {
      puts("if (VL_UNLIKELY(" + recName + ")) " + recName +
           "->endRecord(VL_RDTSC_Q());\n");
    }

    if (v3Global.rootp()->changeRequest()) {
      puts("if (VL_UNLIKELY(++__VclockLoop > " +
           cvtToStr(v3Global.opt.convergeLimit()) + ")) {\n");
      puts("// About to fail, so enable debug to see what's not settling.\n");
      puts("// Note you must run make with OPT=-DVL_DEBUG for debug prints.\n");
      puts("int __Vsaved_debug = Verilated::debug();\n");
      puts("Verilated::debug(1);\n");
      puts("__Vchange = " + topModNameProtected + "__" +
           protect("_change_request") + "(&(vlSymsp->TOP));\n");
    ...
    puts("} while (" +
         (v3Global.rootp()->changeRequest()
              ? std::string{"VL_UNLIKELY(__Vchange)"}
              : std::string{"0"}) +
         ");\n");
  }

  void emitStandardMethods1(AstNodeModule *modp) {
    UASSERT_OBJ(modp->isTop(), modp,
                "Attempting to emitWrapEval for non-top class");
    ...
    puts("\n");
    puts("void " + topModNameProtected + "__" + protect("_eval_initial") +
         selfDecl + ";\n");
    puts("void " + topModNameProtected + "__" + protect("_eval_settle") +
         selfDecl + ";\n");
    puts("void " + topModNameProtected + "__" + protect("_eval") + selfDecl +
         ";\n");
    if (v3Global.rootp()->changeRequest()) {
      puts("QData " + topModNameProtected + "__" + protect("_change_request") +
           selfDecl + ";\n");
    }
    puts("#ifdef VL_DEBUG\n");
    puts("void " + topModNameProtected + "__" +
         protect("_eval_debug_assertions") + selfDecl + ";\n");
    puts("#endif  // VL_DEBUG\n");
    puts("void " + topModNameProtected + "__" + protect("_final") + selfDecl +
         ";\n");

    // _eval_initial_loop
    puts("\nstatic void " + protect("_eval_initial_loop") + "(" +
         symClassVar() + ")" + " {\n");
    puts("vlSymsp->__Vm_didInit = true;\n");
    puts(topModNameProtected + "__" + protect("_eval_initial") +
         "(&(vlSymsp->TOP));\n");
  ...
  void emitStandardMethods2(AstNodeModule *modp) {
    ...
    if (v3Global.opt.threads() == 1) {
      ...
      puts("VL_DEBUG_IF(VL_DBG_MSGF(\"MTask" + cvtToStr(mtaskId) +
           " starting\\n\"););\n");
  ...
  void emitStandardMethods3(AstNodeModule *modp) {
    ...
    puts(topModNameProtected + "__" + protect("_final") +
         "(&(vlSymsp->TOP));\n");
    puts("}\n");
  ...
  void emitTraceMethods(AstNodeModule *modp) {
    ...
    puts(topModNameProtected + "__" + protect("trace_init_top") +
         "(vlSelf, tracep);\n");
    puts("tracep->scopeEscape('.');\n"); // Restore so later traced files won't
                                         // break
    puts("}\n");
  ...
  void emitImplementation(AstNodeModule *modp) {
    ...
    const string filename =
        v3Global.opt.makeDir() + "/" + topClassName() + ".cpp";
  ...
  void emitDpiExportDispatchers(AstNodeModule *modp) {
    ...
    for (AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
      ...
      if (!funcp || !funcp->dpiExportDispatcher())
        continue;
      ...
      if (!m_ofp) {
        string filename =
            v3Global.opt.makeDir() + "/" + topClassName() + "__Dpi_Export";
        ...
        m_lazyDecls.reset();
        m_ofp->putsHeader();
        puts("// DESCRIPTION: Verilator output: Implementation of DPI export "
             "functions.\n");
        puts("//\n");
        puts("#include \"" + topClassName() + ".h\"\n");
        puts("#include \"" + symClassName() + ".h\"\n");
