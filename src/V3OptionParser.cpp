...
struct V3OptionParser::Impl {
  ...
  public:
    virtual bool isValueNeeded() const override final {
      return MODE == en::VALUE;
    }
    virtual bool isOnOffAllowed() const override final {
      return MODE == en::ONOFF;
    }
    virtual bool isPartialMatchAllowed() const override final {
  ...
  class ActionCbPartialMatchVal;        // Callback "-debugi-V3Options 3" for
                                        // "-debugi-"

  // MEMBERS
  std::map<const string, std::unique_ptr<ActionIfs>>
      m_options;             // All actions for option
...
#define V3OPTION_PARSER_DEF_ACT_CLASS(className, type, body, enType)           \
  template <>                                                                  \
  class V3OptionParser::Impl::className<type> final                            \
      : public ActionBase<enType> {                                            \
    type *m_valp; /* Pointer to a option variable*/                            \
                                                                               \
  public:                                                                      \
    explicit className(type *valp) : m_valp(valp) {}                           \
...
#ifndef V3OPTION_PARSER_NO_VOPTION_BOOL
V3OPTION_PARSER_DEF_ACT_CLASS(ActionSet, VOptionBool,
                              m_valp->setTrueOrFalse(true), en::NONE);
#endif
V3OPTION_PARSER_DEF_ACT_CLASS(ActionSet, int, *m_valp = std::atoi(argp),
                              en::VALUE);
V3OPTION_PARSER_DEF_ACT_CLASS(ActionSet, string, *m_valp = argp, en::VALUE);

V3OPTION_PARSER_DEF_ACT_CLASS(ActionOnOff, bool, *m_valp = !hasPrefixNo(optp),
                              en::ONOFF);
#ifndef V3OPTION_PARSER_NO_VOPTION_BOOL
V3OPTION_PARSER_DEF_ACT_CLASS(ActionOnOff, VOptionBool,
                              m_valp->setTrueOrFalse(!hasPrefixNo(optp)),
                              en::ONOFF);
#endif
#undef V3OPTION_PARSER_DEF_ACT_CLASS

#define V3OPTION_PARSER_DEF_ACT_CB_CLASS(className, funcType, body, ...)       \
  class V3OptionParser::Impl::className final                                  \
      : public ActionBase<__VA_ARGS__> {                                       \
    std::function<funcType> m_cb; /* Callback function */                      \
                                                                               \
  public:                                                                      \
    using CbType = std::function<funcType>;                                    \
    explicit className(CbType cb) : m_cb(std::move(cb)) {}                     \
    virtual void exec(const char *optp, const char *argp) override { body; }   \
  }

V3OPTION_PARSER_DEF_ACT_CB_CLASS(ActionCbCall, void(void), m_cb(), en::NONE);
V3OPTION_PARSER_DEF_ACT_CB_CLASS(ActionCbOnOff, void(bool),
                                 m_cb(!hasPrefixNo(optp)), en::ONOFF);
template <>
V3OPTION_PARSER_DEF_ACT_CB_CLASS(ActionCbVal<int>, void(int),
                                 m_cb(std::atoi(argp)), en::VALUE);
template <>
V3OPTION_PARSER_DEF_ACT_CB_CLASS(ActionCbVal<const char *>, void(const char *),
                                 m_cb(argp), en::VALUE);
V3OPTION_PARSER_DEF_ACT_CB_CLASS(ActionCbPartialMatch, void(const char *),
                                 m_cb(optp), en::NONE, true);
V3OPTION_PARSER_DEF_ACT_CB_CLASS(ActionCbPartialMatchVal,
...
V3OptionParser::ActionIfs &V3OptionParser::add(const std::string &opt,
                                               ARG arg) {
  ...
  UASSERT(opt[0] == '-' || opt[0] == '+',
          opt << " does not start with either '-' or '+'");
  UASSERT(!(opt[0] == '-' && opt[1] == '-'),
          "Option must have single '-', but " << opt);
...
void V3OptionParser::finalize() {
  ...
  for (auto &&opt : m_pimpl->m_options) {
    if (opt.second->isUndocumented())
      continue;
    m_pimpl->m_spellCheck.pushCandidate(opt.first);
    if (opt.second->isOnOffAllowed())
      m_pimpl->m_spellCheck.pushCandidate("-no" + opt.first);
  }
  m_pimpl->m_isFinalized = true;
}

V3OptionParser::V3OptionParser() : m_pimpl{new Impl{}} {}
...
V3OPTION_PARSER_DEF_OP(CbVal, Impl::ActionCbVal<const char *>::CbType,
                       ActionCbVal<const char *>)
#undef V3OPTION_PARSER_DEF_OP

V3OptionParser::ActionIfs &V3OptionParser::AppendHelper::operator()(
    const char *optp, CbPartialMatch,
...
V3OptionParser::ActionIfs &V3OptionParser::AppendHelper::operator()(
    const char *optp, CbPartialMatchVal,
    Impl::ActionCbPartialMatchVal::CbType cb) const {
  const size_t prefixLen = std::strlen(optp);
  auto wrap = [prefixLen, cb](const char *optp, const char *argp) {
    cb(optp + prefixLen, argp);
  };
