...
public:
  ...
  bool pretendError() const {
    return (m_e == ASSIGNIN || m_e == BADSTDPRAGMA || m_e == BLKANDNBLK ||
            m_e == BLKLOOPINIT || m_e == CONTASSREG || m_e == IMPURE ||
            m_e == PINNOTFOUND || m_e == PKGNODECL ||
  ...
  bool lintError() const {
    return (m_e == ALWCOMBORDER || m_e == BSSPACE || m_e == CASEINCOMPLETE ||
            m_e == CASEOVERLAP || m_e == CASEWITHX || m_e == CASEX ||
            m_e == CASTCONST || m_e == CMPCONST || m_e == COLONPLUS ||
            m_e == ENDLABEL || m_e == IMPLICIT || m_e == LATCH ||
            m_e == LITENDIAN || m_e == PINMISSING || m_e == REALCVT ||
            m_e == UNSIGNED || m_e == WIDTH);
  }
  // Warnings that are style only
  bool styleError() const {
    return (m_e == ASSIGNDLY // More than style, but for backward compatibility
            || m_e == BLKSEQ || m_e == DEFPARAM || m_e == DECLFILENAME ||
            m_e == EOFNEWLINE || m_e == IMPORTSTAR || m_e == INCABSPATH ||
            m_e == PINCONNECTEMPTY || m_e == PINNOCONNECT ||
            m_e == SYNCASYNCNET || m_e == UNDRIVEN || m_e == UNUSED ||
            m_e == VARHIDDEN);
  }
};
...
inline bool operator==(const V3ErrorCode &lhs, V3ErrorCode::en rhs) {
  return lhs.m_e == rhs;
}
inline bool operator==(V3ErrorCode::en lhs, const V3ErrorCode &rhs) {
  return lhs == rhs.m_e;
}
...
private:
  ...
  static int
      s_tellManual; // Tell user to see manual, 0=not yet, 1=doit, 2=disable
...
public:
  ...
  static void v3errorEnd(std::ostringstream &sstr,
                         const string &locationStr = "");
};

// Global versions, so that if the class doesn't define a operator, we get the
// functions anyways.
...
// Note the commas are the comma operator, not separating arguments. These are
// needed to ensure evaluation order as otherwise we couldn't ensure v3errorPrep
// is called first.
#define v3warnCode(code, msg)                                                  \
  v3errorEnd((V3Error::v3errorPrep(code), (V3Error::v3errorStr() << msg),      \
              V3Error::v3errorStr()))
...
// Use this when normal v3fatal is called in static method that overrides
// fileline.
...
#define UASSERT_OBJ(condition, obj, stmsg)                                     \
  do {                                                                         \
    if (VL_UNCOVERABLE(!(condition)))                                          \
      (obj)->v3fatalSrc(stmsg);                                                \
  } while (false)
// For use in V3Ast static functions only
#define UASSERT_STATIC(condition, stmsg)                                       \
  do {                                                                         \
    if (VL_UNCOVERABLE(!(condition))) {                                        \
      std::cerr << "Internal Error: " << __FILE__ << ":" << std::dec           \
                << __LINE__ << ":" << (stmsg) << std::endl;                    \
      V3Error::vlAbort();                                                      \
    }                                                                          \
  } while (false)
...
#define VL_DEBUG_FUNC                                                          \
  static int debug() {                                                         \
    ...
    if (VL_UNLIKELY(level < 0)) {                                              \
      ...
      if (!v3Global.opt.available())                                           \
        return debugSrcLevel;                                                  \
      level = debugSrcLevel;                                                   \
    }                                                                          \
    return level;                                                              \
  }
