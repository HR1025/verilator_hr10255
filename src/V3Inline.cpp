...
// CONFIG
static const int INLINE_MODS_SMALLER =
    100; // If a mod is < this # nodes, can always inline it
...
private:
  // NODE STATE
  // Output
  //  AstNodeModule::user1()  // OUTPUT: bool. User request to inline this
  //  module
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    if (VN_IS(m_modp, Iface)) {
      // Inlining an interface means we no longer have a cell handle to resolve
      // to. If inlining moves post-scope this can perhaps be relaxed.
  ...
  virtual void visit(AstNetlist *nodep) override {
    ...
    for (auto it = m_allMods.rbegin(); it != m_allMods.rend(); ++it) {
      ...
      for (LocalInstanceMap::iterator iti = localsr.begin();
           iti != localsr.end(); ++iti) {
      ...
      // If --flatten is specified, then force everything to be inlined that can
      // be. inlineMult = 2000 by default. If a mod*#refs is < this # nodes, can
      // inline it
...
public:
  // CONSTRUCTORS
  explicit InlineCollectVisitor(
      AstNodeModule *nodep) { // passed OLD module, not new one
    iterate(nodep);
  }
...
private:
  ...
  std::unordered_set<std::string>
      m_renamedInterfaces; // Name of renamed interface variables
  ...
  virtual void visit(AstVar *nodep) override {
    if (nodep->user2p()) {
      ...
      UASSERT_OBJ(exprconstp || exprvarrefp, nodep,
                  "Unknown interconnect type; pinReconnectSimple should have "
                  "cleared up");
      if (exprconstp) {
        ...
        // Public variable at the lower module end - we need to make sure we
        // propagate the logic changes up and down; if we aliased, we might
        // remove the change detection on the output variable.
        UINFO(9, "public pin assign: " << exprvarrefp << endl);
        UASSERT_OBJ(!nodep->isNonOutput(), nodep,
                    "Outputs only - inputs use AssignAlias");
        m_modp->addStmtp(new AstAssignW(
            nodep->fileline(),
            new AstVarRef(nodep->fileline(), exprvarrefp->varp(),
                          VAccess::WRITE),
            new AstVarRef(nodep->fileline(), nodep, VAccess::READ)));
      } else if (nodep->isSigPublic() &&
                 VN_IS(nodep->dtypep(), UnpackArrayDType)) {
        // Public variable at this end and it is an unpacked array. We need to
        // assign instead of aliased, because otherwise it will pass V3Slice and
        // invalid code will be emitted.
    ...
    // Iterate won't hit AstIfaceRefDType directly as it is no longer underneath
    // the module
    ...
    const string name = m_cellp->name() + "__DOT__" + nodep->name();
    if (!nodep->isFuncLocal() && !nodep->isClassMember())
      nodep->inlineAttrReset(name);
    if (!m_cellp->isTrace())
      nodep->trace(false);
    if (debug() >= 9)
      nodep->dumpTree(cout, "varchanged:");
    if (debug() >= 9 && nodep->valuep())
  ...
  virtual void visit(AstVarXRef *nodep) override {
    ...
    const string newdots =
        VString::dot(m_cellp->name(), ".", nodep->inlinedDots());
  ...
  virtual void visit(AstScopeName *nodep) override {
    // If there's a %m in the display text, we add a special node that will
    // contain the name() Similar code in V3Begin To keep correct visual order,
    // must add before other Text's
    AstNode *afterp = nodep->scopeAttrp();
    if (afterp)
      afterp->unlinkFrBackWithNext();
    nodep->scopeAttrp(
        new AstText(nodep->fileline(), string("__DOT__") + m_cellp->name()));
    if (afterp)
      nodep->scopeAttrp(afterp);
...
public:
  // CONSTRUCTORS
  InlineRelinkVisitor(AstNodeModule *cloneModp, AstNodeModule *oldModp,
                      AstCell *cellp)
      : m_modp{oldModp}, m_cellp{cellp} {
    iterate(cloneModp);
  }
...
private:
  ...
  //  Input:
  //   AstNodeModule::user1p()    // bool. True to inline this module (from
  //   InlineMarkVisitor)
  // Cleared each cell
  //   AstVar::user2p()       // AstVarRef*/AstConst*  Points to signal this
  //                          // is a direct connect to
  //   AstVar::user3()        // bool    Don't alias the user2, keep it as
  //   signal AstCell::user4         // AstCell* of the created clone
  ...
  virtual void visit(AstIfaceRefDType *nodep) override {
    if (nodep->user5()) {
      // The cell has been removed so let's make sure we don't leave a reference
      // to it This dtype may still be in use by the AstAssignVarScope created
      // earlier but that'll get cleared up later
      nodep->cellp(nullptr);
    }
  }
  virtual void visit(AstNodeModule *nodep) override {
  ...
  virtual void visit(AstCell *nodep) override {
    if (nodep->modp()->user1()) { // Marked with inline request
      ...
      for (AstPin *pinp = nodep->pinsp(); pinp;
        ...
        UASSERT_OBJ(VN_IS(connectRefp, Const) || VN_IS(connectRefp, VarRef),
                    pinp,
                    "Unknown interconnect type; pinReconnectSimple should have "
                    "cleared up");
      ...
      // Remove the cell
      VL_DO_DANGLING(newmodp->deleteTree(),
                     newmodp); // Clear any leftover ports, etc
      nodep->unlinkFrBack();
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
      if (debug() >= 9)
        m_modp->dumpTree(cout, "donemod:");
    }
  }
...
void V3Inline::inlineAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("inline", 0,
