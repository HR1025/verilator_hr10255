...
//       V3ParseImp             Internals to parser, common to across flex &
//       bison
...
V3ParseImp::~V3ParseImp() {
  for (auto &itr : m_stringps)
    VL_DO_DANGLING(delete itr, itr);
  m_stringps.clear();
  for (auto &itr : m_numberps)
    VL_DO_DANGLING(delete itr, itr);
  m_numberps.clear();
  lexDestroy();
  parserClear();
...
void V3ParseImp::timescaleMod(FileLine *fl, AstNodeModule *modp, bool unitSet,
...
void V3ParseImp::lexVerilatorCmtLintSave(const FileLine *fl) {
  m_lexLintState.push_back(*fl);
}
...
void V3ParseImp::lexVerilatorCmtLint(FileLine *fl, const char *textp,
                                     bool warnOff) {
  const char *sp = textp;
  while (*sp && !isspace(*sp))
    ++sp;
  ...
  if (!(parsep()->lexFileline()->warnOff(msg, warnOff))) {
    if (!v3Global.opt.isFuture(msg)) {
      fl->v3error("Unknown verilator lint message code: '" << msg << "', in '"
                                                           << textp << "'");
    }
  }
}
...
void V3ParseImp::lexErrorPreprocDirective(FileLine *fl, const char *textp) {
  ...
  for (V3LanguageWords::const_iterator it = V3LanguageWords::begin();
    ...
    if (ppDirective[0] == '`')
      speller.pushCandidate(ppDirective);
  }
...
void V3ParseImp::preprocDumps(std::ostream &os) {
  if (v3Global.opt.dumpDefines()) {
    ...
    const bool noblanks =
        v3Global.opt.preprocOnly() && v3Global.opt.preprocNoLine();
...
void V3ParseImp::parseFile(
  ...
  const bool ok =
      V3PreShell::preproc(fileline, modfilename, m_filterp, this, errmsg);
  if (!ok) {
    if (errmsg != "")
      return; // Threw error already
  ...
  if (v3Global.opt.preprocOnly() || v3Global.opt.keepTempFiles()) {
    ...
    string vppfilename = v3Global.opt.hierTopDataDir() + "/" +
                         v3Global.opt.prefix() + "_" + modname + ".vpp";
...
const V3ParseBisonYYSType *V3ParseImp::tokenPeekp(size_t depth) {
  ...
  // Returns pointer to token, which is no longer valid after changing
  // m_tokensAhead
  while (m_tokensAhead.size() <= depth)
    tokenPull();
...
void V3ParseImp::tokenPipeline() {
  ...
  if (m_tokensAhead.empty())
    tokenPull(); // corrupts yylval
  ...
  if (token == '('              //
    ...
    if (debugFlex() >= 6) {
      cout << "   tokenPipeline: reading ahead to find possible strength"
           << endl;
    }
    V3ParseBisonYYSType curValue =
    ...
    if (token == '(' && (nexttok == ygenSTRENGTH || nexttok == ySUPPLY0 ||
                         nexttok == ySUPPLY1)) {
      ...
      if (nexttok == yCLASS) {
        ...
        token = yVIRTUAL__INTERFACE;
                 ...
                 || nexttok == yaID__LEX) {
        // || nexttok == yaID__aINTERFACE  // but we may not know interfaces
        // yet.
      ...
      if (nexttok == yP_COLONCOLON) {
            ...
            yylval; // Remember value, as about to read ahead
        {
          ...
          if (tokenPeekp(depth)->token == yP_COLONCOLON)
            token = yaID__CC;
        }
        yylval = curValue;
      }
...
void V3ParseImp::tokenPipelineSym() {
  ...
  if (token == yaID__LEX || token == yaID__CC) {
    ...
    if (VSymEnt *look_underp = V3ParseImp::parsep()->symp()->nextId()) {
      UINFO(7, "   tokenPipelineSym: next id lookup forced under "
                   << look_underp << endl);
      ...
      // " -findtree: ", true);
      foundp = V3ParseImp::parsep()->symp()->symCurrentp()->findIdFallback(
          *(yylval.strp));
    }
    if (foundp) {
...
void V3Parse::parseFile(FileLine *fileline, const string &modname,
                        bool inLibrary, const string &errmsg) {
