...
class V3PreProc VL_NOT_FINAL {
  // This defines a preprocessor.  Functions are virtual so implementation can
  // be hidden. After creating, call open(), then getline() in a loop.  The
  // class will to the rest...
...
public:
  // CONSTANTS
  enum MiscConsts {
    DEFINE_RECURSION_LEVEL_MAX =
        1000,                // How many `def substitutions before an error
  ...
  virtual void openFile(FileLine *fileline, VInFilter *filterp,
  ...
  static int keepComments() {
    return 2;
  } // Return comments, 0=no, 1=yes, 2=callback
  ...
  static bool pedantic() {
    return false;
  } // Obey standard; Don't substitute `error

  // CALLBACK METHODS
  // This probably will want to be overridden for given child users of this
  // class.
  virtual void
  comment(const string &cmt) = 0; // Comment detected (if keepComments==2)
  virtual void
  include(const string &filename) = 0; // Request a include file be processed

  virtual void undef(const string &name) = 0; // Remove a definition
  virtual void
  define(FileLine *fileline, const string &name, const string &value,
         const string &params = "",
         bool cmdline = false) = 0; // `define without any parameters
  virtual void
  defineCmdLine(FileLine *fileline, const string &name,
                const string &value) { // `define without any parameters
    define(fileline, name, value, "", true);
  }
  virtual string
  removeDefines(const string &text) = 0; // Remove defines in a text string

  // UTILITIES
  void error(const string &msg) {
    fileline()->v3error(msg);
  } ///< Report an error
  void fatal(const string &msg) {
    fileline()->v3fatalSrc(msg);
  }                                               ///< Report a fatal error
  virtual void dumpDefines(std::ostream &os) = 0; ///< Print list of `defines
  virtual void candidateDefines(
      VSpellCheck *spellerp) = 0; ///< Spell check candidate defines
