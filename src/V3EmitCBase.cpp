...
EmitCParentModule::EmitCParentModule() {
  const auto setAll = [](AstNodeModule *modp) -> void {
    for (AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
      if (VN_IS(nodep, CFunc) || VN_IS(nodep, Var)) {
        nodep->user4p(modp);
      }
    }
  };
...
string EmitCBaseVisitor::funcNameProtect(const AstCFunc *nodep,
...
AstCFile *EmitCBaseVisitor::newCFile(const string &filename, bool slow,
                                     bool source) {
...
void EmitCBaseVisitor::emitCFuncHeader(const AstCFunc *funcp,
                                       const AstNodeModule *modp,
                                       bool withScope) {
  if (funcp->slow())
    puts("VL_ATTR_COLD ");
  ...
  if (funcp->isConst().trueKnown() && funcp->isProperMethod())
    puts(" const");
}

void EmitCBaseVisitor::emitCFuncDecl(const AstCFunc *funcp,
                                     const AstNodeModule *modp, bool cLinkage) {
  ensureNewLine();
  if (!funcp->ifdef().empty())
    puts("#ifdef " + funcp->ifdef() + "\n");
  if (cLinkage)
    puts("extern \"C\" ");
  if (funcp->isStatic() && funcp->isProperMethod())
    puts("static ");
  if (funcp->isVirtual()) {
...
void EmitCBaseVisitor::emitVarDecl(const AstVar *nodep, bool asRef) {
  ...
  const auto emitDeclArrayBrackets = [this](const AstVar *nodep) -> void {
    ...
    for (const AstUnpackArrayDType *arrayp =
             VN_CAST(nodep->dtypeSkipRefp(), UnpackArrayDType);
         arrayp;
  ...
  if (nodep->isIO() && nodep->isSc()) {
    ...
    puts("," + cvtToStr(basicp->lo() + nodep->width() - 1) + "," +
         cvtToStr(basicp->lo()));
    if (nodep->isWide())
    ...
    if (nodep->isFuncLocal() && nodep->isString()) {
      ...
      // string temporary variable for DPI-C needs to be static because c_str()
      // will be passed to C code and the lifetime of the variable must be long
      // enough. See also Issue 2622.
      const bool beStatic = name.size() >= suffix.size() &&
                            name.substr(name.size() - suffix.size()) == suffix;
      if (beStatic)
        puts("static VL_THREAD_LOCAL ");
    }
...
void EmitCBaseVisitor::emitModCUse(const AstNodeModule *modp,
                                   VUseType useType) {
  string nl;
...
void EmitCBaseVisitor::emitTextSection(const AstNodeModule *modp,
                                       AstType type) {
  int last_line = -999;
  for (AstNode *nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
