...
protected:
  ...
  bool preproc(FileLine *fl, const string &modname, VInFilter *filterp,
               V3ParseImp *parsep,
               const string &errmsg) { // "" for no error
    debug(true); // Recheck if debug on - first check was before command line
                 // passed
  ...
  void preprocInclude(FileLine *fl, const string &modname) {
    if (modname[0] == '/' || modname[0] == '\\') {
      fl->v3warn(INCABSPATH, "Suggest `include with absolute path be made "
                             "relative, and use +include: "
                                 << modname);
    }
...
private:
  string preprocOpen(FileLine *fl, VInFilter *filterp, const string &modname,
    ...
    if (filename == "") {
      // Allow user to put `defined names on the command line instead of
      // filenames, then convert them properly.
...
bool V3PreShell::preproc(FileLine *fl, const string &modname,
                         VInFilter *filterp, V3ParseImp *parsep,
                         const string &errmsg) {
