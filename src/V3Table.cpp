...
// 64MB is close to max memory of some systems (256MB or so), so don't get out
// of control
...
public:
  explicit TableBuilder(FileLine *fl) : m_fl{fl} {}
  ...
  void addValue(unsigned index, const V3Number &value) {
    ...
    if (value.isString() ? value.toString().empty() : value.isEqZero())
      return;
  ...
  AstVarScope *varScopep() {
    if (!m_varScopep) {
      m_varScopep = v3Global.rootp()->constPoolp()->findTable(m_initp);
    }
    return m_varScopep;
  }
};
...
class TableOutputVar final {
  ...
  bool m_mayBeUnassigned =
      false; // If true, then this variable may be unassigned through
...
public:
  TableOutputVar(AstVarScope *varScopep, unsigned ord)
      : m_varScopep{varScopep}, m_ord{ord}, m_tableBuilder{
                                                varScopep->fileline()} {}
  ...
  void setTableSize(unsigned size) {
    m_tableBuilder.setTableSize(varScopep()->dtypep(), size);
  }
  void addValue(unsigned index, const V3Number &value) {
    m_tableBuilder.addValue(index, value);
  }
...
private:
  ...
  bool m_assignDly =
      false; // Consists of delayed assignments instead of normal assignments
...
public:
  void simulateVarRefCb(AstVarRef *nodep) {
    ...
    if (nodep->access().isWriteOrRW()) {
      // We'll make the table with a separate natural alignment for each output
      // var, so always have 8, 16 or 32 bit widths, so use widthTotalBytes
...
private:
  bool treeTest(AstAlways *nodep) {
    ...
    const double space =
        std::pow<double>(2.0, m_inWidthBits) * (m_outWidthBytes + chgWidth);
    // Instruction count bytes (ok, it's space also not time :)
    const double time // max(_, 1), so we won't divide by zero
        = std::max<double>(
            chkvis.instrCount() * TABLE_BYTES_PER_INST + chkvis.dataCount(), 1);
    ...
    UINFO(4, "  Test: Opt=" << (chkvis.optimizable() ? "OK" : "NO")
                            ...
                            << " Data=" << chkvis.dataCount()
                            << " in width (bits)=" << m_inWidthBits
                            << " out width (bytes)=" << m_outWidthBytes
                            << " Spacetime=" << (space / time) << "(" << space
                            << "/" << time << ")"
                            << ": " << nodep << endl);
  ...
  void replaceWithTable(AstAlways *nodep) {
    ...
    AstVar *const indexVarp = new AstVar(fl, AstVarType::BLOCKTEMP,
                                         "__Vtableidx" + cvtToStr(m_modTables),
                                         VFlagBitPacked(), m_inWidthBits);
    m_modp->addStmtp(indexVarp);
    AstVarScope *const indexVscp =
        new AstVarScope(indexVarp->fileline(), m_scopep, indexVarp);
    ...
    outputAssignedTableBuilder.setTableSize(
        nodep->findBitDType(m_outVarps.size(), m_outVarps.size(),
                            VSigning::UNSIGNED),
        VL_MASK_I(m_inWidthBits));

    // Set sizes of output tables
    for (TableOutputVar &tov : m_outVarps) {
      tov.setTableSize(VL_MASK_I(m_inWidthBits));
    }
    ...
    if (debug() >= 6)
      nodep->dumpTree(cout, "  table_new: ");
  }

  void createTables(AstAlways *nodep,
                    TableBuilder &outputAssignedTableBuilder) {
    // Create table
    ...
    for (uint32_t i = 0; i <= VL_MASK_I(m_inWidthBits); ++i) {
      ...
      for (AstVarScope *invscp : m_inVarps) {
        ...
        const AstConst cnst(invscp->fileline(), AstConst::WidthedValue(),
                            invscp->width(),
                            VL_MASK_I(invscp->width()) & (inValue >> shift));
        simvis.newValue(invscp, &cnst);
        shift += invscp->width();
        // We are using 32 bit arithmetic, because there's no way the input
        // table can be 2^32 bytes!
      ...
      for (TableOutputVar &tov : m_outVarps) {
        if (V3Number *const outnump =
                simvis.fetchOutNumberNull(tov.varScopep())) {
  ...
  AstNode *createLookupInput(FileLine *fl, AstVarScope *indexVscp) {
    ...
    return new AstAssign(fl, new AstVarRef(fl, indexVscp, VAccess::WRITE),
                         concatp);
  }
  ...
  void createOutputAssigns(AstNode *nodep, AstNode *stmtsp,
    ...
    for (TableOutputVar &tov : m_outVarps) {
      ...
      // If this output is unassigned on some code paths, wrap the assignment in
      // an If
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    // It's nearly impossible to have a large enough assign to make this
    // worthwhile For now we won't bother. Accelerated: no iterate
  }
...
void V3Table::tableAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("table", 0,
