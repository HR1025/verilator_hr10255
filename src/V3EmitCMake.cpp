...
class CMakeEmitter final {
  ...
  template <typename List> static string cmake_list(const List &strs) {
    ...
    if (strs.begin() != strs.end()) {
      ...
      s.append("\"");
      for (typename List::const_iterator it = ++strs.begin(); it != strs.end();
           ++it) {
        s.append(" \"");
  ...
  static void cmake_set_raw(std::ofstream &of, const string &name,
  ...
  static void cmake_set(std::ofstream &of, const string &name,
                        const string &value, const string &cache_type = "",
                        const string &docstring = "") {
  ...
  static void emitOverallCMake() {
    const std::unique_ptr<std::ofstream> of{V3File::new_ofstream(
        v3Global.opt.makeDir() + "/" + v3Global.opt.prefix() + ".cmake")};
    ...
    *of << "# which becomes available after executing "
    ...
    *of << "# Threaded tracing output mode?  0/1/N threads (from "
           "--trace-threads)\n";
    cmake_set_raw(*of, name + "_TRACE_THREADS",
                  cvtToStr(v3Global.opt.traceThreads()));
    *of << "# Struct output mode?  0/1 (from --trace-structs)\n";
    cmake_set_raw(*of, name + "_TRACE_STRUCTS",
                  cvtToStr(v3Global.opt.traceStructs()));
    *of << "# VCD Tracing output mode?  0/1 (from --trace)\n";
    cmake_set_raw(*of, name + "_TRACE_VCD",
                  (v3Global.opt.trace() &&
                   (v3Global.opt.traceFormat() == TraceFormat::VCD))
                      ? "1"
                      : "0");
    ...
    *of << "\n### Sources...\n";
    ...
    if (v3Global.opt.trace()) {
      global.emplace_back("${VERILATOR_ROOT}/include/" +
    ...
    if (!v3Global.opt.protectLib().empty()) {
      global.emplace_back(v3Global.opt.makeDir() + "/" +
    ...
    *of << "# Generated module classes, non-fast-path, compile with low/medium "
           "optimization\n";
    cmake_set_raw(*of, name + "_CLASSES_SLOW",
                  deslash(cmake_list(classes_slow)));
    *of << "# Generated module classes, fast-path, compile with highest "
           "optimization\n";
    cmake_set_raw(*of, name + "_CLASSES_FAST",
                  deslash(cmake_list(classes_fast)));
    *of << "# Generated support classes, non-fast-path, compile with "
           "low/medium optimization\n";
    cmake_set_raw(*of, name + "_SUPPORT_SLOW",
                  deslash(cmake_list(support_slow)));
    *of << "# Generated support classes, fast-path, compile with highest "
           "optimization\n";
    cmake_set_raw(*of, name + "_SUPPORT_FAST",
                  deslash(cmake_list(support_fast)));

    *of << "# All dependencies\n";
    cmake_set_raw(*of, name + "_DEPS",
                  deslash(cmake_list(V3File::getAllDeps())));

    *of << "# User .cpp files (from .cpp's on Verilator command line)\n";
    cmake_set_raw(*of, name + "_USER_CLASSES",
                  deslash(cmake_list(v3Global.opt.cppFiles())));
      ...
      for (V3HierBlockPlan::HierVector::const_iterator it = hierBlocks.begin();
        ...
        *of << "target_link_libraries(${TOP_TARGET_NAME}  PRIVATE " << prefix
        ...
        if (!children.empty()) {
          *of << "target_link_libraries(" << prefix << " INTERFACE";
          for (V3HierBlock::HierBlockSet::const_iterator child =
                   children.begin();
        ...
        *of << "verilate(" << prefix << " PREFIX " << prefix << " TOP_MODULE "
        ...
        for (V3HierBlock::HierBlockSet::const_iterator child = children.begin();
             child != children.end(); ++child) {
          *of << " "
              << deslash(v3Global.opt.makeDir() + "/" +
                         (*child)->hierWrapper(true));
        }
        *of << " ";
        ...
        for (const string &i : vFiles)
          *of << V3Os::filenameRealPath(i) << " ";
        *of << " VERILATOR_ARGS ";
        *of << "-f " << deslash(hblockp->commandArgsFileName(true))
            << " -CFLAGS -fPIC" // hierarchical block will be static, but may be
                                // linked with .so
            << ")\n";
      }
      *of << "\n# Verilate the top module that refers protect-lib wrappers of "
             "above\n";
      *of << "verilate(${TOP_TARGET_NAME} PREFIX " << v3Global.opt.prefix()
          << " TOP_MODULE " << v3Global.rootp()->topModulep()->name()
          << " DIRECTORY " << deslash(v3Global.opt.makeDir()) << " SOURCES ";
      for (const auto &itr : *planp) {
        *of << " "
            << deslash(v3Global.opt.makeDir() + "/" +
                       itr.second->hierWrapper(true));
      }
