...
private:
  ...
  vluint64_t m_testsCovering =
      0;                  //< Number tests with non-zero coverage of this point
  vluint64_t m_count = 0; //< Count of hits across all tests

public:
  // CONSTRUCTORS
  VlcPoint(const string &name, vluint64_t pointNum)
      : m_name{name}, m_pointNum{pointNum} {}
  ~VlcPoint() = default;
  // ACCESSORS
  ...
  string thresh() const {
    return keyExtract(VL_CIK_THRESH);
  } // string as maybe ""
  ...
  string keyExtract(const char *shortKey) const {
    ...
    for (const char *cp = namestr.c_str(); *cp; ++cp) {
      if (*cp == '\001') {
        if (0 == std::strncmp(cp + 1, shortKey, shortLen) &&
            cp[shortLen + 1] == '\002') {
...
private:
  // MEMBERS
  using NameMap =
      std::map<const std::string, vluint64_t>; // Sorted by name (ordered)
