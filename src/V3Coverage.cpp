...
private:
  ...
  std::unordered_map<std::string, int>
      m_varnames;     // Uniquification of inserted variable names
  ...
  AstCoverInc *newCoverInc(FileLine *fl, const string &hier,
    ...
    // a feature as it allows for each parameterized block to be counted
    // separately. Someday the user might be allowed to specify a different page
    // suffix
    ...
    if (!trace_var_name.empty() && v3Global.opt.traceCoverage()) {
      AstVar *varp = new AstVar(incp->fileline(), AstVarType::MODULETEMP,
      ...
      AstAssign *assp = new AstAssign(
          incp->fileline(),
          new AstVarRef(incp->fileline(), varp, VAccess::WRITE),
          new AstAdd(
              incp->fileline(),
              new AstVarRef(incp->fileline(), varp, VAccess::READ),
  ...
  string traceNameForLine(AstNode *nodep, const string &type) {
    string name = "vlCoverageLineTrace_" +
                  nodep->fileline()->filebasenameNoExt() + "__" +
  ...
  void lineTrack(const AstNode *nodep) {
    if (m_state.lineCoverageOn(nodep) &&
        m_state.m_nodep->fileline()->filenameno() ==
            nodep->fileline()->filenameno()) {
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    VL_RESTORER(m_state);
    {
      createHandle(nodep);
      m_modp = nodep;
      m_state.m_inModOff = nodep->isTop(); // Ignore coverage on top module;
                                           // it's a shell we created
      if (!origModp) {
  ...
  void iterateProcedure(AstNode *nodep) {
    ...
    VL_RESTORER(m_inToggleOff);
    {
      ...
      if (m_state.lineCoverageOn(nodep)) {
        lineTrack(nodep);
        AstNode *newp = newCoverInc(nodep->fileline(), "", "v_line", "block",
                                    linesCov(m_state, nodep), 0,
                                    traceNameForLine(nodep, "block"));
  ...
  virtual void visit(AstVar *nodep) override {
    iterateChildren(nodep);
    if (m_modp && !m_inToggleOff && !m_state.m_inModOff &&
      ...
      if (disablep) {
        ...
        //    Treat like tracing, where a end-of-timestamp action sees all
        //    changes
        //      Works ok, but would be quite slow as need to reform
        //      vectors before the calls
        //    Convert to "always @ (posedge signal[#]) coverinc"
        //      Would mark many signals as clocks, precluding many later
        //      optimizations
        //    Convert to "if (x & !lastx) CoverInc"
        //      OK, but we couldn't later detect them to schedule where the IFs
        //      get called
        ...
        ToggleEnt newvec(
            string(""), new AstVarRef(nodep->fileline(), nodep, VAccess::READ),
  ...
  void toggleVarRecurse(AstNodeDType *dtypep, int depth, // per-iteration
                        const ToggleEnt &above, AstVar *varp,
                        AstVar *chgVarp) { // Constant
    if (const AstBasicDType *bdtypep = VN_CAST(dtypep, BasicDType)) {
      if (bdtypep->isRanged()) {
        for (int index_docs = bdtypep->lo(); index_docs < bdtypep->hi() + 1;
          ...
          ToggleEnt newent(
              above.m_comment + string("[") + cvtToStr(index_docs) + "]",
      ...
      for (int index_docs = adtypep->lo(); index_docs <= adtypep->hi();
           ++index_docs) {
        const int index_code = index_docs - adtypep->lo();
        ToggleEnt newent(
            above.m_comment + string("[") + cvtToStr(index_docs) + "]",
            new AstArraySel(varp->fileline(), above.m_varRefp->cloneTree(true),
                            index_code),
            new AstArraySel(varp->fileline(), above.m_chgRefp->cloneTree(true),
                            index_code));
        toggleVarRecurse(adtypep->subDTypep()->skipRefp(), depth + 1, newent,
                         varp, chgVarp);
        newent.cleanup();
      }
      ...
      for (AstMemberDType *itemp = adtypep->membersp(); itemp;
        ...
        ToggleEnt newent(
            above.m_comment + string(".") + itemp->name(),
            new AstSel(varp->fileline(), above.m_varRefp->cloneTree(true),
      ...
      dtypep->v3fatalSrc(
          "Unexpected node data type in toggle coverage generation: "
  ...
  virtual void visit(AstIf *nodep) override {
    ...
    if (m_state.m_on) {
      ...
      const bool elsif = nodep->ifsp() && VN_IS(nodep->elsesp(), If) &&
                         !nodep->elsesp()->nextp();
      if (elsif)
        VN_AS(nodep->elsesp(), If)->user1(true);
      ...
      if (!(first_elsif || cont_elsif || final_elsif) &&
          ifState.lineCoverageOn(nodep) && elseState.lineCoverageOn(nodep)) {
        ...
        // As "if" and "else" are more than one character wide, this won't
        // overlap another token
  ...
  virtual void visit(AstCaseItem *nodep) override {
    ...
    if (m_state.lineCoverageOn(nodep)) {
      VL_RESTORER(m_state);
      {
        ...
        if (m_state.lineCoverageOn(
                nodep)) { // if the case body didn't disable it
          lineTrack(nodep);
  ...
  virtual void visit(AstCover *nodep) override {
    ...
    VL_RESTORER(m_state);
    {
      ...
      if (!nodep->coverincp() && v3Global.opt.coverageUser()) {
        ...
        lineTrack(nodep);
        nodep->coverincp(newCoverInc(nodep->fileline(), m_beginHier, "v_user",
  ...
  virtual void visit(AstBegin *nodep) override {
    ...
    VL_RESTORER(m_inToggleOff);
    {
      ...
      if (nodep->name() != "") {
        m_beginHier =
            m_beginHier + (m_beginHier != "" ? "." : "") + nodep->name();
      }
      iterateChildren(nodep);
      lineTrack(nodep);
    }
...
void V3Coverage::coverage(AstNetlist *rootp) {
  ...
  V3Global::dumpCheckGlobalTree("coverage", 0,
