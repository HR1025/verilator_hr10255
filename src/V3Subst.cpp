...
class SubstVarEntry final {
  ...
  std::vector<SubstVarWord>
      m_words; // Data for every word, if multi word variable
...
public:
  ...
  AstNode *substWhole(AstNode *errp) {
    if (!m_varp->isWide() && !m_whole.m_complex && m_whole.m_assignp &&
        !m_wordAssign) {
  ...
  AstNode *substWord(AstNode *errp, int word) {
    if (!m_whole.m_complex && !m_whole.m_assignp && !m_words[word].m_complex) {
      ...
      UASSERT_OBJ(assp, errp,
                  "Reading a word that was never assigned, or bad word #");
      return (assp->rhsp());
    } else {
      return nullptr;
    }
  ...
  void deleteUnusedAssign() {
    ...
    for (unsigned i = 0; i < m_words.size(); i++) {
      if (!m_whole.m_use && !m_words[i].m_use && m_words[i].m_assignp &&
          !m_words[i].m_complex) {
        VL_DO_CLEAR(deleteAssign(m_words[i].m_assignp),
                    m_words[i].m_assignp = nullptr);
      }
    }
  }
};
...
private:
  ...
  SubstVarEntry *findEntryp(AstVarRef *nodep) {
    return reinterpret_cast<SubstVarEntry *>(
        nodep->varp()->user1p()); // Might be nullptr
  }
  // VISITORS
  virtual void visit(AstVarRef *nodep) override {
    SubstVarEntry *entryp = findEntryp(nodep);
    if (entryp) {
      // Don't sweat it.  We assign a new temp variable for every new
      // assignment, so there's no way we'd ever replace a old value.
    } else {
      // A simple variable; needs checking.
      if (m_origStep < nodep->varp()->user2()) {
        if (m_ok) {
          UINFO(9, "   RHS variable changed since subst recorded: " << nodep
                                                                    << endl);
        }
        m_ok = false;
      }
...
public:
  // CONSTRUCTORS
  SubstUseVisitor(AstNode *nodep, int origStep) : m_origStep{origStep} {
...
private:
  ...
  // AstVar::user2            -> int step number for last assignment, 0=not set
  // yet
  AstUser1InUse m_inuser1;
  AstUser2InUse m_inuser2;

  // STATE
  std::vector<SubstVarEntry *>
      m_entryps;         // Nodes to delete when we are finished
  ...
  virtual void visit(AstWordSel *nodep) override {
    ...
    if (varrefp && isSubstVar(varrefp->varp()) &&
        varrefp->access().isReadOnly() && constp) {
...
void V3Subst::substituteAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("subst", 0,
