...
//              widthMin() = Alternative acceptable width for linting, or
//              width() if sized Determine this subop's width, can be either:
//                  Fixed width X
...
//                      For == and similar ops, if multibit underneath, add a
//                      REDOR If subop larger, add a EXTRACT If subop smaller,
//                      add a EXTEND
...
std::ostream &operator<<(std::ostream &str, const Castable &rhs) {
  static const char *const s_det[] = {"UNSUP", "COMPAT", "DYN_ENUM", "DYN_CLS",
                                      "INCOMPAT"};
  return str << s_det[rhs];
}
...
public:
  ...
  WidthVP(Determ determ, Stage stage) : m_dtypep{nullptr}, m_stage{stage} {
    if (determ != SELF && stage != PRELIM)
      v3fatalSrc(
          "Context-determined width request only allowed as prelim step");
  }
  ...
  AstNodeDType *dtypep() const {
    ...
    if (!m_dtypep)
      v3fatalSrc("Width dtype request on self-determined or preliminary VUP");
    return m_dtypep;
  }
  ...
  AstNodeDType *dtypeOverridep(AstNodeDType *defaultp) const {
    if (m_stage == PRELIM)
      v3fatalSrc("Parent dtype should be a final-stage action");
    return m_dtypep ? m_dtypep : defaultp;
  }
  int width() const {
    if (!m_dtypep)
      v3fatalSrc("Width request on self-determined or preliminary VUP");
...
class WidthClearVisitor final {
  ...
  void clearWidthRecurse(AstNode *nodep) {
    for (; nodep; nodep = nodep->nextp()) {
      ...
      if (nodep->op1p())
        clearWidthRecurse(nodep->op1p());
      if (nodep->op2p())
        clearWidthRecurse(nodep->op2p());
      if (nodep->op3p())
        clearWidthRecurse(nodep->op3p());
      if (nodep->op4p())
...
private:
  // TYPES
  using TableMap =
      std::map<std::pair<const AstNodeDType *, AstAttrType>, AstVar *>;
  ...
  enum ExtendRule : uint8_t {
    EXTEND_EXP,  // Extend if expect sign and node signed, e.g. node=y in
                 // ADD(x,y), "x + y"
    EXTEND_ZERO, // Extend with zeros. e.g. node=y in EQ(x,y), "x == y"
    EXTEND_LHS,  // Extend with sign if node signed. e.g. node=y in ASSIGN(y,x),
                 // "x = y"
  ...
  virtual void visit(AstEqCase *nodep) override {
    visit_cmp_eq_gt(nodep, true);
  }
  virtual void visit(AstNeqCase *nodep) override {
    visit_cmp_eq_gt(nodep, true);
  }
  // ...    These comparisons don't allow reals
  virtual void visit(AstEqWild *nodep) override {
    visit_cmp_eq_gt(nodep, false);
  }
  virtual void visit(AstNeqWild *nodep) override {
    visit_cmp_eq_gt(nodep, false);
  }
  // ...    Real compares
  ...
  virtual void visit(AstSub *nodep) override {
    visit_add_sub_replace(nodep, true);
  }
  virtual void visit(AstDiv *nodep) override {
    visit_add_sub_replace(nodep, true);
  }
  virtual void visit(AstMul *nodep) override {
    visit_add_sub_replace(nodep, true);
  }
  // These can't promote to real
  virtual void visit(AstModDiv *nodep) override {
    visit_add_sub_replace(nodep, false);
  }
  virtual void visit(AstModDivS *nodep) override {
    visit_add_sub_replace(nodep, false);
  }
  virtual void visit(AstMulS *nodep) override {
    visit_add_sub_replace(nodep, false);
  }
  virtual void visit(AstDivS *nodep) override {
    visit_add_sub_replace(nodep, false);
  }
  ...
  virtual void visit(AstNodeSystemBiop *nodep) override {
    visit_real_add_sub(nodep);
  }
  // Real: Output real
  virtual void visit(AstNegateD *nodep) override { visit_real_neg_ceil(nodep); }
  virtual void visit(AstNodeSystemUniop *nodep) override {
    visit_real_neg_ceil(nodep);
  }
  ...
  virtual void visit(AstRToIRoundS *nodep) override {
    ...
    if (!nodep->dtypep()->widthSized())
      nodep->v3fatalSrc("RToIRoundS should be presized");
  }
  ...
  virtual void visit(AstPutcN *nodep) override {
    ...
    if (m_vup && m_vup->prelim()) {
      ...
      nodep->dtypeSetString(); // AstPutcN returns the new string to be assigned
                               // by AstAssign
    }
  }
  ...
  virtual void visit(AstNodeCond *nodep) override {
    ...
    //   Real: Output real if either expression is real, non-real argument gets
    //   converted
    if (m_vup->prelim()) { // First stage evaluation
      ...
      userIterateAndNext(nodep->expr1p(),
                         WidthVP(m_vup->dtypeNullp(), PRELIM).p());
      userIterateAndNext(nodep->expr2p(),
                         WidthVP(m_vup->dtypeNullp(), PRELIM).p());
      ...
      // Second call (final()) m_vup->width() is probably the expression size,
      // so
    ...
    if (m_vup->final()) {
      ...
      iterateCheck(nodep, "Conditional True", nodep->expr1p(), CONTEXT, FINAL,
                   subDTypep, EXTEND_EXP);
      iterateCheck(nodep, "Conditional False", nodep->expr2p(), CONTEXT, FINAL,
                   subDTypep, EXTEND_EXP);
    }
  }
  virtual void visit(AstConcat *nodep) override {
    ...
    if (VN_IS(vdtypep, QueueDType)) {
      ...
      auto *newp =
          new AstConsQueue(nodep->fileline(), nodep->rhsp()->unlinkFrBack(),
    ...
    if (VN_IS(vdtypep, DynArrayDType)) {
      auto *newp =
          new AstConsDynArray(nodep->fileline(), nodep->rhsp()->unlinkFrBack(),
                              nodep->lhsp()->unlinkFrBack());
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
      userIterateChildren(newp, m_vup);
  ...
  virtual void visit(AstDelay *nodep) override {
    if (VN_IS(m_procedurep, Final)) {
      nodep->v3error(
          "Delays are not legal in final blocks (IEEE 1800-2017 9.2.3)");
    ...
    nodep->v3warn(STMTDLY,
                  "Unsupported: Ignoring delay on this delayed statement.");
    VL_DO_DANGLING(pushDeletep(nodep->unlinkFrBack()), nodep);
  }
  ...
  virtual void visit(AstRange *nodep) override {
    ...
    checkConstantOrReplace(nodep->leftp(),
                           "left side of bit range isn't a constant");
    checkConstantOrReplace(nodep->rightp(),
                           "right side of bit range isn't a constant");
    if (m_vup->prelim()) {
      ...
      if (nodep->littleEndian() && !VN_IS(nodep->backp(), UnpackArrayDType) &&
        ...
        nodep->v3warn(LITENDIAN,
                      "Little bit endian vector: left < right of bit range: ["
  ...
  virtual void visit(AstSel *nodep) override {
    ...
    // We also use SELs to shorten a signed constant etc, in this case they are
    // signed.
    if (nodep->didWidth())
      return;
    ...
    if (m_vup->prelim()) {
      ...
      if (VN_IS(nodep->lsbp(), Const) &&
        ...
        nodep->widthp()->replaceWith(
            new AstConst(nodep->widthp()->fileline(), width));
        nodep->lsbp()->replaceWith(new AstConst(nodep->lsbp()->fileline(), 0));
      }
      // We're extracting, so just make sure the expression is at least wide
      // enough.
      if (nodep->fromp()->width() < width) {
        nodep->v3warn(SELRANGE, "Extracting " << width << " bits from only "
                                              ...
                                              << " bit number");
        // Extend it.
        AstNodeDType *subDTypep = nodep->findLogicDType(
      ...
      int fromlsb = 0;
      const int elw =
          nodep->declElWidth(); // Must adjust to tell user bit ranges
      ...
      const int selwidth =
          V3Number::log2b(frommsb + 1 - 1) + 1; // Width to address a bit
      AstNodeDType *selwidthDTypep = nodep->findLogicDType(
      ...
      if (!m_doGenerate) {
        ...
        widthCheckSized(nodep, "Extract Range", nodep->lsbp(), selwidthDTypep,
  ...
  virtual void visit(AstArraySel *nodep) override {
    ...
    if (m_vup->prelim()) {
      ...
      if (const AstUnpackArrayDType *adtypep =
              VN_CAST(fromDtp, UnpackArrayDType)) {
        frommsb = adtypep->hi();
        fromlsb = adtypep->lo();
        if (fromlsb > frommsb) {
          int t = frommsb;
  ...
  virtual void visit(AstAssocSel *nodep) override {
    ...
    if (m_vup->prelim()) {
      ...
      if (!adtypep) {
        UINFO(1, "    Related dtype: " << fromDtp << endl);
        nodep->v3fatalSrc(
            "Associative array reference is not to associative array");
      }
      iterateCheckTyped(nodep, "Associative select", nodep->bitp(),
                        adtypep->keyDTypep(), BOTH);
  ...
  virtual void visit(AstSliceSel *nodep) override {
    ...
    if (m_vup->prelim()) {
      ...
      // Build new array Dtype based on the original's base type, but with new
      // bounds
      AstNodeDType *newDtp = new AstUnpackArrayDType(
          nodep->fileline(), adtypep->subDTypep(),
      ...
      if (!m_doGenerate) {
        ...
        if ((declRange.hi() > adtypep->declRange().hi()) ||
          ...
          nodep->v3error("Slice selection index '" << declRange << "'"
                                                   ...
                                                   << adtypep->declRange()
                                                   << "'");
        } else if ((declRange.littleEndian() !=
  ...
  virtual void visit(AstSelBit *nodep) override {
    ...
    userIterateAndNext(nodep->fromp(),
                       WidthVP(CONTEXT, PRELIM).p()); // FINAL in AstSel
    userIterateAndNext(nodep->rhsp(),
                       WidthVP(CONTEXT, PRELIM).p()); // FINAL in AstSel
    userIterateAndNext(nodep->thsp(),
                       WidthVP(CONTEXT, PRELIM).p()); // FINAL in AstSel
  ...
  virtual void visit(AstConst *nodep) override {
    ...
    // the number's sign.  So we don't:
    // nodep->dtypeChgSigned(nodep->num().isSigned());
    if (m_vup && m_vup->prelim()) {
    ...
    // to remain unsized, and numbers to remain unsized to avoid backp()
    // warnings
  }
  ...
  virtual void visit(AstPast *nodep) override {
    if (m_vup->prelim()) {
      ...
      if (nodep->ticksp()) {
        ...
        if (!constp) {
          nodep->v3error(
              "$past tick value must be constant (IEEE 1800-2017 16.9.3)");
          nodep->ticksp()->unlinkFrBack()->deleteTree();
        } else if (constp->toSInt() < 1) {
          constp->v3error(
              "$past tick value must be >= 1 (IEEE 1800-2017 16.9.3)");
          nodep->ticksp()->unlinkFrBack()->deleteTree();
        } else {
          if (constp->toSInt() > 10) {
            constp->v3warn(TICKCOUNT,
                           "$past tick value of "
                               << constp->toSInt()
  ...
  virtual void visit(AstRand *nodep) override {
    if (m_vup->prelim()) {
      ...
      if (nodep->seedp())
        iterateCheckSigned32(nodep, "seed", nodep->seedp(), BOTH);
    }
  }
  virtual void visit(AstURandomRange *nodep) override {
    if (m_vup->prelim()) {
      ...
      iterateCheck(nodep, "LHS", nodep->lhsp(), SELF, FINAL, expDTypep,
                   EXTEND_EXP);
      iterateCheck(nodep, "RHS", nodep->rhsp(), SELF, FINAL, expDTypep,
                   EXTEND_EXP);
    }
  }
  virtual void visit(AstUnbounded *nodep) override {
    ...
    if (VN_IS(nodep->backp(), IsUnbounded))
      return; // Ok, leave
    if (VN_IS(nodep->backp(), BracketArrayDType))
      return; // Ok, leave
    ...
    nodep->v3warn(E_UNSUPPORTED,
                  "Unsupported/illegal unbounded ('$') in this context.");
  }
  ...
  virtual void visit(AstUCFunc *nodep) override {
    ...
    if (m_vup->final()) {
      AstNodeDType *expDTypep = m_vup->dtypeOverridep(nodep->dtypep());
      nodep->dtypeFrom(
          expDTypep); // Assume user knows the rules; go with the flow
      if (nodep->width() > 64) {
        nodep->v3warn(E_UNSUPPORTED,
  ...
  virtual void visit(AstPow *nodep) override {
    ...
    if (m_vup->final()) {
      ...
      iterateCheck(nodep, "LHS", nodep->lhsp(), SELF, FINAL, nodep->dtypep(),
                   EXTEND_EXP);
  ...
  virtual void visit(AstCountBits *nodep) override {
    if (m_vup->prelim()) {
      ...
      // If it's a 32 bit number, we need a 6 bit number as we need to return
      // '32'.
  ...
  virtual void visit(AstTimeImport *nodep) override {
    ...
    if (!constp || !constp->isDouble())
      nodep->v3fatalSrc("Times should be doubles");
    if (nodep->timeunit().isNone())
      nodep->v3fatalSrc("$time import no units");
    double time = constp->num().toDouble();
    if (v3Global.rootp()->timeprecision().isNone())
      nodep->v3fatalSrc("Never set precision?");
    time /= nodep->timeunit().multiplier();
    // IEEE claims you should round to time precision here, but no simulator
    // seems to do this
    AstConst *newp =
        new AstConst(nodep->fileline(), AstConst::RealDouble(), time);
    nodep->replaceWith(newp);
    VL_DO_DANGLING(nodep->deleteTree(), nodep);
  }
  ...
  virtual void visit(AstAttrOf *nodep) override {
    ...
    userIterateAndNext(nodep->fromp(), WidthVP(SELF, BOTH).p());
    if (nodep->dimp())
      userIterateAndNext(nodep->dimp(), WidthVP(SELF, BOTH).p());
    ...
    case AstAttrType::ENUM_BASE:
      // Soon to be handled in V3LinkWidth SEL generation, under attrp() and
      // newSubLsbOf
      break;
    ...
    case AstAttrType::DIM_UNPK_DIMENSIONS: {
      ...
      std::pair<uint32_t, uint32_t> dim =
          nodep->fromp()->dtypep()->dimensions(true);
      const int val = (nodep->attrType() == AstAttrType::DIM_UNPK_DIMENSIONS
                           ? dim.second
                           : (dim.first + dim.second));
      nodep->replaceWith(
          new AstConst(nodep->fileline(), AstConst::Signed32(), val));
      VL_DO_DANGLING(nodep->deleteTree(), nodep);
      break;
    }
    ...
    case AstAttrType::DIM_SIZE: {
      ...
      if (VN_IS(nodep->fromp()->dtypep(), QueueDType)) {
        switch (nodep->attrType()) {
        case AstAttrType::DIM_SIZE: {
          AstNode *newp = new AstCMethodHard(nodep->fileline(),
        ...
        case AstAttrType::DIM_RIGHT:
        case AstAttrType::DIM_HIGH: {
          AstNode *sizep = new AstCMethodHard(nodep->fileline(),
                                              nodep->fromp()->unlinkFrBack(),
                                              "size", nullptr);
        ...
        default:
          nodep->v3error("Unhandled attribute type");
        }
      } else {
        ...
        if (!nodep->dimp() || msbdim < 1) {
          const int dim = 1;
          AstConst *newp =
              dimensionValue(nodep->fileline(), nodep->fromp()->dtypep(),
                             nodep->attrType(), dim);
          nodep->replaceWith(newp);
          VL_DO_DANGLING(nodep->deleteTree(), nodep);
          ...
          UASSERT_OBJ(nodep->fromp() && nodep->fromp()->dtypep(), nodep,
                      "Unsized expression");
          AstVar *varp = dimensionVarp(nodep->fromp()->dtypep(),
                                       nodep->attrType(), msbdim);
    ...
    default: {
      ...
      nodep->dtypeSetLogicUnsized(
          32, 1, VSigning::UNSIGNED); // Approximation, unsized 32
  ...
  virtual void visit(AstNodeArrayDType *nodep) override {
    if (nodep->didWidthAndSet())
      return; // This node is a dtype & not both PRELIMed+FINALed
    ...
    if (nodep->subDTypep() == nodep->basicp()) { // Innermost dimension
      ...
      if (basicp->implicit()) {
        UASSERT_OBJ(basicp->width() <= 1, basicp,
                    "must be 1 bit but actually " << basicp->width()
                                                  << " bits");
    ...
    if (auto *adtypep = VN_CAST(nodep, UnpackArrayDType)) {
      // Historically array elements have width of the ref type not the full
      // array
      nodep->widthFromSub(nodep->subDTypep());
      if (nodep->subDTypep()->skipRefp()->isCompound())
        adtypep->isCompound(true);
    } else {
      const int width =
          nodep->subDTypep()->width() * nodep->rangep()->elementsConst();
  ...
  virtual void visit(AstBracketArrayDType *nodep) override {
    ...
    if (VN_IS(elementsp, Unbounded)) {
      newp = new AstQueueDType(nodep->fileline(), VFlagChildDType(), childp,
                               nullptr);
      VL_DO_DANGLING(elementsp->deleteTree(), elementsp);
    } else if (AstNodeDType *keyp = VN_CAST(elementsp, NodeDType)) {
      newp = new AstAssocArrayDType(nodep->fileline(), VFlagChildDType(),
                                    childp, keyp);
    } else {
      ...
      newp = new AstUnpackArrayDType(
          ...
          new AstRange(nodep->fileline(),
                       new AstConst(elementsp->fileline(), 0),
                       new AstSub(elementsp->fileline(), elementsp,
                                  new AstConst(elementsp->fileline(), 1))));
    }
    nodep->replaceWith(newp);
    VL_DO_DANGLING(nodep->deleteTree(), nodep);
    // Normally parent's iteration would cover this, but we might have entered
    // by a specific visit
  ...
  virtual void visit(AstQueueDType *nodep) override {
    ...
    if (VN_IS(nodep->boundp(), Unbounded)) {
      nodep->boundp()
          ->unlinkFrBack()
          ->deleteTree(); // nullptr will represent unbounded
    }
    UINFO(4, "dtWidthed " << nodep << endl);
  }
  ...
  virtual void visit(AstBasicDType *nodep) override {
    ...
    if (nodep->rangep()) {
      ...
      nodep->widthForce(nodep->rangep()->elementsConst(),
                        nodep->rangep()->elementsConst());
    ...
    nodep->cvtRangeConst();
    // TODO: If BasicDType now looks like a generic type, we can convert to a
    // real generic dtype Instead for now doing this in V3WidthCommit
    UINFO(4, "dtWidthed " << nodep << endl);
  }
  virtual void visit(AstConstDType *nodep) override {
    ...
    nodep->dtypep(
        nodep); // Should already be set, but be clear it's not the subDType
    nodep->widthFromSub(nodep->subDTypep());
    UINFO(4, "dtWidthed " << nodep << endl);
  }
  virtual void visit(AstRefDType *nodep) override {
    if (nodep->doingWidth()) { // Early exit if have circular parameter
                               // definition
    ...
    if (nodep->typeofp()) { // type(typeofp_expression)
      ...
      // in type table are still correct (which they wouldn't be if we replaced
      // the node)
    }
    userIterateChildren(nodep, nullptr);
    if (nodep->subDTypep()) {
      // Normally iterateEditMoveDTypep iterate would work, but the refs are
      // under the TypeDef which will upset iterateEditMoveDTypep as it can't
      // find it under this node's childDTypep
      userIterate(nodep->subDTypep(), nullptr);
      nodep->refDTypep(iterateEditMoveDTypep(nodep, nodep->subDTypep()));
      nodep->typedefp(
          nullptr); // Note until line above subDTypep() may have followed this
      // Widths are resolved, but special iterate to check for recurstion
      userIterate(nodep->subDTypep(), nullptr);
    }
    // Effectively nodep->dtypeFrom(nodep->dtypeSkipRefp());
    // But might be recursive, so instead manually recurse into the referenced
    // type
  ...
  virtual void visit(AstTypedef *nodep) override {
    ...
    if (auto *refp = checkRefToTypedefRecurse(nodep, nodep)) {
      nodep->v3error("Typedef has self-reference: "
                     << nodep->prettyNameQ() << '\n'
                     << nodep->warnContextPrimary() << '\n'
                     << refp->warnOther() << "... Location of reference\n"
  ...
  virtual void visit(AstCastDynamic *nodep) override {
    nodep->dtypeChgWidthSigned(32, 1,
    ...
    if (castable == DYNAMIC_CLASS) {
      ...
      UASSERT_OBJ(enumDtp, nodep,
                  "$cast determined as enum, but not enum type");
      uint64_t maxval = enumMaxValue(nodep, enumDtp);
      const int selwidth =
          V3Number::log2b(maxval) + 1; // Width to address a bit
      AstVar *varp =
  ...
  virtual void visit(AstCast *nodep) override {
    ...
    if (m_vup->prelim()) {
      ...
      // The cast may change signing, but we don't know the sign yet.  Make it
      // so. Note we don't sign fromp() that would make the algorithm O(n^2) if
      // lots of casting.
      ...
      if (bad) {
        ...
        if (!basicp->isDouble() && !fromDtp->isDouble()) {
          ...
          iterateCheck(nodep, "value", nodep->lhsp(), SELF, FINAL, fromDtp,
                       EXTEND_EXP, false);
        }
        if (basicp->isDouble() && !nodep->fromp()->isDouble()) {
          if (nodep->fromp()->isSigned()) {
            newp = new AstISToRD(nodep->fileline(),
                                 nodep->fromp()->unlinkFrBack());
          } else {
            newp =
                new AstIToRD(nodep->fileline(), nodep->fromp()->unlinkFrBack());
          }
        } else if (!basicp->isDouble() && nodep->fromp()->isDouble()) {
          if (basicp->isSigned()) {
            newp = new AstRToIRoundS(nodep->fileline(),
                                     nodep->fromp()->unlinkFrBack());
          } else {
            newp =
                new AstUnsigned(nodep->fileline(),
                                new AstRToIS(nodep->fileline(),
                                             nodep->fromp()->unlinkFrBack()));
          }
        } else if (basicp->isSigned() && !nodep->fromp()->isSigned()) {
          newp =
              new AstSigned(nodep->fileline(), nodep->fromp()->unlinkFrBack());
        } else if (!basicp->isSigned() && nodep->fromp()->isSigned()) {
      ...
      if (!newp)
        newp = nodep->fromp()->unlinkFrBack();
      nodep->lhsp(newp);
      if (debug())
        nodep->dumpTree(cout, "  CastOut: ");
      if (debug())
        nodep->backp()->dumpTree(cout, "  CastOutUpUp: ");
    }
    if (m_vup->final()) {
      iterateCheck(nodep, "value", nodep->lhsp(), SELF, FINAL,
                   nodep->lhsp()->dtypep(), EXTEND_EXP, false);
      AstNode *underp = nodep->lhsp()->unlinkFrBack();
      if (debug())
        underp->dumpTree(cout, "  CastRep: ");
  ...
  void castSized(AstNode *nodep, AstNode *underp, int width) {
    ...
    // A cast propagates its size to the lower expression and is included in the
    // maximum width, so 23'(1'b1 + 1'b1) uses 23-bit math, but 1'(2'h2 * 2'h1)
    // uses two-bit math. However the output width is exactly that requested. So
    // two steps, first do the calculation's width (max of the two widths)
    {
      const int calcWidth = std::max(width, underDtp->width());
      AstNodeDType *calcDtp =
          (underDtp->isFourstate() ? nodep->findLogicDType(calcWidth, calcWidth,
                                                           underDtp->numeric())
                                   : nodep->findBitDType(calcWidth, calcWidth,
                                                         underDtp->numeric()));
      nodep->dtypep(calcDtp);
      // We ignore warnings as that is sort of the point of a cast
      iterateCheck(nodep, "Cast expr", underp, CONTEXT, FINAL, calcDtp,
                   EXTEND_EXP, false);
      VL_DANGLING(underp);
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (nodep->doingWidth()) { // Early exit if have circular parameter
      ...
      nodep->v3error(
          "Variable's initial value is circular: " << nodep->prettyNameQ());
    ...
    if (AstUnsizedArrayDType *unsizedp =
    ...
    bool didchk = false;
    const bool implicitParam =
        nodep->isParam() && bdtypep && bdtypep->implicit();
    if (implicitParam) {
      if (nodep->valuep()) {
        userIterateAndNext(nodep->valuep(),
                           WidthVP(nodep->dtypep(), PRELIM).p());
        ...
        // This prevents width warnings at the location the parameter is
        // substituted in
        if (nodep->valuep()->isDouble()) {
          ...
          bool issigned = false;
          if (bdtypep->isNosign()) {
            if (valueBdtypep && valueBdtypep->isSigned())
              issigned = true;
          } else {
          ...
          if (nodep->valuep()->dtypep()->widthSized()) {
            ...
            if (nodep->valuep()->width() > 32) {
              nodep->valuep()->v3warn(WIDTH, "Assigning >32 bit to unranged "
                                             "parameter (defaults to 32 bits)");
            }
            width = 32;
          }
          // Can't just inherit valuep()->dtypep() as mwidth might not equal
          // width
          if (width == 1) {
        ...
        VL_DANGLING(bdtypep);
               ...
               bdtypep->implicit()) { // Implicits get converted to size 1
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    ...
    if (VN_IS(nodep->backp(), NodeAssign) &&
        nodep->access().isWriteOrRW()) { // On LHS
      UASSERT_OBJ(nodep->dtypep(), nodep, "LHS var should be dtype completed");
    }
    // if (debug() >= 9) nodep->dumpTree(cout, "  VRout ");
    if (nodep->access().isWriteOrRW() &&
        nodep->varp()->direction() == VDirection::CONSTREF) {
      nodep->v3error(
          "Assigning to const ref variable: " << nodep->prettyNameQ());
    } else if (nodep->access().isWriteOrRW() && nodep->varp()->isConst() &&
               !m_paramsOnly && (!m_ftaskp || !m_ftaskp->isConstructor()) &&
  ...
  virtual void visit(AstEnumDType *nodep) override {
    ...
    for (AstEnumItem *itemp = nodep->itemsp(); itemp;
      ...
      if (itemp->valuep()) {
        ...
        // TODO IEEE says assigning sized number that is not same size as enum
        // is illegal
      }
      if (!itemp->valuep()) {
        if (num.isEqZero() && itemp != nodep->itemsp()) {
          itemp->v3error(
              "Enum value illegally wrapped around (IEEE 1800-2017 6.19)");
        }
        if (num.isFourState()) {
          itemp->v3error(
              "Enum value that is unassigned cannot follow value with X/Zs "
              "(IEEE 1800-2017 6.19)");
        }
      ...
      if (constp->num().isFourState() && nodep->dtypep()->basicp() &&
        ...
        itemp->v3error(
            "Enum value with X/Zs cannot be assigned to non-fourstate type "
            "(IEEE 1800-2017 6.19)");
      }
  ...
  virtual void visit(AstEnumItem *nodep) override {
    ...
    if (nodep->valuep()) { // else the value will be assigned sequentially
      // Default type is int, but common to assign narrower values, so minwidth
      // from value
      ...
      iterateCheck(nodep, "Enum value", nodep->valuep(), CONTEXT, FINAL,
                   nodep->dtypep(), EXTEND_EXP);
    }
  }
  virtual void visit(AstEnumItemRef *nodep) override {
    if (!nodep->itemp()->didWidth()) {
      ...
      for (; enump; enump = enump->backp()) {
        if (VN_IS(enump, EnumDType))
          break;
      }
      UASSERT_OBJ(enump, nodep, "EnumItemRef can't deref back to an Enum");
      VL_DO_DANGLING(userIterate(enump, m_vup),
                     enump); // Parent enump maybe relinked
    }
  ...
  virtual void visit(AstConsAssoc *nodep) override {
    ...
    UASSERT_OBJ(vdtypep, nodep,
                "ConsAssoc requires assoc upper parent data type");
    if (m_vup->prelim()) {
  ...
  virtual void visit(AstSetAssoc *nodep) override {
    ...
    auto *vdtypep = VN_AS(m_vup->dtypep()->skipRefp(), AssocArrayDType);
    UASSERT_OBJ(vdtypep, nodep,
                "SetsAssoc requires assoc upper parent data type");
    if (m_vup->prelim()) {
      nodep->dtypeFrom(vdtypep);
      userIterateAndNext(nodep->lhsp(), WidthVP(vdtypep, BOTH).p());
      iterateCheck(nodep, "key", nodep->keyp(), CONTEXT, FINAL,
                   vdtypep->keyDTypep(), EXTEND_EXP);
      iterateCheck(nodep, "value", nodep->valuep(), CONTEXT, FINAL,
                   vdtypep->subDTypep(), EXTEND_EXP);
    }
  }
  virtual void visit(AstConsDynArray *nodep) override {
    // Type computed when constructed here
    AstDynArrayDType *vdtypep =
        VN_AS(m_vup->dtypep()->skipRefp(), DynArrayDType);
    UASSERT_OBJ(vdtypep, nodep,
                "ConsDynArray requires queue upper parent data type");
    if (m_vup->prelim()) {
    ...
    if (m_vup->final()) {
      ...
      if (nodep->lhsp()) {
        if (VN_IS(nodep->lhsp()->dtypep(), DynArrayDType) ||
          ...
          // Sub elements are not queues, but concats, must always pass concats
          // down
          iterateCheckTyped(nodep, "LHS", nodep->lhsp(), vdtypep->subDTypep(),
                            FINAL);
        }
      }
      if (nodep->rhsp()) {
        if (VN_IS(nodep->rhsp()->dtypep(), DynArrayDType) ||
          ...
          iterateCheckTyped(nodep, "RHS", nodep->rhsp(), vdtypep->subDTypep(),
                            FINAL);
        }
      }
      nodep->dtypeFrom(vdtypep);
  ...
  virtual void visit(AstConsQueue *nodep) override {
    ...
    UASSERT_OBJ(vdtypep, nodep,
                "ConsQueue requires queue upper parent data type");
    if (m_vup->prelim()) {
      userIterateAndNext(nodep->lhsp(), WidthVP(vdtypep, PRELIM).p());
      userIterateAndNext(nodep->rhsp(), WidthVP(vdtypep, PRELIM).p());
      nodep->dtypeFrom(vdtypep);
  ...
  virtual void visit(AstInitArray *nodep) override {
    ...
    if (m_vup->prelim()) { // First stage evaluation
      ...
      nodep->dtypep(vdtypep);
      if (AstNodeArrayDType *const arrayp =
  ...
  virtual void visit(AstInside *nodep) override {
    ...
    // Apply width
    AstNodeDType *subDTypep = nodep->findLogicDType(
        width, mwidth, nodep->exprp()->dtypep()->numeric());
    iterateCheck(nodep, "Inside expression", nodep->exprp(), CONTEXT, FINAL,
                 subDTypep, EXTEND_EXP);
    for (AstNode *itemp = nodep->itemsp(); itemp; itemp = itemp->nextp()) {
      iterateCheck(nodep, "Inside Item", itemp, CONTEXT, FINAL, subDTypep,
                   EXTEND_EXP);
    }
    nodep->dtypeSetBit();
    if (debug() >= 9)
      nodep->dumpTree(cout, "-inside-in: ");
    ...
    for (AstNode *nextip, *itemp = nodep->itemsp(); itemp; itemp = nextip) {
      ...
      if (AstInsideRange *irangep = VN_CAST(itemp, InsideRange)) {
        ...
        inewp = irangep->newAndFromInside(nodep->exprp(),
                                          irangep->lhsp()->unlinkFrBack(),
                                          irangep->rhsp()->unlinkFrBack());
      } else if (VN_IS(itemDtp, UnpackArrayDType)) {
        nodep->v3error(
            "Unsupported: inside (set membership operator) on unpacked array");
        ...
        nodep->v3error("Inside operator not legal on non-unpacked arrays (IEEE "
                       "1800-2017 11.4.13)");
        continue;
      } else {
        inewp =
            new AstEqWild(itemp->fileline(), nodep->exprp()->cloneTree(true),
                          itemp->unlinkFrBack());
      }
      if (newp) {
  ...
  virtual void visit(AstNodeUOrStructDType *nodep) override {
    ...
    for (AstMemberDType *backip; itemp; itemp = backip) {
      if (itemp->isFourstate())
        nodep->isFourstate(true);
    ...
    nodep->widthForce(width,
                      width); // Signing stays as-is, as parsed from declaration
  ...
  virtual void visit(AstMemberSel *nodep) override {
    ...
    if (debug() >= 9)
      nodep->dumpTree("-mbs-in: ");
    userIterateChildren(nodep, WidthVP(SELF, BOTH).p());
    if (debug() >= 9)
      nodep->dumpTree("-mbs-ic: ");
    // Find the fromp dtype - should be a class
    if (!nodep->fromp()->dtypep())
      nodep->fromp()->v3fatalSrc("Unlinked data type");
    AstNodeDType *fromDtp = nodep->fromp()->dtypep()->skipRefToEnump();
    UINFO(9, "     from dt " << fromDtp << endl);
    if (AstNodeUOrStructDType *adtypep = VN_CAST(fromDtp, NodeUOrStructDType)) {
      if (memberSelStruct(nodep, adtypep))
        return;
      ...
      // Convert this into a method call, and let that visitor figure out what
      // to do next
      AstNode *newp =
  ...
  AstNode *memberSelClass(AstMemberSel *nodep, AstClassRefDType *adtypep) {
    ...
    for (AstClass *classp = first_classp; classp;) {
      if (AstNode *foundp = classp->findMember(nodep->name()))
        return foundp;
  ...
  bool memberSelStruct(AstMemberSel *nodep, AstNodeUOrStructDType *adtypep) {
    ...
    if (AstMemberDType *memberp = adtypep->findMember(nodep->name())) {
      if (m_attrp) { // Looking for the base of the attribute
        ...
        AstSel *newp =
            new AstSel(nodep->fileline(), nodep->fromp()->unlinkFrBack(),
                       memberp->lsb(), memberp->width());
        // Must skip over the member to find the union; as the member may
        // disappear later
  ...
  virtual void visit(AstCMethodHard *nodep) override {
    ...
    UASSERT_OBJ(nodep->dtypep(), nodep,
                "CMETHODCALLs should have already been sized");
  }
  ...
  virtual void visit(AstMethodCall *nodep) override {
    ...
    for (AstArg *argp = VN_CAST(nodep->pinsp(), Arg); argp;
      ...
      if (argp->exprp())
        userIterate(argp->exprp(), WidthVP(SELF, BOTH).p());
    }
    // Find the fromp dtype - should be a class
    UASSERT_OBJ(nodep->fromp() && nodep->fromp()->dtypep(), nodep,
                "Unsized expression");
    ...
    } else if (AstAssocArrayDType *adtypep =
  ...
  AstWith *methodWithArgument(AstMethodCall *nodep, bool required,
                              bool arbReturn, AstNodeDType *returnDtp,
                              AstNodeDType *indexDtp, AstNodeDType *valueDtp) {
  ...
  void methodOkArguments(AstMethodCall *nodep, int minArg, int maxArg) {
    ...
    if (!ok) {
      nodep->v3error(
          "The " << narg << " arguments passed to ." << nodep->prettyName()
  ...
  void methodCallEnum(AstMethodCall *nodep, AstEnumDType *adtypep) {
    ...
    if (nodep->name() == "num"      //
      ...
      if (nodep->name() == "num") {
        int items = 0;
        for (AstNode *itemp = adtypep->itemsp(); itemp; itemp = itemp->nextp())
          ++items;
      ...
      newp->fileline(
          nodep->fileline()); // Use method's filename/line number to be
                              // clearer; may have warning disables
      nodep->replaceWith(newp);
      VL_DO_DANGLING(pushDeletep(nodep), nodep);
    } else if (nodep->name() == "name" || nodep->name() == "next" ||
               nodep->name() == "prev") {
      ...
      if (nodep->name() == "name") {
        ...
        nodep->pinsp()->v3fatalSrc(
            "Unsupported: enum next/prev with non-const argument");
                 ...
                 !(VN_IS(VN_AS(nodep->pinsp(), Arg)->exprp(), Const) &&
                   VN_AS(VN_AS(nodep->pinsp(), Arg)->exprp(), Const)
                           ->toUInt() == 1 &&
                   !nodep->pinsp()->nextp())) {
      ...
      const int selwidth =
          V3Number::log2b(msbdim) + 1; // Width to address a bit
      ...
      AstNode *newp = new AstArraySel(
          ...
          new AstSel(nodep->fileline(), nodep->fromp()->unlinkFrBack(), 0,
                     selwidth));
      nodep->replaceWith(newp);
      VL_DO_DANGLING(nodep->deleteTree(), nodep);
    } else {
  ...
  void methodCallAssoc(AstMethodCall *nodep, AstAssocArrayDType *adtypep) {
    ...
    if (nodep->name() == "num" // function int num()
      ...
      if (!nodep->pinsp()) {
        newp = new AstCMethodHard(nodep->fileline(),
                                  nodep->fromp()->unlinkFrBack(), "clear",
                                  nullptr);
        newp->makeStatement();
      } else {
        AstNode *index_exprp = methodCallAssocIndexExpr(nodep, adtypep);
        newp = new AstCMethodHard(nodep->fileline(),
                                  nodep->fromp()->unlinkFrBack(), "erase",
                                  index_exprp->unlinkFrBack());
        newp->makeStatement();
      }
    ...
    } else if (nodep->name() == "and" || nodep->name() == "or" ||
               nodep->name() == "xor" || nodep->name() == "sum" ||
               nodep->name() == "product") {
      // All value return
      AstWith *withp =
      ...
      if (!nodep->firstAbovep())
        newp->makeStatement();
    } else if (nodep->name() == "min" || nodep->name() == "max" ||
               nodep->name() == "unique" || nodep->name() == "unique_index") {
      methodOkArguments(nodep, 0, 0);
      methodCallLValueRecurse(nodep, nodep->fromp(), VAccess::READ);
      newp =
          new AstCMethodHard(nodep->fileline(), nodep->fromp()->unlinkFrBack(),
      ...
      if (!nodep->firstAbovep())
        newp->makeStatement();
    } else if (nodep->name() == "find_index" ||
               nodep->name() == "find_first_index" ||
      ...
      nodep->v3error("Unknown built-in associative array method "
                     << nodep->prettyNameQ());
  ...
  AstNode *methodCallAssocIndexExpr(AstMethodCall *nodep,
                                    AstAssocArrayDType *adtypep) {
  ...
  void methodCallLValueRecurse(AstMethodCall *nodep, AstNode *childp,
  ...
  void methodCallDyn(AstMethodCall *nodep, AstDynArrayDType *adtypep) {
    ...
    if (nodep->name() == "at") { // Created internally for []
      ...
      methodCallLValueRecurse(nodep, nodep->fromp(), VAccess::WRITE);
      newp = new AstCMethodHard(nodep->fileline(),
                                nodep->fromp()->unlinkFrBack(), "at", nullptr);
      newp->dtypeFrom(adtypep->subDTypep());
    } else if (nodep->name() == "size") {
      methodOkArguments(nodep, 0, 0);
      newp = new AstCMethodHard(
      ...
      methodCallLValueRecurse(nodep, nodep->fromp(), VAccess::WRITE);
      newp = new AstCMethodHard(
          nodep->fileline(), nodep->fromp()->unlinkFrBack(), "clear", nullptr);
      newp->makeStatement();
    } else if (nodep->name() == "and" || nodep->name() == "or" ||
               nodep->name() == "xor" || nodep->name() == "sum" ||
               nodep->name() == "product") {
      ...
      if (nodep->name() == "sort" || nodep->name() == "rsort") {
        withp =
            methodWithArgument(nodep, false, true, nullptr,
                               nodep->findUInt32DType(), adtypep->subDTypep());
      }
      methodOkArguments(nodep, 0, 0);
      methodCallLValueRecurse(nodep, nodep->fromp(), VAccess::WRITE);
    ...
    if (nodep->name() == "at") { // Created internally for []
      ...
      if (!nodep->pinsp()) {
        ...
        if (index_exprp->isZero()) { // delete(0) is a pop_front
          newp = new AstCMethodHard(nodep->fileline(),
                                    nodep->fromp()->unlinkFrBack(), "pop_front",
                                    nullptr);
          newp->dtypeFrom(adtypep->subDTypep());
          newp->makeStatement();
        } else {
      ...
      iterateCheckTyped(nodep, "insert value", argp->exprp(),
                        adtypep->subDTypep(), BOTH);
      if (index_exprp->isZero()) { // insert(0, ...) is a push_front
        newp = new AstCMethodHard(nodep->fileline(),
                                  nodep->fromp()->unlinkFrBack(), "push_front",
      ...
      iterateCheckTyped(nodep, "push value", argp->exprp(),
                        adtypep->subDTypep(), BOTH);
      newp =
          new AstCMethodHard(nodep->fileline(), nodep->fromp()->unlinkFrBack(),
      ...
      nodep->v3warn(E_UNSUPPORTED, "Unsupported/unknown built-in queue method "
                                       << nodep->prettyNameQ());
      nodep->dtypeFrom(adtypep->subDTypep()); // Best guess
    }
    if (newp) {
  ...
  void methodCallClass(AstMethodCall *nodep, AstClassRefDType *adtypep) {
    ...
    }
    {
      VSpellCheck speller;
      for (AstClass *classp = first_classp; classp;) {
        for (AstNode *itemp = classp->membersp(); itemp;
             itemp = itemp->nextp()) {
          if (VN_IS(itemp, NodeFTask))
            speller.pushCandidate(itemp->prettyName());
        }
        classp = classp->extendsp() ? classp->extendsp()->classp() : nullptr;
      }
    ...
    if (methodId) {
      ...
      for (int i = 0; i < adtypep->elementsConst(); ++i) {
        ...
        if (!newp) {
          ...
          switch (methodId) {
          case ARRAY_OR:
            newp = new AstOr(fl, newp, selector);
            break;
          case ARRAY_AND:
            newp = new AstAnd(fl, newp, selector);
            break;
          case ARRAY_XOR:
            newp = new AstXor(fl, newp, selector);
            break;
          case ARRAY_SUM:
            newp = new AstAdd(fl, newp, selector);
            break;
          case ARRAY_PRODUCT:
            newp = new AstMul(fl, newp, selector);
            break;
          default:
            nodep->v3fatalSrc("bad case");
          }
  ...
  void methodCallString(AstMethodCall *nodep, AstBasicDType *) {
    ...
    if (nodep->name() == "len") {
      ...
      AstNode *newp =
          new AstAssign(nodep->fileline(), fromp,
  ...
  AstQueueDType *queueDTypeIndexedBy(AstNodeDType *indexDTypep) {
    // Return a Queue data type with the specified index, remembering so can use
    // again if needed
  ...
  virtual void visit(AstNewCopy *nodep) override {
    ...
    if (!similarDTypeRecurse(nodep->dtypep(), nodep->rhsp()->dtypep())) {
      nodep->rhsp()->v3error(
          "New-as-copier passed different data type '"
  ...
  virtual void visit(AstNewDynamic *nodep) override {
    if (nodep->didWidthAndSet())
      return;
    AstDynArrayDType *adtypep =
        VN_CAST(m_vup->dtypeNullSkipRefp(), DynArrayDType);
    if (!adtypep) { // e.g. int a = new;
      nodep->v3error("dynamic new() not expected in this context (data type "
                     "must be dynamic array)");
      return;
    }
    // The AstNodeAssign visitor will be soon be replacing this node, make sure
    // it gets it
    if (!VN_IS(nodep->backp(), NodeAssign)) {
      if (adtypep)
        UINFO(1, "Got backp " << nodep->backp() << endl);
      nodep->v3error("dynamic new() not expected in this context (expected "
                     "under an assign)");
      return;
    }
    ...
    if (nodep->rhsp()) {
      iterateCheckTyped(nodep, "Dynamic array new RHS", nodep->rhsp(), adtypep,
                        BOTH);
    }
  }
  ...
  virtual void visit(AstPattern *nodep) override {
    ...
    if (!nodep->dtypep() &&
        m_vup->dtypeNullp()) { // Get it from parent assignment/pin/etc
    ...
    }
    {
      ...
      for (AstPatMember *patp = VN_AS(nodep->itemsp(), PatMember); patp;
        ...
        if (patp->lhssp()->nextp()) {
          ...
          while (AstNode *movep = patp->lhssp()->nextp()) {
            ...
            if (patp->keyp())
              newkeyp = patp->keyp()->cloneTree(true);
            AstPatMember *newp =
      ...
      if (auto *vdtypep = VN_CAST(dtypep, NodeUOrStructDType)) {
        ...
        VL_DO_DANGLING(patternQueue(nodep, vdtypep, defaultp), nodep);
                 ...
                 VN_AS(dtypep, BasicDType)->isRanged()) {
  ...
  void patternArray(AstPattern *nodep, AstNodeArrayDType *arrayDtp,
                    AstPatMember *defaultp) {
    ...
    for (int entn = 0, ent = range.left(); entn < range.elements();
      ...
      if (it == patmap.end()) {
        if (defaultp) {
          ...
          nodep->v3error(
              "Assignment pattern missed initializing elements: " << ent);
        }
      } else {
        patp = it->second;
        patmap.erase(it);
    ...
    if (!patmap.empty())
      nodep->v3error("Assignment pattern with too many elements");
    if (newp) {
      nodep->replaceWith(newp);
    } else {
      nodep->v3error("Assignment pattern with no members");
  ...
  void patternAssoc(AstPattern *nodep, AstAssocArrayDType *arrayDtp,
                    AstPatMember *defaultp) {
    AstNode *defaultValuep = nullptr;
    if (defaultp)
      defaultValuep = defaultp->lhssp()->unlinkFrBack();
    ...
    for (AstPatMember *patp = VN_AS(nodep->itemsp(), PatMember); patp;
      ...
      auto *newap = new AstSetAssoc(nodep->fileline(), newp,
                                    keyp->unlinkFrBack(), valuep);
  ...
  void patternDynArray(AstPattern *nodep, AstDynArrayDType *arrayp,
                       AstPatMember *) {
  ...
  void patternBasic(AstPattern *nodep, AstNodeDType *vdtypep,
                    AstPatMember *defaultp) {
  ...
  virtual void visit(AstPatMember *nodep) override {
    ...
    UASSERT_OBJ(vdtypep, nodep,
                "Pattern member type not assigned by AstPattern visitor");
    nodep->dtypep(vdtypep);
    ...
    iterateCheck(nodep, "Pattern value", nodep->lhssp(), ASSIGN, FINAL, vdtypep,
                 EXTEND_LHS);
  }
  ...
  virtual void visit(AstNodeCase *nodep) override {
    ...
    for (AstCaseItem *nextip, *itemp = nodep->itemsp(); itemp; itemp = nextip) {
      nextip = VN_AS(itemp->nextp(),
                     CaseItem); // Prelim may cause the node to get replaced
      if (!VN_IS(nodep, GenCase))
        userIterateAndNext(itemp->bodysp(), nullptr);
    ...
    // Take width as maximum across all items, if any is real whole thing is
    // real
      ...
      for (AstNode *condp = itemp->condsp(); condp; condp = condp->nextp()) {
        if (condp->dtypep() != subDTypep) {
          if (condp->dtypep()->isDouble()) {
            ...
            const int mwidth =
                std::max(subDTypep->widthMin(), condp->widthMin());
    ...
    // Apply width
    iterateCheck(nodep, "Case expression", nodep->exprp(), CONTEXT, FINAL,
                 subDTypep, EXTEND_LHS);
    for (AstCaseItem *itemp = nodep->itemsp(); itemp;
         itemp = VN_AS(itemp->nextp(), CaseItem)) {
      for (AstNode *nextcp, *condp = itemp->condsp(); condp; condp = nextcp) {
        nextcp = condp->nextp(); // Final may cause the node to get replaced
        iterateCheck(nodep, "Case Item", condp, CONTEXT, FINAL, subDTypep,
                     EXTEND_LHS);
      }
    }
  }
  ...
  virtual void visit(AstNodeIf *nodep) override {
    ...
    iterateCheckBool(nodep, "If", nodep->condp(),
                     BOTH); // it's like an if() condition.
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    // if (debug()) nodep->dumpTree(cout, "  AssignPre: ");
    {
      ...
      nodep->dtypeFrom(nodep->lhsp());
      //
      // AstPattern needs to know the proposed data type of the lhs, so pass on
      // the prelim
    ...
    if (AstBasicDType *basicp = nodep->rhsp()->dtypep()->basicp()) {
      if (basicp->isEventValue()) {
        ...
        nodep->v3warn(E_UNSUPPORTED,
                      "Unsupported: assignment of event data type");
      }
    }
    ...
    if (AstNewDynamic *dynp = VN_CAST(nodep->rhsp(), NewDynamic)) {
      ...
      if (!dynp->rhsp()) {
        newp =
            new AstCMethodHard(nodep->fileline(), nodep->lhsp()->unlinkFrBack(),
  ...
  virtual void visit(AstSFormatF *nodep) override {
    ...
    if (m_vup && !m_vup->prelim())
      return; // Can be called as statement or function
    ...
    for (char ch : txt) {
      if (!inPct && ch == '%') {
        ...
        case '%':
          break; // %% - just output a %
        case 'm':
          break; // %m - auto insert "name"
        case 'l':
          break;    // %m - auto insert "library"
  ...
  virtual void visit(AstElabDisplay *nodep) override {
    ...
    if (!m_paramsOnly) {
      ...
      if (text.empty())
        text = "Elaboration system task message (IEEE 1800-2017 20.11)";
      switch (nodep->displayType()) {
      case AstDisplayType::DT_INFO:
        nodep->v3warn(USERINFO, text);
        break;
      case AstDisplayType::DT_ERROR:
        nodep->v3warn(USERERROR, text);
        break;
      case AstDisplayType::DT_WARNING:
        nodep->v3warn(USERWARN, text);
        break;
      case AstDisplayType::DT_FATAL:
        nodep->v3warn(USERFATAL, text);
        break;
      default:
        UASSERT_OBJ(false, nodep, "Unexpected elaboration display type");
      }
      VL_DO_DANGLING(nodep->unlinkFrBack()->deleteTree(), nodep);
    }
  }
  ...
  virtual void visit(AstFOpen *nodep) override {
    // Although a system function in IEEE, here a statement which sets the file
    // pointer (MCD)
    assertAtStatement(nodep);
    iterateCheckFileDesc(nodep, nodep->filep(), BOTH);
  ...
  virtual void visit(AstFError *nodep) override {
    if (m_vup->prelim()) {
      ...
      nodep->dtypeSetLogicUnsized(32, 1,
                                  VSigning::SIGNED); // Spec says integer return
    }
  }
  ...
  virtual void visit(AstFGetC *nodep) override {
    if (m_vup->prelim()) {
      iterateCheckFileDesc(nodep, nodep->filep(), BOTH);
      nodep->dtypeSetLogicUnsized(32, 8,
                                  VSigning::SIGNED); // Spec says integer return
    }
  }
  ...
  virtual void visit(AstNodeReadWriteMem *nodep) override {
    ...
    if (AstAssocArrayDType *adtypep =
      ...
      if (!adtypep->keyDTypep()->skipRefp()->basicp() || !adtypep->keyDTypep()
                                                              ->skipRefp()
                                                              ->basicp()
                                                              ->keyword()
                                                              .isIntNumeric()) {
    ...
    } else if (AstUnpackArrayDType *adtypep = VN_CAST(
                   nodep->memp()->dtypep()->skipRefp(), UnpackArrayDType)) {
      subp = adtypep->subDTypep();
    } else {
      nodep->memp()->v3warn(
          E_UNSUPPORTED,
    ...
    if (subp && (!subp->skipRefp()->basicp() ||
      ...
      nodep->memp()->v3warn(
          E_UNSUPPORTED, "Unsupported: " << nodep->verilogKwd()
      ...
      if (conDTypep == modDTypep // If match, we're golden
        ...
        if (conwidth == pinwidth) {
          ...
          nodep->v3error(ucfirst(nodep->prettyOperatorName())
                         ...
                         << " requires " << pinwidth << " or "
                         << pinwidth * numInsts << " bits, but connection's "
                         << nodep->exprp()->prettyTypeName() << " generates "
                         << conwidth << " bits. (IEEE 1800-2017 23.3.3)");
          subDTypep = conDTypep; // = same expr dtype
        }
        userIterateAndNext(nodep->exprp(), WidthVP(subDTypep, FINAL).p());
      } else {
        ...
        if ((VN_IS(modDTypep, IfaceRefDType) &&
             !VN_IS(conDTypep, IfaceRefDType)) ||
            (VN_IS(conDTypep, IfaceRefDType) &&
             !VN_IS(modDTypep, IfaceRefDType))) {
        ...
        if (exprArrayp && modArrayp &&
            VN_IS(exprArrayp->subDTypep(), IfaceRefDType) &&
            exprArrayp->declRange().elements() !=
                modArrayp->declRange().elements()) {
        ...
        iterateCheckAssign(nodep, "pin connection", nodep->exprp(), FINAL,
                           subDTypep);
      }
    }
  ...
  virtual void visit(AstCell *nodep) override {
    ...
    if (!m_paramsOnly) {
      if (VN_IS(nodep->modp(), NotFoundModule)) {
        // We've resolved parameters and hit a module that we couldn't resolve.
        // It's finally time to report it. Note only here in V3Width as this is
        // first visitor after V3Dead.
        nodep->modNameFileline()->v3error(
            "Cannot find file containing module: '" << nodep->modName() << "'");
    ...
    if (m_vup->prelim()) {
      ...
      if (conwidth == 1 && pinwidth > 1) { // Multiple connections
        AstNodeDType *subDTypep =
            nodep->findLogicDType(1, 1, conDTypep->numeric());
        userIterateAndNext(nodep->exprp(), WidthVP(subDTypep, FINAL).p());
        AstNode *newp = new AstReplicate(
            nodep->fileline(), nodep->exprp()->unlinkFrBack(), numInsts);
        nodep->replaceWith(newp);
      } else {
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    if (nodep->doingWidth()) {
      nodep->v3warn(E_UNSUPPORTED,
                    "Unsupported: Recursive function or task call");
      nodep->dtypeSetBit();
      nodep->didWidth(true);
      return;
    }
    ...
    // Would use user1 etc, but V3Width called from too many places to spend a
    // user
    nodep->doingWidth(true);
    m_ftaskp = nodep;
    userIterateChildren(nodep, nullptr);
    if (nodep->isConstructor()) {
      ...
      UASSERT_OBJ(m_funcp, nodep,
                  "FTask with function variable, but isn't a function");
      nodep->dtypeFrom(
          nodep->fvarp()); // Which will get it from fvarp()->dtypep()
    }
    nodep->didWidth(true);
    nodep->doingWidth(false);
    m_funcp = nullptr;
    ...
    if (nodep->dpiImport() && !nodep->dpiOpenParent() &&
      ...
      nodep->dpiOpenParentInc(); // Mark so V3Task will wait for a child to
                                 // build calling func
    }
  }
  virtual void visit(AstReturn *nodep) override {
    ...
    if (!m_funcp) {
      ...
      if (nodep->lhsp()) {
        ...
        iterateCheckAssign(nodep, "Return value", nodep->lhsp(), FINAL,
                           nodep->dtypep());
      }
    }
  }
  ...
  static bool areSameSize(AstUnpackArrayDType *dtypep0,
                          AstUnpackArrayDType *dtypep1) {
    const std::vector<AstUnpackArrayDType *> dims0 =
        dtypep0->unpackDimensions();
    const std::vector<AstUnpackArrayDType *> dims1 =
        dtypep1->unpackDimensions();
    if (dims0.size() != dims1.size())
      return false;
    for (size_t i = 0; i < dims0.size(); ++i) {
      if (dims0[i]->elementsConst() != dims1[i]->elementsConst())
        return false;
    }
    return true;
  }
    ...
    if (AstUnpackArrayDType *portDtypep =
      ...
      if (AstUnpackArrayDType *pinDtypep =
        ...
        pinp->v3warn(E_UNSUPPORTED,
                     "Argument is not an unpacked array while parameter "
                         << portp->prettyNameQ() << " is");
      }
    }
  }
  void processFTaskRefArgs(AstNodeFTaskRef *nodep) {
    // For arguments, is assignment-like context; see IEEE rules in
    // AstNodeAssign Function hasn't been widthed, so make it so.
    ...
    reloop:
      V3TaskConnects tconnects =
          V3Task::taskConnects(nodep, nodep->taskp()->stmtsp());
      for (const auto &tconnect : tconnects) {
        AstVar *portp = tconnect.first;
        AstArg *argp = tconnect.second;
        AstNode *pinp = argp->exprp();
        if (!pinp)
          continue; // Argument error we'll find later
        ...
        if (portp->attrSFormat() &&
          ...
          AstNRelinker handle;
          argp->unlinkFrBackWithNext(
              &handle); // Format + additional args, if any
          AstNode *argsp = nullptr;
          while (AstArg *nextargp = VN_AS(argp->nextp(), Arg)) {
            ...
            nextargp->unlinkFrBack()
                ->deleteTree(); // Remove the call's Arg wrapper
          }
          string format;
          if (VN_IS(pinp, Const)) {
            format = VN_AS(pinp, Const)->num().toString();
          } else {
            pinp->v3error("Format to $display-like function must have constant "
                          "format string");
          }
          ...
          goto reloop; // so exit early; next loop will correct it
                 ...
                 && !VN_IS(portp->dtypep(),
                           UnpackArrayDType) // Unpacked array must match
                 && !(VN_IS(pinp, VarRef) &&
        ...
        // AstPattern requires assignments to pass datatype on PRELIM
        VL_DO_DANGLING(userIterate(pinp, WidthVP(portp->dtypep(), PRELIM).p()),
                       pinp);
      }
    } while (false);
    // Stage 2
    {
      ...
      for (const auto &tconnect : tconnects) {
        ...
        if (nodep->taskp()->dpiImport())
          checkUnpackedArrayArgs(portp, pinp);
        if (portp->isDouble())
    ...
    // Stage 4
    {
      ...
      for (const auto &tconnect : tconnects) {
        ...
        if (!portp->basicp() || portp->basicp()->isOpaque()) {
          ...
          iterateCheckAssign(nodep, "Function Argument", pinp, FINAL,
                             portp->dtypep());
        }
      }
    }
  ...
  virtual void visit(AstNodeMath *nodep) override {
    if (!nodep->didWidth()) {
      nodep->v3fatalSrc(
          "Visit function missing? Widthed function missing for math node: "
          << nodep);
    }
    userIterateChildren(nodep, nullptr);
  }
  ...
  void visit_Or_Lu64(AstNodeUniop *nodep) {
    ...
    if (m_vup->prelim()) { // First stage evaluation
      nodep->dtypeSetDouble();
      AstNodeDType *subDTypep =
          nodep->findLogicDType(64, 64, VSigning::UNSIGNED);
      // Self-determined operand
      userIterateAndNext(nodep->lhsp(), WidthVP(SELF, PRELIM).p());
      iterateCheck(nodep, "LHS", nodep->lhsp(), SELF, FINAL, subDTypep,
                   EXTEND_EXP);
    }
  }
  ...
  void visit_cmp_eq_gt(AstNodeBiop *nodep, bool realok) {
    ...
    //   TODO: chandle/class handle/iface handle only allowed to self-compare or
    //   against null
    ...
    if (m_vup->prelim()) {
      ...
      if (nodep->lhsp()->isDouble() || nodep->rhsp()->isDouble()) {
        if (!realok)
          nodep->v3error("Real not allowed as operand to in ?== operator");
        ...
        const int width =
            std::max(nodep->lhsp()->width(), nodep->rhsp()->width());
        const int ewidth =
            std::max(nodep->lhsp()->widthMin(), nodep->rhsp()->widthMin());
        AstNodeDType *subDTypep =
            nodep->findLogicDType(width, ewidth, VSigning::fromBool(signedFl));
        bool warnOn = true;
        if (!signedFl && width == 32) {
          // Waive on unsigned < or <= if RHS is narrower, since can't give
          // wrong answer
          if ((VN_IS(nodep, Lt) || VN_IS(nodep, Lte)) &&
              (nodep->lhsp()->width() >= nodep->rhsp()->widthMin())) {
            warnOn = false;
          }
          // Waive on unsigned > or >= if RHS is wider, since can't give wrong
          // answer
  ...
  void visit_shift(AstNodeBiop *nodep) {
    ...
    //   RHS is self-determined. RHS is always treated as unsigned, has no
    //   effect on result.
  ...
  AstNodeBiop *iterate_shift_final(AstNodeBiop *nodep) {
    ...
    if (m_vup->final()) {
      ...
      if (VN_IS(nodep, ShiftRS)) {
        if (AstNodeBiop *newp =
                replaceWithUOrSVersion(nodep, nodep->isSigned())) {
          VL_DANGLING(nodep);
          nodep = newp; // Process new node instead
        }
      }
      ...
      if (nodep->lhsp()->isOne() && VN_IS(nodep->backp(), NodeAssign))
        warnOn = false;
      iterateCheck(nodep, "LHS", nodep->lhsp(), CONTEXT, FINAL, subDTypep,
                   EXTEND_EXP, warnOn);
  ...
  void visit_boolmath_and_or(AstNodeBiop *nodep) {
    ...
    //    Width: max(LHS, RHS)
    UASSERT_OBJ(nodep->rhsp(), nodep, "For binary ops only!");
    // If errors are off, we need to follow the spec; thus we really need to do
    // the max() because the rhs could be larger, and we need to have proper
    // editing to get the widths to be the same for our operations.
    if (m_vup->prelim()) { // First stage evaluation
  ...
  void visit_add_sub_replace(AstNodeBiop *nodep, bool real_ok) {
    ...
    // editing to get the widths to be the same for our operations.
    //
    // if (debug() >= 9) { UINFO(0,"-rus "<<m_vup<<endl); nodep->dumpTree(cout,
    // "-rusin-"); }
    if (m_vup->prelim()) { // First stage evaluation
      // Determine expression widths only relying on what's in the subops
      userIterateAndNext(nodep->lhsp(), WidthVP(CONTEXT, PRELIM).p());
    ...
    if (m_vup->final()) {
      ...
      AstNodeDType *subDTypep = expDTypep;
      nodep->dtypeFrom(expDTypep);
      // We don't use LHS && RHS -- unspecified language corner, see
      // t_math_signed5 test bool expSigned = (nodep->lhsp()->isSigned() &&
      // nodep->rhsp()->isSigned());
      if (AstNodeBiop *newp =
              replaceWithUOrSVersion(nodep, expDTypep->isSigned())) {
        VL_DANGLING(nodep);
        nodep = newp; // Process new node instead
      }
      ...
      if (VN_IS(nodep, Add) || VN_IS(nodep, Sub)) {
        ...
        if (subDTypep->widthMin() == (nodep->lhsp()->widthMin() + 1))
          lhsWarn = false;
        if (subDTypep->widthMin() == (nodep->rhsp()->widthMin() + 1))
          rhsWarn = false;
      } else if (VN_IS(nodep, Mul) || VN_IS(nodep, MulS)) {
        if (subDTypep->widthMin() >= (nodep->lhsp()->widthMin()))
          lhsWarn = false;
        if (subDTypep->widthMin() >= (nodep->rhsp()->widthMin()))
          rhsWarn = false;
      }
  ...
  bool widthBad(AstNode *nodep, AstNodeDType *expDTypep) {
    ...
    UASSERT_OBJ(expWidth != 0, nodep,
                "Node " << nodep->prettyTypeName()
                        << " has no expected width?? Missing Visitor func?");
    if (expWidthMin == 0)
      expWidthMin = expWidth;
    if (nodep->dtypep()->width() == expWidth)
      return false;
    if (nodep->dtypep()->widthSized() && nodep->width() != expWidthMin)
      return true;
    if (!nodep->dtypep()->widthSized() && nodep->widthMin() > expWidthMin)
      return true;
    return false;
  }
  ...
  void fixWidthExtend(AstNode *nodep, AstNodeDType *expDTypep,
    ...
    // for example $unsigned(a)+$signed(b), the SIGNED(B) will be unsigned dtype
    // out
    UINFO(4, "  widthExtend_(r=" << extendRule << ") old: " << nodep << endl);
    if (extendRule == EXTEND_OFF)
      return;
    ...
    if (constp && !constp->num().isNegative()) {
      ...
      case EXTEND_ZERO:
        ...
        break;
      case EXTEND_EXP:
        doSigned = nodep->isSigned() && expDTypep->isSigned();
        break;
      case EXTEND_LHS:
        doSigned = nodep->isSigned();
        break;
      default:
        nodep->v3fatalSrc("bad case");
      }
      AstNode *const newp =
          (doSigned ? static_cast<AstNode *>(
                          new AstExtendS{nodep->fileline(), nodep})
                    : static_cast<AstNode *>(
                          new AstExtend{nodep->fileline(), nodep}));
      linker.relink(newp);
      nodep = newp;
    }
  ...
  bool fixAutoExtend(AstNode *&nodepr, int expWidth) {
    ...
    if (AstConst *constp = VN_CAST(nodepr, Const)) {
      if (constp->num().autoExtend() && !constp->num().sized() &&
          constp->width() == 1) {
        ...
        if (debug() > 4)
          constp->dumpTree(cout, "  fixAutoExtend_old: ");
        if (debug() > 4)
          newp->dumpTree(cout, "               _new: ");
        constp->replaceWith(newp);
        VL_DO_DANGLING(constp->deleteTree(), constp);
      ...
      // X/Z also upper bit extend.  In pre-SV only to 32-bits, SV forever.
               ...
               // Make it the proper size.  Careful of proper extension of
               // 0's/1's
        ...
        // Spec says always unsigned with proper width
        if (debug() > 4)
          constp->dumpTree(cout, "  fixUnszExtend_old: ");
        if (debug() > 4)
          newp->dumpTree(cout, "               _new: ");
        constp->replaceWith(newp);
        VL_DO_DANGLING(constp->deleteTree(), constp);
  ...
  void iterateCheckFileDesc(AstNode *nodep, AstNode *underp, Stage stage) {
    ...
    underp = iterateCheck(nodep, "file_descriptor", underp, SELF, FINAL,
                          expDTypep, EXTEND_EXP);
    if (underp) {
    } // cppcheck
  }
  void iterateCheckSigned32(AstNode *nodep, const char *side, AstNode *underp,
                            Stage stage) {
    ...
    if (stage & FINAL) {
      ...
      underp =
          iterateCheck(nodep, side, underp, SELF, FINAL, expDTypep, EXTEND_EXP);
    }
    if (underp) {
    } // cppcheck
  }
  void iterateCheckReal(AstNode *nodep, const char *side, AstNode *underp,
                        Stage stage) {
  ...
  void iterateCheckString(AstNode *nodep, const char *side, AstNode *underp,
                          Stage stage) {
    if (stage & PRELIM) {
      underp = userIterateSubtreeReturnEdits(underp, WidthVP(SELF, PRELIM).p());
    }
  ...
  void iterateCheckTyped(AstNode *nodep, const char *side, AstNode *underp,
    ...
    if (stage & PRELIM) {
      underp =
          userIterateSubtreeReturnEdits(underp, WidthVP(expDTypep, PRELIM).p());
    }
    if (stage & FINAL) {
      underp =
          iterateCheck(nodep, side, underp, SELF, FINAL, expDTypep, EXTEND_EXP);
  ...
  void iterateCheckAssign(AstNode *nodep, const char *side, AstNode *rhsp,
                          Stage stage, AstNodeDType *lhsDTypep) {
    ...
    bool lhsStream = (VN_IS(nodep, NodeAssign) &&
                      VN_IS(VN_AS(nodep, NodeAssign)->lhsp(), NodeStream));
  ...
  void iterateCheckBool(AstNode *nodep, const char *side, AstNode *underp,
                        Stage stage) {
    UASSERT_OBJ(
        stage == BOTH, nodep,
    ...
    UASSERT_OBJ(
        underp && underp->dtypep(), nodep,
        "Node has no type"); // Perhaps forgot to do a prelim visit on it?
    //
    // For DOUBLE under a logical op, add implied test against zero, never a
    // warning
    if (underp && underp->isDouble()) {
      ...
      nodep->v3error("Logical operator "
                     << nodep->prettyTypeName()
      ...
      if (bad) {
        { // if (warnOn), but not needed here
          if (debug() > 4)
            nodep->backp()->dumpTree(cout, "  back: ");
          nodep->v3warn(WIDTH,
                        "Logical operator "
                            << nodep->prettyTypeName()
  ...
  AstNode *iterateCheck(AstNode *nodep, const char *side, AstNode *underp,
                        ...
                        ExtendRule extendRule, bool warnOn = true) {
    // Perform data type check on underp, which is underneath nodep used for
    // error reporting Returns the new underp Conversion to/from doubles and
    // integers are before iterating.
    UASSERT_OBJ(stage == FINAL, nodep, "Bad state to iterateCheck");
    UASSERT_OBJ(
        underp && underp->dtypep(), nodep,
        "Node has no type"); // Perhaps forgot to do a prelim visit on it?
    if (VN_IS(underp,
              NodeDType)) { // Note the node itself, not node's data type
      ...
      underp->v3error(ucfirst(nodep->prettyOperatorName())
                      ...
                      << underp->name() << "' is a datatype.");
    } else if (expDTypep == underp->dtypep()) { // Perfect
      underp =
          userIterateSubtreeReturnEdits(underp, WidthVP(expDTypep, FINAL).p());
      ...
      if (expBasicp && underBasicp) {
        ...
        if (determ == SELF) {
          underp =
              userIterateSubtreeReturnEdits(underp, WidthVP(SELF, FINAL).p());
          ...
          underp = userIterateSubtreeReturnEdits(underp,
                                                 WidthVP(subDTypep, FINAL).p());
        } else {
          underp = userIterateSubtreeReturnEdits(underp,
                                                 WidthVP(subDTypep, FINAL).p());
        }
        // Note the check uses the expected size, not the child's subDTypep as
        // we want the child node's width to end up correct for the assignment
        // (etc)
        ...
        underp = userIterateSubtreeReturnEdits(underp,
                                               WidthVP(expDTypep, FINAL).p());
      }
    }
    return underp;
  }

  void widthCheckSized(
      AstNode *nodep, const char *side,
    ...
    // Issue warnings on sized number width mismatches, then do appropriate size
    // extension Generally iterateCheck is what is wanted instead of this
    // UINFO(9,"wchk "<<side<<endl<<"  "<<nodep<<endl<<"  "<<underp<<endl<<"
    ...
    if (expDTypep == underp->dtypep()) {
      ...
      // fault.
      nodep->v3error(ucfirst(nodep->prettyOperatorName())
                     << " expected non-complex non-double " << side
                     << " in width check");
#if VL_DEBUG
      nodep->v3fatalSrc(
          "widthCheckSized should not be called on doubles/complex types");
#endif
      return;
    } else {
      const int expWidth = expDTypep->width();
      ...
      if (VN_IS(underp, Const) && VN_AS(underp, Const)->num().isFromString() &&
          ...
          (((expWidth - underp->width()) % 8) ==
           0)) { // At least it's character sized
      ...
      if (bad && warnOn) {
        ...
        nodep->v3warn(WIDTH, ucfirst(nodep->prettyOperatorName())
                                 << " expects " << expWidth
                                 << (expWidth != expWidthMin
                                         ? " or " + cvtToStr(expWidthMin)
                                         : "")
                                 << " bits on the " << side << ", but " << side
                                 << "'s " << underp->prettyTypeName()
                                 << " generates " << underp->width()
                                 << (underp->width() != underp->widthMin()
                                         ? " or " + cvtToStr(underp->widthMin())
                                         : "")
  ...
  AstNode *checkCvtUS(AstNode *nodep) {
    if (nodep && nodep->isDouble()) {
      nodep->v3error("Expected integral (non-"
                     << nodep->dtypep()->prettyDTypeName() << ") input to "
                     << nodep->backp()->prettyTypeName());
  ...
  AstNode *spliceCvtS(AstNode *nodep, bool warnOn, int width) {
    ...
    if (nodep && nodep->dtypep()->skipRefp()->isDouble()) {
      ...
      if (AstConst *constp = VN_CAST(nodep, Const)) {
        // We convert to/from vlsint32 rather than use floor() as want to make
        // sure is representable in integer's number of bits
        if (constp->isDouble() &&
            v3EpsilonEqual(constp->num().toDouble(),
                           static_cast<double>(static_cast<vlsint32_t>(
                               constp->num().toDouble())))) {
          warnOn = false;
        }
      }
      if (warnOn)
        nodep->v3warn(REALCVT, "Implicit conversion of real to integer");
  ...
  AstNode *spliceCvtString(AstNode *nodep) {
    ...
    // 11.8.2: Argument to convert is self-determined
    if (nodep &&
        !(nodep->dtypep()->basicp() && nodep->dtypep()->basicp()->isString())) {
  ...
  AstNodeBiop *replaceWithUOrSVersion(AstNodeBiop *nodep,
                                      bool signedFlavorNeeded) {
    ...
    switch (nodep->type()) {
    case AstType::atEq:
      nodep->dtypeChgSigned(signedFlavorNeeded);
      return nullptr;
    case AstType::atNeq:
      nodep->dtypeChgSigned(signedFlavorNeeded);
      return nullptr;
    ...
    case AstType::atNeqCase:
      nodep->dtypeChgSigned(signedFlavorNeeded);
      return nullptr;
    case AstType::atEqWild:
      nodep->dtypeChgSigned(signedFlavorNeeded);
      return nullptr;
    case AstType::atNeqWild:
      nodep->dtypeChgSigned(signedFlavorNeeded);
      return nullptr;
    case AstType::atAdd:
      nodep->dtypeChgSigned(signedFlavorNeeded);
    ...
    switch (nodep->type()) {
    case AstType::atGt:
      newp = new AstGtS(fl, lhsp, rhsp);
      break;
    case AstType::atGtS:
      newp = new AstGt(fl, lhsp, rhsp);
      break;
    case AstType::atGte:
      newp = new AstGteS(fl, lhsp, rhsp);
      break;
    case AstType::atGteS:
      newp = new AstGte(fl, lhsp, rhsp);
      break;
    case AstType::atLt:
      newp = new AstLtS(fl, lhsp, rhsp);
      break;
    case AstType::atLtS:
      newp = new AstLt(fl, lhsp, rhsp);
      break;
    case AstType::atLte:
      newp = new AstLteS(fl, lhsp, rhsp);
      break;
    case AstType::atLteS:
      newp = new AstLte(fl, lhsp, rhsp);
      break;
    case AstType::atDiv:
      newp = new AstDivS(fl, lhsp, rhsp);
      break;
    case AstType::atDivS:
      newp = new AstDiv(fl, lhsp, rhsp);
      break;
    case AstType::atModDiv:
      newp = new AstModDivS(fl, lhsp, rhsp);
      break;
    case AstType::atModDivS:
      newp = new AstModDiv(fl, lhsp, rhsp);
      break;
    case AstType::atMul:
      newp = new AstMulS(fl, lhsp, rhsp);
      break;
    case AstType::atMulS:
      newp = new AstMul(fl, lhsp, rhsp);
      break;
    case AstType::atShiftR:
      newp = new AstShiftRS(fl, lhsp, rhsp);
      break;
    case AstType::atShiftRS:
      newp = new AstShiftR(fl, lhsp, rhsp);
      break;
  ...
  AstNodeBiop *replaceWithDVersion(AstNodeBiop *nodep) {
    ...
    AstNode *rhsp = nodep->rhsp()->unlinkFrBack();
    AstNodeBiop *newp = nullptr;
    // No width change on output;...                // All below have bool or
    // double outputs
    switch (nodep->type()) {
    case AstType::atAdd:
      newp = new AstAddD(fl, lhsp, rhsp);
      break;
    case AstType::atSub:
      newp = new AstSubD(fl, lhsp, rhsp);
      break;
    case AstType::atPow:
      newp = new AstPowD(fl, lhsp, rhsp);
      break;
    case AstType::atEq:
    case AstType::atEqCase:
      newp = new AstEqD(fl, lhsp, rhsp);
      break;
    case AstType::atNeq:
    case AstType::atNeqCase:
      newp = new AstNeqD(fl, lhsp, rhsp);
      break;
    case AstType::atGt:
    case AstType::atGtS:
      newp = new AstGtD(fl, lhsp, rhsp);
      break;
    case AstType::atGte:
    case AstType::atGteS:
      newp = new AstGteD(fl, lhsp, rhsp);
      break;
    case AstType::atLt:
    case AstType::atLtS:
      newp = new AstLtD(fl, lhsp, rhsp);
      break;
    case AstType::atLte:
    case AstType::atLteS:
      newp = new AstLteD(fl, lhsp, rhsp);
      break;
    case AstType::atDiv:
    case AstType::atDivS:
      newp = new AstDivD(fl, lhsp, rhsp);
      break;
    case AstType::atMul:
    case AstType::atMulS:
      newp = new AstMulD(fl, lhsp, rhsp);
      break;
    default: // LCOV_EXCL_LINE
      nodep->v3fatalSrc(
          "Node needs conversion to double, but bad case: " << nodep);
      break;
    }
  ...
  AstNodeBiop *replaceWithNVersion(AstNodeBiop *nodep) {
    ...
    case AstType::atEq:
    case AstType::atEqCase:
      newp = new AstEqN(fl, lhsp, rhsp);
      break;
    case AstType::atNeq:
    case AstType::atNeqCase:
      newp = new AstNeqN(fl, lhsp, rhsp);
      break;
    case AstType::atGt:
    case AstType::atGtS:
      newp = new AstGtN(fl, lhsp, rhsp);
      break;
    case AstType::atGte:
    case AstType::atGteS:
      newp = new AstGteN(fl, lhsp, rhsp);
      break;
    case AstType::atLt:
    case AstType::atLtS:
      newp = new AstLtN(fl, lhsp, rhsp);
      break;
    case AstType::atLte:
    case AstType::atLteS:
      newp = new AstLteN(fl, lhsp, rhsp);
      break;
    default: // LCOV_EXCL_LINE
      nodep->v3fatalSrc(
          "Node needs conversion to string, but bad case: " << nodep);
      break;
    }
  ...
  AstNodeUniop *replaceWithDVersion(AstNodeUniop *nodep) {
    ...
    switch (nodep->type()) {
    case AstType::atNegate:
      newp = new AstNegateD(fl, lhsp);
      break;
    default: // LCOV_EXCL_LINE
      nodep->v3fatalSrc(
          "Node needs conversion to double, but bad case: " << nodep);
  ...
  void replaceWithSFormat(AstMethodCall *nodep, const string &format) {
    ...
    if (!argp) {
      nodep->v3error("Argument needed for string." + nodep->prettyName() +
                     " method");
      return;
    }
    AstNodeVarRef *fromp = VN_AS(nodep->fromp()->unlinkFrBack(), VarRef);
  ...
  AstNodeDType *iterateEditMoveDTypep(AstNode *parentp, AstNodeDType *dtnodep) {
    UASSERT_OBJ(dtnodep, parentp,
                "Caller should check for nullptr before computing dtype");
    ...
    if (child1 || child2) {
      ...
      UASSERT_OBJ(dtnodep, parentp,
                  "iterateEditMoveDTypep lost pointer to child");
  ...
  AstConst *dimensionValue(FileLine *fileline, AstNodeDType *nodep,
                           AstAttrType attrType, int dim) {
    // Return the dimension value for the specified attribute and constant
    // dimension
    ...
    for (int i = 1; i <= dim; ++i) {
      ...
      if (AstNodeArrayDType *adtypep = VN_CAST(dtypep, NodeArrayDType)) {
        ...
        continue;
      } else if (AstNodeUOrStructDType *adtypep =
    ...
    if (!valp)
      valp = new AstConst(fileline, AstConst::Signed32(), val);
    UINFO(9, " $dimension " << attrType.ascii() << "(" << cvtToHex(dtypep)
                            << "," << dim << ")=" << valp << endl);
    return valp;
  }
  AstVar *dimensionVarp(AstNodeDType *nodep, AstAttrType attrType,
                        uint32_t msbdim) {
    // Return a variable table which has specified dimension properties for this
    // variable
    ...
    AstInitArray *initp =
        new AstInitArray(nodep->fileline(), vardtypep, nullptr);
    ...
    varp->isStatic(true);
    varp->valuep(initp);
    // Add to root, as don't know module we are in, and aids later structure
    // sharing
    ...
    userIterate(varp,
                nullptr); // May have already done $unit so must do this var
  ...
  AstVar *enumVarp(AstEnumDType *nodep, AstAttrType attrType, uint32_t msbdim) {
      ...
      values[i] = nullptr;
    {
      ...
      while (prevp->nextp())
        prevp = VN_AS(prevp->nextp(), EnumItem);
    ...
    for (unsigned i = 0; i < (msbdim + 1); ++i) {
      if (values[i])
        initp->addIndexValuep(i, values[i]);
    }
    userIterate(varp,
                nullptr); // May have already done $unit so must do this var
    m_tableMap.emplace(std::make_pair(nodep, attrType), varp);
  ...
  PatVecMap patVectorMap(AstPattern *nodep, const VNumRange &range) {
    ...
    for (AstPatMember *patp = VN_AS(nodep->itemsp(), PatMember); patp;
      ...
      if (patmap.find(element) != patmap.end()) {
        patp->v3error(
            "Assignment pattern key used multiple times: " << element);
      } else {
  ...
  void makeOpenArrayShell(AstNodeFTaskRef *nodep) {
    ...
    newTaskp->name(newTaskp->name() + "__Vdpioc" +
                   cvtToStr(oldTaskp->dpiOpenParent()));
    ...
    for (AstNode *stmtp = nodep->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
      if (AstVar *portp = VN_CAST(stmtp, Var)) {
        if (portp->isDpiOpenArray() ||
            hasOpenArrayIterateDType(portp->dtypep())) {
  ...
  bool hasOpenArrayIterateDType(AstNodeDType *nodep) {
    ...
    if (VN_IS(nodep, UnsizedArrayDType))
      return true;
    if (nodep->subDTypep())
      return hasOpenArrayIterateDType(nodep->subDTypep()->skipRefp());
    return false;
  }
  ...
  void assertAtStatement(AstNode *nodep) {
    if (VL_UNCOVERABLE(m_vup && !m_vup->selfDtm())) {
      UINFO(1, "-: " << m_vup << endl);
      nodep->v3fatalSrc("No dtype expected at statement "
                        << nodep->prettyTypeName());
    }
  }
  void checkConstantOrReplace(AstNode *nodep, const string &message) {
    // See also V3WidthSel::checkConstantOrReplace
    // Note can't call V3Const::constifyParam(nodep) here, as constify may
    // change nodep on us!
  ...
  AstRefDType *checkRefToTypedefRecurse(AstNode *nodep, AstTypedef *typedefp) {
    ...
    if (auto *refp = VN_CAST(nodep, RefDType)) {
      if (refp->typedefp() == typedefp)
        return refp;
    }
    if (auto *refp = checkRefToTypedefRecurse(nodep->op1p(), typedefp))
      return refp;
    if (auto *refp = checkRefToTypedefRecurse(nodep->op2p(), typedefp))
      return refp;
    if (auto *refp = checkRefToTypedefRecurse(nodep->op3p(), typedefp))
      return refp;
    if (auto *refp = checkRefToTypedefRecurse(nodep->op4p(), typedefp))
      return refp;
    return nullptr;
  }
  ...
  // These functions save/restore the AstNUser information so it can pass to
  // child nodes.
  ...
  void userIterateAndNext(AstNode *nodep, WidthVP *vup) {
    if (!nodep)
      return;
    if (nodep->didWidth())
      return; // Avoid iterating list we have already iterated
    {
      VL_RESTORER(m_vup);
      m_vup = vup;
...
public:
  // CONSTRUCTORS
  WidthVisitor(
      bool paramsOnly, // [in] TRUE if we are considering parameters only.
      bool doGenerate) // [in] TRUE if we are inside a generate statement and
      //                           // don't wish to trigger errors
      : m_paramsOnly{paramsOnly}, m_doGenerate{doGenerate} {}
...
void V3Width::width(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("width", 0,
...
void V3Width::widthCommit(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("widthcommit", 0,
