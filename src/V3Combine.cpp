...
private:
  // NODE STATE
  std::multimap<AstCFunc *, AstCCall *>
      m_callMmap; // Associative array of {function}{call}
                  // METHODS
public:
  void replaceFunc(AstCFunc *oldfuncp, AstCFunc *newfuncp) {
    if (oldfuncp == newfuncp)
      return;
    if (newfuncp) {
    ...
    for (auto it = m_callMmap.find(oldfuncp); it != m_callMmap.end();
      ...
      if (newfuncp) {
        ...
        addCall(
            newp); // Fix the table, in case the newfuncp itself gets replaced
...
private:
  ...
  virtual void visit(AstCCall *nodep) override {
    if (nodep->funcp()->dontCombine())
      return;
    addCall(nodep);
  }
  // LCOV_EXCL_START
  virtual void visit(AstAddrOfCFunc *nodep) override {
    // We cannot yet handle references via AstAddrOfCFunc, but currently those
    // are only used in tracing functions, which are not combined. Blow up in
    // case this changes.
    if (nodep->funcp()->dontCombine())
      return;
    nodep->v3fatalSrc("Don't know how to combine functions that are referenced "
                      "via AstAddrOfCFunc");
  }
  // LCOV_EXCL_END
  // Speed things up
...
private:
  ...
  void walkEmptyFuncs() {
    for (const auto &itr : m_dupFinder) {
      ...
      UINFO(5,
            "     Drop empty CFunc " << itr.first << " " << oldfuncp << endl);
      ...
      for (auto newIt = m_dupFinder.begin(); newIt != m_dupFinder.end();
        ...
        for (; oldIt != m_dupFinder.end(); ++oldIt) {
          ...
          if (newIt->first != oldIt->first)
            break; // Iterate over same hashes only
          if (oldfuncp->user3())
            continue; // Already replaced
          if (!newfuncp->sameTree(oldfuncp))
            continue; // Different functions

          // Replace calls to oldfuncp with calls to newfuncp
          UINFO(5, "     Replace CFunc " << newIt->first << " " << newfuncp
                                         << endl);
          UINFO(5, "              with " << oldIt->first << " " << oldfuncp
                                         << endl);
          ++m_cfuncsCombined;
          oldfuncp->user3SetOnce(); // Mark replaced
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    iterateChildren(nodep);
    if (debug() >= 9)
      m_dupFinder.dumpFilePrefixed("combine");
...
void V3Combine::combineAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("combine", 0,
