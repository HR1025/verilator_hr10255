...
private:
  ...
  AstCFunc *m_initSubFuncp =
      nullptr;                      // Trace function being built (under m_init)
  ...
  AstCFunc *newCFunc(const string &name) {
    ...
    if (m_interface)
      argTypes += ", int scopet, const char* scopep";
  ...
  AstCFunc *newCFuncSub(AstCFunc *basep) {
    ...
    funcp->addInitsp(
        new AstCStmt(fl, "const int c = vlSymsp->__Vm_baseCode;\n"));
    funcp->addInitsp(
        new AstCStmt(fl, "if (false && tracep && c) {}  // Prevent unused\n"));
    if (!m_interface)
      callCFuncSub(basep, funcp, nullptr);
    return funcp;
  }
  ...
  void addTraceDecl(const VNumRange &arrayRange,
                    int widthOverride) { // If !=0, is packed struct/array where
                                         // basicp size misreflects one element
    VNumRange bitRange;
    if (widthOverride) {
      bitRange = VNumRange{widthOverride - 1, 0};
    } else if (const AstBasicDType *const bdtypep =
                   m_traValuep->dtypep()->basicp()) {
      bitRange = bdtypep->nrange();
    }
    AstTraceDecl *const declp = new AstTraceDecl(
        m_traVscp->fileline(), m_traShowname, m_traVscp->varp(),
    ...
    if (cellp && VN_IS(cellp->modp(), Iface)) {
      ...
      int origSubStmts = m_initSubStmts;
      {
        ...
        while (nextIrp) {
          ...
          if (scopeLen > irpName.length())
            continue;
          const string intfScopeName = irpName.substr(0, scopeLen);
          if (scopeName != intfScopeName)
            continue;
  ...
  virtual void visit(AstVarScope *nodep) override {
    ...
    if ((!nodep->varp()->isTemp() || nodep->varp()->isTrace()) &&
      ...
      if (m_interface) {
        ...
        if (m_traShowname.substr(0, 4) == "TOP ")
          m_traShowname.erase(0, 4);
      }
      ...
      if (const char *const ignoreReasonp = vscIgnoreTrace(nodep)) {
        ...
        // Recurse into data type of the signal; the visitors will call
        // addTraceDecl()
  ...
  virtual void visit(AstUnpackArrayDType *nodep) override {
    ...
    if (m_traVscp) {
      if (static_cast<int>(nodep->arrayUnpackedElements()) >
          v3Global.opt.traceMaxArray()) {
        ...
        // Simple 1-D array, use existing V3EmitC runtime loop rather than
        // unrolling This will put "(index)" at end of signal name for us
        ...
        for (int i = nodep->lo(); i <= nodep->hi(); ++i) {
          ...
          VL_RESTORER(m_traValuep);
          {
            ...
            m_traValuep =
                new AstArraySel(nodep->fileline(), m_traValuep->cloneTree(true),
                                i - nodep->lo());
    ...
    if (m_traVscp) {
      if (!v3Global.opt.traceStructs()) {
        ...
        for (int i = nodep->lo(); i <= nodep->hi(); ++i) {
          ...
          {
            m_traShowname += string("[") + cvtToStr(i) + string("]");
            m_traValuep = new AstSel(
                nodep->fileline(), m_traValuep->cloneTree(true),
        ...
        for (const AstMemberDType *itemp = nodep->membersp(); itemp;
          ...
          VL_RESTORER(m_traValuep);
          {
            if (VN_IS(nodep, StructDType)) {
              ...
              m_traShowname += getScopeChar(VLT_TRACE_SCOPE_UNION) + " " +
                               itemp->prettyName();
              iterate(subtypep);
            }
          }
...
void V3TraceDecl::traceDeclAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("tracedecl", 0,
