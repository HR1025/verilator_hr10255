...
class VSymEnt final {
  // Symbol table that can have a "superior" table for resolving upper
  // references MEMBERS
  ...
  VSymEnt *m_fallbackp;  // Table "above" this one in name scope, for fallback
                         // resolution
  VSymEnt *m_parentp; // Table that created this table, dot notation needed to
                      // resolve into it
  AstNodeModule
      *m_classOrPackagep; // Package node is in (for V3LinkDot, unused here)
  string
      m_symPrefix; // String to prefix symbols with (for V3LinkDot, unused here)
  ...
  static int debug() {
    ...
    if (VL_UNLIKELY(level < 0))
      level = v3Global.opt.debugSrcLevel("V3LinkDot.cpp");
    return level;
  }
#else
  static constexpr int debug() {
    return 0;
  } // NOT runtime, too hot of a function
#endif
public:
  ...
  void dumpIterate(std::ostream &os, VSymConstMap &doneSymsr,
                   const string &indent, int numLevels,
                   const string &searchName) const {
    os << indent << "+ " << std::left << std::setw(30)
       << (searchName == "" ? "\"\"" : searchName) << std::setw(0)
       << std::right;
    os << "  se" << cvtToHex(this) << std::setw(0);
    os << "  fallb=se" << cvtToHex(m_fallbackp);
    if (m_symPrefix != "")
      os << "  symPrefix=" << m_symPrefix;
    os << "  n=" << nodep();
    os << '\n';
  ...
  void insert(const string &name, VSymEnt *entp) {
    UINFO(9, "     SymInsert se" << cvtToHex(this) << " '" << name << "' se"
    ...
    if (name != "" && m_idNameMap.find(name) != m_idNameMap.end()) {
      if (!V3Error::errorCount()) { // Else may have just reported warning
        if (debug() >= 9 || V3Error::debugDefault())
          dump(cout, "- err-dump: ", 1);
        entp->nodep()->v3fatalSrc(
            "Inserting two symbols with same name: " << name);
      }
    } else {
  ...
  VSymEnt *findIdFallback(const string &name) const {
    ...
    // First, scan this begin/end block or module for the name
    if (VSymEnt *entp = findIdFlat(name))
      return entp;
    // Then scan the upper begin/end block or module for the name
    if (m_fallbackp)
      return m_fallbackp->findIdFallback(name);
    return nullptr;
  }
  void candidateIdFlat(VSpellCheck *spellerp,
                       const VNodeMatcher *matcherp) const {
    // Suggest alternative symbol candidates without looking upward through
    // symbol hierarchy
    for (IdNameMap::const_iterator it = m_idNameMap.begin();
         it != m_idNameMap.end(); ++it) {
  ...
  void candidateIdFallback(VSpellCheck *spellerp,
                           const VNodeMatcher *matcherp) const {
    // Suggest alternative symbol candidates with looking upward through symbol
    // hierarchy Note VSpellCheck wants the most important (closest) items
    // pushed first
    candidateIdFlat(spellerp, matcherp);
    // Then suggest the upper begin/end block or module
    if (m_fallbackp)
      m_fallbackp->candidateIdFallback(spellerp, matcherp);
  }

private:
  void importOneSymbol(VSymGraph *graphp, const string &name,
                       const VSymEnt *srcp, bool honorExport) {
  ...
  void exportOneSymbol(VSymGraph *graphp, const string &name,
                       const VSymEnt *srcp) const {
    if (srcp->exported()) {
      if (VSymEnt *symp =
...
public:
  ...
  void importFromIface(VSymGraph *graphp, const VSymEnt *srcp,
    ...
    // Import interface tokens from source symbol table into this symbol table,
    // recursively
    UINFO(9, "     importIf  se" << cvtToHex(this) << " from se"
                                 << cvtToHex(srcp) << endl);
    for (IdNameMap::const_iterator it = srcp->m_idNameMap.begin();
         it != srcp->m_idNameMap.end(); ++it) {
  ...
  void cellErrorScopes(AstNode *lookp, string prettyName = "") {
    if (prettyName == "")
      prettyName = lookp->prettyName();
    string scopes;
    for (IdNameMap::iterator it = m_idNameMap.begin(); it != m_idNameMap.end();
         ++it) {
...
public:
  ...
  ~VSymGraph() {
    for (const VSymEnt *entp : m_symsp)
      delete entp;
  }
...
inline VSymEnt::VSymEnt(VSymGraph *graphp, AstNode *nodep) : m_nodep(nodep) {
  // No argument to set fallbackp, as generally it's wrong to set it in the new
  // call, Instead it needs to be set on a "findOrNew()" return, as it may have
  // been new'ed by an earlier search insertion.
  m_fallbackp = nullptr;
  m_parentp = nullptr;
