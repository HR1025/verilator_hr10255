...
public:
  ...
  virtual void runTest() override {
    ...
    UASSERT(i->color() != a->color() && a->color() != g2->color() &&
                g2->color() != q->color(),
...
public:
  ...
  virtual void runTest() override {
    ...
    //    underSBlock: VarRefs on RHS:  consume 'pre' (required to save cutable
    //    tests)
    n = new V3GraphTestVertex(gp, "a_dly<PRE=a");
    {
      new V3GraphEdge(gp, n, a_dlyblk, 2); // Block ordering
      new V3GraphEdge(gp, n, a_dly, 2);
      new V3GraphEdge(gp, ap, n, 2,
                      true); // DESIRED delayed ordering (inp is required)
    ...
    // COMBO
    // Inbound edges are always uncutable, because we must put combo logic after
    // sequential Outbound are cutable, as we may need to evaluate multiple
    // times
