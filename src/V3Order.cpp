...
//                      This ensures the AssignPre gets scheduled before this
//                      logic
...
//      and make a new block.  Add that new activation block to the list of
//      calls to make.
//
//*************************************************************************
...
void OrderGraph::loopsVertexCb(V3GraphVertex *vertexp) {
  ...
  if (OrderVarVertex *vvertexp = dynamic_cast<OrderVarVertex *>(vertexp)) {
    std::cerr << vvertexp->varScp()->fileline()->warnOther()
              << "     Example path: " << vvertexp->varScp()->prettyName()
              << endl;
  }
}
...
class OrderClkMarkVisitor final : public AstNVisitor {
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    if (m_hasClk) {
      ...
      if (lhsp &&
          (lhsp->varp()->attrClocker() == VVarAttrClocker::CLOCKER_UNKNOWN)) {
        lhsp->varp()->attrClocker(
            VVarAttrClocker::CLOCKER_YES); // mark as clocker
        m_newClkMarked =
            true; // enable a further run since new clocker is marked
        UINFO(5,
              "node is newly marked as clocker by assignment " << lhsp << endl);
      }
    }
  }
  virtual void visit(AstVarRef *nodep) override {
    if (m_inAss &&
        nodep->varp()->attrClocker() == VVarAttrClocker::CLOCKER_YES) {
      if (m_inClocked) {
        nodep->v3warn(
            CLKDATA,
  ...
  virtual void visit(AstNodeSel *nodep) override {
    if (m_inAss) {
      ...
      if (m_childClkWidth > nodep->width())
        m_childClkWidth = nodep->width();
    }
  }
  ...
  virtual void visit(AstReplicate *nodep) override {
    if (m_inAss) {
      ...
      if (VN_IS(nodep->rhsp(), Const)) {
        m_childClkWidth =
            m_childClkWidth * VN_AS(nodep->rhsp(), Const)->toUInt();
      } else {
...
private:
  ...
  std::array<OrderVarVertex *, static_cast<size_t>(VarVertexType::POST) + 1>
      m_vertexps;

public:
  // METHODS
  OrderVarVertex *getVarVertex(V3Graph *graphp, AstScope *scopep,
                               AstVarScope *varscp, VarVertexType type) {
...
class OrderBuildVisitor final : public AstNVisitor {
  ...
  OrderInputsVertex *const m_inputsVxp =
      new OrderInputsVertex{m_graphp, nullptr};
  // Singleton DPI Export trigger event vertex
  OrderVarVertex *const m_dpiExportTriggerVxp =
      v3Global.rootp()->dpiExportTriggerp()
          ? getVarVertex(v3Global.rootp()->dpiExportTriggerp(),
                         VarVertexType::STD)
          : nullptr;

  OrderLogicVertex *m_activeSenVxp =
      nullptr; // Sensitivity vertex for clocked logic
  ...
  void iterateLogic(AstNode *nodep) {
    ...
    // If this logic has a clocked activation, add a link from the sensitivity
    // list LogicVertex to this LogicVertex.
    if (m_activeSenVxp)
      new OrderEdge(m_graphp, m_activeSenVxp, m_logicVxp, WEIGHT_NORMAL);
  ...
  virtual void visit(AstSenTree *nodep) override {
    // This should only find the global AstSenTrees under the AstTopScope, which
    // we ignore here. We visit AstSenTrees separately when encountering the
    // AstActive that references them.
  ...
  virtual void visit(AstActive *nodep) override {
    ...
    UASSERT_OBJ(!m_inClocked && !m_activeSenVxp && !m_domainp, nodep,
                "Should not nest");

    m_inClocked = nodep->sensesp()->hasClocked();

    // Analyze variable references in sensitivity list. Note that non-clocked
    // sensitivity lists don't reference any variables (have no clocks), so the
    // sensitivity list vertex would have no incoming dependencies and is hence
    // redundant, therefore we only do this for clocked sensitivity lists.
    if (m_inClocked) {
      // Add LogicVertex for the sensitivity list of this AstActive.
      m_activeSenVxp =
          new OrderLogicVertex(m_graphp, m_scopep, nodep->sensesp(), nodep);
      // Analyze variables in the sensitivity list
      iterateChildren(nodep->sensesp());
    }

    // Ignore the sensitivity domain for combinational logic. We will assign
    // combinational logic to a domain later, based on the domains of incoming
    // variables.
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    // As we explicitly not visit (see ignored nodes below) any subtree that is
    // not relevant for ordering, we should be able to assert this:
    ...
    if (!m_logicVxp) {
      ...
      // Check whether this variable was already generated/consumed in the same
      // logic. We don't want to add extra edges if the logic has many usages of
      // the same variable, so only proceed on first encounter.
      ...
      if (!prevGen && nodep->access().isWriteOrRW()) {
        gen = true;
        if (m_inPostponed) {
          // IEE 1800-2017 (4.2.9) forbids any value updates in the postponed
          // region, but Verilator generated trigger signals for $strobe are
          // cleared after the display is executed. This is both safe to ignore
          // (because their single read is in the same AstAlwaysPostponed, just
          // prior to the clear), and is necessary to ignore to avoid a circular
          // logic (UNOPTFLAT) warning.
          UASSERT_OBJ(prevCon, nodep,
                      "Should have been consumed in same process");
          gen = false;
        }
      }
      ...
      if (!prevCon && nodep->access().isReadOrRW()) {
        ...
        if (prevGen && !m_inClocked) {
          ...
          // If a variable is consumed in the same combinational process that
          // produced it earlier, consider it something like:
          //      foo = 1
          //      foo = foo + 1
          // and still optimize. Note this will break though:
          //      if (sometimes) foo = 1
          //      foo = foo + 1
          // TODO: Do this properly with liveness analysis (i.e.: if live, it's
          // consumed)
          //       Note however that this construct is not nicely synthesizable
          //       (yields latch?).
          con = false;
        }

        // TODO: Explain how the following two exclusions are useful
        if (varscp->varp()->attrClockEn() && !m_inPre && !m_inPost &&
            !m_inClocked) {
          // 'clock_enable' attribute on this signal: user's worrying about it
          // for us
          con = false;
        }
        if (m_inClkAss &&
          ...
          con = false;
          UINFO(4, "nodep used as clock_enable "
                       << varscp << " in " << m_logicVxp->nodep() << endl);
        }
      }
      ...
      if (gen) {
        ...
        if (!m_inClocked || m_inPost) {
          // Combinational logic
          OrderVarVertex *const varVxp =
              getVarVertex(varscp, VarVertexType::STD);
          ...
          } else if (varscp->varp()->attrClocker() ==
          ...
          OrderVarVertex *const postVxp =
              getVarVertex(varscp, VarVertexType::POST);
          ...
          OrderVarVertex *const ordVxp =
              getVarVertex(varscp, VarVertexType::PORD);
      ...
      if (con) {
        ...
        if (!m_inClocked || m_inPost) {
          ...
          // This one is cutable (vs the producer) as there's only one such
          // consumer, but may be many producers
          OrderVarVertex *const preVxp =
              getVarVertex(varscp, VarVertexType::PRE);
  ...
  virtual void visit(AstDpiExportUpdated *nodep) override {
    // This is under a logic block (AstAlways) sensitive to a change in the DPI
    // export trigger. We just need to add an edge to the enclosing logic vertex
    // (the vertex for the AstAlways).
    OrderVarVertex *const varVxp =
        getVarVertex(nodep->varScopep(), VarVertexType::STD);
    new OrderComboCutEdge(m_graphp, m_logicVxp, varVxp);
  ...
  } // Only interested in the respective AstScope
  ...
  virtual void visit(AstCFunc *) override {
    // Calls to DPI exports handled with AstCCall. /* verilator public */
    // functions are ignored for now (and hence potentially mis-ordered), but
    // could use the same or similar mechanism as DPI exports. Every other
    // impure function (including those that may set a non-local variable) must
    // have been inlined in V3Task.
  }
...
public:
  // Process the netlist and return the constructed ordering graph. It's
  // 'process' because this visitor does change the tree (removes some nodes
  // related to DPI export trigger).
...
public:
  V3ListEnt<OrderMoveDomScope *>
      m_readyDomScopeE; // List of next ready dom scope
  V3List<OrderMoveVertex *>
      m_readyVertices; // Ready vertices with same domain & scope
private:
  bool m_onReadyList =
      false; // True if DomScope is already on list of ready dom/scopes
...
public:
  OrderMoveDomScope(const AstSenTree *domainp, const AstScope *scopep)
      : m_domainp{domainp}, m_scopep{scopep} {}
  OrderMoveDomScope *readyDomScopeNextp() const {
    return m_readyDomScopeE.nextp();
  }
  ...
  string name() const {
    return (string("MDS:") + " d=" + cvtToHex(domainp()) +
            " s=" + cvtToHex(scopep()));
  }
};
...
template <class T_MoveVertex> class ProcessMoveBuildGraph {
  ...
  using Logic2Move =
      std::unordered_map<const OrderLogicVertex *, T_MoveVertex *>;
...
public:
  ...
  public:
    ...
    virtual T_MoveVertex *makeVertexp( //
        ...
        const AstScope *scopep, const AstSenTree *domainp) = 0;
...
public:
  ...
  ProcessMoveBuildGraph(
      ...
      : m_graphp{logicGraphp}, m_outGraphp{outGraphp}, m_vxMakerp{vxMakerp} {}
...
private:
  ...
  bool iterate(T_MoveVertex *moveVxp, const V3GraphVertex *origVxp,
    ...
    // moveVxp forward
    for (V3GraphEdge *edgep = origVxp->outBeginp(); edgep;
      ...
      if (const OrderLogicVertex *toLVertexp =
        ...
        if (domainsExclusive(domainp, toLVertexp->domainp()))
          continue;
...
public:
  ...
  OrderMoveVertexMaker(V3Graph *pomGraphp,
      ...
      : m_pomGraphp{pomGraphp}, m_pomWaitingp{pomWaitingp} {}
  // METHODS
  virtual OrderMoveVertex *makeVertexp(OrderLogicVertex *lvertexp,
...
public:
  ...
  virtual MTaskMoveVertex *makeVertexp(OrderLogicVertex *lvertexp,
    ...
    if (domainp->hasInitial() || domainp->hasSettle())
      return nullptr;
    return new MTaskMoveVertex(m_pomGraphp, lvertexp, varVertexp, scopep,
                               domainp);
  }
...
class OrderProcess final : AstNDeleter {
  // NODE STATE
  //  AstNodeModule::user3  -> int: Number of AstCFuncs created under this
  //  module AstNode::user4        -> Used by V3Const::constifyExpensiveEdit
  ...
  AstSenTree
      *const m_deleteDomainp; // Dummy AstSenTree indicating needs deletion
  ...
  V3List<OrderMoveVertex *>
      m_pomWaiting; // List of nodes needing inputs to become ready
  friend class OrderMoveDomScope;
  V3List<OrderMoveDomScope *>
      m_pomReadyDomScope; // List of ready domain/scope pairs, by loopId
  std::vector<OrderVarStdVertex *>
      m_unoptflatVars; // Vector of variables in UNOPTFLAT loop

  // STATS
  std::array<VDouble0, OrderVEdgeType::_ENUM_END>
      m_statCut; // Count of each edge type cut
  ...
  void nodeMarkCircular(OrderVarVertex *vertexp, OrderEdge *edgep) {
    // To be marked circular requires being a clock assigned in a delayed
    // assignment, or having a cutable in or out edge, none of which is true for
    ...
    if ((fromLVtxp && VN_IS(fromLVtxp->nodep(), Initial)) ||
      ...
      if (vertexp->isClock()) {
        ...
        // First v3warn not inside warnIsOff so we can see the suppressions with
        // --debug
        nodep->v3warn(
            UNOPTFLAT,
  ...
  void reportLoopVars(OrderVarVertex *vertexp) {
    ...
    std::stable_sort(
        m_unoptflatVars.begin(), m_unoptflatVars.end(),
    ...
    if (!canSplitList.empty()) {
      std::cerr << V3Error::warnMore()
                << "... Suggest add /*verilator split_var*/ to appropriate "
                   "variables above."
                << std::endl;
    }
  ...
  void reportLoopVarsIterate(V3GraphVertex *vertexp, uint32_t color) {
    if (vertexp->user())
      return; // Already done
    vertexp->user(1);
    if (OrderVarStdVertex *vsvertexp =
            dynamic_cast<OrderVarStdVertex *>(vertexp)) {
  ...
  static OrderInputsVertex &findInputVertex(OrderGraph &graph) {
    for (V3GraphVertex *vtxp = graph.verticesBeginp(); vtxp;
         vtxp = vtxp->verticesNextp()) {
      if (auto *ivtxp = dynamic_cast<OrderInputsVertex *>(vtxp))
        return *ivtxp;
    }
    VL_UNREACHABLE
  }
  ...
  static AstSenTree *makeDeleteDomainSenTree(FileLine *fl) {
    // TODO: Using "Never" instead of "Settle" causes a test failure, it
    // probably shouldn't ...
  ...
  OrderProcess(AstNetlist *netlistp, OrderGraph &graph)
      : m_graph{graph}, m_inputsVtx{findInputVertex(graph)}, m_finder{netlistp},
  ...
  ~OrderProcess() {
    ...
    for (int type = 0; type < OrderVEdgeType::_ENUM_END; type++) {
      ...
      if (count != 0.0) {
        V3Stats::addStat(string("Order, cut, ") + OrderVEdgeType(type).ascii(),
                         count);
      }
    }
  }
...
// Mark one vertex as finished, remove from ready list if done
inline void OrderMoveDomScope::movedVertex(OrderProcess *opp,
                                           OrderMoveVertex *vertexp) {
...
void OrderProcess::processInputs() {
  m_graph.userClearVertices(); // Vertex::user()   // 1 if input recursed, 2 if
                               // marked as input, 3 if out-edges recursed
...
void OrderProcess::processInputsInIterate(OrderEitherVertex *vertexp,
                                          VertexVec &todoVec) {
  ...
  if (false && debug() >= 9) {
    ...
    if (OrderLogicVertex *vvertexp =
            dynamic_cast<OrderLogicVertex *>(vertexp)) {
      vvertexp->nodep()->dumpTree(cout, "-            TT: ");
    }
  }
  vertexp->user(1); // Processing
  // First handle all inputs to this vertex, in most cases they'll be already
  // processed earlier Also, determine if this vertex is an input
  int inonly = 1; // 0=no, 1=maybe, 2=yes until a no
  for (V3GraphEdge *edgep = vertexp->inBeginp(); edgep;
       edgep = edgep->inNextp()) {
    OrderEitherVertex *frVertexp =
        static_cast<OrderEitherVertex *>(edgep->fromp());
  ...
  if (inonly == 2 &&
      vertexp->user() <
          2) { // Set it.  Note may have already been set earlier, too
...
void OrderProcess::processInputsOutIterate(OrderEitherVertex *vertexp,
                                           VertexVec &todoVec) {
  if (vertexp->user() == 3)
    return; // Already out processed
  ...
  {
    // Propagate PrimaryIn through simple assignments, following target of
    // vertex
    for (V3GraphEdge *edgep = vertexp->outBeginp(); edgep;
         edgep = edgep->outNextp()) {
      OrderEitherVertex *toVertexp =
          static_cast<OrderEitherVertex *>(edgep->top());
      if (OrderVarStdVertex *vvertexp =
              dynamic_cast<OrderVarStdVertex *>(toVertexp)) {
        processInputsInIterate(vvertexp, todoVec);
      }
      if (OrderLogicVertex *vvertexp =
              dynamic_cast<OrderLogicVertex *>(toVertexp)) {
...
void OrderProcess::processCircular() {
  ...
  for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
       itp = itp->verticesNextp()) {
    if (OrderVarStdVertex *vvertexp = dynamic_cast<OrderVarStdVertex *>(itp)) {
      ...
      for (V3GraphEdge *edgep = vvertexp->outBeginp(); edgep;
        ...
        if (edgep->weight() == 0) { // was cut
          ...
          UASSERT_OBJ(oedgep, vvertexp->varScp(),
                      "Cutable edge not of proper type");
...
void OrderProcess::processDomainsIterate(OrderEitherVertex *vertexp) {
  ...
  // best->worst order Assign clock domains to each signal.
  //     Sequential logic is forced into the same sequential domain.
  //     Combo logic may be pushed into a seq domain if all its inputs are the
  //     same domain, else, if all inputs are from flops, it's end-of-sequential
  //     code else, it's full combo code
  if (vertexp->domainp())
    return; // Already processed, or sequential logic
  ...
  if (!domainp) {
    for (V3GraphEdge *edgep = vertexp->inBeginp(); edgep;
      ...
      if (edgep->weight() && fromVertexp->domainMatters()) {
        ...
        if (!domainp // First input to this vertex
          ...
          AstSenItem *newtree2p =
              fromVertexp->domainp()->sensesp()->cloneTree(true);
          ...
          newtreep->multi(
              true); // Comment that it was made from 2 clock domains
    ...
    // Default the domain
    // This is a node which has only constant inputs, or is otherwise
    // indeterminate. It should have already been copied into the settle domain.
    // Presumably it has inputs which we never trigger, or nothing it's
    // sensitive to, so we can rip it out.
  ...
  if (vertexp->domainp()) {
    UINFO(5,
          "      done d=" << cvtToHex(vertexp->domainp())
...
void OrderProcess::processEdgeReport() {
  ...
  for (V3GraphVertex *itp = m_graph.verticesBeginp(); itp;
    ...
    if (OrderVarVertex *vvertexp = dynamic_cast<OrderVarVertex *>(itp)) {
      ...
      if (sentreep)
        V3EmitV::verilogForTree(sentreep, os);
...
void OrderProcess::processMove() {
  ...
  //   Move any node with all inputs ready to a "ready" graph mapped by domain
  //   and then scope While waiting graph ! empty  (and also known: something in
  //   ready graph)
  ...
  //           Any children that have all inputs now ready move from
  //           waiting->ready graph (This may add nodes the for loop directly
  //           above needs to detext)
  ...
  while (!m_pomReadyDomScope.empty()) {
    ...
    OrderMoveVertex *topVertexp =
        domScopep->readyVertices().begin(); // lintok-begin-on-ref
    UASSERT(topVertexp,
            "domScope on ready list without any nodes ready under it");
    ...
    while (domScopep) {
      ...
      // Done with scope/domain pair, pick new scope under same domain, or
      // nullptr if none left
  ...
  UASSERT(m_pomWaiting.empty(), "Didn't converge; nodes waiting, none ready, "
                                "perhaps some input activations lost.");
  // Cleanup memory
  processMoveClear();
}
...
void OrderProcess::processMoveReadyOne(OrderMoveVertex *vertexp) {
  ...
  if (vertexp->logicp()) {
    ...
    vertexp->m_readyVerticesE.pushBack(vertexp->domScopep()->m_readyVertices,
                                       vertexp);
...
void OrderProcess::processMoveDoneOne(OrderMoveVertex *vertexp) {
  ...
  if (vertexp->logicp()) {
    vertexp->m_readyVerticesE.unlink(vertexp->domScopep()->m_readyVertices,
                                     vertexp);
  ...
  for (V3GraphEdge *edgep = vertexp->outBeginp(), *nextp; edgep;
       edgep = nextp) {
    nextp = edgep->outNextp();
    OrderMoveVertex *toVertexp = static_cast<OrderMoveVertex *>(edgep->top());
    UINFO(9, "          Clear to "
                 << (toVertexp->inEmpty() ? "[EMP] " : "      ") << toVertexp
                 << endl);
    // Delete this edge
    VL_DO_DANGLING(edgep->unlinkDelete(), edgep);
    if (toVertexp->inEmpty()) {
      // If destination node now has all inputs resolved; recurse to move that
      // vertex This is thus depth first (before width) which keeps the
...
void OrderProcess::processMoveOne(OrderMoveVertex *vertexp,
                                  OrderMoveDomScope *domScopep, int level) {
  UASSERT_OBJ(vertexp->domScopep() == domScopep, vertexp,
              "Domain mismatch; list misbuilt?");
  const OrderLogicVertex *lvertexp = vertexp->logicp();
  const AstScope *scopep = lvertexp->scopep();
  UINFO(5, "    POSmove l" << std::setw(3) << level
                           << " d=" << cvtToHex(lvertexp->domainp()) << " s="
                           << cvtToHex(scopep) << " " << lvertexp << endl);
  AstActive *newActivep = processMoveOneLogic(lvertexp, m_pomNewFuncp /*ref*/,
                                              m_pomNewStmts /*ref*/);
  if (newActivep)
    m_scopetop.addActivep(newActivep);
  processMoveDoneOne(vertexp);
}
...
AstActive *OrderProcess::processMoveOneLogic(const OrderLogicVertex *lvertexp,
  ...
  if (VN_IS(nodep, SenTree)) {
    // Just ignore sensitivities, we'll deal with them when we move statements
    // that need them
  } else { // Normal logic
    // Move the logic into a CFunc
    nodep->unlinkFrBack();

    // Process procedures per statement (unless profCFuncs), so we can split
    // CFuncs within procedures. Everything else is handled in one go
    ...
    while (nodep) {
      ...
      if (v3Global.opt.profCFuncs() ||
        ...
        // Put every statement into a unique function to ease profiling or
      ...
      if (!newFuncpr && domainp != m_deleteDomainp) {
        ...
        AstActive *const newActivep =
            new AstActive(nodep->fileline(), name, domainp);
...
void OrderProcess::processMTasksInitial(InitialLogicE logic_type) {
  ...
  for (V3GraphVertex *initVxp = m_graph.verticesBeginp(); initVxp;
    ...
    if (!initp)
      continue;
    if ((logic_type == LOGIC_INITIAL) && !initp->domainp()->hasInitial())
      continue;
    if ((logic_type == LOGIC_SETTLE) && !initp->domainp()->hasSettle())
      continue;
...
void OrderProcess::processMTasks() {
  ...
  ProcessMoveBuildGraph<MTaskMoveVertex> mtask_pmbg(&m_graph, &logicGraph,
                                                    &create_mtask_vertex);
  mtask_pmbg.build();
  ...
  while ((moveVxp = emit_logic.nextp())) {
    ...
    if (movep->logicp()) {
      ...
      for (const V3GraphEdge *edgep = logicp->inBeginp(); edgep;
           edgep = edgep->inNextp()) {
  ...
  while ((mtaskVxp = emit_mtasks.nextp())) {
    ...
    state.m_execMTaskp =
        new ExecMTask(execGraphp->mutableDepGraphp(), bodyp, mtaskp->id());
...
void OrderProcess::process() {
  ...
  if (debug() && v3Global.opt.dumpTree())
    processEdgeReport();
  ...
  if (!v3Global.opt.mtasks()) {
    ...
    if (debug() >= 4) {
      m_pomGraph.dumpDotFilePrefixed(
          "ordermv_start"); // Different prefix (ordermv) as it's not the same
                            // graph
    }
  ...
  // Any SC inputs feeding a combo domain must be marked, so we can make them
  // sc_sensitive
  UINFO(2, "  Sensitive...\n");
...
void V3Order::orderAll(AstNetlist *netlistp) {
  ...
  // Dump tree
  V3Global::dumpCheckGlobalTree("order", 0,
