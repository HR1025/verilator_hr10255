...
void VFileContent::pushText(const string &text) {
  ...
  while (true) {
    ...
    if (line_end != string::npos) {
      ...
      UINFO(9, "PushStream[ct" << m_id << "+" << (m_lines.size() - 1)
                               << "]: " << oneline);
...
string VFileContent::getLine(int lineno) const {
  // Return error text rather than asserting so the user isn't left without a
  // message cppcheck-suppress negativeContainerIndex
  if (VL_UNCOVERABLE(lineno < 0 || lineno >= (int)m_lines.size())) {
    if (debug() || v3Global.opt.debugCheck()) {
      return ("%Error-internal-contents-bad-ct" + cvtToStr(m_id) + "-ln" +
              cvtToStr(lineno));
    } else {
      return "";
    }
...
FileLine::FileLine(FileLine::EmptySecret) {
  ...
  m_warnOn = 0;
  for (int codei = V3ErrorCode::EC_MIN; codei < V3ErrorCode::_ENUM_MAX;
       codei++) {
...
string FileLine::xmlDetailedLocation() const {
  return "loc=\"" + cvtToStr(filenameLetters()) + "," +
         cvtToStr(firstLineno()) + "," + cvtToStr(firstColumn()) + "," +
         cvtToStr(lastLineno()) + "," + cvtToStr(lastColumn()) + "\"";
}
...
void FileLine::lineDirective(const char *textp, int &enterExitRef) {
  ...
  if (fail && v3Global.opt.pedantic()) {
    v3error("`line was not properly formed with '`line number \"filename\" "
            "level'\n");
  }
...
void FileLine::forwardToken(const char *textp, size_t size, bool trackLines) {
  for (const char *sp = textp; size && *sp; ++sp, --size) {
    ...
    } else { // Tabs are considered one column; hence column means number of
             // chars
      ++m_lastColumn;
    }
  }
...
string FileLine::profileFuncname() const {
  ...
  string::size_type pos;
  while ((pos = name.find_first_not_of("abcdefghijlkmnopqrstuvwxyzABCDEFGHIJLKM"
...
string FileLine::ascii() const {
  // For most errors especially in the parser the lastLineno is more accurate
  // than firstLineno
...
bool FileLine::warnIsOff(V3ErrorCode code) const {
  if (!m_warnOn.test(code))
    return true;
  if (!defaultFileLine().m_warnOn.test(code))
    return true; // Global overrides local
  // UNOPTFLAT implies UNOPT
  if (code == V3ErrorCode::UNOPT && !m_warnOn.test(V3ErrorCode::UNOPTFLAT))
    return true;
  if ((code.lintError() || code.styleError()) &&
      !m_warnOn.test(V3ErrorCode::I_LINT)) {
    return true;
  }
  return false;
}
...
void FileLine::v3errorEnd(std::ostringstream &sstr, const string &locationStr) {
  ...
  if (!m_waive)
    V3Waiver::addEntry(V3Error::errorCode(), filename(), sstr.str());
...
string FileLine::warnContext(bool secondary) const {
  ...
  if (firstLineno() == lastLineno() && firstColumn()) {
    ...
    if (!sourceLine.empty() && sourceLine.length() < SHOW_SOURCE_MAX_LENGTH &&
      ...
      // Can't use UASSERT_OBJ used in warnings already inside the error end
      // handler
      if (lastColumn() > firstColumn()) {
        // Note lastColumn() can be <= firstColumn() in some weird preproc
        // expansions
  ...
  if (!secondary) { // Avoid printing long paths on informational part of error
    for (FileLine *parentFl = parent(); parentFl;
         parentFl = parentFl->parent()) {
      if (parentFl->filenameIsGlobal())
        break;
