...
private:
  ...
  virtual void visit(AstNodeVarRef *nodep) override {
    ...
    if (nodep->varp()) {
      if (nodep->access().isWriteOrRW() && !m_ftaskp &&
  ...
  virtual void visit(AstNodeFTaskRef *nodep) override {
    ...
    if (!taskp)
      return;
    for (AstNode *stmtp = taskp->stmtsp(); stmtp && pinp;
         stmtp = stmtp->nextp()) {
...
public:
  // CONSTRUCTORS
  LinkLValueVisitor(AstNode *nodep, VAccess start) : m_setRefLvalue{start} {
    iterate(nodep);
  }
...
void V3LinkLValue::linkLValue(AstNetlist *nodep) {
  ...
  {
    LinkLValueVisitor visitor{nodep, VAccess::NOCHANGE};
  } // Destruct before checking
  V3Global::dumpCheckGlobalTree("linklvalue", 0,
