...
private:
  ...
  std::unordered_map<AstNodeModule *, AstScope *>
      m_packageScopes;     // Scopes for each package
  ...
  void cleanupVarRefs() {
    for (const auto &itr : m_varRefScopes) {
      ...
      if (nodep->classOrPackagep()) {
        ...
        UASSERT_OBJ(it2 != m_packageScopes.end(), nodep,
                    "Can't locate package scope");
        scopep = it2->second;
      }
  ...
  virtual void visit(AstNodeModule *nodep) override {
    ...
    for (AstNode *cellnextp = nodep->stmtsp(); cellnextp;
         cellnextp = cellnextp->nextp()) {
    ...
    // If this is the first usage of the block ever, we can simply move the
    // reference
  ...
  virtual void visit(AstClass *nodep) override {
    ...
    VL_RESTORER(m_modp);
    {
      ...
      AstNode *abovep = (m_aboveCellp ? static_cast<AstNode *>(m_aboveCellp)
                                      : static_cast<AstNode *>(nodep));
      m_scopep = new AstScope(abovep->fileline(), m_modp, scopename,
                              m_aboveScopep, m_aboveCellp);
      m_packageScopes.emplace(nodep, m_scopep);
  ...
  virtual void visit(AstVar *nodep) override {
    ...
    if (!nodep->user1p()) {
      ...
      if (m_aboveCellp && !m_aboveCellp->isTrace())
        varscp->trace(false);
      nodep->user1p(varscp);
...
void V3Scope::scopeAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("scope", 0,
