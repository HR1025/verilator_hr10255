...
public:
  ...
  // METHODS
  static uint32_t scoreFn(const ScoreboardTestElem *elp) {
    return elp->m_score;
  }
...
void V3ScoreboardBase::selfTest() {
  V3Scoreboard<ScoreboardTestElem, uint32_t> sb(ScoreboardTestElem::scoreFn,
                                                true);
  ...
  UASSERT(sb.needsRescore(),
          "SelfTest: Newly filled sb should need a rescore.");
  ...
  UASSERT(!sb.needsRescore(),
          "SelfTest: Newly rescored sb should not need rescore");
  UASSERT(!sb.needsRescore(&e1),
  ...
  UASSERT(sb.contains(&e2),
          "SelfTest: e2 should be there, despite needing rescore");

  // Now e3 should be our best-scoring element, even though
  // e2 has a better score, since e2 is pending rescore.
  UASSERT(&e3 == sb.bestp(),
          "SelfTest: Expect e3 as best element with known score.");
  sb.rescore();
  UASSERT(&e2 == sb.bestp(),
          "SelfTest: Expect e2 as best element again after Rescore");
}
