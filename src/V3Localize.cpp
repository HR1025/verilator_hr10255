...
private:
  ...
  //  AstVarScope::user2()    ->  Bool indicating VarScope was fully assigned in
  //  the current
  //                              function.
  //  AstVarScope::user3p()   ->  Set of CFuncs referencing this VarScope. (via
  //  m_accessors) AstCFunc::user4p()      ->  Multimap of 'VarScope -> VarRefs
  //  that reference that VarScope'
  ...
  std::vector<AstVarScope *>
      m_varScopeps; // List of variables to consider for localization
  ...
  bool isOptimizable(AstVarScope *nodep) {
    return !nodep->user1() || // Not marked as not optimizable, or ...
           (nodep->varp()->varType() == AstVarType::BLOCKTEMP &&
            m_accessors(nodep).size() ==
                1); // .. a block temp used in a single CFunc
  }

  void moveVarScopes() {
    for (AstVarScope *const nodep : m_varScopeps) {
      if (!isOptimizable(nodep))
        continue; // Not optimizable

      const std::unordered_set<AstCFunc *> &funcps = m_accessors(nodep);
      if (funcps.empty())
        continue; // No referencing functions at all

      UINFO(4, "Localizing " << nodep << endl);
      ++m_statLocVars;

      // Yank the VarScope from it's parent and schedule them for deletion.
      // Leave the Var for now, as not all VarScopes referencing this Var might
      // be localized.
      ...
      for (AstCFunc *const funcp : funcps) {
        ...
        AstVar *const newVarp = new AstVar(
            oldVarp->fileline(), oldVarp->varType(), newName, oldVarp);
  ...
  virtual void visit(AstNodeAssign *nodep) override {
    ...
    iterate(nodep->rhsp());
    // For now we only consider an assignment that is directly under the
    // function, (in particular: not under an AstIf, or other kind of branch).
    // This could be improved with proper data flow analysis.
    if (m_nodeDepth == 0) {
      // Check if simple "VARREF = ..." assignment, i.e.: this assignment sets
      // the whole variable (and in particular, it is not assigned only in
      // part).
  ...
  virtual void visit(AstVarScope *nodep) override {
    if (!nodep->varp()
             ->isPrimaryIO() // Not an IO the user wants to interact with
        && !nodep->varp()
                ->isSigPublic() // Not something the user wants to interact with
        && !nodep->varp()
                ->isFuncLocal() // Not already a function local (e.g.: argument)
  ...
  virtual void visit(AstVarRef *nodep) override {
    ...
    // Remember this function accesses this VarScope (we always need this as we
    // might optimize this VarScope into a local, even if it's not assigned. See
    // 'isOptimizable')
    m_accessors(varScopep).emplace(m_cfuncp);
    // Remember the reference so we can fix it up later (we always need this as
    // well)
    m_references(m_cfuncp).emplace(varScopep, nodep);

    // Check if already marked as not optimizable
    if (!varScopep->user1()) {
      // Note: we only check read variables, as it's ok to localize (and in fact
      // discard) any variables that are only written but never read.
      if (nodep->access().isReadOrRW() && !varScopep->user2()) {
        // Variable is read, but is not known to have been assigned in this
        // function. Mark as not optimizable.
...
void V3Localize::localizeAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("localize", 0,
