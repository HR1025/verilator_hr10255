...
private:
  ...
  virtual void visit(AstNodeFTask *nodep) override {
    ...
    VL_RESTORER(m_unnamedScope);
    {
      ...
      iterateChildren(nodep);
      if (m_liftedp) {
        // Place lifted nodes at beginning of stmtsp, so Var nodes appear before
        // referenced
  ...
  virtual void visit(AstBegin *nodep) override {
    // Begin blocks were only useful in variable creation, change names and
    // delete
    UINFO(8, "  " << nodep << endl);
    VL_RESTORER(m_namedScope);
    VL_RESTORER(m_unnamedScope);
    {
        ...
        while ((pos = dottedname.find("__DOT__")) != string::npos) {
          ...
          if (!m_ftaskp) {
            AstCellInline *inlinep =
                new AstCellInline(nodep->fileline(), m_unnamedScope,
                                  "__BEGIN__", m_modp->timeunit());
      ...
      UASSERT_OBJ(!nodep->genforp(), nodep,
                  "GENFORs should have been expanded earlier");
  ...
  virtual void visit(AstScopeName *nodep) override {
    ...
    // contain the name() Similar code in V3Inline
    if (nodep->user1SetOnce())
      return; // Don't double-add text's
    if (m_namedScope != "") {
      ...
      nodep->scopeAttrp(
          new AstText(nodep->fileline(), string("__DOT__") + m_namedScope));
      if (afterp)
  ...
  // VISITORS - LINT CHECK
  virtual void
  visit(AstIf *nodep) override { // not AstNodeIf; other types not covered
    ...
    } else if (nodep->uniquePragma() || nodep->unique0Pragma() ||
               nodep->priorityPragma()) {
      m_ifDepth = -1;
    } else if (++m_ifDepth > v3Global.opt.ifDepth()) {
      nodep->v3warn(
          IFDEPTH,
          "Deep 'if' statement; suggest unique/priority to avoid slow logic");
      nodep->fileline()->modifyWarnOff(V3ErrorCode::IFDEPTH,
                                       true); // Warn only once
      m_ifDepth = -1;
    }
    iterateChildren(nodep);
...
public:
  // CONSTRUCTORS
  BeginVisitor(AstNetlist *nodep, BeginState *statep) : m_statep{statep} {
    iterate(nodep);
  }
...
private:
  ...
  virtual void visit(AstIfaceRefDType *nodep) override {
    ...
    if (nodep->cellp())
      nodep->cellName(nodep->cellp()->name());
...
void V3Begin::debeginAll(AstNetlist *nodep) {
  ...
  V3Global::dumpCheckGlobalTree("begin", 0,
