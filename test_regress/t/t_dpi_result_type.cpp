...
void check_exports() {
  ...
  if (e_byte() != 10 + n)
    stop();
  if (e_byte_unsigned() != 20 + n)
    stop();
  if (e_shortint() != 30 + n)
    stop();
  if (e_shortint_unsigned() != 40 + n)
    stop();
  if (e_int() != 50 + n)
    stop();
  if (e_int_unsigned() != 60 + n)
    stop();
  if (e_longint() != 70 + n)
    stop();
  if (e_longint_unsigned() != 80 + n)
    stop();
...
#endif
#ifndef NO_SHORTREAL
  if (e_shortreal() != 1.0f * n + 0.25f)
    stop();
#endif
  if (e_chandle())
    stop();
  if ((n % 2) == 0) {
    if (strcmp(e_string(), "Hello") != 0)
      stop();
  } else {
    if (strcmp(e_string(), "World") != 0)
      stop();
  }
  if (e_bit() != (n % 2))
    stop();
  ...
  if (e_byte_t() != 10 + 2 * n)
    stop();
  if (e_byte_unsigned_t() != 20 + 2 * n)
    stop();
  if (e_shortint_t() != 30 + 2 * n)
    stop();
  if (e_shortint_unsigned_t() != 40 + 2 * n)
    stop();
  if (e_int_t() != 50 + 2 * n)
    stop();
  if (e_int_unsigned_t() != 60 + 2 * n)
    stop();
  if (e_longint_t() != 70 + 2 * n)
    stop();
  if (e_longint_unsigned_t() != 80 + 2 * n)
    stop();
#ifndef NO_REAL_EXPORT
  if (e_real_t() != 1.0 * (2 * n) + 0.5)
    stop();
#endif
#ifndef NO_SHORTREAL
  if (e_shortreal_t() != 1.0f * (2 * n) + 0.25f)
    stop();
#endif
  if (e_chandle_t())
    stop();
  if ((n % 2) == 0) {
    if (strcmp(e_string_t(), "Hello") != 0)
      stop();
  } else {
    if (strcmp(e_string_t(), "World") != 0)
      stop();
  }
  if (e_bit_t() != (n % 2))
    stop();
...
#ifndef NO_ARRAY
  // 2-state packed arrays of width <= 32
  if (e_array_2_state_1() != (n % 2))
    stop();
  if (e_array_2_state_32() != 0xffffffff >> n)
    stop();
#endif

#ifndef NO_STRUCT_OR_UNION
  // 2-state packed structures of width <= 32
  if (e_struct_2_state_1() != (n % 2))
    stop();
  if (e_struct_2_state_32() != 0xffffffff >> n)
    stop();

  // 2-state packed unions of width <= 32
  if (e_union_2_state_1() != (n % 2))
    stop();
  if (e_union_2_state_32() != 0xffffffff >> n)
    stop();
#endif
