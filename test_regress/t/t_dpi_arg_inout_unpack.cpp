...
template <typename T> bool compare(const T &act, const T &exp) {
  if (exp == act) {
    if (VERBOSE_MESSAGE)
      std::cout << "OK Exp:" << exp << " actual:" << act << std::endl;
    return true;
  } else {
...
bool compare_scalar(const svScalar v0, sv_longint_unsigned_t val) {
  ...
  if (act_bit != exp_bit) {
    std::cout << "Mismatch at bit:" << 0 << " exp:" << exp_bit
              << " act:" << act_bit;
    return false;
  }
  if (VERBOSE_MESSAGE)
    std::cout << "OK " << val << " as expected " << std::endl;
  return true;
}
...
bool compare(const svLogicVecVal *v0, sv_longint_unsigned_t val, int bitwidth) {
  for (int i = 0; i < bitwidth; ++i) {
    ...
    if (act_bit != exp_bit) {
      std::cout << "Mismatch at bit:" << i << " exp:" << exp_bit
                << " act:" << act_bit;
      return false;
    }
  }
  if (VERBOSE_MESSAGE) {
    std::cout << "OK " << val << " as expected (width:" << bitwidth << ")"
              << std::endl;
  }
  return true;
}
...
template <typename T> bool update_2d(T *v) {
  if (!compare<T>(v[0 * 2 + 1], 45))
    return false;
  if (!compare<T>(v[1 * 2 + 1], 46))
    return false;
  if (!compare<T>(v[2 * 2 + 1], 47))
    return false;
  ++v[0 * 2 + 1];
  ++v[1 * 2 + 1];
  ++v[2 * 2 + 1];
...
template <typename T> bool update_3d(T *v) {
  if (!compare<T>(v[(0 * 3 + 0) * 2 + 0], 48))
    return false;
  if (!compare<T>(v[(1 * 3 + 0) * 2 + 0], 49))
    return false;
  if (!compare<T>(v[(2 * 3 + 0) * 2 + 0], 50))
    return false;
  if (!compare<T>(v[(3 * 3 + 0) * 2 + 0], 51))
    return false;
  ++v[(0 * 3 + 0) * 2 + 0];
  ++v[(1 * 3 + 0) * 2 + 0];
  ++v[(2 * 3 + 0) * 2 + 0];
...
bool update_0d_scalar(svScalar *v) {
  if (!compare_scalar(v[0], sv_0))
    return false;
  v[0] = sv_1;
  return true;
}
bool update_1d_scalar(svScalar *v) {
  if (!compare_scalar(v[0], sv_1))
    return false;
  v[0] = sv_0;
  if (!compare_scalar(v[1], sv_0))
    return false;
  v[1] = sv_1;
  return true;
}
bool update_2d_scalar(svScalar *v) {
  if (!compare_scalar(v[(0 * 2) + 1], sv_1))
    return false;
  v[(0 * 2) + 1] = sv_0;
  if (!compare_scalar(v[(1 * 2) + 1], sv_0))
    return false;
  v[(1 * 2) + 1] = sv_1;
  if (!compare_scalar(v[(2 * 2) + 1], sv_1))
    return false;
  v[(2 * 2) + 1] = sv_0;
  return true;
}
bool update_3d_scalar(svScalar *v) {
  if (!compare_scalar(v[((0 * 3) + 0) * 2 + 0], sv_0))
    return false;
  v[(0 * 3 + 0) * 2 + 0] = sv_1;
  if (!compare_scalar(v[((1 * 3) + 0) * 2 + 0], sv_1))
    return false;
  v[(1 * 3 + 0) * 2 + 0] = sv_0;
  if (!compare_scalar(v[((2 * 3) + 0) * 2 + 0], sv_0))
    return false;
  v[(2 * 3 + 0) * 2 + 0] = sv_1;
  if (!compare_scalar(v[((3 * 3) + 0) * 2 + 0], sv_1))
    return false;
  v[(3 * 3 + 0) * 2 + 0] = sv_0;
  return true;
}
...
template <typename T> bool update_1d(T *v, int bitwidth) {
  ...
  if (!compare(v + unit * 0, 43, bitwidth))
    return false;
  if (!compare(v + unit * 1, 44, bitwidth))
    return false;
...
template <typename T> bool update_2d(T *v, int bitwidth) {
  const int unit = (bitwidth + 31) / 32;
  if (!compare(v + unit * (0 * 2 + 1), 45, bitwidth))
    return false;
  if (!compare(v + unit * (1 * 2 + 1), 46, bitwidth))
    return false;
  if (!compare(v + unit * (2 * 2 + 1), 47, bitwidth))
    return false;
...
template <typename T> bool update_3d(T *v, int bitwidth) {
  const int unit = (bitwidth + 31) / 32;
  if (!compare(v + unit * ((0 * 3 + 0) * 2 + 0), 48, bitwidth))
    return false;
  if (!compare(v + unit * ((1 * 3 + 0) * 2 + 0), 49, bitwidth))
    return false;
  if (!compare(v + unit * ((2 * 3 + 0) * 2 + 0), 50, bitwidth))
    return false;
  if (!compare(v + unit * ((3 * 3 + 0) * 2 + 0), 51, bitwidth))
    return false;
...
template <typename T> bool check_2d(const T (&v)[3][2]) {
  return compare<T>(v[0][1], 46) && compare<T>(v[1][1], 47) &&
         compare<T>(v[2][1], 48);
}
template <typename T> bool check_3d(const T (&v)[4][3][2]) {
  return compare<T>(v[0][0][0], 49) && compare<T>(v[1][0][0], 50) &&
         compare<T>(v[2][0][0], 51) && compare<T>(v[3][0][0], 52);
}
...
bool check_3d(const T (&v)[4][3][2][N], unsigned int bitwidth) {
  return compare(v[0][0][0], 49, bitwidth) &&
         compare(v[1][0][0], 50, bitwidth) &&
...
void i_string_1d(const char **v) {
  ...
  if (!compare<std::string>(v[0], "43"))
    stop();
  if (!compare<std::string>(v[1], "44"))
    stop();
  v[0] = s0;
  v[1] = s1;
}
void i_string_2d(const char **v) {
  ...
  if (!compare<std::string>(v[2 * 0 + 1], "45"))
    stop();
  if (!compare<std::string>(v[2 * 1 + 1], "46"))
    stop();
  if (!compare<std::string>(v[2 * 2 + 1], "47"))
    stop();
  v[2 * 0 + 1] = s0;
  v[2 * 1 + 1] = s1;
  v[2 * 2 + 1] = s2;
...
void i_string_3d(const char **v) {
  ...
  if (!compare<std::string>(v[(0 * 3 + 0) * 2 + 0], "48"))
    stop();
  if (!compare<std::string>(v[(1 * 3 + 0) * 2 + 0], "49"))
    stop();
  if (!compare<std::string>(v[(2 * 3 + 0) * 2 + 0], "50"))
    stop();
  if (!compare<std::string>(v[(3 * 3 + 0) * 2 + 0], "51"))
    stop();
  v[(0 * 3 + 0) * 2 + 0] = s0;
  v[(1 * 3 + 0) * 2 + 0] = s1;
  v[(2 * 3 + 0) * 2 + 0] = s2;
...
void i_unpack_struct_0d(unpack_struct_t *v) {
  if (!compare(v->val, 42, 121))
    stop();
  set_uint(v->val, 43, 121);
}
void i_unpack_struct_1d(unpack_struct_t *v) {
  if (!compare(v[0].val, 43, 121))
    stop();
  if (!compare(v[1].val, 44, 121))
    stop();
  set_uint(v[0].val, 44, 121);
  set_uint(v[1].val, 45, 121);
}
void i_unpack_struct_2d(unpack_struct_t *v) {
  if (!compare(v[0 * 2 + 1].val, 45, 121))
    stop();
  if (!compare(v[1 * 2 + 1].val, 46, 121))
    stop();
  if (!compare(v[2 * 2 + 1].val, 47, 121))
    stop();
...
void i_unpack_struct_3d(unpack_struct_t *v) {
  if (!compare(v[(0 * 3 + 0) * 2 + 0].val, 48, 121))
    stop();
  if (!compare(v[(1 * 3 + 0) * 2 + 0].val, 49, 121))
    stop();
  if (!compare(v[(2 * 3 + 0) * 2 + 0].val, 50, 121))
    stop();
  if (!compare(v[(3 * 3 + 0) * 2 + 0].val, 51, 121))
    stop();
...
void check_exports() {
  {
    ...
    if (!check_0d(byte_array[3][2][1]))
      stop();
    e_byte_1d(&byte_array[2][1][0]);
    if (!check_1d(byte_array[2][1]))
      stop();
    e_byte_2d(&byte_array[1][0][0]);
    if (!check_2d(byte_array[1]))
      stop();
    e_byte_3d(&byte_array[0][0][0]);
    if (!check_3d(byte_array))
      stop();
  }
  {
    ...
    if (!check_0d(byte_unsigned_array[3][2][1]))
      stop();
    e_byte_unsigned_1d(&byte_unsigned_array[2][1][0]);
    if (!check_1d(byte_unsigned_array[2][1]))
      stop();
    e_byte_unsigned_2d(&byte_unsigned_array[1][0][0]);
    if (!check_2d(byte_unsigned_array[1]))
      stop();
    e_byte_unsigned_3d(&byte_unsigned_array[0][0][0]);
    if (!check_3d(byte_unsigned_array))
      stop();
  }
  {
    ...
    if (!check_0d(shortint_array[3][2][1]))
      stop();
    e_shortint_1d(&shortint_array[2][1][0]);
    if (!check_1d(shortint_array[2][1]))
      stop();
    e_shortint_2d(&shortint_array[1][0][0]);
    if (!check_2d(shortint_array[1]))
      stop();
    e_shortint_3d(&shortint_array[0][0][0]);
    if (!check_3d(shortint_array))
      stop();
  }
  {
...

  {
    ...
    if (!check_0d(int_array[3][2][1]))
      stop();
    e_int_1d(&int_array[2][1][0]);
    if (!check_1d(int_array[2][1]))
      stop();
    e_int_2d(&int_array[1][0][0]);
    if (!check_2d(int_array[1]))
      stop();
    e_int_3d(&int_array[0][0][0]);
    if (!check_3d(int_array))
      stop();
  }
  {
    ...
    if (!check_0d(int_unsigned_array[3][2][1]))
      stop();
    e_int_unsigned_1d(&int_unsigned_array[2][1][0]);
    if (!check_1d(int_unsigned_array[2][1]))
      stop();
    e_int_unsigned_2d(&int_unsigned_array[1][0][0]);
    if (!check_2d(int_unsigned_array[1]))
      stop();
    e_int_unsigned_3d(&int_unsigned_array[0][0][0]);
    if (!check_3d(int_unsigned_array))
      stop();
  }

  {
    ...
    if (!check_0d(longint_array[3][2][1]))
      stop();
    e_longint_1d(&longint_array[2][1][0]);
    if (!check_1d(longint_array[2][1]))
      stop();
    e_longint_2d(&longint_array[1][0][0]);
    if (!check_2d(longint_array[1]))
      stop();
    e_longint_3d(&longint_array[0][0][0]);
    if (!check_3d(longint_array))
      stop();
  }
  {
...
#ifndef NO_TIME
  {
    ...
    if (!check_0d(time_array[3][2][1], 64))
      stop();
    e_time_1d(time_array[2][1][0]);
    if (!check_1d(time_array[2][1], 64))
      stop();
    e_time_2d(time_array[1][0][0]);
    if (!check_2d(time_array[1], 64))
      stop();
    e_time_3d(time_array[0][0][0]);
    if (!check_3d(time_array, 64))
      stop();
  }
#endif
...
#ifndef NO_INTEGER
  {
    ...
    if (!check_0d(integer_array[3][2][1], 32))
      stop();
    e_integer_1d(integer_array[2][1][0]);
    if (!check_1d(integer_array[2][1], 32))
      stop();
    e_integer_2d(integer_array[1][0][0]);
    if (!check_2d(integer_array[1], 32))
      stop();
    e_integer_3d(integer_array[0][0][0]);
    if (!check_3d(integer_array, 32))
      stop();
  }
#endif

  {
    ...
    if (!check_0d(real_array[3][2][1]))
      stop();
    e_real_1d(&real_array[2][1][0]);
    if (!check_1d(real_array[2][1]))
      stop();
    e_real_2d(&real_array[1][0][0]);
    if (!check_2d(real_array[1]))
      stop();
    e_real_3d(&real_array[0][0][0]);
    if (!check_3d(real_array))
      stop();
  }
#ifndef NO_SHORTREAL
  {
    ...
    if (!check_0d(shortreal_array[3][2][1]))
      stop();
    e_shortreal_1d(&shortreal_array[2][1][0]);
    if (!check_1d(shortreal_array[2][1]))
      stop();
    e_shortreal_2d(&shortreal_array[1][0][0]);
    if (!check_2d(shortreal_array[1]))
      stop();
    e_shortreal_3d(&shortreal_array[0][0][0]);
    if (!check_3d(shortreal_array))
      stop();
  }
#endif

  {
    ...
    if (chandle_array[1][0][1])
      stop();
    if (chandle_array[1][1][1])
      stop();
    if (chandle_array[1][2][1])
      stop();
    ...
    if (chandle_array[0][0][0])
      stop();
    if (chandle_array[1][0][0])
      stop();
    if (chandle_array[2][0][0])
      stop();
    if (chandle_array[3][0][0])
      stop();
  }

  {
    ...
    if (!compare<std::string>(string_array[3][2][1], "43"))
      stop();

    string_array[2][1][0] = "43";
    string_array[2][1][1] = "44";
    e_string_1d(&string_array[2][1][0]);
    if (!compare<std::string>(string_array[2][1][0], "44"))
      stop();
    if (!compare<std::string>(string_array[2][1][1], "45"))
      stop();
    ...
    if (!compare<std::string>(string_array[1][0][1], "46"))
      stop();
    if (!compare<std::string>(string_array[1][1][1], "47"))
      stop();
    if (!compare<std::string>(string_array[1][2][1], "48"))
      stop();
    ...
    if (!compare<std::string>(string_array[0][0][0], "49"))
      stop();
    if (!compare<std::string>(string_array[1][0][0], "50"))
      stop();
    if (!compare<std::string>(string_array[2][0][0], "51"))
      stop();
    if (!compare<std::string>(string_array[3][0][0], "52"))
      stop();
  }

  {
    ...
    if (!check_0d(bit7_array[3][2][1], 7))
      stop();
    e_bit7_1d(bit7_array[2][1][0]);
    if (!check_1d(bit7_array[2][1], 7))
      stop();
    e_bit7_2d(bit7_array[1][0][0]);
    if (!check_2d(bit7_array[1], 7))
      stop();
    e_bit7_3d(bit7_array[0][0][0]);
    if (!check_3d(bit7_array, 7))
      stop();
  }
  {
    ...
    if (!check_0d(bit121_array[3][2][1], 121))
      stop();
    e_bit121_1d(bit121_array[2][1][0]);
    if (!check_1d(bit121_array[2][1], 121))
      stop();
    e_bit121_2d(bit121_array[1][0][0]);
    if (!check_2d(bit121_array[1], 121))
      stop();
    e_bit121_3d(bit121_array[0][0][0]);
    if (!check_3d(bit121_array, 121))
      stop();
  }

  {
    ...
    if (!check_0d(logic7_array[3][2][1], 7))
      stop();
    e_logic7_1d(logic7_array[2][1][0]);
    if (!check_1d(logic7_array[2][1], 7))
      stop();
    e_logic7_2d(logic7_array[1][0][0]);
    if (!check_2d(logic7_array[1], 7))
      stop();
    e_logic7_3d(logic7_array[0][0][0]);
    if (!check_3d(logic7_array, 7))
      stop();
  }
  {
    ...
    if (!check_0d(logic121_array[3][2][1], 121))
      stop();
    e_logic121_1d(logic121_array[2][1][0]);
    if (!check_1d(logic121_array[2][1], 121))
      stop();
    e_logic121_2d(logic121_array[1][0][0]);
    if (!check_2d(logic121_array[1], 121))
      stop();
    e_logic121_3d(logic121_array[0][0][0]);
    if (!check_3d(logic121_array, 121))
      stop();
  }

  {
    ...
    if (!check_0d(pack_struct_array[3][2][1], 7))
      stop();
    e_pack_struct_1d(pack_struct_array[2][1][0]);
    if (!check_1d(pack_struct_array[2][1], 7))
      stop();
    e_pack_struct_2d(pack_struct_array[1][0][0]);
    if (!check_2d(pack_struct_array[1], 7))
      stop();
    e_pack_struct_3d(pack_struct_array[0][0][0]);
    if (!check_3d(pack_struct_array, 7))
      stop();
  }
...
#ifndef NO_UNPACK_STRUCT
  {
    ...
    if (!compare(unpack_struct_array[3][2][1].val, 43, 121))
      stop();
    ...
    if (!compare(unpack_struct_array[2][1][0].val, 44, 121))
      stop();
    if (!compare(unpack_struct_array[2][1][1].val, 45, 121))
      stop();
    ...
    if (!compare(unpack_struct_array[1][0][1].val, 46, 121))
      stop();
    if (!compare(unpack_struct_array[1][1][1].val, 47, 121))
      stop();
    if (!compare(unpack_struct_array[1][2][1].val, 48, 121))
      stop();
    ...
    if (!compare(unpack_struct_array[0][0][0].val, 49, 121))
      stop();
    if (!compare(unpack_struct_array[1][0][0].val, 50, 121))
      stop();
    if (!compare(unpack_struct_array[2][0][0].val, 51, 121))
      stop();
    if (!compare(unpack_struct_array[3][0][0].val, 52, 121))
      stop();
  }
#endif
}
