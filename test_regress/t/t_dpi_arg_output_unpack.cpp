...
template <typename T> bool check_2d(const T (&v)[3][2]) {
  return compare<T>(v[0][1], 45) && compare<T>(v[1][1], 46) &&
         compare<T>(v[2][1], 47);
}
template <typename T> bool check_3d(const T (&v)[4][3][2]) {
  return compare<T>(v[0][0][0], 48) && compare<T>(v[1][0][0], 49) &&
         compare<T>(v[2][0][0], 50) && compare<T>(v[3][0][0], 51);
}
...
bool check_3d(const T (&v)[4][3][2][N], int bitwidth) {
  return compare(v[0][0][0], 48, bitwidth) &&
         compare(v[1][0][0], 49, bitwidth) &&
...
void check_exports() {
...

  {
    ...
    for (int i = 0; i < 4; ++i)
      for (int j = 0; j < 3; ++j)
        for (int k = 0; k < 2; ++k)
          string_array[i][j][k] = NULL;
    e_string_0d(&string_array[3][2][1]);
    if (!compare<std::string>(string_array[3][2][1], "42"))
      stop();
    e_string_1d(&string_array[2][1][0]);
    if (!compare<std::string>(string_array[2][1][0], "43"))
      stop();
    if (!compare<std::string>(string_array[2][1][1], "44"))
      stop();
    e_string_2d(&string_array[1][0][0]);
    if (!compare<std::string>(string_array[1][0][1], "45"))
      stop();
    if (!compare<std::string>(string_array[1][1][1], "46"))
      stop();
    if (!compare<std::string>(string_array[1][2][1], "47"))
      stop();
    e_string_3d(&string_array[0][0][0]);
    if (!compare<std::string>(string_array[0][0][0], "48"))
      stop();
    if (!compare<std::string>(string_array[1][0][0], "49"))
      stop();
    if (!compare<std::string>(string_array[2][0][0], "50"))
      stop();
    if (!compare<std::string>(string_array[3][0][0], "51"))
      stop();
  }
...
#ifndef NO_UNPACK_STRUCT
  {
    ...
    if (!compare(unpack_struct_array[3][2][1].val, 42, 121))
      stop();
    e_unpack_struct_1d(&unpack_struct_array[2][1][0]);
    if (!compare(unpack_struct_array[2][1][0].val, 43, 121))
      stop();
    if (!compare(unpack_struct_array[2][1][1].val, 44, 121))
      stop();
    e_unpack_struct_2d(&unpack_struct_array[1][0][0]);
    if (!compare(unpack_struct_array[1][0][1].val, 45, 121))
      stop();
    if (!compare(unpack_struct_array[1][1][1].val, 46, 121))
      stop();
    if (!compare(unpack_struct_array[1][2][1].val, 47, 121))
      stop();
    e_unpack_struct_3d(&unpack_struct_array[0][0][0]);
    if (!compare(unpack_struct_array[0][0][0].val, 48, 121))
      stop();
    if (!compare(unpack_struct_array[1][0][0].val, 49, 121))
      stop();
    if (!compare(unpack_struct_array[2][0][0].val, 50, 121))
      stop();
    if (!compare(unpack_struct_array[3][0][0].val, 51, 121))
      stop();
  }
#endif
}
