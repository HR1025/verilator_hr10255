...
template <typename T> bool check_3d(const T *v) {
  return compare<T>(v[(0 * 3 + 0) * 2 + 0], 48) &&
         compare<T>(v[(1 * 3 + 0) * 2 + 0], 49) &&
         compare<T>(v[(2 * 3 + 0) * 2 + 0], 50) &&
         compare<T>(v[(3 * 3 + 0) * 2 + 0], 51);
}

bool check_0d(const svLogicVecVal *v, int bitwidth) {
  return compare(v, 42, bitwidth);
}
bool check_1d(const svLogicVecVal *v, int bitwidth) {
  const int unit = (bitwidth + 31) / 32;
  return compare(v + unit * 0, 43, bitwidth) &&
         compare(v + unit * 1, 44, bitwidth);
}
...
bool check_0d(const svBitVecVal *v, int bitwidth) {
  return compare(v, 42, bitwidth);
}
...
bool check_2d(const char **v) {
  return compare<std::string>(v[0 * 2 + 1], "45") &&
         compare<std::string>(v[1 * 2 + 1], "46") &&
...
void i_bit1_0d(CONSTARG svBit v) {
  if (!compare<svScalar>(v, sv_0))
    stop();
}
void i_bit1_1d(CONSTARG svBit *v) {
  if (!compare<svScalar>(v[0], sv_1))
    stop();
  if (!compare<svScalar>(v[1], sv_0))
    stop();
}
void i_bit1_2d(CONSTARG svBit *v) {
  if (!compare<svScalar>(v[0 * 2 + 1], sv_1))
    stop();
  if (!compare<svScalar>(v[1 * 2 + 1], sv_0))
    stop();
  if (!compare<svScalar>(v[2 * 2 + 1], sv_1))
    stop();
}
void i_bit1_3d(CONSTARG svBit *v) {
  if (!compare<svScalar>(v[(0 * 3 + 0) * 2 + 0], sv_0))
    stop();
  if (!compare<svScalar>(v[(1 * 3 + 0) * 2 + 0], sv_1))
    stop();
  if (!compare<svScalar>(v[(2 * 3 + 0) * 2 + 0], sv_0))
    stop();
  if (!compare<svScalar>(v[(3 * 3 + 0) * 2 + 0], sv_1))
    stop();
}
