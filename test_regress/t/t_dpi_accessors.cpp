...
// Convenience function to log the value of a register in hex. Only in verbose
// mode.
static void logRegHex(int clk, const char *desc, int bitWidth, int val,
                      const char *note) {
#ifdef TEST_VERBOSE
  cout << "clk = " << clk << ", " << desc << " = " << bitWidth << "\'h" << hex
       << setw((bitWidth - 1) / 4 + 1) << setfill('0') << val << setfill(' ')
       << setw(0) << dec << note << endl;
#endif
}
...
int main() {
  ...
  if (!scope)
    vl_fatal(__FILE__, __LINE__, "dut", "No svGetScopeFromName result");
  svSetScope(scope);
  ...
  for (int i = 0; !Verilated::gotFinish() && (i < 4); i++) {
    ...
    int mem32_new =
        (mem32 & 0x38) | ((mem32_slice & 0x18) << 3) | (mem32_slice & 0x7);
  ...
  for (int i = 0; !Verilated::gotFinish() && (i < 4); i++) {
    ...
    checkResult((b_new == b_after) && (mem32_new == mem32_after) &&
                    (e_new == e_after),
