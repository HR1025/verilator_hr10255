...
void mon_do(MyMon *monp) {
  if (!monp->sigsp[0])
    vl_fatal(__FILE__, __LINE__, "", "never registered");
  if (!monp->sigsp[1])
    vl_fatal(__FILE__, __LINE__, "", "never registered");
  *monp->sigsp[1] = (*(monp->sigsp[0])) + 1;
  ...
  VL_PRINTF("-     mon_do(%08x(&%p) -> %08x(&%p));\n", *(monp->sigsp[0]),
            monp->sigsp[0], *(monp->sigsp[1]), monp->sigsp[1]);
#endif
}
...
void mon_class_name(const char *namep) {
  ...
  if (namep && namep[0] == '.')
    vl_fatal(__FILE__, __LINE__, "",
             (std::string("Unexp class name ") + namep).c_str());
}
...
void mon_scope_name(const char *namep) {
  ...
  if (strcmp(namep, "t.sub"))
    vl_fatal(__FILE__, __LINE__, "",
             (std::string("Unexp scope name ") + namep).c_str());
  if (strcmp(modp, "t.sub"))
    vl_fatal(__FILE__, __LINE__, "",
             (std::string("Unexp dpiscope name ") + modp).c_str());
}
...
#endif
  // Print list of all signals - if we didn't register2 anything we'd pick them
  // off here
  const VerilatedScope *scopep = Verilated::dpiScope();
