...
bool check() {
  ...
  if (verbose)
    printf("Read: OE1=%d OE2=%d A1=0x%x A2=0x%x Y1=0x%x Y2=0x%x Y3=0x%x W=0x%x "
           " Expected: "
           "Y1=Y2=Y3=%d and W=0x%x\n",
           tb->OE1, tb->OE2, tb->A1, tb->A2, tb->Y1, tb->Y2, tb->Y3, tb->W, Y,
           W);
  return pass;
}
...
int main() {
  ...
  if (pass) {
    ...
    vl_fatal(__FILE__, __LINE__, "top",
             "Unexpected results from t_tri_select\n");
  }
