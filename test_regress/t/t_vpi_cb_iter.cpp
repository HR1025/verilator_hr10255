...
static void reregister_value_cb() {
  if (vh_value_cb) {
    if (verbose)
      vpi_printf(const_cast<char *>("- Removing cbValueChange callback\n"));
  ...
  if (verbose)
    vpi_printf(const_cast<char *>("- Registering cbValueChange callback\n"));
...
static void reregister_rw_cb() {
  if (vh_rw_cb) {
    if (verbose)
      vpi_printf(const_cast<char *>("- Removing cbReadWriteSynch callback\n"));
  ...
  if (verbose)
    vpi_printf(const_cast<char *>("- Registering cbReadWriteSynch callback\n"));
  t_cb_data cb_data_testcase;
  bzero(&cb_data_testcase, sizeof(cb_data_testcase));
...
static void register_filler_cb() {
  if (verbose) {
    vpi_printf(
        const_cast<char *>("- Registering filler cbReadWriteSynch callback\n"));
  }
  t_cb_data cb_data_1;
  ...
  if (verbose) {
    vpi_printf(
        const_cast<char *>("- Registering filler cbValueChange callback\n"));
  }
  t_cb_data cb_data_2;
...
int main(int argc, char **argv, char **env) {
  ...
  while (vl_time_stamp64() < sim_time && !Verilated::gotFinish()) {
    main_time += 1;
    if (verbose)
      VL_PRINTF("Sim Time %d got_error %d\n", main_time, errors);
    topp->clk = !topp->clk;
    topp->eval();
