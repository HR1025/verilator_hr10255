...
void check_exports() {
  ...
  x_time[1].aval = 0;
  e_time(x_time);
  if (x_time[0].aval != 990 + n || x_time[1].aval != 0)
    stop();
  ...
  x_real = 1.0 * n + 0.50;
  e_real(&x_real);
  if (x_real != 100.0 + 1.0 * n + 0.5)
    stop();

#ifndef NO_SHORTREAL
  x_shortreal = 1.0f * n + 0.25f;
  e_shortreal(&x_shortreal);
  if (x_shortreal != 200.0f + 1.0f * n + 0.25f)
    stop();
#endif
  ...
  if ((n % 2) == 0) {
    if (x_chandle)
      stop();
    if (strcmp(x_string, "Hello") != 0)
      stop();
  } else {
    if (x_chandle)
      stop();
    if (strcmp(x_string, "World") != 0)
      stop();
  }
  ...
  x_time_t[1].aval = 0;
  e_time_t(x_time_t);
  if (x_time_t[0].aval != 999 + 2 * n || x_time_t[1].aval != 0)
    stop();
  ...
  if (x_integer_t[0].aval != 1101 + 2 * n)
    stop();
  ...
  e_real_t(&x_real_t);
  if (x_real_t != 111.0 + 1.0 * (2 * n) + 0.5)
    stop();

#ifndef NO_SHORTREAL
  x_shortreal_t = 1.0f * (2 * n) + 0.25f;
  e_shortreal_t(&x_shortreal_t);
  if (x_shortreal_t != 222.0f + 1.0f * (2 * n) + 0.25f)
    stop();
#endif
  ...
  if ((n % 2) == 0) {
    if (x_chandle_t != NULL)
      stop();
    if (strcmp(x_string_t, "World") != 0)
      stop();
  } else {
    if (x_chandle_t != NULL)
      stop();
    if (strcmp(x_string_t, "Hello") != 0)
      stop();
  }
  ...
  x_array_2_state_128[2] = x_array_2_state_128[1] = x_array_2_state_128[0] =
      0xffffffff;

  e_array_2_state_1(x_array_2_state_1);
  if ((x_array_2_state_1[0] & 1) != !(n % 2))
    stop();

  e_array_2_state_32(x_array_2_state_32);
  if (x_array_2_state_32[0] != 0xffffffff << n)
    stop();

  e_array_2_state_33(x_array_2_state_33);
  if ((x_array_2_state_33[1] & 1) != 1)
    stop();
  if (x_array_2_state_33[0] != 0xffffffff << n)
    stop();

  e_array_2_state_64(x_array_2_state_64);
  if (x_array_2_state_64[1] != 0xffffffff)
    stop();
  if (x_array_2_state_64[0] != 0xffffffff << n)
    stop();

  e_array_2_state_65(x_array_2_state_65);
  if ((x_array_2_state_65[2] & 1) != 1)
    stop();
  if (x_array_2_state_65[1] != 0xffffffff)
    stop();
  if (x_array_2_state_65[0] != 0xffffffff << n)
    stop();

  e_array_2_state_128(x_array_2_state_128);
  if (x_array_2_state_128[3] != 0xffffffff)
    stop();
  if (x_array_2_state_128[2] != 0xffffffff)
    stop();
  if (x_array_2_state_128[1] != 0xffffffff)
    stop();
  if (x_array_2_state_128[0] != 0xffffffff << n)
    stop();
  ...
  x_struct_2_state_128[2] = x_struct_2_state_128[1] = x_struct_2_state_128[0] =
      0xffffffff;

  e_struct_2_state_1(x_struct_2_state_1);
  if ((x_struct_2_state_1[0] & 1) != !(n % 2))
    stop();

  e_struct_2_state_32(x_struct_2_state_32);
  if (x_struct_2_state_32[0] != 0xffffffff << n)
    stop();

  e_struct_2_state_33(x_struct_2_state_33);
  if ((x_struct_2_state_33[1] & 1) != 1)
    stop();
  if (x_struct_2_state_33[0] != 0xffffffff << n)
    stop();

  e_struct_2_state_64(x_struct_2_state_64);
  if (x_struct_2_state_64[1] != 0xffffffff)
    stop();
  if (x_struct_2_state_64[0] != 0xffffffff << n)
    stop();

  e_struct_2_state_65(x_struct_2_state_65);
  if ((x_struct_2_state_65[2] & 1) != 1)
    stop();
  if (x_struct_2_state_65[1] != 0xffffffff)
    stop();
  if (x_struct_2_state_65[0] != 0xffffffff << n)
    stop();

  e_struct_2_state_128(x_struct_2_state_128);
  if (x_struct_2_state_128[3] != 0xffffffff)
    stop();
  if (x_struct_2_state_128[2] != 0xffffffff)
    stop();
  if (x_struct_2_state_128[1] != 0xffffffff)
    stop();
  if (x_struct_2_state_128[0] != 0xffffffff << n)
    stop();
  ...
  x_union_2_state_128[2] = x_union_2_state_128[1] = x_union_2_state_128[0] =
      0xffffffff;

  e_union_2_state_1(x_union_2_state_1);
  if ((x_union_2_state_1[0] & 1) != !(n % 2))
    stop();

  e_union_2_state_32(x_union_2_state_32);
  if (x_union_2_state_32[0] != 0xffffffff << n)
    stop();

  e_union_2_state_33(x_union_2_state_33);
  if ((x_union_2_state_33[1] & 1) != 1)
    stop();
  if (x_union_2_state_33[0] != 0xffffffff << n)
    stop();

  e_union_2_state_64(x_union_2_state_64);
  if (x_union_2_state_64[1] != 0xffffffff)
    stop();
  if (x_union_2_state_64[0] != 0xffffffff << n)
    stop();

  e_union_2_state_65(x_union_2_state_65);
  if ((x_union_2_state_65[2] & 1) != 1)
    stop();
  if (x_union_2_state_65[1] != 0xffffffff)
    stop();
  if (x_union_2_state_65[0] != 0xffffffff << n)
    stop();

  e_union_2_state_128(x_union_2_state_128);
  if (x_union_2_state_128[3] != 0xffffffff)
    stop();
  if (x_union_2_state_128[2] != 0xffffffff)
    stop();
  if (x_union_2_state_128[1] != 0xffffffff)
    stop();
  if (x_union_2_state_128[0] != 0xffffffff << n)
    stop();
  ...
  if ((x_array_4_state_1[0].aval & 1) != !(n % 2))
    stop();

  e_array_4_state_32(x_array_4_state_32);
  if (x_array_4_state_32[0].aval != 0xffffffff << n)
    stop();

  e_array_4_state_33(x_array_4_state_33);
  if ((x_array_4_state_33[1].aval & 1) != 1)
    stop();
  if (x_array_4_state_33[0].aval != 0xffffffff << n)
    stop();

  e_array_4_state_64(x_array_4_state_64);
  if (x_array_4_state_64[1].aval != 0xffffffff)
    stop();
  if (x_array_4_state_64[0].aval != 0xffffffff << n)
    stop();

  e_array_4_state_65(x_array_4_state_65);
  if ((x_array_4_state_65[2].aval & 1) != 1)
    stop();
  if (x_array_4_state_65[1].aval != 0xffffffff)
    stop();
  if (x_array_4_state_65[0].aval != 0xffffffff << n)
    stop();

  e_array_4_state_128(x_array_4_state_128);
  if (x_array_4_state_128[3].aval != 0xffffffff)
    stop();
  if (x_array_4_state_128[2].aval != 0xffffffff)
    stop();
  if (x_array_4_state_128[1].aval != 0xffffffff)
    stop();
  if (x_array_4_state_128[0].aval != 0xffffffff << n)
    stop();
  ...
  if ((x_struct_4_state_1[0].aval & 1) != !(n % 2))
    stop();

  e_struct_4_state_32(x_struct_4_state_32);
  if (x_struct_4_state_32[0].aval != 0xffffffff << n)
    stop();

  e_struct_4_state_33(x_struct_4_state_33);
  if ((x_struct_4_state_33[1].aval & 1) != 1)
    stop();
  if (x_struct_4_state_33[0].aval != 0xffffffff << n)
    stop();

  e_struct_4_state_64(x_struct_4_state_64);
  if (x_struct_4_state_64[1].aval != 0xffffffff)
    stop();
  if (x_struct_4_state_64[0].aval != 0xffffffff << n)
    stop();

  e_struct_4_state_65(x_struct_4_state_65);
  if ((x_struct_4_state_65[2].aval & 1) != 1)
    stop();
  if (x_struct_4_state_65[1].aval != 0xffffffff)
    stop();
  if (x_struct_4_state_65[0].aval != 0xffffffff << n)
    stop();

  e_struct_4_state_128(x_struct_4_state_128);
  if (x_struct_4_state_128[3].aval != 0xffffffff)
    stop();
  if (x_struct_4_state_128[2].aval != 0xffffffff)
    stop();
  if (x_struct_4_state_128[1].aval != 0xffffffff)
    stop();
  if (x_struct_4_state_128[0].aval != 0xffffffff << n)
    stop();
  ...
  if ((x_union_4_state_1[0].aval & 1) != !(n % 2))
    stop();

  e_union_4_state_32(x_union_4_state_32);
  if (x_union_4_state_32[0].aval != 0xffffffff << n)
    stop();

  e_union_4_state_33(x_union_4_state_33);
  if ((x_union_4_state_33[1].aval & 1) != 1)
    stop();
  if (x_union_4_state_33[0].aval != 0xffffffff << n)
    stop();

  e_union_4_state_64(x_union_4_state_64);
  if (x_union_4_state_64[1].aval != 0xffffffff)
    stop();
  if (x_union_4_state_64[0].aval != 0xffffffff << n)
    stop();

  e_union_4_state_65(x_union_4_state_65);
  if ((x_union_4_state_65[2].aval & 1) != 1)
    stop();
  if (x_union_4_state_65[1].aval != 0xffffffff)
    stop();
  if (x_union_4_state_65[0].aval != 0xffffffff << n)
    stop();

  e_union_4_state_128(x_union_4_state_128);
  if (x_union_4_state_128[3].aval != 0xffffffff)
    stop();
  if (x_union_4_state_128[2].aval != 0xffffffff)
    stop();
  if (x_union_4_state_128[1].aval != 0xffffffff)
    stop();
  if (x_union_4_state_128[0].aval != 0xffffffff << n)
    stop();
